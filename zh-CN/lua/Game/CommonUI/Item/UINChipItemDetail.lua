...
UINChipItemDetail.InitChipDetail = function(self, chipData)
  ...
  for k,atrId in ipairs(chipCfg.attribute_id) do
    ...
    if attibuteCfg == nil then
      ...
      local value = (chipCfg.attribute_initial)[k] + ((chipCfg.level_increase)[k] or 0) * (count - 1)
      if value ~= 0 then
  ...
  if holdCount ~= 0 or not "" then
    ...
    if #chipCfg.skill_list == 0 then
      ...
      ;
      ((self.ui).tex_Skill).text = skillCfg.Name
    ...
    if chipCfg.influence == (ChipEnum.eChipInfluenceType).AllHero then
      ...
      if chipCfg.influence == (ChipEnum.eChipInfluenceType).Career then
        ...
        if careerCfg == nil then
          error((string.format)("chip(%d) influence error", chipCfg.id))
        end
        -- DECOMPILER ERROR at PC49: Confused about usage of register: R4 in 'UnsetPending'

        ;
        (((self.ui).tex_Adapt).text).text = (LanguageUtil.GetLocaleText)(careerCfg.name)
      else
        do
          if chipCfg.influence == (ChipEnum.eChipInfluenceType).Camp then
            ...
            ;
            (((self.ui).tex_Adapt).text).text = (LanguageUtil.GetLocaleText)(campCfg.name)
          else
            do
              if chipCfg.influence == (ChipEnum.eChipInfluenceType).Row then
                if chipCfg.arg == 1 then
                  ...
                  if chipCfg.arg == 2 then
                    ...
                    error((string.format)("Unsupported chip(%d) arg:%d", chipCfg.id, chipCfg.arg))
                  end
                end
              else
                if chipCfg.influence == (ChipEnum.eChipInfluenceType).HeroId then
                  ...
                  ;
                  (((self.ui).tex_Adapt).text).text = (LanguageUtil.GetLocaleText)(heroCfg.name)
                else
                  do
                    error("找不到tex_Adapt 芯片适配类型")
                  end
                end
              end
            end
...
UINChipItemDetail.UpdateAdaptHero = function(self, heroList, contain)
  ...
  for k,hero in ipairs(heroList) do
    if not contain or hero:ContainChip(self.chipData) then
      ...
      local texture = (self.resLoader):LoadABAsset(PathConsts:GetCharacterPicPath(hero:GetResPicName()))
...
UINChipItemDetail.__HideAllHeroIcon = function(self)
  ...
  if self.__heroIconList == nil then
    return 
  end
