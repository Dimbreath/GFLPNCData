...
UINLevelDtail.__EnterBattleFormation = function(self)
  -- function num : 0_17 , upvalues : _ENV, eDetailType, JumpManager, cs_MessageCommon, FmtEnum, SpecificHeroDataRuler
  ...
  local startBattleFunc = function(curSelectFormationData, callBack)
    -- function num : 0_17_2 , upvalues : self, eDetailType, _ENV, JumpManager, cs_MessageCommon, fmtCtrl
    ...
    if #(self.stageCfg).assist_buff <= 0 then
      do
        local isEzModel = ((self.detailType ~= eDetailType.PeriodicChallenge or self.detailType == eDetailType.PeriodicChallenge)) and self.detailType ~= eDetailType.Stage
        if isEzModel then
      ...
      -- function num : 0_17_2_0 , upvalues : _ENV, self, curSelectFormationId, callBack, curSelectFormationData
      ExplorationManager:ReqEnterExploration((self.stageCfg).id, curSelectFormationId, proto_csmsg_SystemFunctionID.SystemFunctionID_Exploration, true, callBack, curSelectFormationData:GetSupportHeroData())
    end
, function()
      -- function num : 0_17_2_1 , upvalues : _ENV, self, curSelectFormationId, callBack, curSelectFormationData
      ExplorationManager:ReqEnterExploration((self.stageCfg).id, curSelectFormationId, proto_csmsg_SystemFunctionID.SystemFunctionID_Exploration, false, callBack, curSelectFormationData:GetSupportHeroData())
    end
)
        else
          ExplorationManager:ReqEnterExploration((self.stageCfg).id, curSelectFormationId, proto_csmsg_SystemFunctionID.SystemFunctionID_Exploration, false, callBack, curSelectFormationData:GetSupportHeroData())
        ...
        if self.detailType == eDetailType.Infinity then
          ExplorationManager:ReqEnterExploration(((self.levelData).cfg).id, curSelectFormationId, proto_csmsg_SystemFunctionID.SystemFunctionID_Endless, false, callBack, curSelectFormationData:GetSupportHeroData())
...
UINLevelDtail.OnCliCkViewAvg = function(self)
  -- function num : 0_19 , upvalues : _ENV, eInfoNodeType
  (UIUtil.OnClickBack)()
...
, nil)
    ...
    ;
    (ControllerManager:GetController(ControllerTypeId.Avg, true)):StartAvg((self.avgCfg).script_id, (self.avgCfg).id, function()
    -- function num : 0_19_1 , upvalues : self, eInfoNodeType
    (self.playAvgCompleteFunc)()
    self:ShowNode(eInfoNodeType.LevelNormalInfo)
  end
)
