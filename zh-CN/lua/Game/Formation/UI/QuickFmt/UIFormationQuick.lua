...
local UINHeroCardItem = require("Game.Formation.UI.UINFmtCardItem")
...
UIFormationQuick.InitQuickFmt = function(self, fmtData, fmtCtrl, selectHeroData, specificHeroDataRuler)
  -- function num : 0_1 , upvalues : UINHeroSortList, UINHeroCardItem, _ENV, UINSortButtonGroup, UINAthHeroInfo, HeroSortEnum
...
UIFormationQuick.RefreshFmtData = function(self, formationData)
...
UIFormationQuick.RefreshSelectedHero = function(self, heroData)
  ...
  for k,skillData in pairs(heroData.skillList) do
    if selfHeroData == nil or not (selfHeroData.skillDic)[skillData.dataId] then
      local oriSkillData = skillData.type == eHeroSkillType.LifeSkill or skillData:IsCommonAttack() or nil
    end
    local overriodColor = nil
    if oriSkillData ~= nil and oriSkillData.level ~= skillData.level then
      overriodColor = (self.ui).color_syn
    end
...
UIFormationQuick.OnBtnDetailClick = function(self)
  ...
  if (self.specificHeroDataRuler).heroIds == nil then
    hasSpecialHero = self.specificHeroDataRuler == nil
    UIManager:ShowWindowAsync(UIWindowTypeID.HeroState, function(windows)
    -- function num : 0_13_0 , upvalues : _ENV, self, hasSpecialHero
    if windows == nil then
...
UIFormationQuick.OnClickConfirm = function(self)
  ...
  if (table.count)(oldFmtHeroData) == (table.count)(newFmtHeroData) then
    ...
    do
      changed = true
      if changed then
        (self.fmtCtrl):ModifyFormation(nil, newFmtHeroData)
      end
