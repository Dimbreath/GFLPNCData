...
UIATHMain.OnInit = function(self)
  -- function num : 0_0 , upvalues : _ENV, CS_LeanTouch, UINAthList, UINAthAreaItem, UINAthMainAtrrItem, UINAthSuitMainItem, UINAthInfoTogItem
  ...
  for i = 1, (ConfigData.game_config).athSlotCount do
    ...
    areaItem:InitAthAreaItem(R8_PC177, self, self.__OnDragStart)
    -- DECOMPILER ERROR at PC182: Confused about usage of register: R6 in 'UnsetPending'

    ;
    (areaItem.gameObject).name = tostring(R8_PC177)
  end
  ;
  ...
  for k,go in ipairs((self.ui).infoTogList) do
    ...
    -- DECOMPILER ERROR at PC217: Overwrote pending register: R8 in 'AssignReg'

    infoTogItem:Init(go)
    -- DECOMPILER ERROR at PC220: Overwrote pending register: R8 in 'AssignReg'

    infoTogItem:InitAthInfoTogItem(R9_PC223, self.__selectInfoTogFunc)
    -- DECOMPILER ERROR at PC226: Overwrote pending register: R8 in 'AssignReg'

    ;
    (table.insert)(R8_PC177, R9_PC223)
  end
...
UIATHMain.InitATHMain = function(self, heroData, heroResLoader, addAllTouchFunc, switchHeroFunc)
  -- function num : 0_1 , upvalues : CS_Input, cs_ResLoader, _ENV, cs_MessageCommon
...
UIATHMain.__SelectHero = function(self, heroData, heroResLoader)
  ...
  ;
  (self.heroResLoader):LoadABAssetAsync(PathConsts:GetCharacterBigImgPrefabPath(heroData:GetResPicName()), function(prefab)
...
UIATHMain.PlayATHTweenBackwards = function(self)
  -- function num : 0_5
  for i = 0, (self.ATHTweenDic).Length - 1 do
    local tween = (self.ATHTweenDic)[i]
...
UIATHMain.RefreshAthAttr = function(self, athSlotList, curSlotId, changeSort)
  ...
  ;
  ((self.ui).effAddition):SetActive(false)
  local fightPower = (PlayerDataCenter.allAthData):GetAthFightPower(allAttrDic)
  if isAllAttr then
    local fightPowerNoEfcc = (PlayerDataCenter.allAthData):GetAthFightPower(allAttrDicNoEfcc)
    local fpExtra = fightPower - fightPowerNoEfcc
    if fpExtra > 0 then
      ((self.ui).tex_Efficiency):SetIndex(1, tostring(fightPowerNoEfcc))
      ;
      ((self.ui).tex_EffAddition):SetIndex(0, tostring(fpExtra))
      ;
      ((self.ui).effAddition):SetActive(true)
    else
  ...
  for k,attrId in ipairs(self.attrIdSortList) do
    local value = (self.attrValueNoEfccDic)[attrId]
    local valueExtra = isAllAttr and (self.attrValueDic)[attrId] - value or 0
    local attrItem = (self.attrItemPool):GetOne()
    if value ~= nil and value ~= 0 then
      attrItem:InitAthMainAtrrItem(attrId, value, valueExtra)
      isEmpty = false
    else
  ...
  (table.sort)(suitIdList, function(a, b)
    -- function num : 0_7_0 , upvalues : suitDic
...
UIATHMain.__RefreshAfficiencyAttr = function(self)
  ...
  for k,attrId in ipairs(self.attrIdSortList) do
    local value = (self.attrValueNoEfccDic)[attrId]
    local valueExtra = self.isAllAttr and (self.attrValueDic)[attrId] - value or 0
...
UIATHMain.TryInstallAth = function(self, athItem, inAthTable)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.AthItemDetail, function(window)
    -- function num : 0_14_0 , upvalues : athItem, self, inAthTable
    if window == nil then
      return 
    end
...
UIATHMain.TryReplaceAth = function(self, athItem, replacedAthData)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.AthItemDetail, function(window)
    -- function num : 0_15_0 , upvalues : athItem, self, replacedAthData
    if window == nil then
      return 
    end
    local athData = athItem:GetAthItemData()
...
UIATHMain.ShowHeroPowerSide = function(self, power)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.MessageSide, function(win)
    -- function num : 0_16_0 , upvalues : power, _ENV
    if win ~= nil then
...
UIATHMain.OnDragAthItemStart = function(self, athItem, isInTable)
  -- function num : 0_19 , upvalues : CS_LeanTouch, _ENV
...
UIATHMain.__GetTransparentAthItem = function(self, space)
  -- function num : 0_22 , upvalues : UINAthItem
  ...
  if not inAthTable then
    ...
    for k,v in ipairs(athDataList) do
      if v.id == athData.id and v.uid ~= athData.uid then
        ...
        if v:IsConflictAthSuit(athData) then
          replacedAthData = v
          break
        end
      end
...
UIATHMain.SelectInfoTogItem = function(self, infoState)
  -- function num : 0_37 , upvalues : athInfoState
...
UIATHMain.RefillAth = function(self, athData, gridId, resortAthDic, athInstalledDic, refillOp)
  ...
  for k,v in ipairs(athDataList) do
    ...
    if refillOp == proto_csmsg_AthRefillOperator.AthRefillOperatorReplace then
      if v.uid == athData.uid or v.id == athData.id and (v.bindInfo).grid ~= gridId then
        ...
        if v:IsConflictAthSuit(athData) and (v.bindInfo).grid ~= gridId then
          (cs_MessageCommon.ShowMessageTipsWithErrorSound)(ConfigData:GetTipContent(205))
          return 
        end
      end
  ...
  if gridId == nil then
    ...
    -- function num : 0_39_0 , upvalues : self, athData, resortAthDic, athInstalledDic, _ENV, curAreaId, gridId, refillOp
    ...
    -- function num : 0_39_1 , upvalues : self, athData, installFunc
    if window == nil then
      return 
    end
...
UIATHMain._OnAthItemDetailShow = function(self)
...
UIATHMain.OneKeyInstall = function(self)
  -- function num : 0_48 , upvalues : _ENV, AthAreaGridData, AthUtil
  local slots = {}
...
UIATHMain.OneKeyUninstall = function(self)
  -- function num : 0_50 , upvalues : _ENV, oneKeyUninstallSlotList
  local hasAth = false
...
UIATHMain.OnDelete = function(self)
  -- function num : 0_54 , upvalues : _ENV, CS_LeanTouch, CS_Input, base
