...
AllBuildingData.UpdateBuildLevelRecord = function(self, buildId, curLevel, oriLevel)
  -- function num : 0_13 , upvalues : _ENV, BuildingEnum
  local cfg = (ConfigData.building)[buildId]
  if cfg == nil then
    return 
  end
  if not oriLevel then
    self.stoTotalLevel = self.stoTotalLevel - (cfg.type ~= (BuildingEnum.BuildingType).SectorProduct and cfg.type ~= (BuildingEnum.BuildingType).SectorAddtion or 0)
    self.stoTotalLevel = self.stoTotalLevel + (curLevel or 0)
    do return  end
    if not oriLevel then
      self.careerTotalLevel = self.careerTotalLevel - (cfg.type ~= (BuildingEnum.BuildingType).CareerMaster or 0)
