...
UINHeroInfoStrotyNodeUnlockedItem.InitHISNUnlockedItem = function(self, storyData, HeroInfoTextUtil, playVoiceCallback, isHaveVoiceRes)
  ...
  if self.isVoice then
    ...
    if not (ControllerManager:GetController(ControllerTypeId.Cv, true)):GetCvText(self.heroId, self.voiceId) then
      do
        ((self.ui).tex_Content).text = ConfigData:GetTipContent(TipContent.HeroInformation_notHaveVoice)
        ;
