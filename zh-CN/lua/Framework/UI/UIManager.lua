...
local CreateWindowInternal = function(self, typeID, isAsync, replaceTypeId, fromWhichUI)
  -- function num : 0_12 , upvalues : cs_ResLoader, _ENV, InitWindowInternal, cs_UIManager
  ...
  if isAsync then
    ...
    -- function num : 0_12_0 , upvalues : InitWindowInternal, self, typeID, uiconfig, fromWhichUI, _ENV, tmpId, cs_UIManager
...
UIManager.ShowWindow = function(self, typeID, replaceType, fromWhichUI)
  -- function num : 0_15 , upvalues : CreateWindowInternal
  local target = self:GetWindow(typeID)
  if target then
    target:Show()
    return target
...
UIManager.CreateWindowAsync = function(self, typeID, openCallback, replaceType, fromWhichUI)
  -- function num : 0_17 , upvalues : _ENV, cs_UIManager, CreateWindowInternal
  local target = self:GetWindow(typeID)
  if target ~= nil then
    openCallback(target)
    return 
...
UIManager.ShowWindowAsync = function(self, typeID, openCallback, replaceType, fromWhichUI)
  ...
  if (self.resloaders)[typeID] ~= nil then
    ...
    -- function num : 0_18_0 , upvalues : openCallback
    if win ~= nil then
      win:Show()
    end
    if openCallback ~= nil then
  ...
  self:CreateWindowAsync(typeID, function(window)
    -- function num : 0_18_1 , upvalues : openCallback
    if window ~= nil then
      window:Show()
    end
    if openCallback ~= nil then
...
UIManager.ClearAllAsyncLoad = function(self)
  -- function num : 0_25 , upvalues : _ENV, cs_UIManager
...
UIManager.CreateThreeDCanvas = function(self, typeId, Callback, name)
  ...
  ((self.resloaders)[typeId]):LoadABAssetAsync(path, function(prefab)
    -- function num : 0_30_0 , upvalues : name, Callback
...
UIManager.HideAllWindow = function(self, exceptDic)
  ...
  exceptDic[UIWindowTypeID.Guide] = true
  local hideWinList = {}
