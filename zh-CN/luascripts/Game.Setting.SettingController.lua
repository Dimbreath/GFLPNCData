...
SettingController.InitSettingData = function(self)
    ...
    self:SetTextureLimit(displaySetting.texture_limit or 1)
    self:SetFrameRateIndex(displaySetting.frame_rate or 1)
...
SettingController.SetResolutionIndex = function(self, index)
    -- function num : 0_4 , upvalues : cs_RenderManager
    if index == 0 then
        ...
        if index == 1 then
            ...
            if index == 2 then
                cs_RenderManager:SetResolutionRatio(0.6)
            end
        end
    end
    local displaySetting = self:GetSettingDisplayData()
