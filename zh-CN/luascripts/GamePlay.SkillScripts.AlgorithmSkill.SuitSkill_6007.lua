...
bs_6007.OnSetHurt = function(self, context)
  ...
  if context.target == self.caster then
    local retHurt = (ShieldSkillBase.ShieldBaseFunc)(self, context.hurt, self.caster)
...
bs_6007.OnAfterBattleStart = function(self)
  ...
  ((self.caster).recordTable)[(self.config).shieldKey] = LuaSkillCtrl:CallFormulaNumberWithSkill((self.config).formulaID, self.caster, self.caster, self)
