...
bs_5016.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_5016_3", 4,
                    self.OnAfterHurt)
end

bs_5016.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender.belongNum == (self.caster).belongNum and isCrit then
        LuaSkillCtrl:CallBuff(self, sender, (self.config).buffId, 1,
                              (self.arglist)[2])
    end
end
