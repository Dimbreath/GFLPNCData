...
UINRandomBeforeBattle.OnInit = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_rejudgeAmbush, self,
                               self.RejudgeAmbush);
    (UIUtil.AddButtonListener)((self.ui).btn_rejudgeStealth, self,
                               self.RejudgeSneak);
    (UIUtil.AddButtonListener)((self.ui).btn_stealth, self, self.StartSneak)
    self.focusPointNum = 0
    self.foucusPointItemId =
        (ConfigData.game_config).RandomBeforeBatteleRejudgeItem
    self.focusPointNum = 0
    self.ambushPool = (UIItemPool.New)(UINRandomBeforeBattlePassItem,
                                       (self.ui).ambushItem);
    ((self.ui).ambushItem):SetActive(false)
    self.stealthPool = (UIItemPool.New)(UINRandomBeforeBattlePassItem,
                                        (self.ui).stealthItem);
    ((self.ui).stealthItem):SetActive(false)
    self.__updateData = BindCallback(self, self.UpdateData)
    MsgCenter:AddListener(eMsgEventId.OnBeforeBattleRandomDataChange,
                          self.__updateData)
    self.__refreshFocusPoint = BindCallback(self, self.RefreshFocusPoint)
...
UINRandomBeforeBattle.InitData = function(self, roomData, focusLimit,
                                          focusPointNum)
    -- function num : 0_1
    self.roomData = roomData
    self.roomPosition = roomData.position
...
UINRandomBeforeBattle.UpdateData = function(self, ambush, stealth, position)
    -- function num : 0_2 , upvalues : _ENV, __randomType
    if position ~= self.roomPosition then return end
    for key, value in pairs(__randomType) do
        -- DECOMPILER ERROR at PC10: Confused about usage of register: R9 in 'UnsetPending'
...
UINRandomBeforeBattle.ShowResult = function(self, type, career, judgeResult)
    ...
    if type == __randomType.ambush then
        ...
        if type == __randomType.stealth then pool = self.stealthPool end
    end
    pool:HideAll()
    for index, judgeNum in ipairs(judgeResult) do
        local isPass = judgeNum > 0
        ...
        if isAllPass then
            ...
            if type == __randomType.ambush then
                ...
                if type == __randomType.stealth then
                    (((self.ui).btn_stealth).gameObject):SetActive(false)
                    if #(self.judgeItemList)[type] == 0 then
                        (((self.ui).btn_rejudgeStealth).gameObject):SetActive(
                            false)
                    else
...
UINRandomBeforeBattle.RejudgeAmbush = function(self)
    -- function num : 0_6 , upvalues : cs_MessageCommon
    if self.focusPointNum > 0 then
        (self.explorationNetworkCtrl):CS_EXPLORATION_BATTLE_Focus(
            self.roomPosition, true, function(value)
                -- function num : 0_6_0 , upvalues : self
                (self.roomData):SetAmbushAndSneakData(value,
                                                      (self.roomData).stealthData)
            end)
    else
...
UINRandomBeforeBattle.RejudgeSneak = function(self)
    ...
    if self.focusPointNum > 0 then
        (self.explorationNetworkCtrl):CS_EXPLORATION_BATTLE_Focus(
            self.roomPosition, false, function(value)
                -- function num : 0_7_0 , upvalues : self
                (self.roomData):SetAmbushAndSneakData(
                    (self.roomData).ambushData, value)
            end)
    else
...
UINRandomBeforeBattle.StartSneak = function(self)
    -- function num : 0_8 , upvalues : _ENV
    (self.explorationNetworkCtrl):CS_EXPLORATION_BATTLE_Stealth(
        self.roomPosition);
    ((ExplorationManager.epCtrl).battleCtrl):SetSneak()
    self:Delete()
end

UINRandomBeforeBattle.RefreshFocusPoint =
    function(self, itemNum, focusLimit)
        -- function num : 0_9 , upvalues : _ENV
        self.focusPointNum = itemNum;
        ((self.ui).txt_focusPiontText):SetIndex(0, tostring(itemNum),
                                                tostring(focusLimit))
    end

UINRandomBeforeBattle.OnDelete = function(self)
    -- function num : 0_10 , upvalues : _ENV, base
    MsgCenter:RemoveListener(eMsgEventId.OnBeforeBattleRandomDataChange,
                             self.__updateData)
