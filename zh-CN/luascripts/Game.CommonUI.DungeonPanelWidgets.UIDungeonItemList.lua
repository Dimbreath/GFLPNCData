...
UIDungeonItemList.InjectItemUpdateEvent =
    function(self, updateItemEvent)
        -- function num : 0_2
        self.updateItemEvent = updateItemEvent
    end
...
UIDungeonItemList.LoadItemList = function(self, itemIndex)
    ...
    ((self.ui).loopList).onChangeItem = BindCallback(self, self.__onChangeItem) -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).loopList).onInstantiateItem =
        BindCallback(self, self.__onInstantiateItem) -- DECOMPILER ERROR at PC17: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).loopList).totalCount = self.itemCount;
    ((self.ui).loopList):RefillCells()
    if itemIndex ~= nil then
...
UIDungeonItemList.__onChangeItem = function(self, go, index)
    -- function num : 0_4
    if self.updateItemEvent ~= nil then (self.updateItemEvent)(go, index) end
end

UIDungeonItemList.__onInstantiateItem = function(self, go)
    -- function num : 0_5
    if self.initItemEvent ~= nil then (self.initItemEvent)(go) end
end

UIDungeonItemList.InjectDataAndFilterEvent =
    function(self, dungeonDataList)
        -- function num : 0_6
        self.dungeonDataList = dungeonDataList
        self.itemCount = #dungeonDataList
    end
