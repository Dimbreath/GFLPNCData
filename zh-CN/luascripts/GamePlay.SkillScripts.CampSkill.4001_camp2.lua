...
bs_4001.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_4001_3", 1, self.OnAfterHurt)
end

bs_4001.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
...
bs_4001.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_4001_3", 1,
                    self.OnAfterHurt)
end

bs_4001.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack and
        LuaSkillCtrl:CallRange(1, (self.config).probMax) ==
        (self.config).probHit then
        local targetList = LuaSkillCtrl:CallTargetSelect(self, 1001, 0)
        self:CallCasterLookAt(targetList)
