...
local UINDungeonInfoHeroDetailAttr = require(
                                         "Game.CommonUI.DungeonState.Info.UINDungeonInfoHeroDetailAttr")
local UINDungeonInfoHeroDetailSkill = require(
                                          "Game.CommonUI.DungeonState.Info.UINDungeonInfoHeroDetailSkill")
...
UINDungeonInfoHeroDetail.OnInit = function(self)
    ...
    self.attrPool = (UIItemPool.New)(UINDungeonInfoHeroDetailAttr,
                                     (self.ui).obj_attriItem);
    ((self.ui).obj_attriItem):SetActive(false)
    self.skillPool = (UIItemPool.New)(UINDungeonInfoHeroDetailSkill,
...
UINDungeonInfoHeroDetail.InitHeroInfo = function(self, dynHeroData, resloader)
    ...
    ((self.ui).img_Quality).color = HeroRareColor[(dynHeroData.heroData).rare] -- DECOMPILER ERROR at PC13: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_Power).text = tostring(dynHeroData:GetFightingPower())
    local hpPer = dynHeroData.hpPer / 10000 -- DECOMPILER ERROR at PC18: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_Hp).fillAmount = hpPer
    local maxHp = dynHeroData:GetRealAttr(eHeroAttr.maxHp);
    ((self.ui).tex_Hp):SetIndex(0, tostring((math.floor)(maxHp * hpPer)),
                                tostring(maxHp));
    ...
    for index, skillData in ipairs((dynHeroData.heroData).skillList) do
        if skillData.type == eHeroSkillType.BattleSkill and
            ...
            skillItem:InitSkillItem(skillData, resloader,
...
UINDungeonInfoHeroDetail.__ShowSkillDetail =
    function(self, item, skillData)
        ...
        win:SetTitleAndContext(skillData:GetName(),
                               skillData:GetCurLevelDescribe())
        win:FloatTo(item.transform, HAType.center, VAType.up)
    end

UINDungeonInfoHeroDetail.__HideSkillDetail =
    function(self)
