...
utf8.len = function(s)
    ...
    local len = 0
    for _ in (utf8.byte_indices)(s) do len = len + 1 end
    return len
end
...
utf8.byte_index = function(s, target_ci)
    ...
    for i in (utf8.byte_indices)(s) do
        ci = ci + 1
        if ci == target_ci then return i end
    end
...
utf8.char_index = function(s, target_i)
    ...
    if target_i < 1 or #s < target_i then return end
    local ci = 0
    for i in (utf8.byte_indices)(s) do
        ci = ci + 1
        if i == target_i then return ci end
    end
    error("invalid index")
end
...
utf8.prev = function(s, nexti)
    ...
    if not nexti then nexti = #s + 1 end
    if nexti <= 1 or #s + 1 < nexti then return end
    local lasti, lastvalid = (utf8.next)(s)
    for i, valid in (utf8.byte_indices)(s) do
        if i == nexti then return lasti, lastvalid end
        lasti = i
    end
    if nexti == #s + 1 then return lasti, lastvalid end
    error("invalid index")
end
...
utf8.sub = function(s, start_ci, end_ci)
    ...
    for i in (utf8.byte_indices)(s) do
        ci = ci + 1
        if ci == start_ci then start_i = i end
        if ci == end_ci then end_i = i end
    end
    if ci >= start_ci then
        assert(start_i, "invalid index")
        do return "" end
        if end_ci and not end_i then
            if end_ci < start_ci then return "" end
...
utf8.contains = function(s, i, sub)
    -- function num : 0_8
    if i < 1 or #s < i then return nil end
    for si = 1, #sub do
        if s:byte(i + si - 1) ~= sub:byte(si) then return false end
    end
    return true
end
        ...
        if c >= 0 and c <= 127 then
            ...
            elseif c >= 224 and c <= 239 then
                ...
                if c2 and c3 and (c2 < 160 or c2 > 191 or c3 < 128 or c3 > 191) then
                    ...
                    if c2 and c3 and
                        (c2 < 128 or c2 > 191 or c3 < 128 or c3 > 191) then
                        do return c < 225 or c > 236 end
                        -- DECOMPILER ERROR at PC96: Unhandled construct in 'MakeBoolean' P1

                        if c2 and c3 and
                            (c2 < 128 or c2 > 159 or c3 < 128 or c3 > 191) then
                            do
                                do
                                    return c ~= 237
                                end
                                if c >= 238 and c <= 239 then
                                    if c == 239 and c2 == 191 and
                                        (c3 == 190 or c3 == 191) then
                                        return false
                                    end
                                    return
                                        not c2 or not c3 or
                                            (c2 >= 128 and c2 <= 191 and c3 >=
                                                128 and c3 <= 191)
                                end
                                if c >= 240 and c <= 244 then
                                    local c2 = s:byte(i + 1)
                                    ...
                                    if c2 and c3 and c4 and
                                        (c2 < 144 or c2 > 191 or c3 < 128 or c3 >
                                            191 or c4 < 128 or c4 > 191) then
                                        do
                                            return c ~= 240
                                        end
                                        ...
                                        if c2 and c3 and c4 and
                                            (c2 < 128 or c2 > 191 or c3 < 128 or
                                                c3 > 191 or c4 < 128 or c4 > 191) then
                                            do
                                                return c < 241 or c > 243
                                            end
                                            -- DECOMPILER ERROR at PC213: Unhandled construct in 'MakeBoolean' P1

                                            if c2 and c3 and c4 and
                                                (c2 < 128 or c2 > 143 or c3 <
                                                    128 or c3 > 191 or c4 < 128 or
                                                    c4 > 191) then
                                                do
                                                    do
                                                        return c ~= 244
...
utf8.replace = function(s, f, ...)
    ...
    if type(f) == "table" then return (utf8.replace)(s, table_lookup, f) end
    if s == "" then return s end
    local t = {}
    local lasti = 1
    for i in (utf8.byte_indices)(s) do
        if not (utf8.next)(s, i) then local nexti = #s + 1 end
...
local replace_invalid = function(s, i, j, repl_char)
    ...
    if not (utf8.isvalid)(s, i) then return repl_char end
end

utf8.sanitize = function(s, repl_char)
    -- function num : 0_17 , upvalues : utf8, replace_invalid
    if not repl_char then repl_char = "�" end
