...
UIFormationQuick.OnInit = function(self)
    ...
    HeroSortList:InitHeroSortList(self.resloader, nil,
                                  BindCallback(self, self.OnSelectHero), false,
                                  false, false, false)
...
UIFormationQuick.RefreshFmtData = function(self, fmtId)
    ...
    for i = 1, self.fmtMaxHeroCount do
        ...
        if heroId ~= nil then
            (table.insert)(self.inFmtHeroList, heroId) -- DECOMPILER ERROR at PC39: Confused about usage of register: R8 in 'UnsetPending'
            ;
    ...
    for i = 1, (ConfigData.game_config).max_bench_hero do
        ...
        if unlock then
            ...
            if heroId ~= nil then
                (table.insert)(self.inFmtBenchHeroList, heroId) -- DECOMPILER ERROR at PC68: Confused about usage of register: R9 in 'UnsetPending'
                ;
...
UIFormationQuick.RefreshFmtHeroCount = function(self)
    ...
    ((self.ui).tex_MainNum):SetIndex(0, tostring(#self.inFmtHeroList),
                                     tostring(self.fmtMaxHeroCount));
    ((self.ui).tex_SubNum):SetIndex(0, tostring(#self.inFmtBenchHeroList),
                                    tostring(self.fmtMaxBenchHeroCount))
end
...
UIFormationQuick.RefreshSelectedHero = function(self, heroData)
    ...
    ((self.ui).emptyNode):SetActive(false) -- DECOMPILER ERROR at PC28: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = heroData:GetName() -- DECOMPILER ERROR at PC35: Confused about usage of register: R2 in 'UnsetPending'
    ;
        ...
        if index < 10 then
            do
                if ((ConfigData.attribute)[attrId]).merge_attribute == 0 then
                    index = index + 1
                    attrDataList[index] =
                        {attrId = attrId, attrValue = heroData:GetAttr(attrId)}
                end
    ...
    for k, skillData in pairs(heroData.skillList) do
        if skillData.type == eHeroSkillType.BattleSkill and
            ...
            skillItem:InitFmtSkillItem(skillData, self.resloader, nil,
                                       self.__ShowSkillDescription,
                                       self.__HideSkillDetail)
        end
    end
...
UIFormationQuick.OnSelectHero = function(self, heroData, heroCardItem)
    ...
    if (self.inFormationDic)[heroId] == nil then
        if self.fmtMaxHeroCount + self.fmtMaxBenchHeroCount <=
            (table.count)(self.inFormationDic) then
            (CS_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(TipContent.Formation_MaxHeroCount))
            return
        end
        ...
        end -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
        ...
        end -- DECOMPILER ERROR at PC70: Confused about usage of register: R4 in 'UnsetPending'
        
        (self.inFormationDic)[heroId] = nil
        if heroCardItem ~= nil then heroCardItem:SetSelectActive(false) end
...
UIFormationQuick.OnBtnDetailClick = function(self)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.HeroState, function(windows)
        ...
        if windows == nil then
            error((LanguageUtil.GetLocaleText)((self.heroData).name) ..
                      "Click can\'t show state")
            return
        end
        windows:InitHeroState(self.heroData, self.resloader,
                              (self.heroSortList).curHeroList, function()
...
UIFormationQuick.OnFilterConfirmAction =
...
UIFormationQuick.OnClickConfirm = function(self)
    ...
    for k, heroId in ipairs(self.inFmtHeroList) do newFmtHeroData[k] = heroId end
