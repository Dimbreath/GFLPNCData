...
local UIDormSortListItemFnt = require("Game.Dorm.UI.SortList.UIDormSortListItemFnt")
local UIDormSortListItemRoom = require("Game.Dorm.UI.SortList.UIDormSortListItemRoom")
UIDormSortList.OnInit = function(self)
  ...
  ;
  ((self.ui).scrollView).onInstantiateItem = BindCallback(self, self.OnInstantiateItem)
...
UIDormSortList.RefreshDormItemList = function(self, origineDataList, funcSift, funcSort)
  ...
  if funcSort ~= nil then
    (table.sort)(self.curDataList, funcSort)
  end
...
UIDormSortList.OnItemChange = function(self, go, index)
  ...
  if self.itemType == eItemType.DormRoom then
    dormItem:InitSortListItemRoom(dormData, self.isShop, self.__onDormItemClick)
  else
    if self.itemType == eItemType.DormFurniture then
      dormItem:InitSortListItemFnt(dormData, self.isShop, self.__onDormItemClick, self.__onItemClickInstallEvent)
    end
  end
end
...
UIDormSortList.OnDelete = function(self)
  ...
  if self.itemDic ~= nil then
    for go,item in pairs(self.itemDic) do
      item:OnDelete()
    end
    self.itemDic = nil
  end
