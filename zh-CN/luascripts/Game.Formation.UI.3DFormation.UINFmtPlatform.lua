...
UINFmtPlatform.InitFmtPlatform = function(self, fmtIndex, isBench, lockStr)
    ...
    if not isBench then
        ((self.ui).img_SelectHero).color = (self.ui).selectHeroColor -- DECOMPILER ERROR at PC14: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_SelectHero).color = (self.ui).selectHeroColor
    end
end
...
UINFmtPlatform.RefreshUIFmtPlatform = function(self, heroData)
    ...
    ((self.ui).power):SetActive(false)
    if not (string.IsNullOrEmpty)(self.lockStr) then
        ((self.ui).lock):SetActive(true) -- DECOMPILER ERROR at PC40: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.ui).tex_Condition).text = self.lockStr
    else
        if heroData == nil then
            ...
            local campSprite = CRH:GetSprite((heroData:GetCampCfg()).icon,
            ...
            fightingPower = heroData:GetFightingPower();
            ((self.ui).power):SetActive(true) -- DECOMPILER ERROR at PC91: Confused about usage of register: R4 in 'UnsetPending'
            ;
            ((self.ui).tex_Power).text = tostring(fightingPower)
        end
    end
