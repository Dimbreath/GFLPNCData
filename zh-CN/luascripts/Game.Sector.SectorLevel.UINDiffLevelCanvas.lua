...
UINDiffLevelCanvas.OnInit = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui)
    self.__levelGroupUI = {};
    (UIUtil.LuaUIBindingTable)(((self.ui).levelGroup).transform,
                               self.__levelGroupUI);
    (UIUtil.AddButtonListener)((self.ui).contentVertical, self, self.__OnClickBg);
    (UIUtil.AddButtonListener)((self.ui).contentHorizontal, self,
                               self.__OnClickBg)
    self.__inited = false;
    (((self.ui).switchTween).onComplete):AddListener(
        BindCallback(self, self.OnSwitchTweenComplete));
    (((self.ui).switchTween).onRewind):AddListener(
        BindCallback(self, self.OnSwitchTweenRewind))
    self.scrollItemGoDic = {}
end

UINDiffLevelCanvas.InitDiffLevelCanvas =
    function(self, sectorId, lastEpStateCfg, difficulty, levelItemClickEvent,
             levelAvgMainClickEvent, lAvgSubClickEvent, tweenCompleteEvent,
             resLoader, sectorLevelTipsGuides, clickBackFunc)
        -- function num : 0_1 , upvalues : _ENV
        self.sectorId = sectorId
        self.lastEpStateCfg = lastEpStateCfg
        self.difficulty = difficulty
        ...
        if arrangeCfg == nil then
            error((string.format)(
                      "Can\'t find level arrange Cfg,arrangeId:%s, difficulty:%s, sectorId:%s",
                      levelArrangeType, difficulty, sectorId))
            return
        end
        self.arrangeCfg = arrangeCfg
        self:InitLevelGroupData()
        local isVertical = (arrangeCfg[1]).vertical
        local verticalChange = self.isVertical ~= nil and self.isVertical ~=
                                   isVertical
        self.isVertical = isVertical
        if self.__inited and verticalChange then
            ((self.ui).scrollRect):ClearCells()
        ...
        self:AddScrollRect() -- DECOMPILER ERROR at PC74: Confused about usage of register: R16 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).onInstantiateItem =
            BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC81: Confused about usage of register: R16 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).onChangeItem =
            BindCallback(self, self.__OnChangeItem) -- DECOMPILER ERROR at PC88: Confused about usage of register: R16 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).onReturnItem =
            BindCallback(self, self.__OnReturnItem)
        self.__inited = true
        self:RefillScrollRect()
        -- DECOMPILER ERROR: 3 unprocessed JMP targets
    end
...
UINDiffLevelCanvas.InitLevelGroupData = function(self)
    ...
    local AddTempListFunc = function(cfg, isState)
        ...
        if self.lastEpStateCfg ~= nil and isState and cfg.id ==
            (self.lastEpStateCfg).id then
            (self.lastCompletedStage).groupIndex = groupIndex -- DECOMPILER ERROR at PC54: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.lastCompletedStage).index = index
        end
        -- DECOMPILER ERROR at PC75: Confused about usage of register: R3 in 'UnsetPending'

        if (isState and (PlayerDataCenter.sectorStage):IsStageComplete(cfg.id)) or
            not isState and avgPlayCtrl:IsAvgPlayed(cfg.id) then
            (self.lastCompletedStage).groupIndex = groupIndex -- DECOMPILER ERROR at PC78: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.lastCompletedStage).index = index
        end
    end

    local sectorStageCfg =
        (((ConfigData.sector_stage).sectorDiffDic)[self.sectorId])[self.difficulty]
    for k, stageId in ipairs(sectorStageCfg) do
        local avgCfg = avgPlayCtrl:TryGetAvgCfg(eAvgTriggerType.MainAvg,
                                                stageId, 1)
        if avgCfg ~= nil then AddTempListFunc(avgCfg, false) end
        local stage = (ConfigData.sector_stage)[stageId]
        AddTempListFunc(stage, true)
        avgCfg = avgPlayCtrl:TryGetAvgCfg(eAvgTriggerType.MainAvg, stageId, 2)
        if avgCfg ~= nil then AddTempListFunc(avgCfg, false) end
...
UINDiffLevelCanvas.AddScrollRect = function(self, prefabSource)
    ...
    if self.isVertical then
        (self.ui).scrollRect = ((self.ui).scrollRectGo):AddComponent(
                                   typeof(LoopVerticalScrollRect)) -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).horizontal = false -- DECOMPILER ERROR at PC25: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).content = ((self.ui).contentVertical).transform
    else
        -- DECOMPILER ERROR at PC35: Confused about usage of register: R2 in 'UnsetPending'
        ...
        (self.ui).scrollRect = ((self.ui).scrollRectGo):AddComponent(
                                   typeof(LoopHorizontalScrollRect)) -- DECOMPILER ERROR at PC38: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).vertical = false -- DECOMPILER ERROR at PC44: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).scrollRect).content = ((self.ui).contentHorizontal).transform
    end
    if prefabSource == nil then
        ...
        ((self.ui).scrollRect):SetPrefabSource(prefabSource)
    end -- DECOMPILER ERROR at PC61: Confused about usage of register: R2 in 'UnsetPending'
    
    ((self.ui).scrollRect).rubberScale = 0.4 -- DECOMPILER ERROR at PC66: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).newItemBoundOffset = (self.__levelGroupUI).padding;
    ((self.ui).scrollRectGo):SetActive(true)
end

UINDiffLevelCanvas.LocationSectorStageItem =
    function(self, id, isAvg)
        -- function num : 0_5 , upvalues : _ENV
        if not isAvg then isAvg = false end
        for groupIndex, group in pairs(self.levelGroupDataList) do
            for index, data in ipairs(group) do
                do
                    -- DECOMPILER ERROR at PC23: Unhandled construct in 'MakeBoolean' P1
                    ...
                    if isAvg and data.avgCfg ~= nil and (data.avgCfg).id == id then
                        local groupItem =
                            self:GetLevelGroupItemByIndex(groupIndex)
                        if groupItem ~= nil then
                            return groupItem:GetLevelItemByIndex(index),
                                   groupItem
                        end
                        return nil, nil
                    end
                    ...
                    if data.stageCfg ~= nil and (data.stageCfg).id == id then
                        (self.lastCompletedStage).groupIndex = groupIndex -- DECOMPILER ERROR at PC43: Confused about usage of register: R13 in 'UnsetPending'
                        ;
                        (self.lastCompletedStage).index = index
                        self:RefillScrollRect()
                        local groupItem =
            ...
            if data.stageCfg ~= nil and (data.stageCfg).id == stageId then
                ...
                do
                    do
                        if groupItem ~= nil then
                            local levelItem =
                                groupItem:GetLevelItemByIndex(index)
                            return levelItem
                        end
                        do return end
                        -- DECOMPILER ERROR at PC25: LeaveBlock: unexpected jumping out DO_STMT
...
UINDiffLevelCanvas.SetSectorStageItemBlueDot =
    function(self, stageId, show)
        -- function num : 0_7
        local levelItem = self:GetSectorStageItem(stageId)
        if levelItem ~= nil then levelItem:ShowBlueDotLevelItem(show) end
    end

UINDiffLevelCanvas.GetSectorLAvgMainItem =
    function(self, avgId)
        -- function num : 0_8 , upvalues : _ENV
        for groupIndex, group in pairs(self.levelGroupDataList) do
            for index, data in ipairs(group) do
...
UINDiffLevelCanvas.SetSectorLAvgMainItemBlueDot =
    function(self, avgId, show)
        -- function num : 0_9
        local lAvgMainItem = self:GetSectorLAvgMainItem(avgId)
        if lAvgMainItem ~= nil then
            lAvgMainItem:ShowBlueDotLAvgMain(show)
...
UINDiffLevelCanvas.RefillScrollRect = function(self)
    -- function num : 0_10 , upvalues : _ENV
    local index = (self.lastCompletedStage).index or 1
    local groupIndex = (self.lastCompletedStage).groupIndex or 1 -- DECOMPILER ERROR at PC14: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).totalCount = #self.levelGroupDataList
    local indexOffset = groupIndex - 1
    local posOffset = 0
    do
        ...
        for i = groupIndex, #self.levelGroupDataList do
            ...
            remainLength = remainLength + (math.abs)(endPos) +
                               (self.__levelGroupUI).padding
        end
        remainLength = remainLength - (posOffset)
        if not self.isVertical or not ((self.transform).rect).height then
            local viewLength = ((self.transform).rect).width
...
UINDiffLevelCanvas.__OnChangeItem = function(self, go, index)
    ...
    item:InitLevelGroup(levelGroupData, self.lastEpStateCfg, self.arrangeCfg,
                        (self.ui).lineHolder, (self.ui).bgHolder,
                        self.stageCount, self.levelItemClickEvent,
                        self.levelAvgMainClickEvent, self.lAvgSubClickEvent,
                        self.resLoader, self.sectorLevelTipsGuides)
    for k, v in ipairs(levelGroupData) do
        if v.stageCfg ~= nil then
            local levelItem = item:GetLevelItemByIndex(k)
            if levelItem ~= nil then
                if (PlayerDataCenter.sectorStage):IsStageUnlock((v.stageCfg).id) then
                    local showBlueDot =
                        not (PlayerDataCenter.sectorStage):IsStageComplete(
                            (v.stageCfg).id)
                end
                levelItem:ShowBlueDotLevelItem(showBlueDot)
            end
            ...
            do
                if v.avgCfg ~= nil then
                    ...
                    if lAvgMainItem ~= nil then
                        ...
                        if avgPlayCtrl:IsAvgUnlock(avgId) then
                            local showBlueDot =
                                not avgPlayCtrl:IsAvgPlayed(avgId)
                        end
                        lAvgMainItem:ShowBlueDotLAvgMain(showBlueDot)
                    end
                end
...
UINDiffLevelCanvas.GetLevelGroupItemByIndex =
    function(self, index)
        -- function num : 0_14
        local go = ((self.ui).scrollRect):GetCellByIndex(index - 1)
        return (self.scrollItemGoDic)[go]
...
UINDiffLevelCanvas.RefreshUncompletedEp =
    function(self, lastEpStateCfg)
        -- function num : 0_15 , upvalues : _ENV
        self.lastEpStateCfg = lastEpStateCfg
        for k, v in ipairs(self.levelGroupDataList) do
            local groupItem = self:GetLevelGroupItemByIndex(k)
...
UINDiffLevelCanvas.RefreshLevelState = function(self)
    ...
    for k, v in ipairs(self.levelGroupDataList) do
        local groupItem = self:GetLevelGroupItemByIndex(k)
        if groupItem ~= nil then groupItem:RefreshLevelItemState() end
    end
end

UINDiffLevelCanvas.PlayDiffLevelCanvasSwitchTween =
    function(self, isUpTween)
        -- function num : 0_17 , upvalues : _ENV
        ((self.gameObject).transform):SetAsLastSibling()
        if isUpTween then
            ((self.ui).switchTween):DORestart()
...
UINDiffLevelCanvas.OnSwitchTweenComplete =
    function(self)
        -- function num : 0_18
        if self.__isBackwardsTween then return end
        self:OnSwitchTweenEndEvent()
    end

UINDiffLevelCanvas.OnSwitchTweenRewind =
    function(self)
        -- function num : 0_19
        if not self.__isBackwardsTween then return end
        self.__isBackwardsTween = false
        self:OnSwitchTweenEndEvent()
    end

UINDiffLevelCanvas.OnSwitchTweenEndEvent =
    function(self)
        -- function num : 0_20 , upvalues : _ENV
        UIManager:HideWindow(UIWindowTypeID.ClickContinue);
        ((self.ui).switchTween):DORewind()
...
UINDiffLevelCanvas.PlayDiffLevelContentTween =
    function(self, duration)
        -- function num : 0_21 , upvalues : CS_Ease
        if not self.isVertical then return end
        if self.contentVerticalTween ~= nil then
            (self.contentVerticalTween):Kill()
        end
        self.contentVerticalTween =
            ((((self.ui).contentVertical).transform):DOAnchorPosX(0, duration)):SetEase(
                CS_Ease.InQuad)
    end

UINDiffLevelCanvas.__OnClickBg = function(self)
    -- function num : 0_22
    if self.clickBackFunc ~= nil then (self.clickBackFunc)() end
end
...
UINDiffLevelCanvas.OnDelete = function(self)
    ...
    for k, v in pairs(self.scrollItemGoDic) do v:Delete() end
    self.resLoader = nil;
    (base.OnDelete)(self)
end
