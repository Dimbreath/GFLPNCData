...
bs_1067.PlaySkill = function(self, data)
  ...
  if grid ~= nil then
    ...
    LuaSkillCtrl:CallEffect(target, (self.config).effectId3, self, nil, nil, nil, true)
    self.weaponeffect = LuaSkillCtrl:CallEffect(self.caster, (self.config).effectId4, self, nil, nil, nil, true)
    self:PlaySkillEnd()
    ;
    (self.caster):LookAtTarget(target)
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1002, 1.5, 7, triggerCallBack)
    self.mark = {}
    self.arivedRoles = {}
    self.realTime = 0
    self.effectBall = nil
...
bs_1067.OnCollision = function(self, index, entity)
  ...
  if LuaSkillCtrl:RoleContainsBuffFeature(entity, eBuffFeatureType.NotBeSelected) == true then
    return 
  end
