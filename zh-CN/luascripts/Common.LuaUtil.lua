...
ConcatSafePack = function(safe_pack_l, safe_pack_r)
    -- function num : 0_2
    local concat = {}
    for i = 1, safe_pack_l.n do concat[i] = safe_pack_l[i] end
    for i = 1, safe_pack_r.n do concat[safe_pack_l.n + i] = safe_pack_r[i] end
    concat.n = safe_pack_l.n + safe_pack_r.n
    return concat
end
...
Serialize = function(tb, flag)
    ...
    for k, v in pairs(tb) do
        if type(k) == "number" then
            if type(v) == "table" then
                ...
                if type(v) == "number" then
                    ...
                    if type(v) == "string" then
                        ...
                        if type(v) == "boolean" then
                            result = (string.format)("%s[%d]=%s,", result, k,
                                                     tostring(v))
                        else
                            if flag then
                                result =
                                    (string.format)("%s[%d]=%q,", result, k,
                                                    type(v))
                            else
                                error(
                                    "the type of value is a function or userdata")
                            end
                        end
                    end
                end
            ...
            if type(v) == "table" then
                result = (string.format)("%s%s=%s,", result, k,
                                         Serialize(v, flag))
            else
                if type(v) == "number" then
                    result = (string.format)("%s%s=%d,", result, k, v)
...
CreatEnumTable = function(tbl, index)
    ...
    local enumtbl = {}
    local enumindex = index or 0
    for i, v in ipairs(tbl) do enumtbl[v] = enumindex + i end
    do return enumtbl end
    -- DECOMPILER ERROR: 3 unprocessed JMP targets
end
...
List2Table = function(list)
    -- function num : 0_9 , upvalues : _ENV
    local tab = {}
    for i = 0, list.Count - 1 do (table.insert)(tab, list[i]) end
    return tab
end
...
Dictionary2Table = function(dic)
    -- function num : 0_10 , upvalues : _ENV
    local tab = {}
    for k, v in pairs(dic) do tab[k] = v end
    return tab
end
...
math.clamp = function(v, minValue, maxValue)
    ...
    if maxValue < v then return maxValue end
    return v
end
