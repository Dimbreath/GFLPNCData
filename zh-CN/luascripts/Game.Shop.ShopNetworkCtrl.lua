...
ShopNetworkCtrl.InitNetwork = function(self)
    -- function num : 0_1 , upvalues : _ENV
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STORE_Detail, self,
                         proto_csmsg.SC_STORE_Detail, self.SC_STORE_Detail)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STORE_Purchase, self,
                         proto_csmsg.SC_STORE_Purchase, self.SC_STORE_Purchase)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STORE_Fresh, self,
                         proto_csmsg.SC_STORE_Fresh, self.SC_STORE_Fresh)
end
...
ShopNetworkCtrl.CS_STORE_Detail = function(self, storeId, callback)
    ...
    msg.storeId = storeId
    self.lastStoreId = storeId
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STORE_Detail,
                 proto_csmsg.CS_STORE_Detail, msg)
    cs_WaitNetworkResponse:StartWait(
        proto_csmsg_MSG_ID.MSG_CS_STORE_Detail * 10 + storeId, callback,
        proto_csmsg_MSG_ID.MSG_SC_STORE_Detail)
end
    ...
    do
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            ...
            cs_WaitNetworkResponse:RemoveWait(
                proto_csmsg_MSG_ID.MSG_CS_STORE_Detail)
            return
        end
        if msg.data == nil then
            local storeId = self.lastStoreId
            cs_WaitNetworkResponse:AddWaitData(
                proto_csmsg_MSG_ID.MSG_CS_STORE_Detail * 10 + storeId, msg.data)
        else
            do
                cs_WaitNetworkResponse:AddWaitData(
                    proto_csmsg_MSG_ID.MSG_CS_STORE_Detail * 10 +
                        (msg.data).storeId, msg.data)
            end
        end
    end
end

ShopNetworkCtrl.CS_STORE_Purchase = function(self, storeId, shelfId, cnt,
                                             callback)
    -- function num : 0_4 , upvalues : _ENV, cs_WaitNetworkResponse
    local msg = {}
    msg.storeId = storeId
    ...
    msg.cnt = cnt;
    (table.insert)(self.lastBuyShop, storeId)
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STORE_Purchase,
                 proto_csmsg.CS_STORE_Purchase, msg)
    cs_WaitNetworkResponse:StartWait(proto_csmsg_MSG_ID.MSG_CS_STORE_Purchase,
                                     callback,
                                     proto_csmsg_MSG_ID.MSG_SC_STORE_Purchase)
end
    ...
    if msg.ret ~= proto_csmsg_ErrorCode.None then
        do
            if msg.ret == proto_csmsg_ErrorCode.BACKPACK_ITEM_OVERFLOW then
                local storeId = (table.remove)(self.lastBuyShop, 1)
                cs_WaitNetworkResponse:RemoveWait(
                    proto_csmsg_MSG_ID.MSG_CS_STORE_Purchase);
            ...
            do
                ...
                do return end
                local storeId = (table.remove)(self.lastBuyShop, 1);
                (ControllerManager:GetController(ControllerTypeId.Shop, false)):RefreshGoodsData(
                    msg.data, storeId)
            end
        end
    end
...
ShopNetworkCtrl.CS_STORE_Fresh = function(self, storeId, callback)
    ...
    local msg = {}
    msg.storeId = storeId
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STORE_Fresh,
                 proto_csmsg.CS_STORE_Fresh, msg)
    cs_WaitNetworkResponse:StartWait(proto_csmsg_MSG_ID.MSG_CS_STORE_Fresh,
                                     callback,
                                     proto_csmsg_MSG_ID.MSG_SC_STORE_Fresh)
end
...
ShopNetworkCtrl.SC_STORE_Fresh = function(self, msg)
    ...
    do
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            ...
            if isGameDev then (cs_MessageCommon.ShowMessageTips)(err) end
            cs_WaitNetworkResponse:RemoveWait(
                proto_csmsg_MSG_ID.MSG_CS_STORE_Fresh)
            return
        end
        (ControllerManager:GetController(ControllerTypeId.Shop, false)):RefreshShopData(
            msg.data)
    end
end
