...
local UINCSUpgradeMasterNode = require(
                                   "Game.CommanderSkill.UI.UINCSUpgradeMasterNode")
UINCSInfo.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_TreeLevel, self,
                               self.OnClickAddExperience)
...
UINCSInfo.ShowTreeInfo = function(self, treeData)
    ...
    self.treeData = treeData -- DECOMPILER ERROR at PC5: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = treeData:GetName() -- DECOMPILER ERROR at PC10: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ...
    if treeData.level == self.maxLevel then
        ...
        local masterLevel =
            (PlayerDataCenter.CommanderSkillModualData).masterLevel
        ...
        if (ConfigData.commander_skill_master_level)[masterLevel] ~= nil then
            maxExp =
                ((ConfigData.commander_skill_master_level)[masterLevel]).exp
        ...
        (self.upgradeMasterNode):RefreshUpgrade(treeData.treeId, masterLevel,
                                                masterExp)
    else
        do
            ...
            if (ConfigData.commander_skill_level)[treeData.treeId] ~= nil then
                maxExp =
                    ((ConfigData.commander_skill_level)[treeData.level]).exp
            end -- DECOMPILER ERROR at PC83: Confused about usage of register: R3 in 'UnsetPending'
...
UINCSInfo.ShowLevel = function(self, level)
    ...
    if level > 9 then
        (((self.ui).tex_Trim).gameObject):SetActive(false) -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_Level).text = tostring(level)
    else

        (((self.ui).tex_Trim).gameObject):SetActive(true) -- DECOMPILER ERROR at PC26: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_Level).text = tostring(level)
    end
end
