...
UIAchivUnitItem.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_GetReward, self,
                               self.OnClickGetAchivReward);
    (UIUtil.AddButtonListener)((self.ui).btn_jump2Finish, self,
                               self.OnClickJump2Finish);
...
UIAchivUnitItem.InitAchivUnitItem = function(self, taskData, stcData, resLoader,
                                             getRewardEvent, completedTimeStamp)
    -- function num : 0_1 , upvalues : _ENV, TaskEnum
    self.taskData = taskData
    self.stcData = stcData
    self.id = stcData.id
    self.getRewardEvent = getRewardEvent -- DECOMPILER ERROR at PC11: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).tex_Title).text = (LanguageUtil.GetLocaleText)(stcData.name)
    local path = PathConsts:GetAchievementIconPath(stcData.icon)
    local icon = resLoader:LoadABAsset(path) -- DECOMPILER ERROR at PC21: Confused about usage of register: R8 in 'UnsetPending'
    ;
    ((self.ui).img_Icon).texture = icon
    local stepCfg = ((ConfigData.taskStep)[stcData.id])[1] -- DECOMPILER ERROR at PC33: Confused about usage of register: R9 in 'UnsetPending'
    ;
    ...
    if taskData == nil then
        ...
        if taskData:CheckComplete() then
            ...
            (((self.ui).btn_jump2Finish).gameObject):SetActive(false);
            ((self.ui).notFinish):SetActive(true)
            if ((ConfigData.task)[self.id]).jump_id ~= nil and
                ((ConfigData.task)[self.id]).jump_id ~= 0 then
                ((self.ui).notFinish):SetActive(false);
    ...
    if self.state == (TaskEnum.eTaskState).Picked then
        ...
        local cur = (math.min)(step.schedule, step.aim) -- DECOMPILER ERROR at PC201: Confused about usage of register: R11 in 'UnsetPending'
        ;
        ((self.ui).img_Fill).fillAmount = cur / step.aim;
        ((self.ui).tex_Process):SetIndex(0, tostring(cur), tostring(step.aim))
    end
    do
        local completeDate = self.state == (TaskEnum.eTaskState).Picked and
                                 (os.date)("%Y-%m-%d %H:%M", completedTimeStamp) or
            ...
            if self.taskData == nil then
                (cs_MessageCommon.ShowMessageTips)(
                    ConfigData:GetTipContent(
                        TipContent.achievement_taskCompleted))
            else
