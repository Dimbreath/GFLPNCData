...
PersistentManager.InitManager = function(self)
    ...
    resloader:Put2Pool()
    self.DataModels = {}
    cs_MonoDriver:SetOnApplicationPause(
        BindCallback(self, self.SaveAllModelData))
end
...
PersistentManager.__encodeAndSaveData = function(self, packageId)
    ...
    if dataTable == nil then
        if dataConfig.DataType == (PstConfig.DataType).User and self.playerId ==
            nil then return end
    ...
    local filePath = self:GetDataFilePath(dataConfig.DataType,
                                          dataConfig.FileName)
    if (string.IsNullOrEmpty)(filePath) == nil then return end
...
PersistentManager.LoadAndDecodeData = function(self, packageId)
    ...
    do
        if content ~= nil then
            local status, data = xpcall(DecodeData, debug.traceback,
                                        dataConfig.PbName, content)
            if status then
...
PersistentManager.GetDataFilePath = function(self, dataType, fileName)
    ...
    if dataType == (PstConfig.DataType).User and self.playerId ~= nil and
        self.playerId == PlayerDataCenter.playerId then
        return
            PathConsts:GetPersistentUserDataPath(PlayerDataCenter.strPlayerId)
    end
...
PersistentManager.LoadAllModelData = function(self)
    ...
    for _, v in ipairs(PstConfig.ModelSaveList) do
        if v ~= nil then self:LoadAndDecodeData(v) end
    end
end
...
PersistentManager.SaveAllModelData = function(self)
    ...
    for _, v in ipairs(PstConfig.ModelSaveList) do
        if v ~= nil then self:__encodeAndSaveData(v) end
    end
end
...
PersistentManager.GetDataModelWithConfig =
    function(self, packageId, dataConfig)
        -- function num : 0_9 , upvalues : PstConfig
        if (self.DataModels)[packageId] == nil then
            if dataConfig.DataType == (PstConfig.DataType).User and
                self.playerId == nil then return end
            local dataModel = ((dataConfig.DataModle).New)() -- DECOMPILER ERROR at PC17: Confused about usage of register: R4 in 'UnsetPending'
            ;
            (self.DataModels)[packageId] = dataModel
        end
        do return (self.DataModels)[packageId] end
    end
...
PersistentManager.GetDataModel = function(self, packageId)
    ...
    if (self.DataModels)[packageId] == nil then
        if dataConfig.DataType == (PstConfig.DataType).User and self.playerId ==
            nil then return end
        local dataModel = ((dataConfig.DataModle).New)() -- DECOMPILER ERROR at PC27: Confused about usage of register: R4 in 'UnsetPending'
        ;
        (self.DataModels)[packageId] = dataModel
    end
    do
        if (self.DataModels)[packageId] == nil then
            error("Fail to get model data,model id:" ..
                      tostring((PstConfig.ePackage).UserData))
        end
