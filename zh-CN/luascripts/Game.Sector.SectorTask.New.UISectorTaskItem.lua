...
UISectorTaskItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).Btn_TaskItem, self,
                               self.__OnTaskItemClick);
    ((self.ui).obj_ResNode):SetActive(false)
    self.textUIState = {
        [1] = (self.ui).obj_Picked,
        [2] = ((self.ui).tex_InProgress).gameObject,
        [3] = (self.ui).obj_Completed
    }
...
UISectorTaskItem.RefreshSectorTaskItem =
    function(self, taskData, onClickAction)
        -- function num : 0_1
        self.taskData = taskData
        self.stcData = taskData.stcData
        self.state = taskData.state
...
UISectorTaskItem.__RefreshUI = function(self)
    -- function num : 0_3 , upvalues : _ENV, TaskEnum
    for _, go in pairs(self.textUIState) do go:SetActive(false) end
    (((self.ui).rect_Process).gameObject):SetActive(false) -- DECOMPILER ERROR at PC21: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_Intro).text = (LanguageUtil.GetLocaleText)(self.intro)
    self:__ShowRewardUI()
    if self.state == (TaskEnum.eTaskState).Picked then
...
UISectorTaskItem.__SetProcessUI = function(self)
    ...
    (((self.ui).rect_Process).gameObject):SetActive(true)
    local rate = (math.min)((self.stepCfg).schedule, (self.stepCfg).aim) /
                     (self.stepCfg).aim
    local sizeDelta = {}
    sizeDelta.x = rate * (self.originSizeDelta).x
    sizeDelta.y = (self.originSizeDelta).y -- DECOMPILER ERROR at PC33: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).rect_Process).sizeDelta = sizeDelta
end
...
UISectorTaskItem.__SetInProgressUI = function(self)
    ...
    ((self.ui).img_State):SetIndex(0);
    (((self.ui).tex_InProgress).gameObject):SetActive(true);
    ((self.ui).tex_InProgress):SetIndex(0, tostring(
                                            (math.min)((self.stepCfg).schedule,
                                                       (self.stepCfg).aim)),
                                        tostring((self.stepCfg).aim))
end
...
UISectorTaskItem.__ShowRewardUI = function(self)
    ...
    for i = 1, #(self.stcData).rewardIds do
        ...
        local rewardNum = ((self.stcData).rewardNums)[i]
        local itemCfg = (ConfigData.item)[rewardId];
        ((self.ui).obj_ResNode):SetActive(true) -- DECOMPILER ERROR at PC30: Confused about usage of register: R8 in 'UnsetPending'
        ;
        ((self.ui).img_Icon).sprite = CRH:GetSprite(itemCfg.icon);
        ((self.ui).tex_Count):SetIndex(0, tostring(rewardNum))
    end
