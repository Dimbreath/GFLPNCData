...
-- params : ...
-- function num : 0 , upvalues : _ENV
local ExplorationTreasureCtrl = class("ExplorationTreasureCtrl",
                                      ExplorationCtrlBase)
local cs_MessageCommon = CS.MessageCommon
local ExplorationEnum = require("Game.Exploration.ExplorationEnum")
local MAX_TREASURE_LOCK_NUM = 2
ExplorationTreasureCtrl.ctor = function(self, epCtrl)
    ...
    self.uiWindow = nil
    self.__OnTreasureRoomUpdate = BindCallback(self, self.__RefreshRoomData)
    MsgCenter:AddListener(eMsgEventId.OnTreasureRoomUpdate,
                          self.__OnTreasureRoomUpdate)
end

ExplorationTreasureCtrl.OnTreasureRoomOpen =
    function(self, roomData, isFirstOpen)
        -- function num : 0_1 , upvalues : _ENV
        if not isFirstOpen then isFirstOpen = false end
        ...
        do
            ...
            (self.treasureDataDic)[treasureData.boxId] = roomData
            UIManager:ShowWindowAsync(UIWindowTypeID.EpTreasureRoom,
                                      function(window)
                -- function num : 0_1_0 , upvalues : self, roomData, isFirstOpen
...
ExplorationTreasureCtrl.__RefreshRoomData =
...
ExplorationTreasureCtrl.__GetCfg = function(self, boxId)
    -- function num : 0_3 , upvalues : _ENV
    self.cfgTable = {[1] = {}, [2] = ConfigData.treasure_logic2}
    local treasureCfg = (ConfigData.exploration_treasure)[boxId]
    return ((self.cfgTable)[treasureCfg.logic])[treasureCfg.treasure_id]
end

ExplorationTreasureCtrl.SendItemSelect =
    function(self, idx, action)
        -- function num : 0_4 , upvalues : _ENV, ExplorationEnum
        (self.netWork):CS_EXPLORATION_ITEM_Select(self.currPosition, idx,
                                                  function()
            -- function num : 0_4_0 , upvalues : action, self, _ENV, ExplorationEnum
            ...
            MsgCenter:Broadcast(eMsgEventId.OnExitRoomComplete,
                                (ExplorationEnum.eExitRoomCompleteType).TreasureRoom)
        end)
    end

ExplorationTreasureCtrl.SendTreasureRoomQuit =
    function(self)
        -- function num : 0_5
        (self.netWork):CS_EXPLORATION_ITEM_Quit(self.currPosition)
    end

ExplorationTreasureCtrl.OnTreasureRoomQuit =
    function(self)
        -- function num : 0_6 , upvalues : _ENV, ExplorationEnum
        if self.uiWindow ~= nil then
            (self.uiWindow):Delete()
...
ExplorationTreasureCtrl.SendItemLockOrUnlock =
        ...
        if not self.curlockState and self.lockNum == MAX_TREASURE_LOCK_NUM then
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.exploration_Treasure_AlertNotSelectAllReward))
            return
        end
        local idx = chipItem.idx;
        (self.netWork):CS_EXPLORATION_ITEM_LockUnlock(self.currPosition, idx)
...
ExplorationTreasureCtrl.OnItemLockOrUnlock =
    function(self)
        -- function num : 0_8
        if self.curlockState then
            self.lockNum = self.lockNum - 1 -- DECOMPILER ERROR at PC7: Confused about usage of register: R1 in 'UnsetPending'
            ;
            (self.curlockItem).lockState = false
        else
            self.lockNum = self.lockNum + 1 -- DECOMPILER ERROR at PC13: Confused about usage of register: R1 in 'UnsetPending'
            ;
            (self.curlockItem).lockState = true
        end
        (self.curlockItem):ChangelockUi()
...
ExplorationTreasureCtrl.SendRefreshTreasureRoom =
    function(self, boxId)
        -- function num : 0_9 , upvalues : _ENV, cs_MessageCommon
        local roomData = (self.treasureDataDic)[boxId]
        if roomData == nil then
            error(
                "ExplorationTreasureCtrl:SendRefreshTreasureRoom error: boxId is nil" ..
                    tostring(boxId))
            return
        end
        local refreshTime =
            ((roomData.cfg).times)[(roomData.treasureData).freshCnt + 1]
        if refreshTime ~= nil or refreshTime == -1 then
            if roomData.refreshCostNum <= (self.dynPlayer):GetMoneyCount() then
                (self.netWork):CS_EXPLORATION_ITEM_Fresh(self.currPosition)
            else
            ...
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.exploration_Treasure_RefreshNumInsufficient))
        end
    end

ExplorationTreasureCtrl.OnRefreshTreasureRoom =
...
ExplorationTreasureCtrl.OnDelete = function(self)
    -- function num : 0_11 , upvalues : _ENV
    MsgCenter:RemoveListener(eMsgEventId.OnTreasureRoomUpdate,
                             self.__OnTreasureRoomUpdate)
    self.uiWindow = nil
end
