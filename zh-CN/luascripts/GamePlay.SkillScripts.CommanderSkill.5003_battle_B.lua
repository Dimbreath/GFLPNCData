...
bs_5003.SkillEventFunc = function(self, effect, eventId, target)
  ...
  if eventId == eBattleEffectEvent.Trigger then
    ...
    if highAttRole ~= nil and highAttRole.Count > 0 and highAttRole[0] ~= nil then
      ...
      local hurt = ((highAttRole[0]).targetRole).pow * (self.arglist)[1] * (1000 + exhurt + (self.arglist)[2]) // 1000 // 1000
      if hurt <= 0 then
        hurt = 1
      end
...
bs_5003.SkillEventFunc = function(self, effect, eventId, target)
    ...
    if eventId == eBattleEffectEvent.Trigger then
        ...
        if highAttRole ~= nil and highAttRole.Count > 0 and highAttRole[0] ~=
            nil then
            local exhurt = (target.targetRole).hp * 100 //
                               (target.targetRole).maxHp *
                               ((self.arglist)[3] - (self.arglist)[2]) * 100 //
                               100 // 100
            local hurt = ((highAttRole[0]).targetRole).pow * (self.arglist)[1] *
                             (1000 + exhurt + (self.arglist)[2]) // 1000 // 1000
