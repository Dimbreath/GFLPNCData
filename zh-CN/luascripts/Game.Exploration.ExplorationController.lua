...
local ExplorationPlayerCtrl = require(
                                  "Game.Exploration.Ctrl.ExplorationPlayerCtrl")
local ExplorationMapCtrl = require("Game.Exploration.Ctrl.ExplorationMapCtrl")
local ExplorationTreasureCtrl = require(
                                    "Game.Exploration.Ctrl.ExplorationTreasureCtrl")
local ExplorationStoreCtrl = require(
                                 "Game.Exploration.Ctrl.ExplorationStoreCtrl")
local ExplorationResidentStoreCtrl = require(
                                         "Game.Exploration.Ctrl.ExplorationResidentStoreCtrl")
local ExplorationRecoveryCtrl = require(
                                    "Game.Exploration.Ctrl.ExplorationRecoveryCtrl")
local ExplorationEventCtrl = require(
                                 "Game.Exploration.Ctrl.ExplorationEventCtrl")
local ExplorationResetRoomCtrl = require(
                                     "Game.Exploration.Ctrl.ExplorationResetRoomCtrl")
local ExplorationSceneCtrl = require(
                                 "Game.Exploration.Ctrl.ExplorationSceneCtrl")
local ExplorationBattleCtrl = require(
                                  "Game.Exploration.Ctrl.ExplorationBattleCtrl")
...
ExplorationController.ctor = function(self, mapData, dynPlayer)
    -- function num : 0_0 , upvalues : ExplorationPlayerCtrl, ExplorationMapCtrl, ExplorationTreasureCtrl, ExplorationStoreCtrl, ExplorationRecoveryCtrl, ExplorationEventCtrl, ExplorationResetRoomCtrl, ExplorationSceneCtrl, ExplorationBattleCtrl, ExplorationResidentStoreCtrl, _ENV
    self.ctrls = {}
    ...
    self.recoveryCtrl = (ExplorationRecoveryCtrl.New)(self)
    ...
    MsgCenter:AddListener(eMsgEventId.OnEpGridDetailDiff,
                          self.__onUpdateNextRoomInfo)
    ...
    self.__onEnterEpSceneCompleteAction =
        BindCallback(self, self.__OnEnterEpSceneComplete)
end

ExplorationController.RecordLastSelectChoiceCtrl =
    function(self, roomCtrl)
        -- function num : 0_1
        self.__lastSelectChoiceCtrl = roomCtrl
    end

ExplorationController.GetLastSelectChoiceCtrl =
    function(self)
        -- function num : 0_2
...
ExplorationController.Start = function(self, isReconnect)
    ...
    (self.sceneCtrl):FirstEnterScene(
        BindCallback(self, self.OnSceneLoadComplete),
        BindCallback(self, self.OnStartTimelineComplete))
end

ExplorationController.OnSceneLoadComplete =
    function(self)
        -- function num : 0_4 , upvalues : _ENV
        self.loadSceneComplete = true
        AudioManager:SetSourceSelectorLabel(eAudioSourceType.BgmSource,
                                            (eAuSelct.Sector).name,
                                            (eAuSelct.Sector).normalCombat)
...
ExplorationController.OnStartTimelineComplete =
    function(self)
        -- function num : 0_5
        self:ContinueExploration()
    end

ExplorationController.ContinueExploration =
    function(self)
        ...
        if opState ==
            ...
            if opState ==
                ...
                if opState ==
                    proto_object_ExplorationCurGridState.ExplorationCurGridStateBattleALGWaiting then
                    self:__EnterExplorationScene(ePlayerState.SelectingChip)
                else
                    if opState ==
                        proto_object_ExplorationCurGridState.ExplorationCurGridState_DropAlg then
                        self:__EnterExplorationScene(ePlayerState.DropChip)
                    else
                        if opState ==
                            proto_object_ExplorationCurGridState.ExplorationCurGridState_HalfOver then
...
ExplorationController.__EnterExplorationScene =
    function(self, state)
        -- function num : 0_7 , upvalues : _ENV
        AudioManager:SetSourceSelectorLabel(eAudioSourceType.BgmSource,
                                            (eAuSelct.Sector).name,
                                            (eAuSelct.Sector).roomSelect)
        ...
        if (self.sceneCtrl).inBattleScene then
            ...
            (self.sceneCtrl):BattleToExplorationScene(
                self.__onEnterEpSceneCompleteAction)
        else
            do self:__OnEnterEpSceneComplete() end
        end
    end

ExplorationController.__OnEnterEpSceneComplete =
    function(self)
            ...
            if self.__playerState == ePlayerState.OutsideTheRoom then
                ...
                if self.__playerState == ePlayerState.SelectingChip then
                    self:CheckChipSelect(
                        BindCallback(self, self.__ContinueSelectChipComplete))
                    GuideManager:TryTriggerGuide(
                        eGuideCondition.InExplorationScene)
                else
...
ExplorationController.__ContinueSelectChipComplete =
    function(self)
        -- function num : 0_9 , upvalues : _ENV
        if (self.dynPlayer):GetOperatorDetailState() ~=
            proto_object_ExplorationCurGridState.ExplorationCurGridState_DropAlg then
...
ExplorationController.__CheckBossRoom = function(self)
    ...
    if currentRoom:IsRealBossRoom() then
        if (self.mapData):HasOverBossRoom() then
            ((CS.MessageCommon).ShowMessageBox)(ConfigData:GetTipContent(255),
                                                function()
                -- function num : 0_10_0
            end, function()
...
ExplorationController.StartCompleteExploration =
...
ExplorationController.__CompleteExplorationFloor =
    function(self, endAction)
        -- function num : 0_12 , upvalues : _ENV
        UIManager:ShowWindowAsync(UIWindowTypeID.ExplorationResult,
                                  function(window)
            -- function num : 0_12_0 , upvalues : self, _ENV, endAction
            if window ~= nil then window:CompleteExplorationFloor() end
...
ExplorationController.__CompleteExploration =
    function(self, endAction)
        -- function num : 0_13 , upvalues : _ENV
        UIManager:ShowWindowAsync(UIWindowTypeID.ExplorationResult,
                                  function(window)
            -- function num : 0_13_0 , upvalues : self, _ENV, endAction
            if window ~= nil then window:CompleteExploration() end
            if self.mapCtrl ~= nil then
                (self.mapCtrl):HideMapCavasWithoutBg()
            end
            UIManager:HideWindow(UIWindowTypeID.DungeonStateInfo)
...
ExplorationController.__EnterBattleScene =
    function(self, roomData, isReconnect)
        -- function num : 0_14 , upvalues : _ENV, ExplorationEnum, CS_BattleManager_Ins
        if isGameDev then print("[Dev]battleId:", roomData.battleId) end
        if roomData:IsBossRoom() or roomData:GetRoomType() ==
            (ExplorationEnum.eRoomType).challenge then
            AudioManager:SetSourceSelectorLabel(eAudioSourceType.BgmSource,
                                                (eAuSelct.Sector).name,
                                                (eAuSelct.Sector).bossCombat)
        else
            AudioManager:SetSourceSelectorLabel(eAudioSourceType.BgmSource,
                                                (eAuSelct.Sector).name,
                                                (eAuSelct.Sector).normalCombat)
        ...
        if window ~= nil then window:Hide(true) end
        window = UIManager:GetWindow(UIWindowTypeID.Exploration)
        if window ~= nil then window:Hide(true) end
        (self.battleCtrl):OnEnterBattleScene(roomData)
        local battleCtrl = CS_BattleManager_Ins:StartNewBattle(roomData,
                                                               self.dynPlayer,
                                                               self.battleCtrl)
        ...
        if roomData:IsDeployRoom() or isReconnect then
            (self.sceneCtrl):ExplorationToBattleSceneInReconnected(
                epToBattleAction)
        else
            local epDetail = (self.dynPlayer):GetOperatorDetail()
            if epDetail.state ~=
                proto_object_ExplorationCurGridState.ExplorationCurGridState_Over or
                not #epDetail.path - 1 then
                local index = #epDetail.path
            end
            local lastRoomCoord = (epDetail.path)[index]
            local lastRoomPos =
                ((self.mapCtrl):GetRoomEntity(lastRoomCoord)):GetRoomEntityLocalPos(
                    0)
            local curRoomPos =
                ((self.mapCtrl):GetRoomEntity(roomData.position)):GetRoomEntityLocalPos(
                    0)
            local isUp = lastRoomPos.y < curRoomPos.y
            local roomPos = (self.mapCtrl):GetRoomEntityPos(roomData)
            local mapRoot = self:GetRoomRoot();
            (self.sceneCtrl):ExplorationToBattleSceneNormal(isUp, mapRoot,
                                                            roomPos,
                                                            epToBattleAction)
        end
...
ExplorationController.GenExplorationMap =
    function(self)
        -- function num : 0_16
        (self.mapCtrl):GenMap(self.mapData, self:GetCurrentRoomData())
    end

ExplorationController.SetViewPositionOffset =
    function(self, offsetPosition)
        -- function num : 0_17
...
ExplorationController.OnPlayerMoveStart =
...
ExplorationController.OnPlayerMoveComplete =
    function(self, roomData)
            ...
            if roomType == (ExplorationEnum.eRoomType).chip then
                ...
                if roomType == (ExplorationEnum.eRoomType).store then
                    ...
                    if roomType == (ExplorationEnum.eRoomType).item then
                        (self.treasureCtrl):OnTreasureRoomOpen(roomData)
                    else
                        if roomType == (ExplorationEnum.eRoomType).recovery then
                            (self.recoveryCtrl):OnRecoveryRoomOpen(roomData)
                        else
                            if roomType == (ExplorationEnum.eRoomType).eventroom then
                                (self.eventCtrl):OnEventRoomOpen(roomData)
                            else
                            end
                        end
                    end
        ...
        if roomType ~= (ExplorationEnum.eRoomType).empty or
            (ExplorationEnum.eRoomType).start < roomType then
...
ExplorationController.OnExitEpRoom = function(self)
    ...
    local curRoomData = self:GetCurrentRoomData()
    if curRoomData:IsEndColRoom() then return end
...
ExplorationController.UpdateNextRoomInfo =
    function(self, epGrid)
        -- function num : 0_22 , upvalues : _ENV
        if epGrid.monster ~= nil then
            for k, v in pairs(epGrid.monster) do
                local roomData = (self.mapData):GetRoomByCoord(k)
                if roomData == nil then
                    error("Can\'t find roomData by pos: " .. tostring(x) ..
                              " , " .. tostring(y))
                else
                    roomData:InitMonsterData(v)
                    roomData:ExecuteMonsterChip(self.dynPlayer)
                    roomData:ExecuteMonsterTempChip(
                        (self.dynPlayer):GetHiddenChipDic());
                    (self.mapCtrl):ShowFightingPower(roomData,
                                                     (self.dynPlayer):GetCacheFightPower())
            ...
            do
                if epGrid.store ~= nil then
                    for k, v in pairs(epGrid.store) do
                        ...
                        if roomData == nil then
                            error(
                                "Can\'t find roomData by pos: " .. tostring(x) ..
                                    " , " .. tostring(y))
                        else
                ...
                do
                    if epGrid.evt ~= nil then
                        for k, v in pairs(epGrid.evt) do
                            local roomData = (self.mapData):GetRoomByCoord(k)
                            if roomData == nil then
                                error("Can\'t find roomData by pos: " ..
                    ...
                    do
                        if epGrid.reconstitution ~= nil then
                            for k, v in pairs(epGrid.reconstitution) do
                                local roomData =
                                    (self.mapData):GetRoomByCoord(k)
                                if roomData == nil then
                                    error(
                                        "Can\'t find roomData by pos: " ..
                                            tostring(x) .. " , " .. tostring(y))
                                else
                        ...
                        do
                            if epGrid.buffData ~= nil then
                                ...
                                if curRoomData ~= nil then
                                    curRoomData:InitEpBuffEffective(
                                        (epGrid.buffData).data)
                                end
                            end
                        end
                    end
...
ExplorationController.UpdateResidentDetail =
    function(self, epResident)
        -- function num : 0_23
...
ExplorationController.OpenResidentStore =
    function(self)
        -- function num : 0_24
...
ExplorationController.CheckChipSelect = function(self, noSelectEvent,
                                                 toFakeCamera)
    -- function num : 0_26 , upvalues : _ENV
    if noSelectEvent ~= nil then self.__noChipSelectEvent = noSelectEvent end
    if (self.dynPlayer):GetOperatorDetailState() ~=
        proto_object_ExplorationCurGridState.ExplorationCurGridStateBattleALGWaiting then
    ...
    local window = UIManager:ShowWindow(UIWindowTypeID.SelectChip)
    window:InitSelectChip(chipList, self.dynPlayer,
                          BindCallback(self, self.__SelectChipComplete),
                          toFakeCamera)
    return true
end

ExplorationController.__SelectChipComplete =
    function(self, index, selectComplete)
        -- function num : 0_27
        index = index - 1
        local position = (self:GetCurrentRoomData()):GetRoomPosition();
        (self.epNetwork):CS_EXPLORATION_BATTLE_ALGSelect(position, index,
                                                         function(dataList)
            -- function num : 0_27_0 , upvalues : self, selectComplete
            self:CheckChipSelect()
            if selectComplete ~= nil then selectComplete() end
        end)
    end
...
ExplorationController.DiscardChip = function(self)
    ...
    if win ~= nil and win.active then return end
    (UIManager:ShowWindow(UIWindowTypeID.ClickContinue)):InitContinue(nil, nil,
                                                                      nil,
                                                                      Color.clear,
                                                                      false)
...
ExplorationController.GetCurrentRoomData =
    function(self)
        -- function num : 0_31
...
ExplorationController.GetCurrentRoomTitle =
    function(self)
...
ExplorationController.ExecuteNormalChipBattleRoom =
    function(self)
...
ExplorationController.RollbackNormalChipBattleRoom =
    function(self)
...
ExplorationController.ExecuteTempChipCurBattleRoom =
    function(self, chipTemporaryDic)
        -- function num : 0_35
        local curRoomData = self:GetCurrentRoomData()
        if curRoomData:IsBattleRoom() then
...
ExplorationController.RollbackTempChipCurBattleRoom =
    function(self, chipTemporaryDic)
        -- function num : 0_36
        local curRoomData = self:GetCurrentRoomData()
        if curRoomData:IsBattleRoom() then
...
ExplorationController.ExecuteTempChipNextBattleRoom =
    function(self, chipTemporaryDic)
...
ExplorationController.RollbackTempChipNextBattleRoom =
    function(self, chipTemporaryDic)
...
ExplorationController.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.OnEpGridDetailDiff,
                             self.__onUpdateNextRoomInfo)
    MsgCenter:RemoveListener(eMsgEventId.OnEpResidentDiff,
                             self.__onEpResidentDiff)
...
ExplorationController.EnterBattleSceneInReplay =
    function(self)
        ...
        UIManager:HideWindow(UIWindowTypeID.DungeonStateInfo)
        local battleCtrl = CS_BattleManager_Ins:StartNewBattle(
                               self.roomDataClone, self.dynPlayerClone,
                               self.battleCtrl)
        local epToBattleAction = function()
        ...
        local mapRoot = self:GetRoomRoot();
        (self.sceneCtrl):ExplorationToBattleSceneNormal(true, mapRoot, roomPos,
                                                        epToBattleAction)
    end

ExplorationController.CopyAndGetReplayData =
    function(self)
