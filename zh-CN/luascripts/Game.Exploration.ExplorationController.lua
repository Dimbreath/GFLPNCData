    ...
    function(self)
        ...
        if (self.dynPlayer):GetOperatorDetailState() ~=
            ...
            MsgCenter:Broadcast(eMsgEventId.OnExitRoomComplete,
                                (ExplorationEnum.eExitRoomCompleteType).OutsideSelectChip)
            self:__CheckBossRoom()
        end
    end
            ...
            if (self.autoCtrl):IsAutoModeRunning() then
                ...
                do
                    ...
                    (ControllerManager:GetController(ControllerTypeId.AvgPlay,
                                                     true)):TryPlayAvg(
                        eAvgTriggerType.MainAvgEp, stageId, curFloor, 3,
                        function()
                            -- function num : 0_10_0 , upvalues : _ENV, self
                            ((CS.MessageCommon).ShowMessageBox)(
                                ConfigData:GetTipContent(255), function()
                                    -- function num : 0_10_0_0
                                end, function()
                                    -- function num : 0_10_0_1 , upvalues : self
                                    self:StartCompleteExploration()
...
ExplorationController.__CompleteExplorationFloor =
    function(self, endAction)
        -- function num : 0_12 , upvalues : _ENV
        ExplorationManager:SendFloorSettle(
            function(msg)
                -- function num : 0_12_0 , upvalues : _ENV, self, endAction
                local data = {}
                if msg.Count < 1 then
...
ExplorationController.__CompleteExploration =
    function(self, endAction)
        -- function num : 0_13 , upvalues : _ENV
        local beforeSettleStageState =
            (PlayerDataCenter.sectorStage):GetStageState(
                (ExplorationManager:GetSectorStageCfg()).id)
        ExplorationManager:SendSettle(function(msg)
            -- function num : 0_13_0 , upvalues : _ENV, beforeSettleStageState, self, endAction
            local data = {}
            if msg.Count < 1 then
            ...
            local StageState = (PlayerDataCenter.sectorStage):GetStageState(
                                   (ExplorationManager:GetSectorStageCfg()).id)
            local needFirsPassReward = false
            if StageState ~= beforeSettleStageState and StageState ==
                proto_object_DungeonStageState.DungeonStageStatePicked then
                needFirsPassReward = true
            end
            UIManager:ShowWindowAsync(UIWindowTypeID.ExplorationResult,
                                      function(window)
