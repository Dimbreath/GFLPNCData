...
OasisController.EnterOasis = function(self)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.Oasis, function(window)
        ...
        window.AccelerateEvent = BindCallback(self, self.__SendConfimAcc)
...
OasisController.FocusBuildingTween = function(self, toBuilding, buildingItem)
    ...
    if toBuilding then
        ...
        (cs_CameraController.Instance):SetFocusCamPosition(
            (buildingItem.rootTransform).position, buildingItem:GetUnitySize())
        ...
        if self.focusDirectorStopped == nil then
            self.focusDirectorStopped = BindCallback(self, function(director)
                ...
                if bdItem.buildingData ~= nil and
                    (bdItem.buildingData).waitConfirmOver then
                    self:__SendConfimOver(bdItem.id)
                else
        ...
        for k, v in pairs(self.hidenBuildingItems) do
            v:Show(true) -- DECOMPILER ERROR at PC49: Confused about usage of register: R8 in 'UnsetPending'
            ;
...
OasisController.ConstructComplete = function(self, id)
    ...
    (cs_MessageCommon.ShowMessageTips)((string.format)(
                                           ...
                                           buildingData.name));
    (self.window):UpdateSidleUI(buildingData) -- DECOMPILER ERROR at PC20: Confused about usage of register: R3 in 'UnsetPending'
    ;
...
OasisController.UpgradeComplete = function(self, id)
    -- function num : 0_28 , upvalues : cs_MessageCommon, _ENV
    local builtData = ((self.buildingDatas).built)[id];
    (cs_MessageCommon.ShowMessageTips)((string.format)(
                                           ConfigData:GetTipContent(
    ...
    local buildingItem = (self.buildingItems)[id]
    local nextLevel = builtData.level + 1
    local prefabName = ((builtData.levelConfig)[nextLevel]).modol
...
OasisController.OnExitOasis = function(self)
    ...
    if self.lotteryWindow ~= nil then
        UIManager:DeleteWindow(UIWindowTypeID.LotteryWindow)
        self.lotteryWindow = nil
    end
