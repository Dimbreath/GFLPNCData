...
UISelectChip.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.AddButtonListener)((self.ui).btn_skip, self,
                               self.__OnClickSkipButton);
    ((self.ui).propChipItem):SetActive(false)
    self.__ShowDungeonState = BindCallback(self, self.ShowDungeonState)
    MsgCenter:AddListener(eMsgEventId.OnSettleMentTimeLinePlayToEnd,
                          self.__ShowDungeonState)
end

UISelectChip.InitSelectChip = function(self, chipDataList, dynPlayer,
                                       selectEvent, toFakeCamera)
    -- function num : 0_1 , upvalues : _ENV, UINChipDetailPanel
    self.toFakeCamera = toFakeCamera
    ...
    for k, chipData in ipairs(chipDataList) do
        local go = ((self.ui).propChipItem):Instantiate()
        local panel = (UINChipDetailPanel.New)()
        panel:Init(go) -- DECOMPILER ERROR at PC42: Confused about usage of register: R14 in 'UnsetPending'
        ;
        (panel.gameObject).name = tostring(k)
        panel:Show()
        panel:InitChipDetailPanel(k, chipData, dynPlayer, self.resloader,
                                  clickedPanelAction)
        local isNew = epChipIdDica[chipData.dataId] or false
        panel:SetObjNewTagActive(isNew);
        (table.insert)(self.chipPanelList, panel)
    ...
    if toFakeCamera then
        ...
        if GuideManager:TryTriggerGuide(eGuideCondition.InSelectChip) then
            for k, panelItem in pairs(self.chipPanelList) do
                local chipData = panelItem:GetChipDetailPanelData() -- DECOMPILER ERROR at PC99: Confused about usage of register: R13 in 'UnsetPending'
                ;
                (panelItem.gameObject).name = tostring(chipData.dataId)
            end
        end
    end
    do MsgCenter:Broadcast(eMsgEventId.DungeonHeroListActiveSet, false) end
end
...
UISelectChip.GetMaxPowerChipPanel = function(self)
    ...
    local powerChipPanel = nil
    for k, chipPanel in pairs(self.chipPanelList) do
        if fightPower < chipPanel.fightPower or chipPanel.fightPower ==
            fightPower and quality < chipPanel:GetChipQuality() then
            fightPower = chipPanel.fightPower
            quality = chipPanel:GetChipQuality()
            powerChipPanel = chipPanel
        end
...
UISelectChip.OnChipPanelClicked = function(self, chipPanel)
    -- function num : 0_3 , upvalues : _ENV
    if chipPanel == nil then return end
    local stateInfoWindow = UIManager:GetWindow(UIWindowTypeID.DungeonStateInfo)
    if stateInfoWindow == nil or
        not ((stateInfoWindow.chipList).gameObject).activeInHierarchy then
        return
    end
    if self.selectChip == chipPanel then
        self:ComfirmSelectChip(chipPanel)
        ...
        (stateInfoWindow.chipList):HighlightChipItem(
            chipPanel:GetChipDetailPanelData())
    end
    if self.selectChip ~= nil then
        (self.selectChip):OnSelectChipChanged(false)
    end
    ...
    self.selectChip = chipPanel
    for _, v in pairs(self.chipPanelList) do
        if v ~= self.selectChip then v:SetUnSelectAlpha() end
    end
    if self.ui ~= nil and ((self.ui).obj_TitleName).activeSelf and
        self.selectChip ~= nil then
        ((self.ui).obj_TitleName):SetActive(false)
    end
end
    ...
    if self.selectEvent ~= nil then
        (self.selectEvent)(index, function()
            ...
            if chipPanelItem ~= nil then
                local chipData = chipPanelItem:GetChipDetailPanelData()
                local chipItem = chipPanelItem:GetChipItem()
                local dungeonStateWindow =
                    UIManager:GetWindow(UIWindowTypeID.DungeonStateInfo)
                do
                    do
                        if dungeonStateWindow ~= nil then
                            local uiPos = nil
                            if self.toFakeCamera then
                                uiPos = UIManager:World2UIPosition(
                                            (chipItem.transform).position)
                                uiPos = (Vector3.New)(uiPos.x, uiPos.y, 0)
                            else
                                uiPos = (self.transform):InverseTransformPoint(
                                            (chipItem.transform).position)
                            end
                            dungeonStateWindow:ShowGetChipAni(chipData, uiPos,
                                                              (chipItem.transform).localScale)
                        end
                        (ExplorationManager:GetEpDataCenter()):AddNewChip(
                            chipData.dataId)
                        self:Delete()
                    end
                end
...
UISelectChip.__OnClickSkipButton = function(self)
    -- function num : 0_5 , upvalues : _ENV
    ((CS.MessageCommon).ShowMessageBox)(ConfigData:GetTipContent(1011),
                                        function()
        -- function num : 0_5_0 , upvalues : self
        do
            if self.selectEvent ~= nil then
...
UISelectChip.__ToFackCameraCanvas = function(self)
    ...
    local fakeCameraBattle =
        ((cs_GameObject.Find)("FakeCameraBattleChip")):FindComponent(
...
UISelectChip.ShowDungeonState = function(self)
    ...
    if GuideManager:TryTriggerGuide(eGuideCondition.InSelectChip) then
        for k, panelItem in pairs(self.chipPanelList) do
            local chipData = panelItem:GetChipDetailPanelData() -- DECOMPILER ERROR at PC32: Confused about usage of register: R8 in 'UnsetPending'
            ;
            (panelItem.gameObject).name = tostring(chipData.dataId)
        end
    end
...
UISelectChip.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.OnSettleMentTimeLinePlayToEnd,
                             self.__ShowDungeonState)
    local window = UIManager:GetWindow(UIWindowTypeID.DungeonStateInfo)
    if window ~= nil then (window.chipList):CancelHighlightChipItem() end
    (base.OnDelete)(self)
    if self.resloader ~= nil then
        (self.resloader):Put2Pool()
        self.resloader = nil
