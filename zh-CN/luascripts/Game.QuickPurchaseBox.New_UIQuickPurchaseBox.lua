...
New_UIQuickPurchaseBox.m_RefreshGoodUI = function(self, goodData)
  ...
  if goodData.isLimit and goodData.limitType ~= (ShopEnum.eLimitType).None then
...
New_UIQuickPurchaseBox.OnInit = function(self)
    ...
    (((self.ui).btn_Add).onPress):AddListener(
        BindCallback(self, self.OnPressAdd));
    (((self.ui).btn_Reduce).onPress):AddListener(
        BindCallback(self, self.OnPressMin));
    (((self.ui).tween_side).onRewind):AddListener(
        BindCallback(self, self.OnClickClose))
    self.itemWithCount = (UINBaseItemWithCount.New)();
    (self.itemWithCount):Init((self.ui).itemWithCount)
end
...
New_UIQuickPurchaseBox.InitBuyTarget = function(self, goodData,
                                                BuySuccessCallback, isNeedRes,
                                                resIdList)
    -- function num : 0_4
    self.goodData = goodData
    self.BuySuccessCallback = BuySuccessCallback
...
New_UIQuickPurchaseBox.m_RefreshGoodUI =
    function(self, goodData)
        ...
        (self.itemWithCount):InitItemWithCount(goodData.itemCfg,
                                               goodData.itemNum, nil) -- DECOMPILER ERROR at PC64: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)(
                                        (goodData.itemCfg).name) -- DECOMPILER ERROR at PC72: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_Detail).text = (LanguageUtil.GetLocaleText)(
                                          (goodData.itemCfg).describe) -- DECOMPILER ERROR at PC76: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_currPrice).text = goodData.newCurrencyNum
        local currencyItemCfg = (ConfigData.item)[goodData.currencyId]
        local smallIcon = currencyItemCfg.small_icon -- DECOMPILER ERROR at PC88: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_currencyIcon).sprite = CRH:GetSprite(smallIcon) -- DECOMPILER ERROR at PC95: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_totalCurrencyIcon).sprite = CRH:GetSprite(smallIcon)
        if goodData.discount == 100 then
            ((self.ui).obj_discount):SetActive(false);
            ...
            ((self.ui).tex_Discount):SetIndex(0,
                                              tostring(100 - goodData.discount)) -- DECOMPILER ERROR at PC123: Confused about usage of register: R4 in 'UnsetPending'
            ;
            ((self.ui).tex_oldPrice).text = goodData.oldCurrencyNum;
            ((self.ui).obj_discount):SetActive(true);
            (((self.ui).tex_oldPrice).gameObject):SetActive(true)
...
New_UIQuickPurchaseBox.m_CouldAdd = function(self, count)
    ...
    if (self.goodData).isLimit and (self.goodData).limitTime -
        (self.goodData).purchases < self.buyNum + count then return false end
    local totalMoney = PlayerDataCenter:GetItemCount((self.goodData).currencyId)
    local totalNeedMoney = (self.buyNum + count) *
                               (self.goodData).newCurrencyNum
    if totalMoney < totalNeedMoney then return false end
    return true
end
...
New_UIQuickPurchaseBox.Add2Max = function(self, maxLimit)
    ...
    local totalNeedMoney = (self.buyNum + 1) * (self.goodData).newCurrencyNum
    maxNum = (math.max)(
                 (math.floor)(totalMoney / (self.goodData).newCurrencyNum), 0)
    if (self.goodData).isLimit then
        maxNum = (math.min)((self.goodData).limitTime -
                                (self.goodData).purchases, maxNum)
    end
    if maxLimit then
        self.buyNum = (math.min)(maxNum, 99)
    else
...
New_UIQuickPurchaseBox.OnClickBuy = function(self)
    ...
    self._heroIdSnapShoot = PlayerDataCenter:GetHeroIdSnapShoot();
    (self.ctrl):ReqBuyGoods((self.goodData).shopId, (self.goodData).shelfId,
                            self.buyNum, function()
        -- function num : 0_12_0 , upvalues : _ENV, self
        UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
            -- function num : 0_12_0_0 , upvalues : self
            if window == nil then return end
            window:InitRewardsItem({(self.goodData).itemId},
                                   {self.buyNum * (self.goodData).itemNum},
...
New_UIQuickPurchaseBox.m_RefreshTotalMoney =
    function(self)
        -- function num : 0_14 , upvalues : _ENV
        if self.buyNum == 0 then
            (((self.ui).btn_Buy).gameObject):SetActive(false);
        ...
        ((self.ui).tex_buyCount).text = tostring(self.buyNum)
        local totalMoney = self.buyNum * (self.goodData).newCurrencyNum -- DECOMPILER ERROR at PC41: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_totalcurrPrice).text = tostring(totalMoney)
    end
