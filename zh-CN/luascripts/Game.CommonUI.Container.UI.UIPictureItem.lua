...
UIPictureItem.InitPictureItem = function(self, index, resPath, resloader, url)
    ...
    resloader:LoadABAssetAsync(resPath, function(picture)
        -- function num : 0_1_0 , upvalues : _ENV, self, index
        if IsNull(picture) then return end
        if self.index ~= index then return end -- DECOMPILER ERROR at PC12: Confused about usage of register: R1 in 'UnsetPending'
...
UIPictureItem.OnClick = function(self)
    ...
    ((CS.MessageCommon).ShowMessageBox)(ConfigData:GetTipContent(
                                            TipContent.bannerJumpMention),
                                        ConfigData:GetTipContent(
                                            TipContent.commonConfirm),
                                        ConfigData:GetTipContent(
                                            TipContent.commonCancle),
                                        self.__OnOpenUrl, nil)
end
