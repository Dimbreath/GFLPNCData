...
MailData.IsHaveAtt = function(self)
    ...
    if self.att ~= nil and (self.att).data ~= nil then
        if (table.count)((self.att).data) == 0 then return false end
        return true, (self.att).data,
               self:GetState() == (MailEnum.eMailDetailType).Received
    else
        return false
    end
...
MailData.GetTime = function(self, needRaw)
    ...
    if self.info ~= nil and (self.info).created ~= nil then
        if needRaw then return (self.info).created end
        if (self.info).YMD == nil or (self.info).HM == nil then
            local dataTable = TimestampToDate((self.info).created)
            local YMD = (string.format)("%d/%02d/%02d", dataTable.year,
                                        dataTable.month, dataTable.day)
            local HM = (string.format)("%02d:%02d", dataTable.hour,
                                       dataTable.min) -- DECOMPILER ERROR at PC38: Confused about usage of register: R5 in 'UnsetPending'
            ;
            (self.info).YMD = YMD -- DECOMPILER ERROR at PC40: Confused about usage of register: R5 in 'UnsetPending'
            ;
            (self.info).HM = HM
        end
        do
...
MailData.GetTimeBeforeExpired = function(self)
    ...
    return (math.ceil)(
               (math.max)(self.expiredTm - PlayerDataCenter.timestamp, 0))
end
