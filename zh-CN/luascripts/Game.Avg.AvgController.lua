...
AvgController.StartAvg = function(self, chapterName, avgId, completeFunc,
                                  shieldControlSwitch, isGuideAvg,
                                  ignoreTimeScale)
    -- function num : 0_1 , upvalues : _ENV, util
    if (string.IsNullOrEmpty)(chapterName) then
        error("Avg chapterName IsNullOrEmpty")
    ...
    (table.insert)(self.avgWaitingList, wating)
    if self.__playCoroutine == nil then
        self.__playCoroutine = (GR.StartCoroutine)(
                                   (util.cs_generator)(
                                       BindCallback(self, self.PlayCoroutineFunc)))
    end
end
...
AvgController.PlayCoroutineFunc = function(self)
    ...
    while 1 do
        while 1 do
            if #self.avgWaitingList > 0 then
                if UIManager:GetWindow(UIWindowTypeID.CommonReward) ~= nil or
                    UIManager:IsWindowInLoading(UIWindowTypeID.CommonReward) then
                    (coroutine.yield)(nil)
                    -- DECOMPILER ERROR at PC22: LeaveBlock: unexpected jumping out IF_THEN_STMT
        ...
        self.__waitAvgCompleteNetowrk = waiting.avgId ~= nil
        self:ShowAvg(waiting.chapterName, waiting.completeFunc,
                     waiting.shieldControlSwitch, waiting.ignoreTimeScale)
        while self.avgIsPlaying do (coroutine.yield)(nil) end
        while self.__waitAvgCompleteNetowrk do (coroutine.yield)(nil) end
        while UIManager:GetWindow(UIWindowTypeID.CommonReward) ~= nil or
            UIManager:IsWindowInLoading(UIWindowTypeID.CommonReward) do
            (coroutine.yield)(nil)
        end
        while waiting.isGuideAvg or GuideManager.inGuide do
            (coroutine.yield)(nil)
...
AvgController.ShowAvg = function(self, chapterName, completeFunc,
                                 shieldControlSwitch, ignoreTimeScale)
    -- function num : 0_3 , upvalues : _ENV
    if self.avgIsPlaying then
        if self.avgCachePlayParam ~= nil then
            error(
                "正在进行缓存播放模式,不可以直接播放avg,请先结束缓存播放模式(delete controller)")
            return
        end
        self:ClearCurAvg()
    end
    ...
    self.__originTimeScale = (Time.unity_time).timeScale
    self._hasError = nil
    if not self:LoadAvgCfg(chapterName) then return end
    self.__ignoreTimeScale = ignoreTimeScale or false
    self.__shieldControlSwitch = shieldControlSwitch or false
    self.chapterName = chapterName
    self:ChangeSpeed()
...
AvgController.PlayNextAvgAct = function(self)
    -- function num : 0_5
    if self.curActId == nil then return end
    local actCfg = (self.avgCfg)[self.curActId]
    if actCfg.isEnd then return end
    local nextActId = nil
    if actCfg.nextId == nil then
        nextActId = self.curActId + 1
    else
    ...
    if (self.avgCfg)[nextActId] == nil then return end
    self:PlayAvgAct(nextActId)
end

AvgController.ClickContinueAvg = function(self)
    -- function num : 0_6
    if self.autoPlayMode then self:ClearDelayPlatNextTimer() end
    if self:AvgIsEnd(self.curActId) then
        self:CompleteAllAvg()
        return
    end
...
AvgController.OnAvgActComplete =
    function(self, jumpAct, jumpChapter, contentLen)
        -- function num : 0_7 , upvalues : _ENV
        self.avgActComplete = true;
        (self.avgWindow):StopAvgVoice()
        local avgIsEnd = self:AvgIsEnd(self.curActId)
        if avgIsEnd and not self.autoPlayMode then return end
        if jumpAct ~= nil then
            if (self.avgCfg)[jumpAct] == nil then
                error("没有该分支,id = " .. tostring(jumpAct))
                return
            ...
            if not (string.IsNullOrEmpty)(jumpChapter) then
                ...
                if not self.autoPlayMode then return end
                if self.actDelayRatio ~= 1 or
                    not (ConfigData.buildinConfig).AvgActDelayFactor1 then
                    local ratio = (ConfigData.buildinConfig).AvgActDelayFactor2
                end
                local duration = (ConfigData.buildinConfig).AvgActDelayBase +
                                     contentLen * ratio
                self.__delayPlatNextTimer =
                    (TimerManager:GetTimer(duration, function()
                        -- function num : 0_7_0 , upvalues : self, avgIsEnd
                        self.__delayPlatNextTimer = nil
                        if avgIsEnd then
                            self:CompleteAllAvg()
                    ...
                    end, nil, true, false, self:AvgIgnoreTimeScale())):Start()
            end
        end
    end
...
AvgController.AvgIsEnd = function(self, actId)
    -- function num : 0_8
    local actCfg = self:GetAcgActCfg(actId)
    if actCfg.isEnd then return true end
    if actCfg.nextId ~= nil then return false end
    if (self.avgCfg)[actId + 1] == nil then return true end
    return false
end
...
AvgController.LoadAvgCfg = function(self, storyName)
    ...
    local lang = (CS_LanguageGlobal.GetLanguageStr)()
    local langPath = "AvgConfig." .. storyName .. ".AvgLang_" .. storyName ..
                         "_" .. lang
    local ok, err = pcall(function()
        -- function num : 0_9_1 , upvalues : self, _ENV, langPath
        self.avgLangCfg = require(langPath)
...
AvgController.SetAvgAutoPlayMode = function(self, autoPlayMode, delayRatio)
    -- function num : 0_13
    self.autoPlayMode = autoPlayMode
    if self.autoPlayMode and self.avgActComplete then self:PlayNextAvgAct() end
    self:ClearDelayPlatNextTimer()
    self.actDelayRatio = delayRatio or 1
    return self.autoPlayMode, self.actDelayRatio
end
...
AvgController.ChangeSpeed = function(self)
    -- function num : 0_17 , upvalues : _ENV
    if self:AvgIgnoreTimeScale() then return end -- DECOMPILER ERROR at PC16: Confused about usage of register: R1 in 'UnsetPending'
    
    (Time.unity_time).timeScale = self.speedupMode and
                                      (ConfigData.game_config).AvgSpeedupTimescale or
                                      1
end
...
AvgController.RecordAct = function(self, actCfg, actId)
    -- function num : 0_19 , upvalues : _ENV
    if actCfg.content == nil and actCfg.branch == nil then return end
    local recordData = {actId = actId, selectedActId = nil};
    (table.insert)(self.recordDataList, recordData)
end
...
AvgController.GetAvgRecordData = function(self)
    -- function num : 0_21 , upvalues : _ENV
    local list = {}
    for k, v in ipairs(self.recordDataList) do list[k] = v end
    return list
end
...
AvgController.JumpChapter = function(self, chapterName)
    -- function num : 0_25
    self:ClearCurAvg()
    self:ShowAvg(chapterName, self.completeFunc, self.__shieldControlSwitch,
                 self.__ignoreTimeScale)
end
...
AvgController.CompleteAllAvg = function(self, hasError)
    ...
    self._hasError = hasError
    if self.avgCachePlayParam ~= nil and (self.avgCachePlayParam).avgId ~= nil then
        if (ControllerManager:GetController(ControllerTypeId.AvgPlay)):IsAvgPlayed(
            (self.avgCachePlayParam).avgId) then
            self:OnAvgComplete(nil)
        else

            (ControllerManager:GetController(ControllerTypeId.AvgPlay)):RecordAvgPlayed(
                (self.avgCachePlayParam).avgId)
            self._heroIdSnapShoot = PlayerDataCenter:GetHeroIdSnapShoot();
            (NetworkManager:GetNetwork(NetworkTypeID.Avg)):CS_AVG_Complete(
                (self.avgCachePlayParam).avgId, self.__onAvgComplete)
        end
    else
        self:OnAvgComplete(nil)
    end
...
AvgController.OnAvgComplete = function(self, objList)
    ...
    if objList ~= nil and objList.Count > 0 and self.avgCachePlayParam ~= nil then
        ...
        if ok then
            local avgCfg =
                (ConfigData.story_avg)[(self.avgCachePlayParam).avgId]
            do
                if #avgCfg.rewardIds ~= 0 then
                    UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward,
                        ...
                        -- function num : 0_28_0 , upvalues : avgCfg, self
                        if window == nil then return end
                        window:InitRewardsItem(avgCfg.rewardIds,
                                               avgCfg.rewardNums,
    ...
    do
        self.__waitAvgCompleteNetowrk = false
        self:ClearCurAvg()
        self.avgIsPlaying = false -- DECOMPILER ERROR at PC34: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (Time.unity_time).timeScale = self.__originTimeScale
        AudioManager:RemoveBgmSourceFader()
        AudioManager:ResumeLastBgm()
        ...
        if self.avgCachePlayParam == nil then self:Delete() end
    end
end
...
AvgController.RecordAvgAudioSheet = function(self, sheetName)
    ...
    if self._waitRemoveSheet == nil then self._waitRemoveSheet = {} end -- DECOMPILER ERROR at PC15: Confused about usage of register: R2 in 'UnsetPending'
...
AvgController._RemoveCueSheet = function(self)
    -- function num : 0_31 , upvalues : _ENV
    if self._waitRemoveSheet == nil then return end
    for sheetName, v in pairs(self._waitRemoveSheet) do
        AudioManager:RemoveCueSheet(sheetName)
    end
    self._waitRemoveSheet = nil
