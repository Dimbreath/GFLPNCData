...
AvgController.OnAvgActComplete = function(self, jumpAct, jumpChapter)
    -- function num : 0_7 , upvalues : _ENV
    self.avgActComplete = true;
    (self.avgWindow):StopAvgVoice()
    if self:AvgIsEnd(self.curActId) then return end
    if jumpAct ~= nil then
        ...
        if not (string.IsNullOrEmpty)(jumpChapter) then
            ...
            self.__delayPlatNextTimer = (TimerManager:GetTimer(1, function()
...
AvgController.SwitchAvgAutoPlay = function(self)
    -- function num : 0_12
    local autoPlayMode = not self.autoPlayMode
    return self:SetAvgAutoPlayMode(autoPlayMode)
end

AvgController.SetAvgAutoPlayMode = function(self, autoPlayMode)
    -- function num : 0_13
...
AvgController.CompleteAllAvg = function(self)
            ...
            do
                if #avgCfg.rewardIds ~= 0 then
                    UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward,
                                              function(window)
                        -- function num : 0_28_0 , upvalues : avgCfg
                        if window == nil then return end
