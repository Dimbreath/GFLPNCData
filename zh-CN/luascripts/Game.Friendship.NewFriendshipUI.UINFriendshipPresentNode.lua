...
local UINFriendshipPresentNodeGiftItem =
    require("Game.Friendship.NewFriendshipUI.UINFriendshipPresentNodeGiftItem")
UINFriendshipPresentNode.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINFriendshipPresentNodeGiftItem
    self.networkCtrl = NetworkManager:GetNetwork(NetworkTypeID.Friendship)
    self.resloader = nil
    ...
    self.__RegenerateGifts = BindCallback(self, self.RegenerateGifts)
    MsgCenter:AddListener(eMsgEventId.UpdateItem, self.__RegenerateGifts)
    self.giftPool = (UIItemPool.New)(UINFriendshipPresentNodeGiftItem,
                                     (self.ui).obj_presentItem);
    ((self.ui).obj_presentItem):SetActive(false)
    self.__ChangeSelect = Bind(self, self.ChangeSelect)
end

UINFriendshipPresentNode.InitResloader =
    function(self, resloader, addExpCallback, minExpCallback, friendshipLevelUI)
        -- function num : 0_1
        self.resloader = resloader
        self.addExpCallback = addExpCallback
        self.minExpCallback = minExpCallback
...
UINFriendshipPresentNode.RegenerateGifts =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        for _, item in ipairs((self.giftPool).listItem) do item:Claer() end
        (self.giftPool):HideAll()
        self.selectedGiftId = {}
        self.selectedGitfNums = {}
        local giftList = {}
        ...
        for _, gift in ipairs(giftList) do
            ...
            gitfItem:InitGiftItem(self.targetHeroID, self.resloader, gift.id,
                                  self.addExpCallback, self.minExpCallback,
                                  self.__ChangeSelect)
        end
    end
...
UINFriendshipPresentNode.Send = function(self)
    -- function num : 0_7 , upvalues : _ENV
    if (table.count)(self.selectedGifts) < 1 then return end
    local data = (self.friendshipLevelUI).whenAddData
    self.lastLevel = (PlayerDataCenter.allFriendshipData):GetLevel(
                         self.targetHeroID);
    (self.networkCtrl):CS_INTIMACY_Presentation(self.targetHeroID,
                                                self.selectedGifts, function()
        -- function num : 0_7_0 , upvalues : _ENV, self
        local level = (PlayerDataCenter.allFriendshipData):GetLevel(
                          self.targetHeroID)
        if level ~= self.lastLevel then
            (UIManager:ShowWindow(UIWindowTypeID.HeroLevelUpSuccess)):FriendShipLevelUp(
...
UINFriendshipPresentNode.RefreshPresentConfirmBtn =
    function(self)
        -- function num : 0_9 , upvalues : _ENV
        -- DECOMPILER ERROR at PC4: Confused about usage of register: R1 in 'UnsetPending'

        ((self.ui).img_Give).color = (self.ui).color_gray -- DECOMPILER ERROR at PC7: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ((self.ui).btn_Give).interactable = false;
        (((self.ui).btn_ClearSelect).gameObject):SetActive(false)
        if (PlayerDataCenter.allFriendshipData):IsFriendShipMaxLevel(
            self.targetHeroID) then return end
        local isSelect = false
        if (table.count)(self.selectedGifts) ~= 0 then
            for itemId, num in pairs(self.selectedGifts) do
                if num > 0 then isSelect = true end
            end
        end
        do
            -- DECOMPILER ERROR at PC44: Confused about usage of register: R2 in 'UnsetPending'
            ...
            if isSelect then
                ((self.ui).img_Give).color = (self.ui).color_orange -- DECOMPILER ERROR at PC47: Confused about usage of register: R2 in 'UnsetPending'
                ;
                ((self.ui).btn_Give).interactable = true;
                (((self.ui).btn_ClearSelect).gameObject):SetActive(true)
            end
