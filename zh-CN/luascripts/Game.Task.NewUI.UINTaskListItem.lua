...
UINTaskListItem.OnInit = function(self)
    ...
    self.poolRewardItem = (UIItemPool.New)(UINBaseItemWithReceived,
                                           (self.ui).rewardItem);
...
UINTaskListItem.RefreshUI = function(self)
    ...
    ((self.ui).obj_stepArrow):SetActive(self.state ==
                                            (TaskEnum.eTaskState).Completed)
    ...
    do
        if (self.taskData).steps == nil then
            local stepCfg = (ConfigData.taskStep)[(self.taskCfg).id] -- DECOMPILER ERROR at PC73: Confused about usage of register: R2 in 'UnsetPending'
            ;
        ...
        for stepId, stepData in pairs((self.taskData).steps) do
            ...
            local schedule = stepData.schedule -- DECOMPILER ERROR at PC106: Confused about usage of register: R8 in 'UnsetPending'
            ;
            ((self.ui).tex_TaskIntro).text =
                (LanguageUtil.GetLocaleText)(stepCfg.intro)
            ...
            if self.state == (TaskEnum.eTaskState).Picked then
                ...
                ((self.ui).img_Fill).sizeDelta =
                    (Vector2.New)(schedule / stepData.aim * sizeDelta.x,
                                  sizeDelta.y)
                if schedule < stepData.aim then
                    ((self.ui).tex_Progress):SetIndex(0, tostring(schedule),
...
UINTaskListItem.OnClickBtn = function(self)
    -- function num : 0_4 , upvalues : TaskEnum, JumpManager
    if self.state == (TaskEnum.eTaskState).InProgress then
