...
UIDormEdit.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_RoomConfirm, self, self.OnRoomConfirmClicked)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_RoomCancel, self, self.OnRoomCancelClicked)
  ;
  ...
  self.poolRoomItem = (UIItemPool.New)(UIDormEditRoomItem, (self.ui).presetRoomItem)
...
UIDormEdit.OpenEditRoomList = function(self)
  ...
  if ((self.ui).presetRoomList).activeSelf then
    return 
  end
  ;
  ...
  for spos,roomData in pairs((self.editHouseData).oldStoreRoom) do
    ...
    if roomCfg == nil then
      ...
      roomItem:InitEditPrefabRoom(roomCfg, roomData, self.__onEditRoomClicked)
    end
  end
  ...
  for id,roombpItem in pairs(roombpDic) do
    local count = roombpItem:GetCount() - (((self.editHouseData).newRoomCount)[id] or 0)
    if count > 0 then
...
UIDormEdit.OnEditRoomClicked = function(self, roomItem)
  -- function num : 0_5
  if roomItem == nil or self.selectRoomItem == roomItem then
    return 
  end
...
UIDormEdit.OnRoomConfirmClicked = function(self)
  ...
  ;
  ((self.dormCtrl).houseCtrl):ReplaceHouseRoom((self.selectRoomItem).id, (self.selectRoomItem).roomData)
end
