...
UIViewChips.OnInit = function(self)
    ...
    (self.chipDetailPanel):SetChipEffectHeroNodeActive(false) -- DECOMPILER ERROR at PC35: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).chipList).onInstantiateItem =
        BindCallback(self, self.__LevelChipListInitItem) -- DECOMPILER ERROR at PC42: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).chipList).onChangeItem = BindCallback(self,
                                                     self.__LevelChipListItemChanged)
...
UIViewChips.InitLevelChip = function(self, sectorStageCfg, resLoader)
    ...
    local chip_dic = {}
    for k, v in pairs(sectorStageCfg.function_extra) do chip_dic[v] = true end
    local chip_pool_dic = {}
    ...
    end -- DECOMPILER ERROR at PC73: Confused about usage of register: R6 in 'UnsetPending'
...
UIViewChips.OnChipSetUpdate = function(self, chipSetTab)
    ...
    for _, chipData in ipairs(self.chipDataList) do
        if chipSetTab[chipData.dataId] ~= nil then chipData.isLock = true end
    end
    self:__ChipListUpdateInternal()
end
...
UIViewChips.__LevelChipListInitItem = function(self, go)
    ...
    levelChipItem:Init(go) -- DECOMPILER ERROR at PC6: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.levelChipItemDic)[go] = levelChipItem
end

UIViewChips.__LevelChipListItemChanged =
...
UIViewChips.__OnLevelItemClick = function(self, levelChipItem)
    -- function num : 0_8
    if levelChipItem == nil then return end
    if self.selectedIndex == levelChipItem:GetIndex() then return end
    local lastStoreItem = self:__GetItemByIndex(self.selectedIndex)
    if lastStoreItem ~= nil then lastStoreItem:SetLevelChipItemSelect(false) end
