...
TrainingNetworkCtrl.ctor = function(self)
    ...
    self.redDotTrainingGlobalExpRatio =
        (ConfigData.game_config).redDotTrainingGlobalExpRatio
end

TrainingNetworkCtrl.InitNetwork = function(self)
    -- function num : 0_1 , upvalues : _ENV
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_TRAINING_Detail, self,
                         proto_csmsg.SC_TRAINING_Detail,
                         self.__OnRecvTrainingDetail)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_TRAINING_SlotHero, self,
                         proto_csmsg.SC_TRAINING_SlotHero, self.__OnRecvSlotHero)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_TRAINING_Settle, self,
                         proto_csmsg.SC_TRAINING_Settle,
                         self.__OnRecvTrainingSettle)
end
...
TrainingNetworkCtrl.SendTrainingDetail =
    function(self)
        -- function num : 0_3 , upvalues : _ENV, cs_WaitNetworkResponse
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_TRAINING_Detail,
                     proto_csmsg.CS_TRAINING_Detail, table.emptytable)
        cs_WaitNetworkResponse:StartOrAddWait(
            proto_csmsg_MSG_ID.MSG_CS_TRAINING_Detail,
            proto_csmsg_MSG_ID.MSG_SC_TRAINING_Detail)
    end

TrainingNetworkCtrl.__OnRecvTrainingDetail =
    function(self, msg)
        -- function num : 0_4 , upvalues : _ENV
        for k, v in pairs((msg.detail).data) do self:__UpdateSlotData(v) end
...
TrainingNetworkCtrl.SendSlotHero = function(self, heroId, slotIndex, effectType)
    ...
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_TRAINING_SlotHero,
                 proto_csmsg.CS_TRAINING_SlotHero, tabMsg)
end
...
TrainingNetworkCtrl.__OnRecvSlotHero = function(self, msg)
    ...
    if msg.ret == proto_csmsg_ErrorCode.None then
        ...
        do
            local err = "TrainingNetworkCtrl:__OnRecvSlotHero error:" ..
                            tostring(msg.ret)
            error(err)
...
TrainingNetworkCtrl.SendTrainingSettle =
    function(self, slotIndex)
        -- function num : 0_7 , upvalues : _ENV
        local tabMsg = {slotIdx = slotIndex}
        self.__slotIndex = slotIndex
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_TRAINING_Settle,
                     proto_csmsg.CS_TRAINING_Settle, tabMsg)
    end

TrainingNetworkCtrl.__OnRecvTrainingSettle =
    function(self, msg)
        ...
        if msg.ret == proto_csmsg_ErrorCode.None then
            ...
            do
                local err =
                    "TrainingNetworkCtrl:__OnRecvTrainingSettle error:" ..
                        tostring(msg.ret)
                error(err)
                if isGameDev then
...
TrainingNetworkCtrl.InitRedDotTraining =
    function(self)
        ...
        local globalExpOk = self.redDotTrainingGlobalExpRatio < globalExp /
                                globalExpCeiling
        ...
        for k, v in ipairs(PlayerDataCenter.trainingSlotDatas) do
            ...
            elseif unlock and v.heroId ~= nil then
                local duration, remainSecond, progress =
                    v:UpdateProgress(PlayerDataCenter.timestamp)
                if progress >= 1 then
                    trainingCompleteCount = trainingCompleteCount + 1
                end
            end
        end
        local emptySlotNode = RedDotController:AddRedDotNodeWithPath(
                                  RedDotDynPath.EmptyTrainingSlotPath,
                                  RedDotStaticTypeId.Main,
                                  RedDotStaticTypeId.MainSide,
                                  RedDotStaticTypeId.Training,
                                  RedDotStaticTypeId.EmptyTrainingSlot)
        emptySlotNode:SetRedDotCount(emptySlotCount)
        local completeNode = RedDotController:AddRedDotNodeWithPath(
                                 RedDotDynPath.TrainingCompletePath,
                                 RedDotStaticTypeId.Main,
                                 RedDotStaticTypeId.MainSide,
                                 RedDotStaticTypeId.Training,
                                 RedDotStaticTypeId.TrainingComplete)
        completeNode:SetRedDotCount(trainingCompleteCount)
        self:StartTrainingRedDotTimer()
        -- DECOMPILER ERROR: 3 unprocessed JMP targets
    end

TrainingNetworkCtrl.UpdateRedDotEmptySlot =
    function(self)
...
TrainingNetworkCtrl.UpdateRedDotCompleteSlot =
    function(self)
...
TrainingNetworkCtrl.StartTrainingRedDotTimer =
    function(self)
        -- function num : 0_12 , upvalues : _ENV
        self.redDotTimer = (TimerManager:GetTimer(60, (BindCallback(self,
                                                                    self.UpdateRedDotCompleteSlot)),
                                                  nil, false, false, true)):Start()
    end

TrainingNetworkCtrl.StopTrainingRedDotTimer =
    function(self)
        -- function num : 0_13
