...
local UINFosterUpgradeCostItem = require(
                                     "Game.Friendship.Foster.UINFosterUpgradeCostItem")
...
UINFosterUpgradeNode.OnInit = function(self)
    ...
    self.CostItemPool = (UIItemPool.New)(UINFosterUpgradeCostItem,
                                         (self.ui).obj_materialItem);
    ((self.ui).obj_materialItem):SetActive(false)
end

UINFosterUpgradeNode.InitUpgradekNode = function(self, heroId, heroLevel,
                                                 friendShipLevel)
    -- function num : 0_1
    self.heroId = heroId
...
UINFosterUpgradeNode.RefreshAndShowInfo =
    function(self, fosterCfg, nowlevel)
        ...
        ((self.ui).tex_GName):SetIndex(0, (LanguageUtil.GetLocaleText)(
                                           ((ConfigData.item)[ItemIdOfG]).name))
        ...
        (UIUtil.AddButtonListener)((self.ui).btn_Confirm, self, function()
            ...
            if requireFlevel <= self.friendShipLevel and requireHlevel <=
                self.heroLevel and self:__IsHaveEnoughItem(allItemList) and
                requireGNum <= totalG then
...
UINFosterUpgradeNode.__IsHaveEnoughItem =
    function(self, allItemList)
        -- function num : 0_3 , upvalues : _ENV
        for index, item in ipairs(allItemList) do
            if item:IsHaveEnoughItem() == false then return false end
        end
        return true
    end
