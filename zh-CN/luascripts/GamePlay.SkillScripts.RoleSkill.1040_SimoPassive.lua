...
bs_1040.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1040_3", 1,
                    self.OnAfterHurt)
    self.attackNum = 0
end

bs_1040.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and not isMiss then
        if skill.isCommonAttack then
            ...
            if (self.arglist)[3] <= self.attackNum then
                ...
                if grid ~= nil then
                    ...
                    LuaSkillCtrl:CallBuff(self, self.caster,
                                          ...
                                          (self.arglist)[1])
                    LuaSkillCtrl:MoveRoleToTarget(self, grid, self.caster, true,
                                                  self.OnArrive)
                end
            end
        end
