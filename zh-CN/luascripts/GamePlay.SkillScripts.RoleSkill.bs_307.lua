...
bs_307.OnAttackTrigger = function(self, data)
    -- function num : 0_3 , upvalues : _ENV
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId1, 1) -- DECOMPILER ERROR at PC10: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["307_hurt"] = 3
    local effectconfig = (self.config)["effectId" ..
                             tostring(((self.caster).recordTable)["307_hurt"])]
    self.effectBJ = LuaSkillCtrl:CallEffect(self.caster, effectconfig, self)
end

bs_307.OnSetHurt = function(self, context)
    -- function num : 0_4 , upvalues : _ENV
    if (self.caster):GetBuffTier((self.config).buffId1) ~= 0 and context.sender ==
        self.caster and ((self.caster).recordTable)["307_hurt"] > 0 then
        context.hurt = context.hurt * (self.arglist)[1] * 10 // 1000
        if self.effectBJ ~= nil then
            (self.effectBJ):Die()
            self.effectBJ = nil
        ...
        ((self.caster).recordTable)["307_hurt"] =
            ((self.caster).recordTable)["307_hurt"] - 1
        -- DECOMPILER ERROR at PC59: Confused about usage of register: R2 in 'UnsetPending'

        if (context.target).hp <= context.hurt and
            (self.caster):GetBuffTier((self.config).buffId1) > 0 and
            context.sender == self.caster then
            ((self.caster).recordTable)["307_hurt"] =
                ((self.caster).recordTable)["307_hurt"] + 2
            if (self.arglist)[2] >= 0 then
                LuaSkillCtrl:CallHeal(context.hurt * 500 // 1000, self,
                                      self.caster)
            end
        end
        print(((self.caster).recordTable)["307_hurt"])
        if ((self.caster).recordTable)["307_hurt"] > 0 then
            ...
            local effectconfig = (self.config)["effectId" ..
                                     tostring(
                                         ((self.caster).recordTable)["307_hurt"])]
            self.effectBJ = LuaSkillCtrl:CallEffect(self.caster, effectconfig,
                                                    self)
        end
        do
            if ((self.caster).recordTable)["307_hurt"] == 0 then
