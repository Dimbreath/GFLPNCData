...
AllEffectorData.StartTimer4ARGNum = function(self)
  ...
  if self.ARGNumTimer ~= nil then
    (self.ARGNumTimer):Stop()
  ...
  self.ARGNumTimer = (TimerManager:GetTimer(minTime, self.m_OnARGItemChnage, nil, true, false, false)):Start()
end
...
local EffectorResourceData = require("Game.Effector.Data.EffectorResourceData")
local EffectorUtil = require("Game.Effector.EffectorUtil")
local AutoResourceGeneratorData = require(
                                      "Game.Effector.Data.AutoResourceGeneratorData")
AllEffectorData.ctor = function(self)
    -- function num : 0_0 , upvalues : _ENV
    self.effectorResDataDic = {}
    self.AutoResourceGeneratorDic = {}
...
AllEffectorData.UpdateResourceData = function(self, rg)
    ...
    if resData == nil then
        resData = (EffectorResourceData.New)()
        resData:InitEffectorRes(rg) -- DECOMPILER ERROR at PC13: Confused about usage of register: R3 in 'UnsetPending'
        ;
        (self.effectorResDataDic)[rg.uid] = resData
    else
        resData:UpdateEffectorRes(rg)
...
AllEffectorData.UpdateBuildingResourceData =
    function(self, resData, remove)
        -- function num : 0_2 , upvalues : _ENV
        local buildingId = resData.logicId
        local buildingData =
            ((PlayerDataCenter.AllBuildingData).built)[buildingId]
        if buildingData == nil then
            error("Can\'t find buildingData, id = " .. tostring(buildingId))
            return
        end
...
AllEffectorData.DeleteResourceData = function(self, uid)
    -- function num : 0_3 , upvalues : _ENV
    local resData = (self.effectorResDataDic)[uid] -- DECOMPILER ERROR at PC3: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.effectorResDataDic)[uid] = nil
    local functionId = resData.functionId
    if functionId == proto_csmsg_SystemFunctionID.SystemFunctionID_Building then
...
AllEffectorData.IsAutoGenerateResource =
    function(self, itemId)
        -- function num : 0_4 , upvalues : _ENV
        local itemCfg = (ConfigData.item)[itemId]
        do
            return itemCfg.type == eItemType.AutoGenerateResource and
                       (self.AutoResourceGeneratorDic)[itemId] ~= nil
        end
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end

AllEffectorData.UpdateAutoResourceGenerator =
    function(self, ARGBase)
        -- function num : 0_5 , upvalues : AutoResourceGeneratorData
        local ArgData = (self.AutoResourceGeneratorDic)[ARGBase.itemId]
        if ArgData == nil then
            ArgData = (AutoResourceGeneratorData.New)()
            ArgData:UpdateItem(ARGBase) -- DECOMPILER ERROR at PC13: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.AutoResourceGeneratorDic)[ARGBase.itemId] = ArgData
        else
            ArgData:UpdateItem(ARGBase)
...
AllEffectorData.OnUpdateItemGenerateSpeed =
    function(self)
        -- function num : 0_7 , upvalues : _ENV
        for itemId, data in pairs(self.AutoResourceGeneratorDic) do
            data:UpdateItemSpeed()
...
AllEffectorData.GetCurrentARGSpeed = function(self, itemId, isBase)
    ...
    return ((self.AutoResourceGeneratorDic)[itemId]):GetARGGenSpeedPerSecond(
               isBase)
end
...
AllEffectorData.StartTimer4ARGNum = function(self)
    ...
    local minTime = math.maxinteger
    for itemId, data in pairs(self.AutoResourceGeneratorDic) do
        local num, remainSecond = data:GetCurrentNum() -- DECOMPILER ERROR at PC9: Confused about usage of register: R9 in 'UnsetPending'
        ;
        (self.OldItemDNumDic)[itemId] = num
        if remainSecond ~= 0 then
            minTime = (math.min)(minTime, remainSecond)
    ...
    if minTime == math.maxinteger then return end
    if minTime < 1 then minTime = 1 end
    self.ARGNumTimer = (TimerManager:GetTimer(minTime, self.m_OnARGItemChnage,
