...
local UINResourceGroup = require("Game.CommonUI.ResourceGroup.UINResourceGroup")
local UINBaseItem = require("Game.CommonUI.Item.UINBaseItem")
local UINStarUpStarList = require(
                              "Game.Hero.NewUI.UpgradeStar.UINStarUpStarList")
local UINStarUpPromoteItem = require(
                                 "Game.Hero.NewUI.UpgradeStar.UINStarUpPromoteItem")
local UINBaseSkillItem = require("Game.CommonUI.Item.UINBaseSkillItem")
...
UIHeroStarUp.OnInit = function(self)
    ...
    self.skillItem = (UINBaseSkillItem.New)();
    (self.skillItem):Init((self.ui).skillItem)
    self.promoteItemPool = (UIItemPool.New)(UINStarUpPromoteItem,
                                            (self.ui).promoteItem);
    ((self.ui).promoteItem):SetActive(false)
    self.attrItemPool = (UIItemPool.New)(UINStarUpAttrItem, (self.ui).attriItem);
    ((self.ui).attriItem):SetActive(false)
...
UIHeroStarUp.InitHeroStarUp = function(self, heroData, resloader, hideCallBack,
                                       switchHeroFunc)
    -- function num : 0_2 , upvalues : _ENV
    (UIUtil.Push2BackStack)(self, self.OnClickBack)
    self.heroData = heroData
...
UIHeroStarUp.RefreshItem = function(self)
    -- function num : 0_4
    self.isItemUpdate = true
    if self.isRankUpdate then self:RefreshAll() end
end
...
UIHeroStarUp.RefreshAll = function(self, heroId, reUseBigImgResloader)
    ...
    self.isItemUpdate = false
    self.isRankUpdate = false
    local ableUp, fitFrag, fitCurrency, fitExtr =
        (self.heroData):AbleUpgradeStar()
    if heroId ~= nil and heroId ~= (self.heroData).dataId then return end
    if (self.heroData):IsFullRank() then
        self:RefreshFrag(true)
        self:RefreshCurrency(true)
        self:RefreshExtrItem(true)
...
UIHeroStarUp.RefreshFrag = function(self, isFullLevel)
    ...
    if self.needfragNum <= totalFragNum then
        ((self.ui).tex_FragCount):SetIndex(0, tostring(self.needfragNum),
                                           tostring(totalFragNum))
    else

        ((self.ui).tex_FragCount):SetIndex(2, tostring(self.needfragNum),
                                           tostring(totalFragNum))
    end
end
...
UIHeroStarUp.RefreshCurrency = function(self, isFullLevel, fitCurrency)
    ...
    if self.currencyId ~= nil then
        ...
        if itemCfg == nil then
            ...
            ((self.ui).img_CurrencyItem).sprite =
                CRH:GetSprite(itemCfg.small_icon)
        end
        if fitCurrency then
            ((self.ui).tex_CurrencyCost):SetIndex(1, tostring(
                                                      self.needCurrencyNum))
        else

            ((self.ui).tex_CurrencyCost):SetIndex(0, tostring(
                                                      self.needCurrencyNum))
        end
    else
        do
...
UIHeroStarUp.RefreshExtrItem = function(self, isFullLevel)
    ...
    if self.extrItemId ~= nil then
        ...
        if self.needExtrItemNum <= totalExtrItemNum then
            ((self.ui).tex_ExtrCount):SetIndex(0,
                                               tostring(self.needExtrItemNum),
                                               tostring(totalExtrItemNum))
        else

            ((self.ui).tex_ExtrCount):SetIndex(2,
                                               tostring(self.needExtrItemNum),
                                               tostring(totalExtrItemNum))
        end
    else
...
UIHeroStarUp.RefreshStarUpInfo = function(self, isFullLevel)
    ...
    do
        ...
        if not isFullLevel then
            (self.attrItemPool):HideAll()
            local changeList = (self.heroData):GetDifferAttrWhenRankUp(
                                   (self.heroData).rank + 1, nil)
            if #changeList >= 10 then
                for index, data in ipairs(changeList) do
                    if index <= 10 then
                        do
                            local item = (self.attrItemPool):GetOne()
                            item:InitAttrItem(data.oldAttr, data.newAttr,
                                              data.attrId)
                            -- DECOMPILER ERROR at PC78: LeaveBlock: unexpected jumping out IF_THEN_STMT
            ...
            do
                ...
                do
                    ...
                    for _, attrId in ipairs((ConfigData.attribute).baseAttrIds) do
                        ...
                        if ((ConfigData.attribute)[attrId]).merge_attribute == 0 and
                            ...
                            do
                                local item = (self.attrItemPool):GetOne()
                                item:InitAttrItem(
                                    ((self.heroData):GetAttr(R16_PC188, nil,
                                                             true)), nil,
                                    R16_PC188)
                                -- DECOMPILER ERROR at PC192: LeaveBlock: unexpected jumping out IF_THEN_STMT
                    ...
                    if not isFullLevel then
                        local heroStarCfg =
                            (ConfigData.hero_star)[(self.heroData).dataId]
                        if heroStarCfg == nil then
                            error("Can\'t find heroStarCfg, id = " ..
                                      tostring((self.heroData).dataId))
                        end
                        local currentHeroStarCfg = heroStarCfg[rankCfg.rank]
                        local nextHeroStarCfg = heroStarCfg[nextRankCfg.rank]
                        do
                            if (currentHeroStarCfg.algorithm_space)[1] ~=
                                (nextHeroStarCfg.algorithm_space)[1] then
                                local item = (self.promoteItemPool):GetOne()
                                item:InitPromoteItem(promoteType.athSpace,
                                                     (nextHeroStarCfg.algorithm_space)[1],
                                                     (currentHeroStarCfg.algorithm_space)[1])
                            end
                            do
                                if rankCfg.skill_level_max ~=
                                    nextRankCfg.skill_level_max then
                                    local item = (self.promoteItemPool):GetOne()
                                    item:InitPromoteItem(
                                        promoteType.skillLevelLimit,
                                        nextRankCfg.skill_level_max,
                                        rankCfg.skill_level_max)
                                end
                                if nextHeroStarCfg.rank_text ~= nil then
                                    local str =
                                        (LanguageUtil.GetLocaleText)(
                                            nextHeroStarCfg.rank_text)
                                    if not (string.IsNullOrEmpty)(str) then
                                        local item =
                                            (self.promoteItemPool):GetOne()
                                        item:InitPromoteItem(
                                            promoteType.special, str)
                                    end
                                end
                                do
                                    -- DECOMPILER ERROR at PC273: Confused about usage of register: R5 in 'UnsetPending'
                                    ...
                                    ((self.ui).unlockSkillItem):SetActive(false) -- DECOMPILER ERROR at PC295: Confused about usage of register: R5 in 'UnsetPending'
                                    ;
                                    ((self.ui).Fade_Skill).alpha = 0.4
                                    if not isFullLevel then
                                        local nextRank =
                                            (self.heroData).rank + 1
                                        local newUSkill =
                                            ((ConfigData.hero_rank)[nextRank]).ultimateskill_level
                                        local overSkillData = nil
                                        local hasOpenSkill = false
                                        for _, skillData in
                                            ipairs((self.heroData).skillList) do
                                            if skillData:IsUniqueSkill() and
                                                (self.heroData):GetUltimateSkillLevel() <
                                                newUSkill then
                                                overSkillData = skillData -- DECOMPILER ERROR at PC325: Overwrote pending register: R16 in 'AssignReg'
                                                ;
                                                ((self.ui).Tex_unlockSkillTip):SetIndex(
                                                    R16_PC188)
                                                break
                                            end
                                            if (skillData.skillCfg).open_star ==
                                                nextRank and not hasOpenSkill then
                                                overSkillData = skillData -- DECOMPILER ERROR at PC338: Overwrote pending register: R16 in 'AssignReg'
                                                ;
                                                ((self.ui).Tex_unlockSkillTip):SetIndex(
                                                    R16_PC188)
                                                hasOpenSkill = true
                                        ...
                                        do
                                            if overSkillData ~= nil then
                                                ((self.ui).unlockSkillItem):SetActive(
                                                    true);
                                                (self.skillItem):InitBaseSkillItem(
                                                    overSkillData,
                                                    self.resloader);
                                                (self.skillItem):__SetUnlockUI(
                                                    true) -- DECOMPILER ERROR at PC361: Confused about usage of register: R9 in 'UnsetPending'
                                                ;
                                                ((self.ui).Fade_Skill).alpha = 1
                                            end
                                        end
    ...
    if ableUp then
        ...
        if not fitFrag or not fitCurrency or not fitExtr then
            (cs_MessageCommon.ShowMessageTipsWithErrorSound)(
                ConfigData:GetTipContent(TipContent.hero_CantUpgrade))
        end
    end
end
