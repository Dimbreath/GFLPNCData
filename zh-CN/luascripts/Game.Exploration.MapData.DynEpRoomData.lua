...
DynEpRoomData.SetMapData = function(self, map, epOp)
    ...
    do
        if self:IsBattleRoom() or self.type ==
            (ExplorationEnum.eRoomType).deploy then
            local stageCfg = ExplorationManager:GetSectorStageCfg()
            self.battleMap = (CS_BattleUtility.GenBattleMap)(stageCfg.size_row,
                                                             stageCfg.size_col,
                                                             stageCfg.deploy_rows) -- DECOMPILER ERROR at PC35: Confused about usage of register: R4 in 'UnsetPending'
            ;
            (self.battleMap).IsDeployRoom =
                self.type == (ExplorationEnum.eRoomType).deploy
        end
        -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end
end

DynEpRoomData.SetEpRoomEpOpStateChanged =
    function(self, EpOp)
        -- function num : 0_2 , upvalues : ExplorationEnum, _ENV, CS_BattleUtility
        if not EpOp or EpOp.curPostion ~= self.position then return end
        if EpOp.jumpCat and EpOp.jumpCat > 0 then
            self.jumpCat = EpOp.jumpCat
            if self:IsBattleRoom() or self.type ==
                (ExplorationEnum.eRoomType).deploy then
                local stageCfg = ExplorationManager:GetSectorStageCfg()
                self.battleMap = (CS_BattleUtility.GenBattleMap)(
                                     stageCfg.size_row, stageCfg.size_col,
                                     stageCfg.deploy_rows) -- DECOMPILER ERROR at PC41: Confused about usage of register: R3 in 'UnsetPending'
                ;
                (self.battleMap).IsDeployRoom =
                    self.type == (ExplorationEnum.eRoomType).deploy
            end
...
DynEpRoomData.InitBattleData = function(self, monsterGroup)
    ...
    if monsterGroup.alg ~= nil then
        for k, v in ipairs(monsterGroup.alg) do
            local chipData = (ChipData.New)(v, 1) -- DECOMPILER ERROR at PC23: Confused about usage of register: R8 in 'UnsetPending'
            ;
            (self.rewardChipList)[k] = chipData
        end
    end
    do
        ...
        if monsterGroup.monsterAlg ~= nil then
            for k, v in ipairs(monsterGroup.monsterAlg) do
                local chipData = (ChipData.New)(v, 1) -- DECOMPILER ERROR at PC77: Confused about usage of register: R8 in 'UnsetPending'
                ;
                (self.monsterChipList)[k] = chipData
            end
        end
...
DynEpRoomData.InitMonsterOrNeutralData =
    function(self, groupData)
        -- function num : 0_6 , upvalues : _ENV, ExplorationEnum, DynMonster, DynNeutral
        ((self.battleMap).monsterList):Clear()
        self.monsterList = {};
        ((self.battleMap).neutralList):Clear()
...
DynEpRoomData.__ExecuteMonsterChip = function(self, chipData, isRelative)
    ...
    local validRoleList = chipData:GetValidRoleList(self.monsterList, belong)
    for k, role in pairs(validRoleList) do chipData:ExecuteChipHero(role) end
end

DynEpRoomData.__RollbackMonsterChip = function(self, chipData)
    -- function num : 0_10 , upvalues : _ENV
    for k, role in pairs(self.monsterList) do chipData:RollbackChipHero(role) end
end
...
DynEpRoomData.__ExecuteMonsterBuffChip =
...
DynEpRoomData.__RollbackMonsterBuffChip =
    function(self, buffChip)
        -- function num : 0_14 , upvalues : _ENV
        for k, role in pairs(self.monsterList) do
            buffChip:RollbackChipHero(role)
...
DynEpRoomData.InitTreasureRoomData = function(self, treasureGroup)
    ...
    self.treasureData = {}
    local boxId = nil -- DECOMPILER ERROR at PC11: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.treasureData).boxId = treasureGroup.boxId -- DECOMPILER ERROR at PC14: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.treasureData).logic = treasureGroup.logic -- DECOMPILER ERROR at PC17: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.treasureData).freshCnt = treasureGroup.freshCnt -- DECOMPILER ERROR at PC20: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.treasureData).chipDatas = {}
    if treasureGroup.data ~= nil then
        for k, v in pairs(treasureGroup.data) do
            ...
            if v.state == (ChipEnum.eChipSelectState).StateNone then
                ...
                if v.state == (ChipEnum.eChipSelectState).StateSelected then
                    ...
                    error(
                        "DynEpRoomData:InitTreasureRoomData error:treasureGroup.data.state is nil")
                    break
                end
            end
            Data.state = state
        ...
        do MsgCenter:Broadcast(eMsgEventId.OnTreasureRoomUpdate, self) end
    end
end
...
DynEpRoomData.InitStoreRoomData = function(self, storeGroup)
    ...
    self.storeRoomData = {} -- DECOMPILER ERROR at PC10: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.storeRoomData).currencyId = storeGroup.currency -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.storeRoomData).storeId = storeGroup.storeId -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.storeRoomData).refreshTimes = storeGroup.refreshTimes -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.storeRoomData).discCountLimitTimes = storeGroup.discCountLimitTimes -- DECOMPILER ERROR at PC22: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.storeRoomData).data = {}
    if storeGroup.data ~= nil then
        for idx, elem in ipairs(storeGroup.data) do
...
DynEpRoomData.InitEventAndRecoveryRoomData =
    function(self, eventGroup)
        -- function num : 0_17 , upvalues : _ENV
        if eventGroup == nil then
            error(
                "DynEpRoomData:InitEventAndRecoveryRoomData error:eventGroup is nil")
            return
        end
        eventGroup.choiceDatalist = {}
        local isHaveRandom = false
        for index, value in ipairs(eventGroup.data) do
            ...
            choiceData.isRandom = isRandom
            choiceData.catId = value.cat
            if isRandom then isHaveRandom = true end
            (table.insert)(eventGroup.choiceDatalist, choiceData)
        end
        eventGroup.data = nil
        eventGroup.isHaveRandom = isHaveRandom
...
DynEpRoomData.GetEpBuffEffective = function(self, dynPlayer)
    ...
    do
        if (ConfigData.game_config).epBuffShowMax <= #curRoomActiveBuff or
            self.epBuffEffective ~= nil then
            for buffId, count in pairs(self.epBuffEffective) do
                if (ConfigData.game_config).epBuffShowMax > #curRoomActiveBuff then
                    local buffCfg = (ConfigData.exploration_buff)[buffId]
                    if buffCfg == nil then
                        error("exploration buff cfg is null,id:" ..
                                  tostring(R12_PC51))
                    else
                        if buffCfg.is_show then
                            local i = 1
                            while i <= count and #curRoomActiveBuff <
                                (ConfigData.game_config).epBuffShowMax do
                                (table.insert)(curRoomActiveBuff, R12_PC51)
                                i = i + 1
                            end
                        end
        ...
        if #curRoomActiveBuff == 0 then return nil end
        (table.sort)(curRoomActiveBuff, function(b1, b2)
            -- function num : 0_20_0
            do return b1.id < b2.id end
            -- DECOMPILER ERROR: 1 unprocessed JMP targets
...
DynEpRoomData.GetNextRoom = function(self)
    -- function num : 0_21 , upvalues : ExplorationEnum, _ENV
    if self.nextRooms ~= nil then return self.nextRooms end
    self.nextRooms = {}
    local colTrackType = (self.mapData):GetMapColTrackType(self.x)
    if colTrackType == (ExplorationEnum.eTrackLineType).EndTrack then
        return self.nextRooms
    end
    if colTrackType == (ExplorationEnum.eTrackLineType).ReverseETrack or
        colTrackType == (ExplorationEnum.eTrackLineType).SingleTrack then
        (table.insert)(self.nextRooms, (self.mapData):GetRoomByXY(self.x + 1, 0))
        return self.nextRooms
    end
    if colTrackType == (ExplorationEnum.eTrackLineType).NormalETrack then
    ...
    local ty = self.y
    local tmpRoom = (self.mapData):GetRoomByXY(tx, ty)
    if tmpRoom ~= nil then (table.insert)(self.nextRooms, tmpRoom) end
    if self:IsMaxWidthCol() then
        ty = self.y - 1
    else
        ty = self.y + 1
...
DynEpRoomData.GetLastRoom = function(self)
    -- function num : 0_22 , upvalues : ExplorationEnum, _ENV
    if self.lastRooms ~= nil then return self.lastRooms end
    self.lastRooms = {}
    if self.x == 0 then return self.lastRooms end
    local lastColTrackType = (self.mapData):GetMapColTrackType(self.x - 1)
    if lastColTrackType == (ExplorationEnum.eTrackLineType).NormalETrack or
        lastColTrackType == (ExplorationEnum.eTrackLineType).SingleTrack then
        (table.insert)(self.lastRooms, (self.mapData):GetRoomByXY(self.x - 1, 0))
        return self.lastRooms
    end
    if lastColTrackType == (ExplorationEnum.eTrackLineType).ReverseETrack then
    ...
    local ty = self.y
    local tmpRoom = (self.mapData):GetRoomByXY(tx, ty)
    if tmpRoom ~= nil then (table.insert)(self.lastRooms, tmpRoom) end
    if self:IsMaxWidthCol() then
        ty = self.y - 1
    else
        ty = self.y + 1
...
DynEpRoomData.GetNextDownDirRoom = function(self)
    ...
    local tx = self.x + 1
    local ty = self.y
    if self:IsMaxWidthCol() then ty = ty - 1 end
    local tmpRoom = (self.mapData):GetRoomByXY(tx, ty)
    return tmpRoom
end
...
DynEpRoomData.GetNextUpDirRoom = function(self)
    ...
    local tx = self.x + 1
    local ty = self.y
    if not self:IsMaxWidthCol() then ty = ty + 1 end
    local tmpRoom = (self.mapData):GetRoomByXY(tx, ty)
    return tmpRoom
end
...
DynEpRoomData.IsBattleRoom = function(self)
    -- function num : 0_26 , upvalues : ExplorationEnum
    local roomType = self.type
    local isBattleRoom = roomType == (ExplorationEnum.eRoomType).enemy or
                             roomType == (ExplorationEnum.eRoomType).elite or
                             roomType == (ExplorationEnum.eRoomType).challenge or
                             roomType == (ExplorationEnum.eRoomType).boss
    if self.jumpCat ~= (ExplorationEnum.eRoomType).enemy and self.jumpCat ~=
        (ExplorationEnum.eRoomType).elite and self.jumpCat ~=
        (ExplorationEnum.eRoomType).challenge and self.jumpCat ~=
...
DynEpRoomData.GetTotalFightingPower = function(self)
    -- function num : 0_31 , upvalues : _ENV
    if not self:IsBattleRoom() then return 0 end
    local fightingPower = 0
    for k, dynMonster in pairs(self.monsterList) do
        fightingPower = fightingPower + dynMonster:GetFightingPower()
    end
...
DynEpRoomData.IsHideBossRoom = function(self)
    -- function num : 0_37
    if self:IsBossRoom() then return not self:IsRealBossRoom() end
end
...
DynEpRoomData.GetRoomType = function(self)
    -- function num : 0_40
    local roomType = self.type
    if self.jumpCat and self.jumpCat > 0 then roomType = self.jumpCat end
    return roomType
end

DynEpRoomData.SetAmbushAndSneakData = function(self, ambushData, stealthData)
    -- function num : 0_41 , upvalues : _ENV
    if (ambushData ~= 0 or stealthData ~= 0) and
        (self.ambushData ~= ambushData or self.stealthData ~= stealthData) then
        self.ambushData = ambushData
        self.stealthData = stealthData
        MsgCenter:Broadcast(eMsgEventId.OnBeforeBattleRandomDataChange,
                            ambushData, stealthData, self:GetRoomPosition())
    end
end
