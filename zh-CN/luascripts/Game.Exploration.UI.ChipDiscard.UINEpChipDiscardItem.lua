...
UINEpChipDiscardItem.InitDiscardChipItem =
    function(self, discardId, chipData, clickAction)
        -- function num : 0_1 , upvalues : _ENV, UINChipItem
        self.chipData = chipData
        self.clickAction = clickAction
        self.discardCfg = (ConfigData.exploration_discard)[discardId]
        self.price = self:m_CalculateEpChipSalePrice(
                         ((self.chipData).chipBattleData).level,
                         ((self.chipData).itemCfg).price)
        self:__showSellPrice(
            ((ConfigData.item)[(self.discardCfg).discard_scaleId]).icon)
        self.chipItem = (UINChipItem.New)();
        (self.chipItem):Init((self.ui).chipItem);
        (self.chipItem):InitChipItem(self.chipData) -- DECOMPILER ERROR at PC40: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_ItemName).text = (self.chipData):GetName() -- DECOMPILER ERROR at PC46: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_QuailtyColor).color = (self.chipData):GetColor();
        ((self.ui).tex_ChipLevel):SetIndex(0,
                                           tostring((self.chipData):GetCount())) -- DECOMPILER ERROR at PC68: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_ChipTypeIcon).sprite =
            CRH:GetSprite(eChipCornerSprite[chipData:GetMarkIconIndex()],
                          CommonAtlasType.ExplorationIcon)
    end

UINEpChipDiscardItem.m_CalculateEpChipSalePrice =
    function(self, chipLvl, chipPrice)
        -- function num : 0_2
        local levelCfg = (self.discardCfg).discard_level
        local discountCfg = (self.discardCfg).discard_scaleValues
        local levelCount = #levelCfg
...
UINEpChipDiscardItem.__showSellPrice = function(self, MoneyIconId)
    ...
    ((self.ui).tex_Money).text = tostring(self.price) -- DECOMPILER ERROR at PC12: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_Money).sprite = CRH:GetSprite(MoneyIconId)
end
...
UINEpChipDiscardItem.__OnStoreItemClicked =
    function(self)
        -- function num : 0_5
