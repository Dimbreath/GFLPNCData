...
local RefreshLineRate = function(lineData, timestamp)
  ...
  if lineData.state ~= proto_object_LineState.LineStateRunning then
    ...
    local passTime = lineData.period * OneCycleTime + (math.max)(timestamp + OneCycleTime - lineData.periodOver, 0)
...
FactoryController.OnFactoryOpen = function(self)
  ...
  self.factoryData = {lineCategory = nil, lineCount = 0, sortLineList = nil, lineDic = nil, resource = nil, finalResource = nil}
...
FactoryController.RecvFactoryDetail = function(self, msg)
  ...
  if msg.ret ~= proto_csmsg_ErrorCode.None then
    ((CS.MessageCommon).ShowMessageTips)("Network RecvFactoryDetail error,code:" .. tostring(msg.ret))
    return 
  end
  if not self.factoryOpen then
    return 
  end
  ...
  if uiFactory ~= nil then
    uiFactory:RefreshFactoryAllData((self.factoryData).lineCategory, (self.factoryData).resource, (self.factoryData).finalResource)
  end
end

FactoryController.RefreshFactoryLineData = function(self, detail)
  ...
  for k,lineData in pairs(detail.running) do
    ...
    if factoryLine == nil then
      ...
      factoryLine = {cfg = factoryCfg, cycle = factoryCfg.cycle, isFinalProduct = isFinalProduct, itemCfg = itemCfg, count = 0, 
lines = {}
}
      ...
      if isFinalProduct then
        (lineCategory[id]).totalCycle = ShutdownCycleCount // factoryCfg.cycle
      end
    end
    ...
    do
      do
        ...
        if lineData.isSetCycle and lineData.presetCycle <= lineData.curCycle then
...
FactoryController.GetLineInstallAndAllCount = function(self)
  -- function num : 0_8 , upvalues : _ENV
  return (self.factoryData).lineCount, (PlayerDataCenter.playerBonus):GetFactoryPipelieCount()
end

FactoryController.SendFactoryLineInstall = function(self, id, cycle)
  -- function num : 0_9 , upvalues : _ENV
  if (PlayerDataCenter.playerBonus):GetFactoryPipelieCount() <= (self.factoryData).lineCount then
    ((CS.MessageCommon).ShowMessageTips)(ConfigData:GetTipContent(TipContent.factory_LineNumReachMax))
    return 
  end
  ;
...
FactoryController.RecvFactoryLineInstall = function(self, msg)
  -- function num : 0_10 , upvalues : _ENV
  if msg.ret ~= proto_csmsg_ErrorCode.None then
    ((CS.MessageCommon).ShowMessageTips)("Network RecvFactoryLineInstall error,code:" .. tostring(msg.ret))
    return 
  end
  if not self.factoryOpen then
    return 
...
FactoryController.SendFactoryLineDestruct = function(self, id)
  ...
  if factoryLine == nil or #factoryLine.lines == 0 then
    return 
  end
  local lineData = (factoryLine.lines)[1]
  for k,v in ipairs(factoryLine.lines) do
    if v.period < lineData.period or v.period == lineData.period and lineData.periodOver < v.periodOver then
      lineData = v
    end
  end
  ;
...
FactoryController.RecvFactoryLineDestruct = function(self, msg)
  -- function num : 0_12 , upvalues : _ENV
  if msg.ret ~= proto_csmsg_ErrorCode.None then
    ((CS.MessageCommon).ShowMessageTips)("Network RecvFactoryLineDestruct error,code:" .. tostring(msg.ret))
    return 
  end
  if not self.factoryOpen then
    return 
...
FactoryController.SendFactoryRewardPick = function(self, id)
  -- function num : 0_13
  if ((self.factoryData).finalResource)[id] == nil or ((self.factoryData).finalResource)[id] <= 0 then
    return 
  end
  ;
...
FactoryController.RecvFactoryRewardPick = function(self, msg)
  -- function num : 0_14 , upvalues : _ENV
  if msg.ret ~= proto_csmsg_ErrorCode.None then
    ((CS.MessageCommon).ShowMessageTips)("Network RecvFactoryRewardPick error,code:" .. tostring(msg.ret))
    return 
  end
  if not self.factoryOpen then
    return 
...
FactoryController.OnUpdateSecond = function(self)
  ...
  while 1 do
    if #(self.factoryData).sortLineList > 0 then
      ...
      if timestamp > lineData.periodOver then
        ...
        local factoryLine = ((self.factoryData).lineCategory)[lineData.id]
        ...
        if factoryLine.cycle <= lineData.period then
          ...
          if not ((self.factoryData).finalResource)[productId] then
            do
              ((self.factoryData).finalResource)[productId] = (not factoryLine.isFinalProduct or 0) + productNum
              lineData.itemNum = lineData.itemNum + productNum
              lineData.total = lineData.total + 1
              -- DECOMPILER ERROR at PC79: Confused about usage of register: R9 in 'UnsetPending'

              ;
              ((self.factoryData).resource)[productId] = (((self.factoryData).resource)[productId] or 0) + productNum
              hasProduct = true
              lineData.period = 0
              if factoryLine.isFinalProduct and factoryLine.totalCycle <= lineData.total then
                lineData.state = proto_object_LineState.LineStateOutputOver
                ...
                for k1,consumeId in ipairs((factoryLine.cfg).consumeIds) do
                  local consumeNum = ((factoryLine.cfg).consumeNums)[k1]
                  if ((self.factoryData).resource)[consumeId] == nil or ((self.factoryData).resource)[consumeId] < consumeNum then
                    isAbleRun = false
                    break
                  end
                end
                do
                  do
                    do
                      if isAbleRun and lineData.isSetCycle ~= nil then
                        for k1,consumeId in ipairs((factoryLine.cfg).consumeIds) do
                          local consumeNum = ((factoryLine.cfg).consumeNums)[k1]
                          -- DECOMPILER ERROR at PC150: Confused about usage of register: R14 in 'UnsetPending'

                          ;
                          ((self.factoryData).resource)[consumeId] = ((self.factoryData).resource)[consumeId] - consumeNum
                        end
                        lineData.periodOver = lineData.periodOver + OneCycleTime
                      else
                        lineData.state = proto_object_LineState.LineStateInDonwTime
                        lineData.periodOver = LineStopTime
                      end
                      ;
                      (table.insert)(self.tmpSortAgainLines, lineData)
  ...
  do
      ...
      while 1 do
        while 1 do
          if k <= #(self.factoryData).sortLineList then
            ...
            if tmpLineData.state == proto_object_LineState.LineStateInDonwTime then
              local tmpFactoryLine = ((self.factoryData).lineCategory)[tmpLineData.id]
              local isAbleRun = true
              for k1,consumeId in ipairs((tmpFactoryLine.cfg).consumeIds) do
                local consumeNum = ((tmpFactoryLine.cfg).consumeNums)[k1]
                if ((self.factoryData).resource)[consumeId] == nil or ((self.factoryData).resource)[consumeId] < consumeNum then
                  isAbleRun = false
                  break
                end
              ...
              do
                do
                  if isAbleRun then
                    ...
                    for k1,consumeId in ipairs((tmpFactoryLine.cfg).consumeIds) do
                      local consumeNum = ((tmpFactoryLine.cfg).consumeNums)[k1]
                      -- DECOMPILER ERROR at PC250: Confused about usage of register: R16 in 'UnsetPending'

                      ;
                      ((self.factoryData).resource)[consumeId] = ((self.factoryData).resource)[consumeId] - consumeNum
                    end
                    tmpLineData.state = proto_object_LineState.LineStateRunning
                    tmpLineData.periodOver = timestamp + OneCycleTime
                  end
                  if needSortAgain then
                    (table.insert)(self.tmpSortAgainLines, (table.remove)((self.factoryData).sortLineList, k))
    ...
    do
        ...
        if #self.tmpSortAgainLines > 0 then
          ...
          while 1 do
            while 1 do
              if k <= #(self.factoryData).sortLineList then
                local tmpLineData = ((self.factoryData).sortLineList)[k]
                if lineData.periodOver < tmpLineData.periodOver or lineData.periodOver == tmpLineData.periodOver and lineData.uid < tmpLineData.uid then
                  (table.insert)((self.factoryData).sortLineList, k, lineData)
                  isInsert = true
          ...
          do
            do
              if not isInsert then
                (table.insert)((self.factoryData).sortLineList, lineData)
              end
      ...
      local uiFactory = UIManager:GetWindow(UIWindowTypeID.Factory)
      if uiFactory ~= nil then
        uiFactory:RefreshLineEntityAndResource((self.factoryData).lineCategory, (self.factoryData).resource, (self.factoryData).finalResource)
      end
    end
  end
end
