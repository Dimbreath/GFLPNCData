...
FactoryController.InitBindingData = function(self)
  -- function num : 0_17 , upvalues : CS_GameObject, _ENV, UIN3DFactoryCanvas
  self.OrderDataDic = {}
  self:InitOrderUnlcok()
  self.roomEntityList = {}
  self.roomBind = {}
  local cameraRoot = ((CS_GameObject.Find)("CameraRoot")).transform
  ;
...
FactoryController.TryAddOneOrder = function(self, lindIndex, orderData)
  ...
  elseif orderCfg.type == (FactoryEnum.eOrderType).produce then
    if self.orderData == nil then
      ...
      local usedMat = (self.orderData).usedMat
      local subDic = (self.orderData).assistOrderDic
      local nrealEnergy = self:GetRoomEnegeyByIndex(lindIndex)
      local nowEnergy = nrealEnergy - (self.orderData).usedEnergy
      local couldAdd, arg2 = self:CheckOrderResource(orderData, 1, usedMat, subDic, nowEnergy)
      ...
      if couldAdd then
        ...
        -- DECOMPILER ERROR at PC233: Confused about usage of register: R12 in 'UnsetPending'

        ;
        (self.orderData).usedEnergy = nrealEnergy - arg2
        ...
        if (table.count)(subDic) > 0 then
          ...
          -- DECOMPILER ERROR at PC254: Confused about usage of register: R12 in 'UnsetPending'
    ...
    if nowCount < nowCost then
      ...
      do
        if subOrderCfg ~= nil then
          local needNum = nowCost - nowCount
          ...
          if not couldSub then
            ...
            usedMat[itemId] = (usedMat[itemId] or 0) + nowCount
            nowEnergy = arg2
          end
        else
          do
...
FactoryController.OnInit = function(self)
    ...
    self.productOrderAddDic = nil
    self:InitAllData()
    self.m_CheckUnlockCondiction =
        BindCallback(self, self.CheckUnlockCondiction)
...
FactoryController.OpenFactory = function(self)
    -- function num : 0_2 , upvalues : _ENV, eDynConfigData
    UIManager:DeleteAllWindow();
    ((CS.GSceneManager).Instance):LoadSceneByAB((Consts.SceneName).Factory,
                                                function()
        -- function num : 0_2_0 , upvalues : _ENV, eDynConfigData, self
        ConfigData:LoadDynCfg(eDynConfigData.factory_order)
        self.resloader = ((CS.ResLoader).Create)()
...
FactoryController.CloseFactory = function(self)
    ...
    self.OrderDataDic = nil
    ConfigData:ReleaseDynCfg(eDynConfigData.factory_order);
    ((CS.GSceneManager).Instance):LoadSceneByAB((Consts.SceneName).Main,
                                                function()
        -- function num : 0_3_0 , upvalues : _ENV
        UIManager:HideWindow(UIWindowTypeID.ClickContinue)
        UIManager:ShowWindowAsync(UIWindowTypeID.Home, function(window)
...
FactoryController.SetDefaultCommonLogic =
    function(self)
        -- function num : 0_5 , upvalues : _ENV
        for lineId, cfg in pairs(ConfigData.factory) do
            for index, logic in ipairs(cfg.logic) do
                (PlayerDataCenter.playerBonus):InstallPlayerBonus(
                    ...
                    lineId, logic, (cfg.para1)[index], (cfg.para2)[index],
                    (cfg.para3)[index])
            end
        end
        (PlayerDataCenter.playerBonus):CheckPlayerBonusBroadcast()
...
FactoryController.CheckUnlockCondiction =
    function(self, buildingId)
        ...
        self.unlockedRoom = {}
        self.unlockedCondicton = {}
        local factoryBuildingData =
            ((PlayerDataCenter.AllBuildingData).built)[eBuildingId.OasisFactory]
        if factoryBuildingData == nil then return end
        local levelConfig = factoryBuildingData.levelConfig
        local curLevel = factoryBuildingData.level
        local maxUnlocNum = 0
        for index, logic in ipairs((levelConfig[curLevel]).logic) do
            if logic == eLogicType.FactoryPipelie then
                maxUnlocNum = (math.max)(((levelConfig[curLevel]).para1)[index],
                                         maxUnlocNum)
            end
        end
        for i = 1, maxUnlocNum do
        ...
        do
            for level = curLevel + 1, #levelConfig do
                ...
                for index, logic in ipairs(cfg.logic) do
                    ...
                    if logic == eLogicType.FactoryPipelie and (cfg.para1)[index] <=
                        MAX_ROOM_INDEX and maxUnlocNum < (cfg.para1)[index] and
                        (self.unlockedCondicton)[(cfg.para1)[index]] == 0 then
                        (self.unlockedCondicton)[(cfg.para1)[index]] = level
                    end
                end
            end
...
FactoryController.GetRoomEnegeyByIndex =
    function(self, index)
        -- function num : 0_8 , upvalues : _ENV
        local itemId = ((ConfigData.factory)[index]).cost_item
        local value, nextGenTime =
            (PlayerDataCenter.allEffectorData):GetCurrentARGNum(itemId)
        local ceiling = (PlayerDataCenter.allEffectorData):GetCurrentARGCeiling(
                            itemId)
...
FactoryController.GetRoomEnegeyBaseSpeedByIndex =
    function(self, index)
        -- function num : 0_9 , upvalues : _ENV
        local itemId = ((ConfigData.factory)[index]).cost_item
        local speed = (PlayerDataCenter.allEffectorData):GetCurrentARGSpeed(
...
FactoryController.GetHeroEnterAccrate = function(self, roomIndex, heroIdList)
    ...
    for _, heroId in ipairs(heroIdList) do
        ...
        local rank = heroData.rank
        local starScore = ((ConfigData.star_score)[rank]).score
        local FriendShiplevel = (PlayerDataCenter.allFriendshipData):GetLevel(
                                    heroId)
        accRate = accRate +
                      (starScore + level * (self.factoryEnterArgs)[1] +
                          FriendShiplevel * (self.factoryEnterArgs)[2]) /
                      ((self.factoryEnterArgs)[3] * 1000)
    end
    return accRate
end

FactoryController.GetOneHeroAccrateDetail =
    function(self, roomIndex, heroId)
        -- function num : 0_14 , upvalues : _ENV
        local levelRate = 0
        local friendshipRate = 0
        local RankRate = 0
        ...
        local FriendShiplevel = (PlayerDataCenter.allFriendshipData):GetLevel(
                                    heroId)
        levelRate = level * (self.factoryEnterArgs)[1] / dliverNum
        friendshipRate = FriendShiplevel * (self.factoryEnterArgs)[2] /
                             dliverNum
        RankRate = starScore / dliverNum
        return levelRate, friendshipRate, RankRate
    end

FactoryController.ChangeEnergyGenSpeed =
    function(self, roomIndex, heroIdList)
        -- function num : 0_15 , upvalues : _ENV
        local accRate = self:GetHeroEnterAccrate(roomIndex, heroIdList)
        local itemId = ((ConfigData.factory)[roomIndex]).cost_item;
        (PlayerDataCenter.allEffectorData):SetAccRacte(itemId, accRate)
...
FactoryController.InitBindingData = function(self)
    ...
    self.uiCanvas = (UIN3DFactoryCanvas.New)();
    (self.uiCanvas):Init((self.roomBind).uICanvas);
    (self.uiCanvas):SetClickBackgroundCallback(
        (self.factoryMainUI).m_OnClick3DBG)
    self.__CloseFactory = BindCallback(self, self.CloseFactory);
    ((self.roomBind).factoryToHome):stopped("+", self.__CloseFactory)
    for roomIndex, _ in pairs(self.unlockedRoom) do
        local value, ceiling, speed = self:GetRoomEnegeyByIndex(roomIndex)
        local factoryNode = RedDotController:AddRedDotNodeWithPath(
                                RedDotDynPath.FactoryLine,
                                RedDotStaticTypeId.Main,
                                RedDotStaticTypeId.Factory, roomIndex)
        if ceiling <= value then
            factoryNode:SetRedDotCount(1)
        else
            factoryNode:SetRedDotCount(0)
...
FactoryController.OnUpdateARG = function(self, changedItemNumDic)
    ...
    if self.factoryMainUI ~= nil then
        ...
        for roomIndex, _ in pairs(self.unlockedRoom) do
            local itemId = ((ConfigData.factory)[roomIndex]).cost_item
            if changedItemNumDic == nil or changedItemNumDic[itemId] ~= nil then
                local value, ceiling, speed =
                    self:GetRoomEnegeyByIndex(roomIndex);
                (self.uiCanvas):RefreshRoomEnergy(roomIndex, value, ceiling,
                                                  speed)
                local ok, factoryNode = RedDotController:GetRedDotNode(
                                            RedDotStaticTypeId.Main,
...
FactoryController.InitOrderUnlcok = function(self)
    ...
    for _, orderCfg in pairs(ConfigData.factory_order) do
        ...
        if (self.OrderDataDic)[orderCfg.id] == nil then
            (self.OrderDataDic)[orderCfg.id] =
                {
                    isUnlock = (CheckCondition.CheckLua)(orderCfg.pre_condition,
                                                         orderCfg.pre_para1,
                                                         orderCfg.pre_para2)
                }
        else
            ...
            ((self.OrderDataDic)[orderCfg.id]).isUnlock =
                (CheckCondition.CheckLua)(orderCfg.pre_condition,
                                          orderCfg.pre_para1, orderCfg.pre_para2)
        end
    end
end
...
FactoryController.GetOrders = function(self, type)
    ...
    for _, cfg in pairs(ConfigData.factory_order) do
        if cfg.type == type then
            local efficiencyEnhance =
                (PlayerDataCenter.playerBonus):GetFactoryEfficiency(cfg.id)
            -- DECOMPILER ERROR at PC32: Confused about usage of register: R9 in 'UnsetPending'

            if (self.OrderDataDic)[cfg.id] == nil then
                (self.OrderDataDic)[cfg.id] =
                    {
                        cfg = cfg,
                        efficiencyEnhance = efficiencyEnhance,
                        energyCost = (math.ceil)(
                            cfg.energy_cost * (1 - efficiencyEnhance / 1000))
                    }
            else
                -- DECOMPILER ERROR at PC37: Confused about usage of register: R9 in 'UnsetPending'


                ((self.OrderDataDic)[cfg.id]).cfg = cfg -- DECOMPILER ERROR at PC41: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ((self.OrderDataDic)[cfg.id]).efficiencyEnhance =
                    efficiencyEnhance -- DECOMPILER ERROR at PC52: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ((self.OrderDataDic)[cfg.id]).energyCost =
                    (math.ceil)(cfg.energy_cost * (1 - efficiencyEnhance / 1000))
            end
            (table.insert)(orders, (self.OrderDataDic)[cfg.id])
        end
    end
...
FactoryController.InitRoomEntities = function(self)
    ...
    for index, _ in ipairs(self.unlockedRoom) do
        do
            local obj = nil
            if (self.roomType)[index] == nil or (self.roomType)[index] ==
                (FactoryEnum.eRoomType).dig then
                obj = ((self.roomBind).rooms_dig)[index]
            else
                obj = ((self.roomBind).rooms_normal)[index]
            end
            local roomEntity = (FactoryRoomEntity.New)()
            roomEntity:InitRoomObject(obj, m_OnClickRoom,
                                      (FactoryEnum.eRoomType).dig, index, nil) -- DECOMPILER ERROR at PC35: Confused about usage of register: R9 in 'UnsetPending'
            ;
            (self.roomEntityList)[index] = roomEntity
        end
    end
    for index, unlockLevel in pairs(self.unlockedCondicton) do
        ...
        (self.resloader):LoadABAssetAsync(roomPath, function(prefab)
            -- function num : 0_21_0 , upvalues : self, index, FactoryRoomEntity, m_OnClickRoom, FactoryEnum, unlockLevel
            local obj = prefab:Instantiate((self.roomBind).rooms) -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
            ;
            (obj.transform).position =
                ((((self.roomBind).rooms_normal)[index]).transform).position
            local roomEntity = (FactoryRoomEntity.New)()
            roomEntity:InitRoomObject(obj, m_OnClickRoom,
                                      (FactoryEnum.eRoomType).locked, index,
                                      unlockLevel) -- DECOMPILER ERROR at PC24: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.roomEntityList)[index] = roomEntity
        end)
    end
...
FactoryController.ChangeRoomModelGo = function(self, index, type)
    -- function num : 0_22
    local entity = (self.roomEntityList)[index]
    if entity.type == type then return end -- DECOMPILER ERROR at PC7: Confused about usage of register: R4 in 'UnsetPending'
    
    (self.roomType)[index] = type
    local go = self:GetRoomModelGo(index, type)
    if go ~= nil then entity:ChangeRoomModelGo(go, type) end
end
...
FactoryController.TryAddOneOrder = function(self, lindIndex, orderData)
    ...
    local orderId = orderCfg.id
    local lineEnergy = self:GetRoomEnegeyByIndex(lindIndex)
    if self.orderData ~= nil and
        ((self.orderData).lineIndex ~= lindIndex or (self.orderData).curOrderId ~=
            orderId) then
        error("doesn\'t clean old orderData")
        return false
    end
    if orderCfg.type == (FactoryEnum.eOrderType).dig then
        if self.orderData == nil then
            local warehouseCapacity =
                (PlayerDataCenter.playerBonus):GetWarehouseCapcity(
                    (orderData.cfg).outPutItemId)
            local curwarehouseNum = PlayerDataCenter:GetItemCount(
                                        (orderData.cfg).outPutItemId, false)
            local warehouseNotFull = warehouseCapacity == 0 or
                                         (orderData.cfg).outPutItemNum <=
                                         warehouseCapacity - curwarehouseNum
            local couldAdd = lineEnergy / orderData.energyCost >= 1
            if couldAdd then
                if not warehouseNotFull then
                    return false, (FactoryEnum.eCannotAddReason).warehouseFull
                ...
                self.orderData = {
                    orderType = (FactoryEnum.eOrderType).dig,
                    curOrderId = orderId,
                    curOrderNum = 1,
                    lineIndex = lindIndex,
                    usedEnergy = orderData.energyCost
                }
                return true
            else
                return false, (FactoryEnum.eCannotAddReason).energyInsufficeient
            ...
            local curwarehouseNum = PlayerDataCenter:GetItemCount(
                                        (orderData.cfg).outPutItemId, false)
            local warehouseNotFull = warehouseCapacity == 0 or
                                         ((self.orderData).curOrderNum + 1) *
                                         (orderData.cfg).outPutItemNum <=
                                         warehouseCapacity - curwarehouseNum
            local couldAdd = (lineEnergy - (self.orderData).usedEnergy) /
                                 orderData.energyCost >= 1
            if couldAdd then
                if not warehouseNotFull then
                    return false, (FactoryEnum.eCannotAddReason).warehouseFull
                end -- DECOMPILER ERROR at PC128: Confused about usage of register: R10 in 'UnsetPending'
                
                (self.orderData).curOrderNum = (self.orderData).curOrderNum + 1 -- DECOMPILER ERROR at PC134: Confused about usage of register: R10 in 'UnsetPending'
                ;
                (self.orderData).usedEnergy =
                    orderData.energyCost * (self.orderData).curOrderNum
                return true
            else
                return false, (FactoryEnum.eCannotAddReason).energyInsufficeient
            end
    ...
    elseif orderCfg.type == (FactoryEnum.eOrderType).produce then
        if self.orderData == nil then
            ...
            local subDic = {}
            local nowEnergy = self:GetRoomEnegeyByIndex(lindIndex)
            local couldAdd, arg2 = self:CheckOrderResource(orderData, 1,
                                                           usedMat, subDic,
                                                           nowEnergy)
            if couldAdd then
                self.orderData = {
                    orderType = (FactoryEnum.eOrderType).produce,
                    curOrderId = orderId,
                    curOrderNum = 1,
                    lineIndex = lindIndex,
                    assistOrderDic = subDic,
                    usedEnergy = nowEnergy - arg2,
                    usedMat = usedMat
                }
                if (table.count)(subDic) > 0 then
                    if self.productOrderAddDic == nil then
                        self.productOrderAddDic = {}
                    end -- DECOMPILER ERROR at PC197: Confused about usage of register: R11 in 'UnsetPending'
                    
                    (self.productOrderAddDic)[(self.orderData).curOrderNum] =
                        (table.deepCopy)(self.orderData)
                end
                return true
            else
                return false, arg2
            ...
            if couldAdd then
                (self.orderData).curOrderNum = (self.orderData).curOrderNum + 1 -- DECOMPILER ERROR at PC230: Confused about usage of register: R12 in 'UnsetPending'
                ;
                (self.orderData).assistOrderDic = subDic -- DECOMPILER ERROR at PC233: Confused about usage of register: R12 in 'UnsetPending'
                ;
                (self.orderData).usedEnergy = nrealEnergy - arg2 -- DECOMPILER ERROR at PC235: Confused about usage of register: R12 in 'UnsetPending'
                ;
                (self.orderData).usedMat = usedMat
                if (table.count)(subDic) > 0 then
                    if self.productOrderAddDic == nil then
                        self.productOrderAddDic = {}
                    end -- DECOMPILER ERROR at PC254: Confused about usage of register: R12 in 'UnsetPending'
...
FactoryController.TryMinOneOrder = function(self, lindIndex, orderData)
    ...
    if orderCfg.type == (FactoryEnum.eOrderType).dig then
        if self.orderData == nil or (self.orderData).curOrderNum < 1 then
            ...
            (self.orderData).curOrderNum = (self.orderData).curOrderNum - 1 -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
            ;
            (self.orderData).usedEnergy =
                orderData.energyCost * (self.orderData).curOrderNum
            return true
        ...
        if orderCfg.type == (FactoryEnum.eOrderType).produce then
            ...
            local curNum = (self.orderData).curOrderNum
            if self.productOrderAddDic ~= nil and
                (self.productOrderAddDic)[curNum - 1] ~= nil then
                self.orderData = (table.deepCopy)(
                                     (self.productOrderAddDic)[curNum - 1])
                return true
            else
                -- DECOMPILER ERROR at PC87: Confused about usage of register: R6 in 'UnsetPending'

                if curNum > 0 then
                    (self.orderData).curOrderNum =
                        (self.orderData).curOrderNum - 1 -- DECOMPILER ERROR at PC93: Confused about usage of register: R6 in 'UnsetPending'
                    ;
                    (self.orderData).usedEnergy =
                        orderData.energyCost * (self.orderData).curOrderNum
                    local usedMat = {}
                    for itemId, cost in pairs(orderCfg.raw_material) do
                        usedMat[itemId] = cost * (self.orderData).curOrderNum
                    end -- DECOMPILER ERROR at PC107: Confused about usage of register: R7 in 'UnsetPending'
                    
                    (self.orderData).assistOrderDic = {} -- DECOMPILER ERROR at PC109: Confused about usage of register: R7 in 'UnsetPending'
                    ;
                    (self.orderData).usedMat = usedMat
                    return true
                end
...
FactoryController.TryAddMaxOrder = function(self, lindIndex, orderData)
    ...
    if orderCfg.type == (FactoryEnum.eOrderType).dig then
        ...
        do
            do
                if warehouseCapacity ~= 0 then
                    local num = (warehouseCapacity - curwarehouseNum) //
                                    (orderData.cfg).outPutItemNum
                    if num <= 0 then
                        couldAddNum = 0
                    else
                ...
                if couldAddNum > 0 then
                    self.orderData = {
                        orderType = (FactoryEnum.eOrderType).dig,
                        curOrderId = orderId,
                        curOrderNum = couldAddNum,
                        lineIndex = lindIndex,
                        usedEnergy = orderData.energyCost * couldAddNum
                    }
                    return true
                else
                    return false
                ...
                if orderCfg.type == (FactoryEnum.eOrderType).produce then
                    local couldAddNum = self:GenMaxOrder4Product(orderData,
                                                                 lineEnergy)
                    local usedMat = {}
                    if couldAddNum > 0 then
                        if self.orderData ~= nil and couldAddNum <=
                            (self.orderData).curOrderNum then
                            return false
                        end
                        for itemId, cost in pairs(orderCfg.raw_material) do
                            usedMat[itemId] = cost * couldAddNum
...
FactoryController.SendOrder = function(self, callback)
    -- function num : 0_29
    if self.orderData == nil or (self.orderData).curOrderNum < 1 then return end
    (self.networkCtrl):CS_FACTORY_LinePlaceOrder(self.orderData, callback)
end

FactoryController.CheckOrderResource = function(self, orderData, needNum,
                                                usedMat, subDic, nowEnergy)
    -- function num : 0_30 , upvalues : FactoryEnum, _ENV
    local orderCfg = orderData.cfg
    if orderData.energyCost * needNum <= nowEnergy then
        nowEnergy = nowEnergy - orderData.energyCost * needNum
    ...
    for itemId, cost in pairs(orderCfg.raw_material) do
        ...
        if nowCount < nowCost then
            local subOrderCfg = nil
            for _, orderId in ipairs(
                                  (((ConfigData.factory_order).orderMap)[orderCfg.id]).sudOrderListIds) do
                if ((ConfigData.factory_order)[orderId]).outPutItemId == itemId and
                    ((self.OrderDataDic)[orderId]).isUnlock then
                    subOrderCfg = (ConfigData.factory_order)[orderId]
                    break
                end
            end
            do
                if subOrderCfg ~= nil then
                    local needNum = nowCost - nowCount
                    local couldSub, arg2 =
                        self:CheckOrderResource(
                            (self.OrderDataDic)[subOrderCfg.id], needNum,
                            usedMat, subDic, nowEnergy)
                    if not couldSub then
                        return false, arg2
                    else
                        subDic[subOrderCfg.id] =
                            (subDic[subOrderCfg.id] or 0) + needNum
                        usedMat[itemId] = (usedMat[itemId] or 0) + nowCount
                        nowEnergy = arg2
                    end
                else
                    do
                        do
                            do
                                do
                                    return false,
                                           (FactoryEnum.eCannotAddReason).matInsufficeient
                                end
                                usedMat[itemId] =
                                    (usedMat[itemId] or 0) + nowCost
                                -- DECOMPILER ERROR at PC101: LeaveBlock: unexpected jumping out DO_STMT
...
local COS_45 = (math.cos)(45)
FactoryController.StartMoveRoomToLeftMin =
    function(self, orderUI, roomIndex)
        -- function num : 0_32 , upvalues : _ENV, COS_45
        local cam = (self.roomBind).camera
        local L1x = ((UIManager.UICamera):WorldToScreenPoint(
                        (orderUI.transform).position)).x
        local roomEntity = (self.roomEntityList)[roomIndex]
        local L2 = cam:WorldToScreenPoint((roomEntity.transform).position)
        local targetWPos = cam:ScreenToWorldPoint(
                               (Vector3.New)(L1x / 2, L2.y, 5))
        local cX = targetWPos.x * COS_45 - targetWPos.z * COS_45 -
                       (((roomEntity.transform).position).x * COS_45 -
                           ((roomEntity.transform).position).z * COS_45)
        self.cameraTargetPos = self.cameraDefaultPos +
                                   (Vector3.New)(-cX / (COS_45 * 2), 0,
                                                 cX / (COS_45 * 2))
    end
...
FactoryController.OnMoveRoomToLeftMin = function(self, onMoveCallback, playrate)
    ...
    (((self.roomBind).camera).transform).position =
        (Vector3.Lerp)(self.cameraDefaultPos, self.cameraTargetPos, playrate)
    if onMoveCallback ~= nil then onMoveCallback() end
end
