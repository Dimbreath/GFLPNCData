...
UILottery.OnInit = function(self)
  ...
  self.resultItemPool = (UIItemPool.New)(UILotteryResultItem, (self.ui).resultItemPrefab)
  self.lotteryItemPool = {}
  self.lotteryItems = {}
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_drawOnce, self, self.OnClickDrawOnce)
  ;
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_close, self, self.OnClickCloseButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_closeReselt, self, self.OnClickCloseResultPanel)
  ;
  (UIUtil.AddValueChangedListener)((self.ui).typeFirst, self, self.OnClickTypeFirst)
  ;
  (UIUtil.AddValueChangedListener)((self.ui).typeSecond, self, self.OnClickTypeSecond)
...
UILottery.OnShow = function(self)
  ...
  if self:CheckExpiredAndUpdate(false) then
...
UILottery.EnterLotteryPool = function(self, lotteryPoolId, isClick)
  ...
  if lotteryPoolId == (LotteryEnum.eLotteryPoolType).Main then
    ...
    ;
    ((self.ui).text_LotteryLimt):SetIndex(0, tostring(curNum), tostring(self.dailyLotteryLimtNum))
  else
    do
      ;
      (((self.ui).text_LotteryLimt).gameObject):SetActive(false)
      do
        if not isClick and (lotteryPoolId ~= (LotteryEnum.eLotteryPoolType).Main or not (self.ui).typeFirst) then
          local activeToggle = (self.ui).typeSecond
        end
        activeToggle.isOn = true
        self.lotteryData = (ConfigData.gashapon_para)[self.lotteryId]
        self.cost1Name = (LanguageUtil.GetLocaleText)(((ConfigData.item)[(self.lotteryData).costId1]).name)
        self.cost2Name = (LanguageUtil.GetLocaleText)(((ConfigData.item)[(self.lotteryData).costId2]).name)
        local num1 = PlayerDataCenter:GetItemCount((self.lotteryData).costId1)
        local num2 = PlayerDataCenter:GetItemCount((self.lotteryData).costId2)
        if num1 < (self.lotteryData).costNum1 then
          ((self.ui).drawOnceCost):SetIndex(1, self.cost1Name, (self.lotteryData).costNum1)
        else
          ;
          ((self.ui).drawOnceCost):SetIndex(0, self.cost1Name, (self.lotteryData).costNum1)
        end
        if num2 < (self.lotteryData).costNum2 then
          ((self.ui).drawTenCost):SetIndex(1, self.cost2Name, (self.lotteryData).costNum2)
        else
          ;
          ((self.ui).drawTenCost):SetIndex(0, self.cost2Name, (self.lotteryData).costNum2)
        end
        -- DECOMPILER ERROR at PC136: Confused about usage of register: R5 in 'UnsetPending'

        ;
        ((self.ui).drawOnceCostItemIcon).sprite = CRH:GetSprite(((ConfigData.item)[(self.lotteryData).costId1]).icon)
        -- DECOMPILER ERROR at PC148: Confused about usage of register: R5 in 'UnsetPending'

        ;
        ((self.ui).drawTenCostItemIcon).sprite = CRH:GetSprite(((ConfigData.item)[(self.lotteryData).costId2]).icon)
        -- DECOMPILER ERROR at PC160: Confused about usage of register: R5 in 'UnsetPending'

        ;
        ((self.ui).img_nextTenCostIcon).sprite = CRH:GetSprite(((ConfigData.item)[(self.lotteryData).costId2]).icon)
        -- DECOMPILER ERROR at PC172: Confused about usage of register: R5 in 'UnsetPending'

        ;
        ((self.ui).img_nextOneCosticon).sprite = CRH:GetSprite(((ConfigData.item)[(self.lotteryData).costId1]).icon)
...
UILottery.InitCfg = function(self)
  ...
  if self.enableExcuteFree then
    ...
    self.updateFreeUITimer = (TimerManager:GetTimer(1, BindCallback(self, self.UpdateExcuteFreeUI))):Start()
  end
end
...
UILottery.InitRedDotEvent = function(self, lotteryPoolId)
  ...
  local ok, lotteryMainNode = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Lottery, lotteryPoolId)
  if lotteryMainNode:GetRedDotCount() <= 0 then
    ((self.ui).redDotLotteryMain):SetActive(not ok)
    self.__onRedDotMainPoolChanged = function(node)
    -- function num : 0_6_0 , upvalues : self
    if node:GetRedDotCount() <= 0 then
      ((self.ui).redDotLotteryMain):SetActive(node.nodeId ~= self.lotteryId)
      -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end
  end

    RedDotController:AddListener(RedDotDynPath.LotteryPoolPath, self.__onRedDotMainPoolChanged)
    local ok, lotteryFreeNode = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Lottery, lotteryPoolId, RedDotStaticTypeId.LotteryFree)
    if lotteryFreeNode:GetRedDotCount() <= 0 then
      ...
      self.__onRedDotLotteryFreeChanged = function(node)
    -- function num : 0_6_1 , upvalues : self
    if node:GetRedDotCount() <= 0 then
      ((self.ui).redDotLotteryOnce):SetActive(node:GetParentNodeId() ~= self.lotteryId)
      -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end
  end

      RedDotController:AddListener(RedDotDynPath.LotteryFreePath, self.__onRedDotLotteryFreeChanged)
      local ok, lotteryTenNode = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Lottery, lotteryPoolId, RedDotStaticTypeId.LotteryTen)
      if lotteryTenNode:GetRedDotCount() <= 0 then
        ((self.ui).redDotLotteryTen):SetActive(not ok)
        ;
        ((self.ui).redDotLotteryTen):SetActive(false)
        self.__onRedDotLotteryTenChanged = function(node)
    -- function num : 0_6_2 , upvalues : self
    if node:GetRedDotCount() <= 0 then
      ((self.ui).redDotLotteryTen):SetActive(node:GetParentNodeId() ~= self.lotteryId)
      -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end
  end

        RedDotController:AddListener(RedDotDynPath.LotteryTenPath, self.__onRedDotLotteryTenChanged)
...
UILottery.RemoveRedDotEvent = function(self)
  ...
  RedDotController:RemoveListener(RedDotDynPath.LotteryPoolPath, self.__onRedDotMainPoolChanged)
  RedDotController:RemoveListener(RedDotDynPath.LotteryFreePath, self.__onRedDotLotteryFreeChanged)
  RedDotController:RemoveListener(RedDotDynPath.LotteryTenPath, self.__onRedDotLotteryTenChanged)
end
...
UILottery.OnClickDrawOnce = function(self)
  ...
  if self:CheckExpiredAndUpdate() then
    return 
  end
  local curNum = (PlayerDataCenter.todayLotteryNums)[self.lotteryId] or 0
  if self.lotteryId == (LotteryEnum.eLotteryPoolType).Main and self.dailyLotteryLimtNum - curNum < 1 then
  ...
  if self.enableExcuteFree and (self.lotteryCfg).NextFreeTime <= PlayerDataCenter.timestamp then
    ...
    if self.lotteryId == (LotteryEnum.eLotteryPoolType).Main then
      (PlayerDataCenter.todayLotteryNums)[self.lotteryId] = (PlayerDataCenter.todayLotteryNums)[self.lotteryId] + 1
      ;
      ((self.ui).text_LotteryLimt):SetIndex(0, tostring((PlayerDataCenter.todayLotteryNums)[self.lotteryId]), tostring(self.dailyLotteryLimtNum))
    end
  end
)
  else
    ...
    if self.lotteryId == (LotteryEnum.eLotteryPoolType).Main then
      ...
      ;
      (cs_MessageCommon.ShowMessageTips)(self.cost1Name .. ConfigData:GetTipContent(TipContent.lottery_ItemInsufficient))
      self:ShowQuickPurchaseBox((self.lotteryData).costId1)
    end
  end
  if itemCount - (self.lotteryData).costNum1 < (self.lotteryData).costNum1 then
    ((self.ui).text_CostNextOne):SetIndex(1, self.cost1Name, (self.lotteryData).costNum1)
    ;
    ((self.ui).drawOnceCost):SetIndex(1, self.cost1Name, (self.lotteryData).costNum1)
  else
    ;
    ((self.ui).text_CostNextOne):SetIndex(0, self.cost1Name, (self.lotteryData).costNum1)
    ;
    ((self.ui).drawOnceCost):SetIndex(0, self.cost1Name, (self.lotteryData).costNum1)
  end
  ;
...
UILottery.OnClickDrawTen = function(self)
  ...
  local curNum = (PlayerDataCenter.todayLotteryNums)[self.lotteryId] or 0
  if self.lotteryId == (LotteryEnum.eLotteryPoolType).Main and (ConfigData.game_config).DailyLotteryLimtNum - curNum < 10 then
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.lottery_DailyLimtFull))
    return 
  end
  ...
  if (self.lotteryData).costNum2 <= itemCount then
    ...
    if self.lotteryId == (LotteryEnum.eLotteryPoolType).Main then
      (PlayerDataCenter.todayLotteryNums)[self.lotteryId] = (PlayerDataCenter.todayLotteryNums)[self.lotteryId] + 10
      ;
      ((self.ui).text_LotteryLimt):SetIndex(0, tostring((PlayerDataCenter.todayLotteryNums)[self.lotteryId]), tostring(self.dailyLotteryLimtNum))
    end
  end
    ...
    ;
    (cs_MessageCommon.ShowMessageTips)(self.cost2Name .. ConfigData:GetTipContent(TipContent.lottery_ItemInsufficient))
    self:ShowQuickPurchaseBox((self.lotteryData).costId2)
  end
  if itemCount - (self.lotteryData).costNum2 < (self.lotteryData).costNum2 then
    ((self.ui).text_CostNextTen):SetIndex(1, self.cost2Name, (self.lotteryData).costNum2)
    ;
    ((self.ui).drawTenCost):SetIndex(1, self.cost2Name, (self.lotteryData).costNum2)
  else
    ;
    ((self.ui).text_CostNextTen):SetIndex(0, self.cost2Name, (self.lotteryData).costNum2)
    ;
    ((self.ui).drawTenCost):SetIndex(0, self.cost2Name, (self.lotteryData).costNum2)
  end
  ;
...
UILottery.AutoFlipCards = function(self)
  ...
  if self.isFliping then
    return 
  end
  self.isFliping = true
  self.Flipcoroutine = (cs_coroutine.start)(function()
    ...
    while 1 do
      if self.flipNum ~= nil then
        if self.flipNum > 0 then
          ...
          ;
          (coroutine.yield)(((CS.UnityEngine).WaitForSeconds)(0.35))
...
UILottery.UpdateExcuteFreeUI = function(self)
  ...
  if (self.lotteryCfg).NextFreeTime <= currentTime then
    ...
    if PlayerDataCenter:GetItemCount((self.lotteryData).costId1) < (self.lotteryData).costNum1 then
      ((self.ui).drawOnceCost):SetIndex(1, self.cost1Name, (self.lotteryData).costNum1)
    else
      ;
      ((self.ui).drawOnceCost):SetIndex(0, self.cost1Name, (self.lotteryData).costNum1)
    end
    local remainDuration = TimestampToTime((self.lotteryCfg).NextFreeTime - currentTime)
    ;
...
UILottery.CheckExpiredAndUpdate = function(self, isShowMention)
  ...
  if isShowMention == nil then
    isShowMention = true
  end
  do
    if PlayerDataCenter.lotteryRefreshTimes ~= nil then
      local curExpiredTime = (PlayerDataCenter.lotteryRefreshTimes)[self.lotteryId]
      if curExpiredTime ~= nil and curExpiredTime > 0 and curExpiredTime < PlayerDataCenter.timestamp then
