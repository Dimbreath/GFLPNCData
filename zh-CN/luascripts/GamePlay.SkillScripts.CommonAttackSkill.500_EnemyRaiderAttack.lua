...
bs_500.PlaySkill = function(self, passdata)
    ...
    if bewitch ~= self.lastBewitch then
        ...
        if ((self.caster).recordTable).lastAttackRole ~= nil and
            (self.lastAttackRole).belongNum ~= eBattleRoleBelong.neutral and
    ...
    if targetList.Count ~= 0 then
        ...
        self.lastAttackRole = (targetList[0]).targetRole -- DECOMPILER ERROR at PC111: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.caster).recordTable).lastAttackRole = self.lastAttackRole;
        (self.caster):LookAtTarget(self.lastAttackRole)
        if LuaSkillCtrl:IsAbleAttackCheckExcludedDir(self.caster,
