  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.LotteryWindow, function(win)
    ...
    if win ~= nil then
      if not isNormal then
        (((win.ui).typeFirst).onValueChanged):Invoke(true)
        -- DECOMPILER ERROR at PC13: Confused about usage of register: R1 in 'UnsetPending'

        ;
        ((win.ui).typeFirst).isOn = true
        ...
        ;
        (((win.ui).typeSecond).onValueChanged):Invoke(true)
        -- DECOMPILER ERROR at PC26: Confused about usage of register: R1 in 'UnsetPending'

        ;
        ((win.ui).typeFirst).isOn = false
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.Achievement, function(win)
    ...
    for key,value in pairs(win.achivTypeToggleDic) do
      if key == achievementTypeID then
        (((value.ui).toggle).onValueChanged):Invoke(true)
...
JumpManager.Jump2OasisBuilding = function(self, jumpOverCallback, argList)
  ...
  local oasisController = ControllerManager:GetController(ControllerTypeId.OasisController, false)
