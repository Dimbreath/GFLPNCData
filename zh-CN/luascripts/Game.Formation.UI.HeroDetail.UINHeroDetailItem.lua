...
UINHeroDetailItem.OnInit = function(self)
    ...
    self.skillItemPool = (UIItemPool.New)(UINBaseSkillItem,
...
UINHeroDetailItem.InitFmtHeroDetailItem =
    function(self, fmtIndex, heroId, fmtHeroDetail, resloader, lockStr,
             clickEvent)
        -- function num : 0_1 , upvalues : _ENV, eFmtHeroDetailState, UINHeroCardItem, UINHeroAttributeNode
        ...
        if not (string.IsNullOrEmpty)(lockStr) then
            self.heroDetailState = eFmtHeroDetailState.Lock;
            ((self.ui).lock):SetActive(true) -- DECOMPILER ERROR at PC35: Confused about usage of register: R7 in 'UnsetPending'
            ;
            ((self.ui).tex_Condition).text = lockStr
        else
            if heroId == nil then
                self.heroDetailState = eFmtHeroDetailState.Empty;
                ((self.ui).empty):SetActive(true)
            else
                self.heroDetailState = eFmtHeroDetailState.HasHero;
                ((self.ui).hero):SetActive(true)
                local heroData = PlayerDataCenter:GetHeroData(heroId)
                if heroData == nil then return end
                self.heroData = heroData
                local fightingPower = heroData:GetFightingPower() -- DECOMPILER ERROR at PC69: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ...
                local attrDataList = {}
                for k, attrId in ipairs(
                                     (ConfigData.game_config).FormationDetailAttr) do
                    attrDataList[k] = {
                        attrId = attrId,
...
UINHeroDetailItem.GetFmtHeroDetailState =
    function(self)
        -- function num : 0_3
        return self.heroDetailState
    end

UINHeroDetailItem.GetFmtHeroDetailIndex =
    function(self)
        -- function num : 0_4
        return self.fmtIndex
    end

UINHeroDetailItem.EnableFmtHeroRaycast =
...
UINHeroDetailItem.SetHeroDetailItemDragEvent =
...
UINHeroDetailItem.DragEndTweenHeroDetailItem =
...
UINHeroDetailItem.GetHeroDetailItemRootPos =
    function(self)
        -- function num : 0_12
        if not self.__rootPos then return (self.transform).position end
    end
