...
UIAthDetail.ShowAthDetailMain = function(self, athData, efficiency, heroId,
                                         replaceEvent, uninstallEvent,
                                         closeEvent)
    ...
    (self.detailMain):InitAthDetailNode(athData, efficiency, heroId,
                                        replaceEvent, uninstallEvent);
...
UIAthDetail.CloseAthDetailMain = function(self)
    -- function num : 0_3
    if self.detailMain ~= nil then (self.detailMain):OnClickClose() end
end

UIAthDetail.ShowAthDetailSub = function(self, athData, efficiency, heroId,
                                        installEvent, closeEvent)
...
UIAthDetail.CloseAthDetailSub = function(self)
    -- function num : 0_5
    if self.detailSub ~= nil then (self.detailSub):OnClickClose() end
end
...
UIAthDetail.RefreshButtonState = function(self)
    ...
    if self.detailSub ~= nil and self.detailMain ~= nil and
        (self.detailSub).active and (self.detailMain).active then
        isInstall = false
    end
...
UIAthDetail.LockAth = function(self, uid)
    ...
    if self.__onLockAthCompleteEvent == nil then
        self.__onLockAthCompleteEvent = BindCallback(self,
                                                     self.OnLockAthComplete)
    end
...
UIAthDetail.OnLockAthComplete = function(self)
    -- function num : 0_9
    if self.detailMain ~= nil then (self.detailMain):RefreshAthLock() end
    if self.detailSub ~= nil then (self.detailSub):RefreshAthLock() end
end

UIAthDetail.OnDelete = function(self)
    -- function num : 0_10 , upvalues : base
    if self.detailMain ~= nil then (self.detailMain):Delete() end
    if self.detailSub ~= nil then (self.detailSub):Delete() end
    (base.OnDelete)(self)
end
