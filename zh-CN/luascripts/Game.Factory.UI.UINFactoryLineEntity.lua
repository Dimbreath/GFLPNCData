...
local UINFactoryLineEntityMat = require("Game.Factory.UI.UINFactoryLineEntityMat")
UINFactoryLineEntity.OnInit = function(self)
  ...
  self.matItemPool = (UIItemPool.New)(UINFactoryLineEntityMat, (self.ui).materialItem)
  self.lineMatItems = {}
end

UINFactoryLineEntity.InitFactoryLineEntity = function(self, lineData, factoryLine, allResource, count)
  ...
  if lineData.isSetCycle == true then
    ...
    ;
    ((self.ui).tex_cycle):SetIndex(0, tostring(lineData.presetCycle - lineData.curCycle), tostring(lineData.presetCycle))
  else
    ;
  ...
  ;
  ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)((factoryLine.cfg).name)
  ...
  ;
  ((self.ui).tex_sdate):SetIndex(lineData.state)
  -- DECOMPILER ERROR at PC71: Confused about usage of register: R5 in 'UnsetPending'

  ;
  ((self.ui).img_Quality).color = ItemQualityColor[(factoryLine.itemCfg).quality]
  ;
  ...
  for k,consumeId in ipairs((factoryLine.cfg).consumeIds) do
    ...
    if itemCfg ~= nil then
      ...
      lineMatItem:InitLineEntityResItem(itemCfg, allResource[consumeId] or 0)
...
UINFactoryLineEntity.RefreshFactoryLine = function(self)
  -- function num : 0_2
