...
UIGuide.ShowGuideAniInfo = function(self, info, info_type, info_pos, isFirst,
                                    step_type)
    ...
    local fitSize = (math.clamp)(sizeDeltaMax, (self.ui).arrowNodeMin,
                                 (self.ui).arrowNodeMax)
    ...
    if step_type == (GuideEnum.StepType).Operate then
        (((self.ui).arrowNode).transform).localPosition = position -- DECOMPILER ERROR at PC36: Confused about usage of register: R11 in 'UnsetPending'
        ;
        (((self.ui).arrowNode).transform).sizeDelta = fitSizeDelta
        if isFirst then
            (((self.ui).arrowNode).gameObject):SetActive(true);
            (((self.ui).ani_TargetArea).gameObject):SetActive(true) -- DECOMPILER ERROR at PC58: Confused about usage of register: R11 in 'UnsetPending'
            ;
            (((self.ui).ani_TargetArea).transform).sizeDelta =
                fitSizeDelta - ((self.ui).ani_TargetArea).endValueV2;
    ...
    if (string.IsNullOrEmpty)(info) then return end
    ...
    if info_type > 0 then
        self:ShowHeroSmallTalk(info, info_type, info_pos, position, arrowOffset,
                               offsetRatio)
    else
...
UIGuide.ShowSmallDialogNode = function(self, info, position, arrowOffset,
                                       offsetRatio)
    -- function num : 0_6 , upvalues : _ENV
    if (string.IsNullOrEmpty)(info) then return end
    local dialogOffset = arrowOffset + (Vector2.New)(
                             ((self.smallDialogSize).x / 2 +
                                 ((self.ui).arrowDialogOffset).x) *
                                 offsetRatio.x,
                             ((self.ui).arrowDialogOffset).y * offsetRatio.y) -- DECOMPILER ERROR at PC34: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).smallDialogNode).localPosition =
        position + (Vector3.New)(dialogOffset.x, dialogOffset.y, 0);
    (((self.ui).smallDialogNode).gameObject):SetActive(true) -- DECOMPILER ERROR at PC43: Confused about usage of register: R6 in 'UnsetPending'
    ;
...
UIGuide.ShowHeroSmallTalk = function(self, info, info_type, info_pos, position,
                                     arrowOffset, offsetRatio)
    ...
    if heroCfg ~= nil then
        ...
        if resCfg ~= nil then
            ...
            (self.resloader):LoadABAssetAsync(picPath, function(texture)
                ...
                if self.__talkHeroId ~= info_type then return end -- DECOMPILER ERROR at PC7: Confused about usage of register: R1 in 'UnsetPending'
    ...
    do
        ...
        if not (string.IsNullOrEmpty)(info_pos) then
            ...
            if not IsNull(childNode) then
                ((self.ui).talkDialogNode).localPosition =
                    childNode.localPosition
            end
        else
            do
                local xOffset = 0
                ...
                do
                    local dialogOffset =
                        arrowOffset +
                            (Vector2.New)(
                                (xOffset + ((self.ui).arrowDialogOffset).x) *
                                    offsetRatio.x,
                                ((self.ui).arrowDialogOffset).y * offsetRatio.y) -- DECOMPILER ERROR at PC91: Confused about usage of register: R10 in 'UnsetPending'
                    ;
                    ((self.ui).talkDialogNode).localPosition =
                        position +
                            (Vector3.New)(dialogOffset.x, dialogOffset.y, 0);
...
UIGuide.ShowGuideLargeDialog = function(self, info, info_type, info_pos,
                                        nextAction)
    -- function num : 0_10 , upvalues : _ENV, GuideEnum
    ((self.ui).panelDialog):SetActive(true) -- DECOMPILER ERROR at PC7: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).tex_LargeDialog).text = nil
    if (string.IsNullOrEmpty)(info) then info = "" end
...
UIGuide.ShowGuideHeroSmallTalk = function(self, info, info_type, info_pos,
                                          nextAction)
    -- function num : 0_11 , upvalues : _ENV, GuideEnum
    if (string.IsNullOrEmpty)(info) then info = "" end -- DECOMPILER ERROR at PC12: Confused about usage of register: R5 in 'UnsetPending'
...
UIGuide.__ShowBigHeroPic = function(self, info_type, info_pos)
    ...
    if not info_type then info_type = 0 end
    if info_type > 0 then
...
UIGuide.OnBtnContinueClicked = function(self)
    ...
    if self.step_type == (GuideEnum.StepType).LargeDialog then
        ...
        if isPlaying then
            ...
            if self.nextAction ~= nil then (self.nextAction)() end
        end
    else
        do
            -- DECOMPILER ERROR at PC32: Unhandled construct in 'MakeBoolean' P1

            if self.step_type == (GuideEnum.StepType).HeroSmallTalk and
                self.nextAction ~= nil then (self.nextAction)() end
            if self.step_type == (GuideEnum.StepType).Highlight and
                self.nextAction ~= nil then (self.nextAction)() end
        end
    end
end
...
UIGuide.OnSkipClicked = function(self)
    -- function num : 0_16
    if self.__skipEvent ~= nil then (self.__skipEvent)() end
end
...
UIGuide.CloseGuide = function(self, step_type)
    ...
    if step_type == (GuideEnum.StepType).LargeDialog then
        ...
        if step_type == (GuideEnum.StepType).Operate or step_type ==
            (GuideEnum.StepType).Highlight then
...
UIGuide.PlaySlideGuide = function(self, startUIPos, endUIPos)
    ...
    lineSizeDelta.x = (Vector3.Distance)(startUIPos, endUIPos) -- DECOMPILER ERROR at PC13: Confused about usage of register: R4 in 'UnsetPending'
    ;
    (((self.ui).img_SlideLine).transform).sizeDelta = lineSizeDelta -- DECOMPILER ERROR at PC17: Confused about usage of register: R4 in 'UnsetPending'
    ;
    (((self.ui).img_SlideLine).transform).localPosition = startUIPos
    local angle = (((CS.UnityEngine).Mathf).Atan2)(endUIPos.y - startUIPos.y,
                                                   endUIPos.x - startUIPos.x) *
                      ((CS.UnityEngine).Mathf).Rad2Deg -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (((self.ui).img_SlideLine).transform).localEulerAngles =
        (Vector3.New)(0, 0, angle)
    local offsetY = ((((self.ui).img_SlideArrows).transform).sizeDelta).y / 4
    startUIPos.y = startUIPos.y - offsetY
    endUIPos.y = endUIPos.y - offsetY -- DECOMPILER ERROR at PC59: Confused about usage of register: R6 in 'UnsetPending'
    ;
    (((self.ui).img_SlideArrows).transform).localPosition = startUIPos -- DECOMPILER ERROR at PC62: Confused about usage of register: R6 in 'UnsetPending'
    ;
...
UIGuide.PlayHandClickGuide = function(self, startUIPos)
    ...
    (((self.ui).handClick).transform).localPosition =
        (Vector3.New)(startUIPos.x, startUIPos.y, 0);
