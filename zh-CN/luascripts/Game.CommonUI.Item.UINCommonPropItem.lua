...
UINCommonPropItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).commonPropItem, self,
                               self.OnPropItemClicked)
end

UINCommonPropItem.InitPropItem = function(self, resloader, itemCfg, itemCount)
    -- function num : 0_1 , upvalues : _ENV
    self.itemCfg = itemCfg -- DECOMPILER ERROR at PC7: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).tex_PropName).text = (LanguageUtil.GetLocaleText)(itemCfg.name);
    ((self.ui).propNum):SetActive(itemCount == nil and itemCount ~= nil)
    -- DECOMPILER ERROR at PC25: Confused about usage of register: R4 in 'UnsetPending'
    ...
    local action_type = itemCfg.action_type
    if action_type == eItemActionType.HeroCard or action_type ==
        eItemActionType.HeroCardFrag then
        local heroId = (itemCfg.arg)[1];
        ((self.ui).propItem):SetActive(false);
        ((self.ui).heroItem):SetActive(true);
        ((self.ui).img_Frag):SetActive(action_type ==
                                           eItemActionType.HeroCardFrag)
        local heroCfg = (ConfigData.hero_data)[heroId]
        ...
        if heroResCfg ~= nil then
            resloader:LoadABAssetAsync(PathConsts:GetCharacterPicPath(
                ...
                if self.itemCfg ~= itemCfg then return end -- DECOMPILER ERROR at PC13: Confused about usage of register: R1 in 'UnsetPending'
        ...
        ((self.ui).img_HeroPic).texture = nil;
        ((self.ui).propItem):SetActive(true);
        ((self.ui).heroItem):SetActive(false) -- DECOMPILER ERROR at PC96: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.ui).img_ItemPic).sprite = CRH:GetSprite(itemCfg.icon)
    end
    -- DECOMPILER ERROR: 7 unprocessed JMP targets
