...
bs_10018.config = {
    ...
    hpRecover = "$1-max(GetSkillArg(1),50)/1000*@maxHp"
}
...
bs_10018.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10018_3", 1,
                    self.OnAfterHurt)
    self.heal_config = {}
end

bs_10018.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    -- DECOMPILER ERROR at PC28: Confused about usage of register: R8 in 'UnsetPending'

    if target == self.caster and (self.caster).hp > 0 and
        LuaSkillCtrl:CallFormulaBoolWithSkill((self.config).conditionCheck,
                                              self.caster, self.caster, self,
                                              hurt) then
        (self.heal_config).heal_number =
            LuaSkillCtrl:CallFormulaNumberWithSkill((self.config).hpRecover,
                                                    self.caster, self.caster,
                                                    self, hurt) -- DECOMPILER ERROR at PC32: Confused about usage of register: R8 in 'UnsetPending'
        ;
