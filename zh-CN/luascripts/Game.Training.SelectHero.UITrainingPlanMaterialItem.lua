...
local UITrainingPlanMaterialItem = class("UITrainingPlanMaterialItem",
                                         UIBaseNode)
...
UITrainingPlanMaterialItem.InitMatItem =
    function(self, resloader, id, time)
        ...
        if time < 2 then
            ...
            if time == 2 then
                ((self.ui).img_framePic).color =
                    HeroRareColor[eHeroCardRareType.SR]
            else
                -- DECOMPILER ERROR at PC28: Confused about usage of register: R4 in 'UnsetPending'


                ((self.ui).img_framePic).color =
                    HeroRareColor[eHeroCardRareType.SSR]
            end
        end
        local minutes = time
        ...
        if hours == 0 then
            ...
            if remainSeconds == 0 then
                ...
                ((self.ui).tex_Time):SetIndex(2, tostring(hours),
                                              tostring(remainSeconds))
            end
        end
    end
...
UITrainingPlanMaterialItem.__OnClickRootButton =
    function(self)
        -- function num : 0_3
