...
LayerMask.GetMask = function(...)
    ...
    for i = 1, #arg do
        local n = (LayerMask.NameToLayer)(arg[i])
        if n ~= nil then value = value + 2 ^ n end
    end
    return value
end

LayerMask.unity_layermask = (CS.UnityEngine).LayerMask -- DECOMPILER ERROR at PC25: Confused about usage of register: R5 in 'UnsetPending'
;
(CS.UnityEngine).LayerMask = LayerMask
setmetatable(LayerMask, LayerMask)
return LayerMask
