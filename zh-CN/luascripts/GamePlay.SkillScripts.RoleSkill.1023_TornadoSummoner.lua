...
bs_1023.config = {
    ...
    effectIdTornado1 = 10007,
    effectIdTornado2 = 10005,
...
bs_1023.PlaySkill = function(self, data)
    ...
    if gridTarget ~= nil then
        local feng = ((self.caster).auSource):PlayAudioById(
        ...
        do
            LuaSkillCtrl:StartTimer(self, 5, function()
                ...
                self.effect = LuaSkillCtrl:CallEffect(gridTarget,
                                                      (self.config).effectIdTornado2,
                ...
                LuaSkillCtrl:CallAddCircleColliderForEffect(self.effect,
                                                            (self.config).effetcIdTornadoRadius,
            ...
            LuaSkillCtrl:StartTimer(nil, (self.arglist)[3], function()
                -- function num : 0_2_1 , upvalues : self, feng, _ENV
                if self.effect ~= nil then
                    (self.effect):Die()
                    self.effect = nil
                    if feng ~= nil then
                        AudioManager:StopAudioByBack(feng)
                    end
                end
            end)
        end
...
bs_1023.OnCollisionStay = function(self, role)
    ...
    if (LuaSkillCtrl.battleCtrl).frame == self.colliDetectTime then
        ...
        if (self.arglist)[2] - 1 <= (LuaSkillCtrl.battleCtrl).frame -
            self.colliDetectTime then
