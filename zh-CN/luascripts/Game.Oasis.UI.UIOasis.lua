...
UIOasis.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_CancleHideUI, self, self.DisplayScreenUI)
  self.isScreenUIShow = true
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_editMode, self, self.OnClickEditModeButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_sector, self, self.OnClickSectorButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_StationedOV, self, self.OnStationedOVClick)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_sidebar, self, self.OnClickSidebarButton)
...
UIOasis.Initialize = function(self, buildingUIModel, sidleLoadedCallback)
  ...
  ;
  (self.sidleUI):InitializeSidle(self.buildingUIModel, self.resLoader, sidleLoadedCallback)
  -- DECOMPILER ERROR at PC27: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).OpenSidleEvent = BindCallback(self, self.OnClickSidebarButton)
  -- DECOMPILER ERROR at PC30: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).NewBuildingEvent = self.NewBuildingEvent
  -- DECOMPILER ERROR at PC33: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).BuildingLevelUpEvent = self.BuildingLevelUpEvent
  -- DECOMPILER ERROR at PC36: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).ConstructEvent = self.ConstructQuest
  -- DECOMPILER ERROR at PC39: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).UpdrageEvent = self.UpdrageQueset
  -- DECOMPILER ERROR at PC42: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).ConfirmOverEvent = self.ConfirmOverQuest
  -- DECOMPILER ERROR at PC45: Confused about usage of register: R3 in 'UnsetPending'

  ;
  (self.sidleUI).AccelerateEvent = self.AccelerateEvent
...
UIOasis.UpdateAllOasisAreaLock = function(self, oasisAreaList)
  ...
  for areaId,area in ipairs(oasisAreaList) do
    ...
    if lockItem == nil then
      lockItem = (self.areaLockPool):GetOne()
...
UIOasis.__onCollectResClick = function(self, buildProduction)
  ...
  if self.CollectAllRes ~= nil then
    local collectResCallBack = BindCallback(self, self.__onCollectResCompleted, buildProduction)
    ;
  ...
  for id,resTable in pairs(buildProduction) do
    ...
    if resTable ~= nil and resCount > 0 then
      for i = 1, resCount do
        ...
        if item ~= nil then
          local msg = ConfigData:GetTipContent(TipContent.Building_GainReward) .. (LanguageUtil.GetLocaleText)(item.name) .. " x " .. tostring((resTable[i]).count)
          ;
...
UIOasis.Update = function(self, timestamp, isSecond)
  -- function num : 0_7
  if self.sidleUI ~= nil then
    (self.sidleUI):Update(timestamp, isSecond)
  end
end
...
UIOasis.ShowDetail = function(self, id)
  ...
  if item ~= nil then
    (self.sidleUI):__createDetailItem(item)
  end
end
...
UIOasis.InitRedDotOasisEvent = function(self)
  ...
  local ok, node = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Oasis)
  if node:GetRedDotCount() <= 0 then
    ((self.ui).redDotSideBar):SetActive(not ok)
    self.__oasisBuildableRedDotEvent = function(node)
    -- function num : 0_13_0 , upvalues : self
    ((self.ui).redDotSideBar):SetActive(node:GetRedDotCount() > 0)
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
  end

    RedDotController:AddListener(RedDotDynPath.OasisBuildListPath, self.__oasisBuildableRedDotEvent)
...
UIOasis.RemoveRedDotOasisEvent = function(self)
  ...
  RedDotController:RemoveListener(RedDotDynPath.OasisBuildListPath, self.__oasisBuildableRedDotEvent)
end
...
UIOasis.InitUnlockFunction = function(self)
  ...
  self.sStationedUnlock = funcUnLockCrtl:ValidateUnlock(proto_csmsg_SystemFunctionID.SystemFunctionID_HeroEnter)
...
UIOasis.OnClickEditModeButton = function(self)
  -- function num : 0_19
  if self.EditModeEvent ~= nil then
    (self.EditModeEvent)()
  end
end
...
UIOasis.OnClickSectorButton = function(self)
  ...
  ((CS.GSceneManager).Instance):LoadSceneAsyncByAB((Consts.SceneName).Sector, function()
    ...
    if ExplorationManager:HasUncompletedEp() then
      ...
      ;
      (ControllerManager:GetController(ControllerTypeId.SectorController, true)):SetFrom(AreaConst.Oasis)
    end
  end
)
end
...
UIOasis.UpdateProcessEvent = function(self, id, progress, remainTimeText, waitConfirmOver, second)
  -- function num : 0_24
  (self.sidleUI):UpdateProcessEvent(id, progress, remainTimeText, waitConfirmOver, second)
end
...
UIOasis.DisplayScreenUI = function(self)
  ...
  if self.isScreenUIShow then
    ...
    if not (self.sidleUI).openedDetail then
      ((self.ui).uiHideCanvas).blocksRaycasts = false
    ...
    if queueWin ~= nil and queueWin.active and not (self.sidleUI).openedDetail then
      queueWin:Hide()
    end
  else
    ...
    self.isScreenUIShow = true
    ;
    (((self.ui).btn_CancleHideUI).gameObject):SetActive(false)
    -- DECOMPILER ERROR at PC58: Confused about usage of register: R2 in 'UnsetPending'

    ;
    ((CS.OasisCameraController).Instance).AbandonScale = false
    if queueWin ~= nil and not queueWin.active and not (self.sidleUI).openedDetail then
      queueWin:Show()
    end
  end
end
...
UIOasis.OnDelete = function(self)
  ...
  MsgCenter:RemoveListener(eMsgEventId.BuildingProduceUpdate, self.updateProduce)
