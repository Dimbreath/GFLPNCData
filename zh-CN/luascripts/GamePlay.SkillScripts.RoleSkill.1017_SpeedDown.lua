...
bs_1017.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1017_3", 1,
                    self.OnAfterHurt)
end

bs_1017.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    ...
    if sender == self.caster and target ~= self.caster and
        ...
        LuaSkillCtrl:CallBuff(self, target, (self.config).buffId,
                              (self.config).buffTier, (self.arglist)[3])
    end
end
