...
bs_1055.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1055_3", 1, self.OnAfterHurt)
end
...
bs_1055.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
  -- function num : 0_4 , upvalues : _ENV
  if sender == self.caster and not isMiss and skill.dataId == 1055 and not isRealDmg and target.belongNum ~= (self.caster).belongNum then
    ...
    for i = 0, (skillResult.roleList).Count - 1 do
      ...
      LuaSkillCtrl:CallRealDamage(self, role, nil, (self.config).real_Config, {realHurt}, true)
    end
    skillResult:EndResult()
  end
end
...
bs_1055.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1055_3", 1,
                    self.OnAfterHurt)
end
...
bs_1055.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_4 , upvalues : _ENV
    if sender == self.caster and not isMiss and skill.dataId == 1055 and
        not isRealDmg and target.belongNum ~= (self.caster).belongNum then
        local realHurt = hurt * (self.arglist)[2] // 1000
        local skillResult = LuaSkillCtrl:CallSkillResultNoEffect(self, target,
                                                                 (self.config).Aoe)
        for i = 0, (skillResult.roleList).Count - 1 do
            ...
            LuaSkillCtrl:CallRealDamage(self, role, nil,
                                        (self.config).real_Config, {realHurt},
                                        true)
        end
        skillResult:EndResult()
    end
