...
UICommonItem.InitItem = function(self, itemCfg, num, resloader)
    ...
    if num == nil then
        ...
        ((self.ui).Obj_numImage):SetActive(true) -- DECOMPILER ERROR at PC16: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).text_numText).text = num
    end
    local action_type = itemCfg.action_type
    if action_type == eItemActionType.HeroCard or action_type ==
        ...
        local heroResCfg = (ConfigData.resource_model)[heroCfg.src_id]
        if heroResCfg ~= nil then
            if resloader == nil then return end
        ...
        do
            ...
            ((self.ui).img_itemImage).sprite =
                CRH:GetSprite((self.itemCfg).icon) -- DECOMPILER ERROR at PC76: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).qualityImage).color = ItemQualityColor[itemCfg.quality]
        end
    end
