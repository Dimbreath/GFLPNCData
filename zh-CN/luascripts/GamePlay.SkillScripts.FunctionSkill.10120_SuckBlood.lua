...
bs_10120.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_10120_1", 1,
                    self.OnAfterBattleStart)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10120_3", 1,
                    self.OnAfterHurt)
    self:AddTrigger(eSkillTriggerType.SetHurt, "bs_10120_2", 1, self.OnSetHurt)
end
...
bs_10120.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_3 , upvalues : _ENV
    if sender == self.caster then
        LuaSkillCtrl:CallEffect(self.caster, (self.config).effectId, self)
