...
UIDungeonResult.CompleteDungeon = function(self, rewardMsg, resultData, mvpGrade, dungeonType, ATHRewardInfo)
...
UIDungeonResult.__InitBattleReward = function(self, rewardDic)
  ...
  for id,num in pairs(rewardDic) do
    do
      if (id >= 8000 and id < 9000) or ((ConfigData.item)[id]).type == eItemType.Arithmetic then
        hasRandomAth = true
      else
...
UIDungeonResult.CompleteDungeon = function(self, rewardMsg, resultData,
                                           mvpGrade, dungeonType, ATHRewardInfo)
    -- function num : 0_2 , upvalues : _ENV
    self._auBack = AudioManager:PlayAudioById(3009, function()
        -- function num : 0_2_0 , upvalues : self
        self._auBack = nil
...
UIDungeonResult.__InitBattleReward = function(self, rewardDic)
    ...
    for id, num in pairs(rewardDic) do
        do
            if (id >= 8000 and id < 9000) or ((ConfigData.item)[id]).type ==
                eItemType.Arithmetic then
                hasRandomAth = true
            else
                local itemCfg = (ConfigData.item)[id]
                if itemCfg == nil then
    ...
    if hasRandomAth then
        ...
        do
                ...
                if v.isAth then
                    rewardItem:InitItemWithCount(v.itemCfg, v.count, function()
                        ...
                        UIManager:ShowWindowAsync(
                            UIWindowTypeID.GlobalItemDetail, function(win)
                                -- function num : 0_4_1_0 , upvalues : v
                                if win ~= nil then
                                    win:InitAthDetail(v.itemCfg, v.athData)
                                end
            ...
            if not hasReward or not (Color.New)(1, 1, 1, 0.9) then
                ...
                if (self.ATHRewardInfo).haveDouble <= 0 then
                    showDouble =
                        not self.dungeonType or self.dungeonType ~= 12 or
                            not self.ATHRewardInfo
                    if showDouble then
                        ((self.ui).tex_DropCount):SetIndex(0, tostring(
                                                               (self.ATHRewardInfo).haveDouble -
                                                                   1), tostring(
                                                               (self.ATHRewardInfo).double))
                    end
                    ((self.ui).obj_DropUPTips):SetActive(showDouble);
                    ((self.ui).obj_DropCount):SetActive(showDouble)
                    -- DECOMPILER ERROR: 5 unprocessed JMP targets
...
UIDungeonResult.__InitCharacterExp = function(self, lastHeroList)
    ...
    for k, dynHero in ipairs(heroList) do
        ...
        heroItem:InitCharacterItem(dynHero, self.resloader, nil)
        heroItem:RefershExpData(lastHeroList[k - 1]) -- DECOMPILER ERROR at PC31: Confused about usage of register: R11 in 'UnsetPending'
        ;
        (self.heroHeadDic)[k] = heroItem
        local oldLevel = (lastHeroList[k - 1]).level
        if dynHero:GetLevel() < oldLevel then
