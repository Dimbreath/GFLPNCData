...
local base = UIBaseWindow
local ExplorationEnum = require("Game.Exploration.ExplorationEnum")
local UINDungeonInfoChipDetail = require(
                                     "Game.CommonUI.DungeonState.Info.UINDungeonInfoChipDetail")
local UINDungeonInfoHeroDetail = require(
                                     "Game.CommonUI.DungeonState.Info.UINDungeonInfoHeroDetail")
local UINBattleGirdInfoNode = require(
                                  "Game.Battle.UI.Grid.UINBattleGirdInfoNode")
local CS_ResLoader = CS.ResLoader
UIDungeonInfoDetail.OnInit = function(self)
    -- function num : 0_0 , upvalues : CS_ResLoader, UINDungeonInfoChipDetail, UINDungeonInfoHeroDetail, UINBattleGirdInfoNode, _ENV
    self.resloader = (CS_ResLoader.Create)()
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_background, self, self.OnClickBlank);
    (UIUtil.AddButtonListener)((self.ui).btn_Left, self, self.OnClickLeftButton);
    (UIUtil.AddButtonListener)((self.ui).btn_Right, self,
                               self.OnClickRightButton)
end

UIDungeonInfoDetail.ShowChipDetail = function(self, chipDataList, index,
                                              DeselectAllItemCallBack,
                                              SelectItemCallBack)
    -- function num : 0_1 , upvalues : _ENV
    if self.isShowingHeroDetail then
...
UIDungeonInfoDetail.HideChipDetail = function(self)
    ...
    self.isShowingChipDetail = false;
    (self.chipDetailNode):Hide()
    if self.deselectChipCallBack ~= nil then (self.deselectChipCallBack)() end
end
...
UIDungeonInfoDetail.ShowHeroDetail = function(self, dynHeroData,
                                              deselectCallBack, switchBack)
    -- function num : 0_4 , upvalues : _ENV
    if self.isShowingChipDetail then
        self:OnClickRetreat()
        deselectCallBack()
...
UIDungeonInfoDetail.HideHeroDetail = function(self)
    ...
    (self.heroDetailNode):Hide()
    self.isShowingHeroDetail = false
    if self.deselectHeroCallBack ~= nil then (self.deselectHeroCallBack)() end
end
...
UIDungeonInfoDetail.__ShowHeroEffctGrid =
    function(self, dynHeroData)
        -- function num : 0_7 , upvalues : _ENV
        local needShowGridInfo = false
        if ((CS.BattleManager).Instance).IsInBattle then
            local battleCtrl = ((CS.BattleManager).Instance).CurBattleController
            local effectGrid = (battleCtrl.EfcGridController):GetHeroEffectGrid(
                                   dynHeroData.dataId)
            if effectGrid ~= nil then
                needShowGridInfo = true;
                (self.battleGirdInfoNode):InitBattleGridInfo(effectGrid.gridData)
...
UIDungeonInfoDetail.OnClickLeftButton = function(self)
    -- function num : 0_10
    if self.isShowingChipDetail then self:SwitchChip(false) end
    if self.isShowingHeroDetail then self:SwitchHero(false) end
end

UIDungeonInfoDetail.OnClickRightButton =
    function(self)
        -- function num : 0_11
        if self.isShowingChipDetail then self:SwitchChip(true) end
        if self.isShowingHeroDetail then self:SwitchHero(true) end
    end

UIDungeonInfoDetail.SetSwitchBtnActive =
