...
local SortKindFunc = {[eSortKindType.quality] = function(theSelf)
  -- function num : 0_0
  return function(a, b)
        ...
        if a.timeStemp == b.timeStemp then
          if a.quality == b.quality then
            if b:GetAthDensity() >= a:GetAthDensity() then
              do return aBind ~= bBind end
              do return b.quality < a.quality end
              do return b.timeStemp < a.timeStemp end
              do return aSpaceOk end
              do return bBind end
          ...
          if aHaveViceAttr == bHaveViceAttr then
            if a.quality == b.quality then
              if b:GetAthDensity() >= a:GetAthDensity() then
                do return aBind ~= bBind end
                do return b.quality < a.quality end
                do return aHaveViceAttr end
                do return aHaveMainAttr end
                do return aSpaceOk end
                do return bBind end
                -- DECOMPILER ERROR: 13 unprocessed JMP targets
              end
...
local FliterKindFunc = {[eSortKindType.mainAttr] = function(mainAttrId)
  -- function num : 0_3
  return function(athData)
    -- function num : 0_3_0 , upvalues : mainAttrId
    if athData:IsHaveThisAttr(mainAttrId) or athData:IsHaveThisViceAttr(mainAttrId) then
      return true
    end
    return false
  end
