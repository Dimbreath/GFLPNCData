...
bs_301.OnAttackTrigger = function(self, target, data)
    ...
    local Effect_301 = LuaSkillCtrl:CallEffect(target, (self.config).effectId2,
                                               self, nil, nil, 1, true)
    ...
    local skillEmission = LuaSkillCtrl:CallCircledEmissionStraightly(self,
                                                                     ...
                                                                     Effect_301,
                                                                     false,
                                                                     true, nil) -- DECOMPILER ERROR at PC40: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["301_effect_bj"] =
        LuaSkillCtrl:CallEffect(target, (self.config).effectId3, self)
end

bs_301.CallBack = function(self, target, index, entity)
    ...
    if entity == target then
        LuaSkillCtrl:StartTimer(self, 10, function()
            ...
            LuaSkillCtrl:CallBuff(self, entity, (self.config).buffId2, 1,
                                  (self.arglist)[2])
        end, self, 0, 0)
        LuaSkillCtrl:CallBuff(self, entity, (self.config).buffId3, 1,
                              (self.arglist)[2] + (self.arglist)[3] + 10)
        if (self.arglist)[4] >= 0 then
            LuaSkillCtrl:CallBuff(self, entity, (self.config).buffId4, 1,
                                  (self.arglist)[2] + (self.arglist)[3] + 10)
        end
    end
    if entity == target and ((self.caster).recordTable)["301_effect_bj"] ~= nil then
        (((self.caster).recordTable)["301_effect_bj"]):Die() -- DECOMPILER ERROR at PC84: Confused about usage of register: R5 in 'UnsetPending'
        ;
