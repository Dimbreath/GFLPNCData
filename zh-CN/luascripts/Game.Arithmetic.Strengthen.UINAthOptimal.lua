...
UINAthOptimal.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Optimal, self,
    ...
    self.affixItemPool = (UIItemPool.New)(UINAthOptAffixItem,
                                          (self.ui).tog_SubAttriItem)
...
UINAthOptimal.InitAthOptimal = function(self, athData)
    ...
    for k, v in ipairs(athData.affixList) do
        ...
        affixItem:InitAthOptAffixItem(R11_PC65, v.id, v.quality, v.value,
                                      self.__onSelectAffixEvent,
                                      self.__checkOptEnableEvent)
        if firstAffixIndex == nil and affixItem:OptimizeEnable() then
            firstAffixIndex = affixItem:SetSelected()
        end
    end
...
UINAthOptimal.__UpdateConsumeItem = function(self)
    ...
    ((self.ui).tex_Consume):SetIndex(0, self.__consumeItemName,
                                     tostring(consumeItemCount))
    local icon = CRH:GetSprite(consumeItemCfg.icon) -- DECOMPILER ERROR at PC38: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_ConsumeIconL).sprite = icon -- DECOMPILER ERROR at PC41: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_ConsumeIconR).sprite = icon
    local color = ItemQualityColor[consumeItemCfg.quality] -- DECOMPILER ERROR at PC47: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).img_ConsumeQualityL).color = color -- DECOMPILER ERROR at PC50: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).img_ConsumeQualityR).color = color;
    ((self.ui).tex_TakeNum):SetIndex(0, tostring(
                                         (ConfigData.game_config).athOptimizeConsumeItemNum))
end
...
UINAthOptimal.UpdateCumulate = function(self)
    ...
    if failureCount < curFailure then curFailure = failureCount end -- DECOMPILER ERROR at PC12: Confused about usage of register: R3 in 'UnsetPending'
    
    ((self.ui).img_Cumulate).fillAmount = curFailure / failureCount;
    ((self.ui).tex_Cumulate):SetIndex(0, tostring(curFailure),
...
UINAthOptimal.SeletAffix = function(self, affixIndex)
    ...
    if self.curOptCfg == nil then
        error("Can\'t find ath_optimize cfg, id = " ..
                  tostring((self.curAffixCfg).optimize_id))
        return
    end
...
UINAthOptimal.SetSliderSection = function(self, section)
    ...
    ;
    ((self.ui).slider).maxValue = (self.curOptCfg).density - 1 -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
    ;
...
UINAthOptimal.OnOptimalComplete = function(self, objList)
    ...
    (((self.affixItemPool).listItem)[self.affixIndex]):OptAffixChange(newValue,
                                                                      succeed)
    if succeed then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.arithmetic_optimal_Success));
        ((self.ui).tex_ATHDensity):SetIndex(0, GetPreciseDecimalStr(
                                                (self.athData):GetAthDensity(),
                                                2))
    else

        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.arithmetic_optimal_Failure))
