...
local base = UIBaseNode
local UINItemWithCost = require("Game.Arithmetic.Strengthen.UINItemWithCost")
local UINAthOptAffixItem = require(
                               "Game.Arithmetic.Strengthen.UINAthOptAffixItem")
local UINAthEmptyAffix = require("Game.Arithmetic.Strengthen.UINAthEmptyAffix")
local cs_MessageCommon = CS.MessageCommon
UINAthOptimal.ctor = function(self, affixToggleGroup)
    -- function num : 0_0
...
UINAthOptimal.OnInit = function(self)
    -- function num : 0_1 , upvalues : _ENV, cs_MessageCommon, UINItemWithCost, UINAthEmptyAffix
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_Optimiz, self,
                               self.OnClickOptimalBtn);
    (UIUtil.AddValueChangedListener)((self.ui).slider, self,
                                     self.OnSliderValueChange)
    self.__onUpdateItemEvent = BindCallback(self, self.OnItemUpdateEvent)
    self.__onSelectAffixEvent = BindCallback(self, self.SeletAffix)
    self.__checkOptEnableEvent = BindCallback(self, self.CheckOptEnable);
    (((CS.EventTriggerListener).Get)(((self.ui).slider).gameObject)):onClick(
        "+", function()
            -- function num : 0_1_0 , upvalues : self, cs_MessageCommon, _ENV
            if not self.enableOpt then
                (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(279))
    ...
    self.athNetwork = NetworkManager:GetNetwork(NetworkTypeID.Arithmetic);
    ((self.ui).emptyAffixItem):SetActive(false)
    self.emptyAffixPool = (UIItemPool.New)(UINAthEmptyAffix,
...
UINAthOptimal.InitAthOptimal = function(self, athData, affixItemPool)
    ...
    local firstAffixIndex = nil;
    (self.affixItemPool):HideAll();
    (self.emptyAffixPool):HideAll() -- DECOMPILER ERROR at PC19: Confused about usage of register: R4 in 'UnsetPending'
    ;
    (self.affixToggleGroup).allowSwitchOff = true
    for i = 1, 4 do
        local affix = (athData.affixList)[i]
        if affix ~= nil then
            local affixItem = (self.affixItemPool):GetOne()
            affixItem:InitAthOptAffixItem(i, affix, self.__onSelectAffixEvent,
                                          self.__checkOptEnableEvent)
            local select = (firstAffixIndex == nil and
                               affixItem:OptimizeEnable())
            if select then firstAffixIndex = i end
            affixItem:SetAthAffixSelected(select) -- DECOMPILER ERROR at PC54: Confused about usage of register: R11 in 'UnsetPending'
            ;
            (affixItem.gameObject).name = tostring(i)
        else
            local emptyItem = (self.emptyAffixPool):GetOne()
    ...
    end -- DECOMPILER ERROR at PC65: Confused about usage of register: R4 in 'UnsetPending'
...
UINAthOptimal.__RefreshEnableOpt = function(self, enableOpt)
    -- function num : 0_3
    self.enableOpt = enableOpt -- DECOMPILER ERROR at PC3: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).btn_Optimiz).interactable = enableOpt -- DECOMPILER ERROR at PC6: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).slider).enabled = enableOpt
    -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'

    if not enableOpt then
        ((self.ui).slider).minValue = 0 -- DECOMPILER ERROR at PC14: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).slider).maxValue = 2 -- DECOMPILER ERROR at PC17: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).slider).value = 1
        self:__UpdateConsumeItem();
        ((self.ui).tex_UpValue):SetIndex(0, "0");
...
UINAthOptimal.__UpdateConsumeItem = function(self)
    ...
    for k, itemId in ipairs((ConfigData.game_config).athOptimizeConsumeItemId) do
        local consumeNum =
            ((ConfigData.game_config).athOptimizeConsumeItemNum)[k]
        local itemCfg = (ConfigData.item)[itemId]
        if itemCfg == nil then
            error("Can\'t find itemCfg, id = " .. tostring(itemId))
        else
...
UINAthOptimal.UpdateCumulate = function(self)
    ...
    local curFailure = (PlayerDataCenter.allAthData).failure
    self.nextMustSuccess = false
    if failureCount < curFailure then curFailure = failureCount end -- DECOMPILER ERROR at PC13: Confused about usage of register: R3 in 'UnsetPending'
    
    ((self.ui).img_FailRate).fillAmount = curFailure / failureCount;
    ((self.ui).tex_FailRate):SetIndex(0, tostring(curFailure),
...
UINAthOptimal.SetSliderSection = function(self, section)
    ...
    ((self.ui).slider).minValue = 0 -- DECOMPILER ERROR at PC8: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).slider).maxValue = (self.curAffixLvCfg).density - 1 -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).slider).wholeNumbers = true
    if self.__nextMustSuccess then section = ((self.ui).slider).maxValue end -- DECOMPILER ERROR at PC22: Confused about usage of register: R2 in 'UnsetPending'
    
    ((self.ui).slider).interactable = not self.__nextMustSuccess -- DECOMPILER ERROR at PC25: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).slider).value = section
    self:RefreshPreview(section)
end
...
UINAthOptimal.RefreshPreview = function(self, section)
    ...
    local curMedian = baseMedian + addValue
    local successRate = baseMedian * optCfg.base_success / curMedian / 100
    if self.__nextMustSuccess then successRate = 100 end
    ((self.ui).tex_SuccessValue):SetIndex(0,
                                          GetPreciseDecimalStr(successRate, 1))
    local value1 = (optCfg.base_section)[1] + addValue
    local value2 = (optCfg.base_section)[2] + addValue;
    ((self.ui).tex_UpValue):SetIndex(1, GetPreciseDecimalStr(value1, 0),
                                     GetPreciseDecimalStr(value2, 0))
end

UINAthOptimal.OnSliderValueChange = function(self, value)
    -- function num : 0_10
    if not self.enableOpt then return end
    self:RefreshPreview(value)
end
...
UINAthOptimal.CheckOptEnable = function(self)
    ...
    do if enableIndex == nil then self:__RefreshEnableOpt(false) end end
end
...
UINAthOptimal.OnClickOptimalBtn = function(self)
    ...
    (self.athNetwork):CS_ATH_AthOptimize((self.athData).uid,
                                         self.affixIndex - 1, self.section,
                                         self.__OnOptimalComplete)
end
            ...
            do
                ...
                if fromLv ~= toLv then
                    ...
                    do
                        local toValue = affixElem.value
                        local affixcfg =
                            (ConfigData.ath_affix_pool)[affixElem.id]
                        if affixcfg == nil then
                            error("Cant\'t find ath_affix_pool, id = " ..
                                      tostring(affixElem.id))
                            return
                        end
                        local attrId = affixcfg.affix_para
                        UIManager:ShowWindowAsync(
                            UIWindowTypeID.HeroLevelUpSuccess, function(window)
                                -- function num : 0_13_0 , upvalues : _ENV, fromLv, toLv, fromValue, toValue, attrId
                                (UIManager:ShowWindow(
                                    UIWindowTypeID.HeroLevelUpSuccess)):AthAffixLevelUp(
                                    fromLv, toLv, fromValue, toValue, attrId)
                            end)
                    end
                end
                do
                    ...
                    (cs_MessageCommon.ShowMessageTips)(
                        ConfigData:GetTipContent(
                            TipContent.arithmetic_optimal_Success))
                    self.curAfficElem = affixElem
                end
                (cs_MessageCommon.ShowMessageTips)(
                    ConfigData:GetTipContent(
                        TipContent.arithmetic_optimal_Failure));
                (((self.affixItemPool).listItem)[self.affixIndex]):OptAffixChange(
                    affixElem)
                self:SetSliderSection(self.section)
            end
        end
