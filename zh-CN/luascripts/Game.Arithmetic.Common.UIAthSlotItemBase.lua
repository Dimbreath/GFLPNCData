...
UIAthSlotItemBase.InitAthSlotItem = function(self, slotId, space, heroId)
  ...
  local athDataList = (PlayerDataCenter.allAthData):GetHeroAthList(heroId, slotId)
  local usedSpace = 0
  ...
  do
    ...
    local totalHeight = ((((self.ui).athContent).transform).rect).height - (((self.ui).athContent).padding).vertical - ((self.ui).athContent).spacing * (imgCount - 1)
    ;
    (self.athPool):HideAll()
    if usedSpace < space then
      local athItem = (self.athPool):GetOne()
      local height = totalHeight * (1 - (usedSpace) / space)
      athItem:InitAthItem((self.ui).unusedColor, height, space - (usedSpace), nil)
    end
    do
      for k,athData in ipairs(athDataList) do
        local athItem = (self.athPool):GetOne()
...
UIAthSlotItemBase.__InitRedDotEvent = function(self, heroId, slotId)
  ...
  local ok, AthSlotNode = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.HeroWindow, heroId, slotId)
  if not ok then
    return 
  end
  ;
...
UIAthSlotItemBase.OnDelete = function(self)
  ...
  local ok, AthSlotNode = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.HeroWindow, self.heroId, self.slotId)
  if ok then
    RedDotController:RemoveListener(AthSlotNode.nodePath, self.__onRedDotChanged)
  end
  ;
