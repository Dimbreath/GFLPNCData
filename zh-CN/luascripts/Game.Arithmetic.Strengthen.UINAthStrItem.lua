...
UINAthStrItem.InitAthStrItem = function(self, athData, resLoader)
    -- function num : 0_1 , upvalues : _ENV
    local athCfg = athData.athCfg
    resLoader:LoadABAssetAsync(PathConsts:GetAtlasAssetPath("AthSuitIcon"),
                               function(spriteAtlas)
        -- function num : 0_1_0 , upvalues : self, athCfg
        if spriteAtlas == nil then return end -- DECOMPILER ERROR at PC8: Confused about usage of register: R1 in 'UnsetPending'
        
        ((self.ui).img_Icon).sprite = spriteAtlas:GetSprite(athCfg.icon)
    end) -- DECOMPILER ERROR at PC14: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_Quality).color = ItemQualityColor[athData:GetAthQuality()] -- DECOMPILER ERROR at PC19: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_QualityAttr).color = athData:GetAthColor()
    local suitId = athCfg.suit
    local suitParamCfg = ((ConfigData.ath_suit).suitParamDic)[suitId] -- DECOMPILER ERROR at PC31: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).img_Suit).enabled = suitParamCfg ~= nil
    if suitParamCfg ~= nil then
        resLoader:LoadABAssetAsync(PathConsts:GetAtlasAssetPath("AthSuitIcon"),
    ...
    ((self.ui).tex_Area):SetIndex(0, areaCfg.name1) -- DECOMPILER ERROR at PC65: Confused about usage of register: R7 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = athData:GetName()
    if #(athData.athMainAttrCfg).attrtibute_id > 0 then
        local attrId = ((athData.athMainAttrCfg).attrtibute_id)[1]
        local attrValue = ((athData.athMainAttrCfg).attrtibute_num)[1]
        local name, valueStr, icon = ConfigData:GetAttribute(attrId, attrValue) -- DECOMPILER ERROR at PC84: Confused about usage of register: R12 in 'UnsetPending'
        ;
        ((self.ui).tex_Vlaue).text = valueStr -- DECOMPILER ERROR at PC91: Confused about usage of register: R12 in 'UnsetPending'
        ;
        ((self.ui).img_Icon).sprite = CRH:GetSprite(icon)
    end
    -- DECOMPILER ERROR: 4 unprocessed JMP targets
