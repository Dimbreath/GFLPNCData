...
local UIOasisBuildingUpgradeCostItem = class("UIOasisBuildingUpgradeCostItem", UIBaseNode)
local base = UIBaseNode
local UIOasisCostMatInfo = require("Game.Oasis.UI.OasisSidle.UIOasisCostMatInfo")
local infoItem = require("Game.Oasis.UI.UIOasisBuildingUpgradeCostInfoItem")
local itemStateInfo = {normalUpgrade = 0, normalConstruct = 1, upgrading = 2, constructing = 3}
...
UIOasisBuildingUpgradeCostItem.OnInit = function(self)
  ...
  self.matInfoPool = (UIItemPool.New)(UIOasisCostMatInfo, (self.ui).materialItem)
  self.textInfoPool = (UIItemPool.New)(infoItem, (self.ui).infoItem)
  self.itemInfoCount = 0
end

UIOasisBuildingUpgradeCostItem.ClearItemInfo = function(self)
  -- function num : 0_1
  if self.textInfoPool == nil then
    return 
  end
  ;
...
UIOasisBuildingUpgradeCostItem.InjectData = function(self, resLoader)
  -- function num : 0_2
  self.resLoader = resLoader
end

UIOasisBuildingUpgradeCostItem.SetTitle = function(self, name)
  -- function num : 0_3
...
UIOasisBuildingUpgradeCostItem.SetState = function(self, stateIndex)
...
UIOasisBuildingUpgradeCostItem.UpdateWithData = function(self, resNeedItems, remainTime, currentLevl, nextLevel)
  ...
  if resNeedItems ~= nil then
    ...
    for k,v in ipairs(resNeedItems) do
      if index > 0 and v ~= nil and v.resCount > 0 then
        ...
        matItem:UpdateUIWithName(v.resId, v.name, v.resCount, v.count)
        index = index - 1
      end
    end
  end
...
UIOasisBuildingUpgradeCostItem.InjectBuildingData = function(self, buildingData, isBuild)
  -- function num : 0_6
  self.buildingData = buildingData
  self.isBuild = isBuild
  self:CheckDoCompleteAvailable()
end

UIOasisBuildingUpgradeCostItem.CheckDoCompleteAvailable = function(self)
  -- function num : 0_7
  if self.buildingData ~= nil then
    local available = false
    if self.isBuild then
      available = (self.buildingData):CanUpgrade()
    else
      available = (self.buildingData):CanBuild()
    end
    local color = (((self.ui).btnText).text).color
    -- DECOMPILER ERROR at PC27: Confused about usage of register: R3 in 'UnsetPending'

    if available then
      (((self.ui).btn_DoComplete).image).color = (self.ui).normalBtnColor
      color.a = 1
    else
      -- DECOMPILER ERROR at PC35: Confused about usage of register: R3 in 'UnsetPending'

      ;
      (((self.ui).btn_DoComplete).image).color = (self.ui).disableBtnColor
      color.a = 0.5
    end
    -- DECOMPILER ERROR at PC40: Confused about usage of register: R3 in 'UnsetPending'

    ;
    (((self.ui).btnText).text).color = color
    -- DECOMPILER ERROR at PC44: Confused about usage of register: R3 in 'UnsetPending'

    ;
    (((self.ui).btn_DoComplete).image).raycastTarget = available
  end
end

UIOasisBuildingUpgradeCostItem.HideCostMatList = function(self)
  -- function num : 0_8
  ((self.ui).costItemList):SetActive(false)
end

UIOasisBuildingUpgradeCostItem.UpdateTime = function(self, progress, timeText)
  -- function num : 0_9
...
UIOasisBuildingUpgradeCostItem.UpdateLevel = function(self, currentLevel, nextLevel)
  -- function num : 0_10 , upvalues : _ENV
  -- DECOMPILER ERROR at PC5: Confused about usage of register: R3 in 'UnsetPending'

  ((self.ui).curLvlnum1).text = tostring(currentLevel // 10)
  -- DECOMPILER ERROR at PC11: Confused about usage of register: R3 in 'UnsetPending'

  ;
  ((self.ui).curLvlnum2).text = tostring(currentLevel % 10)
...
UIOasisBuildingUpgradeCostItem.OnClick = function(self)
  -- function num : 0_12
...
UIOasisBuildingUpgradeCostItem.SetNeedBuildingLevel = function(self, name, currentLevel, needLevel)
  -- function num : 0_13
...
UIOasisBuildingUpgradeCostItem.SetBuffChange = function(self, buffDic, isUnbuilt)
  -- function num : 0_14 , upvalues : itemInfoCountMax
  if buffDic ~= nil then
      ...
      for i = self.itemInfoCount, itemInfoCountMax do
        if buffDic[i] ~= nil then
          ...
          if textItem ~= nil then
            textItem:UpdateUI(nil, (buffDic[i]).currentInfo, (buffDic[i]).nextInfoValue, false, isUnbuilt)
          end
        end
      end
    end
    do
      self.itemInfoCount = itemInfoCountMax
    end
  end
end

UIOasisBuildingUpgradeCostItem.OnDelete = function(self)
