...
local UINAchievementLevelNode = class("UINAchievementLevelNode", UIBaseNode)
local base = UIBaseNode
local UINAchievementLevelItem = require(
                                    "Game.Achievement.UI.AchieveLevelUI.UINAchievementLevelItem")
local CommonLogicUtil = require("Game.Common.CommonLogicUtil.CommonLogicUtil")
local UINBaseItemWithCount = require("Game.CommonUI.Item.UINBaseItemWithCount")
local cs_DoTween = ((CS.DG).Tweening).DOTween
local eLevelState = {UnCompleted = 1, Completed = 2, Picked = 3}
UINAchievementLevelNode.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINBaseItemWithCount
    self.achivLevelNetworkCtrl = NetworkManager:GetNetwork(
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_Detail, self, self.OnClickDetail);
    (UIUtil.AddButtonListener)((self.ui).btn_Return, self, self.OnClickDetail) -- DECOMPILER ERROR at PC36: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopList_playerLevelList).onInstantiateItem =
        BindCallback(self, self.m_OnNewItem) -- DECOMPILER ERROR at PC43: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopList_playerLevelList).onChangeItem =
        BindCallback(self, self.m_OnChangeItem);
    (((self.ui).tween_tileSliderNode).onRewind):AddListener(
        BindCallback(self, self.OnSwitchTweenRewind))
    self.__RefreshPlayerLevel = BindCallback(self, self.RefreshPlayerLevel)
    MsgCenter:AddListener(eMsgEventId.UpdatePlayerLevel,
                          self.__RefreshPlayerLevel)
    MsgCenter:AddListener(eMsgEventId.UpdatePickedRewardLevel,
                          self.__RefreshPlayerLevel)
    self.__onGetLevelRewardComplete = BindCallback(self,
                                                   self.GetLevelRewardComplete)
    MsgCenter:AddListener(eMsgEventId.GetAchivLevelRewardComplete,
                          self.__onGetLevelRewardComplete)
    self._GetReward = BindCallback(self, self.GetReward)
    self.levelRewardItemPool = (UIItemPool.New)(UINBaseItemWithCount,
                                                (self.ui).obj_rewardItem);
    (((self.ui).obj_rewardItem).gameObject):SetActive(false) -- DECOMPILER ERROR at PC103: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.ui).col_ExpBar = ((self.ui).Img_ExpBar).color
    self:RegistAndUpdateRedDot()
end

UINAchievementLevelNode.RegistAndUpdateRedDot =
    function(self)
        -- function num : 0_1 , upvalues : _ENV
...
UINAchievementLevelNode.OnSwitchTweenRewind =
    function(self)
        -- function num : 0_3
        ((self.ui).obj_levelAddition):SetActive(false);
        ((self.ui).obj_levelList):SetActive(false)
...
UINAchievementLevelNode.RefreshPlayerLevel =
    function(self, isOpen, playTween)
        -- function num : 0_4 , upvalues : _ENV
        local maxLevel = (ConfigData.game_config).playerMaxLevel
        local curLevel = (PlayerDataCenter.playerLevel).level
        if curLevel < 10 then
        ...
        if maxLevel <= curLevel then
            ...
            ((self.ui).tex_Progress):SetIndex(1) -- DECOMPILER ERROR at PC45: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).Img_ExpBar).color = (self.ui).col_ExpBar -- DECOMPILER ERROR at PC48: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).Img_ExpBar).fillAmount = 1
        else
            local exp = (PlayerDataCenter.playerLevel).exp
            local expCeiling = (PlayerDataCenter.playerLevel):GetNextLevelExp();
            ((self.ui).tex_Progress):SetIndex(0, tostring(exp),
                                              tostring(expCeiling))
            if not isOpen and playTween then
                self:SetProcessTween(exp / expCeiling, 0.8)
            else
                ...
                ((self.ui).Img_ExpBar).color = (self.ui).col_ExpBar -- DECOMPILER ERROR at PC85: Confused about usage of register: R7 in 'UnsetPending'
                ;
                ((self.ui).Img_ExpBar).fillAmount = exp / expCeiling
            end
        end
...
UINAchievementLevelNode.m_RefreshNextLevelInfo =
    function(self, curLevel, isMax)
        -- function num : 0_5 , upvalues : _ENV, CommonLogicUtil
        if isMax then
            ((self.ui).obj_levelReward):SetActive(false);
            ((self.ui).obj_itemList):SetActive(false)
        ...
        if nextLevelCfg ~= nil then
            ...
            local logic, para1, para2, para3 =
                (PlayerDataCenter.achivLevelData):GetLevelLogics(nextLevel)
            local des = ""
            for index, logic in pairs(logic) do
                local para1 = para1[index]
                local para2 = para2[index]
                local para3 = para3[index]
                if (string.IsNullOrEmpty)(des) then
                    des = (CommonLogicUtil.GetDesString)(logic, para1, para2,
                                                         para3)
                else
                    des = des .. "\n" ..
                              (CommonLogicUtil.GetDesString)(logic, para1,
                                                             para2, para3)
                end
            end -- DECOMPILER ERROR at PC91: Confused about usage of register: R10 in 'UnsetPending'
...
UINAchievementLevelNode.RefreshAchFinishedNum =
    function(self, cur, total)
        -- function num : 0_6 , upvalues : _ENV

        ((self.ui).tex_CompleteNum):SetIndex(0, tostring(cur or 0),
                                             tostring(total or 0))
    end

UINAchievementLevelNode.RefreshLevelList =
    function(self, maxLevel, curLevel, isOpen)
        -- function num : 0_7 , upvalues : _ENV, eLevelState
        self.levelDataList = {}
        for i = 2, maxLevel do
            local levelData = {
            ...
            if (PlayerDataCenter.playerLevel).level < i then
                ...
                if ((PlayerDataCenter.achivLevelData).pickedRewardLevels)[i] ==
                    nil then
                    levelData.state = eLevelState.Completed
                else
                    levelData.state = eLevelState.Picked
        ...
        if ((self.ui).loopList_playerLevelList).totalCount ~=
            #self.levelDataList then
            ((self.ui).loopList_playerLevelList).totalCount =
                #self.levelDataList;
            ((self.ui).loopList_playerLevelList):RefillCells()
        else
        ...
        if isOpen then
            ...
            do
                ...
                ((self.ui).loopList_playerLevelList):SrollToCell(rollIndex - 1,
                                                                 5000)
            end
        end
    end

UINAchievementLevelNode.RefreshTotalLogic =
    function(self)
        -- function num : 0_8 , upvalues : _ENV, CommonLogicUtil
        local des = ""
        local cfg =
            (ConfigData.achievement_level)[(PlayerDataCenter.playerLevel).level]
        for index, logic in ipairs(cfg.logic) do
            if (string.IsNullOrEmpty)(des) then
                des = (CommonLogicUtil.GetDesString)(logic, (cfg.para1)[index],
                                                     (cfg.para2)[index],
                                                     (cfg.para3)[index])
            else
                des = des .. "\n" ..
                          (CommonLogicUtil.GetDesString)(logic,
                                                         (cfg.para1)[index],
                                                         (cfg.para2)[index],
                                                         (cfg.para3)[index])
            end
        end -- DECOMPILER ERROR at PC44: Confused about usage of register: R3 in 'UnsetPending'
...
UINAchievementLevelNode.m_OnNewItem = function(self, go)
    ...
    local levelItem = (UINAchievementLevelItem.New)()
    levelItem:Init(go)
    levelItem:InitLevelItem(eLevelState, self._GetReward) -- DECOMPILER ERROR at PC10: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.levelItemDic)[go] = levelItem
end

UINAchievementLevelNode.m_OnChangeItem =
...
UINAchievementLevelNode.GetLevelRewardComplete =
...
UINAchievementLevelNode.SetProcessTween =
    function(self, setValue, duration)
        -- function num : 0_13 , upvalues : _ENV, cs_DoTween
        if setValue <= 0 then return end
        if self.processSeq ~= nil then (self.processSeq):Kill() end -- DECOMPILER ERROR at PC13: Confused about usage of register: R3 in 'UnsetPending'
        
        ((self.ui).Img_ExpBar).color = Color.white
        self.processSeq = (cs_DoTween.Sequence)();
        (self.processSeq):Append(((self.ui).Img_ExpBar):DOFillAmount(setValue,
                                                                     duration));
        (self.processSeq):Join((((self.ui).Img_ExpBar):DOColor(
                                   (self.ui).col_ExpBar, 0.2)):SetLoops(5));
        (self.processSeq):SetAutoKill(false)
    end
...
UINAchievementLevelNode.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.UpdatePlayerLevel,
                             self.__RefreshPlayerLevel)
    MsgCenter:RemoveListener(eMsgEventId.UpdatePickedRewardLevel,
                             self.__RefreshPlayerLevel)
    MsgCenter:RemoveListener(eMsgEventId.GetAchivLevelRewardComplete,
                             self.__onGetLevelRewardComplete);
    (base.OnDelete)(self)
end
