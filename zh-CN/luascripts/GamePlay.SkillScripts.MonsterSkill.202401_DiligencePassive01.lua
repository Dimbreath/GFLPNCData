...
bs_202401.OnBeforePlaySkill = function(self, role, context)
  ...
  if role == self.caster then
    ...
    if targetList.Count > 0 and targetList.Count ~= self.last_num then
      ...
      if again > 0 then
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId1, again)
        self.last_num = self.last_num + again
      end
      if again < 0 then
        local num = -again
        LuaSkillCtrl:DispelBuff(self.caster, (self.config).buffId1, num)
        self.last_num = self.last_num + again
      end
    end
  end
...
bs_202401.OnAfterBattleStart = function(self)
  ...
  local targetall = LuaSkillCtrl:CallTargetSelect(self, 2, 10)
