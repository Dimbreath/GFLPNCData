...
UINHomeCurrencyItem.InitCurrencyItem = function(self, itemCfg)
    ...
    local count = PlayerDataCenter:GetItemCount(itemCfg.id) -- DECOMPILER ERROR at PC11: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ...
    if itemCfg.id >= 1003 then
        local capcity = (PlayerDataCenter.playerBonus):GetWarehouseCapcity(
                            itemCfg.id) -- DECOMPILER ERROR at PC30: Confused about usage of register: R4 in 'UnsetPending'
        ;
...
UINHomeCurrencyItem.UpdateCount = function(self)
    ...
    if (self.itemCfg).id >= 1003 then
        local capcity = (PlayerDataCenter.playerBonus):GetWarehouseCapcity(
                            (self.itemCfg).id) -- DECOMPILER ERROR at PC25: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.ui).tex_Count).text = tostring(count) .. "/" .. tostring(capcity)
    else
        do
