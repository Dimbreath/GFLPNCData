...
bs_202603.OnAttackTrigger = function(self, role)
    ...
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId196, 1,
                          (self.arglist)[1])
    self.loopline = LuaSkillCtrl:CallEffect(role, (self.config).effectIdline,
                                            self)
    self.loopMZ =
        LuaSkillCtrl:CallEffect(role, (self.config).effectIdmiao, self)
    LuaSkillCtrl:CallRoleAction(self.caster, (self.config).antion2)
    local boom = BindCallback(self, self.Onboom, role)
    LuaSkillCtrl:StartTimer(self, (self.arglist)[1], boom, self)
end
...
bs_202603.Onboom = function(self, role)
    ...
    if role.hp >= 0 then
        LuaSkillCtrl:CallRoleAction(self.caster, (self.config).antion3)
        LuaSkillCtrl:CallEffect(role, (self.config).effectIdtrail, self,
                                self.SkillEventFunc)
        if self.loopMZ ~= nil then
            (self.loopMZ):Die()
            self.loopMZ = nil
