...
UISectorTaskStepItem.InitTaskStep = function(self, data, stepCfg, stcData)
    ...
    self.stepCfg = stepCfg -- DECOMPILER ERROR at PC7: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ...
    if data == nil then
        ...
        ((self.ui).progress):SetIndex(0, tostring(
                                          (math.min)(data.schedule, data.aim)),
                                      tostring(data.aim))
    end
    self:__RecycleItems()
    if stepCfg.step ~= 1 then return end
    for i = 1, #stcData.rewardIds do
        ...
        do
            do
                rewardItem:InitRewardsItem(itemCfg, rewardNum) -- DECOMPILER ERROR at PC80: Confused about usage of register: R12 in 'UnsetPending'
                ;
