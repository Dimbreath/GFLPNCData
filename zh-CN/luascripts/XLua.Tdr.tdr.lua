...
local load_metalib, load_metalib_buf, free_metalib, get_meta, table2buf,
      buf2table, str2table, metamaxbufsize, bufalloc, buffree, buf2str = nil,
                                                                         nil,
                                                                         nil,
                                                                         nil,
                                                                         nil,
...
local create_msg_pk = function(meta, buf, buf_size)
    ...
    return {
        ...
        pack = function(obj)
            ...
            if ret_code ~= 0 then return ret_code, used_size end
...
local create_lib = function(metalib)
    ...
    return setmetatable({}, {
        __index = function(obj, k)
            ...
            if ret_code ~= 0 then
                error("libtdrlua.get_meta() failed: errno=" .. ret_code ..
                          ",msg=" .. meta)
            end
            local ret_code, buf_size = (libtdrlua.metamaxbufsize)(metalib, k)
            if ret_code ~= 0 then
                error("libtdrlua.metamaxbufsize() failed: errno=" .. ret_code ..
                          ",msg=" .. buf_size)
            end
            local ret_code, buf = (libtdrlua.bufalloc)(buf_size)
            if ret_code ~= 0 then
                error("libtdrlua.bufalloc() failed: errno=" .. ret_code ..
                          ",msg=" .. buf)
            end
...
m.from_memory = function(str)
    ...
    if ret_code ~= 0 then
        error("libtdrlua.load_metalib_buf() failed: errno=" .. ret_code ..
                  ",msg=" .. metalib)
    end
    return create_lib(metalib)
end
