...
SectorItemEntity.InitSectorItemEntity = function(self, gameObject, sectorId,
                                                 sctInfoItem, sctProgressStage,
                                                 sctProgressBuild, uiCanvas)
    -- function num : 0_1 , upvalues : _ENV, CS_EventTriggerListener
    self.gameObject = gameObject
    self.transform = gameObject.transform
    self.id = sectorId
    ...
    sctProgressStage:SetSctProgressPosConstraint((self.bind).uIPos_Stage)
    sctProgressBuild:SetSctProgressPosConstraint((self.bind).uIPos_Build);
    ((CS_EventTriggerListener.Get)((self.bind).collider)):onClick("+",
                                                                  BindCallback(
                                                                      self,
                                                                      self.__OnClickRoot));
    ((CS_EventTriggerListener.Get)((self.buildBind).collider)):onClick("+",
                                                                       BindCallback(
                                                                           self,
                                                                           self.__OnClickBuilding))
    self:RefreshSctStageProress()
    self:RefreshSctBuildProgress()
    self:UpdateSectorLock()
end
...
SectorItemEntity.__OnClickRoot = function(self)
    -- function num : 0_2
    if self.sctCtrl ~= nil then (self.sctCtrl):OnSectorItemClicked(self.id) end
end

SectorItemEntity.__OnClickBuilding = function(self)
    -- function num : 0_3
    if self.sctCtrl ~= nil then (self.sctCtrl):StartBuildFocusEnter(self.id) end
end

SectorItemEntity.SetSctItemSelect = function(self)
    -- function num : 0_4
    (self.uiCanvas):SetUISctSelect(((self.bind).uIPos_Select).position,
                                   (self.bind).selectSize)
end
...
SectorItemEntity.RefreshSctStageProress =
    function(self)
        -- function num : 0_6 , upvalues : _ENV
        local sectorIdList = ((ConfigData.sector_stage).sectorIdList)[self.id]
        local allCount = 0
        local completeCount = 0
        for k, id in ipairs(sectorIdList) do
            if ((ConfigData.sector_stage)[id]).difficulty ~=
                eSectorStageDifficult.Endless then
                do
                    allCount = allCount + 1
                    if (PlayerDataCenter.sectorStage):IsStageComplete(id) then
                        completeCount = completeCount + 1
...
SectorItemEntity.RefreshSctBuildProgress =
    function(self)
        -- function num : 0_7 , upvalues : _ENV
        local totalCurrentLevel = 0
        local builingMaxLevel = 0
        for _, id in ipairs((self.sectorCfg).building) do
            local builtData =
                ((PlayerDataCenter.AllBuildingData).sectorBuilt)[id]
            do
                if builtData ~= nil then
                    if builtData.state ~=
                ...
                do
                    local maxLevel =
                        ((ConfigData.buildingLevel).maxLevelDic)[id]
                    builingMaxLevel = builingMaxLevel + maxLevel
                    -- DECOMPILER ERROR at PC28: LeaveBlock: unexpected jumping out DO_STMT
...
SectorItemEntity.GetSctCanGetResBuild = function(self, canGetResBuildIdDic)
    ...
    for k, buildId in pairs((self.sectorCfg).building) do
        ...
        if builtData ~= nil then
            local cangetRes = builtData:CanGetBuildRes(true)
            if cangetRes then canGetResBuildIdDic[buildId] = true end
        end
    end
end

SectorItemEntity.UpdateSctBuildResPillar =
    function(self, curSctResDic)
        -- function num : 0_9 , upvalues : _ENV
        local allResList = {}
        for k, v in pairs(curSctResDic) do (table.insert)(allResList, v) end
        (table.sort)(allResList, function(a, b)
            -- function num : 0_9_0
            do return a.id < b.id end
            -- DECOMPILER ERROR: 1 unprocessed JMP targets
        ...
        for k, allResData in ipairs(allResList) do
            if (self.buildBind).pillarCountMax >= k then
                ...
                do
                    if pillar == nil then
                        local pos = (((self.buildBind).pillarPos)[k]).position
                        pillar = (self.sctCtrl):NewSctResPillarEntity(
                                     (self.bind).sectorBuilding, k, pos) -- DECOMPILER ERROR at PC42: Confused about usage of register: R10 in 'UnsetPending'
                        ;
                        (self.resPillarEntityDic)[k] = pillar
                    end
                    do
...
SectorItemEntity.SetIsSectorBuildingUnlock =
    function(self, bool)
        -- function num : 0_14 , upvalues : _ENV
        self.isSectorBuildingUnlock = bool
        if not self.isSectorBuildingUnlock then
...
SectorItemEntity.OnDelete = function(self)
    -- function num : 0_15 , upvalues : _ENV
    for k, v in pairs(self.resPillarEntityDic) do v:OnDelete() end
end
