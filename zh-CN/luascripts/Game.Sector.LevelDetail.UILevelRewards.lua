...
UILevelRewards.OnInit = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).mask_Close, self, self.__OnClickClose);
    (UIUtil.AddButtonListener)((self.ui).getReward, self,
                               self.__OnClickGetReward);
    ((self.ui).firstItem):SetActive(false);
    ((self.ui).regularItem):SetActive(false)
    self.resloader = (cs_ResLoader.Create)()
    self.firstRewardItems = {}
    ...
    self.__OnGetRewardComplete = BindCallback(self, self.OnGetRewardComplete)
    self.__OnSectorStageUpdate = BindCallback(self, self.OnSectorStageUpdate)
    MsgCenter:AddListener(eMsgEventId.PickFirstReward,
                          self.__OnGetRewardComplete)
    MsgCenter:AddListener(eMsgEventId.SectorStateUpdate,
                          self.__OnSectorStageUpdate)
end
    ...
    for _, rewardId in ipairs(sectorStageCfg.first_reward_ids) do
        ...
        do
            do
                rewardItem:Show()
                rewardItem:InitItem(itemCfg, rewardNum, self.resloader)
                rewardItem.clickEvent = BindCallback(self,
                                                     self.__OnClickRewardItem);
                (table.insert)(self.firstRewardItems, rewardItem)
                -- DECOMPILER ERROR at PC49: LeaveBlock: unexpected jumping out DO_STMT
...
UILevelRewards.__OnClickGetReward = function(self)
    -- function num : 0_5 , upvalues : _ENV
    self._heroIdSnapShoot = PlayerDataCenter:GetHeroIdSnapShoot();
    (self.sectorNetworkCtrl):Send_SECTOR_BattleFirstRewardPick(
        (self.__sectorStageCfg).id)
end
...
UILevelRewards.OnSectorStageUpdate = function(self)
    ...
    local stageState = (PlayerDataCenter.sectorStage):GetStageState(
                           (self.__sectorStageCfg).id);
    ((self.ui).tet_getReward):SetIndex(stageState);
    ((self.ui).getReward_image):SetIndex(stageState) -- DECOMPILER ERROR at PC28: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).getReward).enabled = stageState ==
                                        proto_object_DungeonStageState.DungeonStageStateCompleted
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end
...
UILevelRewards.OnDelete = function(self)
    -- function num : 0_9 , upvalues : _ENV, base
    MsgCenter:RemoveListener(eMsgEventId.PickFirstReward,
                             self.__OnGetRewardComplete)
    MsgCenter:RemoveListener(eMsgEventId.SectorStateUpdate,
                             self.__OnSectorStageUpdate);
    (base.OnDelete)(self)
    if self.resloader ~= nil then
        (self.resloader):Put2Pool()
