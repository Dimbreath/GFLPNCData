...
UINLtrPoolItem.InitLtrPoolItem = function(self, index, poolCfg, resLoader,
                                          clickFunc)
    -- function num : 0_1 , upvalues : _ENV
    self.index = index
    self.poolCfg = poolCfg
    self.clickFunc = clickFunc -- DECOMPILER ERROR at PC7: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.gameObject).name = tostring(index) -- DECOMPILER ERROR at PC14: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)(poolCfg.name)
    local iconPath = PathConsts:GetLotteryPicPath(
                         "PoolIcon/" .. poolCfg.name_path)
    resLoader:LoadABAssetAsync(iconPath, function(texture)
        -- function num : 0_1_0 , upvalues : self
