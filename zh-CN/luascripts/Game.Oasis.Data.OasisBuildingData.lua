...
OasisBuildingData.Initialize = function(self, data, stcData)
    ...
    if self.dynData == nil then self.dynData = (DynData.New)() end
    (self.dynData):Initialize(stcData)
    self.name = (self.dynData).name
    self.nameEn = (self.dynData).nameEn
    self.id = (self.dynData).id
...
OasisBuildingData.IsMaxLevel = function(self)
    -- function num : 0_2
    if self.level == nil then return false end
    do return self.maxLevel <= self.level end
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end

OasisBuildingData.InitializeWithDynData =
    function(self, data, dynData)
        -- function num : 0_3
        self.dynData = dynData
        self.name = (self.dynData).name
        self.nameEn = (self.dynData).nameEn
...
OasisBuildingData.Update = function(self, data)
    ...
    if self.level < self.maxLevel then
        ...
        self.upgradeTotalTime = (math.ceil)(
                                    ((self.levelConfig)[nextLevel]).time /
                                        (1 +
...
OasisBuildingData.CanUpgrade = function(self)
    ...
    local nextLevel = self.level + 1
    if self.maxLevel < nextLevel then return false end
    local nextLevelCfg = (self.levelConfig)[nextLevel]
    if not (CheckCondition.CheckLua)(nextLevelCfg.pre_condition,
                                     nextLevelCfg.pre_para1,
                                     nextLevelCfg.pre_para2) then
        return false
    end
    for k, v in ipairs(nextLevelCfg.consumeIds) do
        local resId = v
...
OasisBuildingData.CanGetBuildRes = function(self, withTips)
    -- function num : 0_6 , upvalues : _ENV, cs_MessageCommon
    if self.state == proto_object_BuildingState.BuildingStateCreate or
        self.resDatas == nil then return false end
    local fullWarehouse = true
    local datas = self:GetResDatas()
    for k, v in pairs(datas) do
        local count = v.count
        local capacity =
            (PlayerDataCenter.playerBonus):GetWarehouseCapcity(v.id)
        local playerResCount = PlayerDataCenter:GetItemCount(v.id)
        if playerResCount < capacity then
            fullWarehouse = false
            break
            ...
            if withTips then
                (cs_MessageCommon.ShowMessageTips)(
                    v.name ..
                        ConfigData:GetTipContent(TipContent.ItemInWarehouseFull))
            end
        end
    end
    do
        if fullWarehouse then return false end
        local periodLimit = true
        for k, v in pairs(self.resDatas) do
            if v.fitPeriodCount then periodLimit = false end
        end
        if periodLimit then
            if withTips then
                (cs_MessageCommon.ShowMessageTips)(
                    ConfigData:GetTipContent(
                        TipContent.Building_UnsatisfMinialRewardCycle))
            end
            return false
        end
        return true
    ...
    if self.resDatas ~= nil then
        ...
        for i = 1, #levelConfig.logic do
            if (levelConfig.logic)[i] == eLogicType.ResourceOutput then
                ...
                local resData = (self.resDatas)[id]
                local speed = (levelConfig.para2)[i]
                local resCount, resProgress, resMax, effSpeed =
                    resData:GetResCount(speed, (levelConfig.para3)[i])
                local data = {
                    id = id,
                    name = resData:GetName(),
                    count = resCount,
...
OasisBuildingData.GetProcess = function(self, timestamp)
    ...
    local second = (math.ceil)(remainTime)
    if progress < 1 then
        self.waitConfirmOver = (self.state ~=
                                   proto_object_BuildingState.BuildingStateCreate and
                                   self.state ~=
                                   proto_object_BuildingState.BuildingStateUpgrade) or
                                   self.waitConfirmOver
        do
            if self.waitConfirmOver then
                local HomeEnum = require("Game.Home.HomeEnum")
                if self.sectorId == nil then
                    ...
                    MsgCenter:Broadcast(eMsgEventId.NewNotice,
                                        ...
                                        self.over, {
                        name = self:GetSectorName() .. ":" .. self.name,
                        id = self.id,
...
OasisBuildingData.GetArea = function(self)
    -- function num : 0_9 , upvalues : CoordinateConvert
    if self.__areaList == nil then
        self.__areaList = (CoordinateConvert.GetHexArea)(self.position,
                                                         self.size)
    end
    return self.__areaList
end
        ...
        if levelConfig ~= nil then
            for i = 1, #levelConfig.logic do
                (PlayerDataCenter.playerBonus):InstallPlayerBonus(
                    ...
                    self.id, (levelConfig.logic)[i], (levelConfig.para1)[i],
                    (levelConfig.para2)[i], (levelConfig.para3)[i])
            end
        end
    end
end
    ...
    if self.state ~= proto_object_BuildingState.BuildingStateCreate then
        ...
        for i = 1, #levelConfig.logic do
            (PlayerDataCenter.playerBonus):UninstallPlayerBonus(
                proto_csmsg_SystemFunctionID.SystemFunctionID_Building, self.id,
                (levelConfig.logic)[i])
        end
    end
end
...
OasisBuildingData.GetNextLevelCostItem =
    function(self)
        -- function num : 0_13
        local lvl = nil
        if self.isBuild then
...
OasisBuildingData.GetPreBuildingLevelInfo =
    function(self)
        -- function num : 0_14
        local targetLvl = nil
        if self.isBuild then
...
OasisBuildingData.GetNextLevelNeedTime =
    function(self)
        -- function num : 0_15
        local targetLvl = nil
        if self.isBuild then
...
OasisBuildingData.GetNextLevelBuffItem =
    function(self)
        -- function num : 0_16
        local targetLvl = nil
        local getNext = false
