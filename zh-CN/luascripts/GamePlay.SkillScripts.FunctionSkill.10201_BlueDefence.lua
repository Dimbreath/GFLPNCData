...
bs_10201.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.OnSelfAfterMove, "bs_10201_1", 1,
                    self.OnAfterMove)
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_10201_2", 2,
...
bs_10201.OnAfterBattleStart = function(self)
    ...
    for i = 0, targetlist.Count - 1 do
        local targetRole = targetlist[i]
        if targetRole.roleDataId == 1002 and
            LuaSkillCtrl:GetRoleGridsDistance(self.caster, targetRole) == 1 then
            self.blueCount = self.blueCount + 1
        end
    end
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId,
                          (self.arglist)[1] * self.blueCount)
end
