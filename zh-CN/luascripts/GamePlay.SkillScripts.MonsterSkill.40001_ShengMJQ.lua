...
bs_40001.config = {damageNum = 0, damageEffectId = 10023, healEffectId = 10022, damageFormula = 10014}
...
bs_40001.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_40001_1", 100, self.OnAfterHurt)
end
...
bs_40001.PlaySkill = function(self, data)
  ...
  if targetList.Count > 0 then
    (self.config).damageNum = LuaSkillCtrl:CallFormulaNumber((self.config).damageFormula, self.caster, (targetList[0]).targetRole)
    LuaSkillCtrl:RemoveLife((self.config).damageNum, self, (targetList[0]).targetRole)
    LuaSkillCtrl:CallEffect((targetList[0]).targetRole, (self.config).damageEffectId, self)
  end
end

bs_40001.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
  ...
  if sender == self.caster and skill.dataId == 40001 then
    ...
    if targetList.Count > 0 then
      ...
      LuaSkillCtrl:CallEffect((targetList[0]).targetRole, (self.config).healEffectId, self)
    end
  end
end
