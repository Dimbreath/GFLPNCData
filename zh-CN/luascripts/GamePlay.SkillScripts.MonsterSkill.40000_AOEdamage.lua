...
bs_40000.PlaySkill = function(self, data)
    ...
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1002, 1, 5,
                                           attackTrigger)
...
bs_40000.OnAttackTrigger = function(self, data)
    ...
    for i = 0, targetList.Count - 1 do
        LuaSkillCtrl:CallEffectWithArg((targetList[i]).targetRole,
...
bs_40000.SkillEventFunc = function(self, effect, eventId, target)
    ...
    if effect.dataId == (self.config).effectId2 and eventId ==
        ...
        local damageNum = nil
        damageNum = LuaSkillCtrl:CallFormulaNumberWithSkill(
                        (self.config).damageFormula, self.caster, targetRole,
                        self)
        if damageNum < 1 then damageNum = 1 end
