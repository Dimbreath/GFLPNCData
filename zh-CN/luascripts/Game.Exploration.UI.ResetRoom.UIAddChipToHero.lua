...
local UIHeroDetailItem = require(
                             "Game.Exploration.UI.ResetRoom.UIAddChipHeroDetail")
...
UIAddChipToHero.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Skip, self,
                               self.__OnSkipButtonClick);
    (UIUtil.AddButtonListener)((self.ui).btn_Comfirm, self,
                               self.__OnComfirmButtonClick);
    (UIUtil.AddValueChangedListener)((self.ui).tog_ShowAttri, self,
                                     self.__OnAllHeroPropertyShow)
    self.__onHeroItemClick = BindCallback(self, self.__OnHeroItemClick)
end

UIAddChipToHero.InitAddChipToHero = function(self, ctrl, heroList, chipData,
                                             onSkipAction, onComfirmAction)
    -- function num : 0_1
    self.ctrl = ctrl
...
UIAddChipToHero.ExecuteNext = function(self, heroList, chipData)
    ...
    for _, heroData in ipairs(heroList) do
        ...
        if heroItem == nil then heroItem = (self.poolHeroItem):GetOne() end
        heroItem:InitHeroDetailItem(heroData, self.resLoader,
                                    self.__onHeroItemClick);
        (heroItem.gameObject):SetActive(true) -- DECOMPILER ERROR at PC32: Confused about usage of register: R9 in 'UnsetPending'
        ;
...
UIAddChipToHero.__OnAllHeroPropertyShow =
    function(self, value)
...
UIAddChipToHero.__OnSkipButtonClick = function(self)
    -- function num : 0_5
    if self.onSkipAction ~= nil then (self.onSkipAction)(self) end
end

UIAddChipToHero.__OnComfirmButtonClick =
    function(self)
        -- function num : 0_6
        if self.onComfirmAction ~= nil then (self.onComfirmAction)(self) end
    end
