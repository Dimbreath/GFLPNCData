...
UIBattleResult.OnInit = function(self)
    ...
    self.rewardItemPool = (UIItemPool.New)(UIRewardItem,
                                           (self.ui).obj_rewardItem)
    self.heroHeadItemPool = (UIItemPool.New)(UICharacterItem,
                                             (self.ui).obj_heroHeadItem);
    ...
    MsgCenter:AddListener(eMsgEventId.OnShowBattleResultComplete,
                          self.__playAnim)
...
UIBattleResult.__ToFackCameraCanvas = function(self)
    ...
    local fakeCameraBattle =
        ((cs_GameObject.Find)("FakeCameraBattle")):FindComponent(
            eUnityComponentID.Camera)
...
UIBattleResult.InitBattleResultData = function(self, resultData, mvpGrade)
    ...
    local roomType =
        ((ExplorationManager.epCtrl):GetCurrentRoomData()):GetRoomType()
    local roomTypeCfg = (ConfigData.exploration_roomtype)[roomType] -- DECOMPILER ERROR at PC25: Confused about usage of register: R7 in 'UnsetPending'
    ;
    ...
    if GuideManager:TryTriggerGuide(eGuideCondition.InEpBattleResult) then end
end
...
UIBattleResult.__SetGlobalExpNode = function(self, globalExpFrom)
    ...
    if ((PlayerDataCenter.AllBuildingData).oasisBuilt)[eBuildingId.OasisLibrary] ==
        nil then
        ((self.ui).obj_globalExpNode):SetActive(false)
        return
    end
    ((self.ui).obj_globalExpNode):SetActive(true)
    local globalExpTo = PlayerDataCenter:GetItemCount(
                            (ConfigData.game_config).globalExpItemId)
    local globalExpCeiling =
        (PlayerDataCenter.playerBonus):GetGlobalExpCeiling();
    ((self.ui).tex_globalExp):SetIndex(0, tostring(
                                           (math.floor)(
                                               globalExpTo - globalExpFrom)))
    local fromFill = globalExpFrom / globalExpCeiling -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).img_globalExp).fillAmount = fromFill
    if globalExpTo < globalExpCeiling then
        if globalExpFrom == 0 then return end
...
UIBattleResult.__SetCommanderExpNode = function(self)
    ...
    if not isCSUnlock then return end
    ...
    do
        ...
        if needRefreshTree then
            ...
            ((self.ui).tex_AddPlayerSkillExp):SetIndex(0, tostring(
                                                           exp -
                                                               lastDiff.oldTreeExp));
            ((self.ui).text_playerSkillName):SetIndex(0, treeData:GetName()) -- DECOMPILER ERROR at PC90: Confused about usage of register: R12 in 'UnsetPending'
            ;
            ((self.ui).tex_PlayerSkillLv).text = level
            local totalExp = ((ConfigData.commander_skill_level)[level]).exp or
                                 0 -- DECOMPILER ERROR at PC101: Confused about usage of register: R13 in 'UnsetPending'
            ;
        ...
        do
            if neddRefreshMaster then
                ...
                ((self.ui).tex_AddPlayerSkillExp):SetIndex(0, tostring(
                                                               masterExp -
                                                                   lastDiff.oldMasterExp));
                ((self.ui).text_playerSkillName):SetIndex(1) -- DECOMPILER ERROR at PC125: Confused about usage of register: R12 in 'UnsetPending'
                ;
                ((self.ui).tex_PlayerSkillLv).text = masterLevel
                local totalExp = nil
                if not ((ConfigData.commander_skill_master_level)[level]).exp then
                    totalExp =
                        (ConfigData.commander_skill_master_level)[level] == nil or
                            0
                    error("暂时未做满级的循环处理")
                    totalExp = 0 -- DECOMPILER ERROR at PC147: Confused about usage of register: R13 in 'UnsetPending'
                    ;
                    ((self.ui).img_playerSkillExp).fillAmount =
                        masterExp / totalExp
                end
            end
        end
...
UIBattleResult.__InitBattleReward = function(self)
    ...
    local isInfinity = ExplorationManager:GetEpModuleId() ==
    ...
    do
        if isInfinity then
            ...
            isCompletedInfinity =
                ((PlayerDataCenter.infinityData).completed)[stageCfg.dungeonId] or
                    false
        end
        ...
        for k, itemData in ipairs(curRoomData.rewardList) do
            if not isInfinity or isCompletedInfinity or
                not (itemData.itemCfg).explorationHold then
                local dataId = itemData.dataId
                do
                    if (dataId >= 8000 and dataId < 9000) or
                        ((ConfigData.item)[dataId]).type == eItemType.Arithmetic then
                        hasRandomAth = true
                    elseif rewardDic[dataId] == nil then
                        rewardDic[dataId] =
                            {data = itemData, count = itemData:GetCount()}
                    else
                        do
                            -- DECOMPILER ERROR at PC73: Confused about usage of register: R12 in 'UnsetPending'

                            (rewardDic[dataId]).count =
                                (rewardDic[dataId]).count + itemData:GetCount()
        ...
        if curRoomData.rewardExtraDic ~= nil then
            for dataId, count in pairs(curRoomData.rewardExtraDic) do
                if not isInfinity or isCompletedInfinity or
                    not ((ConfigData.item)[dataId]).explorationHold then
        ...
        if moneyData == nil then
            ...
            ((self.ui).obj_ccNode):SetActive(true) -- DECOMPILER ERROR at PC181: Confused about usage of register: R9 in 'UnsetPending'
            ;
        ...
        (table.sort)(rewardList, function(data1, data2)
            ...
            if (data2.data).quality < (data1.data).quality then
                ...
                if (data1.data).dataId >= (data2.data).dataId then
                    do
                        return (data1.data).quality ~= (data2.data).quality
                    end
                    do return false end
        ...
        for k, v in ipairs(rewardList) do
            ...
            if v.isAth then
                rewardItem:InitItemWithCount((v.data).itemCfg, v.count,
                    ...
                    UIManager:ShowWindowAsync(UIWindowTypeID.GlobalItemDetail,
                                              function(win)
        ...
        if self.rewardSequence ~= nil then (self.rewardSequence):Kill() end
...
UIBattleResult.__InitCharacterExp = function(self, lastHeroList)
    ...
    for k, dynHero in ipairs(heroList) do
        ...
        heroItem:RefershExpData(lastHeroList[k - 1]) -- DECOMPILER ERROR at PC30: Confused about usage of register: R11 in 'UnsetPending'
        ;
    ...
    if hasLevelup then ExplorationManager:RequestHeroAttr(levelupRoleIdDic) end
end
...
UIBattleResult.__InitMvpHeroPic = function(self, playerRoleList, mvpGrade)
    ...
    ((self.ui).tex_heroMvp):SetIndex(mvpGrade.MvpType) -- DECOMPILER ERROR at PC15: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).img_heroMvp).color = ((self.ui).color_MVP)[mvpGrade.MvpType + 1] -- DECOMPILER ERROR at PC25: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).tex_mvpName).text = tostring(
                                       (LanguageUtil.GetLocaleText)(
                                           (heroData.heroCfg).name));
...
UIBattleResult.__OnBtnSkadaClick = function(self)
    ...
    if self.skadaPage == nil then
        ...
        (self.skadaPage):InitBattleSkadaPage((self.ui).obj_skadaInfoPage,
                                             cs_BattleStatistics,
                                             (self.resultData).playerRoleList,
                                             (self.resultData).enemyRoleList,
                                             self.resloader)
    end
    (self.skadaPage):Show()
end
...
UIBattleResult.__OnBtnContinueClick = function(self)
    -- function num : 0_10
    if self.continueCallback ~= nil then (self.continueCallback)() end
    self:FadeDelete()
end
...
UIBattleResult.Update = function(self)
    ...
    if not self.__animationStart then return end
    for k, heroItem in pairs(self.heroHeadDic) do heroItem:UpdateExp() end
end
...
UIBattleResult.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.OnShowBattleResultComplete,
                             self.__playAnim);
    (base.OnDelete)(self)
end
