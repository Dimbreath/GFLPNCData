...
-- params : ...
-- function num : 0 , upvalues : _ENV
local BuildingAccelerateController = class("BuildingAccelerateController",
                                           ControllerBase)
local base = ControllerBase
local cs_MessageCommon = CS.MessageCommon
BuildingAccelerateController.ctor = function(self)
...
BuildingAccelerateController.IsCouldAcc =
    function(self, time)
        -- function num : 0_2 , upvalues : _ENV
        self.m_accItemList = {}
        local totalTime = 0
        for itemId, data in pairs(self.accItemDataDic) do
            if time <= totalTime + data.accTime * data.num then
                ...
                (table.insert)(self.m_accItemList,
                               {id = itemId, num = data.num, needNum = num})
                return true, self.m_accItemList
            else
                do
                    do
                        local needNum = (math.ceil)(
                                            (time - totalTime) / data.accTime)
                        totalTime = totalTime + data.accTime * data.num;
                        (table.insert)(self.m_accItemList, {
                            id = itemId,
                            num = data.num,
...
BuildingAccelerateController.m_RefreshItemDataInfo =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        self.accItemDataDic = {}
        for _, id in pairs(
            ...
            if itemData ~= nil then
                (self.accItemDataDic)[id] =
                    {
                        num = itemData:GetCount(),
                        accTime = itemData:GetActionArg(1)
                    }
            else
                local itemCfg = (ConfigData.item)[id] -- DECOMPILER ERROR at PC39: Confused about usage of register: R8 in 'UnsetPending'
                ;
                (self.accItemDataDic)[id] =
                    {num = 0, accTime = (itemCfg.arg)[1] or 0}
            end
        end
    end

BuildingAccelerateController.m_OnItemRefresh =
    function(self)
        -- function num : 0_4
        self:m_RefreshItemDataInfo()
        if self.OnItemChangeCallback ~= nil then
...
BuildingAccelerateController.SetOnItemChangeCallback =
    function(self, OnItemChangeCallback)
        -- function num : 0_5
        self.OnItemChangeCallback = OnItemChangeCallback
    end

BuildingAccelerateController.SendAccRequest =
    function(self, buildId, callback)
        -- function num : 0_6 , upvalues : _ENV
        local cost = {}
        for index, data in ipairs(self.m_accItemList) do
            if data.needNum <= data.num then
        ...
        (NetworkManager:GetNetwork(NetworkTypeID.Building)):SendBuildingAccelerate(
            buildId, cost, callback)
    end
