...
PlayerDataCenter.SyncUserData = function(self, userData)
    ...
    PersistentManager.playerId = self.playerId
    PersistentManager:LoadAndDecodeData((PstConfig.ePackage).UserData)
    if userData.heroGroup ~= nil then self:InitHeroData(userData.heroGroup) end
    if userData.resource ~= nil then
        local resource = userData.resource
        if resource.backpack ~= nil then
            for itemId, v in pairs((resource.backpack).data) do
                local itemData = (ItemData.New)(v.id, v.count) -- DECOMPILER ERROR at PC33: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ((self.itemTypeList)[itemData.type])[itemId] = itemData -- DECOMPILER ERROR at PC35: Confused about usage of register: R9 in 'UnsetPending'
                ;
                (self.itemDic)[itemId] = itemData
            end
        end
    ...
    do
        ...
        if userData.userBase ~= nil then
            (self.playerLevel):UpdatePlayerLevelData((userData.userBase).level,
                                                     (userData.userBase).exp)
            if (userData.userBase).showGirl ~= nil then
                self.showGirlId = (userData.userBase).showGirl
            end
            if (userData.userBase).cliLastSecMent ~= nil then
                    ...
                    if (self.serverLogic)[data[1]] == nil then
                        ...
                        do
                            do
                                ...
                                (table.insert)((self.serverLogic)[data[1]],
                                               value)
                                -- DECOMPILER ERROR at PC172: LeaveBlock: unexpected jumping out DO_STMT
...
PlayerDataCenter.InitHeroData = function(self, heroGroup)
    ...
    if heroGroup ~= nil then
        for k, v in pairs(heroGroup.hero) do
            local heroData = (HeroData.New)(v) -- DECOMPILER ERROR at PC11: Confused about usage of register: R8 in 'UnsetPending'
            ;
            (self.heroDic)[heroData.dataId] = heroData -- DECOMPILER ERROR at PC21: Confused about usage of register: R8 in 'UnsetPending'
            ;
...
PlayerDataCenter.InitFunctionUnlockData =
    function(self, funcUnLockCrtl, functionUnlockData)
        -- function num : 0_8 , upvalues : _ENV
        if funcUnLockCrtl == nil or functionUnlockData == nil then return end
        if functionUnlockData ~= nil then
            for k, v in pairs(functionUnlockData.data) do
                local openCfg = (ConfigData.system_open)[k]
                if openCfg ~= nil and openCfg.screening then
...
PlayerDataCenter.SyncItemUpdateDiff = function(self, resourceData)
    ...
    if resourceData.backpack ~= nil then
        for itemId, v in pairs((resourceData.backpack).deletes) do
            ...
            if deleteItem ~= nil then
                ((self.itemTypeList)[deleteItem.type])[itemId] = nil
            end -- DECOMPILER ERROR at PC19: Confused about usage of register: R10 in 'UnsetPending'
            
            (self.itemDic)[itemId] = nil
            itemUpdate[itemId] = true
            if not isHaveARG then
                isHaveARG =
                    (self.allEffectorData):IsAutoGenerateResource(itemId)
            end
        end
        for itemId, v in pairs((resourceData.backpack).updates) do
            local itemData = (self.itemDic)[itemId]
            if itemData ~= nil then
                ...
                itemData = (ItemData.New)(v.id, v.count) -- DECOMPILER ERROR at PC51: Confused about usage of register: R10 in 'UnsetPending'
                ;
                ((self.itemTypeList)[itemData.type])[itemId] = itemData -- DECOMPILER ERROR at PC53: Confused about usage of register: R10 in 'UnsetPending'
                ;
                (self.itemDic)[itemId] = itemData
            end
            itemUpdate[itemId] = false
    ...
    do
        if not isHaveARG then
            ...
            ((CS.WaitNetworkResponse).Instance):StartWait(
                proto_csmsg_MSG_ID.MSG_SC_EFFECTOR_RGSyncUpdateDiff, function()
                    -- function num : 0_9_0 , upvalues : _ENV, itemUpdate, resourceData
                    MsgCenter:Broadcast(eMsgEventId.UpdateItem, itemUpdate,
                                        resourceData)
                end, proto_csmsg_MSG_ID.MSG_SC_EFFECTOR_RGSyncUpdateDiff)
        end
    end
end
    ...
    if heroUpdateData.update ~= nil then
        ...
        for heroId, hero in pairs(heroUpdateData.update) do
            ...
            if heroData ~= nil then
                ...
                heroData = (HeroData.New)(hero) -- DECOMPILER ERROR at PC24: Confused about usage of register: R10 in 'UnsetPending'
                ;
                (self.heroDic)[heroData.dataId] = heroData -- DECOMPILER ERROR at PC34: Confused about usage of register: R10 in 'UnsetPending'
                ;
                (self.campHeroCount)[heroData.camp] =
                    ((self.campHeroCount)[heroData.camp] or 0) + 1
                heroUpdate[heroId] = true
        ...
        MsgCenter:Broadcast(eMsgEventId.UpdateHero, heroUpdate)
        if hasNew then (self.allFriendshipData):AddFriendshipgBonus() end
    end
end
