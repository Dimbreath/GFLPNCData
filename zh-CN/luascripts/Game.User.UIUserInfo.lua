...
local newBottomItemTab = {
    [1] = {
        func = function()
            -- function num : 0_0 , upvalues : _ENV
            local sectorId, stageIndex =
                (PlayerDataCenter.sectorStage):GetEpStageCfg4UserInfo()
            if sectorId == nil or stageIndex == nil then
                return "NO PASS"
            end
            return "STAGE " .. tostring(sectorId) .. "-" .. tostring(stageIndex)
    ...
    [2] = {
        func = function()
            -- function num : 0_1 , upvalues : _ENV
            return tostring((table.count)(
                                (PlayerDataCenter.infinityData).completed) * 10) ..
                       "m"
        end
    },
    [3] = {
        func = function()
            ...
            for key, value in pairs(
                                  (PlayerDataCenter.AllBuildingData).oasisBuilt) do
                totalLevel = totalLevel + value.level
            end
            return "Lv." .. tostring(totalLevel)
        end
...
UIUserInfo.OnInit = function(self)
    ...
    self.isModifyListOpen = false;
    (UIUtil.CreateTopBtnGroup)((self.ui).obj_topButtonGroup, self, self.Delete);
    (UIUtil.AddButtonListener)((self.ui).btn_toAchievement, self,
                               self.OpenAchievement);
    (UIUtil.AddButtonListener)((self.ui).btn_ExchangeHero, self,
                               self.ExchangeBoardHero);
    (UIUtil.AddButtonListener)((self.ui).btn_Modify, self, self.OpenMofiyList);
    (UIUtil.AddButtonListener)((self.ui).btn_copyUID, self, self.OnClickCopyUID)
    self.bottomInfoPool = (UIItemPool.New)(UINUserInfoBottomItem,
                                           (self.ui).obj_bottomInfoListItem);
    ((self.ui).obj_bottomInfoListItem):SetActive(false)
    self._RefreshBoardHero = BindCallback(self, self.RefreshBoardHero)
    self._RefreshAchievementInfo = BindCallback(self,
                                                self.RefreshAchievementInfo)
    MsgCenter:AddListener(eMsgEventId.UpdatePickedAchivTask,
                          self._RefreshAchievementInfo)
...
UIUserInfo.RefreshPlayerInfo = function(self)
    ...
    ((self.ui).input_userName).text = PlayerDataCenter.playerName -- DECOMPILER ERROR at PC11: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_UID).text = "UID:" .. PlayerDataCenter.strPlayerId
end
...
UIUserInfo.RefreshBoardHero = function(self, heroData, callback)
    -- function num : 0_6 , upvalues : _ENV, cs_ResLoader
    if heroData == nil then
        if PlayerDataCenter.showGirlId == nil or PlayerDataCenter.showGirlId ==
            0 then
            local firtBoardHeroID = (ConfigData.game_config).firtBoardHeroID
            if (PlayerDataCenter.heroDic)[firtBoardHeroID] == nil then
                error("default board hero data is nil id " .. firtBoardHeroID)
            ...
            do
                heroData =
                    (PlayerDataCenter.heroDic)[PlayerDataCenter.showGirlId]
                local resName = heroData:GetResName()
                if self.bigImgResloader ~= nil then
                    (self.bigImgResloader):Put2Pool()
                    self.bigImgResloader = nil
                ...
                (self.bigImgResloader):LoadABAssetAsync(
                    PathConsts:GetCharacterBigImgPrefabPath(resName),
                    function(prefab)
                        -- function num : 0_6_0 , upvalues : _ENV, self, callback
                        DestroyUnityObject(self.bigImgGameObject)
                        self.bigImgGameObject =
                            prefab:Instantiate((self.ui).obj_heroHolder)
                        local comPerspHandle =
                    ...
                    end) -- DECOMPILER ERROR at PC57: Confused about usage of register: R4 in 'UnsetPending'
                ;
                ((self.ui).tex_HeroName).text = heroData:GetName()
                self.currHeroData = heroData
            end
...
UIUserInfo.RefreshHeroNum = function(self)
    -- function num : 0_8 , upvalues : _ENV
    ((self.ui).tex_CollectNum):SetIndex(0, tostring(PlayerDataCenter.heroCount),
                                        tostring(
                                            (ConfigData.hero_data).totalHeroCount))
end

UIUserInfo.RefreshAchievementInfo = function(self)
    -- function num : 0_9 , upvalues : _ENV
    local isUnlock = (ControllerManager:GetController(
                         ControllerTypeId.FunctionUnlock, true)):ValidateUnlock(
...
UIUserInfo.OnClickCopyUID = function(self)
    ...
    ((CS.UnityEngine).GUIUtility).systemCopyBuffer =
        PlayerDataCenter.strPlayerId
    self.isModifyListOpen = false;
    ((self.ui).infoPopNode):SetActive(false);
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(
                                           TipContent.UserInfo_CopyUIDDone))
end
