...
ObjectNetworkCtrl.InitNetwork = function(self)
    ...
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_COUNTER_Detail, self,
                         proto_csmsg.SC_COUNTER_Detail, self.SC_COUNTER_Detail)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_COUNTER_SyncUpdateDiff, self,
                         proto_csmsg.SC_COUNTER_SyncUpdateDiff,
                         self.SC_COUNTER_SyncUpdateDiff)
end
...
ObjectNetworkCtrl.CS_COUNTER_Detail = function(self, callback)
    -- function num : 0_16 , upvalues : _ENV
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_COUNTER_Detail,
                 proto_csmsg.CS_COUNTER_Detail, {});
    ((CS.WaitNetworkResponse).Instance):StartWait(
        proto_csmsg_MSG_ID.MSG_CS_COUNTER_Detail, callback,
        proto_csmsg_MSG_ID.MSG_SC_COUNTER_Detail)
end
...
ObjectNetworkCtrl.SC_COUNTER_Detail = function(self, msg)
    ...
    do
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            local err = "ObjectNetworkCtrl:SC_COUNTER_Detail error:" ..
                            tostring(msg.ret)
            error(err)
            if isGameDev then
                ((CS.MessageCommon).ShowMessageTips)(err)
            ...
            ((CS.WaitNetworkResponse).Instance):RemoveWait(
                proto_csmsg_MSG_ID.MSG_CS_COUNTER_Detail)
            return
        end
        ((CS.WaitNetworkResponse).Instance):AddWaitData(
            proto_csmsg_MSG_ID.MSG_CS_COUNTER_Detail, msg.data)
    end
end

ObjectNetworkCtrl.SC_COUNTER_SyncUpdateDiff =
    function(self, msg)
        -- function num : 0_18 , upvalues : _ENV
        (ControllerManager:GetController(ControllerTypeId.TimePass, true)):OnUpdateTimePassData(
            msg)
    end

ObjectNetworkCtrl.CS_User_SystemOverDay =
    function(self)
        -- function num : 0_19 , upvalues : _ENV
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_User_SystemOverDay,
                     proto_csmsg.CS_User_SystemOverDay, {});
        ((CS.WaitNetworkResponse).Instance):StartWait(
            proto_csmsg_MSG_ID.MSG_CS_User_SystemOverDay, callback,
            proto_csmsg_MSG_ID.MSG_SC_User_SystemOverDay)
    end

ObjectNetworkCtrl.SC_User_SystemOverDay =
    function(self, msg)
        -- function num : 0_20 , upvalues : _ENV
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            local err = "ObjectNetworkCtrl:SC_User_SystemOverDay error:" ..
                            tostring(msg.ret)
            error(err)
            if isGameDev then
                ((CS.MessageCommon).ShowMessageTips)(err)
            ...
            ((CS.WaitNetworkResponse).Instance):RemoveWait(
                proto_csmsg_MSG_ID.MSG_CS_User_SystemOverDay)
            return
        end
    end
