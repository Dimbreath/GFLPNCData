...
UIOasisBuildingItem.OnClickButton = function(self)
  ...
  do
    if self.locked and ((self.buildingData).dynData).mainBdUnlockLevel ~= nil then
      local msg = (string.format)(ConfigData:GetTipContent(TipContent.Oasis_MainBuldingUnlockInfo), tostring(((self.buildingData).dynData).mainBdUnlockLevel), (LanguageUtil.GetLocaleText)((self.buildingData).name))
      ;
...
UIOasisBuildingItem.UpdateUI = function(self)
  ...
  if not self.isBuild then
    ...
    if ((self.buildingData).dynData).unlock then
      ...
      ;
      ((self.ui).tex_Condition):SetIndex(0, ((self.buildingData).dynData).mainBdName, tostring(((self.buildingData).dynData).mainBdUnlockLevel))
      self.locked = true
