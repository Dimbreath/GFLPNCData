...
UIOasisBuildingItem.InitItem = function(self, rowIns, buildingData, resLoader,
                                        OnClickEvent, sidle)
    -- function num : 0_1
    self.rowIns = rowIns
...
UIOasisBuildingItem.UpdateWithData = function(self, buildingData)
    ...
    do
        if self.uiIcon == nil then
            local path = PathConsts:GetOasisBuildingIconPath(
                             ((self.buildingData).dynData).icon_res);
            (self.resLoader):LoadABAssetAsync(path, function(texture)
                -- function num : 0_2_0 , upvalues : self
                self.uiIcon = texture -- DECOMPILER ERROR at PC4: Confused about usage of register: R1 in 'UnsetPending'
                ;
...
UIOasisBuildingItem.OnClickButton = function(self)
    ...
    if (self.UISidle).openedDetail then return end
    do
        if self.locked and ((self.buildingData).dynData).mainBdUnlockLevel ~=
            nil then
            local msg = (string.format)(ConfigData:GetTipContent(
                                            TipContent.Oasis_MainBuldingUnlockInfo),
                                        tostring(
                                            ((self.buildingData).dynData).mainBdUnlockLevel),
                                        (LanguageUtil.GetLocaleText)(
                                            (self.buildingData).name));
            (cs_MessageCommon.ShowMessageTips)(msg)
            return
        end
        if self.onClickEvent ~= nil then (self.onClickEvent)() end
    end
end
...
UIOasisBuildingItem.__ForceUpdateProcess =
...
UIOasisBuildingItem.UpdateUI = function(self)
    ...
    self.locked = false -- DECOMPILER ERROR at PC10: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).img_Pic).texture = self.uiIcon -- DECOMPILER ERROR at PC15: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).name_text).text = (self.buildingData).name -- DECOMPILER ERROR at PC20: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ...
    if not self.isBuild then
        ...
        if ((self.buildingData).dynData).unlock then
            ...
            ((self.ui).tex_Condition):SetIndex(0,
                                               ((self.buildingData).dynData).mainBdName,
                                               tostring(
                                                   ((self.buildingData).dynData).mainBdUnlockLevel))
            self.locked = true
    ...
    if isWorking then
        ...
        if isWorking then self.__forceUpdateProcess = true end
        self:UpdateBuffs()
    end
end
...
UIOasisBuildingItem.ResetConfirmState = function(self)
    ...
    self.waitForConfirm = false
    self.alreadySendConfirm = false -- DECOMPILER ERROR at PC4: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).fillImage).fillAmount = 1
    self.currentProgress = 1
    self.currentRemainSecond = 0 -- DECOMPILER ERROR at PC12: Confused about usage of register: R1 in 'UnsetPending'
    ;
...
UIOasisBuildingItem.UpdateLogicItems = function(self, itemPool)
    ...
    do
        ...
        if buffDispLimit >= buffIndex then end
    end
end

UIOasisBuildingItem.UpdateProcess = function(self, progress, remainTimeText,
    ...
    if waitConfirmOver then
        ((self.ui).fillImage).fillAmount = 1
        self.currentProgress = 1
        self.currentRemainSecond = 0 -- DECOMPILER ERROR at PC12: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.ui).tex_RemainTime).text =
            TimestampToTime(self.currentRemainSecond)
        self:UpdateUI()
        return
    end -- DECOMPILER ERROR at PC18: Confused about usage of register: R5 in 'UnsetPending'
    
    ((self.ui).fillImage).fillAmount = progress -- DECOMPILER ERROR at PC21: Confused about usage of register: R5 in 'UnsetPending'
    ;
