...
UIEpPlayerPos.ShowPlayerPosSmall = function(self, show, withTween, callBack)
    ...
    if withTween then
        if show then
            ...
            self.__smallFadeTween = (((self.ui).smallNode):DOFade(1,
                                                                  (ConfigData.buildinConfig).EpMapCursorShowDuration)):OnComplete(
                                        callBack)
        else
            ...
            self.__smallFadeTween = (((self.ui).smallNode):DOFade(0,
                                                                  (ConfigData.buildinConfig).EpMapCursorFadeDuration)):OnComplete(
                                        callBack)
        end
    else
    ...
    if show then self:ShowPlayerPosLarge(false) end
...
UIEpPlayerPos.ShowPlayerPosLarge = function(self, show, withTween, callBack)
    ...
    if withTween then
        if show then
            ...
            self.__largeFadeTween = (((self.ui).largeNode):DOFade(1,
                                                                  (ConfigData.buildinConfig).EpMapCursorShowDuration)):OnComplete(
                                        callBack)
        else
            -- DECOMPILER ERROR at PC24: Confused about usage of register: R4 in 'UnsetPending'
            ...
            self.__largeFadeTween = (((self.ui).largeNode):DOFade(0,
                                                                  (ConfigData.buildinConfig).EpMapCursorFadeDuration)):OnComplete(
                                        callBack)
        end
    else
    ...
    if show then self:ShowPlayerPosSmall(false) end
