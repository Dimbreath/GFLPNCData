...
bs_40012.PlaySkill = function(self, data)
    ...
    if ((self.caster).recordTable).lastAttackRole ~= nil then
        if LuaSkillCtrl:IsAbleAttackTarget(self.caster,
                                           ((self.caster).recordTable).lastAttackRole,
                                           1) then
            self:CallCasterWait(23);
            (self.caster):LookAtTarget(
                ((self.caster).recordTable).lastAttackRole)
            LuaSkillCtrl:CallRoleAction(self.caster, 1002, 1)
            LuaSkillCtrl:CallEffect(((self.caster).recordTable).lastAttackRole,
        ...
        if targetlist.Count > 0 then
            if LuaSkillCtrl:IsAbleAttackTarget(self.caster,
                ...
                (self.caster):LookAtTarget(targetlist[0])
                LuaSkillCtrl:CallRoleAction(self.caster, 1002, 1)
                LuaSkillCtrl:CallEffect(targetlist[0], (self.config).effectId,
                                        self, self.SkillEventFunc, nil, 1)
            else
                self:BreakSkill()
            end
        end
