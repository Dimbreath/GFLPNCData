...
UIATH.OnInit = function(self)
    ...
    self.__OnNewAthItem = BindCallback(self, self.OnNewAthItem)
    ...
    (self.sortListNode):SetAthListNewItemFunc(self.__OnNewAthItem);
...
UIATH.OnClickAreaBtn = function(self, index)
    ...
    end -- DECOMPILER ERROR at PC53: Confused about usage of register: R2 in 'UnsetPending'
...
UIATH.OnClickAthItem = function(self, athItem)
    -- function num : 0_14
    self:TryInstallAth(athItem)
end

UIATH.OnClickSuit = function(self)
    -- function num : 0_15
    (self.suitDetailNode):Show();
    (self.suitDetailNode):InitAthSuitDetail(self.resLoader, self.heroSuitDic)
end

UIATH.TryInstallAth = function(self, athItem)
    -- function num : 0_16 , upvalues : _ENV
    UIManager:ShowWindowAsync(UIWindowTypeID.AthItemDetail, function(window)
        -- function num : 0_16_0 , upvalues : athItem, _ENV, self
        if window == nil then return end
        ...
        window:ChangeAthItemParent(athItem)
    end)
end
...
UIATH.RefillAth = function(self, athData, gridId, resortAthDic, athInstalledDic,
    ...
    local installFunc = function()
        ...
        do
            ...
            (self.athNetwork):CS_ATH_AthRefillOp((self.heroData).dataId,
                                                 self.curSlotId, athData.uid,
                                                 gridId, oldMoveDic, refillOp,
    ...
    if athData.bindInfo ~= nil and (athData.bindInfo).id ~= (self.heroData).id then
        UIManager:ShowWindowAsync(UIWindowTypeID.AthReplaceHero,
            ...
            window:InitAthReplaceHero((self.heroData).dataId,
                                      (athData.bindInfo).id, installFunc,
                                      self.resLoader)
        end)
    else
        installFunc()
...
UIATH.ReplaceAth = function(self, oldAthData, newAthData)
    ...
    local replaceGrid = (oldAthData.bindInfo).grid
...
UIATH.OnNewAthItem = function(self, athItem)
    -- function num : 0_33
    athItem:SetAthItemDragFunc(self.__OnDragStart)
end
