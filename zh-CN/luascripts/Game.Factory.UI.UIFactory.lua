...
local UINFactoryResProduceItem = require("Game.Factory.UI.UINFactoryResProduceItem")
...
local UIFactorySetMaxCycleBox = require("Game.Factory.UI.UIFactorySetMaxCycleBox")
...
UIFactory.OnInit = function(self)
  ...
  (UIUtil.AddButtonListener)((self.ui).btn_OpenFormula, self, self.OnBtnOpenFactoryLine)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_CloseFormula, self, self.OnBtnCloseFactoryLine)
  ;
  ...
  self.poolFactoryLineEntity = (UIItemPool.New)(UINFactoryLineEntity, (self.ui).lineEntityItem)
  self.poolFactoryRewardItem = (UIItemPool.New)(UINFactoryRewardItem, (self.ui).rewardItemGO)
  ...
  self.__onFactoryRewardClicked = BindCallback(self, self.OnFactoryRewardClicked)
...
UIFactory.ShowAllFactoryRes = function(self)
  ...
  for k,itemId in ipairs(self.allResSortList) do
    ...
    if itemCfg ~= nil then
      ...
      resItem:Init(go)
      resItem:InitResProduce(itemCfg, 0, 0, self.__onResProduceSelect)
...
UIFactory.ShowAllFactoryLine = function(self)
  ...
  for k,dataId in ipairs((ConfigData.factory).id_sort_list) do
    ...
    if lineItem == nil then
      ...
      lineItem:Init(go)
      local isUnlock = (CheckCondition.CheckLua)(factoryCfg.pre_condition, factoryCfg.pre_para1, factoryCfg.pre_para2)
      lineItem:InitLineFormula(factoryCfg, isUnlock, self)
      ...
      do
        ...
        do
          if isUnlock and ((self.ctrl).factoryData).lineCategory ~= nil then
            local lineData = (((self.ctrl).factoryData).lineCategory)[dataId]
...
UIFactory.OnResProduceSelect = function(self, resProduceItem)
  ...
  for k,dataId in ipairs((ConfigData.factory).id_sort_list) do
    ...
    if (table.contain)(factoryCfg.consumeIds, curItemId) or factoryCfg.productId == curItemId then
      isContianItem = true
    end
    local lineItem = (self.dicFactoryLine)[dataId]
    if isContianItem then
      if lineItem == nil then
        ...
        ;
        (lineItem.transform):SetSiblingIndex(k)
        local factoryLine = (((self.ctrl).factoryData).lineCategory)[dataId]
...
UIFactory.RefreshFactoryAllData = function(self, lineCategory, allResource, finalResource)
  -- function num : 0_9 , upvalues : _ENV
  for lineId,lineItem in pairs(self.dicFactoryLine) do
    if lineItem.isUnlock then
      lineItem:SetOpenNumber(0)
    end
  end
  local resProduce = {}
  for lineId,factoryLine in pairs(lineCategory) do
    ...
    if resProduce[productId] == nil then
      resProduce[productId] = 0
    end
    resProduce[productId] = resProduce[productId] + productNum * factoryLine.count / factoryCfg.cycle
    for k,consumeId in ipairs(factoryCfg.consumeIds) do
      ...
      resProduce[consumeId] = resProduce[consumeId] - consumeNum * factoryLine.count
    end
  end
  ;
  ...
  local installLineCount, allLineCount = (self.ctrl):GetLineInstallAndAllCount()
  ;
  ((self.ui).tex_FreeQueue):SetIndex(0, tostring(allLineCount - installLineCount), tostring(allLineCount))
...
UIFactory.RefreshLineEntityAndResource = function(self, lineCategory, allResource, finalResource)
  ...
  for id,factoryLine in pairs(lineCategory) do
    for k,lineData in ipairs(factoryLine.lines) do
      ...
      lineEntityItem:InitFactoryLineEntity(lineData, factoryLine, allResource, count)
    end
  end
...
UIFactory.RefreshResourceCount = function(self, allResource, finalResource)
  ...
  for itemId,ItemCount in pairs(finalResource) do
    ...
    local itemCfg = (ConfigData.item)[itemId]
    factoryRewardItem:InitFactoryRewardItem(itemCfg, ItemCount, self.__onFactoryRewardClicked)
  end
end
...
UIFactory.UpdateStaffInfo = function(self)
  ...
  local slotList = {}
  local buildingData = ((PlayerDataCenter.AllBuildingData).built)[eBuildingId.OasisFactory]
  ...
  for i = 1, UnlockedSlotNum do
    ...
    if isUnlock then
      ...
      window:InitHeroEnterSelect(nil, eBuildingId.OasisFactory, slotList, i)
    end
)
    end
  end
        ...
        if allEnterData[eBuildingId.OasisFactory] ~= nil then
          local heros = (table.keys)((allEnterData[eBuildingId.OasisFactory]).data)
