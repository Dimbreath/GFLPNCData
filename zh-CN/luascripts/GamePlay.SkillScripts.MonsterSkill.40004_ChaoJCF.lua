...
, effectCFId = 123, startEffectId = 10105, endEffectId = 10106, buffId = 67, defBuffId = 1002, maxHPId = 1003, buffTier = 1, startAnimId = 1002, loopAnimId = 1007, endAnimId = 1009, audioId1 = 48, buffId196 = 196}
...
bs_40004.OnAttackTrigger = function(self, target, data)
  ...
  LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId196, 1, (self.arglist)[1])
...
bs_40004.PlaySkill = function(self, data)
    ...
    local attackTrigger = BindCallback(self, self.OnAttackTrigger, self.caster,
                                       data)
    self:CallCasterWait(actionTotalTime)
    self.effect = LuaSkillCtrl:CallEffect(self.caster,
                                          (self.config).startEffectId, self)
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster,
                                           (self.config).startAnimId, 1, 14,
                                           attackTrigger)
...
bs_40004.OnSetHurt = function(self, context)
    -- function num : 0_3 , upvalues : _ENV
    if context.target ~= self.caster then return end
    if (self.caster):GetBuffTier((self.config).defBuffId) > 0 then
        local damageNum = context.hurt
        context.hurt = 0
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).maxHPId,
                              damageNum)
        LuaSkillCtrl:CallHeal(damageNum, self, self.caster)
    end
end
...
bs_40004.OnAttackTrigger = function(self, target, data)
    ...
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId196, 1,
                          (self.arglist)[1])
    LuaSkillCtrl:CallEffect(self.caster, (self.config).effectCFId, self)
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).defBuffId, 1,
                          (self.arglist)[1])
