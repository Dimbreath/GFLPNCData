...
Bounds.Encapsulate = function(self, point)
    ...
    self:SetMinMax((Vector3.Min)(self:GetMin(), point),
                   (Vector3.Max)(self:GetMax(), point))
end
...
Bounds.Intersects = function(self, bounds)
    ...
    do
        return min.x <= max2.x and min2.x <= max.x and min.y <= max2.y and
                   min2.y <= max.y and min.z <= max2.z and min2.z <= max.z
    end
...
Bounds.Contains = function(self, p)
    ...
    local min = self:GetMin()
    local max = self:GetMax()
    if p.x < min.x or p.y < min.y or p.z < min.z or max.x < p.x or max.y < p.y or
        max.z < p.z then return false end
    return true
end
...
Bounds.__tostring = function(self)
    ...
    return (string.format)("Center: %s, Extents %s", tostring(self.center),
                           tostring(self.extents))
end
...
Bounds.unity_bounds = (CS.UnityEngine).Bounds -- DECOMPILER ERROR at PC66: Confused about usage of register: R8 in 'UnsetPending'
;
