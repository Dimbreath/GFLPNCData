...
CherkerFriendshipLevel.LengthCheck = function(param)
    -- function num : 0_0
    if #param >= 3 then return true end
    return false
end

CherkerFriendshipLevel.HeroFriendShipLevel =
    function(heroId, friendshipLevel)
        -- function num : 0_1 , upvalues : _ENV
        local level = (PlayerDataCenter.allFriendshipData):GetLevel(heroId)
        if friendshipLevel <= level then return true end
        return false
    end
...
CherkerFriendshipLevel.GetUnlockInfo = function(param)
    ...
    local hreoName = (LanguageUtil.GetLocaleText)(
                         ((ConfigData.hero_data)[heroId]).name)
    return (string.format)("%s亲密度达到%d级解锁", hreoName,
                           friendshipLevel)
end
