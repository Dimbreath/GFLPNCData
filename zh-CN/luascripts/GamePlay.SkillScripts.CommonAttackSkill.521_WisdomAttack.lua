...
bs_521.OnAttackTrigger =
    function(self, target, data, atkSpeedRatio, atkActionId)
        ...
        if LuaSkillCtrl:IsAbleAttackTarget(self.caster, target,
            ...
            if data.effectStartId ~= nil then
                if atkActionId == data.action1 then
                    LuaSkillCtrl:CallEffect(self.caster, data.effectStartId,
                                            self)
                else
                    LuaSkillCtrl:CallEffect(self.caster, data.effectStartId3,
                                            self)
                end
            end
            if data.effectId ~= nil then
                ...
                if LuaSkillCtrl:CallRange(1, 1000) <= roll then
                    LuaSkillCtrl:CallEffectWithArg(target, data.effectIdPJ,
                                                   self, true,
                                                   self.SkillEventFunc, data, 1)
                else
