...
local UINCSUpgradeItem = class("UINCSUpgradeItem", UIBaseNode)
local base = UIBaseNode
local UINBaseItem = require("Game.CommonUI.Item.UINBaseItem")
UINCSUpgradeItem.OnInit = function(self)
...
UINCSUpgradeItem.InitExpItem = function(self, itemData, numChangeFunc)
...
UINCSUpgradeItem.InitEmptyExpItem = function(self, itemId)
...
UINCSUpgradeItem.UpdateSelectedNum = function(self, num)
...
UINCSUpgradeItem.SetAddExpLimt = function(self, maxAddExp, addExp)
  -- function num : 0_4
  self.addableExp = maxAddExp - addExp
end

UINCSUpgradeItem.PlayRing = function(self)
  -- function num : 0_5
...
UINCSUpgradeItem.PlayRingBack = function(self)
  -- function num : 0_6
  local time = (self.ui).float_ringTime
  if self.fillTween ~= nil then
    (self.fillTween):Kill(true)
...
UINCSUpgradeItem.AddOne = function(self)
  -- function num : 0_7
...
UINCSUpgradeItem.MinusOne = function(self)
  -- function num : 0_9
...
UINCSUpgradeItem.PressMinus = function(self)
...
UINCSUpgradeItem.CleanAll = function(self)
  -- function num : 0_11
  self.selectedNum = 0
  self:UpdateSelectedNum()
end

UINCSUpgradeItem.OnDelete = function(self)
...
UINCSUpgradeItem.OnInit = function(self)
    ...
    (self.baseItem):Init((self.ui).obj_BaseItem);
    (UIUtil.AddButtonListener)((self.ui).btn_Reduce, self, self.MinusOne);
    ((((self.baseItem).ui).btn_Root).onPress):AddListener(
        BindCallback(self, self.PressAdd));
    (((self.ui).btn_Reduce).onPress):AddListener(
        BindCallback(self, self.PressMinus))
end
...
UINCSUpgradeItem.InitExpItem = function(self, itemData, numChangeFunc)
    ...
    self.itemNum = itemData:GetCount()
    self.numChangeFunc = numChangeFunc
    self.exp = itemData:GetActionArg(1) -- DECOMPILER ERROR at PC11: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).CanvasGroup).alpha = 1 -- DECOMPILER ERROR at PC18: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)(itemCfg.name);
    ((self.ui).tex_TotalCount):SetIndex(0, tostring(self.itemNum));
    (self.baseItem):InitBaseItem(itemCfg, BindCallback(self, self.AddOne))
...
UINCSUpgradeItem.InitEmptyExpItem = function(self, itemId)
    -- function num : 0_2 , upvalues : _ENV
    local itemCfg = (ConfigData.item)[itemId]
    self.itemNum = 0 -- DECOMPILER ERROR at PC6: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).CanvasGroup).alpha = 0.5;
    (((self.ui).btn_Reduce).gameObject):SetActive(false) -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)(itemCfg.name);
    ((self.ui).tex_TotalCount):SetIndex(0, "0");
    (self.baseItem):InitBaseItem(itemCfg)
...
UINCSUpgradeItem.UpdateSelectedNum = function(self, num)
    -- function num : 0_3 , upvalues : _ENV
    if not num then num = self.selectedNum end -- DECOMPILER ERROR at PC8: Confused about usage of register: R2 in 'UnsetPending'
    ...
    if self.numChangeFunc ~= nil then (self.numChangeFunc)(num) end
end
...
UINCSUpgradeItem.PlayRing = function(self)
    ...
    (((self.ui).img_RingAnima).gameObject):SetActive(true)
    local time = (self.ui).float_ringTime
    if self.fillTween ~= nil then (self.fillTween):Kill(true) end -- DECOMPILER ERROR at PC17: Confused about usage of register: R2 in 'UnsetPending'
    
    ((self.ui).img_RingAnima).fillAmount = 0 -- DECOMPILER ERROR at PC20: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_RingAnima).fillClockwise = true
    self.fillTween = ((self.ui).img_RingAnima):DOFillAmount(1, time);
    (self.fillTween):OnComplete(self.ringPlayback)
...
UINCSUpgradeItem.PlayRingBack = function(self)
    -- function num : 0_6
    local time = (self.ui).float_ringTime
    if self.fillTween ~= nil then (self.fillTween):Kill(true) end -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
    
    ((self.ui).img_RingAnima).fillAmount = 1 -- DECOMPILER ERROR at PC14: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_RingAnima).fillClockwise = false
    self.fillTween = ((self.ui).img_RingAnima):DOFillAmount(0, time)
end
...
UINCSUpgradeItem.PressAdd = function(self)
    ...
    local addNum = (math.ceil)(pressedTime * pressedTime / 10)
    do
        if self.itemNum <= self.selectedNum + addNum or self.addableExp < addNum *
            self.exp then
            local newAddNum = (math.min)(self.itemNum - self.selectedNum,
                                         (math.ceil)(self.addableExp / self.exp))
            if newAddNum > 0 then
                addNum = newAddNum
            else
                return
...
UINCSUpgradeItem.MinusOne = function(self)
    -- function num : 0_9
    if self.selectedNum - 1 < 0 then return end
    self.selectedNum = self.selectedNum - 1
    self:UpdateSelectedNum()
end
