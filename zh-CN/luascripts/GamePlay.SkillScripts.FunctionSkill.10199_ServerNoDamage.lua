...
bs_10199.OnSetHurt = function(self, context)
    -- function num : 0_2 , upvalues : _ENV
    if context.sender ~= nil and (context.sender).belongNum == 0 and
        (context.target).belongNum == eBattleRoleBelong.enemy then
        context.hurt = (math.max)(1, context.hurt + context.hurt *
                                      (self.arglist)[2] // 1000)
    else
        if context.sender ~= nil and (context.sender).belongNum == 0 and
            (context.target).belongNum == eBattleRoleBelong.player then
            context.hurt = (math.max)(1, context.hurt - context.hurt *
                                          (self.arglist)[1] // 1000)
        end
    end
