...
UINDeployPlayerSkill.InitPlayerSkill = function(self, index, playerSkillData,
                                                resloader, clickEvent)
    -- function num : 0_1 , upvalues : _ENV
    self.posIndex = index
    self.skillData = playerSkillData
    local isNull = playerSkillData == nil;
    ...
    if isNull then
        ...
        resloader:LoadABAssetAsync(PathConsts:GetSkillIconPath(
                                       playerSkillData:GetIcon()),
                                   function(texture)
            -- function num : 0_1_0 , upvalues : _ENV, self, playerSkillData
        ...
        ((self.ui).tex_SkillName):SetIndex(0, playerSkillData:GetName()) -- DECOMPILER ERROR at PC50: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).tex_SkillLevel).text = tostring(playerSkillData.level)
    end
    self.clickEvent = clickEvent
