...
bs_40032.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.SetHurt, "bs_40032_2", 1, self.OnSetHurt)
end
...
bs_40032.OnSetHurt = function(self, context)
  ...
  if context.target == self.caster and context.sender ~= self.caster and LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[1] and (context.sender).roleType == 1 and (context.skill).isCommonAttack then
    ...
    LuaSkillCtrl:CallBuff(self, context.sender, (self.config).buffId, 1)
  end
end
...
bs_40032.OnSetHurt = function(self, context)
    -- function num : 0_2 , upvalues : _ENV
    if context.target == self.caster and context.sender ~= self.caster and
        LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[1] and
        (context.sender).roleType == 1 and (context.skill).isCommonAttack then
        context.hurt = 0
