...
AutoResourceGeneratorData.UpdateItem = function(self, ARGBase)
    ...
    self.frag = ARGBase.frag
    self.originalValue = PlayerDataCenter:GetItemCount(self.itemId, true)
    self.ceiling = (PlayerDataCenter.playerBonus):GetWarehouseCapcity(
                       self.itemId)
    self.genSpeed = (PlayerDataCenter.playerBonus):GetAutoRecoverItemSpeed(
                        self.itemId)
    if self.itemId == ItemIdOfKey then
        (PlayerDataCenter.stamina):BroadcastStamina()
    end
...
AutoResourceGeneratorData.UpdateItemCeil =
    function(self)
        -- function num : 0_2 , upvalues : _ENV
        self.ceiling = (PlayerDataCenter.playerBonus):GetWarehouseCapcity(
                           self.itemId)
...
AutoResourceGeneratorData.UpdateItemSpeed =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        if self.accRate == nil then
            self.genSpeed =
                (PlayerDataCenter.playerBonus):GetAutoRecoverItemSpeed(
                    self.itemId)
        else
            self.genSpeed =
                (PlayerDataCenter.playerBonus):GetAutoRecoverItemSpeed(
                    self.itemId) * (1 + self.accRate)
        end
        if self.itemId == ItemIdOfKey then
            (PlayerDataCenter.stamina):BroadcastStamina()
        end
...
AutoResourceGeneratorData.SetAccRate = function(self, accRate)
    -- function num : 0_4 , upvalues : _ENV
    self.accRate = accRate
    local oringSpeed = (PlayerDataCenter.playerBonus):GetAutoRecoverItemSpeed(
                           self.itemId)
    self.genSpeed = oringSpeed * (1 + accRate)
    if self.itemId == ItemIdOfKey then
        (PlayerDataCenter.stamina):BroadcastStamina()
...
AutoResourceGeneratorData.GetCurrentNum =
    function(self)
        -- function num : 0_5 , upvalues : _ENV
        if self.startTm == nil or self.genSpeed == nil then return 0, 0 end
        local num = self.originalValue
        local remainSecond = 0
        if self.ceiling <= num then return num, remainSecond end
        local timestamp = PlayerDataCenter.timestamp
        local interval = timestamp - self.startTm
        if interval < -1 then return num, remainSecond end
        local frag = self.frag + interval * self.genSpeed
        local num = num + (math.floor)(frag / 100000)
        if self.ceiling <= num then
            num = self.ceiling
        ...
        do return num, remainSecond end
    end

AutoResourceGeneratorData.GetARGCeiling =
    function(self)
        -- function num : 0_6
        return self.ceiling
    end

AutoResourceGeneratorData.GetARGGenSpeedPerSecond =
    function(self, isBase)
        -- function num : 0_7 , upvalues : _ENV
        do
            if isBase then
                local oringSpeed =
