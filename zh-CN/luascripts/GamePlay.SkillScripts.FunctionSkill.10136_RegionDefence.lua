...
bs_10136.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_10136_1", 1,
...
bs_10136.CallBack = function(self)
    ...
    for i = 0, targetlist.Count - 1 do
        ...
        if layer1 < (self.arglist)[1] and layer2 < (self.arglist)[1] then
            LuaSkillCtrl:CallBuff(self, targetRole, (self.config).buffAttackId,
                                  (self.arglist)[1])
            LuaSkillCtrl:CallBuff(self, targetRole, (self.config).buffDefenceId,
                                  (self.arglist)[1])
...
bs_10136.CallBack1 = function(self)
    ...
    for i = 0, targetlist.Count - 1 do
        local targetRole = (targetlist[i]).targetRole
        LuaSkillCtrl:DispelBuff(targetRole, (self.config).buffAttackId,
                                (self.arglist)[1])
        LuaSkillCtrl:DispelBuff(targetRole, (self.config).buffDefenceId,
                                (self.arglist)[1])
    end
end
