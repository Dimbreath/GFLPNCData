...
local DormRoomCharacterCtrl = require(
                                  "Game.Dorm.Character.DormRoomCharacterCtrl")
...
DormCharacterCtrl.OnEnterDormScene = function(self)
    ...
    self.interPointHolder =
        (((CS.UnityEngine).GameObject)("InterPointHolder")).transform -- DECOMPILER ERROR at PC10: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.interPointHolder).position = Vector3.zero
    self.__interPointPool = {}
    self.__dragIndicatorGo =
        (((self.dormCtrl).comRes).fntBottomPrefab):Instantiate();
    (self.__dragIndicatorGo):SetActive(false)
end

DormCharacterCtrl.OnEnterDormHouse = function(self)
    -- function num : 0_2 , upvalues : _ENV
    if self.charResloader ~= nil then (self.charResloader):Put2Pool() end
...
DormCharacterCtrl.__InitCharacterCtrl = function(self)
    ...
    for pos, v in pairs((self.houseCtrl).roomEnityDic) do
        if not v:IsEmptyRoom() then
            local roomCharacterCtrl = (DormRoomCharacterCtrl.New)(self, v,
                                                                  self.charResloader) -- DECOMPILER ERROR at PC18: Confused about usage of register: R7 in 'UnsetPending'
            ;
            (self.roomCharacter)[pos] = roomCharacterCtrl
            self.__activeRoomCount = self.__activeRoomCount + 1
        end
    end
    if self.__activeRoomCount > 0 then self:AddUnBindCharacterEvenly() end
    for _, v in pairs(self.roomCharacter) do v:StartCharacterLogic() end
end

DormCharacterCtrl.AddUnBindCharacterEvenly =
    function(self)
        ...
        do
            while nowRoleCount < (ConfigData.game_config).DormRoleMaxCount and
                #self.unbindCharList > 0 do
            ...
            do
                if (ConfigData.game_config).DormRoleMaxCount <= nowRoleCount or
                    #self.unbindCharList <= 0 or not isAllRoomFull then end
            end
        end
    end

DormCharacterCtrl.RandChangeUnBindCharacter =
    function(self, roomCtrl, oldEntity)
        -- function num : 0_6 , upvalues : _ENV
        local index = (math.random)(#self.unbindCharList)
        local heroId = (self.unbindCharList)[index]
        local heroData = (PlayerDataCenter.heroDic)[heroId]
        local charEntity = roomCtrl:AddUnBindCharacter(heroData,
                                                       (roomCtrl.roomData):GetDoorCoord())
        if charEntity ~= nil then
            (table.remove)(self.unbindCharList, index)
        end
...
DormCharacterCtrl.RangeOtherNoFullRoomCtrl =
    function(self, roomCtrl)
        ...
        if #activeRoomList == 0 then return nil end
...
DormCharacterCtrl.ResetDormCharacter = function(self)
    ...
    for k, v in pairs(self.roomCharacter) do v:OnDelete() end
...
DormCharacterCtrl.ResetRoomCharacter = function(self, pos)
    ...
    if roomEntity == nil or roomEntity:IsEmptyRoom() then return end
    local roomCharacterCtrl = (DormRoomCharacterCtrl.New)(self, roomEntity,
                                                          self.charResloader) -- DECOMPILER ERROR at PC39: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.roomCharacter)[pos] = roomCharacterCtrl
    local nowRoleCount = 0
    for _, v in pairs(self.roomCharacter) do
    ...
    do
        while nowRoleCount < (ConfigData.game_config).DormRoleMaxCount and
            #self.unbindCharList > 0 and roomCharacterCtrl:IsRoomCharacterFull() ==
            false do
            local index = (math.random)(#self.unbindCharList)
            local heroId = (self.unbindCharList)[index]
            local heroData = (PlayerDataCenter.heroDic)[heroId]
        ...
        do roomCharacterCtrl:StartCharacterLogic() end
    end
end

DormCharacterCtrl.SetBindCharacterChange =
    function(self, fntData, oldHeroId, newHeroId)
        ...
        if oldHeroId ~= nil then
            for _, roomCtrl in pairs(self.roomCharacter) do
                ...
                if not findNewHero then
                    newCharacterEntity =
                        roomCtrl:GetCharacterEntityById(newHeroId)
        ...
        do
            ...
            if roomCtrl == nil or heroData == nil then return end
            if newCharacterEntity ~= nil then
...
DormCharacterCtrl.OnEnterDormEditMode = function(self)
    ...
    self.__timeActive = false
    for k, v in pairs(self.roomCharacter) do v:HidePauseRoomCharacter() end
end
...
DormCharacterCtrl.OnExitRoomEditMode = function(self, success, pos)
    ...
    for k, v in pairs(self.roomCharacter) do
        if k ~= pos then v:ShowResumeRoomCharacter() end
    end
...
DormCharacterCtrl.SetCurRoomCharacterCtrl =
    function(self)
        -- function num : 0_16
...
DormCharacterCtrl.GenInterPointEntity = function(self, interPoint, unityPos)
    ...
    if #self.__interPointPool > 0 then
        ...
        gameObject = (((self.dormCtrl).comRes).interPointPrefab):Instantiate(
                         self.interPointHolder)
    end
...
DormCharacterCtrl.RecoveryInterPoint = function(self, pointEntity)
    ...
    if IsNull(pointEntity.gameObject) then return end
...
DormCharacterCtrl.OnUpdate = function(self)
    ...
    if not self.__timeActive or self.roomCharacter == nil then return end
...
DormCharacterCtrl.SetSelectCharacter = function(self, charEntity)
    ...
    self:SetDragIndicatorPosition((self.__selectCharEntity).x,
                                  (self.__selectCharEntity).y)
end

DormCharacterCtrl.SetDragIndicatorPosition =
    function(self, x, y)
        -- function num : 0_22 , upvalues : DormUtil
        local pos = (DormUtil.GetFntUnityCoord)((self.__selectCharEntity).x,
                                                (self.__selectCharEntity).y) -- DECOMPILER ERROR at PC14: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.__dragIndicatorGo).transform).position =
            (((self.curRoomCharCtrl).roomEntity).fntHolder):TransformPoint(pos)
    end
...
DormCharacterCtrl.OnFingerDown = function(self, finger)
    ...
    if self.curRoomCharCtrl == nil then return end
    if self.__selectCharEntity ~= nil then return end
    local hits = (CS_Physics.Raycast)(((self.dormCtrl).bind).camera,
    ...
    do if self.__fingerId == nil then end end
end
...
DormCharacterCtrl.OnFingerSet = function(self, finger)
    ...
    for i = 0, hits.Length - 1 do
        ...
        if not IsNull(hitCollider) and hitCollider.tag == TagConsts.DormFloor then
            (self.curRoomCharCtrl):RefreshInterPointDistanceState(
                finger.ScreenPosition)
            local hitPos =
                (((self.curRoomCharCtrl).roomEntity).fntHolder):InverseTransformPoint(
                    (hits[i]).point)
            hitPos.y = 0
            local move, newX, newY =
                (self.__selectCharEntity):SetCharacterPosFromUnity(hitPos)
            if move then self:SetDragIndicatorPosition(newX, newY) end
        end
    end
end
