...
DynBattleRole.GetCarrerIcon = function(self)
    ...
    if career == nil then return nil end
    return career.icon
end
...
DynBattleRole.GetOwnedChipsById = function(self, chipId)
    ...
    for chipData, v in pairs(self.ownedChips) do
        if chipData.dataId == chipId then return chipData end
    end
...
DynBattleRole.GetRealAttr = function(self, attrId)
    ...
    return (((self.originAttr)[attrId] or 0) + ((self.baseAttr)[attrId] or 0)) *
               (eHeroAttrPercent + ((self.ratioAttr)[attrId] or 0)) //
               eHeroAttrPercent + ((self.extraAttr)[attrId] or 0)
end
...
DynBattleRole.GetFormulaAttr = function(self, attrEnum)
    ...
    if attrId == eHeroAttr.hp then
        ...
        if hp == 0 and self.hpPer > 0 then hp = 1 end
        return hp
    else
        do
                                ...
                                fightingPower +
                                    ((((skillCfg.combatFormula):BindOption(
                                        ...
                                        ((CS.FormulaOperand).OperandValue)(
                                            battleSkill.level))):GetValue(self,
                                                                          self)):AsLong()
                        else
                            fightingPower =
                                fightingPower +
...
DynBattleRole.GetFightingPower = function(self, fullHp)
    ...
    if fullHp then
        fightingPower =
            (((((CS.BattleConsts).BattleRoleFightPowerFormula).battleFormula):BindOption(
                ((CS.BattleFormula).eFormationOption).ReplaceHpWithMaxHp)):GetValue(
                self, self)):AsLong()
    else
        fightingPower =
