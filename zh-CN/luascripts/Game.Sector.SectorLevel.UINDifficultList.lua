...
UINDifficultList.__InitRedDotEvent = function(self)
    ...
    self.__onLevelItemRedDotEvent = function(node)
        ...
        if node:GetRedDotCount() <= 0 then
            do
                local show = sectorId ~= self.sectorId or difficulty ~=
                                 (PlayerDataCenter.sectorStage).lastSelectDiff;
                (self.diffLevelCanvas):SetSectorStageItemRedDot(node.nodeId,
                                                                show)
    ...
    RedDotController:AddListener(RedDotDynPath.SectorLevelBtnPath,
                                 self.__onLevelItemRedDotEvent)
    self.__onLAvgMainItemRedDotEvent = function(node)
        ...
        local sectorId =
            ((node:GetParentNode()):GetParentNode()):GetParentNodeId()
        if node:GetRedDotCount() <= 0 then
            do
                local show = sectorId ~= self.sectorId or difficulty ~=
                                 (PlayerDataCenter.sectorStage).lastSelectDiff;
                (self.diffLevelCanvas):SetSectorLAvgMainItemRedDot(node.nodeId,
                                                                   show)
                -- DECOMPILER ERROR: 2 unprocessed JMP targets
            end
        end
    ...
    RedDotController:AddListener(RedDotDynPath.SectorMainAvgPath,
                                 self.__onLAvgMainItemRedDotEvent)
end
...
UINDifficultList.OnDelete = function(self)
    ...
    RedDotController:RemoveListener(RedDotDynPath.SectorLevelBtnPath,
                                    self.__onLevelItemRedDotEvent)
    RedDotController:RemoveListener(RedDotDynPath.SectorMainAvgPath,
                                    self.__onLAvgMainItemRedDotEvent)
    if self.infinityLevelCanvas ~= nil then
