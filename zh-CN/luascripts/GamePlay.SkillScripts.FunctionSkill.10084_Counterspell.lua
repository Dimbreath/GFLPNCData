        ...
        do
          do
            if buffTier >= 2 then
              ...
              if not buff:ContainFeature(eBuffFeatureType.Taunt) and LuaSkillCtrl:RoleContainsBuffFeature(buff.make, eBuffFeatureType.Exiled) and LuaSkillCtrl:RoleContainsBuffFeature(buff.make, eBuffFeatureType.NotBeSelected) then
                LuaSkillCtrl:CallBuff(self, buff.maker, buff.dataId, buff.tier, buff.durationTime)
              end
            end
...
bs_10084.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.BeforeAddBuff, "bs_10084_beforeBuff", 1,
                    self.OnBeforeAddBuff)
    self:AddTrigger(eSkillTriggerType.BuffDie, "bs_10084_buffDie", 1,
                    self.OnBuffDie)
end

bs_10084.OnBeforeAddBuff = function(self, target, context)
    -- function num : 0_2 , upvalues : _ENV
    if (context.buff).buffType == 2 and target == self.caster and
        self:IsReadyToTake() then
        if ((self.caster).recordTable).CounterSpellCount ~= nil and
            ((self.caster).recordTable).CounterSpellCount > 0 then
            local counterSpellBuffId = (target.recordTable).CounterSpellBuffId
            if counterSpellBuffId ~= nil then
                local buffTier = (self.caster):GetBuffTier(counterSpellBuffId)
                do
                    do
                        if buffTier >= 2 then
                            local buff = context.buff
                            if not buff:ContainFeature(eBuffFeatureType.Taunt) and
                                LuaSkillCtrl:RoleContainsBuffFeature(buff.make,
                                                                     eBuffFeatureType.Exiled) and
                                LuaSkillCtrl:RoleContainsBuffFeature(buff.make,
                                                                     eBuffFeatureType.NotBeSelected) then
                                LuaSkillCtrl:CallBuff(self, buff.maker,
                                                      buff.dataId, buff.tier,
                                                      buff.durationTime)
                            end
                        end
                        context.active = false -- DECOMPILER ERROR at PC69: Confused about usage of register: R5 in 'UnsetPending'
                        ;
                        ((self.caster).recordTable).CounterSpellCount =
                            ((self.caster).recordTable).CounterSpellCount - 1
                        if ((self.caster).recordTable).CounterSpellCount <= 0 then
                            LuaSkillCtrl:DispelBuff(self.caster,
                                                    counterSpellBuffId, buffTier) -- DECOMPILER ERROR at PC83: Confused about usage of register: R5 in 'UnsetPending'
                            ;
                            ((self.caster).recordTable).CounterSpellCount = nil
                        end
                        self:OnSkillTake()
...
bs_10084.OnBuffDie = function(self, buff, target, removeType)
    -- function num : 0_3 , upvalues : _ENV
    ((self.caster).auSource):PlayAudioById((self.config).audioId2)
    if target ~= self.caster or (target.recordTable).CounterSpellBuffId == nil or
        (target.recordTable).buffDieHealRatio == nil or
        (target.recordTable).CounterSpellCount <= 0 then return end
    if removeType == eBuffRemoveType.Timeout and buff.dataId ==
        (target.recordTable).CounterSpellBuffId then
        local healValue = LuaSkillCtrl:CallFormulaNumber(
                              (self.config).healFormula, target, target,
                              (target.recordTable).buffDieHealRatio)
        LuaSkillCtrl:CallHeal(healValue, self, target)
    end
end
