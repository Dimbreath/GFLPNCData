...
UINCSTDetail_OverloadItem.OnInit = function(self)
  ...
  ;
  (UIUtil.AddValueChangedListener)((self.ui).tog_skillDetailItem, self, self.OnTogChange)
end

UINCSTDetail_OverloadItem.InitOverladItem = function(self, skillData, resloader, treeData)
  ...
  resloader:LoadABAssetAsync(PathConsts:GetAtlasAssetPath("CommanderSkillIcons"), function(spriteAtlas)
    ...
    ;
    ((self.ui).img_Icon).sprite = spriteAtlas:GetSprite(skillData:GetIcon())
  end
)
...
UINCSTDetail_OverloadItem.OnTogChange = function(self, bool)
  ...
  if bool then
    ...
    ;
    (self.treeData):SetSlotSkillList((self.skillData).place, (self.skillData).dataId)
  else
    ;
