...
local UIBattleSkadaInfoPage = require(
                                  "Game.BattleResult.SkadaInfo.UIBattleSkadaInfoPage")
...
UIBattleSkadaPage.OnInit = function(self)
    ...
    (((self.ui).tog_Player).onValueChanged):AddListener(
        BindCallback(self, self.OnClickRoleChange, RoleType.Player));
    (((self.ui).tog_Enemy).onValueChanged):AddListener(
        BindCallback(self, self.OnClickRoleChange, RoleType.Enemy));
    (((self.ui).tog_DPS).onValueChanged):AddListener(
        BindCallback(self, self.OnClickDetail,
                     (BattleSkadaEnum.eBattleSkadaType).Damage));
    (((self.ui).tog_Tank).onValueChanged):AddListener(
        BindCallback(self, self.OnClickDetail,
                     (BattleSkadaEnum.eBattleSkadaType).TakeDamage));
    (((self.ui).tog_Heal).onValueChanged):AddListener(
        BindCallback(self, self.OnClickDetail,
                     (BattleSkadaEnum.eBattleSkadaType).Heal));
    ((self.ui).skadaItem):SetActive(false)
    self.skadaItemPool =
        (UIItemPool.New)(UIBattleSkadaItem, (self.ui).skadaItem)
end

UIBattleSkadaPage.InitBattleSkadaPage = function(self, skadaInfoPage,
                                                 battleStatistics,
                                                 playerRoleList, enemyRoleList,
                                                 resLoader)
    -- function num : 0_1 , upvalues : cs_BattleStatistics, BattleSkadaEnum, RoleType
    cs_BattleStatistics = battleStatistics -- DECOMPILER ERROR at PC2: Confused about usage of register: R6 in 'UnsetPending'
    ;
    (self.ui).skadaInfoPage = skadaInfoPage
    self.resLoader = resLoader
    self.playerSkada = self:GenSkada(playerRoleList,
                                     cs_BattleStatistics.playerDamage,
                                     cs_BattleStatistics.playerTakeDamage,
                                     cs_BattleStatistics.playerHeal)
    self.enemySkada = self:GenSkada(enemyRoleList,
                                    cs_BattleStatistics.enemyDamage,
                                    cs_BattleStatistics.enemyTakeDamage,
                                    cs_BattleStatistics.enemyHeal)
    self.skadaType = (BattleSkadaEnum.eBattleSkadaType).Damage
    self.roleType = RoleType.Player -- DECOMPILER ERROR at PC25: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).tog_Player).isOn = true -- DECOMPILER ERROR at PC28: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).tog_DPS).isOn = true
    self:RefreshSkadaUI()
end

UIBattleSkadaPage.GenSkada = function(self, roleList, damageDic, takeDamageDic,
                                      healDic)
    ...
    for i = 0, roleList.Count - 1 do
        ...
        if not (role.character).onBench then
            ...
            end -- DECOMPILER ERROR at PC32: Confused about usage of register: R18 in 'UnsetPending'
            ...
            end -- DECOMPILER ERROR at PC47: Confused about usage of register: R21 in 'UnsetPending'
            
            (skada.detail)[(BattleSkadaEnum.eBattleSkadaType).TakeDamage] =
                takeDamage
            ...
            end -- DECOMPILER ERROR at PC62: Confused about usage of register: R24 in 'UnsetPending'
    ...
    for k, v in ipairs(skadaList) do
        ...
        ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).Damage]).proportion =
            ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).Damage]).value /
                (damageTotal) -- DECOMPILER ERROR at PC94: Confused about usage of register: R14 in 'UnsetPending'
        ;
        ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).TakeDamage]).proportion =
            ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).TakeDamage]).value /
                (takeDamageTotal) -- DECOMPILER ERROR at PC105: Confused about usage of register: R14 in 'UnsetPending'
        ;
        ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).Heal]).proportion =
            ((v.detail)[(BattleSkadaEnum.eBattleSkadaType).Heal]).value /
                (healTotal)
    end
    local skadaDetail = {}
...
local SkadaSortFunc = {
    [(BattleSkadaEnum.eBattleSkadaType).Damage] = function(skada1, skada2)
        ...
        local damage1 =
            ((skada1.detail)[(BattleSkadaEnum.eBattleSkadaType).Damage]).value
        local damage2 =
            ((skada2.detail)[(BattleSkadaEnum.eBattleSkadaType).Damage]).value
    ...
    [(BattleSkadaEnum.eBattleSkadaType).TakeDamage] = function(skada1, skada2)
        ...
        local takeDamage1 =
            ((skada1.detail)[(BattleSkadaEnum.eBattleSkadaType).TakeDamage]).value
        local takeDamage2 =
            ((skada2.detail)[(BattleSkadaEnum.eBattleSkadaType).TakeDamage]).value
    ...
    [(BattleSkadaEnum.eBattleSkadaType).Heal] = function(skada1, skada2)
        ...
        local heal1 =
            ((skada1.detail)[(BattleSkadaEnum.eBattleSkadaType).Heal]).value
        local heal2 =
            ((skada2.detail)[(BattleSkadaEnum.eBattleSkadaType).Heal]).value
        if heal2 >= heal1 then
...
UIBattleSkadaPage.RefreshSkadaUI = function(self)
    ...
    if self.roleType == RoleType.Player then
        ...
        if self.roleType == RoleType.Enemy then skada = self.enemySkada end
    end
    (table.sort)(skada.list, SkadaSortFunc[self.skadaType])
    for k, v in ipairs(skada.list) do
        local item = (self.skadaItemPool):GetOne()
        item:InitSkadaItem(self.resLoader,
                           BindCallback(self, self.OnClickHeroDetail))
        item:RefreshSkadaItem(k, v, self.skadaType,
...
UIBattleSkadaPage.OnClickHeroDetail = function(self, roleSkada, roleIndex)
    ...
    (self.skadaInfoPage):InitSkadaInfoPage(roleSkadaDetail, belongPlayer,
                                           self.resLoader, self.skadaType,
                                           BindCallback(self,
                                                        self.OnSwitchPlayer))
...
UIBattleSkadaPage.GenRoleSkadaDetail = function(self, roleSkada, belongPlayer)
    ...
    local damageDic = belongPlayer and cs_BattleStatistics.playerDamage or
                          cs_BattleStatistics.enemyDamage
    ...
    local takeDamageDic =
        belongPlayer and cs_BattleStatistics.playerTakeDamage or
            cs_BattleStatistics.enemyTakeDamage
    ...
    local healDic = belongPlayer and cs_BattleStatistics.playerHeal or
                        cs_BattleStatistics.enemyHeal
...
UIBattleSkadaPage.OnDelete = function(self)
    ...
    if self.skadaInfoPage ~= nil then (self.skadaInfoPage):Delete() end
    (base.OnDelete)(self)
end
