...
UIDungeonChapterList.UpdateIsDouble = function(self)
  -- function num : 0_6
  ((self.ui).obj_double):SetActive((self.UIData):IsDouble(((self.selectChapterItem).cfg).dungeon_type))
end
...
UIDungeonChapterList.CreatePool = function(self, chapterItemClass,
                                           fstRewardItemClass, mbDropItemClass)
    -- function num : 0_1 , upvalues : _ENV
    local itemPool, fstRewardPool, mbDropPool = nil, nil, nil
    if chapterItemClass ~= nil then
        itemPool = (UIItemPool.New)(chapterItemClass, (self.ui).chapterItem)
    ...
    if fstRewardItemClass ~= nil then
        fstRewardPool = (UIItemPool.New)(fstRewardItemClass,
                                         (self.ui).firstReward)
    end
    if mbDropItemClass ~= nil then
        mbDropPool = (UIItemPool.New)(mbDropItemClass, (self.ui).maybeReward)
...
UIDungeonChapterList.UpdateWithChapterList =
    function(self, chapterItemList, completeCount, totalCount,
             onStartBattleEvent, UIData)
        -- function num : 0_2 , upvalues : _ENV, ChapterState
        self.chapterItemList = chapterItemList
        self.chapterCount = totalCount
        self.onStartBattleEvent = onStartBattleEvent
        self.UIData = UIData
        local itemClickEvent = BindCallback(self,
                                            self.OnShowDetailRectAndSetSelectChapter)
        local updateLimitDisplayEvent = (BindCallback(self,
                                                      self.UpdateChapterLimitDisplay))
        local lastUnlockedItem = nil
        for k, v in ipairs(chapterItemList) do
            if v ~= nil then
                v.onClickAction = itemClickEvent
        ...
        if lastUnlockedItem ~= nil then lastUnlockedItem:__onClick() end
        self:UpdateChapterProgress(completeCount, totalCount)
    end

UIDungeonChapterList.UpdateChapterLimitDisplay =
    function(self, matDungeonItem)
        -- function num : 0_3 , upvalues : _ENV
        ((self.ui).tex_LimitCount):SetIndex(0,
                                            tostring(matDungeonItem.usedLimit),
                                            tostring(matDungeonItem.dailyLimit))
    end

UIDungeonChapterList.UpdateChapterProgress =
    function(self, completeCount, totalCount)
        -- function num : 0_4
        -- DECOMPILER ERROR at PC5: Confused about usage of register: R3 in 'UnsetPending'
...
UIDungeonChapterList.OnShowDetailRectAndSetSelectChapter =
    function(self, show, chapterItem)
        -- function num : 0_5 , upvalues : _ENV, ChapterState, cs_MessageCommon
        if chapterItem ~= nil then
            self.selectChapterItem = chapterItem;
            ((self.ui).tex_Point):SetIndex(0, tostring(
                                               chapterItem.costStrengthNum))
            if chapterItem.state == ChapterState.lock then
                (((self.ui).btn_Battle).gameObject):SetActive(false);
                (cs_MessageCommon.ShowMessageTips)(chapterItem.lockReason)
            else
            ...
            (((self.ui).img_chapterSelect).transform):SetParent(
                chapterItem.transform, false);
            ((self.ui).img_chapterSelect):SetActive(true)
            self:UpdateIsDouble(chapterItem)
        end
        if chapterItem == nil then
...
UIDungeonChapterList.UpdateIsDouble = function(self)
    ...
    ((self.ui).obj_double):SetActive((self.UIData):IsDouble(
                                         ((self.selectChapterItem).cfg).dungeon_type))
end

UIDungeonChapterList.__onBattleStart = function(self)
    -- function num : 0_7
    if self.onStartBattleEvent ~= nil then (self.onStartBattleEvent)() end
end
