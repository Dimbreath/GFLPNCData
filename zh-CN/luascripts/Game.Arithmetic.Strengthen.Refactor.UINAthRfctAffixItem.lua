...
UINAthRfctAffixItem.InitAthRfctAffix = function(self, index, affixElem,
                                                clickFunc, parent, withoutLock)
    -- function num : 0_1
    self.index = index
    self.clickFunc = clickFunc;
    (self.transform):SetParent(parent)
...
UINAthRfctAffixItem.RefreshAthRfctAffixItem =
    function(self, affixElem, withoutLock)
        -- function num : 0_2 , upvalues : _ENV, ArthmeticEnum
        local affixcfg = (ConfigData.ath_affix_pool)[affixElem.id]
        if affixcfg == nil then
            error("Cant\'t find ath_affix_pool, id = " .. tostring(affixElem.id))
        ...
        local name, valueStr, icon = ConfigData:GetAttribute(
                                         (self.affixcfg).affix_para,
                                         affixElem.value) -- DECOMPILER ERROR at PC27: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.ui).img_Icon).sprite = CRH:GetSprite(icon) -- DECOMPILER ERROR at PC33: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.ui).img_Icon).color =
            (ArthmeticEnum.AthQualityColor)[affixElem.quality]
        local isMaxLevel = (ConfigData.ath_affix_lv).maxAffixLevel <=
                               affixElem.level
        if isMaxLevel then
            ((self.ui).tex_Num):SetIndex(1, valueStr)
        else
...
UINAthRfctAffixItem.GetAffixCanvasGroup =
    function(self)
        -- function num : 0_5
        return (self.ui).canvasGroup
    end
