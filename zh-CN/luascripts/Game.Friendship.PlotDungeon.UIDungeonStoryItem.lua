...
local UICondition = require(
                        "Game.Friendship.PlotDungeon.UIDungeonStoryCondition")
...
UIDungeonStoryItem.InitWithData = function(self, cfg)
    ...
    self.cfg = cfg -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_StoryName).text = (LanguageUtil.GetLocaleText)(cfg.name) -- DECOMPILER ERROR at PC15: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.gameObject).name = (self.cfg).id
    self:UpdateUnLockCondition(cfg)
end

UIDungeonStoryItem.UpdateUnLockCondition =
    function(self, cfg)
        ...
        for i = 1, conditionCount do
            local isMeet = (CheckCondition.CheckLua)({(cfg.pre_condition)[i]},
                                                     {(cfg.pre_para1)[i]},
                                                     {(cfg.pre_para2)[i]}) -- DECOMPILER ERROR at PC42: Confused about usage of register: R8 in 'UnsetPending'
            ;
            (self.conditionTextDict)[(cfg.pre_condition)[i]] =
                (AvgUtil.GetConditionText)((cfg.pre_condition)[i],
                                           (cfg.pre_para1)[i],
                                           (cfg.pre_para2)[i]) -- DECOMPILER ERROR at PC46: Confused about usage of register: R8 in 'UnsetPending'
            ;
        ...
        if not self.isUnlock then
            if self.conditionPool == nil then
                self.conditionPool = (UIItemPool.New)(UICondition,
                                                      (self.ui).conditionItem)
            end
            (self.conditionPool):HideAll()
            for i = 1, conditionCount do
                local conditionItem = (self.conditionPool):GetOne()
                local condId = (cfg.pre_condition)[i]
                conditionItem:InitWithData((self.conditionTextDict)[condId],
                                           (self.conditionLockedDict)[condId])
            end
        end
    end
