...
UINHomeNoticeItem.InitNoticeItem = function(self, noticeData, OnTweenPlayOver,
                                            resloader)
    -- function num : 0_1 , upvalues : _ENV, CS_DoTween
    self.OnTweenPlayOver = OnTweenPlayOver -- DECOMPILER ERROR at PC5: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).tex_Content).text = noticeData:GetNoticeStr()
    resloader:LoadABAssetAsync(PathConsts:GetAtlasAssetPath("UI_HomeSide"),
...
UINHomeNoticeItem.OnPlayOver = function(self)
    -- function num : 0_2
    if self.OnTweenPlayOver ~= nil then (self.OnTweenPlayOver)() end
    self.sequence = nil
end
