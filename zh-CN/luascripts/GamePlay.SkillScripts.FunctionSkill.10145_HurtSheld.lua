...
bs_10145.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
...
bs_10145.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    -- DECOMPILER ERROR at PC14: Confused about usage of register: R8 in 'UnsetPending'

    if sender == self.caster and skill.isCommonAttack and not isMiss then
        ((self.caster).recordTable)["10145_AttackTime"] =
            ((self.caster).recordTable)["10145_AttackTime"] + 1
        if (self.arglist)[1] <= ((self.caster).recordTable)["10145_AttackTime"] then
            local targetlist = LuaSkillCtrl:CallTargetSelect(self, 30, 10)
            ...
            LuaSkillCtrl:CallBuff(self, (targetlist[0]).targetRole,
                                  ...
                                  (self.arglist)[3]) -- DECOMPILER ERROR at PC48: Confused about usage of register: R9 in 'UnsetPending'
            ;
            ((self.caster).recordTable)["10145_AttackTime"] = 0
        end
    end
