...
UIAthInfoNode.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_BarStrengthen, self, self.OnClickSlotUpgrade)
  ;
  ...
  self.slotSwitchPool = (UIItemPool.New)(UIAthSlotItemSwitch, (self.ui).uI_ATHBarItem_Switch)
  self.athDetailPool = (UIItemPool.New)(UIAthItemDetail, (self.ui).athItemDetail)
  self.athAttrPool = (UIItemPool.New)(UIAthInfoAttrItem, (self.ui).attriDetail_Item)
  ...
  self.__onAthDetailMainCloseEvent = BindCallback(self, self.OnAthDetailMainClose)
  self.__onAthDetailSubCloseEvent = BindCallback(self, self.OnAthDetailSubClose)
...
UIAthInfoNode.ShowSlotDetail = function(self, slotId)
  ...
  local slotLevel = (PlayerDataCenter.allAthData):GetHeroAthSlotLevel((self.heroData).dataId, slotId)
  ...
  local athDataList, athDic = (PlayerDataCenter.allAthData):GetHeroAthList((self.heroData).dataId, slotId)
  ...
  if lockSpace <= 0 or not imgCount + 1 then
    ...
    local totalHeight = ((verLayout.transform).rect).height - (verLayout.padding).vertical - verLayout.spacing * (imgCount - 1)
    local curSlotDensity = self:GetSlotDensity(slotId, curSpace)
    ;
    ((self.ui).tex_ATHDensity):SetIndex(0, GetPreciseDecimalStr(curSlotDensity, 2))
    ;
    (self.athDetailPool):HideAll()
    if lockSpace > 0 then
      local athItem = (self.athDetailPool):GetOne()
      local height = lockSpace / fullSpace * totalHeight
      athItem:InitAthItem((self.ui).lockColor, height, lockSpace, nil, false)
    end
    do
      if emptySpace > 0 then
        local athItem = (self.athDetailPool):GetOne()
        local height = emptySpace / fullSpace * totalHeight
        athItem:InitAthItem((self.ui).emptyColor, height, emptySpace, nil, true)
      ...
      do
        for k,athData in ipairs(athDataList) do
          ...
          athItem:InitAthItem(athData:GetColor(), height, athData.size, athData)
          athItem:SetClickAthEvent(self.__onClickAth)
        end
        ;
...
UIAthInfoNode.GetSlotDensity = function(self, slotId, size)
  ...
  local attrDic = setmetatable({}, {__index = function(tab, key)
    -- function num : 0_4_0
    return 0
  end
})
  local athList = (PlayerDataCenter.allAthData):GetHeroAthList((self.heroData).dataId, slotId)
...
UIAthInfoNode.RefreshAthAttr = function(self, athSlotList, curSlotId)
  ...
  for k,attrId in ipairs(allAttrIdList) do
    ...
    if attrConfig.merge_attribute ~= 0 then
      ...
      if not (valueList[mergeID]).v1 then
        do
          (valueList[mergeID]).v1 = (attrConfig.num_type ~= 2 or 0) + allAttrDic[attrId] * (self.heroData):GetAttr(mergeID, true) // 1000
          -- DECOMPILER ERROR at PC83: Confused about usage of register: R15 in 'UnsetPending'

          ;
          (valueList[mergeID]).v2 = (slotAttrTab[curSlotId] == nil or 0) + (not (valueList[mergeID]).v2 and (slotAttrTab[curSlotId])[attrId] or 0) * (self.heroData):GetAttr(mergeID, true) // 1000
          -- DECOMPILER ERROR at PC93: Confused about usage of register: R15 in 'UnsetPending'

          ;
          (valueList[mergeID]).v1 = ((valueList[mergeID]).v1 or 0) + allAttrDic[attrId]
          -- DECOMPILER ERROR at PC109: Confused about usage of register: R15 in 'UnsetPending'

          ;
          (valueList[mergeID]).v2 = (slotAttrTab[curSlotId] == nil or 0) + (not (valueList[mergeID]).v2 and (slotAttrTab[curSlotId])[attrId] or 0)
          ...
          if attrConfig.num_type == 2 then
            if attrId > 17 then
              ...
              if not (slotAttrTab[curSlotId])[attrId] then
                do
                  (valueList[attrId]).v2 = slotAttrTab[curSlotId] == nil or 0
                  -- DECOMPILER ERROR at PC150: Confused about usage of register: R14 in 'UnsetPending'

                  ;
                  (valueList[attrId]).v1 = ((valueList[attrId]).v1 or 0) + allAttrDic[attrId] * (self.heroData):GetAttr(attrId, true) // 1000
                  -- DECOMPILER ERROR at PC173: Confused about usage of register: R14 in 'UnsetPending'

                  ;
                  (valueList[attrId]).v2 = (slotAttrTab[curSlotId] == nil or 0) + (not (valueList[attrId]).v2 and (slotAttrTab[curSlotId])[attrId] or 0) * (self.heroData):GetAttr(attrId, true) // 1000
                  -- DECOMPILER ERROR at PC183: Confused about usage of register: R14 in 'UnsetPending'

                  ;
                  (valueList[attrId]).v1 = ((valueList[attrId]).v1 or 0) + allAttrDic[attrId]
                  -- DECOMPILER ERROR at PC199: Confused about usage of register: R14 in 'UnsetPending'

                  ;
                  (valueList[attrId]).v2 = (slotAttrTab[curSlotId] == nil or 0) + (not (valueList[attrId]).v2 and (slotAttrTab[curSlotId])[attrId] or 0)
  ...
  local content = ""
  local suitIdList = {}
  for k,v in pairs(suitDic) do
    (table.insert)(suitIdList, k)
  end
  ;
  ...
  for k,suitId in ipairs(suitIdList) do
    ...
    content = content .. ((self.ui).tex_EffectDetail):GetIndex(0, suitName, suitNum, suitDetail)
  end
  ;
...
UIAthInfoNode.OnClickAth = function(self, athItem)
  ...
  if athData ~= nil then
    ...
    window:ShowAthDetailMain(athData, self.curSlotEfficiency, (self.heroData).dataId, self.__onAthInstallEvent, self.__uninstallAthEvent, self.__onAthDetailMainCloseEvent)
...
UIAthInfoNode.ShowAthSortList = function(self, athData)
  ...
  if athData ~= nil then
    usableSpace = usableSpace + athData.size
  end
  ...
  ;
  (self.sortListNode):InitAthSortList(self.curSlotId, usableSpace, self.curSlotAthDataDic)
  ;
...
UIAthInfoNode.OnClickSortListAth = function(self, athData)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.AthDetail, function(window)
    ...
    window:ShowAthDetailSub(athData, self.curSlotEfficiency, (self.heroData).dataId, self.__onAthInstallEvent, self.__onAthDetailSubCloseEvent)
    self.athDetailWindow = window
  end
)
end
...
UIAthInfoNode.OnAthInstall = function(self)
  ...
  if (self.curSlotAthDataDic)[athData.uid] ~= nil then
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_HasInstalledInfo))
    return 
  end
  if self.curSlotUsableSpace < athData.size then
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_UsableSpaceInsufficient))
    return 
  end
  if self.selectAthItem == nil then
...
UIAthInfoNode.InstallAth = function(self, athData)
  ...
  ;
  (self.athNetwork):CS_ATH_AthInstall((self.heroData).dataId, self.curSlotId, athData.uid, self.__onInstallComplete)
end

UIAthInfoNode.OnInstallComplete = function(self)
  -- function num : 0_14 , upvalues : cs_MessageCommon, _ENV
  (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_InstallSuccess))
...
UIAthInfoNode.UninstallAth = function(self, uid)
  ...
  if self.__onUninstallComplete == nil then
    self.__onUninstallComplete = BindCallback(self, self.OnUninstallComplete)
  end
  ;
...
UIAthInfoNode.OnUninstallComplete = function(self)
  ...
  (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_UninstallSuccess))
  self:ShowSlotDetail(self.curSlotId)
end
...
UIAthInfoNode.ReplaceAth = function(self, newAthData)
  ...
  ;
  (self.athNetwork):CS_ATH_AthReplace(((self.selectAthItem).athData).uid, newAthData.uid, self.__onReplaceComplete)
end

UIAthInfoNode.OnReplaceComplete = function(self)
  -- function num : 0_18 , upvalues : cs_MessageCommon, _ENV
  (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_ReplaceSuccess))
  self:ShowSlotDetail(self.curSlotId)
end
