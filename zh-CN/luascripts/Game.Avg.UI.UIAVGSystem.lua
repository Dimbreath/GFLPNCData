...
UIAVGSystem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINAvgImgItem
    (UIUtil.AddButtonListener)((self.ui).btn_ShwoLogList, self,
                               self.OnClickShwoLogList);
    (UIUtil.AddButtonListener)((self.ui).btn_Skip, self, self.OnClickSkip);
    (UIUtil.AddButtonListener)((self.ui).btn_AutoPlay, self,
                               self.OnClickAutoPlay);
    (UIUtil.AddButtonListener)((self.ui).btn_bg, self, self.OnClickBackground);
    ((self.ui).imgItem):SetActive(false);
    ((self.ui).heroItem):SetActive(false);
    ((self.ui).communication):SetActive(false) -- DECOMPILER ERROR at PC47: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).img_bg).color = Color.clear
    self.imgItemPool = (UIItemPool.New)(UINAvgImgItem, (self.ui).imgItem)
    self.distantViewImgDic = {}
    ...
    self.audioBackDic = {}
    self.imgCount = 0
    self.__onAvgImgTweenComplete =
        BindCallback(self, self.OnAvgImgTweenComplete)
end
...
UIAVGSystem.__UpdateBlackEdge = function(self)
    ...
    local needEdge = curRatio < defaultScreenRatio;
    ((self.ui).blackEdge):SetActive(needEdge)
    if not needEdge then return end
    local height = (UIManager.BackgroundStretchSize).x / defaultScreenRatio
    local extraHeight = ((UIManager.BackgroundStretchSize).y - height) / 2
    local sizeDelta = ((self.ui).blackEdge_Top).sizeDelta
    sizeDelta.y = extraHeight -- DECOMPILER ERROR at PC32: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).blackEdge_Top).sizeDelta = sizeDelta -- DECOMPILER ERROR at PC35: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).blackEdge_Bottom).sizeDelta = sizeDelta;
    (self.dialogNode):SetAvgDialogBottom(extraHeight)
    -- DECOMPILER ERROR: 2 unprocessed JMP targets
...
UIAVGSystem.PlayAvgAct = function(self, actCfg)
    ...
    self:RefreshAvgImg(actCfg.images)
    self.hasImgTween =
        actCfg.imgTween ~= nil and (table.count)(actCfg.imgTween) > 0
    self.hasContent = actCfg.content ~= nil
    self.hasBranch = actCfg.branch ~= nil and (table.count)(actCfg.branch) > 0
    self.hasVideo = actCfg.vedioPath ~= nil
    self:PlayAvgVideoLoopBg();
...
UIAVGSystem.PlayAvgOrder = function(self)
    ...
    self:CheckActComplete()
    if (self.actCfg).autoContinue then (self.avgCtrl):ClickContinueAvg() end
end

UIAVGSystem.CheckActComplete = function(self)
    -- function num : 0_6
    if self.hasContent or self.voiceIsPlaying or self.hasVideo or self.hasBranch or
        self.hasImgTween then return end
...
UIAVGSystem.GetAvgSpeakerName = function(self, heroId)
    -- function num : 0_7 , upvalues : _ENV
    if heroId == -1 then return heroId end
    local heroCfg = (ConfigData.avg_character)[heroId]
    if heroCfg == nil then
        warn("Can\'t find avg_character cfg, speakerHeroId = " ..
                 tostring(heroId))
        return
    end
    return (LanguageUtil.GetLocaleText)(heroCfg.name)
...
UIAVGSystem.ShowAvgContent = function(self)
    ...
    local content = (self.avgCtrl):GetAvgLanguage(actCfg.content)
    if actCfg.contentType == eAvgContentType.Chapter then
        (self.chapterNode):ShowAvgChapter(content, actCfg.contentShake,
                                          self.skipTween,
                                          actCfg.scrambleTypeWriter)
    else
        if actCfg.contentType == eAvgContentType.Narratage then
            (self.dialogNode):ShowAvgDialog(content, actCfg.contentShake,
                                            self.skipTween,
                                            actCfg.scrambleTypeWriter, nil, true)
        else
            if actCfg.contentType == eAvgContentType.NarratageWithSpeaker then
                local speakerName = (self.avgCtrl):GetAvgLanguage(
                                        actCfg.speakerName);
                (self.dialogNode):ShowAvgDialog(content, actCfg.contentShake,
                                                self.skipTween,
                                                actCfg.scrambleTypeWriter,
                ...
                do
                    if not actCfg.speakerHeroPosId then
                        local posId = actCfg.contentType ~=
                                          eAvgContentType.HeroDialog or 2
                    end
                    if eAvgImgPosType[posId] == nil then
                        posId = 2
                        error("speakerHeroPosId error, id = " ..
                                  actCfg.speakerHeroPosId)
                    end
                    do
                        local posX = eAvgDialogPosX[posId];
                        (self.dialogNode):ShowAvgDialog(content,
                                                        actCfg.contentShake,
                                                        self.skipTween,
                                                        actCfg.scrambleTypeWriter,
                                                        self:GetAvgSpeakerName(
                                                            actCfg.speakerHeroId),
                                                        false, posX)
                        if not actCfg.tipsShowDuration then
                            (CS_MessageCommon.ShowMessageTips)(content, true,
                                                               actCfg.contentType ~=
                                                                   eAvgContentType.Tips or
                                                                   0,
                                                               actCfg.tipsTypeWriter)
                            self:ShowTextComplete()
                            self:PlayAvgVoice()
                        end
                    end
...
UIAVGSystem.DoCustomCode = function(self)
    -- function num : 0_10 , upvalues : _ENV
    if (string.IsNullOrEmpty)((self.actCfg).customCode) then return end
    local code = load((self.actCfg).customCode)
    if code == nil then return end
    local status, current = xpcall(code, debug.traceback)
    if not status then error(current) end
end
...
UIAVGSystem.PlayAvgVideoLoopBg = function(self)
    -- function num : 0_13
    if (self.actCfg).vedioLoopPath ~= nil then
        (self.videoNode):PlayAvgVideoLoop((self.actCfg).vedioLoopPath,
                                          (self.actCfg).vedioLoopFrame)
    else
        if (self.actCfg).vedioLoopFrame ~= nil then
            (self.videoNode):SetAvgVideoLoop((self.actCfg).vedioLoopFrame)
        end
    end
    if (self.actCfg).vedioLoopStop then (self.videoNode):StopAvgVideoLoop() end
end

UIAVGSystem.PlayAvgAudio = function(self)
    -- function num : 0_14 , upvalues : _ENV
    if (self.actCfg).audio == nil then return end
    local audioCfg = (self.actCfg).audio
    if audioCfg.bgm ~= nil then
        if (audioCfg.bgm).stop then
            AudioManager:StopSource(eAudioSourceType.BgmSource)
            ...
            AudioManager:SetBgmSourceFadeInAndOutTime(fadeInTime, fadeOutTime)
            AudioManager:PlayAudio((audioCfg.bgm).cue, (audioCfg.bgm).sheet,
                                   eAudioSourceType.BgmSource)
        end
    end
    do
        do
            if audioCfg.sfx ~= nil then
                local auBack = AudioManager:PlayAudio((audioCfg.sfx).cue,
                                                      (audioCfg.sfx).sheet,
                                                      eAudioSourceType.SfxSource);
                (self.avgCtrl):RecordAvgAudioSheet((audioCfg.sfx).sheet)
                -- DECOMPILER ERROR at PC73: Confused about usage of register: R3 in 'UnsetPending'
            ...
            if audioCfg.stopAudioId ~= nil then
                for k, id in ipairs(audioCfg.stopAudioId) do
                    local auBack = (self.audioBackDic)[id]
                    if auBack == nil then
                        warn("[avg] stopAudioId is invalid, id : " ..
                                 tostring(id))
                    else
                        AudioManager:StopAudioByBack(auBack) -- DECOMPILER ERROR at PC98: Confused about usage of register: R8 in 'UnsetPending'
                        ;
                        (self.audioBackDic)[id] = nil
                    end
                end
...
UIAVGSystem.PlayAvgVoice = function(self)
    ...
    if audioCfg.voice ~= nil then
        self.voiceIsPlaying = true
        if self.__onAudioPlayComplete == nil then
            self.__onAudioPlayComplete =
                BindCallback(self, self.AudioPlayComplete)
        end
        AudioManager:PlayAudio((audioCfg.voice).cue, (audioCfg.voice).sheet,
                               eAudioSourceType.VoiceSource,
                               self.__onAudioPlayComplete)
    end
end
...
UIAVGSystem.RefreshAvgImg = function(self, imgCfgList)
    -- function num : 0_19 , upvalues : _ENV
    if imgCfgList == nil then return end
    self.imgNeedSort = {}
    for k, v in ipairs(imgCfgList) do
        if v.delete then
            self:RecycleImgItem(v.imgId)
    ...
    self:SortAvtImgItem();
    (((self.ui).heroesNode).gameObject):SetActive(
        (table.count)(self.heroPicDic) > 0)
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end
...
UIAVGSystem.NewImgItem = function(self, imgCfg)
    ...
    if imgType == eAvgImgType.Background then
        imgItem = (self.bgImgDic)[imgId]
        if imgItem == nil then
            imgItem = (self.imgItemPool):GetOne() -- DECOMPILER ERROR at PC15: Confused about usage of register: R5 in 'UnsetPending'
            ;
            (self.bgImgDic)[imgId] = imgItem
        end
        imgItem:InitAvgImgItem(imgCfg, (self.ui).background,
                               self.__onAvgImgTweenComplete)
    else
        if imgType == eAvgImgType.Character then
            imgItem = (self.heroPicDic)[imgId]
            if imgItem == nil then
                imgItem = (UINAvgHeroPic.New)() -- DECOMPILER ERROR at PC34: Confused about usage of register: R5 in 'UnsetPending'
                ;
                (self.heroPicDic)[imgId] = imgItem
            end
            imgItem:InitAvgHeroPic(imgCfg, (self.ui).heroItem,
                                   self.__onAvgImgTweenComplete)
        else
            if imgType == eAvgImgType.Foreground then
                imgItem = (self.foregroundImgDic)[imgId]
                if imgItem == nil then
                    imgItem = (self.imgItemPool):GetOne() -- DECOMPILER ERROR at PC54: Confused about usage of register: R5 in 'UnsetPending'
                    ;
                    (self.foregroundImgDic)[imgId] = imgItem
                end
                imgItem:InitAvgImgItem(imgCfg, (self.ui).foreground,
                                       self.__onAvgImgTweenComplete)
            else
                if imgType == eAvgImgType.DistantView then
                    imgItem = (self.distantViewImgDic)[imgId]
                    if imgItem == nil then
                        imgItem = (self.imgItemPool):GetOne() -- DECOMPILER ERROR at PC74: Confused about usage of register: R5 in 'UnsetPending'
                        ;
                        (self.distantViewImgDic)[imgId] = imgItem
                    end
                    imgItem:InitAvgImgItem(imgCfg, (self.ui).distantView,
                                           self.__onAvgImgTweenComplete)
                else
                    local imgItem = self:GetImgItem(imgId, true)
                    if imgItem == nil then
                        ...
                        local chapterName = (self.avgCtrl):GetCurChapterName()
                        warn((string.format)(
                                 "Unsupported avg imgType, imgType = %s, imgId = %s, actId = %s, chapterName = %s",
                                 imgType, imgId, actId, chapterName))
                        return
                    else
                        do
                            do
                                if imgCfg.order ~= nil then
                                    imgType = imgItem:GetAvgImgType()
                                    imgItem:ChangeAvgImgOrder(imgCfg.order) -- DECOMPILER ERROR at PC116: Confused about usage of register: R6 in 'UnsetPending'
                                    ;
                                    (self.imgNeedSort)[imgType] = true
                                    return
                                ...
                                if imgMaxCount < self.imgCount then
                                    warn(
                                        "Avg image count is beyond limit, imgCount = " ..
                                            tostring(self.imgCount))
                                end
                            end
                        end
                    end
...
UIAVGSystem.RecycleImgItem = function(self, imgId)
    ...
    local imgItem = nil
    if (self.bgImgDic)[imgId] ~= nil then
        imgItem = (self.bgImgDic)[imgId] -- DECOMPILER ERROR at PC8: Confused about usage of register: R3 in 'UnsetPending'
        ;
        (self.bgImgDic)[imgId] = nil;
        (self.imgItemPool):HideOne(imgItem)
    else
        if (self.heroPicDic)[imgId] ~= nil then
            imgItem = (self.heroPicDic)[imgId] -- DECOMPILER ERROR at PC21: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.heroPicDic)[imgId] = nil
            imgItem:Delete()
        else
            if (self.foregroundImgDic)[imgId] ~= nil then
                imgItem = (self.foregroundImgDic)[imgId] -- DECOMPILER ERROR at PC32: Confused about usage of register: R3 in 'UnsetPending'
                ;
                (self.foregroundImgDic)[imgId] = nil;
                (self.imgItemPool):HideOne(imgItem)
            else
                if (self.distantViewImgDic)[imgId] ~= nil then
                    imgItem = (self.distantViewImgDic)[imgId] -- DECOMPILER ERROR at PC45: Confused about usage of register: R3 in 'UnsetPending'
                    ;
                    (self.distantViewImgDic)[imgId] = nil;
                    (self.imgItemPool):HideOne(imgItem)
                else
                    warn("Can\'t find avg img to delete, imgId = " ..
                             tostring(imgId))
                    return
                end
            end
        ...
        if (self.heroPicDic)[imgId] ~= nil then
            ...
            if (self.foregroundImgDic)[imgId] ~= nil then
                ...
                if (self.distantViewImgDic)[imgId] ~= nil then
                    ...
                    if not withoutWarn then
                        warn("Can\'t find avg img item, imgId = " ..
                                 tostring(imgId))
                        return
                    end
                end
...
UIAVGSystem.RefreshAvgImgTween = function(self, tweenCfgList)
    -- function num : 0_24 , upvalues : _ENV
    if tweenCfgList == nil then return end
    self.tweenImgItemDic = {}
    for k, tweenCfg in ipairs(tweenCfgList) do
        local imgId = tweenCfg.imgId
        local imgItem = self:GetImgItem(imgId)
        if imgItem ~= nil then
            imgItem:AddAvgImgTween(tweenCfg) -- DECOMPILER ERROR at PC19: Confused about usage of register: R9 in 'UnsetPending'
            ;
            (self.tweenImgItemDic)[imgId] = imgItem
        end
    end
    for k, v in pairs(self.tweenImgItemDic) do v:PlayAvgImgTween() end
end
...
UIAVGSystem.OnClickShwoLogList = function(self)
    -- function num : 0_27
    self.__lastAutoPlay = (self.avgCtrl):GetAvgAutoPlayMode()
    if self.__lastAutoPlay then (self.avgCtrl):SetAvgAutoPlayMode(false) end
    (self.recordNode):ShowAcgRecord((self.avgCtrl):GetAvgRecordData())
end
...
UIAVGSystem.RefreshAutoPlay = function(self, autoPlay, delayRatio)
    ...
    if self.firstAutoPlay == nil then
        ...
        self.autoIconColor = ((self.ui).img_AutoIcon).color
        self.firstAutoPlay = false
    end -- DECOMPILER ERROR at PC17: Confused about usage of register: R3 in 'UnsetPending'
    
    (((self.ui).img_AutoIcon).transform).position = self.autoIconPos -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.autoIconColor).a = 1 -- DECOMPILER ERROR at PC23: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).img_AutoIcon).color = self.autoIconColor
    local twSequence = nil
    -- DECOMPILER ERROR at PC29: Confused about usage of register: R4 in 'UnsetPending'
    ...
    if autoPlay then
        ...
        twSequence = (CS_DoTween.Sequence)()
        twSequence:Append((((self.ui).img_AutoIcon).transform):DOLocalMoveX(160,
                                                                            1.2))
        twSequence:Join(((self.ui).img_AutoIcon):DOFade(0, 1.2))
        twSequence:SetLoops(-1)
        twSequence:SetUpdate(true)
...
UIAVGSystem.OnClickSkip = function(self)
    -- function num : 0_31 , upvalues : CS_MessageCommon, _ENV
    (CS_MessageCommon.ShowMessageBox)(ConfigData:GetTipContent(
                                          TipContent.Avg_SkipAllAvg),
                                      function()
        -- function num : 0_31_0 , upvalues : self
        (self.avgCtrl):CompleteAllAvg()
    end, nil)
...
UIAVGSystem.OnClickBackground = function(self)
    ...
    if self.hasContent or self.hasImgTween or self.hasBranch then
        ...
        if self.hasImgTween then
            local list = {}
            for k, v in pairs(self.tweenImgItemDic) do list[k] = v end
            for k, v in pairs(list) do v:AvgImgTweenDoComplete() end
        else
            do
                if self.hasContent then
                    if (self.actCfg).contentType == eAvgContentType.Chapter then
...
UIAVGSystem.GetHeroCommItem = function(self)
    -- function num : 0_33 , upvalues : _ENV, UINAvgHeroCommItem
    if self.heroCommItemPool == nil then
        self.heroCommItemPool = (UIItemPool.New)(UINAvgHeroCommItem,
                                                 (self.ui).communication)
    end
    return (self.heroCommItemPool):GetOne(), (self.ui).commHeroMat
end
...
UIAVGSystem.OnDelete = function(self)
    ...
    (self.videoNode):Delete();
    (self.imgItemPool):DeleteAll()
    for k, v in pairs(self.heroPicDic) do v:Delete() end
    self:StopAvgVoice()
    if self.twSequence ~= nil then
        (self.twSequence):Kill()
        self.twSequence = nil
