...
local base = LuaSkillBase
bs_5004.config = {
    effectIdAttack = 10127,
    effectIdAttack2 = 10127,
    effectId1 = 10297,
    effectId2 = 10003,
    effectId3 = 10299
}
...
bs_5004.OnCallback2 = function(self, target)
    ...
    local highAttRole = LuaSkillCtrl:CallTargetSelect(self, 31, 20)
    if highAttRole ~= nil and highAttRole.Count > 0 and highAttRole[0] ~= nil then
        local hurt = ((highAttRole[0]).targetRole).pow * (self.arglist)[1] //
                         1000
        ...
        if LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[2] then
            ((self.caster).recordTable)["5005_time"] = 0
            self.callback = BindCallback(self, self.OnCallback, hurt, target)
            local targetListall = LuaSkillCtrl:CallTargetSelect(self, 2, 20,
                                                                target)
            if targetListall.Count > 1 then
                self:OnCallback(hurt, target)
            end
...
bs_5004.OnCallback = function(self, hurt, target)
    -- function num : 0_4 , upvalues : _ENV
    local highAttRole2 = LuaSkillCtrl:CallTargetSelect(self, 20, 20, target)
    LuaSkillCtrl:CallEffect((highAttRole2[0]).targetRole,
                            (self.config).effectId3, self, nil, target)
    local callback3 = BindCallback(self, self.OnCallback3,
                                   (highAttRole2[0]).targetRole, hurt)
    LuaSkillCtrl:StartTimer(self, 3, callback3) -- DECOMPILER ERROR at PC36: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["5005_time"] =
        ((self.caster).recordTable)["5005_time"] + 1
end

bs_5004.OnCallback3 = function(self, target, hurt)
    -- function num : 0_5 , upvalues : _ENV
    LuaSkillCtrl:RemoveLife(hurt, self, target)
    if LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[2] and
        ((self.caster).recordTable)["5005_time"] < (self.arglist)[3] and
        target.hp > 0 then self:OnCallback(hurt, target) end
end
