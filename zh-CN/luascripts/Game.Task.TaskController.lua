...
TaskController.GetDatas4Task = function(self, typeId)
  -- function num : 0_11 , upvalues : TaskEnum, _ENV, TaskData
...
TaskController.OnTaskCommit = function(self, stcTask)
  ...
  if stcTask.type == (TaskEnum.eTaskType).MainTask or stcTask.type == (TaskEnum.eTaskType).SideTask then
    ...
    -- function num : 0_17_0 , upvalues : stcTask, self
    if window == nil then
      return 
    end
    ...
    if stcTask.type == (TaskEnum.eTaskType).DailyTask or stcTask.type == (TaskEnum.eTaskType).WeeklyTask then
      ...
      (self.network):SendQuestPeriodDetail()
      UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
    -- function num : 0_17_1 , upvalues : stcTask, self
    if window == nil then
      return 
    end
    window:InitRewardsItem(stcTask.rewardIds, stcTask.rewardNums, self._heroIdSnapShoot)
...
TaskController.SendCommitTaskPeriod = function(self, index)
    ...
    self._heroIdSnapShoot = PlayerDataCenter:TakeHeroIdSnapShoot();
    (self.network):SendQuestPeriodRewardPicked(index, self.showingActiveType,
                                               function()
        -- function num : 0_20_0 , upvalues : self, _ENV, index
        (self.network):SendQuestPeriodDetail()
        UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
...
TaskController.IsDailyCompleteToRefresh =
    function(self)
        -- function num : 0_21
        if self.isDailyCompleteToRefresh then return not self.isFrist end
    end
