...
TaskController.OnInit = function(self)
    ...
    self.isDailyTaskUnlock = funcUnLockCrtl:ValidateUnlock(
                                 proto_csmsg_SystemFunctionID.SystemFunctionID_DailyTask)
    self.checkExpiredTimer = TimerManager:GetTimer(3, (BindCallback(self,
                                                                    self.UpdateDailyAndWeeklyExpired)),
                                                   nil, false, false);
...
TaskController.UpdateDailyAndWeeklyExpired =
    function(self)
        -- function num : 0_2 , upvalues : TaskEnum
        if self.refreshTime == nil then return end
        local expiredTime =
            (self.refreshTime)[(TaskEnum.eTaskPeriodType).DailyTask]
...
TaskController.RecvPeriodTaskDetail = function(self, msg)
    ...
    local win = UIManager:GetWindow(UIWindowTypeID.Task)
    if win == nil or self.showingPageType == nil then return end
...
TaskController.RefreshPeriodData = function(self, msg)
    ...
    for typeId, activeDatas in pairs(ConfigData.active) do
        ...
        for _, stcData in ipairs(activeDatas) do
            ...
            periodData.stcData = stcData -- DECOMPILER ERROR at PC59: Confused about usage of register: R19 in 'UnsetPending'
            ;
            (((self.allTaskData).taskPeriodDatas)[typeId])[stcData.id] =
                periodData
        end
    end
    do
        if self.refreshTime ~= nil then
            if msg.dailyFreshTm == nil or msg.dailyFreshTm <= 0 or
                msg.weeklyFreshTm == nil or msg.weeklyFreshTm <= 0 then
                warn("Daily or Weekly RefreshTime is null,please check proto！")
            end -- DECOMPILER ERROR at PC86: Confused about usage of register: R2 in 'UnsetPending'
            
            (self.refreshTime)[(TaskEnum.eTaskPeriodType).DailyTask] =
                msg.dailyFreshTm -- DECOMPILER ERROR at PC91: Confused about usage of register: R2 in 'UnsetPending'
            ;
            (self.refreshTime)[(TaskEnum.eTaskPeriodType).WeeklyTask] =
                msg.weeklyFreshTm
        end
    end
end
...
TaskController.PeriodTaskRedDot = function(self)
    ...
    if not self.isDailyTaskUnlock then return end
    for actionTypeId, _ in pairs(ConfigData.active) do
        ...
        local taskPeriodNode = taskPageNode:AddChildWithPath(actionTypeId,
                                                             RedDotDynPath.TaskPeriodPath)
...
TaskController.CheckExpiredAndUpdate = function(self, expiredTime)
    ...
    if expiredTime ~= nil and expiredTime > 0 and expiredTime <
...
TaskController.ShowTaskUI = function(self, selectPageTypeId, openCallback)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.Task, function(win)
        ...
        if openCallback ~= nil then openCallback() end
        win:RefreshPages()
        if selectPageTypeId ~= nil then win:SelectPage(selectPageTypeId) end
    end)
end
...
TaskController.GetPageName = function(self, index, subIndex)
    ...
    if subIndex == nil then
        local str = (string.split)(ConfigData:GetTipContent(
                                       (taskNames[index])[1]), "|")
        name = str[1]
        name_en = str[2]
    else
        do
            do
                local str = (string.split)(
                                ConfigData:GetTipContent(
                                    (taskNames[index])[subIndex]), "|")
                name = str[1]
                name_en = str[2]
                return name, name_en
            end
...
TaskController.GetDatas4Task = function(self, typeId)
    ...
    if typeId == (TaskEnum.eTaskType).DailyTask then
        local newTaskDatas = {}
        for id, value in pairs(taskDatas) do newTaskDatas[id] = value end
        ...
        return newTaskDatas,
               self:GetDatas4Peroid((TaskEnum.eTaskPeriodType).DailyTask)
    else
        do
            do
                if typeId == (TaskEnum.eTaskType).WeeklyTask then
                    ...
                    for _, id in ipairs(self.AllWeeklyTaskList) do
                        if newTaskDatas[id] == nil then
                            local stcData = (ConfigData.task)[id]
                            local taskData =
                                (TaskData.CreateTaskData)({}, stcData)
                            taskData.isPicked = true
                            newTaskDatas[id] = taskData
                        end
                    end
                    return newTaskDatas, self:GetDatas4Peroid(
                               (TaskEnum.eTaskPeriodType).WeeklyTask)
                end
                return taskDatas
            end
        end
...
TaskController.IsChangeTaskType = function(self, typeId)
    ...
    if typeId == (TaskEnum.eTaskType).DailyTask or typeId ==
        (TaskEnum.eTaskType).WeeklyTask then
...
TaskController.SendCommitQuestReward = function(self, taskData)
    ...
    do
        if self.showingActiveType ~= nil and self.refreshTime ~= nil then
            ...
            if self:CheckExpiredAndUpdate(expiredTime) then return end
        end
...
TaskController.OnTaskCommit = function(self, stcTask)
    ...
    local win = UIManager:GetWindow(UIWindowTypeID.Task)
    if win == nil or stcTask == nil then return end
    local isSector = (self.allTaskData):IsSectorType(stcTask.type)
    if isSector == true then return end
    if self.showingPageType == stcTask.type then
        win:OnSelectPage(self.showingPageType)
    end
    if stcTask.type == (TaskEnum.eTaskType).MainTask or stcTask.type ==
        (TaskEnum.eTaskType).SideTask then
        ...
        if stcTask.type == (TaskEnum.eTaskType).DailyTask or stcTask.type ==
            (TaskEnum.eTaskType).WeeklyTask then
            (self.network):SendQuestPeriodDetail()
...
TaskController.SendCommitTaskPeriod = function(self, index)
    ...
    do
        ...
        (self.network):SendQuestPeriodRewardPicked(index,
                                                   self.showingActiveType,
                                                   function()
            ...
            UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward,
                ...
                if window == nil then return end
                local activeCfg =
                    ((ConfigData.active)[self.showingActiveType])[index]
