...
-- params : ...
-- function num : 0 , upvalues : _ENV
local ExplorationResetRoomCtrl = class("ExplorationResetRoomCtrl",
                                       ExplorationCtrlBase)
local cs_MessageCommon = CS.MessageCommon
local ExplorationEnum = require("Game.Exploration.ExplorationEnum")
ExplorationResetRoomCtrl.ctor = function(self, epCtrl)
    ...
    self.waitRefersh = false
    self.__onResetRoomUpdate = BindCallback(self, self.__OnResetRoomUpdate)
    MsgCenter:AddListener(eMsgEventId.OnResetRoomUpdate,
                          self.__onResetRoomUpdate)
    self.__onChipListUpdate = BindCallback(self, self.__OnChipListUpdate)
    MsgCenter:AddListener(eMsgEventId.OnEpChipListChange,
                          self.__onChipListUpdate)
    self.__onChipToHeroQuict = BindCallback(self, self.AddChipToHeroClose)
    self.__onChipToHeroConfirm = BindCallback(self, self.ChipToHeroConfirm)
end

ExplorationResetRoomCtrl.OnResetRoomOpen =
    function(self, roomData)
        -- function num : 0_1 , upvalues : _ENV
        if roomData.roomRecordList == nil then return end
        self.currPosition = roomData.position
        self.dynPlayer = ExplorationManager:GetDynPlayer()
        if self.playerChipDatas == nil then
            self.playerChipDatas = (self.dynPlayer):GetChipList()
...
ExplorationResetRoomCtrl.SendInfomationOperation =
    function(self, btnInfo, selectChipItemList)
        -- function num : 0_2 , upvalues : ExplorationEnum, _ENV
        if btnInfo.currBtnId == (ExplorationEnum.eResetRoomType).Duplicate then
            self.uiHeroWindow = UIManager:ShowWindow(
                                    UIWindowTypeID.AddChipToHero)
            if self.uiHeroWindow ~= nil then
                (self.uiHeroWindow):InitAddChipToHero(self,
                                                      (self.dynPlayer).heroList,
                                                      (selectChipItemList[1]).chipData,
                                                      self.__onChipToHeroQuict,
                                                      self.__onChipToHeroConfirm)
            end
        else
            if btnInfo.currBtnId == (ExplorationEnum.eResetRoomType).Mix then
                (self.netWork):CS_EXPLORATION_RECONSTITUTION_Fusion(
                    self.currPosition, selectChipItemList)
            else

                (self.netWork):CS_EXPLORATION_RECONSTITUTION_Reconstitution(
                    self.currPosition, (selectChipItemList[1]).chipData)
            end
        end
        self.waitRefersh = true
    end

ExplorationResetRoomCtrl.SendDuplicateOperation =
    function(self, chipData, heroId)
        -- function num : 0_3
        (self.netWork):CS_EXPLORATION_RECONSTITUTION_Reproduction(
            self.currPosition, chipData, heroId)
    end

ExplorationResetRoomCtrl.SendResetRoomQuit =
    function(self)
        -- function num : 0_4
        (self.netWork):CS_EXPLORATION_RECONSTITUTION_Quit(self.currPosition)
    end

ExplorationResetRoomCtrl.OnSendMsgSuccess =
    function(self, algId)
        -- function num : 0_5 , upvalues : cs_MessageCommon, _ENV
        if algId ~= nil then
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.exploration_Reconstitution_AchieveChip) ..
                    (LanguageUtil.GetLocaleText)(((ConfigData.item)[algId]).name))
        else

            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.exploration_Reconstitution_ChipOperateSuccess))
        end
        if self.uiHeroWindow ~= nil then
            (self.uiHeroWindow):CloseAddChipToHero()
        end
...
ExplorationResetRoomCtrl.CloseResetRoom =
    function(self)
        -- function num : 0_6 , upvalues : _ENV, ExplorationEnum
        if self.uiWindow ~= nil then
            (self.uiWindow):CloseResetRoom(self, self.resetRoomDataList)
        ...
        MsgCenter:Broadcast(eMsgEventId.OnExitRoomComplete,
                            (ExplorationEnum.eExitRoomCompleteType).ResetRoom)
    end

ExplorationResetRoomCtrl.AddChipToHeroClose =
    function(self, uiWindow)
        -- function num : 0_7
        self.uiHeroWindow = uiWindow;
        (self.uiHeroWindow):CloseAddChipToHero()
        self.uiHeroWindow = nil
...
ExplorationResetRoomCtrl.ChipToHeroConfirm =
    function(self, uiWindow)
        -- function num : 0_8 , upvalues : cs_MessageCommon, _ENV
        self.uiHeroWindow = uiWindow
        if uiWindow.currHeroItem ~= nil then
            self:SendDuplicateOperation(uiWindow.currChipData,
                                        (uiWindow.currHeroItem).heroId)
        else

            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.exploration_Reconstitution_SelectOneHero))
        end
    end

ExplorationResetRoomCtrl.__OnResetRoomUpdate =
    function(self, roomData)
        -- function num : 0_9
        if self.waitRefersh and self.currPosition == roomData.position then
            self:OnResetRoomOpen(roomData)
...
ExplorationResetRoomCtrl.__OnChipListUpdate =
...
ExplorationResetRoomCtrl.OnDelete = function(self)
    -- function num : 0_11 , upvalues : _ENV
    MsgCenter:RemoveListener(eMsgEventId.OnResetRoomUpdate,
                             self.__onResetRoomUpdate)
