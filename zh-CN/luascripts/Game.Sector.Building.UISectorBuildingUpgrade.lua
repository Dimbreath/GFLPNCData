...
UISectorBuildingUpgrade.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_Construct, self, self.OnClickConfirmButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).closeConstruct, self, self.OnClickCloseButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).closeUpgrade, self, self.OnClickCloseButton)
  ;
  (UIUtil.AddButtonListener)((self.ui).cancelUpgrade, self, self.OnClickCancelButton)
  self.resNeedItems = {}
...
UISectorBuildingUpgrade.__ForceUpdateProcess = function(self, timestamp)
...
UISectorBuildingUpgrade.UpdateProcessEvent = function(self, id, progress, remainTimeText, waitConfimOver)
  -- function num : 0_3 , upvalues : cs_MessageCommon, _ENV
  if id ~= (self.buildingData).id or not self.active then
    return 
  end
  if waitConfimOver then
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(TipContent.Sector_BuildingConfirmFinish))
  ...
  ;
  ((self.ui).tex_Process):SetIndex(0, tostring((math.floor)(progress * 100)))
  local color = (Color.Lerp)((self.ui).fillLeftColor, (self.ui).fillRightColor, progress)
...
UISectorBuildingUpgrade.RefreshUpgrade = function(self, buildingData, isUnbuilt)
  ...
  if isUnbuilt then
    ...
    if buildingData.state == proto_object_BuildingState.BuildingStateNormal then
      ...
      if buildingData.level < buildingData.maxLevel then
        ((self.ui).tex_BulidLevel):SetIndex(1, tostring(buildingData.level))
        ;
...
UISectorBuildingUpgrade.__UpdateUpgradePage = function(self, buildingData, level)
  -- function num : 0_6 , upvalues : _ENV
  -- DECOMPILER ERROR at PC7: Confused about usage of register: R3 in 'UnsetPending'

  ((self.ui).tex_BulidName).text = (LanguageUtil.GetLocaleText)((buildingData.stcData).name)
  ...
  local totalTime = (math.ceil)(levelCfg.time / (1 + (PlayerDataCenter.playerBonus):GetBuildSpeed() / 1000))
...
UISectorBuildingUpgrade.__UpdateLogic = function(self, isUnbuilt)
  ...
  if logic == eLogicType.ResourceOutput then
    ...
    ;
    ((self.ui).tex_Info):SetIndex(1, (LanguageUtil.GetLocaleText)(itemCfg.name))
    if isUnbuilt then
      ...
      do
        if nextLevelCfg ~= nil and #nextLevelCfg.logic > 0 then
          local nextOutput = (math.floor)((nextLevelCfg.para2)[logicIndex1] * 36)
          ...
          do
            do
              ...
              if logic == eLogicType.CampBuff then
                ...
                if campCfg == nil then
                  error("can\'t find camp, id=" .. tostring(campId))
                  return 
                end
                ;
                ((self.ui).tex_Info):SetIndex(2, (LanguageUtil.GetLocaleText)(campCfg.name), attrName)
                self:__SetTextInfoNum(isUnbuilt, isRatio, levelCfg, nextLevelCfg)
              elseif logic == eLogicType.CareerBuff then
                ...
                ;
                ((self.ui).tex_Info):SetIndex(3, (LanguageUtil.GetLocaleText)(careerCfg.name), attrName)
                self:__SetTextInfoNum(isUnbuilt, isRatio, levelCfg, nextLevelCfg)
              elseif logic == eLogicType.AllHeroBuff then
                local attrId = (levelCfg.para1)[logicIndex1]
                local value = (levelCfg.para2)[logicIndex1]
                local attrName, valueDes = ConfigData:GetAttribute(attrId, value)
                ;
                ((self.ui).tex_Info):SetIndex(4, attrName)
                if isUnbuilt then
                  local originOutputStr = "0"
                  ;
                  ((self.ui).tex_InfoNum):SetIndex(1, originOutputStr, valueDes)
                elseif nextLevelCfg ~= nil and #nextLevelCfg.logic > 0 then
                  local newxtValue = (nextLevelCfg.para2)[logicIndex1]
                  local _, nextValueDes = ConfigData:GetAttribute(attrId, newxtValue)
                  ;
                  ((self.ui).tex_InfoNum):SetIndex(1, valueDes, nextValueDes)
                else
...
UISectorBuildingUpgrade.__SetTextInfoNum = function(self, isUnbuilt, isRatio, levelCfg, nextLevelCfg)
  ...
  if isUnbuilt then
    if hasAtrr2 then
      ((self.ui).tex_InfoNum):SetIndex(2, "0", countStr, "0", countStr2)
    else
  ...
  elseif nextLevelCfg ~= nil and #nextLevelCfg.logic > 0 then
    ...
    if hasAtrr2 then
      ...
      if countStr2 == nil then
        countStr2 = "0"
      end
      ;
      ((self.ui).tex_InfoNum):SetIndex(2, countStr, nextCountStr, countStr2, nextCountStr2)
    else
...
UISectorBuildingUpgrade.__GetResNeedItem = function(self, index)
...
UISectorBuildingUpgrade.OnClickConfirmButton = function(self)
  -- function num : 0_10
...
UISectorBuildingUpgrade.OnClickCancelButton = function(self)
  -- function num : 0_11
...
UISectorBuildingUpgrade.OnClickCloseButton = function(self)
  -- function num : 0_12
