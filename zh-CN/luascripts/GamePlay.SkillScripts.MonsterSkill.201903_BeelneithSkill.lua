...
bs_201903.PlaySkill = function(self, data)
    ...
    LuaSkillCtrl:CallEffect(self.caster, (self.config).effectstart, self)
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1008, 1, 15,
                                           triggerCallBack)
end
...
bs_201903.OnActionCallBack = function(self, target)
    ...
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId196, 1,
                          (self.arglist)[1] + 3)
    local targetgrid = LuaSkillCtrl:GetTargetWithGrid(3, 2)
    self.effectQ = LuaSkillCtrl:CallEffect(targetgrid, (self.config).effectQ,
                                           self)
    LuaSkillCtrl:CallEffect(self.caster, (self.config).effectup, self)
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId, 1)
    local cusEffect = LuaSkillCtrl:CallEffect(self.caster,
                                              (self.config).effectdamge, self)
    local collisionTrigger = BindCallback(self, self.OnCollision)
    local EmissionArrive = BindCallback(self, self.OnEmissionArrive, targetgrid,
                                        target)
...
bs_201903.OnEmissionArrive = function(self, targetgrid, lasttarget,
                                      skillEmission)
    -- function num : 0_5 , upvalues : _ENV
    local target_new = LuaSkillCtrl:CallTargetSelect(self, 42, 10)
    if target_new.Count > 0 then
        ...
        if (target_new[0]).targetRole == lasttarget then
            if target_new.Count > 1 then
                ...
                local targetgrid =
                    LuaSkillCtrl:GetTargetWithGrid(grid.x, grid.y)
                target_real = targetgrid
            end
        end
        do
            ...
            do
                local EmissionArrive = BindCallback(self, self.OnEmissionArrive,
                                                    targetgrid, lasttarget)
                LuaSkillCtrl:CallRestartEmit(self, skillEmission, 10,
                                             target_real, true, false,
                                             EmissionArrive)
                LuaSkillCtrl:CallEffect(targetgrid, (self.config).effectQhit,
                                        self)
                self:OnOver()
                if self.timeover ~= nil then
                    (self.timeover):Stop()
