...
bs_102702.PlaySkill = function(self, data)
    ...
    LuaSkillCtrl:StartTimer(self, 15, function()
        -- function num : 0_2_0 , upvalues : _ENV, self
        LuaSkillCtrl:CallEffectWithArgAndSpeed(self.caster,
                                               (self.config).effectId1, self,
                                               (self.config).speed, true,
                                               self.SkillEventFunc)
    end)
end
...
bs_102702.CallRandomBuff = function(self, role)
    ...
    if (self.arglist)[4] == 1 then
        local randIndex2 = randIndex1 + LuaSkillCtrl:CallRange(1, 2)
        if randIndex2 > 3 then randIndex2 = randIndex2 - 3 end
        local buffId2 = ((self.config).buffIdList)[randIndex2]
        self:SetBuffScale(randIndex2, scale)
        LuaSkillCtrl:CallBuff(self, role, buffId2, 1, (self.arglist)[5])
    end
    do
        LuaSkillCtrl:CallBuffLifeEvent(self, role, buffId1, 1,
                                       (self.arglist)[5], BindCallback(self,
...
bs_102702.OnBuffLifeEvent = function(self, role, scale, lifeType, arg)
    ...
    if lifeType == eBuffLifeEvent.NewAdd then
        ...
        LuaSkillCtrl:StartTimer(self, 3, function()
            -- function num : 0_6_0 , upvalues : role, self, _ENV
            if role:GetBuffTier(((self.config).buffIdList)[1]) > 0 or
                role:GetBuffTier(((self.config).buffIdList)[2]) > 0 or
                role:GetBuffTier(((self.config).buffIdList)[3]) > 0 then
                (role.lsObject):StartLocalScale((Vector3.New)(0.2, 2, 0.2), 0.2)
            end
        end)
        LuaSkillCtrl:StartTimer(self, 6, function()
