  ...
  LuaSkillCtrl:StartTimer(nil, 5, function()
        ...
        if debuffs.Count > 0 then
          ...
          for i = 0, debuffs.Count - 1 do
            if (debuffs[i]).buffType == 2 then
              num = num + 1
              LuaSkillCtrl:DispelBuff(role, (debuffs[i]).buffId, 0)
            end
          end
          if num > 0 then
            ...
            if buffs.Count > 0 then
              ...
              for i = 0, buffs.Count - 1 do
                if (buffs[i]).buffType == 1 then
                  num = num + 1
                  LuaSkillCtrl:DispelBuff(role, (buffs[i]).buffId, 0)
                end
              end
              if num > 0 then
                LuaSkillCtrl:CallEffect(role, (self.config).effectRole, self)
    ...
    if hurtList ~= nil and hurtList.Count > 0 then
      for i = 0, hurtList.Count - 1 do
        ...
        LuaSkillCtrl:HurtResult(skillResult, (self.config).hurt_config, {allBuffTier})
...
bs_103503.PlayUltEffect = function(self)
  ...
  self:AddTrigger(eSkillTriggerType.AfterSelfUltMovieEnd, "bs_103503_2", 1, self.OnMovieEnd)
...
bs_103503.OnMovieEnd = function(self)
  -- function num : 0_6
end
...
bs_103503.PlaySkill = function(self, data)
    ...
    local targetList = nil
    local preList = LuaSkillCtrl:CallTargetSelect(self, 5, 10)
    if preList.Count <= 0 then return end
    local effectTarget = LuaSkillCtrl:GetTargetWithGrid(3, 2)
    local map = LuaSkillCtrl:GetMapBorder()
    if map.x == 6 then
        LuaSkillCtrl:CallEffect(effectTarget, (self.config).startEffect1, self,
                                nil, nil, 1)
    else
        LuaSkillCtrl:CallEffect(effectTarget, (self.config).startEffect2, self,
                                nil, nil, 1)
    end
    local allBuffTier = 0
    LuaSkillCtrl:StartTimer(nil, 5, function()
        ...
        for i = 0, targetList.Count - 1 do
            ...
            if role.belongNum == (self.caster).belongNum then
                ...
                if debuffs.Count > 0 then
                    ...
                    if num > 0 then
                        LuaSkillCtrl:CallEffect(role, (self.config).effectRole,
                                                self)
                        allBuffTier = allBuffTier + (num)
                    end
                end
                    ...
                    do
                        if buffs.Count > 0 then
                            ...
                            for i = 0, buffs.Count - 1 do
                                if (buffs[i]).buffType == 1 then
                                    num = num + 1
                                    LuaSkillCtrl:DispelBuff(role,
                                                            (buffs[i]).buffId, 0)
                                end
                            end
                            if num > 0 then
                                LuaSkillCtrl:CallEffect(role,
                                                        (self.config).effectRole,
                                                        self)
                                allBuffTier = allBuffTier + (num)
                            end
                        end
...
bs_103503.PlayUltEffect = function(self)
    ...
    LuaSkillCtrl:CallFocusTimeLine(self.caster)
    LuaSkillCtrl:CallBuff(self, self.caster, 196, 1, 15)
    self:AddTrigger(eSkillTriggerType.AfterSelfUltMovieEnd, "bs_103503_2", 1,
                    self.OnMovieEnd)
    self:AddTrigger(eSkillTriggerType.OnSelfUltMovieFadeOut, "bs_103503_3", 1,
                    self.OnMovieFadeOut)
end
