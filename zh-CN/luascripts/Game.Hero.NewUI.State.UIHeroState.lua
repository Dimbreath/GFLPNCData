...
UIHeroState.InitHeroState = function(self, heroData, resloader, heroDataList, returnFunc)
  ...
  ;
  (self.athNode):SetShowAthWindowParam(self.bigImgResloader, self.__addAllTouch, self.__removeAllTouch)
...
UIHeroState.OnClickStarUP = function(self)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.HeroStarUp, function(windows)
    ...
    self:RemoveAllTouch()
    windows:InitHeroStarUp(self.heroData, self.resloader, self.__addAllTouch, self.__SwitchHeroState)
...
UIHeroState.OnDelete = function(self)
  -- function num : 0_45 , upvalues : CS_coroutine, _ENV, base
  self:RemoveAllTouch()
  if self.chageCoroutine ~= nil then
    (CS_coroutine.stop)(self.chageCoroutine)
    self.chageCoroutine = nil
...
UIHeroState.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_LevelUp, self, self.OnClickLevelUP);
    (UIUtil.AddButtonListener)((self.ui).btn_StarUp, self, self.OnClickStarUP);
    (UIUtil.AddButtonListener)((self.ui).btn_Friendship, self,
                               self.OnClickFriendship);
    ...
    self.__onItemUpdate = BindCallback(self, self.OnItemUpdate)
    MsgCenter:AddListener(eMsgEventId.UpdateItem, self.__onItemUpdate)
    self.__onFriendshipDataUpdate = BindCallback(self,
                                                 self.OnFriendshipDataUpdate)
    MsgCenter:AddListener(eMsgEventId.OnHeroFriendshipDataChange,
                          self.__onFriendshipDataUpdate)
    self.__onAthDataUpdate = BindCallback(self, self.OnAthDataUpdate)
    MsgCenter:AddListener(eMsgEventId.OnAthDataUpdate, self.__onAthDataUpdate)
    self.tagPool = (UIItemPool.New)(UINHeroTag, (self.ui).obj_tagItem);
    ((self.ui).obj_tagItem):SetActive(false)
    ...
    self:UnlockStarup(isStarUpUnlock)
    local isFriendshipUnlock = funcUnLockCrtl:ValidateUnlock(
...
UIHeroState.InitHeroState = function(self, heroData, resloader, heroDataList,
    ...
    self.heroId = heroData.dataId
    self.heroData = heroData
    self.returnFunc = returnFunc -- DECOMPILER ERROR at PC13: Confused about usage of register: R6 in 'UnsetPending'
    ;
...
UIHeroState.UpdateBlueDot = function(self)
    ...
    local funcUnLockCrtl = ControllerManager:GetController(
                               ControllerTypeId.FunctionUnlock, true)
    local isSkillLevelUpUnlock = funcUnLockCrtl:ValidateUnlock(
...
UIHeroState.UpdateSkill = function(self)
    ...
    local sortTable = self:__SortSkillTab()
    for k, skillData in ipairs(sortTable) do
        if skillData.type ~= eHeroSkillType.LifeSkill and
...
UIHeroState.OnClickFriendship = function(self)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.FriendShip, function(windows)
        ...
        self:RemoveAllTouch()
        windows:InitFriendshipSkillUpgrade(self.heroData, self.resloader,
                                           function()
            -- function num : 0_20_0_0 , upvalues : self, _ENV
            self:AddAllTouch()
            UIManager:ShowWindow(UIWindowTypeID.HeroState)
