...
UINSortKindItem.InitSortKindItem = function(self, kindType, index, onSelectFunc)
    ...
    if self.kindType == (HeroFilterEnum.eKindType).Rare then
        ...
        if self.kindType == (HeroFilterEnum.eKindType).Camp then
            local campCfg = (ConfigData.camp)[index] -- DECOMPILER ERROR at PC34: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).tex_KindName).text =
                (LanguageUtil.GetLocaleText)(campCfg.name)
        else
            do
                local carrerCfg = (ConfigData.career)[index] -- DECOMPILER ERROR at PC45: Confused about usage of register: R5 in 'UnsetPending'
                ;
                ((self.ui).tex_KindName).text =
                    (LanguageUtil.GetLocaleText)(carrerCfg.name)
            end
        end
    end
end
...
UINSortKindItem.SetSelectUIActive = function(self)
    ...
    if not self.select or not Color.black then local selColor = Color.white end -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
    
    ((self.ui).tex_KindName).color = selColor -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
    ;
