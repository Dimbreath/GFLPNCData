...
UIAddChipHeroDetail.InitHeroDetailItem =
    function(self, heroData, resLoader, onClickAction)
        -- function num : 0_1
        self.heroData = heroData
        self.heroId = heroData.dataId;
        (self.heroItem):InitHeroHeroData(heroData.heroData, resLoader,
                                         onClickAction)
        self:UpdateProperty()
    end
...
UIAddChipHeroDetail.UpdateProperty = function(self)
    ...
    for k, v in ipairs(eHeroShowAttrList) do
        ...
        if item == nil then
            ...
            go:SetActive(true)
            item:Init(go) -- DECOMPILER ERROR at PC22: Confused about usage of register: R8 in 'UnsetPending'
            ;
