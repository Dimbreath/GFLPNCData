...
UIAthDecomposeFliter.InitFliter = function(self, fliterList)
    ...
    if fliterList == nil then return end
...
UIAthDecomposeFliter.SetConfirmCallback =
...
UIAthDecomposeFliter.ResetBtn = function(self)
    -- function num : 0_4
    if self.resetCallback ~= nil then (self.resetCallback)() end
    self:Hide()
end

UIAthDecomposeFliter.ConfirmBtn = function(self)
    -- function num : 0_5
    if self.confirmCallback ~= nil then (self.confirmCallback)() end
end

UIAthDecomposeFliter.CleanSelect = function(self)
    -- function num : 0_6 , upvalues : _ENV
    for index, value in ipairs(self.itemList) do value:CleanSelect() end
end

UIAthDecomposeFliter.OrangeAddOrRemoveSwitch =
    function(self, isAdd)
        -- function num : 0_7 , upvalues : eFliterKindColorName, _ENV, eFliterKindTypeOptionName, eFliterKindType
        if isAdd then
            ((self.allKindItem)[eFliterKindColorName[eItemQualityType.Orange] ..
                eFliterKindTypeOptionName[eFliterKindType.Quality]]):AddOrangeEvent(
                function(isSelect)
            ...
            (self.mainAttrItem):Show();
            ((self.allKindItem)[eFliterKindColorName[eItemQualityType.Orange] ..
                eFliterKindTypeOptionName[eFliterKindType.Quality]]):RemoveOrangeEvent()
        end
    end
...
UIAthDecomposeFliter.AutoDecoSelect = function(self, autoDeco)
    ...
    if lastData.blue then
        ((self.allKindItem)[eFliterKindColorName[eItemQualityType.Blue] ..
            eFliterKindTypeOptionName[eFliterKindType.Quality]]):onClick()
    end
    if lastData.purple then
        ((self.allKindItem)[eFliterKindColorName[eItemQualityType.Purple] ..
            eFliterKindTypeOptionName[eFliterKindType.Quality]]):onClick()
    end
    if (table.count)(lastData.orange) > 0 then
        ...
        for key, _ in pairs(lastData.orange) do
            ((self.allKindItem)[eFliterKindTypeOptionName[eFliterKindType.MainAttr] ..
                (LanguageUtil.GetLocaleText)(((ConfigData.attribute)[key]).name) ..
                key]):onClick()
        end
    else
        do
