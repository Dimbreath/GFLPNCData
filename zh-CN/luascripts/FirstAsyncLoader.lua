...
FirstAsyncLoader.LoadFunc = function()
    ...
    for _, tdata in pairs(configNameList) do
        ...
        ConfigData[tdata[1]] = require(ConfigDataLoader.LoadConfigHead ..
                                           tdata[2])
        loadedConfigCount = loadedConfigCount + 1
        if frameLoadConfigCount <= loadedConfigCount then
            loadedConfigCount = 0;
            (coroutine.yield)()
            progress = (math.min)(1, progress + OneConfigProgress *
                                      frameLoadConfigCount)
            cs_DownloadWindow:UpdateProgress(progress)
        end
    end
    (ConfigDataLoader.AfterLoadConfigComplete)() -- DECOMPILER ERROR at PC54: Confused about usage of register: R8 in 'UnsetPending'
    ;
    (package.loaded)["Game.ConfigData.ConfigDataLoader"] = nil;
    (coroutine.yield)()
    progress = (math.min)(1, progress + 0.01)
    ...
    NetworkManager:InitNetwork()
    PersistentManager:InitManager();
    (ControllerManager:GetController(ControllerTypeId.Login, true)):FirstGameStart(
        function()
            -- function num : 0_0_0 , upvalues : cs_DownloadWindow, _ENV
            cs_DownloadWindow:Dispose()
            cs_DownloadWindow = nil -- DECOMPILER ERROR at PC7: Confused about usage of register: R0 in 'UnsetPending'
            ;
            (package.loaded).FirstAsyncLoader = nil
        end)
end
...
FirstAsyncLoader.StartFirstLoad = function()
    -- function num : 0_1 , upvalues : _ENV, xlua_util, FirstAsyncLoader
    ((CS.GlobalRegister).StartCoroutine)(
        (xlua_util.cs_generator)(FirstAsyncLoader.LoadFunc))
end
