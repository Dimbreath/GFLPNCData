...
UIGetHero.ShowTimeAndTimeZone = function(self)
  ...
  local time = (((CS.System).DateTime).Now):ToUniversalTime()
...
UIGetHero.OnInit = function(self)
    ...
    self.TagList = {};
    (table.insert)(self.TagList, (self.ui).tagItem);
    (UIUtil.AddButtonListener)((self.ui).btn_Close, self, self.OnClickClose) -- DECOMPILER ERROR at PC30: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.ui).mainEffect = ((self.ui).Eff_QualityList)[1]
    local codeActionBinding = ((self.ui).ui_timeline):FindComponent(
                                  eUnityComponentID.CodeActionBinding)
    codeActionBinding:RegisterAction("ZoomIn",
                                     BindCallback(self, self._OnCamZoomIn))
    codeActionBinding:RegisterAction("ZoomOut",
                                     BindCallback(self, self._OnCamZoomOut));
    ((self.ui).heroConvertItem):SetActive(false)
    self.cardConvertItemPool = (UIItemPool.New)(UINHeroCardConvert,
                                                (self.ui).heroConvertItem)
end

UIGetHero.InitGetHeroList = function(self, heroIdList, useHeroData,
                                     withGetHeroSound, newHeroIndexDic,
                                     closeFunc)
    -- function num : 0_2
    if #heroIdList < 1 then return end
    self.withGetHeroSound = withGetHeroSound
    self.haveNext = true
    self.heroIdList = heroIdList
    self.useHeroData = useHeroData
...
UIGetHero.ShowNext = function(self)
    -- function num : 0_3
    ((self.ui).ui_timeline):Stop()
    self.isNew = self.newHeroIndexDic ~= nil and
                     (self.newHeroIndexDic)[self.showIndex] or false
    self:_InitGetHero((self.heroIdList)[self.showIndex], self.isNew)
    self.showIndex = self.showIndex + 1
    if #self.heroIdList < self.showIndex then self.haveNext = false end
    self.isSkipAllTween = false
end
    ...
    if isNew then
        ...
        do
            ...
            do
                ...
                ((self.ui).isNew):SetActive(isNew) -- DECOMPILER ERROR at PC149: Confused about usage of register: R5 in 'UnsetPending'
                ;
                ((self.ui).tex_HeroID_Big).text = HeroIdTo4Hex(heroId) -- DECOMPILER ERROR at PC155: Confused about usage of register: R5 in 'UnsetPending'
                ;
                ((self.ui).tex_HeroID_Small).text = HeroIdTo4Hex(heroId)
                local campCfg = (ConfigData.camp)[heroCfg.camp]
                local campIcon = (LanguageUtil.GetLocaleText)(campCfg.icon);
                (self.resloader):LoadABAssetAsync(
                    PathConsts:GetCampPicPath(campIcon), function(texture)
                        -- function num : 0_4_0 , upvalues : _ENV, self
                        if IsNull(self.transform) then
                            return
                ...
                if resCfg == nil then
                    error("resource model Cfg is null,id:" ..
                              tostring(heroCfg.src_id))
                end
                self.bigImgResloader = (cs_ResLoader.Create)();
                (self.bigImgResloader):LoadABAssetAsync(
                    PathConsts:GetCharacterBigImgPrefabPath(resCfg.res_Name),
                    function(prefab)
                        -- function num : 0_4_1 , upvalues : _ENV, self
                        DestroyUnityObject(self.bigImgGameObject)
                ...
                ((self.ui).img_Carrer):SetIndex(heroCfg.career - 1) -- DECOMPILER ERROR at PC215: Confused about usage of register: R8 in 'UnsetPending'
                ;
                ((self.ui).tex_Name).text =
                    (LanguageUtil.GetLocaleText)(heroCfg.name)
                local enName = (LanguageUtil.GetLocaleText)(heroCfg.name_en) -- DECOMPILER ERROR at PC222: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ((self.ui).tex_NameEN_Big).text = enName -- DECOMPILER ERROR at PC225: Confused about usage of register: R9 in 'UnsetPending'
                ;
                ((self.ui).tex_NameEN_small).text = enName
                self:ShowTags(heroCfg.tag)
                local heroStar, heroRare = nil, nil
                ...
                do
                    self:ShowStars(heroStar)
                    local qualityColor = HeroRareColor[heroRare] -- DECOMPILER ERROR at PC259: Confused about usage of register: R12 in 'UnsetPending'
                    ;
                    ((self.ui).img_Quality).color = qualityColor
                    self:SetPlayEffectColor(qualityColor);
                    ((self.ui).dialogue):SetActive(false)
                    self:ShowTimeAndTimeZone() -- DECOMPILER ERROR at PC274: Confused about usage of register: R12 in 'UnsetPending'
                    ;
                    ((self.ui).tex_Signet).text = PlayerDataCenter.playerName
                end
            end
...
UIGetHero.SetPlayEffectColor = function(self, qualityColor)
    -- function num : 0_5 , upvalues : _ENV, cs_ParticleSystem
    if (self.ui).mainEffect == nil then return end
    if ((self.ui).mainEffect).isPlaying then
        ((self.ui).mainEffect):Stop();
        (((self.ui).mainEffect).gameObject):SetActive(false)
    end
    ...
    for _, effect in ipairs((self.ui).Eff_QualityList) do
        local minMaxGradient = cs_ParticleSystem:MinMaxGradient()
        minMaxGradient.color = qualityColor -- DECOMPILER ERROR at PC40: Confused about usage of register: R8 in 'UnsetPending'
        ;
        (effect.main).startColor = minMaxGradient
    end
end
...
UIGetHero.ShowTimeAndTimeZone = function(self)
    -- function num : 0_6 , upvalues : _ENV
    local time = (((CS.System).DateTime).Now):ToUniversalTime() -- DECOMPILER ERROR at PC24: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_Time).text = tostring(time.Month) .. "/" ..
                                    tostring(time.Day) .. " " ..
                                    tostring(time.Hour) .. ":" ..
...
UIGetHero.PlayAllTween = function(self)
    -- function num : 0_7 , upvalues : m_moviePlayer, cs_MovieManager, _ENV
    self:_StopAllAudio() -- DECOMPILER ERROR at PC4: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).ui_timeline).time = 0;
    ((self.ui).ui_timeline):Play()
    if m_moviePlayer ~= nil then
...
UIGetHero.CheckIsTweening = function(self)
    ...
    if not IsNull(self.allTweens) then
        for i = 0, (self.allTweens).Length - 1 do
            ...
            if loopNum ~= -1 then
                local isPlaying = tween:IsPlaying()
                if isPlaying then return true end
            end
        end
    end
    do return false end
...
UIGetHero.GotoTweenAnimation = function(self, rate, keepPlay)
    -- function num : 0_9 , upvalues : _ENV
    self:_StopAllAudio() -- DECOMPILER ERROR at PC4: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).ui_timeline).time = 3.5
    if not IsNull(self.allTweens) then
        for i = 0, (self.allTweens).Length - 1 do
...
UIGetHero.ShowTags = function(self, tagList)
    ...
    for index, tagId in ipairs(tagList) do
        if (self.TagList)[index] ~= nil then
            ...
            textItemInfo:SetIndex(tagId)
            local imgItem = ((self.TagList)[index]):GetComponentsInChildren(
                                typeof(((CS.UnityEngine).UI).Image))
            local spriteName = eHeroShowTag[tagId] -- DECOMPILER ERROR at PC50: Confused about usage of register: R10 in 'UnsetPending'
            ;
            (imgItem[1]).sprite = CRH:GetSprite(spriteName)
        else
            do
                local item = ((self.ui).tagItem):Instantiate()
                local textItemInfo = item:GetComponentInChildren(
                                         typeof(CS.UiTextItemInfo))
                local imgItem = item:GetComponentsInChildren(
                                    typeof(((CS.UnityEngine).UI).Image));
                (table.insert)(self.TagList, item)
                textItemInfo:SetIndex(tagId)
                do
                    local spriteName = eHeroShowTag[tagId] -- DECOMPILER ERROR at PC85: Confused about usage of register: R11 in 'UnsetPending'
                    ;
                    (imgItem[1]).sprite = CRH:GetSprite(spriteName)
                    -- DECOMPILER ERROR at PC86: LeaveBlock: unexpected jumping out DO_STMT
...
UIGetHero._OnCamZoomOut = function(self)
    ...
    if self.withGetHeroSound then
        if self.starCount == 1 then
            ...
            if self.starCount == 2 then
                self.auBack_ShowHero = AudioManager:PlayAudioById(1021,
                                                                  function()
                    -- function num : 0_13_1 , upvalues : self
                    self.auBack_ShowHero = nil
                end)
                ...
                if self.starCount == 3 then
                    self.auBack_ShowHero =
                        AudioManager:PlayAudioById(1022, function()
                            -- function num : 0_13_2 , upvalues : self
                            self.auBack_ShowHero = nil
                        end)
                end
...
UIGetHero.OnClickClose = function(self)
    -- function num : 0_15 , upvalues : m_moviePlayer
    if m_moviePlayer ~= nil and self.isNew then return end
    if not self.isNew and self:CheckIsTweening() then
        if not self.isSkipAllTween then
            self:GotoTweenAnimation(0.4, true)
            self.isSkipAllTween = true
    ...
    if not self:CheckIsTweening() then self.isNew = false end
    do return end
    if self.haveNext then
        self:ShowNext()
    else
