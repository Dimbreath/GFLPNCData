...
UINBaseItem.InitBaseItem = function(self, itemCfg, clickEvent)
    -- function num : 0_1 , upvalues : _ENV
    self.itemCfg = itemCfg
    self.clickEvent = clickEvent -- DECOMPILER ERROR at PC8: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).Img_ItemPic).sprite = CRH:GetSprite(itemCfg.icon) -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).Img_Quality).sprite = CRH:GetSprite(
                                         ItemQualitySprite[itemCfg.quality],
                                         CommonAtlasType.BaseItemQuailty) -- DECOMPILER ERROR at PC25: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).img_QualityColor).color = ItemQualityColor[itemCfg.quality];
    ((self.ui).obj_IsFrag):SetActive(false);
    ((self.ui).obj_isHreo):SetActive(false);
    ...
    if itemCfg.action_type == eItemActionType.HeroCardFrag then
        ...
        if smallIcon ~= "" and smallIcon ~= nil then
            ((self.ui).obj_IsFrag):SetActive(true);
            (((self.ui).img_isSmallIcon).gameObject):SetActive(true) -- DECOMPILER ERROR at PC80: Confused about usage of register: R4 in 'UnsetPending'
            ;
            ((self.ui).img_isSmallIcon).sprite = CRH:GetSprite(smallIcon)
        end
    end
...
UINBaseItem.OnClickItemRoot = function(self)
    ...
    if self.clickEvent ~= nil then
        ...
        UIManager:ShowWindowAsync(UIWindowTypeID.GlobalItemDetail, function(win)
            -- function num : 0_2_0 , upvalues : self
            if win ~= nil then win:InitCommonItemDetail(self.itemCfg) end
        end)
    end
end
