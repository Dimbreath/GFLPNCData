...
bs_1041.PlaySkill = function(self, data)
    ...
    while self.attackTime ~= 0 do
        for i = 0, targetList.Count - 1 do
            if (targetList[i]).targetRole ~= nil and
                ((targetList[i]).targetRole).intensity ~= 0 then
                LuaSkillCtrl:StartTimer(self, (6 - self.attackTime) * 3,
                                        (BindCallback(self, self.RealPlaySkill,
    ...
    do if self.attackTime > 0 then end end
end
