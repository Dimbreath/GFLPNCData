...
bs_1035.InitSkill = function(self, isMidwaySkill)
    ...
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.SetHurt, "bs_1035_2", 1, self.OnSetHurt)
    self:AddTrigger(eSkillTriggerType.BeforeBattleEnd, "bs_1035_3", 1,
                    self.BeforeEndBattle) -- DECOMPILER ERROR at PC22: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)[(self.config).shieldKey] = 0
end
...
bs_1035.PlaySkill = function(self, data)
    ...
    self:CallCasterWait(15 + (self.arglist)[4] + 10)
    local attackTrigger = BindCallback(self, self.OnAttackTrigger, data)
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1002, 1, 9,
                                           attackTrigger)
end
...
bs_1035.OnAttackTrigger = function(self, data)
    ...
    do
        ...
        local shieldValue = LuaSkillCtrl:CallFormulaNumberWithSkill(
                                (self.config).shieldFormula, self.caster, nil,
                                self)
        if shieldValue > 0 then
        ...
        LuaSkillCtrl:StartTimer(nil, (self.arglist)[4], function()
            ...
            if ((self.caster).recordTable)[(self.config).shieldKey] > 0 then
                ...
                LuaSkillCtrl:CallEffect(self.caster, 10068, self, nil, nil, nil,
                                        nil)
                if self.hudunEffect ~= nil then
                    (self.hudunEffect):Die()
                    self.hudunEffect = nil
...
bs_1035.audioLoop = function(self)
    -- function num : 0_4
    if ((self.caster).recordTable)[(self.config).shieldKey] > 0 then
        self.loop = ((self.caster).auSource):PlayAudioById(
