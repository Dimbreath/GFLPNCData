...
bs_1035.PlaySkill = function(self, data)
    ...
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1002, 1, 11,
                                           attackTrigger)
end
...
bs_1035.OnAttackTrigger = function(self, data)
    ...
    LuaSkillCtrl:CallEffect(self.caster, (self.config).effectCFId, self) -- DECOMPILER ERROR at PC22: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["1035_Realhurt"] = 0
    local targetList = LuaSkillCtrl:CallTargetSelect(self, 9, 10)
    for i = 0, targetList.Count - 1 do
        LuaSkillCtrl:CallBuff(self, (targetList[i]).targetRole,
                              (self.config).buffId2, 1)
    end
    LuaSkillCtrl:CallRoleAction(self.caster, 1007) -- DECOMPILER ERROR at PC57: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["1035_HuDun"] =
        (self.caster).maxHp * (self.arglist)[1] // 1000
    if ((self.caster).recordTable)["1035_HuDun"] > 0 then
        self.hudunEffect = LuaSkillCtrl:CallEffect(self.caster,
                                                   (self.config).effectstartId,
                                                   self, nil, nil, nil, nil)
        self:AddTrigger(eSkillTriggerType.SetHurt, "bs_1035_2", 1,
                        self.OnSetHurt)
    end
...
bs_1035.OnSetHurt = function(self, context)
    ...
    if context.target == self.caster and
        ((self.caster).recordTable)["1035_HuDun"] > 0 then
        ...
        LuaSkillCtrl:CallEffect(self.caster, roleeffect, self, nil, nil, nil,
                                nil) -- DECOMPILER ERROR at PC41: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["1035_HuDun"] =
            ((self.caster).recordTable)["1035_HuDun"] - context.hurt -- DECOMPILER ERROR at PC49: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["1035_fanshang"] =
            context.hurt * (self.arglist)[2] // 1000
        ...
        do
            do
                ...
                end -- DECOMPILER ERROR at PC75: Confused about usage of register: R4 in 'UnsetPending'
                
                ((self.caster).recordTable)["1035_Realhurt"] = context.hurt * 1
                context.hurt = 0
                LuaSkillCtrl:CallFloatText(self.caster, 7)
                if ((self.caster).recordTable)["1035_HuDun"] <=
                    ((self.caster).recordTable)["1035_Realhurt"] then
                    if self.loop ~= nil then
                        AudioManager:StopAudioByBack(self.loop)
                    end
                    LuaSkillCtrl:CallEffect(self.caster, 10068, self, nil, nil,
                                            nil, nil)
                    ...
                    local targetList =
                        LuaSkillCtrl:CallTargetSelect(self, 9, 10)
                    for i = 0, targetList.Count - 1 do
                        LuaSkillCtrl:DispelBuff((targetList[i]).targetRole,
                                                (self.config).buffId2, 1)
                    end
                end
            end
