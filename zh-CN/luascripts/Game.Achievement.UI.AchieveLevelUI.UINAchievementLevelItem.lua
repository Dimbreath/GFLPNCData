...
UINAchievementLevelItem.InitLevelItem = function(self, eLevelState,
                                                 getRewardCallback)
    -- function num : 0_1
    self.eLevelState = eLevelState
    self.getRewardCallback = getRewardCallback
...
UINAchievementLevelItem.RefreshLevelItem =
    function(self, levelData)
        ...
        ((self.ui).tex_Level):SetIndex(0, tostring(levelData.level));
        ((self.ui).obj_Effective):SetActive(
            levelData.level <= (PlayerDataCenter.playerLevel).level)
        local des = ""
        local logic_list, para1_list, para2_List, para3_list =
            (PlayerDataCenter.achivLevelData):GetLevelLogics(levelData.level)
        for index, logic in pairs(logic_list) do
            local para1 = para1_list[index]
            local para2 = para2_List[index]
            local para3 = para3_list[index]
            if (string.IsNullOrEmpty)(des) then
                ...
                des = des .. "\n" ..
                          (CommonLogicUtil.GetDesString)(logic, para1, para2,
                                                         para3)
            end
        end -- DECOMPILER ERROR at PC121: Confused about usage of register: R7 in 'UnsetPending'
...
UINAchievementLevelItem.OnClickGetReward =
    function(self)
        -- function num : 0_3
        if self.getRewardCallback ~= nil then
            (self.getRewardCallback)((self.levelData).level)
