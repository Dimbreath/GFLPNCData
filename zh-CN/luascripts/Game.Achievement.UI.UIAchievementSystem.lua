...
local UINAchievementTog = require("Game.Achievement.UI.UINAchievementTog")
local UINAchievementItem = require("Game.Achievement.UI.UINAchievementItem")
local UINAchievementLevelNode = require(
                                    "Game.Achievement.UI.AchieveLevelUI.UINAchievementLevelNode")
local defaultTogType = 1
UIAchievementSystem.OnInit = function(self)
    -- function num : 0_0 , upvalues : cs_ResLoader, _ENV, UINAchievementLevelNode, UINAchievementTog
    self.isFromHome = nil
    ...
    (self.achievementLevelNode):Init((self.ui).achievementLevelNode);
    (UIUtil.CreateTopBtnGroup)((self.ui).obj_topButtonGroup, self, self.Delete)
    self.typeTogPool = (UIItemPool.New)(UINAchievementTog,
                                        (self.ui).obj_achivTypeTogHolder);
    ((self.ui).obj_achivTypeTogHolder):SetActive(false) -- DECOMPILER ERROR at PC48: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopList_achieveListNode).onInstantiateItem =
        BindCallback(self, self.m_OnNewItem) -- DECOMPILER ERROR at PC55: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopList_achieveListNode).onChangeItem =
...
UIAchievementSystem.InitAchievement = function(self, openTagType, isFromHome,
                                               JumpCallabck)
    -- function num : 0_1 , upvalues : _ENV
    self.isFromHome = isFromHome
    self.JumpCallabck = JumpCallabck
    ...
    MsgCenter:Broadcast(eMsgEventId.CleanNotice,
                        ((require("Game.Home.HomeEnum")).eNoticeType).Achievement)
end
...
UIAchievementSystem.m_HandaySelectSubpage =
    function(self, achievementType)
        -- function num : 0_3 , upvalues : defaultTogType, _ENV
        if not achievementType then achievementType = defaultTogType end
        for _, toggle in pairs((self.typeTogPool).listItem) do
            if toggle.type == achievementType then
                toggle:SetSelected(true)
                break
...
UIAchievementSystem.InitRedDotEvent = function(self)
    ...
    for _, item in ipairs((self.typeTogPool).listItem) do
        local ok, node = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main,
                                                        ...
                                                        RedDotStaticTypeId.AchivLevelPage,
                                                        item.task_type)
        item:SetRedDot(not ok or node:GetRedDotCount() > 0)
    end
    self.__onAchievTogRedDotEvent = function(node)
...
UIAchievementSystem.ShowSubPage = function(self, achievementType)
    ...
    for _, data in pairs(achivDatas) do
        ...
        (table.insert)(self.achieveDataList,
                       {taskData = data, state = state, stcData = data.stcData})
    end
    for _, pickedAchivData in pairs(pickedAchivs) do
        (table.insert)(self.achieveDataList, {
            taskData = nil,
    ...
    end) -- DECOMPILER ERROR at PC79: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).loopList_achieveListNode).totalCount = #self.achieveDataList;
    ((self.ui).loopList_achieveListNode):RefillCells()
end
...
UIAchievementSystem.m_OnNewItem = function(self, go)
    ...
    local achieveItem = (UINAchievementItem.New)()
    achieveItem:Init(go)
    achieveItem:InitAchieveItem(self.resloader, self._GetReward,
                                self._JumpToTarget) -- DECOMPILER ERROR at PC11: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.achieveItemDic)[go] = achieveItem
end
...
UIAchievementSystem.m_GetItemGoByIndex =
    function(self, index)
        -- function num : 0_9
        local go = ((self.ui).loopList_achieveListNode):GetCellByIndex(index)
        if go ~= nil then return (self.achieveItemDic)[go] end
        return nil
    end
    ...
    if achieveData.state == (TaskEnum.eTaskState).Completed then
        ...
        if achieveData.state == (TaskEnum.eTaskState).Picked then
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(TipContent.achievement_taskCompleted))
        else
...
UIAchievementSystem.JumpToTarget = function(self, taskCfg)
    ...
    (UIUtil.PopFromBackStack)()
    self:Delete()
    if self.JumpCallabck ~= nil then (self.JumpCallabck)() end
    if jumpId ~= nil and jumpId ~= 0 then
        JumpManager:Jump(jumpId, nil, jumpArgs)
    end
end
...
UIAchievementSystem.OnTaskUpdate = function(self, taskData)
    ...
    if (taskData.stcData).type ~=
        ((ConfigData.achievement)[self.currAchievementType]).task_type then
        return
    end
    local achieveItem, index = self:m_GetItemByTaskId((taskData.stcData).id)
    if index ~= nil then
        ...
        achieveData.taskData = taskData
        achieveData.state = state
        if achieveItem ~= nil then achieveItem:RefreshUI() end
    end
end
...
UIAchievementSystem.OnTaskDelete = function(self, id)
    ...
    local _, index = self:m_GetItemByTaskId(id)
    if index ~= nil then
        (table.remove)(self.achieveDataList, index) -- DECOMPILER ERROR at PC14: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).loopList_achieveListNode).totalCount = #self.achieveDataList;
        ((self.ui).loopList_achieveListNode):RefillCells()
    end
...
UIAchievementSystem.OpenAchievementInfo =
    function(self)
        -- function num : 0_17
    end
