...
UINSkadaTeamItem.OnClickRoleHead = function(self, roleData)
  -- function num : 0_4
...
local SkadaEnum = require("Game.BattleResult.SkadaSystem.SkadaEnum")
local UINRoleHeadItem = require("Game.CommonUI.Hero.UINRoleHeadItem")
local UINSkadaValueItem = require(
                              "Game.BattleResult.SkadaSystem.UINSkadaValueItem")
UINSkadaTeamItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui)
    self.__OnClickRoleHead = BindCallback(self, self.OnClickRoleHead);
...
UINSkadaTeamItem.InitSkadaItem = function(self, resLoader, clickCallback)
    ...
    for id = 1, SkadaEnum.MaxSkadaTypeNum do
        ...
        local valueItem = (UINSkadaValueItem.New)()
        valueItem:Init(go)
        valueItem:InitSkadaValueItem(self:__GetValueItemColor(id)) -- DECOMPILER ERROR at PC39: Confused about usage of register: R9 in 'UnsetPending'
        ;
        (self.valueItems)[id] = valueItem
    end
end

UINSkadaTeamItem.RefreshSkadaItem = function(self, roleIndex, roleSkada,
                                             skadaType, belongPlayer)
    -- function num : 0_2 , upvalues : _ENV
    self.roleIndex = roleIndex
    self.roleSkada = roleSkada
    local dynRole = (roleSkada.role).character;
    (self.roleHeadItem):InitRoleHeadItem(dynRole, self.resLoader,
                                         self.__OnClickRoleHead, belongPlayer) -- DECOMPILER ERROR at PC15: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).tex_HeroName).text = dynRole:GetName()
    for k, v in ipairs(self.valueItems) do
        local detail = (roleSkada.detail)[k]
        local alpha = 1
        if k ~= skadaType then alpha = (self.ui).normal_alpha end
        v:SetSkadaValue(detail.proportion, detail.value, alpha)
        if skadaType ~= k then (v.transform):SetAsLastSibling() end
    end
end
