...
BattleDungeonController.ctor = function(self, dungeonData, formationData, ATHRewardInfo)
  ...
  if dungeonData.dungeonId == (GuideManager.firstBattleGuideCtrl).guideDungeonId then
    ...
    do
      ...
      self:InitData(dungeonData, formationData, ATHRewardInfo)
    end
  end
end

BattleDungeonController.InitData = function(self, dungeonData, formationData, ATHRewardInfo)
...
local Messenger = require("Framework.Common.Messenger")
local DungeonDynPlayer = require("Game.BattleDungeon.Data.DungeonDynPlayer")
local BattleDungeonSceneCtrl = require(
                                   "Game.BattleDungeon.Ctrl.BattleDungeonSceneCtrl")
...
BattleDungeonController.ctor = function(self, dungeonData, formationData,
                                        ATHRewardInfo)
    -- function num : 0_0 , upvalues : Messenger, _ENV, BattleDungeonBattleCtrl, BattleDungeonObjectCtrl, BattleDungeonSceneCtrl
    self.ctrls = {}
    self.__dungeonLogicMessage = (Messenger.New)()
    ...
    if dungeonData.dungeonId ==
        (GuideManager.firstBattleGuideCtrl).guideDungeonId then
        self.battleGuideType =
            (GuideManager.firstBattleGuideCtrl).battleGuideType
        local BattleDungeonGuideBatteCtrl =
            require("Game.BattleDungeon.Guide.BattleDungeonGuideBatteCtrl")
        self.battleCtrl = (BattleDungeonGuideBatteCtrl.New)(self,
                                                            self.battleGuideType)
        local BattleDungeonGuideObjectCtrl =
            require("Game.BattleDungeon.Guide.BattleDungeonGuideObjectCtrl")
        self.objectCtrl = (BattleDungeonGuideObjectCtrl.New)(self,
                                                             self.battleGuideType)
    else
        do
            self.battleCtrl = (BattleDungeonBattleCtrl.New)(self)
            self.objectCtrl = (BattleDungeonObjectCtrl.New)(self)
            self.sceneCtrl = (BattleDungeonSceneCtrl.New)(self)
            self.battleNetwork = NetworkManager:GetNetwork(
...
BattleDungeonController.InitData = function(self, dungeonData, formationData,
                                            ATHRewardInfo)
    -- function num : 0_1 , upvalues : _ENV, DungeonDynPlayer
    self.dungeonData = dungeonData
    self.dungeonId = dungeonData.dungeonId
    ...
    self.dynPlayer = (DungeonDynPlayer.CreateDungeonPlayer)(dungeonData.battle,
                                                            formationData,
                                                            self.dungeonCfg)
    if self.dynPlayer ~= nil and dungeonData.player ~= nil then
        (self.dynPlayer):InitDynPlayerAttr(dungeonData.player)
...
BattleDungeonController.ExitBattleDungeon =
    function(self, battleWin)
        -- function num : 0_3 , upvalues : _ENV
        ...
        if battleWin then
            local winEvent = BattleDungeonManager:GetBattleWinEvent()
            if winEvent ~= nil then winEvent() end
        end
    end

BattleDungeonController.StartRunNextLogic =
    function(self)
        -- function num : 0_4 , upvalues : _ENV
        if self:__TryRunTopLogic() then return end
        self.__runLogicTimer = (TimerManager:GetTimer(1, self.__TryRunTopLogic,
                                                      self, false, true)):Start()
    end

BattleDungeonController.__TryRunTopLogic =
    function(self)
        -- function num : 0_5 , upvalues : _ENV
        if #self.__cacheDungeonLogic > 0 then
            if self.__runLogicTimer ~= nil then
            ...
            local logicData = (table.remove)(self.__cacheDungeonLogic, 1);
            (self.__dungeonLogicMessage):Broadcast(logicData.logicType,
                                                   logicData.logicContent)
            return true
        end
        do return false end
    end

BattleDungeonController.AddDungeonLogic =
    function(self, logicType, msgData)
        -- function num : 0_6 , upvalues : _ENV
        local logicCacheData = {logicType = logicType, logicContent = msgData};
        (table.insert)(self.__cacheDungeonLogic, logicCacheData)
    end

BattleDungeonController.RegisterDungeonLogic =
    function(self, logicId, action)
        -- function num : 0_7
        (self.__dungeonLogicMessage):AddListener(logicId, action)
    end

BattleDungeonController.UnRegisterDungeonLogic =
    function(self, logicId, action)
        -- function num : 0_8
        (self.__dungeonLogicMessage):RemoveListener(logicId, action)
    end
