...
HeroData.ctor = function(self, data)
  ...
  if heroCfg == nil then
    error("hero cfg is null,id:" .. tostirng(self.dataId))
    return 
  end
...
HeroData.GetUpgradeLevelProcess = function(self, oldLevel, oldExp, getExp)
  ...
  for i = oldLevel, (ConfigData.game_config).heroMaxLevel - 1 do
    ...
    if getExp < curLevelTotalExp then
      ...
      if getExp == curLevelTotalExp then
        ...
        ;
        (table.insert)(tolist, curLevelTotalExp)
        if i + 1 == (ConfigData.game_config).heroMaxLevel then
          (table.insert)(fromlist, 0)
          ;
          (table.insert)(tolist, 0)
          break
...
HeroData.AddTestExp = function(self, exp)
  ...
  for i = self.level, (ConfigData.game_config).heroMaxLevel - 1 do
    local levelCfg = (ConfigData.hero_level)[i]
    if levelCfg == nil then
  ...
  if testLevel == (ConfigData.game_config).heroMaxLevel then
    nextExp = 0
    overflowExp = testExp
  else
    nextExp = testExp
...
HeroData.GetSkillFightingPower = function(self)
  ...
  for k,skill in pairs(self.skillDic) do
    if skill.type == eHeroSkillType.BattleSkill and skill:GetIsUnlock() and (skill.battleSkillCfg).combatFormula ~= nil then
