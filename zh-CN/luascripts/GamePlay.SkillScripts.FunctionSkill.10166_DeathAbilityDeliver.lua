...
bs_10166.OnCasterDie = function(self)
    ...
    local targetList = LuaSkillCtrl:CallTargetSelect(self, 34, 10)
    if targetList.Count > 0 then
        local atkTier = LuaSkillCtrl:CallFormulaNumberWithSkill(
                            (self.config).atkFormula, self.caster, targetRole,
                            self)
        local defTier = LuaSkillCtrl:CallFormulaNumberWithSkill(
                            (self.config).skillIntensityFormula, self.caster,
                            targetRole, self)
        if atkTier < 1 or defTier < 1 then return end
        local target = (targetList[0]).targetRole
        target:AddRoleProperty(eHeroAttr.pow, atkTier, eHeroAttrType.Extra)
        target:AddRoleProperty(eHeroAttr.skill_intensity, defTier,
                               eHeroAttrType.Extra)
    end
    do
