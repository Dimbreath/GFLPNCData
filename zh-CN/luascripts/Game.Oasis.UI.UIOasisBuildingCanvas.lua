...
UIOasisBuildingCanvas.OnInit = function(self)
    ...
    (base.OnInit)(self)
    local nameRoot = self:FindComponent("NameRoot", eUnityComponentID.Transform) -- DECOMPILER ERROR at PC10: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.ui).nameRoot = {};
    (UIUtil.LuaUIBindingTable)(nameRoot, (self.ui).nameRoot) -- DECOMPILER ERROR at PC20: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).nameRoot).gameObject = nameRoot.gameObject;
    (((self.ui).nameRoot).namePrefab):SetActive(false)
    self.nameItems = {}
    self.nameItemPool = {}
    local progressRoot = self:FindComponent("ProgressRoot",
                                            eUnityComponentID.Transform) -- DECOMPILER ERROR at PC38: Confused about usage of register: R3 in 'UnsetPending'
    ;
...
UIOasisBuildingCanvas.CreateNameItem = function(self, id, UIbulidData, position)
    ...
    if item == nil then
        if #self.nameItemPool > 0 then
            ...
            item = (NameItem.New)()
            item:Init(go)
            item:Split(((self.ui).nameRoot).bgRoot,
                       ((self.ui).nameRoot).textRoot)
        end
    end
    do
        item:InitCanvasNameItem(id, UIbulidData, position) -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
        ;
        (self.nameItems)[id] = item
    end
end

UIOasisBuildingCanvas.UpdateNameItemLevel =
    function(self, id, level)
        -- function num : 0_2
        local item = (self.nameItems)[id]
        if item ~= nil then item:UpdateBuildingLevel(level) end
    end

UIOasisBuildingCanvas.UpdateNamePos = function(self, id, position)
    -- function num : 0_3
    local item = (self.nameItems)[id]
    if item ~= nil then item:UpdateNamePos(position) end
end
...
UIOasisBuildingCanvas.RecycleNameItem = function(self, id)
    ...
    local item = (self.nameItems)[id]
    if item ~= nil then
        item:OnRecycle() -- DECOMPILER ERROR at PC7: Confused about usage of register: R3 in 'UnsetPending'
        ;
...
UIOasisBuildingCanvas.SetNameRootActive =
...
UIOasisBuildingCanvas.__GetProcessItem =
    function(self)
        ...
        if #self.progressItemPool > 0 then
            ...
            item:Init(go)
            item:Split(((self.ui).progressRoot).bgRoot,
                       ((self.ui).progressRoot).fillRoot,
                       ((self.ui).progressRoot).textRoot)
        end
        do return item end
    end

UIOasisBuildingCanvas.UpdateProcess = function(self, id, position, progress,
                                               text)
    -- function num : 0_7
    local item = (self.progressItem)[id]
    if item == nil then
        item = self:__GetProcessItem()
        item:InitCanvasProcessItem(id, position, progress, text) -- DECOMPILER ERROR at PC14: Confused about usage of register: R6 in 'UnsetPending'
        ;
...
UIOasisBuildingCanvas.UpdateProcessPos =
    function(self, id, position)
        -- function num : 0_8
        local item = (self.progressItem)[id]
        if item ~= nil then item:UpdateProcessPos(position) end
    end

UIOasisBuildingCanvas.RecycleProcessItem =
    function(self, id)
...
UIOasisBuildingCanvas.OnDelete = function(self)
    ...
    for k, v in pairs(self.nameItems) do v:OnDelete() end
    self.nameItems = {}
    for k, v in pairs(self.progressItem) do v:OnDelete() end
    self.progressItem = {}
    for k, v in ipairs(self.progressItemPool) do v:OnDelete() end
    self.progressItemPool = {};
    (base.OnDelete)(self)
end
