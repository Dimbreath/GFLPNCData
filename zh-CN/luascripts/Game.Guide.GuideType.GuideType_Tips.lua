...
GuideType_Tips.StartTipsGuide = function(self)
    ...
    if (self.tipsGuideCfg).type == (GuideEnum.TipsGuideType).Normal then
        if (self.tipsGuideCfg).target_type == 0 then
            ...
            if (self.tipsGuideCfg).target_type == 1 then
                ...
                if (self.tipsGuideCfg).target_type == 2 then
                    self:NextWaitScene(self.tipsGuideCfg, BindCallback(self,
                    ...
                    if (self.tipsGuideCfg).target_type == 3 then
                        ...
                        error("tipsGuideCfg target_type error,id:" ..
                                  tostring((self.tipsGuideCfg).id))
                        self:EndGuide(false)
                    end
                end
            end
        ...
        if (self.tipsGuideCfg).type == (GuideEnum.TipsGuideType).Code then
            ...
            error("tipsGuideCfg type error,id:" ..
                      tostring((self.tipsGuideCfg).id))
            self:EndGuide(false)
        end
    end
    ...
    if (self.tipsGuideCfg).type == (GuideEnum.TipsGuideType).Normal then
        ...
        if (self.tipsGuideCfg).type == (GuideEnum.TipsGuideType).Code then
            ...
            if guideCode == nil then
                warn((string.format)("tips guide load code error id[%d],",
                                     (self.tipsGuideCfg).id))
                self:EndGuide(false)
                return
            end
...
GuideType_Tips.__PlayNormalUITipsGuide =
    function(self)
        -- function num : 0_6 , upvalues : _ENV
        if (self.tipsGuideCfg).target_type == 1 then
            local targetWindow = UIManager:GetWindow(
                                     (self.tipsGuideCfg).target_parent)
            if targetWindow == nil then
                self:EndGuide(false)
                return
            end
            local targetTrans = (targetWindow.transform):Find(
                                    (self.tipsGuideCfg).target_name)
            if IsNull(targetTrans) then
                warn((string.format)(
                         "guide error,%s Id[%d],target Transform is null",
                         (self.__class_type).__cname, (self.tipsGuideCfg).id))
                self:EndGuide(false)
                return
            end
            self.curTargetTransform = targetTrans
        end
        do self:__PlayTipsGuide() end
    end
...
GuideType_Tips.__PlayTipsGuide = function(self)
    ...
    if IsNull(self.curTargetTransform) then
        warn((string.format)(
                 "guide error,%s Id[%d],play target Transform is null",
                 (self.__class_type).__cname, (self.tipsGuideCfg).id))
        self:EndGuide(false)
        return
    end
    if not ((self.curTargetTransform).gameObject).activeInHierarchy then
        warn((string.format)("guide error,%s Id[%d],target is Hide",
                             (self.__class_type).__cname, (self.tipsGuideCfg).id))
        self:EndGuide(false)
        return
    end
    ...
    if not isRectTrasform then
        if (self.tipsGuideCfg).target_type == 2 then
            local bindObj =
                (CS_GameObject.Find)((self.tipsGuideCfg).target_name)
            if IsNull(bindObj) then
                warn((string.format)(
                         "guide error,%s Id[%d],bind transform is null",
                         (self.__class_type).__cname, (self.tipsGuideCfg).id))
                self:EndGuide(false)
                return
            end
                ...
                if (self.tipsGuideCfg).target_type == 3 then
                    ...
                    do
                        ...
                        if isRectTrasform then
                            (GuideManager.resloader):LoadABAssetAsync(
                                PathConsts:GetUIPrefabPath(
                                    GuideUtil.TipsGuidePrefabName),
...
GuideType_Tips.ShowUITipsGuide = function(self, guidePrefab)
    ...
    if self.isTipsGuideEnd then return end
    if IsNull(self.curTargetTransform) then
        self:EndGuide(false)
        return
    end
    (GuideUtil.ShowTipsGuide)(self.curTargetTransform, guidePrefab,
                              (self.tipsGuideCfg).show_dir)
    self:EndGuide(true)
end
...
GuideType_Tips.TryClearTipsGuide = function(id)
    ...
    if tipsGuideCfg.type == (GuideEnum.TipsGuideType).Normal then
        if tipsGuideCfg.target_type ~= 0 or tipsGuideCfg.target_type == 1 then
            ...
            if targetWindow ~= nil then
                curTargetTransform = (targetWindow.transform):Find(
                                         tipsGuideCfg.target_name)
            end
        else
            do
                do
                    ...
                    if tipsGuideCfg.target_type == 2 and
                        ((string.IsNullOrEmpty)(tipsGuideCfg.target_parent) or
                            CS_GSceneManager_Ins.curSceneName ==
                            tipsGuideCfg.target_parent) then
                        local targetObj =
                            (CS_GameObject.Find)(tipsGuideCfg.target_name)
                    ...
                    do
                        if tipsGuideCfg.type == (GuideEnum.TipsGuideType).Code then
                            if not (string.IsNullOrEmpty)(
                                tipsGuideCfg.custom_code_clear) then
                                local guideCode =
                                    load(tipsGuideCfg.custom_code_clear)
                                if guideCode ~= nil then
                            ...
                            do
                                ...
                                if not IsNull(curTargetTransform) then
                                    local tipsGuideNodeName =
                                        (GuideUtil.GetTipsGuideNodeName)(
                                            curTargetTransform.name)
                                    local tipsGuideNode =
                                        curTargetTransform:Find(
                                            tipsGuideNodeName)
                                    if not IsNull(tipsGuideNode) then
                                        DestroyUnityObject(
                                            tipsGuideNode.gameObject)
                                    end
                                end
                            end
                        end
