...
UISectorLevelDetail.InitSectorLevelDetail = function(self, sectorId, sectorStageId)
  ...
  ;
  (self.levelDetailNode):InitLevelDetailNode(self.stageCfg)
end
...
UISectorLevelDetail.InitSectorLevelAvgDetail = function(self, sectorId, avgCfg, playAvgCompleteFunc)
  ...
  self:GetBackgroundTexture(sectorId)
  ;
  (self.levelDetailNode):InitAvgDetail(avgCfg, playAvgCompleteFunc)
end
...
UISectorLevelDetail.InitInfinityLevelDetailNode = function(self, sectorId, levelData)
  ...
  self:GetBackgroundTexture(sectorId)
  ;
  (self.levelDetailNode):InitInfinityLevelDetailNode(levelData)
end
...
UISectorLevelDetail.OnInit = function(self)
    ...
    (UIUtil.CreateTopBtnGroup)((self.ui).topButtonGroup, self,
                               self.OnClickSectorLevelDetailBackBtn, true);
    (UIUtil.AddButtonListener)((self.ui).btn_Close, self,
                               self.OnClickSectorLevelDetailBackBtn)
    self.resloader = ((CS.ResLoader).Create)()
    self.levelDetailNode = (UINLevelDetail.New)();
    (self.levelDetailNode):Init((self.ui).detailNode);
    (self.levelDetailNode):InitLevelDtail(self.resloader)
...
UISectorLevelDetail.InitSectorLevelDetail =
    function(self, sectorId, sectorStageId)
        -- function num : 0_1 , upvalues : _ENV
        if self.isPushBack2Stack then (UIUtil.PopFromBackStack)() end
        (UIUtil.Push2BackStack)(self, self.OnClickSectorLevelDetailBackBtn)
        self.isPushBack2Stack = true
        self.stageId = sectorStageId
        local stageCfg = (ConfigData.sector_stage)[sectorStageId]
        if stageCfg == nil then
            error("Can\'t find sector stageCfg, stageId = " ..
                      tostring(sectorStageId))
            return
        end
        self.stageCfg = stageCfg
        self:GetBackgroundTexture(sectorId);
...
UISectorLevelDetail.InitSectorLevelAvgDetail =
    function(self, sectorId, avgCfg, playAvgCompleteFunc)
        -- function num : 0_2 , upvalues : _ENV
        if self.isPushBack2Stack then (UIUtil.PopFromBackStack)() end
        (UIUtil.Push2BackStack)(self, self.OnClickSectorLevelDetailBackBtn)
        self.isPushBack2Stack = true
...
UISectorLevelDetail.InitInfinityLevelDetailNode =
    function(self, sectorId, levelData)
        -- function num : 0_3 , upvalues : _ENV
        if self.isPushBack2Stack then (UIUtil.PopFromBackStack)() end
        (UIUtil.Push2BackStack)(self, self.OnClickSectorLevelDetailBackBtn)
        self.isPushBack2Stack = true
...
UISectorLevelDetail.GetLevelDetailWidthAndDuration =
    function(self)
        -- function num : 0_4
        return (self.levelDetailNode):GetNLevelDetailWidthAndDuration()
    end

UISectorLevelDetail.SetLevelDetaiHideStartEvent =
    function(self, hideStartEvent)
        -- function num : 0_5
        self.hideStartEvent = hideStartEvent
    end

UISectorLevelDetail.SetLevelDetaiHideEndEvent =
    function(self, hideEndEvent)
        -- function num : 0_6
        self.hideEndEvent = hideEndEvent
    end

UISectorLevelDetail.OnClickSectorLevelDetailBackBtn =
    function(self)
        -- function num : 0_7
        self.isPushBack2Stack = false
        if self.levelDetailNode ~= nil then
        ...
        if self.hideStartEvent ~= nil then (self.hideStartEvent)() end
    end
...
UISectorLevelDetail.OnHide = function(self)
    -- function num : 0_9
    (self.levelDetailNode):OnHide()
    if self.hideEndEvent ~= nil then (self.hideEndEvent)() end
end

UISectorLevelDetail.GetBackgroundTexture =
    function(self, sectorId)
        ...
        if self.sectorId ~= sectorId then
            ...
            local sectorCfg = (ConfigData.sector)[sectorId]
            if sectorCfg ~= nil then
                local path = PathConsts:GetSectorBackgroundPath(
                                 sectorCfg.pic_small)
                -- DECOMPILER ERROR at PC25: Confused about usage of register: R4 in 'UnsetPending'

                if not (string.IsNullOrEmpty)(path) then
                    ((self.ui).img_LevelPic).texture =
                        (self.resloader):LoadABAsset(path)
                end
            end
        end
    end
