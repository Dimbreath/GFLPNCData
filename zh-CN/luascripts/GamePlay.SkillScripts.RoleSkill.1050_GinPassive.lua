...
bs_1050.InitSkill = function(self, isMidwaySkill)
    ...
    ;
    ((self.caster).recordTable).BJ = true -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["arglist[1]"] = (self.arglist)[1]
    self:AddTrigger(eSkillTriggerType.AfterHeal, "bs_1050_1", 1,
                    self.OnAfterHeal)
    self:AddTrigger(eSkillTriggerType.AfterAddBuff, "bs_1050_1", 1,
                    self.OnAfterAddBuff)
    self:AddTrigger(eSkillTriggerType.BuffDie, "bs_1050_1", 1, self.OnBuffDie)
end
...
bs_1050.OnAfterHeal = function(self, sender, target, skill, heal, isStealHeal)
    -- function num : 0_2 , upvalues : _ENV
    if skill.dataId == 14 and sender == self.caster and (self.arglist)[5] > 0 and
        not isStealHeal then
