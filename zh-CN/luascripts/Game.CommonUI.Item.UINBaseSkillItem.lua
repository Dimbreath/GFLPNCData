...
UINBaseSkillItem.__SetUnlockUI = function(self, active)
    -- function num : 0_6 , upvalues : _ENV
    ((self.ui).lock):SetActive(not active);
    ((self.ui).unlock):SetActive(active) -- DECOMPILER ERROR at PC20: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_icon).sprite = CRH:GetSprite((self.skillData):GetIcon(),
                                                CommonAtlasType.SkillIcon)
    local skillTag = (self.skillData):GetSkillTag();
    ((self.ui).img_type):SetIndex(skillTag);
    ((self.ui).img_typeLine):SetIndex(skillTag) -- DECOMPILER ERROR at PC42: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (((self.ui).img_type).image).color =
        eBattleSkillTypeColor[(self.skillData):GetBattleSkillTypeColor()]
end
