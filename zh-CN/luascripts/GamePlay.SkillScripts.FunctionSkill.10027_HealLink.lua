...
bs_10027.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHeal, "bs_10027_5", 1,
                    self.OnAfterHeal)
end
...
bs_10027.OnAfterHeal = function(self, sender, target, skill, heal)
    ...
    if sender == self.caster and target ~= self.caster then
        ...
        if realHeal > 0 then
            self:PlayChipEffect()
            LuaSkillCtrl:CallEffectWithArg(self.caster, (self.config).effectId,
                                           self, true, self.SkillBack, realHeal)
        end
    end
end
