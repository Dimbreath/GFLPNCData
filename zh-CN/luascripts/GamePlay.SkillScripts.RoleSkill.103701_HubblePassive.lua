...
bs_1054.config = {effectX_numId1 = 10320, effectX_numId2 = 10321, effectX_numId3 = 10322, effectX_numId4 = 10323, effectX_numId5 = 10324, effectLoop = 10331, buffId1 = 169, 
Aoe = {effect_shape = 3, aoe_select_code = 4, aoe_range = 1}
, 
HurtConfig = {hit_formula = 0, def_formula = 0, basehurt_formula = 10063, minhurt_formula = 9994, crit_formula = 0, crithur_ratio = 0, correct_formula = 9989, lifesteal_formula = 1001, spell_lifesteal_formula = 1002, returndamage_formula = 1000}
}
...
bs_1054.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddSelfTrigger(eSkillTriggerType.AfterPlaySkill, "bs_1054_12", 1, self.OnAfterPlaySkill)
...
bs_1054.OnAfterPlaySkill = function(self, skill, role)
  ...
  if role == self.caster and skill.isCommonAttack then
    ...
    if (self.arglist)[1] <= ((self.caster).recordTable).Passive and ((self.caster).recordTable).X_num < 5 then
      ...
      local X_effect = (self.config)["effectX_numId" .. tostring(X_number)]
      ...
      if self.Passiveloop == nil and (self.arglist)[5] > 0 then
        ...
        LuaSkillCtrl:CallEffect(self.caster, (self.config).effectLoop, self, nil, nil, nil, false)
        self.Passiveloop = LuaSkillCtrl:StartTimer(nil, (self.arglist)[3], CallBack, self, -1)
      end
    end
  end
end
