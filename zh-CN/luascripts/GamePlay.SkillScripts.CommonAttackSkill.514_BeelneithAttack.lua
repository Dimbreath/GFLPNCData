...
bs_514.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_514_2", 2, self.OnAfterHurt)
end
...
bs_514.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
  -- function num : 0_5 , upvalues : _ENV
  if sender == self.caster and target:GetBuffTier((self.config).buffIdSY) > 0 and skill.dataId == 514 then
    local arg6 = ((self.caster).recordTable)["514_arg6"]
...
bs_514.RealPlaySkill = function(self, target, data)
    -- function num : 0_2 , upvalues : _ENV, base
    if ((self.caster).recordTable)["514_skilltip"] == true then return end
    if ((self.caster).recordTable).SYRecordRoles ~= nil and
        (((self.caster).recordTable).SYRecordRoles):Count() > 0 then
        local role = (((self.caster).recordTable).SYRecordRoles):Peek()
        do
            for i = 0, 10 do
                if role ~= nil and role:GetBuffTier((self.config).buffIdSY) == 0 then
        ...
        do
            if role ~= nil or role ~= nil then
                if LuaSkillCtrl:GetRoleGridsDistance(self.caster, role) > 1 then
                    ...
                    if grid == nil then
                        local targetlist =
                            LuaSkillCtrl:CallTargetSelect(self, 33, 10)
                        if targetlist.Count > 0 then
                            for i = 0, targetlist.Count - 1 do
                                if ((targetlist[i]).targetRole):GetBuffTier(
                                    (self.config).buffIdSY) >= 1 then
                                    if LuaSkillCtrl:GetRoleGridsDistance(
                                        self.caster, role) > 1 then
                                        grid =
                                            LuaSkillCtrl:FindEmptyGridAroundRole(
                                                role)
                                        if grid ~= nil then
                                            role = (targetlist[i]).targetRole
                                            break
                    ...
                    do
                        do
                            ...
                            do
                                do
                                    local SYTrigger =
                                        BindCallback(self, self.OnSYTrigger,
                                                     role, data, arg6)
                                    self:CallCasterWait(36);
                                    (self.caster):LookAtTarget(role)
                                    LuaSkillCtrl:CallRoleActionWithTrigger(self,
                                                                           self.caster,
                                                                           1021,
                                                                           1,
                                                                           11,
                                    ...
                                    LuaSkillCtrl:CallEffect(role, 10404, self)
                                    do return end
                                    if ((self.caster).recordTable)["514_skill"] ==
                                        true then
                                        local Roll =
                                            ((self.caster).recordTable)["514_Roll"]
                                        if Roll == nil then
                                            Roll = 0
                                        end
                                        if LuaSkillCtrl:CallRange(1, 1000) <=
                                            Roll then
                                            self.attackTime = 3
                                            local targetList =
                                                LuaSkillCtrl:CallTargetSelect(
                                                    self, 9, 0)
                                            if targetList.Count >= 0 then
                                                LuaSkillCtrl:CallRoleAction(
                                                    self.caster, 1004)
                                                self:CallCasterWait(21)
                                                LuaSkillCtrl:StartTimer(self, 6,
                                                                        function()
                                                    -- function num : 0_2_0 , upvalues : self, targetList, _ENV, target
                                                    while self.attackTime ~= 0 do
                                                        for i = 0, targetList.Count -
                                                            1 do
                                                            if (targetList[i]).targetRole ~=
                                                                nil and
                                                                LuaSkillCtrl:IsRoleAdjacent(
                                                                    (targetList[i]).targetRole,
                                                                    target) then
                                                                LuaSkillCtrl:StartTimer(
                                                                    self, (3 -
                                                                        self.attackTime) *
                                                                        3,
                                                                    BindCallback(
...
bs_514.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit,
                              isRealDmg)
    -- function num : 0_5 , upvalues : _ENV
    if sender == self.caster and target:GetBuffTier((self.config).buffIdSY) > 0 and
        skill.dataId == 514 then
        local arg6 = ((self.caster).recordTable)["514_arg6"]
        local healback = BindCallback(self, self.Onhealback, target, arg6, hurt)
        LuaSkillCtrl:StartTimer(self, 14, healback, self)
    end
