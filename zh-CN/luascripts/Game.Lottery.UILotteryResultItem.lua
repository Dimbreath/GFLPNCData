...
UILotteryResultItem.SetStopAutoFlipCards = function(self, self, fun)
  -- function num : 0_3 , upvalues : StopAutoFlipCards, _ENV
  StopAutoFlipCards = Bind(self, fun)
end

UILotteryResultItem.InitLotteryResultItem = function(self, itemId, count, resloader)
  ...
  ;
  ((self.ui).Obj_ConvertToFragment):SetActive(false)
  ...
  if itemCfg.action_type == eItemActionType.HeroCard then
    ...
    local heroResCfg = (ConfigData.resource_model)[(self.heroCfg).src_id]
    local rankCfg = (ConfigData.hero_rank)[(self.heroCfg).rank]
    if rankCfg == nil then
      error("Can\'t find rankCfg,id = " .. tostring((self.heroCfg).rank))
      return 
    end
    ...
    ;
    ((self.ui).tex_HeroName).text = (LanguageUtil.GetLocaleText)((self.heroCfg).name)
    ;
    ((self.ui).tex_HeroLevel):SetIndex(0, tostring(1))
    -- DECOMPILER ERROR at PC116: Confused about usage of register: R8 in 'UnsetPending'

    ;
    ((self.ui).img_HeroColor).color = HeroRareColor[(self.heroRankCfg).rare]
    -- DECOMPILER ERROR at PC130: Confused about usage of register: R8 in 'UnsetPending'

    ;
    ((self.ui).img_HeroCareer).sprite = CRH:GetSprite(((ConfigData.career)[(self.heroCfg).career]).icon, CommonAtlasType.CareerCamp)
    ...
    do
      ...
      if heroData ~= nil then
        ...
        ;
        ((self.ui).tex_ConvertToFragment):SetIndex(0, tostring(fragCount))
      end
      do
    ...
    ;
    ((self.ui).OBJ_rawImageHeroPic):SetActive(true)
    -- DECOMPILER ERROR at PC12: Confused about usage of register: R1 in 'UnsetPending'

    ;
    ((self.ui).rawImage_HeroPic).texture = texture
  end
)
          end
        else
          do
            ...
            ;
            ((self.ui).OBJ_rawImageHeroPic):SetActive(false)
...
UILotteryResultItem.FlipSelf = function(self)
  ...
  if not self.isFliped then
    self.isFliped = true
    self.sequence = (((((cs_DoTween.Sequence)()):Append((((self.transform):DOScale((Vector3.New)(0, 1, 1), 0.3)):SetEase((((CS.DG).Tweening).Ease).InBack)):OnComplete(function()
    -- function num : 0_5_0 , upvalues : self, repeatNewHeroTable, StopAutoFlipCards, _ENV
    if self.isHero then
      ((self.ui).GameObject_hero):SetActive(true)
      if not self.isFragment or not repeatNewHeroTable[self.heroId] then
        StopAutoFlipCards()
...
))):Append((((self.transform):DOScale((Vector3.New)(1, 1, 1), 0.2)):SetEase((((CS.DG).Tweening).Ease).OutQuart)):OnComplete(function()
...
))):Append((self.transform):DOScale((Vector3.New)(0.9, 1, 1), 0.05))):Append((self.transform):DOScale((Vector3.New)(1, 1, 1), 0.1))
    MinFlipNumFun()
  end
end
