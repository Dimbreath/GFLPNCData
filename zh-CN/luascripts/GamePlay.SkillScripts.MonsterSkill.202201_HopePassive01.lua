...
bs_202201.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_202201_3", 1,
                    self.OnAfterHurt) -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).hurt = 0
    self.summonerEffect = {}
    self.timenum = {}
    self.summnerList = {}
    self.startClean = false -- DECOMPILER ERROR at PC23: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).num = 0
end

bs_202201.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                 isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    -- DECOMPILER ERROR at PC17: Confused about usage of register: R8 in 'UnsetPending'

    if target == self.caster and sender ~= self.caster and
        ((self.caster).recordTable).num < 5 then
        ((self.caster).recordTable).hurt =
            ((self.caster).recordTable).hurt + hurt
        -- DECOMPILER ERROR at PC30: Confused about usage of register: R8 in 'UnsetPending'

        if target.maxHp * (self.arglist)[1] // 1000 <=
            ((self.caster).recordTable).hurt then
            ((self.caster).recordTable).hurt = 0
            local grid = nil
            local targetlist = LuaSkillCtrl:CallTargetSelect(self, 42, 10)
            if targetlist.Count > 0 then
                for i = 0, targetlist.Count - 1 do
                    grid = LuaSkillCtrl:FindEmptyGridAroundRole(
            ...
            do
                if grid ~= nil or grid ~= nil then
                    LuaSkillCtrl:CallBreakAllSkill(self.caster)
                    local attackTrigger =
                        BindCallback(self, self.OnAttackTrigger, grid, data)
                    self:CallCasterWait(30)
...
bs_202201.OnAttackTrigger = function(self, grid, data)
    ...
    local target = LuaSkillCtrl:GetTargetWithGrid(grid.x, grid.y)
    LuaSkillCtrl:CallEffect(target, (self.config).effectId2, self)
    local summoner = LuaSkillCtrl:CreateSummoner(self, (self.config).monsterId,
                                                 grid.x, grid.y)
    summoner:SetAttr(eHeroAttr.maxHp,
                     (self.caster).maxHp * (self.arglist)[2] // 1000)
    summoner:SetAttr(eHeroAttr.pow, (self.caster).pow)
    summoner:SetAttr(eHeroAttr.speed, (self.caster).speed)
    summoner:SetAttr(eHeroAttr.def, (self.caster).def)
    summoner:SetAttr(eHeroAttr.skill_intensity, (self.caster).skill_intensity)
    ...
    if summonerEntity ~= nil then
        (self.summonerEffect)[summonerEntity] =
            LuaSkillCtrl:CallEffect(summonerEntity, (self.config).effectIdline,
                                    self, nil, nil, nil, false)
        local cb = BindCallback(self, self.CheckSummoner, summonerEntity) -- DECOMPILER ERROR at PC94: Confused about usage of register: R7 in 'UnsetPending'
        ;
        (self.timenum)[summonerEntity] =
            LuaSkillCtrl:StartTimer(nil, 30, cb, nil, -1, 0) -- DECOMPILER ERROR at PC101: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.caster).recordTable).num = ((self.caster).recordTable).num + 1
    end
end
...
bs_202201.CheckSummoner = function(self, summonerEntity)
    -- function num : 0_4 , upvalues : _ENV
    if self.startClean then return end
    if summonerEntity.hp > 0 then
        local cusEffect = LuaSkillCtrl:CallEffect(self.caster,
        ...
        do
            if self.summonerEffect ~= nil then
                if (self.summonerEffect)[summonerEntity] ~= nil then
                    ((self.summonerEffect)[summonerEntity]):Die() -- DECOMPILER ERROR at PC49: Confused about usage of register: R2 in 'UnsetPending'
                    ;
                    (self.summonerEffect)[summonerEntity] = nil
                end
                if (self.timenum)[summonerEntity] ~= nil then
                    ((self.timenum)[summonerEntity]):Stop() -- DECOMPILER ERROR at PC59: Confused about usage of register: R2 in 'UnsetPending'
                    ;
                    (self.timenum)[summonerEntity] = nil
                end
            end
            (table.removebyvalue)(self.summnerList, summonerEntity) -- DECOMPILER ERROR at PC71: Confused about usage of register: R2 in 'UnsetPending'
            ;
            ((self.caster).recordTable).num =
                ((self.caster).recordTable).num - 1
        end
    end
end
...
bs_202201.OnCasterDie = function(self)
    ...
    if self.summnerList ~= nil then
        for i = 1, #self.summnerList do
            local summonerEntity = (self.summnerList)[i]
            if (self.summonerEffect)[summonerEntity] ~= nil then
                ((self.summonerEffect)[summonerEntity]):Die() -- DECOMPILER ERROR at PC23: Confused about usage of register: R6 in 'UnsetPending'
                ;
                (self.summonerEffect)[summonerEntity] = nil
            end
            if (self.timenum)[summonerEntity] ~= nil then
                ((self.timenum)[summonerEntity]):Stop() -- DECOMPILER ERROR at PC33: Confused about usage of register: R6 in 'UnsetPending'
                ;
                (self.timenum)[summonerEntity] = nil
            end
        end
