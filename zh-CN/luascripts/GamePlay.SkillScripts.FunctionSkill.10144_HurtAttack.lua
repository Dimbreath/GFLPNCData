...
bs_10144.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10144_3", 1,
                    self.OnAfterHurt)
    self:AddTrigger(eSkillTriggerType.BeforePlaySkill, "bs_10144_1", 1,
                    self.OnBeforePlaySkill) -- DECOMPILER ERROR at PC20: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["10144_atkNum"] = 0 -- DECOMPILER ERROR at PC23: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["10144_atkFlag"] = false
end

bs_10144.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if target == self.caster and not isMiss and LuaSkillCtrl:CallRange(1, 1000) <
        (self.arglist)[1] * 2 and self:IsReadyToTake() then
        self:PlayChipEffect() -- DECOMPILER ERROR at PC23: Confused about usage of register: R8 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["10144_atkFlag"] = true
        self:OnSkillTake()
    end
    if target ~= self.caster and target.belongNum == (self.caster).belongNum and
        not isMiss and LuaSkillCtrl:CallRange(1, 1000) < (self.arglist)[1] and
        self:IsReadyToTake() then
        self:PlayChipEffect() -- DECOMPILER ERROR at PC53: Confused about usage of register: R8 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["10144_atkFlag"] = true
        self:OnSkillTake()
    end
    if sender == self.caster and skill.isCommonAttack then
        local tier = (self.caster):GetBuffTier((self.config).buffId) -- DECOMPILER ERROR at PC73: Confused about usage of register: R9 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["10144_atkNum"] =
            ((self.caster).recordTable)["10144_atkNum"] - 1
        if tier > 0 and ((self.caster).recordTable)["10144_atkNum"] <= 0 then
            LuaSkillCtrl:DispelBuff(self.caster, (self.config).buffId, 0)
            if self.effect ~= nil then
                (self.effect):Die()
...
bs_10144.OnBeforePlaySkill = function(self, role, context)
    ...
    if (context.skill).maker == self.caster and (context.skill).isCommonAttack and
        ((self.caster).recordTable)["10144_atkFlag"] then
        ((self.caster).recordTable)["10144_atkFlag"] = false
