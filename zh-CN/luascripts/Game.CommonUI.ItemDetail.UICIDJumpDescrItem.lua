...
UICIDJumpDescrItem.InitCIDJumpDescrItem =
    function(self, jumpData)
        -- function num : 0_1 , upvalues : _ENV
        self.id = jumpData.jumpId -- DECOMPILER ERROR at PC11: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).tex_DropInfo).text = tostring(
                                            (LanguageUtil.GetLocaleText)(
                                                (jumpData.cfg).content))
        local funcUnLockCrtl = ControllerManager:GetController(
                                   ControllerTypeId.FunctionUnlock, true)
        local isUnlock = funcUnLockCrtl:ValidateUnlock(
                             (jumpData.cfg).sys_open_id);
...
UICIDJumpDescrItem.OnBtnGotoClick = function(self)
    -- function num : 0_3
    if self.jumpCallback ~= nil then (self.jumpCallback)(self.id) end
end
