...
bs_10082.OnSetHurt = function(self, context)
    -- function num : 0_2 , upvalues : _ENV
    if self:IsReadyToTake() and context.target == self.caster and
        ((self.caster).recordTable).SkillTime > 0 and not context.isTriggerSet then
        local damage = (math.max)(context.hurt * (self.arglist)[2] // 1000)
        LuaSkillCtrl:RemoveLife(damage, self, context.sender, false, true)
        context.hurt = 0
        LuaSkillCtrl:CallFloatText(self.caster, 8)
        self:PlayChipEffect() -- DECOMPILER ERROR at PC46: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.caster).recordTable).SkillTime =
            ((self.caster).recordTable).SkillTime - 1
        self:OnSkillTake()
    end
end
