...
bs_202301.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_202301_3", 1,
                    self.OnAfterHurt)
end

bs_202301.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                 isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if target.belongNum == (self.caster).belongNum and hurt > 0 and target ~=
        self.caster then
        local tier = (target.maxHp - target.hp) * 1000 // target.maxHp //
                         (self.arglist)[1]
        if (self.arglist)[3] // (self.arglist)[2] < tier then
            tier = (self.arglist)[3] // (self.arglist)[2]
        end
