...
-- function num : 0 , upvalues : _ENV
local CommanderSkillModulaData = class("CommanderSkillModulaData")
local CommanderSkillTreeData = require(
                                   "Game.CommanderSkill.CommanderSkillTreeData")
CommanderSkillModulaData.ctor = function(self, masterLevel, masterExp, treeMsg)
    -- function num : 0_0
    self.CommanderSkillTreeDataDic = {}
    self.masterLevel = masterLevel
...
CommanderSkillModulaData.InitCommanderSkillTreeDataDic =
    function(self, treeMsg)
        -- function num : 0_1 , upvalues : _ENV, CommanderSkillTreeData
        local treeCfg = ConfigData.commander_skill
        if treeCfg == nil then error("commander_skill read error") end
        if treeMsg == nil then
            for treeId, cfg in pairs(treeCfg) do
                local SkillTreeData = (CommanderSkillTreeData.New)(treeId)
                if (CheckCondition.CheckLua)(cfg.pre_condition, cfg.pre_para1,
                    ...
                    (NetworkManager:GetNetwork(NetworkTypeID.CommanderSkill)):CS_COMMANDSKILL_Unlock(
                        treeId)
                    SkillTreeData.isUnlock = true
                end -- DECOMPILER ERROR at PC34: Confused about usage of register: R9 in 'UnsetPending'
            ...
            do
                for treeId, cfg in pairs(treeCfg) do
                    ...
                    if treeData == nil then
                        SkillTreeData = (CommanderSkillTreeData.New)(treeId)
                        if (CheckCondition.CheckLua)(cfg.pre_condition,
                                                     cfg.pre_para1,
                                                     cfg.pre_para2) then
                            (NetworkManager:GetNetwork(
                                NetworkTypeID.CommanderSkill)):CS_COMMANDSKILL_Unlock(
                                treeId)
                            SkillTreeData.isUnlock = true
                        end
                    else
...
CommanderSkillModulaData.UpdateMasterLevelAndExp =
    function(self, masterLevel, masterExp)
        -- function num : 0_2 , upvalues : _ENV
        if not masterLevel then
            self.masterLevel = self.masterLevel
            if not masterExp then
                ...
                self:UpdateMasterPoint(masterLevel)
                MsgCenter:Broadcast(
                    eMsgEventId.OnCommanderSkillMasterLevelDiffer)
            end
        end
    end

CommanderSkillModulaData.UpdateMasterPoint =
    function(self, mastLevel)
        -- function num : 0_3 , upvalues : _ENV
        if mastLevel == 0 then return end
        local levelCfg = ConfigData.commander_skill_master_level
        if levelCfg == nil or levelCfg[mastLevel] == nil then
            error("commander_skill_master_level read error")
            return
...
CommanderSkillModulaData.CreatDefaultFormationCST =
    function(self, formId)
        -- function num : 0_4 , upvalues : _ENV
        local defaultTreeId =
            (ConfigData.game_config).FormationDefaultCommanderSkillTree
        local CST = {}
        local defaultCSTSaving = {}
        defaultCSTSaving.skillActive = {}
        defaultCSTSaving.skillProficient = {}
        local unlockSkillCfg =
            (ConfigData.commander_skill_unlock)[defaultTreeId]
        local defaultTreeLevel =
            ((self.CommanderSkillTreeDataDic)[defaultTreeId]).level
        for level, cfgs in pairs(unlockSkillCfg) do
            if defaultTreeLevel >= level then
                for skillId, skillCfg in pairs(cfgs) do
                    -- DECOMPILER ERROR at PC40: Confused about usage of register: R17 in 'UnsetPending'
                    ...
                    if skillCfg.skill_type == 1 then
                        if (defaultCSTSaving.skillActive)[skillCfg.place] ~= nil and
                            (defaultCSTSaving.skillActive)[skillCfg.place] <
                            skillId then
                            (defaultCSTSaving.skillActive)[skillCfg.place] =
                                skillId
                        else
                            ...
                            if (defaultCSTSaving.skillActive)[skillCfg.place] ==
                                nil then
                                (defaultCSTSaving.skillActive)[skillCfg.place] =
                                    skillId
                            end
        ...
        (NetworkManager:GetNetwork(NetworkTypeID.CommanderSkill)):CS_COMMANDSKILL_SaveFromFormation(
            formId, defaultTreeId, defaultCSTSaving)
        CST.saving = defaultCSTSaving
        CST.treeId = defaultTreeId
        return CST
    end

CommanderSkillModulaData.CreateSavingData =
    function(self, treeId)
        ...
        for place, masterSkillList in ipairs(treeData.masterSkillList) do
            for _, skillData in ipairs(masterSkillList) do
                ...
                if level > 0 then
                    (defaultCSTSaving.skillProficient)[skillData.dataId] =
                        {id = skillData.dataId, level = level}
                end
            end
        end
        return defaultCSTSaving
...
CommanderSkillModulaData.GetCSLastDiff =
    function(self)
        -- function num : 0_6
        return self.lastDiff
    end
