...
local UISkadaTeamPage = class("UISkadaTeamPage", UIBaseNode)
local base = UIBaseNode
local UINSkadaTeamItem = require(
                             "Game.BattleResult.SkadaSystem.UINSkadaTeamItem")
local SkadaEnum = require("Game.BattleResult.SkadaSystem.SkadaEnum")
local RoleType = {Player = 0, Enemy = 1}
UISkadaTeamPage.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINSkadaTeamItem
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_ChangeTeam, self,
                               self.OnBtnChangedTeamClicked)
    self.__OnClickHeroDetail = BindCallback(self, self.OnClickHeroDetail)
    self.__OnInfoPageReturn = BindCallback(self, self.OnInfoPageReturn);
    ((self.ui).skadaItem):SetActive(false)
    self.skadaItemPool = (UIItemPool.New)(UINSkadaTeamItem, (self.ui).skadaItem)
...
UISkadaTeamPage.InitSkadaTeamPage = function(self, battleStatistics,
                                             playerRoleList, enemyRoleList,
                                             resLoader, panelSkadaInfoPage)
    -- function num : 0_1 , upvalues : RoleType
    self.battleStatistics = battleStatistics
    self.resLoader = resLoader
    self.panelSkadaInfoPage = panelSkadaInfoPage
    self.playerSkada = self:GenSkada(playerRoleList,
                                     battleStatistics.playerDamage,
                                     battleStatistics.playerTakeDamage,
                                     battleStatistics.playerHeal)
    self.enemySkada = self:GenSkada(enemyRoleList, battleStatistics.enemyDamage,
                                    battleStatistics.enemyTakeDamage,
                                    battleStatistics.enemyHeal)
    self.roleType = RoleType.Player
end

UISkadaTeamPage.GenSkada = function(self, roleList, damageDic, takeDamageDic,
                                    healDic)
    -- function num : 0_2 , upvalues : SkadaEnum, _ENV
    local skadaList = {}
    local damageTotal = 0
    ...
    for k, v in ipairs(skadaList) do
        ...
        if damageTotal ~= 0 or not 0 then
            ((v.detail)[(SkadaEnum.eSkadaType).Damage]).proportion =
                ((v.detail)[(SkadaEnum.eSkadaType).Damage]).value /
                    (damageTotal)
            -- DECOMPILER ERROR at PC104: Confused about usage of register: R14 in 'UnsetPending'

            if takeDamageTotal ~= 0 or not 0 then
                ((v.detail)[(SkadaEnum.eSkadaType).TakeDamage]).proportion =
                    ((v.detail)[(SkadaEnum.eSkadaType).TakeDamage]).value /
                        (takeDamageTotal)
                -- DECOMPILER ERROR at PC120: Confused about usage of register: R14 in 'UnsetPending'

                if healTotal ~= 0 or not 0 then
                    do
                        ((v.detail)[(SkadaEnum.eSkadaType).Heal]).proportion =
                            ((v.detail)[(SkadaEnum.eSkadaType).Heal]).value /
                                (healTotal)
                        -- DECOMPILER ERROR at PC121: LeaveBlock: unexpected jumping out IF_THEN_STMT
...
UISkadaTeamPage.OnClickHeroDetail = function(self, roleSkada, roleIndex)
    ...
    (self.panelSkadaInfoPage):ShowSkadaInfoPage(roleIndex, temaSkada,
                                                belongPlayer,
                                                self.__OnInfoPageReturn)
    self:Hide()
    -- DECOMPILER ERROR: 3 unprocessed JMP targets
end

UISkadaTeamPage.OnBtnChangedTeamClicked =
    function(self)
        -- function num : 0_7
        self.roleType = 1 - self.roleType;
        ((self.ui).tex_ChangeTeam):SetIndex(self.roleType)
