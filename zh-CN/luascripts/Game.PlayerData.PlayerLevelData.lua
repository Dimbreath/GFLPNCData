...
PlayerLevelData.UpdatePlayerLevelData = function(self, level, exp)
    ...
    if self.level == level and self.exp == exp then return end
    ...
    self.exp = exp;
    (PlayerDataCenter.stamina):UpdateStaminaCfgByLevel(self.level)
    MsgCenter:Broadcast(eMsgEventId.UpdatePlayerLevel)
end
...
PlayerLevelData.__AddLevelBunos = function(self, level)
    ...
    for k, logic in ipairs(levelCfg.logic) do
        ...
        (PlayerDataCenter.playerBonus):InstallPlayerBonus(
            proto_csmsg_SystemFunctionID.SystemFunctionID_Achievement, level,
