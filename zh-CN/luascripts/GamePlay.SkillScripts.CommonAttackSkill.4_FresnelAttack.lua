...
bs_4.OnAttackTrigger = function(self, target, data, atkSpeedRatio, atkActionId)
    ...
    if LuaSkillCtrl:IsAbleAttackTarget(self.caster, target,
        ...
        if atkActionId == (self.config).action1 then
            ...
            LuaSkillCtrl:CallEffectWithArg(target, (self.config).effectId1,
                                           self, true, self.SkillEventFunc, data)
        else
            if (self.config).audioId6 ~= nil then
                ((self.caster).auSource):PlayAudioById((self.config).audioId6)
            end
            ...
            LuaSkillCtrl:CallEffectWithArg(target, (self.config).effectId2,
                                           self, true, self.SkillEventFunc, data)
        end
    else
        self:BreakSkill()
