...
local UINOverclockChipLableItem = require("Game.Sector.Overclock.UINOverclockChipLableItem")
...
UINOverclockChipSelectNode.OnInit = function(self)
  ...
  self.chipLablePool = (UIItemPool.New)(UINOverclockChipLableItem, (self.ui).obj_sortLabelItem)
  ;
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_closeChipDetail, self, self.HideChipDetail)
  self.__HideChipDetailWittScroll = BindCallback(self, self.HideChipDetailWittScroll)
  ;
  (((self.ui).scrollList).onValueChanged):AddListener(self.__HideChipDetailWittScroll)
  self.selectedChipItem = nil
  self.chipItemDic = {}
end

UINOverclockChipSelectNode.InitChipSelectNode = function(self, overclockId, overclockNode)
  -- function num : 0_1
...
UINOverclockChipSelectNode.__ReopenSelect = function(self)
  -- function num : 0_2
...
UINOverclockChipSelectNode.RefreshChips = function(self, chipDic)
...
, function(quality, chipList)
    -- function num : 0_3_1 , upvalues : self, _ENV
    if #chipList > 0 then
      local item = (self.chipLablePool):GetOne(true)
      local lableChipItemDic = item:InitLable(quality, chipList, BindCallback(self, self.SelectedChipItem))
      ;
...
UINOverclockChipSelectNode.SelectedChipItem = function(self, chipItem, transform, bool)
  -- function num : 0_4
  if not bool then
    local selectBool = not chipItem.isSelected
  end
  chipItem:SetSelect(selectBool)
  if selectBool then
    ((self.ui).scrollList):StopMovement()
    if self.selectedChipItem ~= nil and self.selectedChipItem ~= chipItem then
...
UINOverclockChipSelectNode.ChipDataClassify = function(self, chipDic)
...
UINOverclockChipSelectNode.ShowChipDetail = function(self, chipItem, transform)
  -- function num : 0_6 , upvalues : _ENV, HAType, VAType
  local pos = transform.position
  do
    if pos.y < 0 then
      local oldPos = (((self.ui).rect).transform).position
      -- DECOMPILER ERROR at PC24: Confused about usage of register: R5 in 'UnsetPending'

      ;
      (((self.ui).rect).transform).position = (Vector3.__add)(oldPos, (Vector3.New)(0, (math.abs)(pos.y, 0)))
      pos = (Vector3.__add)(pos, (Vector3.New)(0, (math.abs)(pos.y, 0)))
    end
    ;
...
UINOverclockChipSelectNode.HideChipDetailWittScroll = function(self, scrollPos)
...
UINOverclockChipSelectNode.HideChipDetail = function(self)
  -- function num : 0_8
  (((self.ui).trs_chipDetail).gameObject):SetActive(false)
end

UINOverclockChipSelectNode.ScrollToTop = function(self)
  -- function num : 0_9
...
UINOverclockChipSelectNode.OnClickConfirm = function(self)
  ...
  if self.selectedChipItem ~= nil then
    (self.OverclockCtrl):SetChip2OverClockOption(self.overclockId, ((self.selectedChipItem).data).dataId)
  else
    ;
