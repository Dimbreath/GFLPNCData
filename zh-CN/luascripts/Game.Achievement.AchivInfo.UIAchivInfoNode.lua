...
UIAchivInfoNode.UpdateAchivInfo = function(self, level)
    ...
    local icon = (self.resLoader):LoadABAsset(path) -- DECOMPILER ERROR at PC23: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).level_Icon).texture = icon;
    ((self.ui).tex_level):SetIndex(0, tostring(level));
    ((self.ui).tex_Content):SetIndex(0, tostring(levelCfg.stamina), tostring(
                                         FormatNum(levelCfg.resource / 10)),
                                     tostring(
                                         FormatNum(levelCfg.battle_exp / 10)))
end
