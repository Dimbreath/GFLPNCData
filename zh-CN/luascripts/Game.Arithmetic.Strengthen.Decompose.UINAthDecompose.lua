...
UINAthDecompose.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINBaseItemWithCount
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_Dismantle, self,
                               self._OnClickDecompose);
    (UIUtil.AddButtonListener)((self.ui).btn_AutoSelect, self,
                               self._OnClickAutoSelect) -- DECOMPILER ERROR at PC25: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC32: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ...
    self.athNetwork = NetworkManager:GetNetwork(NetworkTypeID.Arithmetic);
    ((self.ui).resItem):SetActive(false)
    self.shardItemPool = (UIItemPool.New)(UINBaseItemWithCount,
                                          (self.ui).resItem)
end
...
UINAthDecompose._RefreshSelectedList = function(self)
    ...
    for uid, v in pairs(self.athSelectedDic) do
        ...
        if shardQCfg == nil then
            ...
            if shardCfg == nil then
                error((string.format)(
                          "Cant get ath_shard, quality: %s, size: %s", quality,
                          size))
            else
                for index, itemId in ipairs(shardCfg.shardIdList) do
                    local itemNum = (shardCfg.shardNumList)[index]
    ...
    local selectedNum = #curSelectedAthList
    local canDecompose = selectedNum > 0;
    ((self.ui).empty):SetActive(not canDecompose) -- DECOMPILER ERROR at PC87: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).btn_Dismantle).interactable = canDecompose -- DECOMPILER ERROR at PC90: Confused about usage of register: R6 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).totalCount = selectedNum;
    ((self.ui).scrollRect):RefillCells();
    (table.sort)(shardItemIdList);
...
UINAthDecompose._OnAutoSelect = function(self)
    ...
    for k, athData in ipairs(tempList) do
        if not athData.lockUnlock then
            local athArea = athData:GetAthAreaType()
            local qualityConfig =
                (self.filtData)[(AthFilterEnum.eKindType).Area]
            if not qualityConfig.nocondition then
                local areaOk = (qualityConfig.selectIndexs)[athArea]
            end
            if areaOk then
                local athQualityId = athData:GetAthQuality()
                local qualityConfig =
                    (self.filtData)[(AthFilterEnum.eKindType).Quality]
                if not qualityConfig.nocondition then
                    local qualityOk =
                ...
                if qualityOk then
                    ...
                    local attrId = athData:GetAthMainAttrId()
                    for k, id in ipairs(AthFilterEnum.eAttribute) do
                        if attrId == id then attrIdx = k end
                    end
                    local attrConfig =
                        (self.filtData)[(AthFilterEnum.eKindType).Attribute]
                    if not attrConfig.nocondition then
                        local attrOk = (attrConfig.selectIndexs)[attrIdx]
                    end
                    if attrOk then
                        local subAttrConfig =
                            (self.filtData)[(AthFilterEnum.eKindType).SubAttribute]
                        local subAttrOk = subAttrConfig.nocondition
                        if not subAttrOk then
                            local subAttrList = athData:GetAthSubAttrIdList()
                            for k, subAttrId in ipairs(subAttrList) do
                                local subAttrIdx =
                                    (AthFilterEnum.GetAttrIndex)(subAttrId)
                                subAttrOk =
                                    (subAttrConfig.selectIndexs)[subAttrIdx]
                            end
                        end
                        do
                            do
...
UINAthDecompose._OnClickDecompose = function(self)
    -- function num : 0_6 , upvalues : _ENV
    if self.__OnDecomposeCompleteFunc == nil then
        self.__OnDecomposeCompleteFunc =
            BindCallback(self, self._OnDecomposeComplete)
    end
    self._heroIdSnapShoot = PlayerDataCenter:TakeHeroIdSnapShoot();
    (self.athNetwork):CS_ATH_OneKeyDeco(self.athSelectedDic,
                                        self.__OnDecomposeCompleteFunc)
end
