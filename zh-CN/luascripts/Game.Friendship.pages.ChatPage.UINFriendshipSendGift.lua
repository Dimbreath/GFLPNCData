...
local UINFriendshipGiftItem = require(
                                  "Game.Friendship.pages.ChatPage.UINFriendshipGiftItem")
UINFriendshipSendGift.OnInit = function(self)
    ...
    (((self.ui).btn_Add).onPress):AddListener(
        BindCallback(self, self.AccelerateAddOne));
    ...
    ((self.ui).giftItem):SetActive(false) -- DECOMPILER ERROR at PC99: Confused about usage of register: R1 in 'UnsetPending'
    ;
...
UINFriendshipSendGift.InitSendGift = function(self, heroID,
                                              refreshIntimacyBarCallBack)
    -- function num : 0_2 , upvalues : _ENV
    self.targetHeroID = heroID
    self.refreshIntimacyBarCallBack = refreshIntimacyBarCallBack
    self.sendNum = 0 -- DECOMPILER ERROR at PC8: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_Count).text = tostring(self.sendNum)
...
UINFriendshipSendGift.RegenerateGifts = function(self)
    ...
    for key, value in pairs(ConfigData.friendship_gift) do
        do (table.insert)(giftList, value) end
    end
    ...
    for _, gift in ipairs(giftList) do
        ...
        gitfItem:InitGiftItem(self.targetHeroID, self.resloader, gift.id,
                              itemNum, function()
            -- function num : 0_3_0 , upvalues : self, gift, _ENV, itemNum, gitfItem
            if self.selectedGiftId == gift.id then return end
            self.selectedGiftId = gift.id
            self.sendNum = 0 -- DECOMPILER ERROR at PC13: Confused about usage of register: R0 in 'UnsetPending'
            ;
            ((self.ui).tex_Count).text = tostring(self.sendNum)
            self.selectedGitfNum = itemNum
            self.selectedGitgIntimacyValue = gitfItem:GetIntimacyAddNum()
            self.selectedGitfMaxSendNum =
                (math.ceil)(self:GetMaxAddableEXP() /
                                self.selectedGitgIntimacyValue);
            (self.refreshIntimacyBarCallBack)()
        end, function()
            -- function num : 0_3_1 , upvalues : self, _ENV
            self.selectedGiftId = nil
            self.sendNum = 0 -- DECOMPILER ERROR at PC7: Confused about usage of register: R0 in 'UnsetPending'
            ;
            ((self.ui).tex_Count).text = tostring(self.sendNum)
            self.selectedGitfNum = 0;
            (self.refreshIntimacyBarCallBack)()
...
UINFriendshipSendGift.SortGifts = function(self, giftA, giftB)
    ...
    if giftB.id >= giftA.id then
        do
            return (PlayerDataCenter:GetItemCount(giftA.id) > 0 or
                       PlayerDataCenter:GetItemCount(giftB.id) <= 0) and isLikeA ~=
                       isLikeB ~= isDislikeA ~= isDislikeB
        end
...
UINFriendshipSendGift.AddOne = function(self)
    ...
    if self.selectedGitfNum < self.sendNum + 1 or self.selectedGitfMaxSendNum <
        self.sendNum + 1 then return end
    self.sendNum = self.sendNum + 1 -- DECOMPILER ERROR at PC19: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_Count).text = tostring(self.sendNum);
    (self.refreshIntimacyBarCallBack)(self.sendNum *
                                          self.selectedGitgIntimacyValue)
end

UINFriendshipSendGift.AccelerateAddOne =
    function(self)
        -- function num : 0_6 , upvalues : _ENV
        local pressedTime = ((self.ui).btn_Add):GetPressedTime()
        local addNum = (math.ceil)(pressedTime)
        do
            if self.selectedGitfNum < self.sendNum + addNum or
                self.selectedGitfMaxSendNum < self.sendNum + addNum then
                local newAddNum = (math.min)(
                                      self.selectedGitfNum - self.sendNum,
                                      self.selectedGitfMaxSendNum - self.sendNum)
                if newAddNum > 0 then
                    addNum = newAddNum
                else
                    return
            ...
            self.sendNum = self.sendNum + addNum -- DECOMPILER ERROR at PC40: Confused about usage of register: R3 in 'UnsetPending'
            ;
            ((self.ui).tex_Count).text = tostring(self.sendNum);
            (self.refreshIntimacyBarCallBack)(
                self.sendNum * self.selectedGitgIntimacyValue)
        end
    end
...
UINFriendshipSendGift.SubOne = function(self)
    ...
    if self.sendNum > 0 then
        self.sendNum = self.sendNum - 1 -- DECOMPILER ERROR at PC11: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ((self.ui).tex_Count).text = tostring(self.sendNum);
        (self.refreshIntimacyBarCallBack)(
            self.sendNum * self.selectedGitgIntimacyValue)
...
UINFriendshipSendGift.OnMax = function(self)
    ...
    if self.selectedGitfMaxSendNum <= self.selectedGitfNum then
        self.sendNum = self.selectedGitfMaxSendNum -- DECOMPILER ERROR at PC11: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ((self.ui).tex_Count).text = tostring(self.sendNum);
        (self.refreshIntimacyBarCallBack)(
            self.sendNum * self.selectedGitgIntimacyValue)
    else
        self.sendNum = self.selectedGitfNum -- DECOMPILER ERROR at PC25: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ((self.ui).tex_Count).text = tostring(self.sendNum);
        (self.refreshIntimacyBarCallBack)(
            self.sendNum * self.selectedGitgIntimacyValue)
...
UINFriendshipSendGift.OnReset = function(self)
    ...
    self.sendNum = 0 -- DECOMPILER ERROR at PC6: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_Count).text = tostring(self.sendNum);
    (self.refreshIntimacyBarCallBack)()
end
...
UINFriendshipSendGift.Send = function(self)
    -- function num : 0_10
    if self.targetHeroID == nil or self.sendNum <= 0 or self.selectedGiftId ==
        nil then return end
    (self.networkCtrl):CS_INTIMACY_Presentation(self.targetHeroID,
                                                self.selectedGiftId,
                                                self.sendNum)
end

UINFriendshipSendGift.GetMaxAddableEXP =
    function(self)
        -- function num : 0_11 , upvalues : _ENV
        local nowLevel = (PlayerDataCenter.allFriendshipData):GetLevel(
                             self.targetHeroID)
        local nowExp = (PlayerDataCenter.allFriendshipData):GetExp(
                           self.targetHeroID)
        local AddableEXP = 0
        for level = nowLevel, #ConfigData.friendship_level do
            AddableEXP = AddableEXP +
                             ((ConfigData.friendship_level)[level]).friendship
        end
