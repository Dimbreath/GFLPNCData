...
UINBattleDPSItem.InitDPSItem = function(self, id, damage, maxDamage)
    ...
    if not ConfigData:GetTipContent(TipContent.CommanderDPSName) then
        ((self.ui).tex_Name).text = id ~= 0 or "Commander" -- DECOMPILER ERROR at PC21: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_Career).sprite = CRH:GetSprite("career_commander",
                                                      CommonAtlasType.CareerCamp)
        self.damage = damage -- DECOMPILER ERROR at PC26: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_Bar).color = CareerToColor[0]
        self:RefreshDamage(maxDamage)
        do return end
        local heroCfg = (ConfigData.hero_data)[id]
        if heroCfg == nil then error("read heroCfg error id" .. id) end
        local careerCfg = (ConfigData.career)[heroCfg.career]
        if careerCfg == nil then
            error("read careerCfg error id" .. heroCfg.career)
        end -- DECOMPILER ERROR at PC58: Confused about usage of register: R6 in 'UnsetPending'
        
        ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)(heroCfg.name) -- DECOMPILER ERROR at PC67: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).img_Career).sprite = CRH:GetSprite(careerCfg.icon,
                                                      CommonAtlasType.CareerCamp)
        self.damage = damage -- DECOMPILER ERROR at PC73: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).img_Bar).color = CareerToColor[careerCfg.id]
        self:RefreshDamage(maxDamage)
    end
...
UINBattleDPSItem.RefreshDamage = function(self, maxDamage)
    ...
    ((self.ui).tex_Percent).text = tostring(self.damage) -- DECOMPILER ERROR at PC10: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_Bar).fillAmount = self.damage / maxDamage
end
...
UINBattleDPSItem.Clean = function(self)
    -- function num : 0_4
    self.damage = 0 -- DECOMPILER ERROR at PC3: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_Percent).text = 0 -- DECOMPILER ERROR at PC6: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).img_Bar).fillAmount = 0
end
