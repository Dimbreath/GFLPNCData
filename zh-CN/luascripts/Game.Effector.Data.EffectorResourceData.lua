...
EffectorResourceData.GetResCount = function(self, speed, period)
  ...
  do
    if self.relativeStartTm < limitTime then
      ...
      extraNum = extraNum + effectiveTime * speed * (PlayerDataCenter.AllBuildingData):GetResOutputEfficiency(self.itemId) // 1000
    end
    local onePeriodCount = speed * period
    local notFullPeriodCount = (resCount) % onePeriodCount
    local notFullPeriodExtraCount = (extraNum) % (onePeriodCount * (PlayerDataCenter.AllBuildingData):GetResOutputEfficiency(self.itemId) // 1000)
