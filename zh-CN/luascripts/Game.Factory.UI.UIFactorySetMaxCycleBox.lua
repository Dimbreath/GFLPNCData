...
UIFactorySetMaxCycleBox.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_Confirm, self, self.OnBtnConfirmClick)
  ;
...
UIFactorySetMaxCycleBox.InitSetMaxCycleBox = function(self, uiParent)
  -- function num : 0_1
...
UIFactorySetMaxCycleBox.ShowSetMaxCycleBox = function(self, lineItem)
  -- function num : 0_2 , upvalues : _ENV
  self.lineItem = lineItem
  self.cycle = ((self.lineItem).factoryCfg).cycle
  -- DECOMPILER ERROR at PC12: Confused about usage of register: R2 in 'UnsetPending'

  ;
  ((self.ui).tex_Name).text = (LanguageUtil.GetLocaleText)((lineItem.factoryCfg).name)
  self:InitProduce(lineItem.factoryCfg)
  self:InitConsume(lineItem.factoryCfg)
...
UIFactorySetMaxCycleBox.InitConsume = function(self, factoryCfg)
  ...
  for k,consumeId in ipairs(factoryCfg.consumeIds) do
    ...
    if itemCfg == nil then
      ...
      local consumeItem = (table.remove)(self.consumeItems, #self.consumeItems)
      do
        do
...
local cycleData = {}
UIFactorySetMaxCycleBox.OnSliderValueChange = function(self, value)
...
UIFactorySetMaxCycleBox.ShowInfomation = function(self, cycleData)
  ...
  if cycleData.setCycle ~= 0 and cycleData.setCycleTime ~= nil then
    ...
    ;
    ((self.ui).tex_Info):SetIndex(0, tostring((math.floor)(cycleData.setCycle)), time)
  else
    do
      ;
...
UIFactorySetMaxCycleBox.OnBtnConfirmClick = function(self)
  -- function num : 0_7 , upvalues : cycleData
  if cycleData.setCycle ~= nil then
    (self.uiParent):RequestAddOneFactoryLine(self.lineItem, cycleData.setCycle)
    self:Hide()
  end
end

UIFactorySetMaxCycleBox.OnBtnCancelClick = function(self)
  -- function num : 0_8
  self:Hide()
end
