...
local UIAchivLevelRewardPage = require("Game.Achievement.LevelRewardPage.UIAchivLevelRewardPage")
...
UIAchievement.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_LevelReward, self, self.OnClickAllLevelReward)
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_AchieventIcon, self, self.OnClickAchivInfo)
  self.levelRewardItemPool = (UIItemPool.New)(UICommonItem, (self.ui).levelRewardItem)
  self.achivTypeTogglePool = (UIItemPool.New)(UIAchivTypeToggle, (self.ui).tog_PageType)
  self.achivItemPool = (UIItemPool.New)(UIAchivUnitItem, (self.ui).achievementUnitItem)
...
UIAchievement.__InitAchievementRedDot = function(self)
  ...
  RedDotController:AddListener(RedDotDynPath.AchivTaskPagePath, self.__onAchivTaskRedDotEvent)
  ...
  if node:GetRedDotCount() <= 0 then
    ...
    self.__onLevelRewardRedDotEvent = function(node)
    -- function num : 0_1_1 , upvalues : self
    ((self.ui).redDotLevelReward):SetActive(node:GetRedDotCount() > 0)
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
  end

    RedDotController:AddListener(RedDotDynPath.AchivLevelReward, self.__onLevelRewardRedDotEvent)
    -- DECOMPILER ERROR: 2 unprocessed JMP targets
  end
end

UIAchievement.__UpdatePageToggleRedDot = function(self)
  -- function num : 0_2 , upvalues : _ENV
  for taskType,v in pairs(self.achivTypeToggleDic) do
    local ok, node = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.AchivLevel, RedDotStaticTypeId.AchivLevelPage, taskType)
    if node:GetRedDotCount() <= 0 then
      do
...
UIAchievement.__UpdateAchivUI = function(self)
  ...
  for k,v in ipairs(ConfigData.achievement) do
    ...
    toggle:InitAchivTypeToggle(k, v.name, isOn, BindCallback(self, self.OnAchivPageChange))
...
UIAchievement.__UpdateAllAchivItem = function(self)
  ...
  for k,v in ipairs(achivDatas) do
    ...
    achivItem:InitAchivUnitItem(v, v.stcData, self.resLoader, BindCallback(self, self.GetAchivReward))
...
UIAchievement.__RefreshCompletedCount = function(self)
  ...
  for k,v in pairs(ConfigData.achievement) do
    local datas = ((PlayerDataCenter.allTaskData).achievementDatas)[v.task_type]
    if datas ~= nil then
      allCount = allCount + #datas
    end
  end
  ;
  ((self.ui).tex_Completed):SetIndex(0, tostring(completedCount), tostring(allCount))
end
...
UIAchievement.__UpdateCompletedAchiv = function(self)
  ...
  for k,v in pairs(pickedAchivs) do
    ...
    if achivItem == nil then
      achivItem = (self.achivItemPool):GetOne()
      ...
      achivItem:InitAchivUnitItem(nil, stcData, self.resLoader, nil, v.completedTimestamp)
      achivItem:SetJump2Finish(JumpManager, self.uiTopStatus)
    end
  end
...
UIAchievement.OnTaskUpdate = function(self, taskData)
  ...
  local taskTypeId = ((ConfigData.achievement)[self.achivPageId]).task_type
  if (taskData.stcData).type ~= taskTypeId then
    return 
  end
  ...
  achivItem:InitAchivUnitItem(taskData, taskData.stcData, self.resLoader, BindCallback(self, self.GetAchivReward))
  achivItem:SetJump2Finish(JumpManager, self.uiTopStatus)
  self:__SortAchivItem()
end
...
UIAchievement.OnDelete = function(self)
  ...
  RedDotController:RemoveListener(RedDotDynPath.AchivTaskPagePath, self.__onAchivTaskRedDotEvent)
  RedDotController:RemoveListener(RedDotDynPath.AchivLevelReward, self.__onLevelRewardRedDotEvent)
