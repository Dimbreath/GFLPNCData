...
SectorStageData.InitLevelFirstClearRedDot = function(self)
  ...
  for _,id in ipairs((ConfigData.sector).id_sort_list) do
    ...
    local sectorStageList = ((ConfigData.sector_stage).sectorIdList)[sectorId]
    for _,i in ipairs(sectorStageList) do
      local sectorStage = (ConfigData.sector_stage)[i]
      RedDotController:AddRedDotNodeWithPath(RedDotDynPath.SectorLevelDifficultPath, RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, sectorId, sectorStage.difficulty)
      local node = RedDotController:AddRedDotNodeWithPath(RedDotDynPath.SectorLevelBtnPath, RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, sectorId, sectorStage.difficulty, sectorStage.id)
      local count = 0
...
SectorStageData.UpdateSctStageItemRedDot = function(self, stageId)
  -- function num : 0_8 , upvalues : _ENV
  local sectorStage = (ConfigData.sector_stage)[stageId]
  local ok, node = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, sectorStage.sector, sectorStage.difficulty, sectorStage.id)
  if ok then
    local count = 0
...
SectorStageData.UpdateLevelFirstClearRedDot = function(self, data)
  -- function num : 0_9
end
