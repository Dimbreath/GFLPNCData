...
, nil, Times)
    ...
    -- function num : 0_2_1 , upvalues : _ENV, self, targetNext
    local skillResult3 = LuaSkillCtrl:CallSkillResultNoEffect(self, targetNext)
    LuaSkillCtrl:HurtResult(skillResult3, (self.config).HurtConfig2)
...
bs_1027.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1027_3", 1,
                    self.OnAfterHurt)
end

bs_1027.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.dataId == 3 and not isMiss then
        local targets = (self:GetBehindTargetsPos(2, (self.caster).x,
                                                  (self.caster).y, target.x,
                                                  target.y))
        do
            local targetNext = nil
            if targets ~= nil and (table.length)(targets) >= 2 and
                (targets[2]).belongNum ~= (self.caster).belongNum then
                targetNext = targets[2]
            end
            local hurt1 = LuaSkillCtrl:CallFormulaNumberWithSkill(
                              (self.config).hurt_formula, self.caster, target,
                              self)
            local hurt2 = hurt * (self.arglist)[5] // 1000
            local Times = (self.arglist)[3] // (self.arglist)[4] - 1
            self.damTimer = LuaSkillCtrl:StartTimer(self, (self.arglist)[4],
                                                    function()
                -- function num : 0_2_0 , upvalues : _ENV, self, target, hurt1
            ...
            if targetNext ~= nil and (self.arglist)[2] == 1 then
                LuaSkillCtrl:CallEffect(targetNext, 10013, self)
                local skillResult2 = LuaSkillCtrl:CallSkillResultNoEffect(self,
                                                                          targetNext)
                LuaSkillCtrl:HurtResult(skillResult2, (self.config).HurtConfig1,
                                        {hurt2})
                skillResult2:EndResult()
                self.damTimer2 = LuaSkillCtrl:StartTimer(self,
                                                         (self.arglist)[4],
                                                         function()
                    -- function num : 0_2_1 , upvalues : _ENV, self, targetNext
                    local skillResult3 =
                        LuaSkillCtrl:CallSkillResultNoEffect(self, targetNext)
                    LuaSkillCtrl:HurtResult(skillResult3,
                                            (self.config).HurtConfig2)
                    skillResult3:EndResult()
                    if targetNext.hp < hurt3 and self.damTimer2 ~= nil then
                        (self.damTimer2):Stop()
                        self.damTimer2 = nil
