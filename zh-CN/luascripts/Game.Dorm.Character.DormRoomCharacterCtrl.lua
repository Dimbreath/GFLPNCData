...
local DormCharacterEntity = require(
                                "Game.Dorm.Character.Entity.DormCharacterEntity")
local DormUtil = require("Game.Dorm.DormUtil")
DormRoomCharacterCtrl.ctor =
    function(self, characterCtrl, roomEntity, resloader)
...
DormRoomCharacterCtrl.GetRoomCharacterCount =
    function(self)
        -- function num : 0_1
        return self.__charCount
    end

DormRoomCharacterCtrl.IsRoomCharacterFull =
    function(self)
        -- function num : 0_2 , upvalues : _ENV
        do
            return (ConfigData.game_config).DormRoomRoleMaxCount <=
                       self.__charCount
        end
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end

DormRoomCharacterCtrl.InitDormRoomCharacterCtrl =
    function(self)
        ...
        for k, fntData in pairs((self.roomData).fntDataList) do
            ...
            (DormUtil.AddFntToMapDic)(self.mapDic, fntData.x, fntData.y, sizeX,
                                      sizeY, fntData.r)
        end
...
DormRoomCharacterCtrl.StartCharacterLogic =
    function(self)
...
DormRoomCharacterCtrl.AddUnBindCharacter =
    function(self, heroData, posX, posY)
        -- function num : 0_5 , upvalues : _ENV, DormCharacterEntity
        if posX == nil or posY == nil then
            local randX =
                (math.random)((ConfigData.game_config).HouseGridCount) - 1
            do
                local randY = (math.random)(
                                  (ConfigData.game_config).HouseGridCount) - 1
        ...
        do
            if posX ~= nil and posY ~= nil then
                ...
                local charEntity = (DormCharacterEntity.New)(heroData, posX,
                                                             posY, self, false) -- DECOMPILER ERROR at PC40: Confused about usage of register: R5 in 'UnsetPending'
                ;
                (self.characterDic)[heroData.dataId] = charEntity;
                (self.resloader):LoadABAssetAsync(
                    PathConsts:GetCharacterDormModelPath(heroData:GetResName()),
                    function(prefab)
                        ...
                        local go = prefab:Instantiate(
                                       (self.roomEntity).characterHolder)
...
DormRoomCharacterCtrl.FindActivePosNearby =
    function(self, x, y)
        -- function num : 0_6 , upvalues : DormUtil, _ENV
        if not (DormUtil.IsFntCoordLegal)(x, y) then return end
        local pos = (DormUtil.XYCoord2Fnt)(x, y)
        if (self.mapDic)[pos] == 0 then return x, y end
        local pointQue = {}
        local closeQue = {};
...
DormRoomCharacterCtrl.AddBindCharacter =
    function(self, heroData, fntData)
        ...
        self.__charCount = self.__charCount + 1
        local charEntity = (DormCharacterEntity.New)(heroData, posX, posY, self,
                                                     true) -- DECOMPILER ERROR at PC21: Confused about usage of register: R6 in 'UnsetPending'
        ;
        (self.characterDic)[heroData.dataId] = charEntity;
        (self.resloader):LoadABAssetAsync(
            PathConsts:GetCharacterDormModelPath(heroData:GetResName()),
            function(prefab)
                ...
                if IsNull(prefab) or self.__isDelete then return end
                local go = prefab:Instantiate((self.roomEntity).characterHolder)
                charEntity:InitCharacterGo(go)
            end)
    end

DormRoomCharacterCtrl.GetCharacterEntityById =
...
DormRoomCharacterCtrl.RemoveCharacterEntity =
    function(self, charEntity)
        -- function num : 0_9
...
DormRoomCharacterCtrl.DeleteCharacterById =
    function(self, heroId)
        -- function num : 0_10
        local findEntity = (self.characterDic)[heroId]
...
DormRoomCharacterCtrl.AddCharacterEntity =
    function(self, charEntity)
        -- function num : 0_11
        local heroId = (charEntity.heroData).dataId
        if (self.characterDic)[heroId] ~= nil then return end -- DECOMPILER ERROR at PC8: Confused about usage of register: R3 in 'UnsetPending'
...
DormRoomCharacterCtrl.FindNearActivePoint =
    function(self, count, x, y)
        ...
        while #pointQue > 0 do
            ...
            for k, v in ipairs(DormUtil.FntForOffset) do
                ...
                local idy = dy + v.y
                local ipos = (DormUtil.XYCoord2Fnt)(idx, idy)
                if not closeQue[ipos] and (DormUtil.IsFntCoordLegal)(idx, idy) and
                    (self.mapDic)[ipos] == 0 then
                    (table.insert)(findQue, ipos);
                    (table.insert)(pointQue, ipos)
                    closeQue[ipos] = true
                end
            end
            if count <= #findQue then return findQue end
        end
        do return findQue end
    end

DormRoomCharacterCtrl.GetPointPath = function(self, pos1, pos2)
    -- function num : 0_13 , upvalues : DormUtil, _ENV
    local path = {}
    if pos1 == pos2 then return path end
    local startX, startY = (DormUtil.FntCoord2XY)(pos1)
    local destX, destY = (DormUtil.FntCoord2XY)(pos2)
    if not (DormUtil.IsFntCoordLegal)(startX, startY) or
        not (DormUtil.IsFntCoordLegal)(destX, destY) then return end
    local pointQue = {}
    local closeQue = {}
    local parentDic = {}
    closeQue[pos1] = true;
...
DormRoomCharacterCtrl.IsPointDirectMove =
...
DormRoomCharacterCtrl.OnUpdate = function(self)
    ...
    if self.characterDic == nil then return end
    for heroId, charEntity in pairs(self.characterDic) do
        charEntity:OnUpdate()
    end
end

DormRoomCharacterCtrl.HidePauseRoomCharacter =
    function(self)
        -- function num : 0_16 , upvalues : _ENV
        for k, v in pairs(self.characterDic) do v:HidePauseCharacter() end
    end

DormRoomCharacterCtrl.ShowResumeRoomCharacter =
    function(self)
        -- function num : 0_17 , upvalues : _ENV
        for k, v in pairs(self.characterDic) do v:ShowResumeCharacter() end
    end
...
DormRoomCharacterCtrl.UnSetCurEnterRoom =
    function(self)
        -- function num : 0_19
        if not self.__isCurEnterRoom then return end
        self.__isCurEnterRoom = false
        self:ClearSelectInterPoint()
        self:RecoveryInterPointEntity()
    end

DormRoomCharacterCtrl.GenRoomInterPointEntity =
    function(self)
        ...
        for _, interPoint in pairs((self.roomData).interpoint) do
            ...
            local offsetWorld = (fntEntity.transform):TransformPoint(
                                    interPoint:GetInterOffsetCoord())
            offsetWorld.y = offsetWorld.y + interPoint:GetInterHeight()
            local pointEntity = (self.characterCtrl):GenInterPointEntity(
                                    interPoint, offsetWorld);
            (table.insert)(self.pointEntityList, pointEntity)
        end
    end

DormRoomCharacterCtrl.GetInterPointEntityList =
    function(self)
        -- function num : 0_21
        return self.pointEntityList
    end

DormRoomCharacterCtrl.RefreshPointEntityPos =
    function(self)
        -- function num : 0_22 , upvalues : _ENV
        if self.pointEntityList == nil then return end
        for _, pointEntity in pairs(self.pointEntityList) do
            local fntEntity = (self.roomEntity):GetFntByData(
                                  (pointEntity.interPoint).fntData)
            local offsetWorld = (fntEntity.transform):TransformPoint(
                                    (pointEntity.interPoint):GetInterOffsetCoord()) -- DECOMPILER ERROR at PC20: Confused about usage of register: R8 in 'UnsetPending'
            ;
            (pointEntity.transform).position = offsetWorld
        end
    end

DormRoomCharacterCtrl.RefreshInterPointDistanceState =
    function(self, touchScreenPos)
        ...
        if selectPoint == self.selectPoint then return end
...
DormRoomCharacterCtrl.SetAllPointNormalActive =
...
DormRoomCharacterCtrl.ClearSelectInterPoint =
    function(self)
        -- function num : 0_25
        if self.selectPoint == nil then return end
        (self.selectPoint):SetPointSelectActive(false)
    end

DormRoomCharacterCtrl.RecoveryInterPointEntity =
    function(self)
...
DormRoomCharacterCtrl.FinishCharacterOperate =
    function(self, charEntity)
        -- function num : 0_27
        do
            if self.selectPoint ~= nil then
                local charEntity =
                    ((self.selectPoint).interPoint):GetBindCharacter()
...
DormRoomCharacterCtrl.ExchangeUnBindCharacter =
    function(self, oldEntity)
        -- function num : 0_30
        if oldEntity ~= nil then
            self:DeleteCharacterById((oldEntity.heroData).dataId)
        end
        local charEntity = (self.characterCtrl):RandChangeUnBindCharacter(self,
                                                                          oldEntity)
        if charEntity ~= nil then charEntity:StartAIBehavior() end
    end

DormRoomCharacterCtrl.HaveOtherNoFullRoom =
    function(self)
...
DormRoomCharacterCtrl.RandRemoveUnBindCharacter =
    function(self)
        ...
        if #unbindList == 0 then return end
...
DormRoomCharacterCtrl.OnDelete = function(self)
    ...
    if self.characterDic ~= nil then
        for k, v in pairs(self.characterDic) do v:OnDelete() end
    end
    do self.__isDelete = true end
end
