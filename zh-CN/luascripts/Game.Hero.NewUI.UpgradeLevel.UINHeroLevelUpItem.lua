...
UINHeroLevelUpItem.OnInit = function(self)
    ...
    (((self.ui).btn_Minus).onPress):AddListener(
        BindCallback(self, self.PressMinus))
end

UINHeroLevelUpItem.InitExpItem = function(self, itemData, numChangeFunc,
                                          checkItemUseFunc)
    -- function num : 0_1 , upvalues : _ENV
    local itemCfg = itemData.itemCfg
    self.id = itemCfg.id
    ...
    self.numChangeFunc = numChangeFunc
    self.checkItemUseFunc = checkItemUseFunc
    self.exp = itemData:GetActionArg(1) -- DECOMPILER ERROR at PC14: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).CanvasGroup).alpha = 1;
    ((self.ui).tex_TotalCount):SetIndex(0, tostring(self.itemNum));
    (self.baseItem):InitBaseItem(itemCfg, BindCallback(self, self.AddOne))
...
UINHeroLevelUpItem.InitEmptyExpItem = function(self, itemId)
    ...
    local itemCfg = (ConfigData.item)[itemId]
    self.id = itemId;
    (((self.ui).obj_UseCount).gameObject):SetActive(false) -- DECOMPILER ERROR at PC12: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).CanvasGroup).alpha = 0.5;
    (((self.ui).btn_Minus).gameObject):SetActive(false);
    ((self.ui).tex_TotalCount):SetIndex(0, "0");
        ...
        if self.checkItemUseFunc ~= nil then
            ...
            if not canUse then
                ((CS.MessageCommon).ShowMessageTips)(
                    ConfigData:GetTipContent(TipContent.LevelUp_Limit))
                return
            end
        end
        if self.itemNum < self.selectedNum + 1 or self.addableExp <= 0 then
...
UINHeroLevelUpItem.PressAdd = function(self)
    ...
    do
        ...
        local addNum = (math.ceil)(pressedTime * pressedTime / 5)
        do
            if self.itemNum <= self.selectedNum + addNum or self.addableExp <
                addNum * self.exp then
                local newAddNum = (math.min)(self.itemNum - self.selectedNum,
                                             (math.ceil)(
                                                 self.addableExp / self.exp))
                if newAddNum > 0 then
                    addNum = newAddNum
                else
                    return
