...
FmtHeroEntity.InitFmtHeroEntity = function(self, go, heroData, camera)
    ...
    local coroutineFunc = function()
        ...
        self.heroGo = (modelWait.Result):Instantiate(self.transform) -- DECOMPILER ERROR at PC25: Confused about usage of register: R1 in 'UnsetPending'
        ;
...
FmtHeroEntity.__UpdateInfoPos = function(self)
    -- function num : 0_3
    if self.fmtWindow ~= nil then
        (self.fmtWindow):UpdateFmtHeroInfoPos((self.heroData).dataId,
                                              self:__GetInfoPos())
    end
end
