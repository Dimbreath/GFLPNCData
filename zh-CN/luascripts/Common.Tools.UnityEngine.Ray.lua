...
Ray.__tostring = function(self)
    -- function num : 0_5 , upvalues : _ENV
    return (string.format)("Origin:(%f,%f,%f),Dir:(%f,%f, %f)", (self.origin).x,
                           (self.origin).y, (self.origin).z, (self.direction).x,
                           (self.direction).y, (self.direction).z)
end

Ray.unity_ray = (CS.UnityEngine).Ray -- DECOMPILER ERROR at PC29: Confused about usage of register: R5 in 'UnsetPending'
;
(CS.UnityEngine).Ray = Ray
setmetatable(Ray, Ray)
return Ray
