...
UITask.OnInit = function(self)
    ...
    self._RefreshSubPages = BindCallback(self, self.RefreshSubPages)
    self._OnSelectPage = BindCallback(self, self.OnSelectPage)
    self.pageTogPool = (UIItemPool.New)(UINTaskPageTog,
                                        (self.ui).obj_taskPageTog)
    self.subPageTogPool = (UIItemPool.New)(UINTaskSubPageTog,
                                           (self.ui).obj_subPageTog);
    ((self.ui).obj_taskPageTog):SetActive(false);
    ((self.ui).obj_subPageTog):SetActive(false)
    self.taskListNode = (UINTaskList.New)();
    (self.taskListNode):Init((self.ui).obj_taskListNode)
...
UITask.RefreshPages = function(self)
    ...
    local autoSelectPage = 1
    if (self.ctrl).isDailyTaskUnlock then autoSelectPage = 4 end
    for groupIndex, group in ipairs(self.pageGroupList) do
        if isDailyTaskUnlock or not (table.contain)(group, 4) then
            local pageTog = (self.pageTogPool):GetOne()
            pageTog:InitPageTog(groupIndex, group, self._RefreshSubPages,
                                self._OnSelectPage)
            local totalRedDotCount = 0
            for index, taskTypeId in ipairs(group) do
                local OK, taskPageNode =
...
UITask.RefreshSubPages = function(self, idList, nameList)
    ...
    for i, typeId in ipairs(idList) do
        ...
        local OK, taskPageNode = RedDotController:GetRedDotNode(
                                     RedDotStaticTypeId.Main,
                                     RedDotStaticTypeId.Task, typeId)
        subPageTog:SetRedDotActive(not OK or taskPageNode:GetRedDotCount() > 0)
    end
    -- DECOMPILER ERROR: 3 unprocessed JMP targets
end
...
UITask.SetTaskNodeParent = function(self, isBase)
    -- function num : 0_3 , upvalues : _ENV
    if isBase then
        (((self.ui).obj_taskListNode).transform):SetParent(
            ((self.ui).basePos).transform)
    else

        (((self.ui).obj_taskListNode).transform):SetParent(
            ((self.ui).peroidPos).transform)
    end -- DECOMPILER ERROR at PC24: Confused about usage of register: R2 in 'UnsetPending'
    
    (((self.ui).obj_taskListNode).transform).localPosition = Vector3.zero -- DECOMPILER ERROR at PC30: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (((self.ui).obj_taskListNode).transform).sizeDelta = Vector2.zero
end
    ...
    if peroidDatas ~= nil then
        do
            ...
            if peroidDatas == nil then
                ...
                for index, togItem in ipairs((self.pageTogPool).listItem) do
                    if (table.contain)(togItem.group, typeId) then
                        for index, subTogItem in
                            ipairs((self.subPageTogPool).listItem) do
                            if subTogItem.id == typeId then
                                subTogItem:SetMissionCount(taskDatas)
                            else
                                subTogItem:SetMissionCount(
                                    (self.ctrl):GetDatas4Task(subTogItem.id))
                            end
                        end
                    end
                end
        ...
        if pageTog.isMult then
            for subIndex, subPageTypeId in ipairs(pageTog.group) do
                ...
                if typeId == subPageTypeId then
                    ((pageTog.ui).tog_taskPageTog).isOn = true
                    local subPageTog =
                        ((self.subPageTogPool).listItem)[subIndex] -- DECOMPILER ERROR at PC22: Confused about usage of register: R13 in 'UnsetPending'
                    ;
                    ((subPageTog.ui).tog_subPageTog).isOn = true
                    return
                end
...
UITask.RefreshTask = function(self, taskDatas, needSort)
    -- function num : 0_6 , upvalues : _ENV
    local taskList = {}
    for key, value in pairs(taskDatas) do (table.insert)(taskList, value) end
    (self.taskListNode):RefreshTaskItems(taskList, needSort)
end
...
UITask.RegistRedDot = function(self)
    ...
    self.__onHeroCardRedDotEvent = function(node)
        ...
        local OK, taskPageNode = RedDotController:GetRedDotNode(
                                     RedDotStaticTypeId.Main,
                                     RedDotStaticTypeId.Task, node.nodeId)
        for groupIndex, group in ipairs(self.pageGroupList) do
            if (table.contain)(group, node.nodeId) then
                local pageTog = ((self.pageTogPool).listItem)[groupIndex]
                if pageTog == nil then return end
                local totalRedDotCount = 0
                for index, taskTypeId in ipairs(group) do
                    local OK, taskPageNode =
                        RedDotController:GetRedDotNode(RedDotStaticTypeId.Main,
                    ...
                    if OK then
                        totalRedDotCount =
                            totalRedDotCount + taskPageNode:GetRedDotCount()
                    end
                end
                pageTog:SetRedDotActive(totalRedDotCount > 0)
            end
    ...
    RedDotController:AddListener(RedDotDynPath.TaskPagePath,
                                 self.__onHeroCardRedDotEvent)
end

UITask.RemoveRedDot = function(self)
    -- function num : 0_10 , upvalues : _ENV
    RedDotController:RemoveListener(RedDotDynPath.TaskPagePath,
                                    self.__onHeroCardRedDotEvent)
end
...
UITask.SetTaskAllTweens = function(self)
    ...
    local moveY = 20
    local pageLocalPos = (((self.ui).taskPage).transform).localPosition
    pageLocalPos = (Vector3.New)(pageLocalPos.x, pageLocalPos.y - moveY,
                                 pageLocalPos.z) -- DECOMPILER ERROR at PC16: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (((self.ui).taskPage).transform).localPosition = pageLocalPos -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).taskPage).alpha = 0
    self.pageSequence = (cs_DoTween.Sequence)();
    (self.pageSequence):Append(((((self.ui).taskPage).transform):DOLocalMoveY(
                                   moveY, 0.3)):SetRelative(true));
    (self.pageSequence):Join(((self.ui).taskPage):DOFade(1, 0.3));
    (self.pageSequence):AppendCallback(function()
        -- function num : 0_12_0
    end);
