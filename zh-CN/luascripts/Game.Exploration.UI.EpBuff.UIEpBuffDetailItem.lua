...
UIEpBuffDetailItem.InitEpBuffDetail = function(self, epBuff)
    ...
    (self.buffItem):InitEpBuffByCfg(epBuff, buffCfg) -- DECOMPILER ERROR at PC25: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_BuffName).text = (LanguageUtil.GetLocaleText)(buffCfg.name) -- DECOMPILER ERROR at PC32: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_BuffInfomation).text =
        (LanguageUtil.GetLocaleText)(buffCfg.describe)
    self.expireTm = epBuff.expireTm;
    (((self.ui).tex_BuffTime).gameObject):SetActive(self.expireTm > 0)
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end

UIEpBuffDetailItem.RefershBuffDetailTime =
    function(self, curTime)
        -- function num : 0_2 , upvalues : _ENV
        if self.expireTm <= 0 then return end
        local lastTime = (math.ceil)((math.max)(self.expireTm - curTime, 0)) -- DECOMPILER ERROR at PC18: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.ui).tex_BuffTime).text = TimestampToTime(lastTime)
    end
