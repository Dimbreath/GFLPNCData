...
bs_1038.OnSetHurt = function(self, context)
    ...
    if context.target == self.caster and LuaSkillCtrl:CallRange(1, 100) <=
        (self.arglist)[3] and (context.skill).dataId ~= 202003 and
        (LuaSkillCtrl:GetGridsDistance((context.sender).x, (context.sender).y,
                                       (self.caster).x, (self.caster).y) > 1 or
            (context.sender).attackRange > 1 or (context.skill).dataId == 201903) then
        ((self.caster).auSource):PlayAudioById((self.config).audioId1) -- DECOMPILER ERROR at PC47: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["1038_hurt"] = context.hurt
