...
bs_10137.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_10137_1", 1,
...
bs_10137.OnAfterBattleStart = function(self)
    ...
    LuaSkillCtrl:CallCircledEmissionStraightly(self, self.caster, self.caster,
                                               200, 0,
                                               eColliderInfluenceType.Enemy,
                                               collisionEnter, nil,
                                               collisionExit, cusEffect, false,
                                               false, nil, self.caster)
end

bs_10137.OnCollisionEnter = function(self, index, entity)
    -- function num : 0_3 , upvalues : _ENV
    local layer = (self.caster):GetBuffTier((self.config).buffDefenceId)
    local defenceBuffTier =
        entity.pow * (self.arglist)[1] * (self.arglist)[2] // 1000000
    if (self.arglist)[1] <= layer then
        LuaSkillCtrl:DispelBuff(entity, (self.config).buffBloodId, 0)
        LuaSkillCtrl:CallBuff(self, entity, (self.config).buffBloodId,
                              (self.arglist)[1])
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffDefenceId,
                              defenceBuffTier)
    else
        LuaSkillCtrl:CallBuff(self, entity, (self.config).buffBloodId,
                              (self.arglist)[1])
