...
local bs_202202 = class("bs_202202", LuaSkillBase)
local base = LuaSkillBase
bs_202202.config = {realDamageConfig = {basehurt_formula = 502}}
bs_202202.ctor = function(self)
    -- function num : 0_0
end
    ...
    LuaSkillCtrl:StartTimer(nil, 3, function()
        ...
        if targetList.Count > 0 then
            for i = 0, targetList.Count - 1 do
                LuaSkillCtrl:CallEffect((targetList[i]).targetRole, 10434, self)
                local number =
                    (self.arglist)[1] * (self.caster).skill_intensity // 1000
                if number <= 0 then number = 1 end
                LuaSkillCtrl:CallRealDamage(self, (targetList[i]).targetRole,
                                            nil, (self.config).realDamageConfig,
                                            {number}, true)
            end
        end
        do
            ...
            if targetlist2.Count > 0 then
                for i = 0, targetlist2.Count - 1 do
                    local number2 = (self.arglist)[1] *
                                        (self.caster).skill_intensity // 1000
                    if number2 <= 0 then number2 = 1 end
                    LuaSkillCtrl:CallHeal(number2, self,
                                          (targetlist2[i]).targetRole)
                end
            end
        end
    end)
