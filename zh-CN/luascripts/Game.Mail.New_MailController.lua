...
New_MailController.m_InitDataCallback = function(self, args)
    ...
    if args ~= nil and args.Count > 0 then
        ...
        self:InitOrUpdateData(mailDetails)
    else
        do error("InitData args error") end
    end
end
...
New_MailController.ReqReceiveAttachment =
    function(self, uid)
...
New_MailController.ReqOneClickPickUp = function(self)
    ...
    if self.haveNotGetRewardMail then
        (self.network):CS_MAIL_OneClickPickUp(
            function(args)
                ...
                if args ~= nil and args.Count > 0 then
                    ...
                    do
                        ...
                        UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward,
                                                  function(window)
                            -- function num : 0_9_0_0 , upvalues : rewardIds, rewardNums
                            if window == nil then
                                return
                            end
                            window:InitRewardsItem(rewardIds, rewardNums)
...
New_MailController.ReqOneClickDelete = function(self)
    -- function num : 0_10
    if self.haveCouldDeleteMail then (self.network):CS_MAIL_OneKeyDelete() end
end
...
New_MailController.HasMailNotify = function(self)
    ...
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(
                                           TipContent.mail_Notice))
    local mailNode = RedDotController:AddRedDotNode(RedDotStaticTypeId.Main,
    ...
    if mailWin ~= nil then self:ReqMailFetch(false) end
end
...
New_MailController.InitOrUpdateData = function(self, mailDetails)
    -- function num : 0_14 , upvalues : _ENV, MailData
    local hasDiff = false
    local hasNew = false
    local diffDatas = {}
    for _, mailDetail in pairs(mailDetails) do
        if (self.mailDataDic)[mailDetail.uid] ~= nil then
            ...
            local mailData = (MailData.CreateMailData)(mailDetail) -- DECOMPILER ERROR at PC32: Confused about usage of register: R11 in 'UnsetPending'
            ;
    ...
    if self.isUIOpen then self:StartAutoDeleteTimer() end
...
New_MailController.DeleteData = function(self, delete)
    ...
    for uid, _ in pairs(delete) do
        if (self.mailDataDic)[uid] ~= nil then
            ((self.mailDataDic)[uid]):Delete() -- DECOMPILER ERROR at PC13: Confused about usage of register: R7 in 'UnsetPending'
            ;
...
New_MailController.m_RefreshDeleteAndNewState =
    function(self)
        ...
        for _, mailData in pairs(self.mailDataDic) do
            ...
            if isHaveAtt and not isPicked then
                ...
                if isHaveAtt and isPicked then
                    ...
                    if not isHaveAtt and mailData:GetState() ~=
                        (MailEnum.eMailDetailType).None then
                        self.haveCouldDeleteMail = true
                    end
                end
            end
...
New_MailController.GetMailDataList = function(self)
    ...
    local list = {}
    for _, data in pairs(self.mailDataDic) do (table.insert)(list, data) end
    (table.sort)(list, function(a, b)
        -- function num : 0_17_0
        if b:GetTime(true) >= a:GetTime(true) then
...
New_MailController.StartAutoDeleteTimer =
    function(self)
        ...
        if (table.count)(self.mailDataDic) > 0 then
            ...
            self.m_outDataTimer = (TimerManager:GetTimer(time, (BindCallback(
                                                             self,
                                                             self.DeletAllOutDataMail,
                                                             true)), nil, true,
                                                         false, true)):Start()
...
New_MailController.DeletAllOutDataMail =
    function(self, isFromTimer)
        -- function num : 0_20 , upvalues : _ENV
        local delete = {}
        for uid, data in pairs(self.mailDataDic) do
            if data:GetTimeBeforeExpired() <= 0 then
                ((self.mailDataDic)[uid]):Delete() -- DECOMPILER ERROR at PC14: Confused about usage of register: R8 in 'UnsetPending'
                ;
                (self.mailDataDic)[uid] = nil
                delete[uid] = true
            end
        ...
        if isFromTimer == true then self.m_outDataTimer = nil end
    end
...
New_MailController.RefrshMailRedDot = function(self)
    ...
    for uid, mailData in pairs(self.mailDataDic) do
        if mailData:GetState() == (MailEnum.eMailDetailType).None then
            ...
            if mailData:GetState() == (MailEnum.eMailDetailType).Read and
                mailData:IsHaveAtt() then
                mailNode:SetRedDotCount(1)
                return
            end
        end
