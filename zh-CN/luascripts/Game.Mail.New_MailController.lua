...
New_MailController.HasMailNotify = function(self)
    -- function num : 0_11 , upvalues : _ENV, HomeEnum, cs_MessageCommon
    self.hasNotify = true
    MsgCenter:Broadcast(eMsgEventId.NewNotice, (HomeEnum.eNoticeType).Mail,
...
New_MailController.InitOrUpdateData = function(self, mailDetails, isInit)
    ...
    for _, mailDetail in pairs(mailDetails) do
        if (self.mailDataDic)[mailDetail.uid] ~= nil then
            ...
            local mailData = (MailData.CreateMailData)(mailDetail) -- DECOMPILER ERROR at PC32: Confused about usage of register: R12 in 'UnsetPending'
            ;
            (self.mailDataDic)[mailDetail.uid] = mailData
            hasNew = true
        end
    ...
    if isInit then
        ...
        if latestTime ~= nil then
            MsgCenter:Broadcast(eMsgEventId.NewNotice,
                                (HomeEnum.eNoticeType).Mail, latestTime)
        end
    end
end
