...
bs_1028.PlaySkill = function(self, data)
  ...
  self.loop = ((self.caster).auSource):PlayAudioById((self.config).audioId)
end
...
bs_1028.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.BeforePlaySkill, "bs_1028_1", 1,
                    self.OnBeforePlaySkill)
    self:AddTrigger(eSkillTriggerType.BeforeBattleEnd, "bs_1028_2", 1,
...
bs_1028.RealPlaySkill = function(self)
    ...
    self:CallCasterWait(15)
    LuaSkillCtrl:CallRoleAction(self.caster, 1002, (self.config).speed)
    LuaSkillCtrl:CallEffectWithArgAndSpeed(self.caster, (self.config).effectId1,
                                           self, (self.config).speed, true,
                                           self.SkillEventFunc)
    self.aoeRestTime = self.aoeRestTime - 1
    if self.aoeRestTime == 0 then
        LuaSkillCtrl:StartTimer(nil, 15, function()
