...
local CreateWindowInternal = function(self, typeID, isAsync, replaceTypeId)
...
UIManager.ShowWindow = function(self, typeID, replaceType)
  ...
  local window = CreateWindowInternal(self, typeID, false, replaceType)
  window:Show()
  return window
end
...
UIManager.CreateWindowAsync = function(self, typeID, openCallback, replaceType)
  -- function num : 0_14 , upvalues : _ENV, cs_UIManager, CreateWindowInternal
  local target = self:GetWindow(typeID)
  if target ~= nil then
    openCallback(target)
    return 
  ...
  CreateWindowInternal(self, typeID, true, replaceType)
end

UIManager.ShowWindowAsync = function(self, typeID, openCallback, replaceType)
  -- function num : 0_15
  local target = self:GetWindow(typeID)
  if target then
    target:Show()
  ...
  self:CreateWindowAsync(typeID, function(window)
    -- function num : 0_15_0 , upvalues : openCallback
    if window ~= nil then
      window:Show()
    end
    if openCallback ~= nil then
...
UIManager.ClearAllAsyncLoad = function(self)
  -- function num : 0_19 , upvalues : _ENV, cs_UIManager
...
UIManager.PlayFov = function(self, fov, endValue, duration, delay)
    ...
    (self.UICamera).fieldOfView = fov;
    (((self.UICamera):DOFieldOfView(endValue, duration)):SetDelay(delay)):SetEase(
        (((CS.DG).Tweening).Ease).OutCubic)
end

UIManager.PlayDoFade = function(self, canvasGroup, alpha, endValue, duraction,
                                func)
    -- function num : 0_2
    canvasGroup.alpha = alpha
    local tween = canvasGroup:DOFade(endValue, duraction)
    if func ~= nil then tween.onComplete = func end
    return tween
end
...
UIManager.ctor = function(self)
    ...
    local referenceResolution =
        (cs_UIManager.UICanvasScaler).referenceResolution
    self.Resolution =
        (Vector2.New)(referenceResolution.x, referenceResolution.y)
    self.__update_handle = BindCallback(self, OnUpdate)
    UpdateManager:AddUpdate(self.__update_handle)
end
...
UIManager.World2UIPosition = function(self, position, rectTransform, uiCam,
                                      mainCam)
    -- function num : 0_5 , upvalues : cs_UIManager
    return
        cs_UIManager:World2UIPosition(position, rectTransform, uiCam, mainCam)
end
...
UIManager.Screen2WorldPosition = function(self, screenPoint, rectTransform,
                                          camera)
    -- function num : 0_7 , upvalues : cs_UIManager
    return cs_UIManager:Screen2WorldPosition(screenPoint, rectTransform, camera)
end
...
local InitWindowInternal = function(self, typeID, uiconfig, prefab)
    -- function num : 0_8 , upvalues : _ENV
    if IsNull(prefab) then
        ((self.resloaders)[typeID]):Put2Pool() -- DECOMPILER ERROR at PC10: Confused about usage of register: R4 in 'UnsetPending'
        ;
        (self.resloaders)[typeID] = nil
        error("Can\'t find window\'s prefab, window type is " ..
                  uiconfig.PrefabName)
        return nil
    end
    local obj = prefab:Instantiate((self.layers)[uiconfig.LayoutLevel])
    ...
    rectTransform.offsetMin = (Vector2.New)(0, 0)
    LocalizeManager:SetUILocalizeConfig(rectTransform)
    local window = ((uiconfig.WindowClass).New)() -- DECOMPILER ERROR at PC62: Confused about usage of register: R7 in 'UnsetPending'
    ;
    (self.windows)[typeID] = window;
    (UIUtil.LuaUIBindingTable)(obj, window.ui)
    window.__typeID = typeID
...
local CreateWindowInternal = function(self, typeID, isAsync, replaceTypeId)
    -- function num : 0_9 , upvalues : cs_ResLoader, _ENV, InitWindowInternal, cs_UIManager
    local resloader = (self.resloaders)[typeID]
    if not resloader then resloader = (cs_ResLoader.Create)() end
    local uiconfig = UIWindowGlobalConfig[typeID]
    if replaceTypeId ~= nil then
        uiconfig = UIWindowGlobalConfig[replaceTypeId]
    end
    assert(uiconfig)
    assert(not (string.IsNullOrEmpty)(uiconfig.PrefabName)) -- DECOMPILER ERROR at PC24: Confused about usage of register: R6 in 'UnsetPending'
    ;
    (self.resloaders)[typeID] = resloader
    if isAsync then
        local tmpId = typeID
        do
            resloader:LoadABAssetAsync(PathConsts:GetUIPrefabPath(
                                           uiconfig.PrefabName),
                                       function(prefab)
                -- function num : 0_9_0 , upvalues : InitWindowInternal, self, typeID, uiconfig, _ENV, tmpId, cs_UIManager
                local window =
                    InitWindowInternal(self, typeID, uiconfig, prefab)
                for k, cb in ipairs((self.async_loaders)[tmpId]) do
                    cb(window)
                end -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
        ...
        do
            local prefab = resloader:LoadABAsset(
                               PathConsts:GetUIPrefabPath(uiconfig.PrefabName))
            do
...
UIManager.CreateWindow = function(self, typeID, replaceType)
    ...
    if (self.resloaders)[typeID] ~= nil then return nil end
    return CreateWindowInternal(self, typeID, false, replaceType)
end
...
UIManager.ShowWindowAsync = function(self, typeID, openCallback, replaceType)
    ...
    if target then
        target:Show()
        if openCallback ~= nil then openCallback(target) end
        return target
    end
    if (self.resloaders)[typeID] ~= nil then return nil end
    self:CreateWindowAsync(typeID, function(window)
        -- function num : 0_15_0 , upvalues : openCallback
        if window ~= nil then window:Show() end
        if openCallback ~= nil then openCallback(window) end
    end, replaceType)
end
...
UIManager.DeleteWindow = function(self, typeID)
    ...
    if target ~= nil then
        (self.windows)[typeID] = nil
        if target.active then target:OnHide() end
        target:OnDelete()
    end
    local resloader = (self.resloaders)[typeID]
    -- DECOMPILER ERROR at PC19: Confused about usage of register: R4 in 'UnsetPending'
...
UIManager.DeleteAllWindow = function(self)
    -- function num : 0_18 , upvalues : _ENV
    for k, window in pairs(self.windows) do
        if not window.__permanent then self:DeleteWindow(k) end
    end
end
...
UIManager.CopyUI3DModifier = function(self, srcObj, destObj, useSimpleRotation)
    -- function num : 0_22 , upvalues : _ENV
    if not useSimpleRotation then useSimpleRotation = false end
    ((CS.UIManager).CopyUI3DModifier)(srcObj, destObj, useSimpleRotation)
end
