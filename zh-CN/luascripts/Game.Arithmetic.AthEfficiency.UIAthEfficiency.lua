...
local UINAthEfficiencyOneKeyFiltrate = require("Game.Arithmetic.AthEfficiency.UINAthEfficiencyOneKeyFiltrate")
...
UIAthEfficiency.__OnClickAutoSelect = function(self)
  ...
  if self.siftCondition == nil then
    ...
    ;
    (self.siftCondition):InitAthEfccOneKeyFilt(BindCallback(self, self.OnFilterConfirmAction))
  end
  ;
    ...
    do
      if self.ableAddExp > 0 then
        ...
        for k,athData in ipairs(tempList) do
          ...
          if ((self._onekeyFiltData)[qualityID] or noSelectAny) and not athData.lockUnlock then
