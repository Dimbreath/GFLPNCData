...
local UIFactoryHeroListCover = class("UIFactoryHeroListCover", UIBaseWindow)
local base = UIBaseWindow
local UINFactoryHeroList = require(
                               "Game.Factory.UI.FactoryHeroList.UINFactoryHeroList")
local cs_MessageCommon = CS.MessageCommon
UIFactoryHeroListCover.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINFactoryHeroList
    ...
    (UIUtil.CreateTopBtnGroup)((self.ui).topButtonGroup, self,
                               self.OnReturnClicked, true)
    self.factoryHeroList = (UINFactoryHeroList.New)();
    (self.factoryHeroList):Init((self.ui).uI_HeroList);
    (self.factoryHeroList):SetCallback(
        BindCallback(self, self.UpdateEnergyAddUI),
        BindCallback(self, self.OnConfirm))
    self.baseAccRate = 0
end
...
UIFactoryHeroListCover.UpdateEnergyAddUI =
    function(self, heroIdList, isInit)
        -- function num : 0_2 , upvalues : _ENV
        local accRate = (self.factoryController):GetHeroEnterAccrate(
                            self.roomIndex, heroIdList)
        local baseSpeed =
            (self.factoryController):GetRoomEnegeyBaseSpeedByIndex(
                self.roomIndex)
        if #heroIdList > 0 then
            ((self.ui).tex_TipsContent):SetIndex(0, GetPreciseDecimalStr(
                                                     baseSpeed * 3600, 1),
            ...
            ((self.ui).tex_TipsContent):SetIndex(1, GetPreciseDecimalStr(
                                                     baseSpeed * 3600, 1))
        end
    end
...
UIFactoryHeroListCover.OnConfirm = function(self)
    ...
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(
                                           TipContent.Factory_HeroEnterSuccess))
    self:OnReturnClicked();
    (UIUtil.PopFromBackStack)(self.clickBackFunc)
end

UIFactoryHeroListCover.OnReturnClicked =
    function(self)
        -- function num : 0_4
