...
bs_304.OnAttackTrigger = function(self, data)
    ...
    if targetListall.Count ~= 0 then
        for i = 0, targetListall.Count - 1 do
            ...
            if skills ~= nil and ((self.caster).recordTable)["304_T"] > 0 and
                skills.Count > 0 then
                for j = 0, skills.Count - 1 do
                    if (skills[j]).isCommonAttack ~= true and
                        (skills[j]).useLimitTime == 0 then
                        ...
                        LuaSkillCtrl:CallResetCDForSingleSkill(skills[j],
                                                               curTotalCd - 5) -- DECOMPILER ERROR at PC64: Confused about usage of register: R13 in 'UnsetPending'
                        ;
                        ((self.caster).recordTable)["304_T"] =
                            ((self.caster).recordTable)["304_T"] - 1
                        if (self.arglist)[1] >= 0 then
                            LuaSkillCtrl:CallBuff(self,
                                                  (targetListall[i]).targetRole,
                                                  (self.config).buffId, 1, 75)
                        end
                        break
                    end
                end
