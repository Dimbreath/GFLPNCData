...
local UINRandomBeforeBattlePassItem = require(
                                          "Game.Battle.UI.UINRandomBeforeBattlePassItem")
local UINEventRoomRandomOptionButton = require(
                                           "Game.Exploration.UI.EventRoom.UINEventRoomRandomOptionButton")
local UINEventRoomRandomResult = require(
                                     "Game.Exploration.UI.EventRoom.UINEventRoomRandomResult")
...
UINEventRoomRandom.OnInit = function(self)
    ...
    self.resultList = {}
    self.focusItemNum = 0;
    (UIUtil.AddButtonListener)((self.ui).btn_selectButton, self,
                               self.OnClickSelect);
    (UIUtil.AddButtonListener)((self.ui).btn_rejudgeButton, self,
                               self.OnClickRejudge);
    (UIUtil.AddButtonListener)((self.ui).btn_acquireButton, self,
                               self.OnClickCollectReward)
    self.resultItemPool = (UIItemPool.New)(UINEventRoomRandomResult,
                                           (self.ui).obj_resultItem);
    ((self.ui).obj_resultItem):SetActive(false)
    self.optionButtonItemPool = (UIItemPool.New)(UINEventRoomRandomOptionButton,
                                                 (self.ui).obj_optionButton);
    ((self.ui).obj_optionButton):SetActive(false)
    self.randomItemPool = (UIItemPool.New)(UINRandomBeforeBattlePassItem,
                                           (self.ui).obj_randomItem);
    ...
    MsgCenter:AddListener(eMsgEventId.EpFocusPointChange,
                          self.__refreshFocusPoint);
    ((ExplorationManager.epCtrl).eventCtrl):SetRejudegeOverCallback(
        function(judgeValue)
...
UINEventRoomRandom.InitRandomEvent = function(self, datas, dynPlayData,
                                              onClickActionNormal)
    ...
    end -- DECOMPILER ERROR at PC16: Confused about usage of register: R6 in 'UnsetPending'
...
UINEventRoomRandom.__InitAfterSelect = function(self, datas)
    ...
    (self.optionButtonItemPool):HideAll()
    for index, choiceData in ipairs(datas.choiceDatalist) do
        if choiceData.isRandom == true and choiceData.choiceId ==
            datas.selectedRandomId then
            local item = (self.optionButtonItemPool):GetOne(true)
            cfg = (ConfigData.event_random)[choiceData.choiceId]
...
UINEventRoomRandom.NormalOptionClick = function(self, cfg, idx)
    ...
    (self.resultItemPool):HideAll()
    self.passNum = 0;
    (((self.ui).txt_NoremalChoice).gameObject):SetActive(true) -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).txt_NoremalChoice).text =
        (LanguageUtil.GetLocaleText)(cfg.choice_describe_txt)
    self.selectedCfg = cfg
    self.selectedInedx = idx
end
...
UINEventRoomRandom.OnClickCollectReward =
    function(self)
        ...
        ((ExplorationManager.epCtrl).eventCtrl):SendCollectReward(des,
                                                                  couldContinue)
    end

UINEventRoomRandom.RefreshDifferentResultProbability =
    function(self, cfg, isRejudege)
        -- function num : 0_9 , upvalues : _ENV
        local n = cfg.judge_num;
        (self.resultItemPool):HideAll()
        local baseRate =
            (ConfigData.game_config).RandomBeforeBatteleProbabilityBase or 0.6
        local eachRate =
            (ConfigData.game_config).RandomBeforeBatteleProbabilityEachOne or
                0.06
        local careeNum = 0
        ...
        for i = 1, n + 1 do
            ...
            if passNum < definitePassNum then
                ...
                probability = self:_combination(remainPssNum, wantPassNum) *
                                  (Mathf.Pow)(successRate, wantPassNum) *
                                  (Mathf.Pow)(1 - successRate,
                                              remainPssNum - wantPassNum)
            end
            do
                ...
                do
                    ...
                    item:InitRandomResult(passNum, probability, logic, para1,
                                          para2, jumpTarget) -- DECOMPILER ERROR at PC90: Confused about usage of register: R20 in 'UnsetPending'
                    ;
...
UINEventRoomRandom._factorial = function(self, n)
    ...
    local temp = 1
    for i = 1, n do temp = temp * i end
    return temp
end

UINEventRoomRandom.__RefreshRandomItem =
    function(self, career, judgeResult)
        ...
        for index, judgeNum in ipairs(judgeResult) do
            ...
            if judgeNum == 2 then item:LuckPass() end
            (table.insert)(self.randomItemList, item)
            if not isPass then
                self.isAllpass = false
            else
...
UINEventRoomRandom.RefreshFocusPoint = function(self, focusItemNum, focusLimit)
    ...
    ((self.ui).txt_focusPiont):SetIndex(0, tostring(focusItemNum),
                                        tostring(focusLimit))
end

UINEventRoomRandom.OnDelete = function(self)
    -- function num : 0_15 , upvalues : _ENV, base
    MsgCenter:RemoveListener(eMsgEventId.EpFocusPointChange,
                             self.__refreshFocusPoint);
    (base.OnDelete)(self)
end
