...
local base = UIBaseNode
local UINHeroEnterSlotItem = require("Game.HeroEnter.UINHeroEnterSlotItem")
local UINHeroEnterOverViewBuffItem = require(
                                         "Game.HeroEnter.UINHeroEnterOverViewBuffItem")
local UINHeroEnterOverViewSkillitem = require(
                                          "Game.HeroEnter.UINHeroEnterOverViewSkillitem")
local HeroEnterDataUtil = require("Game.HeroEnter.HeroEnterDataUtil")
UINHeroEnterOverViewItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINHeroEnterSlotItem, UINHeroEnterOverViewBuffItem, UINHeroEnterOverViewSkillitem
    self.slotList = {};
    ...
    self.BuffItemPool = (UIItemPool.New)(UINHeroEnterOverViewBuffItem,
                                         (self.ui).tex_BuildAdd)
    self.BuffItemList = {}
    self.SkillItemPool = (UIItemPool.New)(UINHeroEnterOverViewSkillitem,
...
UINHeroEnterOverViewItem.InitOverViewItem =
    function(self, bulindingId, resloader)
        -- function num : 0_1 , upvalues : _ENV
        self.bulindingId = bulindingId
        local buildingCfg = (ConfigData.building)[bulindingId] -- DECOMPILER ERROR at PC10: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_BuildName).text =
            (LanguageUtil.GetLocaleText)(buildingCfg.name) -- DECOMPILER ERROR at PC23: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_CampIcon).sprite =
            CRH:GetSprite(((ConfigData.camp)[buildingCfg.camp_id]).icon,
                          CommonAtlasType.CareerCamp)
        self.__resloader = resloader
        self.bulidingLevelCfg =
            ((ConfigData.buildingLevel)[bulindingId])[(((PlayerDataCenter.AllBuildingData).oasisBuilt)[bulindingId]).level]
        for i = 1, #(self.bulidingLevelCfg).buff_logic do
            local buffItem = (self.BuffItemPool):GetOne(true)
            buffItem:InitBuff(self.bulidingLevelCfg, i);
            (table.insert)(self.BuffItemList, buffItem)
...
UINHeroEnterOverViewItem.RefreshInfo = function(self)
    ...
    local allEnterData = PlayerDataCenter.allEnterData
    local level =
        (((PlayerDataCenter.AllBuildingData).built)[buildingCfg.build_id]).level
    local UnlockedSlotNum =
        (((((PlayerDataCenter.AllBuildingData).built)[buildingCfg.build_id]).levelConfig)[level]).hero_job_slot
    local enteredNum = 0
    local heroScoreRate = 0
    local heroIDList = {}
    for i = 1, UnlockedSlotNum do
            ...
            do
                ...
                if allEnterData[buildingCfg.build_id] ~= nil then
                    local heros = (table.keys)(
                                      (allEnterData[buildingCfg.build_id]).data)
                    heroScoreRate = (math.max)(heroScoreRate,
    ...
    for _, Item in ipairs(self.BuffItemList) do Item:SetValue(heroScoreRate) end
    self:RefreshLifeSkill(heroIDList);
    ((self.ui).tex_StationedNum):SetIndex(0, tostring(enteredNum),
                                          tostring(UnlockedSlotNum))
end

UINHeroEnterOverViewItem.RefreshLifeSkill =
    function(self, heroIDList)
        -- function num : 0_3 , upvalues : _ENV
        (self.SkillItemPool):HideAll()
        local lifeSkillData = nil
        local isHeroHaveLifeSkill = function(skillDic)
