...
bs_10159.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10159_3", 1,
                    self.OnAfterHurt)
end

bs_10159.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and not isMiss and LuaSkillCtrl:CallRange(1, 1000) <
        (self.arglist)[3] then
        ...
        if layer ~= (self.arglist)[2] then
            if layer + (self.arglist)[1] <= (self.arglist)[2] then
                ...
                if (self.arglist)[2] < layer + (self.arglist)[1] then
                    ...
                    LuaSkillCtrl:CallBuff(self, self.caster,
                                          (self.config).buffId,
                                          (self.arglist)[2])
                end
            end
