...
UINLevelChipQualityItem.InitChipQualityItem =
    function(self, quality, chipDataList, OnChipClickCallback)
        -- function num : 0_1 , upvalues : _ENV
        self.OnChipClickCallback = OnChipClickCallback -- DECOMPILER ERROR at PC5: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_tileLine).color = ItemQualityColor[quality] -- DECOMPILER ERROR at PC10: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_Quailty).color = ItemQualityColor[quality];
        ((self.ui).texInfo_Quailty):SetIndex(quality - 1);
        (self.chipItemPool):HideAll()
        for _, chipData in ipairs(chipDataList) do
            local chipItem = (self.chipItemPool):GetOne()
            chipItem:InitChipItem(chipData, false, self._OnChipClick) -- DECOMPILER ERROR at PC39: Confused about usage of register: R10 in 'UnsetPending'
            ;
            (chipItem.ui).img_lock = ((chipItem.transform):FindComponent(
                                         "Img_isLock", eUnityComponentID.Image)).gameObject;
            ((chipItem.ui).img_lock):SetActive(chipData.isLock)
        end
    end
...
UINLevelChipQualityItem.OnChipClick = function(self, chipData, chipItem)
    ...
    if chipData.isLock then
        ((CS.MessageCommon).ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.exploration_Level_Chip_Unlock));
        (self.OnChipClickCallback)()
        return
    end
    if self.OnChipClickCallback ~= nil then
