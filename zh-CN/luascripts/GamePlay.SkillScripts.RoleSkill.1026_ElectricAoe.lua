...
, 
hurt_config1 = {basehurt_formula = 10076}
, 
hurt_config2 = {basehurt_formula = 9988}
}
...
bs_1026.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_1026_3", 1,
                    self.OnAfterHurt)
end

bs_1026.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                               isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack then
        ...
        if (skillResult.roleList).Count ~= 0 then
            local prob = LuaSkillCtrl:CallRange(0,
                                                (skillResult.roleList).Count - 1)
            for i = 0, (skillResult.roleList).Count - 1 do
                local role = (skillResult.roleList)[i]
                if i == prob then
                    if (self.arglist)[2] == 1 then
                        LuaSkillCtrl:CallEffectWithArgOverride(role, 10009,
                                                               self, target,
                                                               true,
                                                               self.SkillEventFunc,
                                                               (self.config).hurt_config2)
                    else
                        LuaSkillCtrl:CallEffectWithArgOverride(role, 10009,
                                                               self, target,
                                                               true,
                                                               self.SkillEventFunc,
                                                               (self.config).hurt_config1)
                    end
                else
                    LuaSkillCtrl:CallEffectWithArgOverride(role, 10009, self,
                                                           target, true,
                                                           self.SkillEventFunc,
                                                           (self.config).hurt_config1)
                end
