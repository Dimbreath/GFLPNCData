...
UINAthSortListAll.OnInit = function(self)
    ...
    ((self.ui).scrollRect).OnDragStartHorizontal =
        BindCallback(self, self.__OnDragStartHorizontal);
    ((self.ui).obj_HasSuit):SetActive(false)
    self.athItemPoolList = {}
    for k, go in ipairs((self.ui).athSpaceItemList) do
        (go.transform):SetParent((self.ui).prefabHolder) -- DECOMPILER ERROR at PC35: Confused about usage of register: R6 in 'UnsetPending'
        ;
    ...
    for k, go in ipairs((self.ui).athSpaceGroupList) do
        (go.transform):SetParent((self.ui).prefabHolder) -- DECOMPILER ERROR at PC56: Confused about usage of register: R6 in 'UnsetPending'
        ;
        (self.athGroupPoolList)[k] =
            (UIItemPool.New)(UINAthSortListAllGroup, go)
    end
    self.athHasSuitPool = (UIItemPool.New)(UINAthHasSuitItem,
                                           (self.ui).obj_HasSuit)
end

UINAthSortListAll.InitAthSortListAll = function(self, areaId, clickItemFunc,
                                                resLoader, ignoreInstalled)
    -- function num : 0_1
...
UINAthSortListAll.__RefillList = function(self)
    ...
    for k, pool in ipairs(self.athItemPoolList) do pool:HideAll() end
    for k, pool in ipairs(self.athGroupPoolList) do pool:HideAll() end
    self.athItemGoDic = {}
    for k1, dataGroup in ipairs(self.curAthDataGroupList) do
        ...
        (groupItem.transform):SetParent(((self.ui).scrollRect).content) -- DECOMPILER ERROR at PC46: Confused about usage of register: R10 in 'UnsetPending'
        ;
        ...
        for k2, athData in ipairs(dataGroup) do
            ...
            (athItem.transform):SetParent(groupItem.transform) -- DECOMPILER ERROR at PC63: Confused about usage of register: R17 in 'UnsetPending'
            ;
            ...
            if self.athMultSeletedUidDic ~= nil and
                (self.athMultSeletedUidDic)[athData.uid] ~= nil then
                athItem:SetAthItemSelect(true, true)
            end
            local suitId = athData:GetAthSuit()
            do
                do
                    if self.curAthSuitDic ~= nil and
                        (self.curAthSuitDic)[suitId] then
                        local hasSuitItem = (self.athHasSuitPool):GetOne()
                        hasSuitItem:InitAthHasSuitItem(athItem.transform,
                                                       (self.ui).hasSuitHolder)
                    end -- DECOMPILER ERROR at PC110: Confused about usage of register: R18 in 'UnsetPending'
    ...
    (CS_LayoutRebuilder.ForceRebuildLayoutImmediate)(
        ((self.ui).scrollRect).content)
end
...
UINAthSortListAll.OnDelete = function(self)
    ...
    for k, pool in ipairs(self.athGroupPoolList) do pool:DeleteAll() end
