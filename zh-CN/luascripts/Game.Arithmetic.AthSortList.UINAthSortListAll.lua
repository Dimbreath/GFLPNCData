...
UINAthSortListAll.OnAthItemClick = function(self, athItem)
  ...
  ;
  ((self.ui).scrollRect):SrollToCell(rollIndex - 1, 2500, function()
...
local base = UIBaseNode
local UINAthListItem = require("Game.Arithmetic.AthSortList.UINAthListItem")
local UINAthSortListAllGroup = require(
                                   "Game.Arithmetic.AthSortList.UINAthSortListAllGroup")
local UINAthHasSuitItem = require(
                              "Game.Arithmetic.AthSortList.UINAthHasSuitItem")
local UINAthMatUpgrade = require("Game.Arithmetic.AthSortList.UINAthMatUpgrade")
local AthTableUtil = require("Game.Arithmetic.AthMain.Table.AthTableUtil")
local CS_EventSystem = ((CS.UnityEngine).EventSystems).EventSystem
local CS_LayoutRebuilder = ((CS.UnityEngine).UI).LayoutRebuilder
...
UINAthSortListAll.OnInit = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui)
    self.__onReturnAthItem = BindCallback(self, self.ReturnAthItem)
    self.__onReturnAthMatNode = BindCallback(self, self.ReturnAthMatNode) -- DECOMPILER ERROR at PC21: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).OnDragStartHorizontal =
        BindCallback(self, self.__OnDragStartHorizontal) -- DECOMPILER ERROR at PC28: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC35: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).onChangeItem =
        BindCallback(self, self.__OnChangeItem) -- DECOMPILER ERROR at PC42: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ...
    self.athItemGoDic = {}
    self.athGroupItemDic = {}
    self.athHasSuitPool = (UIItemPool.New)(UINAthHasSuitItem,
                                           (self.ui).obj_HasSuit)
    self.__onAthItemClick = BindCallback(self, self.OnAthItemClick)
end

UINAthSortListAll.InitAthSortListAll = function(self, areaId, quality,
                                                clickItemFunc,
                                                itemStartDragFunc, resLoader,
                                                withMat)
    -- function num : 0_1 , upvalues : UINAthMatUpgrade
    self.resLoader = resLoader
    self.areaId = areaId
...
UINAthSortListAll.RefreshAthSortListData =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        local ignoreInstalled = self.withMat
        self.oriAthDataList = nil
        if self.areaId == nil then
            self.oriAthDataList = (PlayerDataCenter.allAthData):GetAllAthList(
                                      nil, ignoreInstalled, self.quality)
        else
            self.oriAthDataList =
                (PlayerDataCenter.allAthData):GetAllAthSlotList(self.areaId,
                                                                nil,
                                                                ignoreInstalled)
        end
    end

UINAthSortListAll.RefillAthSortListAll =
    function(self, funcSift, funcSort, athSuitDic, useLastPos, curFreeSpace)
        -- function num : 0_4 , upvalues : _ENV
        if funcSift ~= nil then self.funcSift = funcSift end
        if funcSort ~= nil then self.funcSort = funcSort end
        self.curAthSuitDic = athSuitDic
        self.curAthDataList = {}
        for index, athData in ipairs(self.oriAthDataList) do
            if self.funcSift == nil or (self.funcSift)(athData) then
        ...
        for k, athData in ipairs(self.curAthDataList) do
            ...
            local size = athData:GetAthSize()
            local forceNewLine = false
            if not freeSpaceNewLine and curFreeSpace ~= nil and curFreeSpace <
                size then
                if #tempGroup ~= 0 then forceNewLine = true end
                freeSpaceNewLine = true
            end
            if forceNewLine or
                not self:__Insert2TempGroup(tempGroupIdList, tempGroup, athData) then
                tempGroup = {allSize = 0}
                tempGroupIdList = {};
                (table.insert)(self.curAthDataGroupList, tempGroup)
                self:__Insert2TempGroup(tempGroupIdList, tempGroup, athData)
...
UINAthSortListAll.__Insert2TempGroup = function(self, tempGroupIdList,
                                                tempGroup, athData)
    -- function num : 0_5 , upvalues : groupMaxCount, AthTableUtil, groupXMax, _ENV
    local athSize = athData:GetAthSize()
    for i = 1, groupMaxCount do
        if (athSize ~= 4 and athSize ~= 8) or i % 2 == 1 then
            local gridIdList = (AthTableUtil.GetAthGridIdList)(athData, i,
                                                               groupXMax)
            local canInsert = true
            for k, gridId in ipairs(gridIdList) do
                if groupMaxCount < gridId or tempGroupIdList[gridId] == true then
            ...
            do
                do
                    if canInsert then
                        ...
                        tempGroup.allSize = tempGroup.allSize + athSize;
                        (table.insert)(tempGroup,
                                       {gridId = i, athData = athData})
                        return true
                    end
                    -- DECOMPILER ERROR at PC57: LeaveBlock: unexpected jumping out DO_STMT
...
UINAthSortListAll.__RefillList = function(self, useLastPos)
    -- function num : 0_6
    local totalCount = #self.curAthDataGroupList
    if self.withMat then totalCount = totalCount + 1 end
    if ((self.ui).scrollRect).totalCount == 0 then useLastPos = false end -- DECOMPILER ERROR at PC14: Confused about usage of register: R3 in 'UnsetPending'
...
UINAthSortListAll.SetAthListMultSeletedUidDic =
    function(self, dic)
        -- function num : 0_7
        self.athMultSeletedUidDic = dic
    end

UINAthSortListAll.__OnDragStartHorizontal =
    function(self)
        -- function num : 0_8 , upvalues : CS_EventSystem
        local curGo = (CS_EventSystem.current).currentSelectedGameObject
        local item = (self.athItemGoDic)[curGo]
...
UINAthSortListAll.GetSortListAthMatUpNode =
    function(self)
        -- function num : 0_9
        return self.athMatUpNode
    end
...
UINAthSortListAll.__OnNewItem = function(self, go)
    ...
    local item = (UINAthSortListAllGroup.New)()
    item:Init(go)
    item:InitAthSortListAllGroup(self.__onReturnAthItem,
                                 self.__onReturnAthMatNode) -- DECOMPILER ERROR at PC10: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.athGroupItemDic)[go] = item
end
...
UINAthSortListAll.__OnChangeItem = function(self, go, index)
    ...
    do
        for k, v in ipairs(groupData) do
            local athSize = (v.athData):GetAthSize()
            local itemPool = (self.athItemPoolList)[spaceIdDic[athSize]]
            local athItem = itemPool:GetOne() -- DECOMPILER ERROR at PC72: Confused about usage of register: R13 in 'UnsetPending'
            ;
            (athItem.gameObject).name = tostring(R15_PC71)
            if self.newItemFunc ~= nil then
                (self.newItemFunc)(athItem)
            ...
            athItem:InitAthListItem(R15_PC71, self.__onAthItemClick,
                                    self.resLoader)
            local selected = self.athMultSeletedUidDic ~= nil and
                                 (self.athMultSeletedUidDic)[(v.athData).uid] ~=
                                 nil
            -- DECOMPILER ERROR at PC105: Overwrote pending register: R15 in 'AssignReg'
            ...
            if R15_PC71 ~= nil and R15_PC71 then
                R15_PC71 = R15_PC71(R15_PC71)
                local hasSuitItem = nil
                hasSuitItem:InitAthHasSuitItem(athItem.transform,
                                               (self.ui).hasSuitHolder)
                athItem:SetAthItemSuit(hasSuitItem)
            end -- DECOMPILER ERROR at PC132: Confused about usage of register: R15 in 'UnsetPending'
...
UINAthSortListAll.GetAthItemFromListAll =
    function(self, uid)
        -- function num : 0_15
        local index = self:__GetAthDataGroupIndexByUid(uid)
        local go = ((self.ui).scrollRect):GetCellByIndex(index - 1)
...
UINAthSortListAll.__GetAthDataGroupIndexByUid =
    function(self, uid)
        -- function num : 0_16 , upvalues : _ENV
        for index, v in ipairs(self.curAthDataGroupList) do
            for k, v2 in ipairs(v) do
                if (v2.athData).uid == uid then return index end
            end
        end
    end
...
UINAthSortListAll.OnDelete = function(self)
    -- function num : 0_18 , upvalues : _ENV, base
    for k, pool in ipairs(self.athItemPoolList) do pool:DeleteAll() end
    if self.athMatUpNode ~= nil then (self.athMatUpNode):Delete() end
    (self.athHasSuitPool):DeleteAll();
    (base.OnDelete)(self)
end
