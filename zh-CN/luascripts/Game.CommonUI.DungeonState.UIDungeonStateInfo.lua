...
local CS_ResLoader = CS.ResLoader
local ExplorationEnum = require("Game.Exploration.ExplorationEnum")
local UINDungeonChipList = require(
                               "Game.CommonUI.DungeonState.UINDungeonChipList")
local UINDungeonHeroList = require(
                               "Game.CommonUI.DungeonState.UINDungeonHeroList")
local UINDungeonBuffList = require(
                               "Game.CommonUI.DungeonState.UINDungeonBuffList")
local UINChipItem = require("Game.CommonUI.Item.UINChipItem")
local CS_DOTween = ((CS.DG).Tweening).DOTween
local ScrambleMode = ((CS.DG).Tweening).ScrambleMode
UIDungeonStateInfo.OnInit = function(self)
    ...
    self.__onSaveMoneyChange = BindCallback(self, self.RefreshSaveMoney)
    MsgCenter:AddListener(eMsgEventId.EpMoneyChange, self.__onMoneyChange)
    MsgCenter:AddListener(eMsgEventId.EpSaveMoneyChange,
                          self.__onSaveMoneyChange)
    self.__DungeonHeroListActiveSet = BindCallback(self,
                                                   self.DungeonHeroListActiveSet)
    MsgCenter:AddListener(eMsgEventId.DungeonHeroListActiveSet,
                          self.__DungeonHeroListActiveSet)
end

UIDungeonStateInfo.InitHeroAndChip = function(self, dynPlayer)
    -- function num : 0_1
    (self.heroList):InitHeroList(dynPlayer.heroList, self.resloader,
                                 (self.chipList).selectHeroCallback,
                                 dynPlayer:GetEpBuffList());
    (self.chipList):InitChipList(dynPlayer:GetChipList(),
                                 (self.heroList).selectWithChipIdCallback,
                                 dynPlayer:GetChipLimitInfo());
    (self.buffList):InitBuffList(dynPlayer:GetEpBuffList())
    self:RefreshMoney(true)
end

UIDungeonStateInfo.DungeonHeroListActiveSet =
...
UIDungeonStateInfo.SetHeroListActive = function(self, isActive)
    -- function num : 0_3
    if isActive then (self.heroList):SetHpBarActive(true) end
    (self.heroList):SetDeployStation(not isActive)
end
...
UIDungeonStateInfo.ShowHero = function(self, hero)
    ...
    for index, value in ipairs(((self.heroList).itemPool).listItem) do
        if (value.dynHeroData).dataId == hero.dataId then
            (self.heroList):OnHeroClick(
                (((self.heroList).itemPool).listItem)[index])
            break
        end
    end
end
...
UIDungeonStateInfo.RefreshMoney = function(self, isInit)
    ...
    if ExplorationManager ~= nil then
        ...
        if dynplayer ~= nil then
            local theMoney = dynplayer:GetMoneyCount()
            if theMoney ~= nil then money = theMoney end
        end
    end
    do
        if self.texMoneyTween ~= nil then (self.texMoneyTween):Kill(true) end
        self.texMoneyTween = ((self.ui).tex_Money):DOText(tostring(money), 1,
                                                          true,
        ...
        if self.LoadMoneyIcon == nil and dynplayer ~= nil then
            local icon = dynplayer:GetMoneyIconId() -- DECOMPILER ERROR at PC56: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).img_Money).sprite = CRH:GetSprite(icon)
            self.LoadMoneyIcon = true
        end
...
UIDungeonStateInfo.SetSaveMoneyActive = function(self, active)
    -- function num : 0_9
    ((self.ui).obj_saveMoney):SetActive(active)
    if active then self:RefreshSaveMoney() end
end
...
UIDungeonStateInfo.RefreshSaveMoney = function(self)
    ...
    do
        if self.dynplayer ~= nil then
            local theMoney = (self.dynplayer):GetEpSaveMoney()
            if theMoney ~= nil then money = theMoney end
        end -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
        
        ((self.ui).tex_SaveMoney).text = (math.floor)(money);
        ((self.ui).obj_saveMoney):SetActive(
            (money ~= 0 and ((self.ui).obj_money).activeSelf))
        -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end
end
...
UIDungeonStateInfo.Hide = function(self, withTween)
    ...
    (base.Hide)(self)
    local win = UIManager:GetWindow(UIWindowTypeID.DungeonInfoDetail)
    if win ~= nil then win:OnClickRetreat() end
    if withTween then
        self:__ClearFadeTween()
        self.__fadeTween =
...
UIDungeonStateInfo.ShowGetChipAni = function(self, chipData, position, scale)
    -- function num : 0_15 , upvalues : _ENV
    if not ((self.chipList).gameObject).activeInHierarchy then return end
    (table.insert)(self.getChipAniCache,
                   {chipData = chipData, position = position, scale = scale})
    if not self.__startGetChipAni then self:__StartShowGetChipAni() end
end

UIDungeonStateInfo.__StartShowGetChipAni =
    function(self)
        -- function num : 0_16 , upvalues : _ENV, CS_DOTween
        if #self.getChipAniCache == 0 then
            self.__startGetChipAni = false;
        ...
        self.__startGetChipAni = true
        local getChipData = (table.remove)(self.getChipAniCache, 1)
        local chipData, position, scale = getChipData.chipData,
                                          getChipData.position,
                                          getChipData.scale
        if not (self.chipList):ListContainChip(chipData.dataId) then
            self:__StartShowGetChipAni()
        end
        (self.chipList):ScrollChipItemInScreen(chipData, function(chipItem)
            ...
            if chipItem ~= nil then
                (self.chipList):SetChipGetEffectEnable(true)
                local destPosition = (self.transform):InverseTransformPoint(
                                         chipItem:GetChipItemWorldPos())
                do
                    local chipItem = (self.getChipPool):GetOne()
                    chipItem:InitChipItem(chipData)
                    chipItem:Show() -- DECOMPILER ERROR at PC21: Confused about usage of register: R3 in 'UnsetPending'
                    ;
                    (chipItem.transform).localPosition = position
                    local curScale = scale or self.getChipOriginScale -- DECOMPILER ERROR at PC27: Confused about usage of register: R4 in 'UnsetPending'
                    ;
                    (chipItem.transform).localScale = curScale;
                    (((self.ui).eff_GetItem).gameObject):SetActive(true) -- DECOMPILER ERROR at PC37: Confused about usage of register: R4 in 'UnsetPending'
                    ;
                    (((self.ui).eff_GetItem).transform).localPosition =
                        destPosition;
                    (((self.ui).eff_GetItem).material):SetFloat("_UI_Player", 0)
                    local sequence = (CS_DOTween.Sequence)()
                    sequence:Append((chipItem.transform):DOLocalMove(
                                        destPosition,
                                        (ConfigData.game_config).getChipMoveTime))
                    sequence:Join((chipItem.transform):DOScale(
                                      self.getChipOriginScale,
                                      (ConfigData.game_config).getChipMoveTime))
                    ...
                    sequence:Append((((self.ui).eff_GetItem).material):DOFloat(
                                        1, "_UI_Player",
                                        (ConfigData.game_config).getChipEffectTime))
                    sequence:AppendCallback(
                        function()
                            -- function num : 0_16_0_1 , upvalues : self
                            (self.chipList):SetChipGetEffectEnable(false)
                ...
                do self:__StartShowGetChipAni() end
            end
        end)
    end

UIDungeonStateInfo.ChipListWeakenTween =
    function(self, weaken)
        -- function num : 0_17
        if weaken then
            ((self.ui).topWeakenTween):DORestart()
        else
...
UIDungeonStateInfo.CheckChipListBgShow =
    function(self, check)
        -- function num : 0_19
        (self.chipList):CheckChipListBgShow(check)
    end
...
UIDungeonStateInfo.OnDelete = function(self)
    ...
    (self.buffList):Delete()
    MsgCenter:RemoveListener(eMsgEventId.EpMoneyChange, self.__onMoneyChange)
    MsgCenter:RemoveListener(eMsgEventId.DungeonHeroListActiveSet,
                             self.__DungeonHeroListActiveSet);
    (base.OnDelete)(self)
end
