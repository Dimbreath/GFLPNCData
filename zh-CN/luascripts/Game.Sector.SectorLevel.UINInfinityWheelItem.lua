...
UINInfinityWheelItem.InitWheel = function(self, listCount, viewSizeY, elementSizeY)
  -- function num : 0_1
  self.lastDegree = 0
  self.lastAudioDegree = 0
  self.listCount = listCount
  if not viewSizeY then
    viewSizeY = 1080
  end
  if not elementSizeY then
    elementSizeY = 207
  end
...
UINInfinityWheelItem.RoteWheel = function(self, value)
  ...
  do
    ...
    if value >= 0 and value <= 1 and (math.abs)(degree - self.lastAudioDegree) > 30 then
...
UINInfinityWheelItem.RoteWheel = function(self, value)
    ...
    do
        ...
        if value >= 0 and value <= 1 and
            (math.abs)(degree - self.lastAudioDegree) > 30 then
            self.lastAudioDegree = degree
            AudioManager:PlayAudioById(1037)
        end
    end
