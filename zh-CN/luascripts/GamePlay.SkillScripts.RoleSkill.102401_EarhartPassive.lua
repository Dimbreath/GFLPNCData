...
bs_102401.config = {buffId = 236, buffId2 = 235, effectId = 10738}
...
bs_102401.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddSelfTrigger(eSkillTriggerType.AfterHurt, "bs_102401_3", 1, self.OnAfterHurt)
end

bs_102401.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg, isTriggerSet)
  -- function num : 0_2 , upvalues : _ENV
  if sender == self.caster and isCrit and sender:GetBuffTier((self.config).buffId) == 0 and skill.isCommonAttack and (self.arglist)[2] > 0 then
  ...
  if sender == self.caster and isCrit and skill.isCommonAttack then
    LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId2, 1, (self.arglist)[4], true)
  end
end
