...
UINFactoryLine.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btnSubtract, self,
                               self.OnBtnSubLineClicked);
...
UINFactoryLine.InitLineFormula = function(self, factoryCfg, isUnlock, uiParent)
    ...
    if not self.isUnlock then
        ...
        ((self.ui).tex_Fail):SetIndex(0,
                                      (CheckCondition.GetUnlockInfoLua)(
                                          factoryCfg.pre_condition,
                                          factoryCfg.pre_para1,
                                          factoryCfg.pre_para2))
    end -- DECOMPILER ERROR at PC34: Confused about usage of register: R4 in 'UnsetPending'
...
UINFactoryLine.InitConsume = function(self)
    ...
    for k, consumeId in ipairs((self.factoryCfg).consumeIds) do
        local consumeNum = ((self.factoryCfg).consumeNums)[k] *
                               (self.factoryCfg).cycle
...
UINFactoryLine.SetHighlightItem = function(self, itemId)
    ...
    for k, consumeItem in ipairs(self.consumeItems) do
        consumeItem:SetHightlightActive(itemId ~= nil and itemId ==
                                            consumeItem.itemId)
    end
...
UINFactoryLine.OnBtnAddLineClicked = function(self)
    -- function num : 0_6
    if self.uiParent ~= nil then (self.uiParent):SetFactoryLineMaxCycle(self) end
end
