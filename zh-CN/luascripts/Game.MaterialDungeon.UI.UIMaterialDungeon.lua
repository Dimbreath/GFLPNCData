...
, self.selectItemId))
  ...
  fmtCtrl:InitFromationCtrl(eFmtFromModule.MaterialDungeon, ((self.selectChapterItem).cfg).id, enterFormationFunc, exitFormationFunc, startBattleFunc, (self.selectChapterItem).costStrengthNum)
end
    ...
    function(self)
        ...
        for k, v in pairs(ConfigData.material_dungeon) do
            if v ~= nil and v.ui_type == eDungeonUIType.MaterialDungeon then
                ...
                matItemData:CalcUnLockedAndProgress()
                matItemData:CalcSelfUnlock()
                local TotalDailyLimitAdd =
                    (self.dungeonUtil):TotalDailyLimitAddFromAchievement(k)
                matItemData.totalDailyLimit =
                    matItemData.totalDailyLimit + TotalDailyLimitAdd;
                (table.insert)(self.matItemDataList, matItemData)
            end
        end
    end
