...
DormController.OnInit = function(self)
    ...
    self.__onChangeDormBindComplete = BindCallback(self,
                                                   self.OnChangeDormBindComplete)
end
...
DormController.EnterDorm = function(self)
    ...
    local preLoadFunc = function()
        ...
        local effectWait = (self.comResloader):LoadABAssetAsyncAwait(
                               "Res/Effect/Prefabs/FX_Pick.prefab");
        ...
        local interPointWait = (self.comResloader):LoadABAssetAsyncAwait(
                                   "Res/Effect/Prefabs/DragPoint.prefab");
        ...
        local fntBottomWait = (self.comResloader):LoadABAssetAsyncAwait(
                                  PathConsts:GetDormFntPath("FntBottom"));
        ...
        local gridWallWait = (self.comResloader):LoadABAssetAsyncAwait(
                                 PathConsts:GetDormRoomPath("DormWallGrid"));
        ...
        local gridFloorWait = (self.comResloader):LoadABAssetAsyncAwait(
                                  PathConsts:GetDormRoomPath("DormFloorGrid"));
    ...
    ((CS.GSceneManager).Instance):LoadSceneAsyncByAB((Consts.SceneName).Dorm,
                                                     function(ok)
        -- function num : 0_2_1 , upvalues : self, _ENV
        self.__isInDormScene = true
        for _, v in pairs(self.ctrls) do v:OnEnterDormScene() end
        if self.allDormData ~= nil then self:InitDorm() end
...
DormController.ExitDorm = function(self)
    ...
    self:__ClearData()
    UIManager:DeleteAllWindow();
    ((CS.GSceneManager).Instance):LoadSceneAsyncByAB((Consts.SceneName).Main,
                                                     function(ok)
...
DormController.InitDorm = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(
        (((CS.DormCameraController).Instance).transform).parent, self.bind);
    (CS_LeanTouch.OnFingerTap)("+", self.__onDormTouchTap);
    (CS_LeanTouch.OnGesture)("+", self.__onDormTouchGesture)
    if #(self.allDormData).houseIdList == 0 then return end
...
DormController.GetCurRoom = function(self)
    -- function num : 0_7
    if (self.roomCtrl).roomEntity == nil then return end
...
DormController.GetBindFntDataList = function(self)
    ...
    if self.state == eDormState.House or self.state == eDormState.HouseEdit then
        ...
        do
            if self.state == eDormState.Room or self.state ==
...
DormController.ChangeDormBind = function(self, fntData, newHeroId)
    ...
    if oldHeroId == newHeroId then return end
    ...
    (self.dormNetwork):CS_DORM_BindUnbindHero(heroId, bind, houseId, roomPos,
                                              index,
                                              self.__onChangeDormBindComplete)
    -- DECOMPILER ERROR: 3 unprocessed JMP targets
end

DormController.OnChangeDormBindComplete =
    function(self, dataList)
        -- function num : 0_12
        local success = dataList[0]
        if success then
            local oldBindId = ((self._dormBindDatas).fntData):GetFntParam();
            ((self._dormBindDatas).fntData):SetFntParam(
                (self._dormBindDatas).newHeroId)
            local curRoomData = ((self._dormBindDatas).fntData):GetFntRoom()
            local allBindFntData = self:GetAllBindFntData()
            local newHeroBindFntData =
                (allBindFntData.boundDic)[(self._dormBindDatas).newHeroId]
            ...
            do
                do
                    ...
                    (self.characterCtrl):SetBindCharacterChange(
                        (self._dormBindDatas).fntData, oldBindId,
                        (self._dormBindDatas).newHeroId)
...
DormController.RecvPurchaseHouse = function(self, houseId)
    ...
    if uiDormOverview ~= nil then uiDormOverview:RefreshDormOverview() end
    if self.dormWindow ~= nil then (self.dormWindow):RefreshDormWindow() end
end

DormController.EnterDormEditor = function(self, callback)
    -- function num : 0_15 , upvalues : eDormState, _ENV
    if self.dormWindow ~= nil then (self.dormWindow):Hide() end
    if self.state == eDormState.House or self.state == eDormState.HouseEdit then
        ...
        UIManager:ShowWindowAsync(UIWindowTypeID.DormEdit, function(window)
            ...
            if window == nil then return end
            window:InitDormEdit()
            if callback ~= nil then callback(window) end
        end)
    else
...
DormController.OpenDormWarehouse = function(self)
    ...
    if (self.state == eDormState.HouseEdit and self.state == eDormState.Room) or
        self.state == eDormState.RoomEdit then
...
DormController.OnDormTouchTap = function(self, finger)
    ...
    if finger.StartedOverGui then return end
    if (self.state == eDormState.House or self.state == eDormState.Room) and
        self.dormWindow ~= nil then (self.dormWindow):Show() end
end
...
DormController.__ClearData = function(self)
    ...
    if self.ctrls ~= nil then
        for _, v in pairs(self.ctrls) do v:OnDelete() end
        self.ctrls = nil
    end
end
