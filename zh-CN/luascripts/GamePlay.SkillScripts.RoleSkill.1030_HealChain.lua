...
bs_1030.RealPlaySkill = function(self, sender, target, healId)
  ...
  if sender == nil then
    LuaSkillCtrl:CallEffectWithArg(target, (self.config).effectId2, self, true, self.SkillEventFunc, healId)
  else
    LuaSkillCtrl:CallEffectWithArgOverride(target, (self.config).effectId2, self, sender, true, self.SkillEventFunc, healId)
  end
...
bs_1030.SkillEventFunc = function(self, healId, effect, eventId, target)
  ...
  if effect.dataId == (self.config).effectId2 and eventId == eBattleEffectEvent.Trigger then
    ...
    LuaSkillCtrl:StartTimer(self, 2, BindCallback(self, self.CallBack, target.targetRole, healId))
...
bs_1030.CallBack = function(self, targetRole, healId)
  ...
  (targetRole.auSource):PlayAudioById((self.config).audioId1)
  local skillResult = LuaSkillCtrl:CallSkillResultNoEffect(self, targetRole)
  LuaSkillCtrl:HealResult(skillResult, (self.config).heal_config, {healId})
...
bs_1030.PlaySkill = function(self, data)
    ...
    if ((self.caster).recordTable)["1030_Time"] > 0 and
        (targetList[0]).targetRole ~= nil then
        LuaSkillCtrl:CallBreakAllSkill(self.caster);
        (self.caster):LookAtTarget((targetList[0]).targetRole)
        self:CallCasterWait(30)
        local attackTrigger = BindCallback(self, self.OnAttackTrigger,
                                           (targetList[0]).targetRole, data)
        LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, 1002, 1, 6,
                                               attackTrigger)
    end
end
...
bs_1030.RealPlaySkill = function(self, sender, target, healId)
    -- function num : 0_4 , upvalues : _ENV
    if sender == nil then
        LuaSkillCtrl:CallEffectWithArg(target, (self.config).effectId2, self,
                                       true, self.SkillEventFunc, healId)
    else
        LuaSkillCtrl:CallEffectWithArgOverride(target, (self.config).effectId2,
                                               self, sender, true,
                                               self.SkillEventFunc, healId)
    end
    LuaSkillCtrl:CallEffect(target, (self.config).effectId1, self, nil, sender)
end
...
bs_1030.SkillEventFunc = function(self, healId, effect, eventId, target)
    -- function num : 0_5 , upvalues : _ENV
    if effect.dataId == (self.config).effectId2 and eventId ==
        eBattleEffectEvent.Trigger then
        if healId == 1 and (self.arglist)[4] > 0 then
            LuaSkillCtrl:CallBuff(self, target.targetRole, (self.config).buffId,
                                  1)
        end
        LuaSkillCtrl:StartTimer(self, 2, BindCallback(self, self.CallBack,
                                                      target.targetRole, healId)) -- DECOMPILER ERROR at PC40: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.caster).recordTable)["1030_Time"] =
            ((self.caster).recordTable)["1030_Time"] - 1
        if ((self.caster).recordTable)["1030_Time"] > 0 then
            local targetList = LuaSkillCtrl:CallTargetSelect(self, 24, 10)
            if targetList.Count == 0 then return end
            for i = 0, targetList.Count - 1 do
                if (targetList[i]).targetRole ~= nil and
                    (targetList[i]).targetRole ~= target.targetRole then
                    self:RealPlaySkill(target.targetRole,
                                       (targetList[i]).targetRole, healId + 1)
                end
            end
        end
    end
