...
DormAIFntState.StateStateByInterPoint = function(self, interPoint)
    ...
    for k, v in pairs(interPoint.logicpoint) do
        local logicPointX, logicPointY =
            (DormUtil.GetFntOffsetCoord)((interPoint.fntData).x,
                                         (interPoint.fntData).y,
                                         (interPoint.fntData).r, v[1], v[2])
    ...
    do
        ...
        self.fntEntity = ((self.charEntity).roomEntity):GetFntByData(
                             self.fntData)
        local offsetWorld = ((self.fntEntity).transform):TransformPoint(
                                (self.interPoint):GetInterOffsetCoord())
        local unityPos =
            (((self.charEntity).roomEntity).fntHolder):InverseTransformPoint(
                offsetWorld);
...
DormAIFntState.RandOneInterPoint = function(self)
    ...
    for k, v in ipairs(roomData.interpoint) do
        if v:GetPointActive() and v:GetInterPointType() ==
            (self.actionCfg).interact_point_type then
            for k1, v1 in pairs(v.logicpoint) do
                local logicPointX, logicPointY =
                    (DormUtil.GetFntOffsetCoord)((v.fntData).x, (v.fntData).y,
                                                 (v.fntData).r, v1[1], v1[2])
                if (DormUtil.IsFntCoordLegal)(logicPointX, logicPointY) then
                    local point = {
                        logicX = logicPointX,
                        logicY = logicPointY,
...
DormAIFntState.MoveToInterPoint = function(self)
    ...
    (self.charEntity):DoMoveUnityPos(unityPos,
                                     BindCallback(self, self.StartInteractFnt))
end
...
DormAIFntState.StartInteractFnt = function(self)
    ...
    local angle = (Vector3.New)(0, (self.interPoint):GetInterAngle() +
                                    (self.fntData).r, 0);
...
DormAIFntState.OnUpdate = function(self)
    ...
    if self.isFailToEnd then return end
...
DormAIFntState.ExitFntState = function(self)
    ...
    if not (DormUtil.IsFntCoordLegal)(self.logicX, self.logicX) then return end
    local pos = (DormUtil.XYCoord2Fnt)(self.logicX, self.logicY);
    (self.charEntity):AnimatorWalk1();
...
DormAIFntState.UnBindFntInterPoint = function(self)
    -- function num : 0_9
    if self.interPoint == nil then return end
