...
EpSceneTimeLineCtr.OnSceneLoaded = function(self)
    -- function num : 0_2 , upvalues : _ENV
    self:TLExecute((TimeLineType.TLEventType).OnSceneLoadedTL,
                   (TimeLineType.TLAction).Stop)
    self:TLExecute((TimeLineType.TLEventType).OnSceneLoadedTL,
                   (TimeLineType.TLAction).Play)
end

EpSceneTimeLineCtr.OnStartBattle = function(self)
    -- function num : 0_3 , upvalues : _ENV
    self:TLExecute((TimeLineType.TLEventType).OnStartBattleTL,
                   (TimeLineType.TLAction).Stop)
    self:TLExecute((TimeLineType.TLEventType).OnStartBattleTL,
                   (TimeLineType.TLAction).Play)
end
...
EpSceneTimeLineCtr.OnExitBattle = function(self)
    -- function num : 0_4 , upvalues : _ENV
    self:TLExecute((TimeLineType.TLEventType).OnExitBattleTL,
                   (TimeLineType.TLAction).Stop)
    self:TLExecute((TimeLineType.TLEventType).OnExitBattleTL,
                   (TimeLineType.TLAction).Play)
end

EpSceneTimeLineCtr.OnReconnectToBattle =
    function(self)
        -- function num : 0_5 , upvalues : _ENV
        self:TLExecute((TimeLineType.TLEventType).OnReconnectToBattleTL,
                       (TimeLineType.TLAction).Stop)
        self:TLExecute((TimeLineType.TLEventType).OnReconnectToBattleTL,
                       (TimeLineType.TLAction).Play)
    end
...
EpSceneTimeLineCtr.SkipTL = function(self, tlEventType)
    -- function num : 0_6
    local pbDirector = ((self.TLEventDic)[tlEventType]).playableDirector
    if pbDirector ~= nil then pbDirector.time = pbDirector.duration end
end
...
EpSceneTimeLineCtr.RegistTLEvent = function(self, tlEventType, plyDire,
                                            onComplete, onStart)
    -- function num : 0_8 , upvalues : _ENV
    -- DECOMPILER ERROR at PC2: Confused about usage of register: R5 in 'UnsetPending'

    (self.TLEventDic)[tlEventType] = {} -- DECOMPILER ERROR at PC5: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.TLEventDic)[tlEventType]).playableDirector = plyDire -- DECOMPILER ERROR at PC9: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.TLEventDic)[tlEventType]).event = {}
    local TLEventTB = ((self.TLEventDic)[tlEventType]).event
    if onStart ~= nil then
    ...
    if (self.TLEventDic)[tlEventType] ~= nil then
        ...
        if ((self.TLEventDic)[tlEventType]).event ~= nil then
            ...
            if tlEvntTB[eventTick] ~= nil then
                ...
                end -- DECOMPILER ERROR at PC40: Confused about usage of register: R5 in 'UnsetPending'
...
EpSceneTimeLineCtr.TLExecute = function(self, tlEventType, TLActionType)
    -- function num : 0_11 , upvalues : _ENV
    if (self.TLEventDic)[tlEventType] ~= nil and
        ((self.TLEventDic)[tlEventType]).playableDirector ~= nil then
        if TLActionType == (TimeLineType.TLAction).Play then
            (((self.TLEventDic)[tlEventType]).playableDirector):Play()
        else
            if TLActionType == (TimeLineType.TLAction).Stop then
...
EpSceneTimeLineCtr.OnDelete = function(self)
    -- function num : 0_13 , upvalues : _ENV
    for k, v in pairs(self.TLEventDic) do self:RemoveAllTLEvent(k) end
    self.TLEventDic = table.emptytable
    self.VCamDic = table.emptytable
end
