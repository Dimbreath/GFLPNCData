...
AllAthData.InitAllAthData = function(self, msg)
    ...
    for uid, v in pairs(msg.data) do self:__NewAthData(v) end
    ...
    for uid, v in pairs(msg.updateAth) do
        ...
        if athData == nil then
            ...
            if oldBindInfo ~= nil and newBindInfo == nil then
                self:__RemoveFromHero(oldBindInfo.id, oldBindInfo.idx,
                                      athData.uid)
            else
                if oldBindInfo ~= nil and newBindInfo ~= nil and oldBindInfo.id ~=
                    newBindInfo.id then
                    self:__RemoveFromHero(oldBindInfo.id, oldBindInfo.idx,
                                          athData.uid)
    ...
    for uid, v in pairs(msg.deleteAth) do
        ...
        if athData ~= nil then
            ...
            if athData.bindInfo ~= nil then
                self:__RemoveFromHero((athData.bindInfo).id,
                                      (athData.bindInfo).idx, uid)
            end -- DECOMPILER ERROR at PC85: Confused about usage of register: R8 in 'UnsetPending'
...
AllAthData.__NewAthData = function(self, msg)
    -- function num : 0_4 , upvalues : ArithmeticData
    local athData = (ArithmeticData.New)(msg) -- DECOMPILER ERROR at PC5: Confused about usage of register: R3 in 'UnsetPending'
    ;
...
AllAthData.__AddToSlot = function(self, athData)
    ...
    if slotAthList == nil then
        error("Can\'t find arithmetic slot. slot id = " ..
                  tostring((athData.bindInfo).idx))
        return
    end
...
AllAthData.__RemoveFromSlot = function(self, athData)
    -- function num : 0_6
    local slotAthList = (self.slotAthDic)[athData.area_type]
    if slotAthList == nil then return end
...
AllAthData.__AddToHero = function(self, athData)
    ...
    if athData.bindInfo ~= nil then
        ...
        if heroAth == nil then
            heroAth = {} -- DECOMPILER ERROR at PC14: Confused about usage of register: R5 in 'UnsetPending'
            ;
...
AllAthData.__RemoveFromHero = function(self, heroId, slotId, uid)
    ...
    local heroAth = (self.heroAthDic)[heroId]
    if heroAth == nil then return end
    local heroAthSlot = heroAth[slotId]
    if heroAthSlot == nil then return end
    heroAthSlot[uid] = nil;
    (PlayerDataCenter.attributeBonus):DirtyHeroAthBonus(heroId)
end
...
AllAthData.GetHeroAthList = function(self, heroId, slotId)
    ...
    local heroAth = (self.heroAthDic)[heroId]
    if heroAth == nil then return table.emptytable, table.emptytable end
    local athDic = heroAth[slotId]
    if athDic == nil then return table.emptytable, table.emptytable end
    local athList = {}
    for uid, data in pairs(athDic) do (table.insert)(athList, data) end
...
AllAthData.GetAllAthList = function(self, space, ignoreInstalled)
    ...
    for k, v in pairs(dic) do
        if not ignoreInstalled or v.bindInfo == nil then
            if space == nil then
                (table.insert)(list, v)
            else
                if v.size == space then (table.insert)(list, v) end
            end
        end
    end
    return list
...
AllAthData.GetAllAthSlotList = function(self, slotId, space, ignoreInstalled,
                                        withoutIgnoreHeroId)
    ...
    local list = {}
    for k, v in pairs(dic) do
        if not ignoreInstalled or v.bindInfo == nil or (v.bindInfo).id ==
            withoutIgnoreHeroId then
            if space == nil then
                (table.insert)(list, v)
            else
                if v.size == space then (table.insert)(list, v) end
...
AllAthData.HeroContainAthById = function(self, heroId, slotId, athId)
    ...
    local heroAth = (self.heroAthDic)[heroId]
    if heroAth == nil then return false end
    if slotId == nil then
        for k, athDic in pairs(heroAth) do
            for uid, data in pairs(athDic) do
                if data.id == athId then return true, data end
            end
        end
    else
        do
            local athDic = heroAth[slotId]
            if athDic == nil then return false end
            for uid, data in pairs(athDic) do
                if data.id == athId then return true, data end
            end
            do return false end
...
AllAthData.HeroContainAthByUid = function(self, heroId, uid)
    ...
    for k, athDic in pairs(heroAth) do
        for installUid, data in pairs(athDic) do
            if uid == installUid then return true end
        end
    end
end
...
AllAthData.GetHeroAthFreeSpace = function(self, heroData, slotId)
    ...
    do
        ...
        if heroAthSpace == nil then return ok, freeSpace end
        freeSpace = heroAthSpace - (usedSpace)
        if freeSpace > 0 then ok = true end
        return ok, freeSpace
    end
end
...
AllAthData.GetHeroCanLoaded = function(self, heroData)
    ...
    for slotId, v in ipairs(athslotList) do
        if self:GetHeroCanLoadedSlot(heroData, slotId) then return true end
    end
    return false
end
...
AllAthData.GetHeroCanLoadedSlot = function(self, heroData, slotId)
    ...
    if not ok then return canLoad, ATHCount end
    ...
    for k, athData in pairs(athDic) do installedAthIdDic[athData.id] = true end
    ...
    do
        if ATHCount > 0 then canLoad = true end
...
AllAthData.GetAthFightPower = function(self, athAttrDic)
    ...
    do
        if self.__fightingPowerFun == nil then
            ...
            self.__fightingPowerFun = (load(
                                          "return function(ath) return " ..
                                              formula .. " end"))()
        end
...
AllAthData.GetHeroAthSuitIdDic = function(self, heroId, ignoreSlotId)
    ...
    local suitIdDic = {}
    local heroAth = (self.heroAthDic)[heroId]
    if heroAth == nil then return suitIdDic end
    for slotId, athDic in pairs(heroAth) do
        if ignoreSlotId ~= slotId then
            for uid, athData in pairs(athDic) do
                ...
                if suitId ~= nil then suitIdDic[suitId] = true end
            end
        end
    end
    return suitIdDic
    ...
    for slotId, v in ipairs(athSlotList) do
        ...
        for k, athData in ipairs(athList) do
            for k2, elem in ipairs(athData.attrList) do
                ...
                local attrValue = elem.value * efficiency // 1000
                attrDic[attrId] = attrValue + attrDic[attrId]
                slotAttrDicNoEfcc[attrId] =
                    elem.value + slotAttrDicNoEfcc[attrId]
                allAttrDic[attrId] = attrValue + allAttrDic[attrId]
                allAttrDicNoEfcc[attrId] = elem.value + allAttrDicNoEfcc[attrId]
            end
            for k2, elem in ipairs(athData.affixList) do
                ...
                if cfg == nil then
                    ...
                    attrDic[attrId] = attrValue + attrDic[attrId]
                    slotAttrDicNoEfcc[attrId] =
                        attrValue + slotAttrDicNoEfcc[attrId]
                    allAttrDic[attrId] = attrValue + allAttrDic[attrId]
                    allAttrDicNoEfcc[attrId] =
                        attrValue + allAttrDicNoEfcc[attrId]
                end
            end
            ...
            if suitId ~= 0 and suitDic[suitId] == nil then
                ...
                if suitAthList == nil then
                    error("Can\'t find ConfigData.arithmetic.suitDic,id = " ..
                              tostring(suitId))
                end
                local suitAthCount = 0
                local suitAthDic = {}
                for k2, athId in ipairs(suitAthList) do
                    local installed, instAthData =
                        self:HeroContainAthById(heroId, nil, athId)
                    if installed then
                ...
                for k2, v2 in ipairs(suitCfg) do
                    if v2.num <= suitAthCount then
                        ...
                        for k2, attrId in ipairs(suitDetailCfg.attrIdList) do
                            print((suitDetailCfg.attrValueList)[k2])
