...
UINResourceItem.UpdateCount = function(self)
    ...
    if ItemIdOfKey == (self.itemCfg).id then
        ...
        end -- DECOMPILER ERROR at PC33: Confused about usage of register: R1 in 'UnsetPending'
        
        ((self.ui).tex_Count).text =
            tostring((PlayerDataCenter.stamina).value) .. "/" ..
                tostring((PlayerDataCenter.stamina).ceiling)
        return
    end
            ...
            if not (self.shopCtrl).isUnlocked then
                ((CS.MessageCommon).ShowMessageTips)(
                    ConfigData:GetTipContent(
                        TipContent.notUnlockShopCantBuyStamina))
                return
            end
