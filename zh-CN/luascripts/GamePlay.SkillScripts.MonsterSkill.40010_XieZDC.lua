...
, monsterEffectId = 10221, buffId = 1007, buffDamageFormula = 10007, effecthurtId = 10275, audioId1 = 78, audioId2 = 79, audioId3 = 80, audioId4 = 81}
  ...
  if eventId == eBattleEffectEvent.Trigger then
    ...
    if roles ~= nil then
      for i = 0, roles.Count - 1 do
        LuaSkillCtrl:CallBuffRepeated(self, roles[i], (self.config).buffId, 1, (self.arglist)[4], self.OnBuffExecute)
        ;
        ((roles[i]).auSource):PlayAudioById((self.config).audioId3)
      end
    end
  end
, self, Cishu)
...
bs_40010.OnBuffExecute = function(self, buff, targetRole)
  ...
  (targetRole.auSource):PlayAudioById((self.config).audioId4)
