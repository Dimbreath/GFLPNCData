...
local UINPlayerSkillNode = class("UINPlayerSkillNode", UIBaseNode)
local base = UIBaseNode
local UINPlayerSkillItem = require(
                               "Game.Battle.UI.SkillModule.PlayerSkill.UINPlayerSkillItem")
local FloatAlignEnum = require("Game.CommonUI.FloatWin.FloatAlignEnum")
local HAType = FloatAlignEnum.HAType
local VAType = FloatAlignEnum.VAType
local PlayerMaxMp = (ConfigData.game_config).playerMaxMp
...
UINPlayerSkillNode.OnInit = function(self)
    ...
    self.__HideSkillInfo = BindCallback(self, self.HideSkillInfo);
    ((self.ui).skillItem):SetActive(false)
    self.skillItemPool = (UIItemPool.New)(UINPlayerSkillItem,
                                          (self.ui).skillItem)
end

UINPlayerSkillNode.InitPlayerSkillNode =
    function(self, battlePlayerController, skillList)
        -- function num : 0_2 , upvalues : _ENV, PlayerMaxMp
        self.playerCtrl = battlePlayerController
        local curMp = battlePlayerController.CurMp;
        (self.skillItemPool):HideAll()
        ...
        for i = 0, skillList.Count - 1 do
            local battleSkill = skillList[i]
            local skillItem = (self.skillItemPool):GetOne() -- DECOMPILER ERROR at PC21: Confused about usage of register: R11 in 'UnsetPending'
            ;
            (skillItem.gameObject).name = tostring(i + 1)
            skillItem:InitPlayerSkillItem(battleSkill, self.resloader,
                                          self.__OnItemClick,
                                          self.__ShowSkillInfo,
                                          self.__HideSkillInfo,
                                          self.__OnItemPointDown)
            if dynplayer then
                skillItem:SetSkillItemLock(dynplayer:IsEpBattleSkillLock(i))
            end
            skillItem:RefreshPlayerSkillItemMp(curMp, PlayerMaxMp <= curMp)
...
UINPlayerSkillNode.OnPlayerMpChange = function(self, mp)
    -- function num : 0_3 , upvalues : _ENV, PlayerMaxMp
    ((self.ui).tex_Energy):SetIndex(0, tostring(mp), tostring(PlayerMaxMp)) -- DECOMPILER ERROR at PC15: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).img_Energy).fillAmount = mp / PlayerMaxMp
    self.isMax = PlayerMaxMp <= mp
    for k, skillItem in ipairs((self.skillItemPool).listItem) do
...
UINPlayerSkillNode.OnPlayerSkillItemClick =
    function(self, battleSkill)
        -- function num : 0_4
        if battleSkill.isPassive then return end
        (self.playerCtrl):OnSkillItemMonoCliked(battleSkill)
    end

UINPlayerSkillNode.OnPlayerSkillItemPointDown =
    function(self, battleSkill)
        -- function num : 0_5
        if battleSkill.isPassive or not battleSkill.isManualMode then
            return
...
UINPlayerSkillNode.OnUpdateLogic_PlayerSkillNode =
    function(self)
        -- function num : 0_8 , upvalues : _ENV
        for k, skillItem in ipairs((self.skillItemPool).listItem) do
            skillItem:OnUpdateLogic_PlayerSkillItem()
...
UINPlayerSkillNode.GetSkillItemById = function(self, skillId)
    -- function num : 0_10 , upvalues : _ENV
    for _, skillItem in pairs((self.skillItemPool).listItem) do
        if skillItem.skillId == skillId then return skillItem end
    end
    return nil
end
...
UINPlayerSkillNode.OnUpdateRender_PlayerSkillNode =
