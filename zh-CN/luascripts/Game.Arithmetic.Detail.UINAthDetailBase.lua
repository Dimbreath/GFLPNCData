...
UINAthDetailBase.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_Strengthen, self, self.OnClickStrengthen)
  ;
...
UINAthDetailBase.InitAthDetailNode = function(self, athData, efficiency)
  ...
  ;
  ((self.ui).tex_ATHName):SetIndex(0, athData:GetName(), tostring(athData.area_type))
  ;
  ...
  local strengthenEnable = StrengthenQuality <= athData.quality
...
UINAthDetailBase.OnClickLock = function(self)
  -- function num : 0_7
  if self.lockEvent ~= nil then
    (self.lockEvent)((self.athData).uid)
  end
end
