...
UINLevelItem.SeletedLevelItem = function(self, select, withTween)
    -- function num : 0_6 , upvalues : CS_DoTween, bottomTweenSizeDelta, aniSelectTweenSizeDelta, _ENV
    self:__ClearSelectedSequence()
    local seq = nil
    if withTween then seq = (CS_DoTween.Sequence)() end
    -- DECOMPILER ERROR at PC16: Confused about usage of register: R4 in 'UnsetPending'

    if select and withTween then
        (((self.ui).ani_Select).transform).sizeDelta =
            self.aniSelectTweenSizeDeltaDefault;
        ...
        AudioManager:PlayAudioById(1027)
    end
    do
...
UINLevelItem.ShowRedDotLevelItem = function(self, show)
    -- function num : 0_7
