...
UINLevelGroup.InitLevelGroup = function(self, levelGroupData, lastEpStateCfg, arrangeCfg, lineHolder, bgHolder, stageCount, levelItemClickEvent, levelAvgMainClickEvent, lAvgSubClickEvent, resLoader, sectorLevelTipsGuides)
  ...
  -- DECOMPILER ERROR at PC157: Confused about usage of register: R19 in 'UnsetPending'
  ...
  self.lAvgSubItemDic = {}
  local selectedLvId = (UIManager:GetWindow(UIWindowTypeID.SectorLevel)):GetSelectedStageId()
  local selectedLAvgMainId = (UIManager:GetWindow(UIWindowTypeID.SectorLevel)):GetSelectedLAvgMainId()
  self.__itemList = {}
  for k,v in ipairs(levelGroupData) do
    ...
    if v.stageCfg ~= nil then
      ...
      -- DECOMPILER ERROR at PC249: Confused about usage of register: R32 in 'UnsetPending'
    ...
    do
      ...
      -- DECOMPILER ERROR at PC430: Confused about usage of register: R35 in 'UnsetPending'
...
UINLevelGroup.OnInit = function(self)
    ...
    self.levelItemPool = (UIItemPool.New)(UINLevelItem, (self.ui).levelItem);
    ((self.ui).plotItem):SetActive(false)
    self.lAvgMainItemPool =
        (UIItemPool.New)(UINLevelAvgMain, (self.ui).plotItem);
    ((self.ui).subPlotItem):SetActive(false)
    self.lAvgSubItemPool = (UIItemPool.New)(UINLevelAvgSub,
                                            (self.ui).subPlotItem);
    ((self.ui).lineItem):SetActive(false)
    self.levelLineItem = (UIItemPool.New)(UINLevelLine, (self.ui).lineItem)
    self.UICanvasBg = (UICanvasBg.New)();
...
UINLevelGroup.InitLevelGroup = function(self, levelGroupData, lastEpStateCfg,
                                        arrangeCfg, lineHolder, bgHolder,
                                        stageCount, levelItemClickEvent,
                                        levelAvgMainClickEvent,
                                        lAvgSubClickEvent, resLoader,
                                        sectorLevelTipsGuides)
    -- function num : 0_1 , upvalues : _ENV, eLevelLineType
    self.arrangeCfg = arrangeCfg
    self.lastEpStateCfg = lastEpStateCfg
    self.levelItemClickEvent = levelItemClickEvent
    ...
    local lastCfg = arrangeCfg[#levelGroupData]
    if #lastCfg.pos ~= 2 then
        error((string.format)("levelArrange cfg pos error, typeId:%s, id:%s",
                              lastCfg.typeId, lastCfg.id))
        return
    end
    local isFirstGroup = levelGroupData.startIndex == 1
    local startIsLeft = false
    ...
    if isVertical then
        ...
        width = (math.abs)((lastCfg.pos)[1]) + (self.ui).padding
        startIsLeft = ((arrangeCfg[2]).pos)[2] - ((arrangeCfg[1]).pos)[2] > 0
    end -- DECOMPILER ERROR at PC147: Confused about usage of register: R18 in 'UnsetPending'
    
    ((self.ui).layoutElement).preferredWidth = width -- DECOMPILER ERROR at PC150: Confused about usage of register: R18 in 'UnsetPending'
    ;
    ((self.ui).layoutElement).preferredHeight = height
    local sizeDelta = (Vector2.New)(width, height) -- DECOMPILER ERROR at PC157: Confused about usage of register: R19 in 'UnsetPending'
    ;
    (self.transform).sizeDelta = sizeDelta -- DECOMPILER ERROR at PC160: Confused about usage of register: R19 in 'UnsetPending'
    ;
    ((self.ui).lineList).sizeDelta = sizeDelta;
    (self.levelItemPool):HideAll();
    (self.lAvgMainItemPool):HideAll();
    ...
    self.lAvgSubItemDic = {}
    local selectedLvId =
        (UIManager:GetWindow(UIWindowTypeID.SectorLevel)):GetSelectedStageId()
    local selectedLAvgMainId =
        (UIManager:GetWindow(UIWindowTypeID.SectorLevel)):GetSelectedLAvgMainId()
    self.__itemList = {}
    for k, v in ipairs(levelGroupData) do
        local startPos = nil
        local randomSeed = 0
        ...
        if v.stageCfg ~= nil then
            ...
            local levelItem = (self.levelItemPool):GetOne()
            tempItem.levelItem = levelItem
            levelItem:InitSectorLevelItem(stageCfg, arrangeCfg[k],
                                          self.levelItemClickEvent, resLoader)
            if self.lastEpStateCfg ~= nil then
                if (self.lastEpStateCfg).num == stageCfg.num then
                    levelItem:OnClickLevelItem()
                    levelItem:LevelItemShowContinue(true)
            ...
            startPos = (levelItem.transform).localPosition
            randomSeed = stageCfg.id
            self:__ShowLAvgSubItem(levelItem, isVertical) -- DECOMPILER ERROR at PC249: Confused about usage of register: R32 in 'UnsetPending'
            ;
            (self.__itemList)[k] = levelItem
            self:SectorLevelTryTipsGuide(levelItem, stageCfg.id, false)
        elseif v.avgCfg ~= nil then
            ...
            local lAvgMainItem = (self.lAvgMainItemPool):GetOne()
            tempItem.lAvgMainItem = lAvgMainItem
            lAvgMainItem:InitLAvgMain(avgCfg, arrangeCfg[k],
                                      self.lAvgMainClickEvent, resLoader)
            startPos = (lAvgMainItem.transform).localPosition
            randomSeed = avgCfg.id -- DECOMPILER ERROR at PC274: Confused about usage of register: R32 in 'UnsetPending'
            ;
            (self.__itemList)[k] = lAvgMainItem
            if selectedLAvgMainId == avgCfg.id then
                lAvgMainItem:SelectedLAvgMain(true)
        ...
        local length, lineType = nil, nil
        local isFirstStage = not isFirstGroup or k == 1
        if not isVertical or not 90 then angle = not isFirstStage or 180 end
        length = (self.ui).padding
        lineType = eLevelLineType.BetweenLevelFull
        if k == 1 then
            lastPos = (Vector3.New)(startPos.x, startPos.y, startPos.z)
            if isVertical then
                lastPos.y = lastPos.y - (math.abs)(((arrangeCfg[1]).pos)[2]) -
                                (self.ui).padding
                lastPos.x = lastPos.x +
                                (((arrangeCfg[#arrangeCfg]).pos)[1] -
                                    ((arrangeCfg[1]).pos)[1])
            else
                lastPos.x = lastPos.x - (math.abs)(((arrangeCfg[1]).pos)[1]) -
                                (self.ui).padding
                lastPos.y = lastPos.y +
                                (((arrangeCfg[#arrangeCfg]).pos)[2] -
                                    ((arrangeCfg[1]).pos)[2])
            end
        end
        length = (Vector3.Distance)(lastPos, startPos)
        do
            ...
            if v.stageCfg ~= nil and
                (PlayerDataCenter.sectorStage):IsStageComplete((v.stageCfg).id) then
                lineType = eLevelLineType.BetweenLevelFull
            elseif v.avgCfg ~= nil and
                (ControllerManager:GetController(ControllerTypeId.AvgPlay)):IsAvgPlayed(
                    (v.avgCfg).id) then
                lineType = eLevelLineType.BetweenLevelFull
            else
                lineType = eLevelLineType.BetweenLevel
            ...
            (math.randomseed)(randomSeed)
            local isLeft = self:__IsOuNumber(levelGroupData.startIndex + k)
            if not startIsLeft then isLeft = not isLeft end
            local isLastStage = stageCount == levelGroupData.startIndex + k - 1
            angle = self:__GetDecorateLineAngle(isVertical, isFirstStage,
                                                isLastStage, 1, isLeft)
            local lineItem = (self.levelLineItem):GetOne()
            lineItem:InitLevelLine(lineType, startPos, angle)
            angle = self:__GetDecorateLineAngle(isVertical, isFirstStage,
                                                isLastStage, 2, isLeft)
            do
                local lineItem = (self.levelLineItem):GetOne()
                lineItem:InitLevelLine(lineType, startPos, angle)
                lastPos = startPos
        ...
        if tipsGuideDic[id] ~= nil then
            ...
            do
                (self.resLoader):LoadABAssetAsync(
                    PathConsts:GetUIPrefabPath(GuideUtil.TipsGuidePrefabName),
                    function(guidePrefab)
                        -- function num : 0_3_0 , upvalues : _ENV, item, show_dir, self
                        local guideItem =
                            (GuideUtil.ShowTipsGuide)(item.transform,
                                                      guidePrefab, show_dir)
                        if not self.lastSectorTipsGuide then
                            self.lastSectorTipsGuide = {} -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
                            ;
                            (self.lastSectorTipsGuide)[item] = guideItem
                        end
                    end)
...
UINLevelGroup.SectorLevelClearTipsGuide =
    function(self, item)
        -- function num : 0_4 , upvalues : _ENV
        if IsNull(item.gameObject) then return end
        local tipsGuideNodeName = (GuideUtil.GetTipsGuideNodeName)(
...
UINLevelGroup.__ShowLAvgSubItem = function(self, levelItem, isVertical)
    ...
    if (self.lAvgSubItemDic)[levelItem] ~= nil then return end
    local avgPlayCtrl =
        ControllerManager:GetController(ControllerTypeId.AvgPlay)
    local avgCfg = avgPlayCtrl:TryGetAvgCfg(eAvgTriggerType.SubAvg, stageCfg.id,
                                            nil)
    if avgCfg == nil then return end
    if not avgPlayCtrl:IsAvgUnlock(avgCfg.id) then return end
    local startPos = (levelItem.transform).localPosition
    local offset = isVertical and 0 or -90;
    (math.randomseed)(stageCfg.id)
    local angle = (math.random)((self.ui).range_subAvg_angleA + offset,
                                (self.ui).range_subAvg_angleB + offset)
    local length = (math.random)((self.ui).range_subAvg_lengthA,
                                 (self.ui).range_subAvg_lengthB)
    local rad = (math.rad)(angle)
    local y = (math.sin)(rad) * length + startPos.y
    local x = (math.cos)(rad) * length + startPos.x
    local endPos = (Vector3.New)(x, y, startPos.z)
    ...
    lineItem:InitLevelLine(eLevelLineType.BetweenLevel, startPos, angle, length)
    local lAvgSubItem = (self.lAvgSubItemPool):GetOne()
    lAvgSubItem:InitLAvgSub(endPos, avgCfg, self.lAvgSubClickEvent) -- DECOMPILER ERROR at PC105: Confused about usage of register: R16 in 'UnsetPending'
    ;
    (self.lAvgSubItemDic)[levelItem] = lAvgSubItem
end
...
UINLevelGroup.__GetDecorateLineAngle = function(self, isVertical, isFirstStage,
                                                isLastStage, lineIndex, isLeft)
    -- function num : 0_7 , upvalues : _ENV
    local angle = nil
    local offset = isVertical and 0 or 90
    if isFirstStage then
        if lineIndex == 1 then
            angle = (math.random)((self.ui).range_start1A + offset,
                                  (self.ui).range_start1B + offset)
        else
            angle = (math.random)((self.ui).range_start2A + offset,
                                  (self.ui).range_start2B + offset)
        end
    else
        if isLastStage then
            if lineIndex == 1 then
                angle = (math.random)((self.ui).range_end1A + offset,
                                      (self.ui).range_end1B + offset)
            else
                angle = (math.random)((self.ui).range_end2A + offset,
                                      (self.ui).range_end2B + offset)
            end
        else
            if isLeft then
                if lineIndex == 1 then
                    angle = (math.random)((self.ui).range_left1A + offset,
                                          (self.ui).range_left1B + offset)
                else
                    angle = (math.random)((self.ui).range_left2A + offset,
                                          (self.ui).range_left2B + offset)
                end
            else
                if lineIndex == 1 then
                    angle = (math.random)((self.ui).range_right1A + offset,
                                          (self.ui).range_right1B + offset)
                else
                    angle = (math.random)((self.ui).range_right2A + offset,
                                          (self.ui).range_right2B + offset)
                end
            end
        end
    end
...
UINLevelGroup.RefreshLevelConnectLineState =
    function(self)
        -- function num : 0_11 , upvalues : _ENV, eLevelLineType
        for k, lineItem in pairs(self.itemConnectedLineDic) do
            if k.levelItem ~= nil then
                local stageCfg = (k.levelItem):GetLevelStageData()
                if (PlayerDataCenter.sectorStage):IsStageComplete(stageCfg.id) then
                    lineItem:RefreshLevelLineState(
                        eLevelLineType.BetweenLevelFull)
                else
                    lineItem:RefreshLevelLineState(eLevelLineType.BetweenLevel)
                end
            else
