...
local eSortKindType, SortKindFunc, FliterKindFunc = (table.unpack)(require("Game.Arithmetic.SortList.AthSortKindHelper"))
UIAthMultSortList.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_AthSortKind, self, self.OnClickSortAndFliter)
...
UIAthMultSortList.InitAthSortList = function(self, slotId, space, curSlotAthDataDic)
  -- function num : 0_3
...
UIAthMultSortList.RefreshAthList = function(self, funcSift, funcSort, otherAthList)
  ...
  if funcSort ~= nil then
    (table.sort)(self.curAthList, funcSort)
  end
...
UIAthMultSortList.SetClickSortAthEvent = function(self, event)
  -- function num : 0_7
...
UIAthMultSortList.OnAthItemClicked = function(self, athData)
  -- function num : 0_8
  if self.clickAthEvent ~= nil then
    (self.clickAthEvent)(athData)
  end
end
...
UIAthMultSortList.OnAthDataUpdate = function(self)
  ...
  self.originAthList = (PlayerDataCenter.allAthData):GetAllAthSlotList(self.slotId)
  self:RefreshAthList(function(athData)
    -- function num : 0_11_0
    if athData.lockUnlock or athData.bindInfo ~= nil then
      return false
...
, function(a, b)
  ...
  if self.selectedSort ~= nil then
    self.CurAthListBeforeFliter = self.curAthList
    self:RefreshAthList(self.selectedFliter, self.selectedSort, self.CurAthListBeforeFliter)
  end
  return 
end
...
UIAthMultSortList.__GetAthItemGoByIndex = function(self, index)
  -- function num : 0_15
...
UIAthMultSortList.OnClickSortAndFliter = function(self)
  -- function num : 0_17
...
UIAthMultSortList.__InitSortAndFliterKind = function(self)
          ...
          for key,value in pairs(ConfigData.attribute) do
            do
              if value.algor_main_attribute == 1 then
                local go = ((self.ui).obj_manner_Item):Instantiate()
                go:SetActive(true)
                local btn = (go.transform):FindComponent("", eUnityComponentID.Button)
                local texName = (go.transform):FindComponent("Tex_Name", eUnityComponentID.Text)
                texName.text = (LanguageUtil.GetLocaleText)(value.name)
                ;
                (btn.onClick):AddListener(function()
    -- function num : 0_18_0 , upvalues : self, SortKindFunc, index, value, FliterKindFunc, texName
    if SortKindFunc[index] then
      self.selectedSort = (SortKindFunc[index])(self, value.id)
      if FliterKindFunc[index] then
        self.selectedFliter = (FliterKindFunc[index])(value.id)
        self:RefreshAthList(self.selectedFliter, self.selectedSort, self.CurAthListBeforeFliter)
        -- DECOMPILER ERROR at PC28: Confused about usage of register: R0 in 'UnsetPending'

        ;
        ((self.ui).tex_manner_info).text = texName.text
