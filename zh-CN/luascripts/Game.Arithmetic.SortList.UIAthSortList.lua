...
UIAthSortList.RefreshAthList = function(self, funcSift, funcSort, otherAthList)
  ...
  for k,athData in ipairs(list) do
    ...
    if athData == self.selectAth then
      curSelectAth = athData
    end
  end
...
UIAthSortList.__RefreshSelectAth = function(self, athData)
  ...
  do
    if self.selectAth ~= nil then
      ...
      if lastAthItem ~= nil then
        lastAthItem:SelectAth(false)
      end
      self.selectAth = nil
    end
    if athData ~= nil then
...
UIAthSortList.__OnChangeItem = function(self, go, index)
  ...
  if athData.AthUIState ~= nil then
    ...
    if athData.AthUIState == eAthUIState.EquiptoHero and athData.bindInfo ~= nil then
...
UIAthSortList.__GetAthItemGoByIndex = function(self, index)
  -- function num : 0_14
  if (((self.ui).scrollView).content).childCount <= index then
    return nil
  end
...
UIAthSortList.InitForInherit = function(self, selectData)
  ...
  self:RefreshAthList(function(athDta)
    ...
    do return athDta.id == (self.selectAth).id and athDta ~= self.selectAth end
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
  end
, self.inheritSort)
  self:__InitSortAndFliterKind()
...
UIAthSortList.OnAthDataUpdateForInherit = function(self)
