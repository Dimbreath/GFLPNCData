...
UINMailContent.OnInit = function(self)
    ...
    self.poolMailItem = (UIItemPool.New)(UINBaseItemWithReceived,
                                         (self.ui).rewardItem);
    ((self.ui).rewardItem):SetActive(false);
    (((self.ui).tex_Content).onHrefClick):AddListener(
        BindCallback(self, self.OnClickHerf));
...
UINMailContent.UpdateContent = function(self, mailData)
    ...
    self.timer = (TimerManager:GetTimer(1, self.m_OutOfDataTime, self, false,
                                        false, true)):Start()
end
...
UINMailContent.m_RefreshStaticUI = function(self, mailData)
    ...
    ((self.ui).tex_Tile).text = mailData:GetTitle() -- DECOMPILER ERROR at PC9: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_Frome).text = mailData:GetSender()
    local ymd, hm = mailData:GetTime();
    ((self.ui).tex_Time):SetIndex(0, ymd, hm) -- DECOMPILER ERROR at PC23: Confused about usage of register: R4 in 'UnsetPending'
    ;
            ...
            if time > 3600 then
                ...
                do
                    if time > 60 then
                        ...
                        ((self.ui).tex_ExpiryDate):SetIndex(tiemType.min,
                                                            tostring(num))
                    else
                        do
                            if time > 0 then
                                ((self.ui).tex_ExpiryDate):SetIndex(
                                    tiemType.second, tostring(time))
                            end
                        end
                    end
                end
