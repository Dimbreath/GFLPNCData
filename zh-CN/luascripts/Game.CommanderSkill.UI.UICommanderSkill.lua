...
UICommanderSkill.OnInit = function(self)
    ...
    MsgCenter:AddListener(eMsgEventId.OnCommanderSkillTreeDataDiffer,
                          self.__OnTreeDataChnage)
    ...
    MsgCenter:AddListener(eMsgEventId.OnCommanderSkillMasterLevelDiffer,
                          self.__OnMasterLevelChange)
end

UICommanderSkill.InitCommanderSkillWin =
    function(self, formId)
        ...
        if fmtCSTData == nil then (FormationData.CreateDefultCST)(formId) end
        local selectedTreeId = fmtCSTData.treeId
        local selectedTreeData = nil
        for treeId, treeData in pairs(
                                    (PlayerDataCenter.CommanderSkillModualData).CommanderSkillTreeDataDic) do
        ...
        (self.treeList):InitTreeList(self.treeDataList, self.__ShowTree,
                                     self.resloader);
...
UICommanderSkill.OnTreeDataChnage = function(self, treeId)
    ...
    if treeId == self.selectedTreeId then
        ...
        local treeItem = (self.treeList):__GetTreeItemByData(
                             self.selectedTreeData)
        if treeItem ~= nil then treeItem:RefreshLevel() end
    end
end
...
UICommanderSkill.FormSelectSkillTree = function(self, saveData)
    ...
    if self.networkCtrl ~= nil then
        (self.networkCtrl):CS_COMMANDSKILL_SaveFromFormation(self.formId,
                                                             self.selectedTreeId,
                                                             saveData)
    end
end
...
UICommanderSkill.OnClickBack = function(self)
    ...
    local saveData =
        (PlayerDataCenter.CommanderSkillModualData):CreateSavingData(
            self.selectedTreeId)
    ...
    if fmtCSTData.treeId == self.selectedTreeId then
        ...
        for index, skillId in pairs(saveData.skillActive) do
            if ((fmtCSTData.saving).skillActive)[index] == nil or
                ((fmtCSTData.saving).skillActive)[index] ~= skillId then
                changed = true
            end
        end
        for id, data in pairs((fmtCSTData.saving).skillProficient) do
            if (saveData.skillProficient)[id] == nil or
                ((saveData.skillProficient)[id]).level ~= data.level then
                changed = true
            end
        end
        for id, data in pairs(saveData.skillProficient) do
            if ((fmtCSTData.saving).skillProficient)[id] == nil or
                (((fmtCSTData.saving).skillProficient)[id]).level ~= data.level then
                changed = true
            end
        end
    end
...
UICommanderSkill.OnDelete = function(self)
    ...
    self.resloader = nil
    MsgCenter:RemoveListener(eMsgEventId.OnCommanderSkillTreeDataDiffer,
                             self.__OnTreeDataChnage)
