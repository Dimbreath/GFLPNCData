...
UINCSTList.__GetTreeItemByData = function(self, treeData)
  ...
  for k,v in ipairs(self.treeDataList) do
    if v == treeData then
      local index = k - 1
      return self:__GetTreeItemGoByIndex(index)
    end
  end
end
...
UINCSTList.OnInit = function(self)
    ...
    self.treeItemDic = {}
    self.selectedTreeData = nil;
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui) -- DECOMPILER ERROR at PC16: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).treeLoopList).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC23: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).treeLoopList).onChangeItem =
        BindCallback(self, self.__OnChangeItem)
end

UINCSTList.InitTreeList = function(self, treeDataList, showDetailCallback,
                                   resloader)
    -- function num : 0_1
    self.treeDataList = treeDataList
    self.showDetailCallback = showDetailCallback
    self.resloader = resloader -- DECOMPILER ERROR at PC6: Confused about usage of register: R4 in 'UnsetPending'
    ;
    ((self.ui).treeLoopList).totalCount = #treeDataList;
    ((self.ui).treeLoopList):RefillCells()
end
...
UINCSTList.__OnNewItem = function(self, go)
    ...
    local treeItem = (UINCSTListItem.New)()
    treeItem:Init(go)
    treeItem:InitListItem(self.resloader, self.showDetailCallback) -- DECOMPILER ERROR at PC10: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.treeItemDic)[go] = treeItem
end
...
UINCSTList.__OnChangeItem = function(self, go, index)
    ...
    treeItem:RefreshListItem(treeData)
    if self.selectedTreeData == treeData then treeItem:SetSelected() end
end
...
UINCSTList.__GetTreeItemGoByIndex = function(self, index)
    -- function num : 0_6
    if (((self.ui).treeLoopList).content).childCount <= index then return nil end
    local go = ((self.ui).treeLoopList):GetCellByIndex(index)
    do
        if go ~= nil then
            local treeItem = (self.treeItemDic)[go]
...
UINCSTList.OnDelete = function(self)
    -- function num : 0_7 , upvalues : _ENV, base
    for _, item in pairs(self.treeItemDic) do item:Delete() end
    (base.OnDelete)(self)
end
