...
bs_10079.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10079_3", 1, self.OnAfterHurt)
...
bs_10079.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10079_3", 1,
                    self.OnAfterHurt) -- DECOMPILER ERROR at PC9: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).summonerFlag = true
end

bs_10079.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    -- DECOMPILER ERROR at PC20: Confused about usage of register: R8 in 'UnsetPending'

    if target.belongNum == (self.caster).belongNum and target.hp < target.maxHp *
        (self.arglist)[3] // 1000 and ((self.caster).recordTable).summonerFlag then
        ((self.caster).recordTable).summonerFlag = false
        local summoner = LuaSkillCtrl:CreateSummoner(self,
                                                     (self.config).monsterId,
                                                     target.x, target.y)
        do
            for k, skillId in pairs((self.config).monsterSkillList) do
                summoner:AddSkill(skillId, self.level)
