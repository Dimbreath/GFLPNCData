...
UINAthSuitDetailItem.InitAthSuitDetailItem =
    function(self, suitId, clickFunc, resLoader, curCount)
        -- function num : 0_1 , upvalues : _ENV
        self.suitId = suitId
        self.clickFunc = clickFunc
        local suitList = (ConfigData.ath_suit)[suitId]
        ...
        if curCount == nil then
            ...
            ((self.ui).tex_SuitCount):SetIndex(0, tostring(curCount),
                                               tostring(maxCount))
        end -- DECOMPILER ERROR at PC57: Confused about usage of register: R10 in 'UnsetPending'
        
        ((self.ui).tex_SuitName).text = (LanguageUtil.GetLocaleText)(
                                            suitParamCfg.name)
    end

UINAthSuitDetailItem.SetAthSuitDetailItemSelect =
...
UINAthSuitDetailItem.GetAthSuitDetailItemId =
    function(self)
        -- function num : 0_4
        return self.suitId
    end
