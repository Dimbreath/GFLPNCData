...
local base = UIBaseNode
local UINCSUpgradeItem = require("Game.CommanderSkill.UI.UINCSUpgradeItem")
local UINCSUpgradeSkillList = require(
                                  "Game.CommanderSkill.UI.UINCSUpgradeSkillList")
local CommanderSkillData = require("Game.CommanderSkill.CommanderSkillData")
UINCSUpgradeNode.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINCSUpgradeSkillList, UINCSUpgradeItem
    self.selectedTreeId = nil
    ...
    self.unlockableList = (UINCSUpgradeSkillList.New)();
    (self.unlockableList):Init((self.ui).obj_skillList)
    self.expItemPool = (UIItemPool.New)(UINCSUpgradeItem,
...
UINCSUpgradeNode.UpdateLevel = function(self, level)
    -- function num : 0_4
    if not self.needFresh then return end
    self.newLevel = level
    -- DECOMPILER ERROR at PC12: Confused about usage of register: R2 in 'UnsetPending'
...
UINCSUpgradeNode.RefreshConfirmBtn = function(self)
    ...
    ((self.ui).img_Confirm).color = (self.ui).color_gray -- DECOMPILER ERROR at PC9: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).img_Clear).color = (self.ui).color_gray -- DECOMPILER ERROR at PC12: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).btn_Confirm).interactable = false -- DECOMPILER ERROR at PC15: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).btn_Clear).interactable = false
    if (table.count)(self.expItemNums) ~= 0 then
        for itemId, num in pairs(self.expItemNums) do
            ...
            if num > 0 then
                ((self.ui).img_Confirm).color = (self.ui).color_orange -- DECOMPILER ERROR at PC37: Confused about usage of register: R6 in 'UnsetPending'
                ;
                ((self.ui).img_Clear).color = (self.ui).color_black -- DECOMPILER ERROR at PC40: Confused about usage of register: R6 in 'UnsetPending'
                ;
                ((self.ui).btn_Confirm).interactable = true -- DECOMPILER ERROR at PC43: Confused about usage of register: R6 in 'UnsetPending'
                ;
                ((self.ui).btn_Clear).interactable = true
            end
        end
...
UINCSUpgradeNode.CalAndRefreshExpBar = function(self)
    ...
    if maxAddExp == 0 then
        ((self.ui).img_Process).fillAmount = 0 -- DECOMPILER ERROR at PC44: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).img_RateProcess).fillAmount = 0;
        ((self.ui).tex_Exp):SetIndex(1);
        (((self.ui).tex_RateExp).gameObject):SetActive(false)
        ...
        if maxAddExp <= addExp then
            ((self.ui).img_Process).fillAmount = 0 -- DECOMPILER ERROR at PC68: Confused about usage of register: R6 in 'UnsetPending'
            ;
            ((self.ui).img_RateProcess).fillAmount = 1;
            ((self.ui).tex_Exp):SetIndex(1);
            ((self.ui).tex_RateExp):SetIndex(0, tostring(addExp))
    ...
    if curExp + addExp < requireExp then
        ((self.ui).img_Process).fillAmount = curExp / requireExp -- DECOMPILER ERROR at PC116: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).img_RateProcess).fillAmount = (curExp + addExp) / requireExp;
        ...
        end -- DECOMPILER ERROR at PC156: Confused about usage of register: R7 in 'UnsetPending'
        
        ((self.ui).img_Process).fillAmount = 0 -- DECOMPILER ERROR at PC160: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.ui).img_RateProcess).fillAmount = (remainEXP) / requireExp;
...
UINCSUpgradeNode.UpdateGrowupItem = function(self)
    ...
    if self.needFresh then
        (self.expItemPool):HideAll()
        for _, id in ipairs(
                         ((ConfigData.item).growUpIds)[eItemActionType.CommanderSkillExp]) do
            if (PlayerDataCenter.itemDic)[id] ~= nil then
                local itemData = (PlayerDataCenter.itemDic)[id]
                do
                    local item = (self.expItemPool):GetOne(true)
...
UINCSUpgradeNode.OnClickClear = function(self)
    -- function num : 0_10 , upvalues : _ENV
    for _, item in ipairs(self.expItemList) do item:CleanAll() end
    for itemData, _ in pairs(self.expItemNums) do
        -- DECOMPILER ERROR at PC13: Confused about usage of register: R6 in 'UnsetPending'
...
UINCSUpgradeNode.OnClickConfirm = function(self)
    -- function num : 0_11 , upvalues : _ENV
    if (table.count)(self.expItemNums) == 0 then return end
    local itemTab = {}
    for itemData, num in pairs(self.expItemNums) do
        itemTab[itemData.dataId] = num
    end
    if self.networkCtrl ~= nil then
        (self.networkCtrl):CS_COMMANDSKILL_Upgrade(self.selectedTreeId, itemTab,
...
UINCSUpgradeNode.RefreshUnlockSkill = function(self, level)
    ...
    if self.curLevel < level then
        ...
        do
            local LevelSkillData = self:LevelSkillData(self.curLevel,
                                                       self.curLevel + 1);
            (self.unlockableList):RefreshList(LevelSkillData)
        end
    end
...
UINCSUpgradeNode.LevelSkillData = function(self, startLevel, endLevel)
    ...
    local skillDatas = {}
    for i = startLevel + 1, endLevel do
        if (ConfigData.commander_skill_unlock)[self.selectedTreeId] ~= nil and
            ((ConfigData.commander_skill_unlock)[self.selectedTreeId])[i] ~= nil then
            for skillId, _ in pairs(
                                  ((ConfigData.commander_skill_unlock)[self.selectedTreeId])[i]) do
                local skillData =
                    ((self.treeData).commanderSkillDataDic)[skillId]
                if skillData.type ~= (CommanderSkillData.skillType).master then
...
UINCSUpgradeNode.PopUnlockSkillMessageTip =
    function(self)
        -- function num : 0_15 , upvalues : _ENV
        UIManager:ShowWindowAsync(UIWindowTypeID.MessageSide, function(window)
            -- function num : 0_15_0 , upvalues : self, _ENV
            if window == nil then return end
            if self.newLevel == self.curLevel then return end
            local LevelSkillData = self:LevelSkillData(self.curLevel,
                                                       self.newLevel)
            if #LevelSkillData == 0 then return end
            for _, data in pairs(LevelSkillData) do
                data.isNewUnlock = true
                local msg = ConfigData:GetTipContent(TipContent.CST_unlockTip)
                msg = (string.format)(msg, (data.battleSkillCfg).Name)
