...
UINSortButtonGroup.InitSortButtonGroup =
    function(self, SortMannerDefine, itemClickAction)
        -- function num : 0_1 , upvalues : _ENV, UINSortButtonItem, HeroSortEnum
        if SortMannerDefine == nil then return end
        ...
        for type, sortData in ipairs(SortMannerDefine) do
            ...
            do
                do
                    sortItem:InitSortButtonItem(type, false, self.__onItemClick) -- DECOMPILER ERROR at PC33: Confused about usage of register: R9 in 'UnsetPending'
                    ;
...
UINSortButtonGroup.__RefreshButtonGroupUI =
...
UINSortButtonGroup.__OnItemClick = function(self, sortType)
    ...
    if sortItem == nil then return end
...
UINSortButtonGroup.__GetSortFunc = function(self, selectItem)
    ...
    if self.itemClickAction ~= nil then (self.itemClickAction)(self.sortFunc) end
end

UINSortButtonGroup.OnDelete = function(self)
    -- function num : 0_5 , upvalues : _ENV
    if self.itemDic ~= nil then
        for type, item in pairs(self.itemDic) do item:OnDelete() end
        self.itemDic = nil
    end
end
