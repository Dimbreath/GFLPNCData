...
UILogin.OnInit = function(self)
    ...
    local listString = ((((CS.System).Collections).Generic).List)(
                           (CS.System).String)
    ...
    for i = 0, ((self.serverConfigAsset).ServerConfigs).Length - 1 do
        ...
        (self.serverList):Add(config.ServerName .. " - " .. config.IP .. ":" ..
                                  config.Port)
    end
    ((self.ui).serverDropdown):ClearOptions();
    ((self.ui).serverDropdown):AddOptions(self.serverList);
    (UIUtil.AddButtonListener)((self.ui).btn_SwitchAccount, self,
                               self.AccountSwitch);
    (UIUtil.AddButtonListener)((self.ui).btnLogin, self, self.OnClickLogin);
    (UIUtil.AddButtonListener)((self.ui).btnSkipLogin, self,
                               self.OnClickSkipLogin);
    (UIUtil.AddValueChangedListener)((self.ui).tog_Mode, self,
                                     self.OnServerModeChange) -- DECOMPILER ERROR at PC106: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tog_Mode).isOn = (CS_PlayerPrefsExtension.GetBool)(
                                    PlayerPrefsConsts.CustomServerMode) -- DECOMPILER ERROR at PC112: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).iptServer).text = (CS_PlayerPrefs.GetString)(
                                     PlayerPrefsConsts.CustomServerStr) -- DECOMPILER ERROR at PC118: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).serverDropdown).value = (CS_PlayerPrefs.GetInt)(
                                           PlayerPrefsConsts.ServerId) -- DECOMPILER ERROR at PC124: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).iptAcount).text = (CS_PlayerPrefs.GetString)(
                                     PlayerPrefsConsts.UserName) -- DECOMPILER ERROR at PC130: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).iptPassword).text = (CS_PlayerPrefs.GetString)(
                                       PlayerPrefsConsts.UserPassword)
...
UILogin.RequestGateInfo = function(self)
    ...
    ((CS.WaitNetworkResponse).Instance):StartWait(eCustomWaitType.WaitGateInfo,
                                                  eCustomWaitType.WaitGateInfo)
    CS_MicaSDKManager_Ins:GetGateInfo(function(success)
        -- function num : 0_1_0 , upvalues : _ENV, self
        ((CS.WaitNetworkResponse).Instance):RemoveWait(
            eCustomWaitType.WaitGateInfo)
        if not success then
            ((CS.MessageCommon).ShowMessageBoxConfirm)(
                ConfigData:GetTipContent(TipContent.not_get_gateinfo),
                self.__requestGateInfo)
        end
    end)
end
...
UILogin.AccountSwitch = function(self)
    ...
    if not isUseSDK then return end
    CS_MicaSDKManager_Ins:AccountSwitch(function()
        ...
        ((CS.WaitNetworkResponse).Instance):StartWait(
            eCustomWaitType.WaitCheckVersion, eCustomWaitType.WaitCheckVersion);
        (CS_HotUpdateController.TryCheckNewResVersion)(
...
UILogin.OnClickLogin = function(self)
    ...
    (CS_PlayerPrefs.SetInt)(PlayerPrefsConsts.ServerId,
                            ((self.ui).serverDropdown).value)
    local userName = ((self.ui).iptAcount).text;
    (CS_PlayerPrefs.SetString)(PlayerPrefsConsts.UserName, userName);
    (CS_PlayerPrefs.SetString)(PlayerPrefsConsts.UserPassword,
                               ((self.ui).iptPassword).text);
    (CS_PlayerPrefsExtension.SetBool)(PlayerPrefsConsts.CustomServerMode,
                                      ((self.ui).tog_Mode).isOn);
    (CS_PlayerPrefs.SetString)(PlayerPrefsConsts.CustomServerStr,
                               ((self.ui).iptServer).text)
    if not isUseSDK then
        if (string.IsNullOrEmpty)(userName) then
            ((CS.MessageCommon).ShowMessageTips)(
                ConfigData:GetTipContent(TipContent.login_NoticeUserName))
            return
        end
        self:StartConnect()
    else
        CS_MicaSDKManager_Ins:StartLogin(
            function()
                -- function num : 0_4_0 , upvalues : _ENV, CS_HotUpdateController, self
                ((CS.WaitNetworkResponse).Instance):StartWait(
                    eCustomWaitType.WaitCheckVersion,
                    eCustomWaitType.WaitCheckVersion);
                (CS_HotUpdateController.TryCheckNewResVersion)(
                    function(hasNewVersion)
...
UILogin.__OnCheckVersionComplete = function(self, hasNewVersion)
    ...
    if hasNewVersion then
        ((CS.MessageCommon).ShowMessageBoxConfirm)(
            ConfigData:GetTipContent(TipContent.HasNewVersionToUpdate),
            function()
...
UILogin.StartConnectFromSDK = function(self)
    ...
    if ok and isUseSDK then
        ...
        do
            ...
            ((CS.MessageCommon).ShowMessageTips)(
                ConfigData:GetTipContent(TipContent.login_IllegalServer))
            do return end
            ((CS.WaitNetworkResponse).Instance):StartWait(
                eCustomWaitType.WaitGateInfo, eCustomWaitType.WaitGateInfo)
            CS_MicaSDKManager_Ins:GetGateInfo(
                function(success)
...
UILogin.StartConnect = function(self, gateInfo)
    ...
    if not ((self.ui).tog_Mode).isOn then
        ...
        if isGameDev then print("[Dev]server:", ip, port) end
        ((CS.NetworkEntry).Instance):StartConnect(ip, port)
    else
        do
...
UILogin.OnConnectComplete = function(self)
    ...
    if isUseSDK then
        (self.LoginNetworkCtrl):SendLogin(CS_MicaSDKManager_Ins.openId,
                                          CS_MicaSDKManager_Ins.accessToken)
    else
...
UILogin.OnRecvUserData = function(self)
    ...
    ((CS.GSceneManager).Instance):LoginLoading((Consts.SceneName).Main,
                                               loadedFunc,
