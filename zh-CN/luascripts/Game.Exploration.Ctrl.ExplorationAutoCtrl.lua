...
ExplorationAutoCtrl.ctor = function(self, epCtrl)
    ...
    self.__onEpSceneStateChanged =
        BindCallback(self, self.OnEpSceneStateChanged)
    MsgCenter:AddListener(eMsgEventId.OnEpSceneStateChanged,
                          self.__onEpSceneStateChanged)
    self.__onEpBattleReady = BindCallback(self, self.OnEpBattleReady)
    MsgCenter:AddListener(eMsgEventId.OnBattleReady, self.__onEpBattleReady)
    self.__onEpExitRoomComplete = BindCallback(self, self.OnEpExitRoomComplete)
    MsgCenter:AddListener(eMsgEventId.OnExitRoomComplete,
                          self.__onEpExitRoomComplete)
    self.__onEnterEpChipDiscard = BindCallback(self, self.OnEnterEpChipDiscard)
    MsgCenter:AddListener(eMsgEventId.OnChipDiscardChanged,
                          self.__onEnterEpChipDiscard)
end
...
ExplorationAutoCtrl.EnableEpAutoMode = function(self)
    -- function num : 0_6 , upvalues : _ENV
    if ((ExplorationManager:GetDynPlayer()):GetOperatorDetail()).state ~=
    ...
    local originFightPower =
        (ExplorationManager:GetDynPlayer()):GetMirrorTeamFightPower(true, false)
    local recommendPower = (ExplorationManager:GetSectorStageCfg()).combat
    if originFightPower < recommendPower then
        ((CS.MessageCommon).ShowMessageBox)(ConfigData:GetTipContent(280),
                                            ConfigData:GetTipContent(281),
                                            ConfigData:GetTipContent(282),
                                            function()
            -- function num : 0_6_0 , upvalues : self
            self:__RealEnableEpAutoMode()
        end, nil)
...
ExplorationAutoCtrl.__RealEnableEpAutoMode =
    function(self, isRunning)
        -- function num : 0_7 , upvalues : _ENV
        self.__enableAutoMode = true
        self:CalcAutoEpPath()
        local curRoomData = (self.epCtrl):GetCurrentRoomData();
        ((self.epCtrl).mapCtrl):RefreshMapShowState(
            (ExplorationManager:GetDynPlayer()):GetOperatorDetail(), curRoomData)
        local explorationWindow =
            UIManager:GetWindow(UIWindowTypeID.Exploration)
        if explorationWindow ~= nil then
            explorationWindow:RefreshAutoModeState(true, isRunning)
        end
    end
...
ExplorationAutoCtrl.CloseAutoMode = function(self)
    -- function num : 0_9
    if not self:IsAutoModeRunning() then return end
    self:__ClearAutoData()
end
...
ExplorationAutoCtrl.OnExplorationStart =
    function(self)
        -- function num : 0_11
        if not self:IsDefaultAutoEp() then return end
        if self:IsEnableAutoMode() then return end
        self:EnableEpAutoMode()
        self:StartOrStopEpAutoMode()
    end

ExplorationAutoCtrl.StartOrStopEpAutoMode =
    function(self)
        -- function num : 0_12 , upvalues : _ENV
        if not self:IsEnableAutoMode() then return false, false end
        if self:IsAutoModeRunning() then
            self:DisableEpAutoMode()
            return false, false
        end
        local aliveRoleCount =
            (ExplorationManager:GetDynPlayer()):GetDeployAliveHeroCount()
        if aliveRoleCount <= 0 then
            ((CS.MessageCommon).ShowMessageTips)(ConfigData:GetTipContent(284))
            return true, false
        end
...
ExplorationAutoCtrl.BreakAutoMode = function(self)
    ...
    if self.__autoWaitTimer ~= nil then
        ...
        self.__autoWaitTimer = nil
        self.__isBreakAutoMode = true;
        (self.epAutoWindow):SetAutoTitleState(
            (ExplorationEnum.eAutoTitleType).Normal)
        return true
    end
    return false
end

ExplorationAutoCtrl.BreakAutoModeForce =
    function(self)
        -- function num : 0_14 , upvalues : ExplorationEnum
        if not self:IsAutoModeRunning() then return end
        self.__isBreakAutoMode = true
        if self.epAutoWindow == nil then return end
        (self.epAutoWindow):SetAutoTitleState(
            (ExplorationEnum.eAutoTitleType).Normal);
        (self.epAutoWindow):SetAutoTitleActive(false)
    end
...
ExplorationAutoCtrl.OnEpExitRoomComplete =
    function(self, exitFromType)
        -- function num : 0_16 , upvalues : ExplorationEnum
        if exitFromType == nil or exitFromType ==
            (ExplorationEnum.eExitRoomCompleteType).BattleToEp then
            return
        end
        self:OnAutoStageOver()
    end

ExplorationAutoCtrl.OnEpOpStateChanged =
    function(self, opDetail)
        -- function num : 0_17 , upvalues : _ENV
        if self:IsAutoModeRunning() and opDetail.state ==
            proto_object_ExplorationCurGridState.ExplorationCurGridState_Over then
            for k, position in pairs(self.__autoPath) do
                if position == opDetail.curPostion and k < #self.__autoPath then
                    local nextRoomPos = (self.__autoPath)[k + 1]
...
ExplorationAutoCtrl.OnEpSceneStateChanged =
    function(self, inbattleScene)
        -- function num : 0_18
        if not self:IsAutoModeRunning() then return end
        if not inbattleScene or self.__waitSelectRoom ~= nil then
            local nextRoomPos = self.__waitSelectRoom
...
ExplorationAutoCtrl.__StartSelectRoomTimer =
    function(self, nextRoomPos)
        -- function num : 0_19 , upvalues : _ENV, ExplorationEnum
        self.__autoTime = ((ConfigData.game_config).autoEpTime).selectRoom;
        (self.epAutoWindow):SetAutoTitleState(
            (ExplorationEnum.eAutoTitleType).SelectRoom, self.__autoTime)
        self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
            -- function num : 0_19_0 , upvalues : self, ExplorationEnum, _ENV, nextRoomPos
            self.__autoTime = self.__autoTime - 1
            if self.__autoTime > 0 then
            ...
            (self.epAutoWindow):SetAutoTitleState(
                (ExplorationEnum.eAutoTitleType).Normal)
        end, nil, false, false, false)):Start()
    end

ExplorationAutoCtrl.OnEpBattleReady = function(self)
    -- function num : 0_20 , upvalues : _ENV
    if not self:IsAutoModeRunning() then return end
    if self.__isBreakAutoMode then return end
    self.__autoWaitTimer = (TimerManager:GetTimer(
                               ((ConfigData.game_config).autoEpTime).meaningless,
                               function()
            -- function num : 0_20_0 , upvalues : self
            if self.__autoWaitTimer ~= nil then
                (self.__autoWaitTimer):Stop()
            ...
            ((self.epCtrl).battleCtrl):EnterEpBattleRunning()
        end, nil, true, false, false)):Start()
end

ExplorationAutoCtrl.OnEpBattleResultShow =
    function(self)
        -- function num : 0_21 , upvalues : _ENV
        if not self:IsAutoModeRunning() then return end
        (self.epAutoWindow):SetAutoMaskActive(true)
        self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
            ...
            local battleResultWindow = UIManager:GetWindow(
...
ExplorationAutoCtrl.OnEpBattleSelectChip =
    function(self)
        -- function num : 0_22 , upvalues : _ENV
        if not self:IsAutoModeRunning() then return end
        local selectChipWindow = UIManager:GetWindow(UIWindowTypeID.SelectChip)
        if selectChipWindow == nil then return end
        local chipPanel = selectChipWindow:GetMaxPowerChipPanel()
        local autoChipHolder = chipPanel:GetAutoTipsHolder();
        (self.epAutoWindow):SetAutoOperatorActive(true, autoChipHolder)
        self.__autoTime = ((ConfigData.game_config).autoEpTime).roomOperator;
...
ExplorationAutoCtrl.OnEnterEpTreasureRoom =
    function(self)
        -- function num : 0_23 , upvalues : _ENV
        if not self:IsAutoModeRunning() then return end
        (self.epAutoWindow):SetAutoMaskActive(true)
        self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
            -- function num : 0_23_0 , upvalues : self, _ENV
            self.__autoWaitTimer = nil
            local treasureWindow = UIManager:GetWindow(
                                       UIWindowTypeID.EpTreasureRoom)
            if treasureWindow == nil then return end
            local chipPanel = treasureWindow:GetMaxPowerChipPanel()
            local autoChipHolder = chipPanel:GetAutoTipsHolder();
            (self.epAutoWindow):SetAutoOperatorActive(true, autoChipHolder)
            self.__autoTime = ((ConfigData.game_config).autoEpTime).roomOperator;
        ...
        end, nil, true, true, false)):Start()
    end

ExplorationAutoCtrl.OnEnterEpStoreRoom =
    function(self)
        -- function num : 0_24 , upvalues : _ENV, ExplorationEnum
        if not self:IsAutoModeRunning() then return end
        local storeWindow = UIManager:GetWindow(UIWindowTypeID.EpStoreRoom)
        if storeWindow == nil then return end
        self.__autoTime = ((ConfigData.game_config).autoEpTime).roomOperator;
        (self.epAutoWindow):SetAutoTitleState(
            (ExplorationEnum.eAutoTitleType).ExitRoom, self.__autoTime);
        (self.epAutoWindow):SetAutoMaskActive(true)
        self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
            -- function num : 0_24_0 , upvalues : self, ExplorationEnum
            self.__autoTime = self.__autoTime - 1
...
ExplorationAutoCtrl.OnEnterEpEventRoom =
    function(self, eventRoomData, isFirstOpen)
        -- function num : 0_25 , upvalues : _ENV, ExplorationEnum
        if not self:IsAutoModeRunning() then return end
        if self.__isBreakAutoMode then return end
        (self.epAutoWindow):SetAutoMaskActive(true)
        self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
            ...
            self.__autoWaitTimer = nil
            local eventWindow = UIManager:GetWindow(UIWindowTypeID.EpEventRoom)
            if eventWindow == nil then return end
            local ableChoiceList = {}
            local autoChoiceList = {}
            local choiceDatalist = eventRoomData.choiceDatalist
            for _, choiceData in pairs(choiceDatalist) do
                if choiceData.isAble then
                    ...
                    local choiceCfg = nil
                    if choiceData.catId ==
                        (ExplorationEnum.eEventRoomChoiceType).Normal then
                        choiceCfg =
                            (ConfigData.event_choice)[choiceData.choiceId]
                    else
                        if choiceData.catId ==
                            (ExplorationEnum.eEventRoomChoiceType).Upgrade then
                            choiceCfg =
                                (ConfigData.event_upgrade)[choiceData.choiceId]
                        end
                    end
                    if choiceCfg ~= nil and choiceCfg.auto_choice_type ~= 0 then
                        local dynPlayer = ExplorationManager:GetDynPlayer()
                        local autoSuccess = false
                        if choiceCfg.auto_choice_type ==
                            (ExplorationEnum.eAutoEventChoiceType).AllRoleHpLess then
                            local hp = (choiceCfg.auto_choice_arg)[1] * 10
                            autoSuccess = true
                            for _, dynHero in ipairs(dynPlayer.heroList) do
                                if hp < dynHero.hpPer then
                            ...
                            do
                                if choiceCfg.auto_choice_type ==
                                    (ExplorationEnum.eAutoEventChoiceType).OneRoleHpLess then
                                    local hp =
                                        (choiceCfg.auto_choice_arg)[1] * 10
                                    for _, dynHero in ipairs(dynPlayer.heroList) do
                                        if dynHero.hpPer <= hp then
                                            autoSuccess = true
                                            break
                                    ...
                                    do
                                        do
                                            if choiceCfg.auto_choice_type ==
                                                (ExplorationEnum.eAutoEventChoiceType).HasItemCount and
                                                (choiceCfg.auto_choice_arg)[2] <=
                                                dynPlayer:GetItemCount(
                                                    (choiceCfg.auto_choice_arg)[1]) then
                                                autoSuccess = true
                                            end
                                            if autoSuccess then
                                                (table.insert)(autoChoiceList,
                                                               choiceData)
                                            end
                                            -- DECOMPILER ERROR at PC115: LeaveBlock: unexpected jumping out DO_STMT
            ...
            if #autoChoiceList > 0 then
                resultIndex =
                    (autoChoiceList[(math.random)(#autoChoiceList)]).idx + 1
            else
                resultIndex =
                    (ableChoiceList[(math.random)(#ableChoiceList)]).idx + 1
            end
            if resultIndex == 0 then return end
            local choiceItem = eventWindow:GetEventChoiceItem(resultIndex)
            if choiceItem == nil then return end
            local choiceData = choiceDatalist[resultIndex]
            local choiceCfg = nil
            if choiceData.catId == (ExplorationEnum.eEventRoomChoiceType).Normal then
                choiceCfg = (ConfigData.event_choice)[choiceData.choiceId]
            ...
            self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
                ...
                ((self.epCtrl).eventCtrl):OnChoiceItemClick(choiceCfg,
                                                            resultIndex - 1,
                                                            true);
                (self.epAutoWindow):SetAutoMaskActive(false);
                (self.epAutoWindow):SetAutoOperatorActive(false)
...
ExplorationAutoCtrl.OnEnterEpEventRoomUpgrade =
    function(self, eventRoomData, isFirstOpen)
        -- function num : 0_26 , upvalues : _ENV, ExplorationEnum
        if not self:IsAutoModeRunning() then return end
        if self.__isBreakAutoMode then return end
        if isFirstOpen then
            self.__autoTime = ((ConfigData.game_config).autoEpTime).roomOperator;
            (self.epAutoWindow):SetAutoTitleState(
                (ExplorationEnum.eAutoTitleType).UpgradeChip, self.__autoTime);
            (self.epAutoWindow):SetAutoMaskActive(true)
            self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
                -- function num : 0_26_0 , upvalues : self, ExplorationEnum
                self.__autoTime = self.__autoTime - 1
                if self.__autoTime > 0 then
                    (self.epAutoWindow):SetAutoTitleState(
                        (ExplorationEnum.eAutoTitleType).UpgradeChip,
                        self.__autoTime)
                    return
                end
                if self.__autoWaitTimer ~= nil then
...
ExplorationAutoCtrl.AutoEpEventRoomUpgradeLogic =
    function(self)
        -- function num : 0_27 , upvalues : _ENV
        local epUpgradeWindow =
            UIManager:GetWindow(UIWindowTypeID.EpUpgradeRoom)
        if epUpgradeWindow == nil then return end
        if not epUpgradeWindow:CheckRefreshTimeEnough() then return end
        local dynPlayer = ExplorationManager:GetDynPlayer()
        local currencyNum = dynPlayer:GetItemCount(
                                (epUpgradeWindow.cfg).currency)
        local upgradePrice = ConfigData:CalculateEpChipUpgradePrice(
                                 epUpgradeWindow.roomId,
                                 epUpgradeWindow.refreshTime)
        if currencyNum < upgradePrice then
            ((self.epCtrl).eventCtrl):SendSpecifyExit()
            return
        end
        ...
        for k, tmpChipData in pairs(chipDataList) do
            if not tmpChipData:IsChipFullLevel() then
                local curFightPower = dynPlayer:GetChipCombatEffect(tmpChipData,
                                                                    true)
                if fightPower < curFightPower then
                    fightPower = curFightPower
                    chipData = tmpChipData
...
ExplorationAutoCtrl.OnEnterEpChipDiscard =
    function(self, isFirstOpen)
        -- function num : 0_28 , upvalues : _ENV, ExplorationEnum
        if not self:IsAutoModeRunning() then return end
        if self.__autoWaitTimer ~= nil then return end
        if self.__isBreakAutoMode and not isFirstOpen then return end
        if isFirstOpen then
            self.__autoTime = ((ConfigData.game_config).autoEpTime).roomOperator;
            (self.epAutoWindow):SetAutoTitleState(
                (ExplorationEnum.eAutoTitleType).DiscardChip, self.__autoTime);
            (self.epAutoWindow):SetAutoMaskActive(true)
            self.__autoWaitTimer = (TimerManager:GetTimer(1, function()
                -- function num : 0_28_0 , upvalues : self, ExplorationEnum
                self.__autoTime = self.__autoTime - 1
                if self.__autoTime > 0 then
                    (self.epAutoWindow):SetAutoTitleState(
                        (ExplorationEnum.eAutoTitleType).DiscardChip,
                        self.__autoTime)
                    return
                end
                if self.__autoWaitTimer ~= nil then
...
ExplorationAutoCtrl.AutoEpChipDiscardLogic =
    function(self)
        -- function num : 0_29 , upvalues : _ENV
        local chipDiscardWindow = UIManager:GetWindow(
                                      UIWindowTypeID.EpChipDiscard)
        if chipDiscardWindow == nil then return end
        local dynPlayer = ExplorationManager:GetDynPlayer()
        local isOverLimit, nowCount, nowLimit = dynPlayer:IsChipOverLimitNum()
        if not isOverLimit then
            if ((CS.WaitNetworkResponse).Instance):ContainWait(
                proto_csmsg_MSG_ID.MSG_CS_EXPLORATION_AlgUpperLimit_Exit) then
                return
            end
            chipDiscardWindow:CloseEpDiscard()
            return
        ...
        do
            ...
            local currentItemNum = dynPlayer:GetItemCount(
                                       chipDiscardWindow.costItemId)
            if chipDiscardWindow.costItemNum <= currentItemNum then
                chipDiscardWindow:AddChipLimit()
                return
            end
            ...
            for k, tmpRoomData in pairs(nextRoom) do
                ...
                if priorRoom == nil then
                    ...
                    if curPriority < priorValue then
                        ...
                        if curPriority == priorValue and
                            (curFightPower < priorFightPower or curFightPower ~=
                                priorFightPower or tmpRoomData.y < priorRoom.y) then
                            priorRoom = tmpRoomData
                            priorValue = curPriority
                        end
                    end
...
ExplorationAutoCtrl.CheckAutoModeRoomClick =
    function(self, keyRoom)
        -- function num : 0_32 , upvalues : _ENV
        if not self:IsEnableAutoMode() then return false end
        if self:IsAutoModeRunning() then return true end
        local keyIndex = nil
        local newAutoPath = {}
        for k, position in pairs(self.__autoPath) do
            local x, y = (ExplorationManager.Coordination2Pos)(position)
            if x == keyRoom.x and y ~= keyRoom.y then keyIndex = k end
            newAutoPath[k] = position
        end
        if keyIndex == nil then return true end
        newAutoPath[keyIndex] = keyRoom.position
        local curRoomData = keyRoom
        for i = keyIndex - 1, 1, -1 do
            local roomList = (curRoomData:GetLastRoom())
            ...
            if cRoom == nil then return true end
            curRoomData = cRoom
        end
        if newAutoPath[1] ~= (self.__autoPath)[1] then return true end
        curRoomData = keyRoom
        for i = keyIndex + 1, #newAutoPath do
            local roomList = (curRoomData:GetNextRoom())
            local cRoom = nil
...
ExplorationAutoCtrl.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.OnExitRoomComplete,
                             self.__onEpExitRoomComplete)
    MsgCenter:RemoveListener(eMsgEventId.OnChipDiscardChanged,
                             self.__onEnterEpChipDiscard)
    self:__ClearAutoData();
    (base.OnDelete)(self)
end
