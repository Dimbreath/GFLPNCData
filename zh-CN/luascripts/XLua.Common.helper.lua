...
local list_iter = function(cs_ilist, index)
    -- function num : 0_7
    index = index + 1
    if index < cs_ilist.Count then return index, cs_ilist[index] end
end
...
local hotfix_safe = function(cs, field, func)
    ...
    local func_safe = function(self, ...)
        ...
        (table.remove)(ret, 1)
        if not status and self then
            error("hotfix_safe " .. field ..
                      " failed(call cs func instead) with err : " .. err)
            local cs_func = load("return function(self, ...) self:" .. field ..
                                     "(...) end")
            ret = {(cs_func())(self, ...)}
        end
        do
