...
bs_10006.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10006_1", 1,
                    self.OnAfterHurt)
    self.heal_config = {}
end

bs_10006.OnAfterHurt = function(self, sender, target, skill, realValue, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack and
        target:ContainFeature((self.config).featureType) then
        self:PlayChipEffect()
        if (self.caster).hp <= 0 then return end
