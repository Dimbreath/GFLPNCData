...
UIClickContinue.InitContinue = function(self, clickEvent, showText, textIndex,
                                        bgColor, clickClose)
    -- function num : 0_1
    ...
    if not showText then showText = false end
    self.clickEvent = clickEvent;
    (((self.ui).text).gameObject):SetActive(showText)
    if showText then ((self.ui).text):SetIndex(textIndex) end
    -- DECOMPILER ERROR at PC27: Confused about usage of register: R6 in 'UnsetPending'

    if bgColor ~= nil then ((self.ui).bgImage).color = bgColor end
...
UIClickContinue.OnClickBg = function(self)
    ...
    if self.clickClose then self:Delete() end
end
