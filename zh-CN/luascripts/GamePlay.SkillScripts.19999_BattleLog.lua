...
bs_19999.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
...
bs_19999.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_19999_1", 1,
                    self.OnAfterBattleStart)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_19999_3", 1,
                    self.OnAfterHurt)
    self:AddTrigger(eSkillTriggerType.AfterHeal, "bs_19999_5", 1,
                    self.OnAfterHeal)
    self:AddTrigger(eSkillTriggerType.AfterAddBuff, "bs_19999_7", 1,
                    self.OnAfterAddBuff)
    self:AddTrigger(eSkillTriggerType.RoleDie, "bs_19999_10", 1, self.OnRoleDie)
    self:AddTrigger(eSkillTriggerType.AfterPlaySkill, "bs_19999_12", 1,
...
bs_19999.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_3 , upvalues : _ENV
    if sender.belongNum == 1 and target.belongNum == 2 then
        file:write("伤害 " .. sender.name .. " " .. tostring(skill.dataId) ..
                       " " .. tostring(hurt) .. " " .. tostring(isMiss) .. " " ..
                       tostring(isCrit) .. "\n")
    end
end
...
bs_19999.OnAfterHeal = function(self, sender, target, skill, heal)
    -- function num : 0_4 , upvalues : _ENV
    if sender.belongNum == 1 and target.belongNum == 1 then
        file:write("治疗 " .. sender.name .. " " .. tostring(skill.dataId) ..
                       " " .. tostring(heal) .. "\n")
    end
end
...
bs_19999.OnRoleDie = function(self, killer, role)
    -- function num : 0_6 , upvalues : _ENV
    if role.belongNum == 1 then
        file:write("死亡 " .. role.name ..
                       tostring((LuaSkillCtrl.battleCtrl).frame) .. "\n")
    end
end
