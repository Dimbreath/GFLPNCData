...
local DefineConfigDataFunc = function()
    ...
    (((ConfigData.item).__basemetatable).__index).GetHeroResCfg =
        function(self)
            -- function num : 0_0_0 , upvalues : _ENV
            if self.action_type == eItemActionType.HeroCard or self.action_type ==
                eItemActionType.HeroCardFrag then
                local heroId = (self.arg)[1]
                local heroCfg = (ConfigData.hero_data)[heroId]
                if heroCfg == nil then return heroCfg end
                local resCfg = (ConfigData.resource_model)[heroCfg.src_id]
                return resCfg
            end
            do return nil end
        end -- DECOMPILER ERROR at PC11: Confused about usage of register: R0 in 'UnsetPending'
    ;
    (((ConfigData.hero_data).__basemetatable).__index).GetHeroResCfg =
        function(self)
            -- function num : 0_0_1 , upvalues : _ENV
            local resCfg = (ConfigData.resource_model)[self.src_id]
            return resCfg
        end -- DECOMPILER ERROR at PC17: Confused about usage of register: R0 in 'UnsetPending'
    ;
    (((ConfigData.camp_connection).__basemetatable).__index).GetDescribe =
        function(self)
            -- function num : 0_0_2 , upvalues : _ENV
            return (GR.StringFormatIntArray)(
                       (LanguageUtil.GetLocaleText)(self.text), self.para)
        end -- DECOMPILER ERROR at PC23: Confused about usage of register: R0 in 'UnsetPending'
    ;
    (((ConfigData.guide).__basemetatable).__index).GetFirstCondition =
        function(self)
            -- function num : 0_0_3 , upvalues : _ENV
            local firstId = (self.step_list)[1]
            if firstId == nil then return 0 end
            local stepCfg = (ConfigData.guide_step)[firstId]
            if stepCfg == nil then
                error("guide step cfg is null,id:" .. tostring(firstId))
                return 0
...
InitSectorStageAvg = function()
    ...
    (ConfigData.sector).avgUnlockDic = avgUnlockDic -- DECOMPILER ERROR at PC47: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (ConfigData.sector).stageUnlockDic = stageUnlockDic
end
...
ConfigData.InitConfigData = function(self)
    ...
    DefineConfigDataFunc()
    InitSectorStageAvg()
    self.buildinConfig = require("Game.ConfigData.BuildinConfig") -- DECOMPILER ERROR at PC16: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.game_config).heroMaxCamp = (table.count)(ConfigData.camp) -- DECOMPILER ERROR at PC23: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.game_config).heroMaxCareer = (table.count)(ConfigData.career)
end
...
ConfigData.ReleaseDynCfg = function(self, type)
    ...
    if (self.DynConfigNum)[type] <= 0 then
        ConfigData[type] = nil -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (package.loaded)["LuaConfigs." .. type] = nil -- DECOMPILER ERROR at PC21: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (self.DynConfigNum)[type] = nil
    end
end
...
ConfigData.ForceReleaseDynCfg = function(self, type)
    ...
    ConfigData[type] = nil -- DECOMPILER ERROR at PC7: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (package.loaded)["LuaConfigs." .. type] = nil
end
...
ConfigData.GetCampFetter = function(self, camp, number)
    -- function num : 0_6
    local fetterList = ((self.camp_connection).fetterList)[camp]
    if fetterList == nil then return nil end
    for i = #fetterList, 1, -1 do
        local curNum = fetterList[i]
        if curNum <= number then
            return ((self.camp_connection)[camp])[curNum]
...
ConfigData.GetCareerIcon = function(self, career)
    -- function num : 0_7
    local careerCfg = (self.career)[career]
    if careerCfg == nil then return nil end
    return careerCfg.icon
end
...
ConfigData.GetCampIcon = function(self, camp)
    -- function num : 0_8
    local campCfg = (self.camp)[camp]
    if campCfg == nil then return nil end
    return campCfg.icon
end
...
ConfigData.GetTipContent = function(self, id)
    ...
    if tipCfg == nil then
        return (LanguageUtil.GetLocaleText)(
                   ((ConfigData.tip_language)[1]).content)
    end
    return (LanguageUtil.GetLocaleText)(tipCfg.content)
end
...
ConfigData.GetFormationHeroCount = function(self)
    ...
    for i = 1, (ConfigData.game_config).max_bench_hero do
        local sysFuncId =
            proto_csmsg_SystemFunctionID["SystemFunctionID_bench" .. tostring(i)]
        local unlock = funcUnLockCrtl:ValidateUnlock(sysFuncId)
        if unlock then count = count + 1 end
    end
    return count
end
...
ConfigData.GetChipinfluenceIntro = function(self, chipId)
    -- function num : 0_15 , upvalues : _ENV
    local chipCfg = (ConfigData.chip)[chipId]
    if chipCfg == nil then return "" end
    local introCfg1 = (ConfigData.chip_intro)[chipCfg.influence]
    if introCfg1 == nil then return "" end
    local intro_type =
        ((ConfigData.chip_intro).influence_type)[chipCfg.influence]
    local content = ""
    if intro_type == 0 then
        local introCfg2 = introCfg1[chipCfg.arg]
        if introCfg2 ~= nil then
            ...
            if intro_type == 1 then
                ...
                if careerCfg ~= nil and introCfg2 ~= nil then
                    content = (string.format)(
                                  (LanguageUtil.GetLocaleText)(introCfg2.intro),
...
ConfigData.CalculateEpChipSalePrice = function(self, epRoomId, chipLvl,
                                               chipPrice)
    -- function num : 0_17 , upvalues : _ENV
    local levels = ((ConfigData.exploration_shop)[epRoomId]).discount_level
    local disCount = ((ConfigData.exploration_shop)[epRoomId]).discount_scale
...
ConfigData.CalculateEpChipUpgradePrice =
    function(self, epRoomId, refreshTimes)
        -- function num : 0_18 , upvalues : _ENV
        local currentTime = refreshTimes + 1
        local refreshTimeCfg =
            ((ConfigData.event_upgrade)[epRoomId]).refresh_times
        local price = ((ConfigData.event_upgrade)[epRoomId]).prices
        local refreshTimeCfgCount = #refreshTimeCfg
        for i = 1, refreshTimeCfgCount do
            -- DECOMPILER ERROR at PC20: Unhandled construct in 'MakeBoolean' P1
            ...
            if refreshTimeCfg[i - 1] < currentTime and currentTime <=
                refreshTimeCfg[i] then return price[i] end
        end
        return price[refreshTimeCfgCount]
    end
