...
CheckerInfinityDungeon.IsFulfillInfinityLevel =
    function(infinitySategId, needLevel)
        -- function num : 0_1 , upvalues : _ENV
        if ((PlayerDataCenter.infinityData).completed)[infinitySategId] == nil then
            do return needLevel ~= -1 end
            do
                local processing =
                    ((PlayerDataCenter.infinityData).processingUpdate)[infinitySategId]
...
CheckerInfinityDungeon.GetUnlockInfo = function(param)
    ...
    if needLevel == -1 then
        ...
        return (string.format)("通关%d扇区%dm第%d层解锁", sectorId,
                               index * 10, needLevel)
    end
