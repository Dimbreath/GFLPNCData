...
LoginNetWorkCtrl.InitNetwork = function(self)
  ...
  self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_Login, self, proto_csmsg.SC_Login, self.OnRecvLogin)
...
LoginNetWorkCtrl.SendLogin = function(self, osdkUserId, token)
  ...
  ;
  ((CS.WaitNetworkResponse).Instance):StartWait(proto_csmsg_MSG_ID.MSG_CS_Login, proto_csmsg_MSG_ID.MSG_SC_Login)
end

LoginNetWorkCtrl.OnRecvLogin = function(self, msg)
  -- function num : 0_3 , upvalues : _ENV, cs_MessageCommon
  if msg.ret ~= proto_csmsg_ErrorCode.LOGIN_USER_NOT_FOUNT or msg.ret == proto_csmsg_ErrorCode.None then
