...
LoginNetWorkCtrl.InitNetwork = function(self)
    ...
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_Login, self,
                         proto_csmsg.SC_Login, self.SC_Login)
...
LoginNetWorkCtrl.CS_Login = function(self, osdkUserId, token, callBack)
    ...
    ((CS.WaitNetworkResponse).Instance):StartWait(
        proto_csmsg_MSG_ID.MSG_CS_Login, callBack,
        proto_csmsg_MSG_ID.MSG_SC_Login)
end
...
LoginNetWorkCtrl.SC_Login = function(self, msg)
    ...
    if msg.ret == proto_csmsg_ErrorCode.None then
        ...
        ((CS.WaitNetworkResponse).Instance):AddWaitData(
            proto_csmsg_MSG_ID.MSG_CS_Login, true)
    else
        local err = "LoginNetWorkCtrl:OnRecvLogin error:" .. tostring(msg.ret)
        warn(err)
        if isGameDev then (cs_MessageCommon.ShowMessageTips)(err) end
        ((CS.WaitNetworkResponse).Instance):AddWaitData(
            proto_csmsg_MSG_ID.MSG_CS_Login, false)
    end
end
...
LoginNetWorkCtrl.SC_Logout = function(self, msg)
    ...
    do
        ...
        if reason == proto_csmsg_UserStatus.StatusInvalidGuestToken or reason ==
            proto_csmsg_UserStatus.StatusInvalidUserToken then
            ((CS.NetworkManager).Instance):CloseAndQuitToLogin(
                ConfigData:GetTipContent(402))
            return
        end
        ((CS.NetworkManager).Instance):CloseAndQuitToLogin(
