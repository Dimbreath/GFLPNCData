...
SectorTaskController.OpenSectorTaskWindows =
    function(self)
        -- function num : 0_1 , upvalues : _ENV
        if not self.isInit then return end
        if self.uiWindow == nil then
            UIManager:ShowWindowAsync(UIWindowTypeID.SectorTask,
                                      function(window)
...
SectorTaskController.InitCurrSectorData =
    function(self, sectorId, sectorCfg)
        ...
        for k, Data in pairs(self.originDataDic) do
            ...
            end -- DECOMPILER ERROR at PC38: Confused about usage of register: R8 in 'UnsetPending'
            
            (self.taskDataDic)[k] = Data
        end
        self:__OnSectorAchivUpdate(sectorId,
                                   (PlayerDataCenter.sectorDatas)[sectorId])
        self.isInit = true
    end
...
SectorTaskController.OnTaskItemClick = function(self, taskId)
    ...
    if taskData == nil then error("Can`t Find taskData :ID = " .. taskId) end
...
SectorTaskController.__OnTaskCommit = function(self, taskCfg)
    ...
    self:__OnSectorAchivUpdate(self.sectorId,
...
SectorTaskController.OnAchivItemClick = function(self, achivId)
    ...
    if achivData == nil then error("Can`t Find achivData :ID = " .. achivId) end
...
SectorTaskController.__OnPickedAchivItem =
    function(self, sectorId, id)
        ...
        UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
            -- function num : 0_8_0 , upvalues : rewardIds, rewardNums
            if window == nil then return end
            window:InitRewardsItem(rewardIds, rewardNums)
        end)
    end

SectorTaskController.__OnSectorAchivUpdate =
    function(self, sectorId, sectorData)
        ...
        if completed == nil then completed = {} end
        for taskId, v in pairs(completed) do
            ...
            taskData.id = taskId -- DECOMPILER ERROR at PC75: Confused about usage of register: R12 in 'UnsetPending'
            ;
...
SectorTaskController.GetSectorTaskProcess =
    function(self)
        -- function num : 0_10
...
SectorTaskController.SetWindowQuitEvent =
    function(self, QuitAction)
        -- function num : 0_11
        self.__OnQuitAction = QuitAction
    end

SectorTaskController.__OnItemUpdateEvent =
    function(self)
        ...
        if self.uiWindow ~= nil then
            ...
            (self.uiWindow):RefreshSectorTask(self.sortDataArr,
                                              self.achivDataArr)
        end
    end
...
SectorTaskController.OnUiWindowClose = function(self)
    -- function num : 0_15
    if self.__OnQuitAction ~= nil then (self.__OnQuitAction)() end
