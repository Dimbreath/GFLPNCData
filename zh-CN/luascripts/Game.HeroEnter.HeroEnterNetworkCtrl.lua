...
HeroEnterNetworkCtrl.InitNetwork = function(self)
    ...
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STATION_Detail, self,
                         proto_csmsg.SC_STATION_Detail, self.SC_STATION_Detail)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STATION_Install, self,
                         proto_csmsg.SC_STATION_Install, self.SC_STATION_Install)
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_STATION_Uninstall, self,
                         proto_csmsg.SC_STATION_Uninstall,
                         self.SC_STATION_Uninstall)
end

HeroEnterNetworkCtrl.CS_STATION_Detail =
    function(self, isAddAttr)
        ...
        if not funcUnLockCrtl:ValidateUnlock(
            proto_csmsg_SystemFunctionID.SystemFunctionID_HeroEnter) then
            return
        end
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STATION_Detail,
                     proto_csmsg.CS_STATION_Detail, table.emptytable)
        cs_WaitNetworkResponse:StartWait(
            proto_csmsg_MSG_ID.MSG_CS_STATION_Detail, function()
...
HeroEnterNetworkCtrl.SC_STATION_Detail =
    function(self, msg)
...
HeroEnterNetworkCtrl.CS_STATION_Install =
    function(self, heroId, buildingId, callBack)
        ...
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STATION_Install,
                     proto_csmsg.CS_STATION_Install, msgTable)
        self:CS_STATION_Detail()
        cs_WaitNetworkResponse:StartWait(
            proto_csmsg_MSG_ID.MSG_CS_ATH_AthRefillOp, function()
...
HeroEnterNetworkCtrl.SC_STATION_Install =
    function(self, msg)
...
HeroEnterNetworkCtrl.CS_STATION_Uninstall =
    function(self, heroId, callBack)
        -- function num : 0_6 , upvalues : _ENV, cs_WaitNetworkResponse, heroEnterDataUtil
        local msgTable = {heroId = heroId}
        self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_STATION_Uninstall,
                     proto_csmsg.CS_STATION_Uninstall, msgTable)
        self:CS_STATION_Detail()
        cs_WaitNetworkResponse:StartWait(
            proto_csmsg_MSG_ID.MSG_CS_STATION_Uninstall, function()
...
HeroEnterNetworkCtrl.SC_STATION_Uninstall =
    function(self, msg)
        ...
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            ...
            if isGameDev then (cs_MessageCommon.ShowMessageTips)(err) end
            cs_WaitNetworkResponse:RemoveWait(
                proto_csmsg_MSG_ID.MSG_CS_STATION_Uninstall)
        end
    end
