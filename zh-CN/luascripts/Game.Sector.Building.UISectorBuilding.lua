...
UISectorBuilding.OnInit = function(self)
  ...
  self.__OnItemChangeFunc = BindCallback(self, self._OnItemChange)
  MsgCenter:AddListener(eMsgEventId.UpdateItem, self.__OnItemChangeFunc)
  MsgCenter:AddListener(eMsgEventId.UpdateARGItem, self.__OnItemChangeFunc)
end
...
UISectorBuilding._OnItemChange = function(self, updateItem)
  -- function num : 0_2 , upvalues : _ENV
  for itemId,v in pairs(updateItem) do
  ...
  do
    if self.upgradeWindow ~= nil and (self.upgradeWindow).active and self.clickedBdItem ~= nil then
      self:__UpgradeBuilding(self.clickedBdItem)
    end
  end
end
...
UISectorBuilding.ConstructComplete = function(self, id)
  ...
  self:__UpdateAllItemUI()
  ;
  (self.controller):ConfirmOver(id)
end
  ...
  if not isbuilt then
    ...
    do
      do
        ...
        ;
        (self.upgradeWindow):InjectBuildingData(builtData, isbuilt)
        ;
...
UISectorBuilding.OnDelete = function(self)
  ...
  MsgCenter:RemoveListener(eMsgEventId.UpdateARGItem, self.__OnItemChangeFunc)
