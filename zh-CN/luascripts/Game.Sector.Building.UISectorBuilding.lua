...
local UINSectorBuildingGroup = require("Game.Sector.Building.UINSectorBuildingGroup")
local UpgradeCostItem = require("Game.Oasis.UI.UIOasisBuildingUpgradeCostItem")
local UINOBUAccelerateNode = require("Game.Oasis.UI.Accelerate.UINOBUAccelerateNode")
...
UISectorBuilding.OnInit = function(self)
  ...
  self.controller = ControllerManager:GetController(ControllerTypeId.SectorController)
  if self.controller ~= nil then
    (self.controller):SetUIBuildingWindow(self)
  end
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_Back, self, self.__OnClickReturnButton)
  ;
...
UISectorBuilding.InitBuilding = function(self, buildingIdList, returnFunc, sectorCfg)
  -- function num : 0_1 , upvalues : _ENV, UINSectorBuildingGroup
  self.sectorCfg = sectorCfg
  self.buildingIdList = buildingIdList
  self.returnFunc = returnFunc
  ;
  ((self.ui).titleText):SetIndex(0, (LanguageUtil.GetLocaleText)((self.sectorCfg).name))
  ...
  for k,v in ipairs(buildIdGroupList) do
    ...
    go:SetActive(true)
    buildGroup:Init(go)
    buildGroup:InitSctBuildGroup(buildIdList, buildType, self.resloader, self.__OnClickBuildingItem)
...
UISectorBuilding.Update = function(self, timestamp, isSecond)
  ...
  for k,v in pairs(self.buildingItemDic) do
    ...
    do
      if (item.buildingData).maxLevel <= (item.buildingData).level then
        ...
        if (item.buildingData).waitConfirmOver then
          (self.buildingNetworkCtrl):SendBuildingConfirmOver((item.buildingData).id)
        else
          if (item.buildingData).state == proto_object_BuildingState.BuildingStateCreate or (item.buildingData).state == proto_object_BuildingState.BuildingStateUpgrade then
            ...
            if self.accLevel ~= nil then
              (self.accelerateNode):UpdateWithData(self.accLevel, self.accLevel + 1)
            end
            ;
            (self.accelerateNode):Show()
            item.accelerateNode = self.accelerateNode
            ;
            (self.accelerateNode):InitAccelerateNode((item.buildingData).remainSecond, BindCallback(self, self.__SendConfimAcc), (item.buildingData).id, function()
...
UISectorBuilding.UpdateProcessEvent = function(self, id, progress, remainTimeText, waitConfirmOver)
  -- function num : 0_9 , upvalues : _ENV
  local buildingData = ((PlayerDataCenter.AllBuildingData).built)[id]
  local buildingItem = (self.buildingItemDic)[buildingData.id]
  if buildingItem ~= nil then
    buildingItem:UpdateProgressUI(progress, remainTimeText, waitConfirmOver)
    if waitConfirmOver and self.accelerateNode ~= nil and (self.accelerateNode).active and (self.accelerateNode).buildingId == id then
      (self.accelerateNode):OnClickReturn()
    end
  end
end
...
UISectorBuilding.__ShowUpgradeWindow = function(self)
  ...
  if self.upgradeWindow == nil then
    ...
    ;
    (self.upgradeWindow):InjectData(self.resloader)
    -- DECOMPILER ERROR at PC20: Confused about usage of register: R1 in 'UnsetPending'

    ;
    (self.upgradeWindow).closeEvent = BindCallback(self, function()
...
UISectorBuilding.UpdateResCost = function(self, curLvl, nextLvl, nextLevelCfg, costStateIndex)
  ...
  for i = 1, #nextLevelCfg.consumeIds do
    ...
    resNeedItems[resNeedItemIndex] = {}
    -- DECOMPILER ERROR at PC31: Confused about usage of register: R16 in 'UnsetPending'

    ;
    (resNeedItems[resNeedItemIndex]).name = name
    -- DECOMPILER ERROR at PC33: Confused about usage of register: R16 in 'UnsetPending'

    ;
    (resNeedItems[resNeedItemIndex]).count = count
    -- DECOMPILER ERROR at PC35: Confused about usage of register: R16 in 'UnsetPending'

    ;
    (resNeedItems[resNeedItemIndex]).resCount = resCount
  ...
  local remainTime = (math.ceil)(nextLevelCfg.time / (1 + (PlayerDataCenter.playerBonus):GetBuildSpeed() / 1000))
  ;
  (self.upgradeWindow):UpdateWithData(resNeedItems, TimestampToTime(remainTime), curLvl, nextLvl)
  ;
...
UISectorBuilding.__UpgradeBuilding = function(self, item)
  ...
  if not isbuilt then
    ...
    ;
    (self.upgradeWindow).clickEvent = BindCallback(self, self.__ConstrctConfirm)
  else
    do
      do
        local nextLevelCfg = (builtData.levelConfig)[builtData.level + 1]
        self:UpdateResCost(builtData.level, builtData.level + 1, nextLevelCfg, item.costInfoState)
        self.accLevel = builtData.level
        -- DECOMPILER ERROR at PC42: Confused about usage of register: R5 in 'UnsetPending'

        ;
        (self.upgradeWindow).clickEvent = BindCallback(self, self.__UpgradeConfirm)
        ...
        ;
        (self.upgradeWindow):SetTitle((LanguageUtil.GetLocaleText)(builtData.name))
        ;
...
UISectorBuilding.__OnClickReturnButton = function(self)
  -- function num : 0_20
...
UISectorBuilding.OnDelete = function(self)
  ...
  for k,v in pairs(self.buildGroupDic) do
    v:OnDelete()
  end
  if self.upgradeWindow ~= nil then
  ...
  if self.controller ~= nil then
    (self.controller):SetUIBuildingWindow(nil)
  end
