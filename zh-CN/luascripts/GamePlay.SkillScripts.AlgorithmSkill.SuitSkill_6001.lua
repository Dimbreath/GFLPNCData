...
bs_6001.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.SetHurt, "bs_6001_2", 2, self.OnSetHurt)
...
bs_6001.OnSetHeal = function(self, context)
  ...
  if context.sender == self.caster then
    ...
    if heal1 < context.heal then
      ...
      ;
      ((context.target).recordTable)[(self.config).shieldKey] = (((context.target).recordTable)[(self.config).shieldKey] or 0) + shield
      if shield > 0 and (context.target):GetBuffTier((self.config).buffId) == 0 then
        LuaSkillCtrl:CallBuff(self, context.target, (self.config).buffId, 1)
      end
    end
  end
end
...
bs_6001.OnSetHurt = function(self, context)
  ...
  if shield ~= nil and shield >= 0 then
    local retHurt = (ShieldSkillBase.ShieldBaseFunc)(self, context.hurt, context.target)
