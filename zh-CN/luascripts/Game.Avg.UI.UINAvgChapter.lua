...
UINAvgChapter.OnInit = function(self)
    -- function num : 0_1 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (((self.ui).tween_ChapterText).onComplete):AddListener(
        BindCallback(self, self.OnChapterTextTweenComplete)) -- DECOMPILER ERROR at PC19: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tween_ChapterText).isIndependentUpdate =
        (self.avgSystem):AvgIgnoreTimeScale()
end

UINAvgChapter.ShowAvgChapter = function(self, content, shake, immediate,
                                        scramble)
    -- function num : 0_2 , upvalues : CS_ScrambleMode, _ENV
    self:Show()
    self.shake = shake
    ...
    local extents = ((((self.ui).tex_ChapterNarrator).rectTransform).rect).size
    extents.y = 0;
    ((self.ui).tex_ChapterNarrator):PreSetTextSizeDelta(content, false, true,
                                                        extents)
    -- DECOMPILER ERROR at PC24: Confused about usage of register: R6 in 'UnsetPending'
    ...
    if immediate then
        ...
        ((self.ui).tex_ChapterNarrator).text = nil;
        ((self.ui).tween_ChapterText):DOKill() -- DECOMPILER ERROR at PC43: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).tween_ChapterText).optionalScrambleMode =
            scramble and CS_ScrambleMode.Uppercase or CS_ScrambleMode.None;
        ((self.ui).tween_ChapterText):CreateTween()
        local textSpeed = (ConfigData.buildinConfig).AvgTextTweenSpeed;
        (((self.ui).tween_ChapterText).tween):ChangeEndValue(content, textSpeed,
                                                             true);
        ((self.ui).tween_ChapterText):DORestart()
    end
    do self.contentLenth = #content end
end

UINAvgChapter.OnChapterTextTweenComplete =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        if self.tweenComplete then return end
        self.tweenComplete = true;
        (self.avgSystem):ShowTextComplete(self.contentLenth)
        if self.shake then
            self.shakeTween =
                ((((self.ui).tex_ChapterNarrator).transform):DOShakePosition(
                    0.4, (Vector3.New)(10, 10, 0), 20)):SetUpdate(
                    (self.avgSystem):AvgIgnoreTimeScale())
        end
    end
