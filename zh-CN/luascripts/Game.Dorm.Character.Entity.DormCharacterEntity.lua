...
DormCharacterEntity.ctor = function(self, heroData, x, y, roomCharCtrl, isBind)
    ...
    self.roomEntity = roomCharCtrl.roomEntity
    self.isBind = isBind or false
    if self.isBind then self.originRoomCharCtrl = self.roomCharCtrl end
    self.isOperate = false
    self.endUnBindTime = false
    local aiCfg = (ConfigData.dorm_ai)[((self.heroData).heroCfg).dorm_ai]
    if aiCfg == nil then
        error("hero ai config is null,heroId:" ..
                  tostring((self.heroData).dataId) .. " aiId:" ..
                  tostring(((self.heroData).heroCfg).dorm_ai))
        return
    end
    self.aiCtrl = (DormCharacterAI.New)(aiCfg, self)
end
...
DormCharacterEntity.InitCharacterGo = function(self, gameObject)
    -- function num : 0_1 , upvalues : DormUtil, _ENV
    self.gameObject = gameObject
    self.transform = gameObject.transform -- DECOMPILER ERROR at PC8: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.transform).localPosition = (DormUtil.GetFntUnityCoord)(self.x, self.y)
    self.animator = (self.gameObject):FindComponent(eUnityComponentID.Animator)
    self:StartAIBehavior()
...
DormCharacterEntity.StartAIBehavior = function(self)
    ...
    if self.__getReady then
        ...
        (self.aiCtrl):RandNewAction()
        if not self.isBind then
            self.unbindTimer = (TimerManager:GetTimer(UnBindActiveTime,
                                                      self.EndUnBindCharacter,
                                                      self, true)):Start()
        end
    else
        self.__getReady = true
...
DormCharacterEntity.EndUnBindCharacter =
    function(self)
        -- function num : 0_3
        self.endUnBindTime = true
        self.unbindTimer = nil
...
DormCharacterEntity.StartExChangeUnBindChar =
    function(self)
        -- function num : 0_4
        (self.roomCharCtrl):ExchangeUnBindCharacter(self)
    end

DormCharacterEntity.ChangeOtherDormRoom =
    function(self)
        -- function num : 0_5
        return (self.roomCharCtrl):ChangeCharacterToOtherRoom(self)
    end
...
DormCharacterEntity.AnimatorCrossFade = function(self, aniName, transTime)
    -- function num : 0_7
    if not transTime then transTime = 0.25 end
    (self.animator):CrossFade(aniName, transTime)
end
...
DormCharacterEntity.HidePauseCharacter =
    function(self)
        -- function num : 0_11 , upvalues : DormUtil
        self.__position = (self.transform).position -- DECOMPILER ERROR at PC5: Confused about usage of register: R1 in 'UnsetPending'
        ;
        (self.transform).position = DormUtil.CharacterNoShowPos -- DECOMPILER ERROR at PC7: Confused about usage of register: R1 in 'UnsetPending'
        ;
        (self.animator).speed = 0
        if self.curTweener ~= nil then (self.curTweener):Pause() end
        if self.operateTimer ~= nil then (self.operateTimer):Pause() end
        if self.unbindTimer ~= nil then (self.unbindTimer):Pause() end
    end

DormCharacterEntity.ShowResumeCharacter =
    function(self)
        -- function num : 0_12
        -- DECOMPILER ERROR at PC5: Confused about usage of register: R1 in 'UnsetPending'
        ...
        if self.__position ~= nil then
            (self.transform).position = self.__position
        end -- DECOMPILER ERROR at PC7: Confused about usage of register: R1 in 'UnsetPending'
        
        (self.animator).speed = 1
        if self.curTweener ~= nil then (self.curTweener):Play() end
        if self.operateTimer ~= nil then (self.operateTimer):Resume() end
        if self.unbindTimer ~= nil then (self.unbindTimer):Resume() end
    end
...
DormCharacterEntity.SetCharacterOperateStart =
    function(self)
        -- function num : 0_14
        self.isOperate = true
        if self.curTweener ~= nil then
...
DormCharacterEntity.SetCharacterOperateEnd =
    function(self, interPointEntity)
        -- function num : 0_15 , upvalues : _ENV
        self.isOperate = false
        if interPointEntity ~= nil then
            (self.aiCtrl):StartFntInterPointState(interPointEntity.interPoint)
        else
            local newX, newY = (self.roomCharCtrl):FindActivePosNearby(self.x,
                                                                       self.y)
            if self.x ~= nil and self.x ~= nil then
                newX = self.x
                newY = self.y
...
DormCharacterEntity.OnUpdate = function(self)
    -- function num : 0_18
    if self.isOperate then return end
    (self.aiCtrl):OnUpdate()
end
...
DormCharacterEntity.SetLogicPos = function(self, x, y)
    ...
    self.x = x
    self.y = y
    local destPos = (DormUtil.GetFntUnityCoord)(x, y) -- DECOMPILER ERROR at PC7: Confused about usage of register: R4 in 'UnsetPending'
    ;
    (self.transform).localPosition = destPos
end
...
DormCharacterEntity.DoMoveDirectLogicPos =
    function(self, pos, completeAction)
        -- function num : 0_21 , upvalues : DormUtil, _ENV, speed
        local x, y = (DormUtil.FntCoord2XY)(pos)
        local destPos = (DormUtil.GetFntUnityCoord)(x, y)
        local moveTime = (Vector3.Distance)((self.transform).localPosition,
                                            destPos) / speed
        local forward = destPos - (self.transform).localPosition
        -- DECOMPILER ERROR at PC29: Confused about usage of register: R8 in 'UnsetPending'

        if forward:Magnitude() > 0 then
            (self.transform).rotation = (Quaternion.LookRotation)(forward,
                                                                  Vector3.up)
        end
        self.curTweener =
            ((self.transform):DOLocalMove(destPos, moveTime)):OnComplete(
                function()
                    -- function num : 0_21_0 , upvalues : self, x, y, completeAction
                    self.curTweener = nil
                    self.x = x
...
DormCharacterEntity.DoMoveLogicPath = function(self, pathList, completeAction)
    -- function num : 0_22 , upvalues : _ENV, DormUtil, speed
    if pathList == nil or #pathList == 0 then
        if completeAction ~= nil then completeAction() end
        return
    end
    local seq = ((((CS.DG).Tweening).DOTween).Sequence)()
    local lastPos = (self.transform).localPosition
...
DormCharacterEntity.DoMoveUnityPos = function(self, pos, completeAction)
    ...
    local destPos = pos
    local moveTime =
        (Vector3.Distance)((self.transform).localPosition, destPos) / speed
    local forward = destPos - (self.transform).localPosition
    forward = (Vector3.New)(forward.x, forward.y, forward.z)
    -- DECOMPILER ERROR at PC30: Confused about usage of register: R6 in 'UnsetPending'
...
DormCharacterEntity.SetCharacterPosFromUnity =
    function(self, unityPos)
        -- function num : 0_26 , upvalues : DormUtil, eDormFntType
        local newX, newY = (DormUtil.UnityCoord2Fnt)(unityPos,
                                                     eDormFntType.Furniture)
        local oldX = self.x
        local oldY = self.y
        local move = newX ~= oldX or newY ~= oldY
        if move and not (DormUtil.IsFntCoordLegal)(newX, newY) then
        ...
        if move then self:SetLogicPos(newX, newY) end
        do return move, newX, newY end
        -- DECOMPILER ERROR: 3 unprocessed JMP targets
    end
