...
UINTaskPeroidNodeItem.InitPeroidItem = function(self, index, eTaskState, point, itemCfg, num, viewReward, data)
  ...
  ;
  (self.baseItem):InitBaseItem(itemCfg, self._OnClick)
...
UINTaskPeroidNodeItem.InitPeroidItem = function(self, index, eTaskState, point,
    ...
    ;
    ((self.ui).tex_Count).text = tostring(num)
    self.viewReward = viewReward -- DECOMPILER ERROR at PC25: Confused about usage of register: R8 in 'UnsetPending'
    ;
    ((self.ui).img_trangle).color = (self.ui).color_black;
    ((self.ui).obj_isPicked):SetActive(false);
    ((self.ui).obj_canPick):SetActive(false)
