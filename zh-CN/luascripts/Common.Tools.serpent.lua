...
for _, g in
    ipairs({"coroutine", "debug", "io", "math", "string", "table", "os"}) do
    if type(G[g]) ~= "table" or not G[g] then
        for k, v in pairs({}) do globals[v] = g .. "." .. k end
        do
            -- DECOMPILER ERROR at PC104: LeaveBlock: unexpected jumping out IF_THEN_STMT
...
local s = function(t, opts)
    -- function num : 0_1 , upvalues : _ENV, snum, globals, keyword, getmetatable, pairs, badtype
    local name, indent, fatal, maxnum = opts.name, opts.indent, opts.fatal,
                                        opts.maxnum
    local sparse, custom, huge = opts.sparse, opts.custom, not opts.nohuge
    if not opts.maxlevel then
        local space, maxl = opts.compact and "" or " ", math.huge
    ...
    local gensym = function(val)
        -- function num : 0_1_0 , upvalues : _ENV, syms, symn
        return "_" ..
                   ((tostring(tostring(val))):gsub("[^%w]", "")):gsub("(%d%w+)",
                                                                      function(s)
                -- function num : 0_1_0_0 , upvalues : syms, symn, _ENV
                if not syms[s] then
                    symn = symn + 1
    ...
    local safestr = function(s)
        -- function num : 0_1_1 , upvalues : _ENV, huge, snum, numformat
        if (type(s) ~= "number" or
            ((huge and snum[tostring(s)]) or not tostring(numformat:format(s))) and
            (type(s) == "string" or not tostring(s))) then
            return ((("%q"):format(s)):gsub("\n", "n")):gsub("\026", "\\026")
        end
    end
    ...
    local comment = function(s, l)
        -- function num : 0_1_2 , upvalues : comm, _ENV
        return (l or not comm or 0 < comm) and " --[[" ..
                   select(2, pcall(tostring, s)) .. "]]" or ""
    end

    local globerr = function(s, l)
        -- function num : 0_1_3 , upvalues : globals, comment, fatal, safestr, _ENV
        if (not globals[s] or not globals[s] .. comment(s, l)) and
            (fatal or not safestr(select(2, pcall(tostring, s)))) then
            return error("Can\'t serialize " .. tostring(s))
        end
    end
    ...
    local safename = function(path, name)
        -- function num : 0_1_4 , upvalues : _ENV, keyword, safestr
        local n = name == nil and "" or name
        local plain = (type(n) ~= "string" or n:match("^[%l%u_][%w_]*$")) and
                          not keyword[n]
        if not plain or not n then local safe = "[" .. safestr(n) .. "]" end
        do
    ...
    local val2str = function(t, name, indent, insref, path, plainindex, level)
        ...
        local ttype, level, mt = type(t), level or 0, getmetatable(t)
        local spath, sname = safename(path, name)
        local tag = ((not plainindex or (type(name) == "number" and "") or
                        not name .. space .. "=" .. space) and
        ...
        if type(mt) == "table" then
            ...
            if not insref then
                do
                    seen[t] =
                        (opts.metatostring == false or not to) and not so or
                            spath
                    t = so and sr or tr
                    ttype = type(t)
                    if ttype == "table" then
                        ...
                        if not maxnum then
                            ...
                            do
                                ...
                                do
                                    ...
                                    local sparse = not sparse or maxn < #o
                                    for n, key in ipairs(o) do
                                        local value, ktype, plainindex = t[key],
                                                                         type(
                                                                             key),
                                                                         (n <=
                                                                             maxn and
                                                                             not sparse)
                                        if (((opts.valignore and
                                            (opts.valignore)[value]) or
                                            (opts.keyallow and
                                                not (opts.keyallow)[key]) or
                                            (opts.keyignore and
                                                (opts.keyignore)[key]) or
                                            (not opts.valtypeignore or
                                                not (opts.valtypeignore)[type(
                                                    value)]))) then
                                            -- DECOMPILER ERROR at PC266: Unhandled construct in 'MakeBoolean' P3

                                            if (sparse and value == nil and
                                                ktype == "table") or
                                                badtype[ktype] then
                                                if not seen[key] and
                                                    not globals[key] then
                                                    sref[#sref + 1] =
                                                        "placeholder"
                                                    local sname =
                                                        safename(iname,
                                    ...
                                    local tail =
                                        indent and "\n" .. prefix .. "}" or "}"
                                    if not custom or
                                        not custom(tag, head, body, tail, level) then
                                        do
                                            do
                                                return
                                                    tag .. head .. body .. tail ..
    ...
    if name or not body .. warn then
        return
            "do local " .. body .. sepr .. tail .. "return " .. name .. sepr ..
                "end"
    end
end
