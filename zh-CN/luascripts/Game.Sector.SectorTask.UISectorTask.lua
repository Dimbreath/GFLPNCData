...
UISectorTask.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).closeShowReward, self, self.__OnClickCloseShowReward)
  ;
...
UISectorTask.UpdateAllTask = function(self)
  ...
  local taskDatas = ((PlayerDataCenter.allTaskData).sectorTaskDatas)[self.type]
  for k,v in pairs(taskDatas) do
    local taskItem = (self.__taskItems)[k]
    if taskItem == nil then
      taskItem = self:__GetTaskItem()
...
UISectorTask.__SortTask = function(self)
  ...
  local sortTab = {}
  for k,v in pairs(self.__taskItems) do
...
UISectorTask.OnTaskCommit = function(self, taskCfg)
  ...
  self:OnSectorAchivUpdate(self.sectorId, (PlayerDataCenter.sectorDatas)[self.sectorId])
end
  ...
  for i = 1, #self.achivCfg do
    ...
    do
      if achivItem == nil then
        ...
        achivItem.OnClickEvent = BindCallback(self, self.__OnClickAchivItem)
      ...
      do
        ...
        if boxPicked ~= nil and boxPicked[i] ~= nil then
          ...
          if ((PlayerDataCenter.sectorDatas).CheckAchivCondition)(achivCfg) then
...
UISectorTask.OnPickedAchivItem = function(self, sectorId, id)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward, function(window)
    -- function num : 0_14_0 , upvalues : rewardIds, rewardNums, self
    if window == nil then
      return 
    end
...
UISectorTask.OnDelete = function(self)
  ...
  for k,v in ipairs(self.__taskItemPool) do
    v:OnDelete()
  end
  self.__taskItemPool = {}
  for k,v in pairs(self.__taskItems) do
