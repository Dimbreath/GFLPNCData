...
bs_10065.OnRoleDie = function(self, killer, role)
  ...
  if (self.caster).belongNum == role.belongNum then
    ...
    ;
    ((self.caster).recordTable).sheildValue_10044 = LuaSkillCtrl:CallFormulaNumberWithSkill((self.config).damageFormula, role, role, self)
    if ((self.caster).recordTable).sheildValue_10044 >= 0 then
      (ShieldSkillBase.UpdateShieldView)(self.caster, (self.config).shieldKey, ((self.caster).recordTable).sheildValue_10044)
    end
  end
...
bs_10065.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.SetHurt, "bs_10065_2", 1, self.OnSetHurt)
    self:AddTrigger(eSkillTriggerType.RoleDie, "bs_10065_10", 1, self.OnRoleDie) -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).sheildValue_10044 = 0
end
...
bs_10065.OnSetHurt = function(self, context)
    ...
    if context.target == self.caster and
        ((self.caster).recordTable).sheildValue_10044 ~= 0 and
        (context.target).roleType == 1 then
        local realHurt = context.hurt -
                             ((self.caster).recordTable).sheildValue_10044
        if realHurt >= 0 then
            context.hurt = realHurt -- DECOMPILER ERROR at PC23: Confused about usage of register: R3 in 'UnsetPending'
            ;
            ((self.caster).recordTable).sheildValue_10044 = 0
        else
            context.hurt = 0 -- DECOMPILER ERROR at PC29: Confused about usage of register: R3 in 'UnsetPending'
            ;
            ((self.caster).recordTable).sheildValue_10044 = 0 - realHurt
        end
        if ((self.caster).recordTable).sheildValue_10044 >= 0 then
        ...
        if ((self.caster).recordTable).sheildValue_10044 == 0 and
...
bs_10065.OnRoleDie = function(self, killer, role)
    ...
    if (self.caster).belongNum == role.belongNum then
        self:PlayChipEffect()
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId, 1) -- DECOMPILER ERROR at PC25: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.caster).recordTable).sheildValue_10044 =
            LuaSkillCtrl:CallFormulaNumberWithSkill((self.config).damageFormula,
                                                    role, role, self)
        if ((self.caster).recordTable).sheildValue_10044 >= 0 then
            (ShieldSkillBase.UpdateShieldView)(self.caster,
                                               (self.config).shieldKey,
