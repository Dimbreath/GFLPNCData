...
local count = function(hashtable)
    ...
    local count = 0
    for _, _ in pairs(hashtable) do count = count + 1 end
    return count
end

local length = function(array)
    -- function num : 0_1 , upvalues : _ENV
    if array.n ~= nil then return array.n end
    local count = 0
    for i, _ in pairs(array) do if count < i then count = i end end
    return count
end
...
local merge = function(dest_hashtable, src_hashtable)
    ...
    for k, v in pairs(src_hashtable) do dest_hashtable[k] = v end
end

local insertto = function(dest_array, src_array, begin)
    -- function num : 0_6 , upvalues : _ENV
    assert(begin == nil or type(begin) == "number")
    if begin == nil or begin <= 0 then begin = #dest_array + 1 end
    local src_len = #src_array
    for i = 0, src_len - 1 do dest_array[i + (begin)] = src_array[i + 1] end
...
table.removebyvalue = function(array, value, removeall)
    ...
    do if removeall then return remove_count end end
end

local map = function(tb, func)
    -- function num : 0_10 , upvalues : _ENV
    for k, v in pairs(tb) do tb[k] = func(k, v) end
end

local walk = function(tb, func)
    -- function num : 0_11 , upvalues : _ENV
    for k, v in pairs(tb) do func(k, v) end
end
...
local walksort = function(tb, sort_func, walk_func)
    ...
    for i = 1, (table.length)(keys) do walk_func(keys[i], tb[keys[i]]) end
end

local filter = function(tb, func)
    -- function num : 0_13 , upvalues : _ENV
    local filter = {}
    for k, v in pairs(tb) do if not func(k, v) then filter[k] = v end end
    return filter
end
...
local choose = function(tb, func)
    ...
    local choose = {}
    for k, v in pairs(tb) do if func(k, v) then choose[k] = v end end
    return choose
end
    ...
    local _dump = function(tb, level)
        ...
        for k, v in pairs(tb) do
            ...
            if not tostring(k) then
                str =
                    str .. rep("\t", level + 1) .. "[" .. rep("\"", k_is_str) ..
                        type(k) .. rep("\"", k_is_str) .. "]" .. " = "
                if type(v) == "table" then
                    if not lookup_table[v] and
                        (not max_level or level < max_level) then
                        lookup_table[v] = true
...
local deepCopy = function(object)
    ...
    local _copy = function(object)
        ...
        if type(object) ~= "table" then
            ...
            if lookup_table[object] then return lookup_table[object] end
        end
        local new_table = {}
