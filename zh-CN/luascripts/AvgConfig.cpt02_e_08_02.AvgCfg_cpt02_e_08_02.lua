-- params : ...
-- function num : 0 , upvalues : _ENV
local AvgCfg_cpt02_e_08_02 = {
    [1] = {
        images = {
            {
                imgId = 99,
                imgPath = "cpt02/cpt02_e_bg001",
                imgType = 2,
                fullScreen = true
            }, {
                imgId = 10,
                imgPath = "evelyn_avg",
                imgType = 3,
                pos = {-100, -400, 0},
                rot = {0, 180, 0},
                scale = {1.8, 1.8, 1.8},
                alpha = 0.2
            }
        },
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 10,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2,
        audio = {bgm = {cue = "Mus_Story_BattleTension", sheet = "Music"}}
    },
    [2] = {
        content = 20,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [3] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 30,
        contentType = 2
    },
    [4] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 40,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [5] = {
        content = 50,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [6] = {
        content = 60,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [7] = {
        content = 70,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [8] = {
        content = 80,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [9] = {
        content = 90,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [10] = {
        content = 100,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [11] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 110,
        contentType = 2
    },
    [12] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 120,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [13] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 130,
        contentType = 2
    },
    [14] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 140,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [15] = {
        content = 150,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [16] = {
        images = {
            {
                imgId = 98,
                imgPath = "cpt02/cpt02_e_bg002",
                imgType = 2,
                alpha = 0,
                fullScreen = true
            }, {
                imgId = 97,
                imgPath = "cpt00/cpt00_e_bg001",
                imgType = 4,
                fullScreen = true,
                alpha = 0
            }
        },
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {-100, -400, 0},
                alpha = 0,
                isDark = false
            }, {imgId = 97, duration = 0.5, alpha = 1},
            {imgId = 97, delay = 0.5, duration = 0.5, alpha = 0},
            {imgId = 98, delay = 0.5, alpha = 1},
            {imgId = 99, delay = 0.5, alpha = 0}
        },
        content = 160,
        contentType = 2
    },
    [17] = {
        images = {
            {
                imgId = 4,
                imgPath = "simo",
                imgType = 3,
                pos = {100, -50, 0},
                scale = {1.15, 1.15, 1.15},
                alpha = 0.2
            }, {imgId = 97, delete = true}, {imgId = 99, delete = true}
        },
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 170,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [18] = {
        content = 180,
        contentType = 3,
        speakerHeroId = -1,
        speakerHeroPosId = 2,
        contentShake = true
    },
    [19] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {100, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 190,
        contentType = 2
    },
    [20] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 200,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [21] = {
        content = 210,
        contentType = 3,
        speakerHeroId = -1,
        speakerHeroPosId = 2,
        contentShake = true
    },
    [22] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 220,
        contentType = 2
    },
    [23] = {
        content = 230,
        contentType = 3,
        speakerHeroId = -1,
        speakerHeroPosId = 2
    },
    [24] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 240,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [25] = {
        content = 250,
        contentType = 3,
        speakerHeroId = -1,
        speakerHeroPosId = 2
    },
    [26] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 260,
        contentType = 2
    },
    [27] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 270,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [28] = {
        content = 280,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [29] = {
        content = 290,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [30] = {
        content = 300,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [31] = {
        content = 310,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [32] = {
        content = 320,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [33] = {
        content = 330,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [34] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {-100, -400, 0},
                alpha = 0,
                isDark = false
            },
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 340,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2,
        contentShake = true
    },
    [35] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 350,
        contentType = 2
    },
    [36] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 360,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2,
        contentShake = true
    },
    [37] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {100, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 370,
        contentType = 2
    },
    [38] = {content = 380, contentType = 2, contentShake = true},
    [39] = {content = 390, contentType = 2},
    [40] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 400,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [41] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 410,
        contentType = 2
    },
    [42] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 420,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [43] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {100, -50, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 430,
        contentType = 2
    },
    [44] = {content = 440, contentType = 2, contentShake = true},
    [45] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {50, -50, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 450,
        contentType = 3,
        speakerHeroId = 1004,
        speakerHeroPosId = 2
    },
    [46] = {
        imgTween = {
            {
                imgId = 4,
                duration = 0.2,
                pos = {100, -50, 0},
                alpha = 0,
                isDark = false
            },
            {
                imgId = 10,
                duration = 0.2,
                pos = {50, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 460,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [47] = {
        content = 470,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [48] = {
        content = 480,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [49] = {
        content = 490,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [50] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {-100, -400, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 500,
        contentType = 2
    },
    [51] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 510,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [52] = {
        content = 520,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2
    },
    [53] = {
        imgTween = {
            {
                imgId = 10,
                duration = 0.2,
                pos = {-100, -400, 0},
                alpha = 0,
                isDark = false
            }
        },
        content = 530,
        contentType = 2
    },
    [54] = {
        imgTween = {
            {imgId = 10, pos = {0, -250, 0}, alpha = 0.2},
            {
                imgId = 10,
                duration = 0.2,
                pos = {0, -400, 0},
                alpha = 1,
                isDark = false
            }
        },
        content = 540,
        contentType = 3,
        speakerHeroId = 1010,
        speakerHeroPosId = 2,
        contentShake = true
    }
}
return AvgCfg_cpt02_e_08_02

