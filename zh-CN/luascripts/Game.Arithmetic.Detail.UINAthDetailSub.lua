...
UINAthDetailSub.InitAthDetailNode = function(self, athData, efficiency, heroId,
                                             installEvent)
...
UINAthDetailSub.RefreshSuit = function(self, replaceAthData)
    ...
    if suitAthList ~= nil then
        ...
        for k, athId in ipairs(suitAthList) do
            ...
            local installed = nil
            if (replaceAthData ~= nil and replaceAthData.id == athId) or athId ==
                athData.id then
                installed = false
            else
            ...
            content = content ..
                          ((self.ui).tex_TierGear):GetIndex(textIndex, name,
                                                            tostring(slotId))
        ...
        do
            ...
            for k, v in ipairs(suitCfg) do
                ...
                if v.num == suitNumPreview then textIndex = 4 end
                local effectStr = (LanguageUtil.GetLocaleText)(v.describe)
                content = content ..
                              ((self.ui).tex_TierGear):GetIndex(textIndex,
                                                                tostring(v.num),
                                                                effectStr)
            end
            do
...
UINAthDetailSub.OnClickInstall = function(self)
    -- function num : 0_4
    if self.installEvent ~= nil then (self.installEvent)() end
end
