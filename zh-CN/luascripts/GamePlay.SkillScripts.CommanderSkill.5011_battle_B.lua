...
bs_5011.PlaySkill = function(self, data)
    ...
    LuaSkillCtrl:CallBattleCamShake(2) -- DECOMPILER ERROR at PC8: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable)["5011_time"] = (self.arglist)[2]
    self.callback = BindCallback(self, self.OnCallback)
    self:OnCallback()
end
            ...
            if ((self.caster).recordTable)["5011_time"] > 0 then
                ...
                do
                    ...
                    LuaSkillCtrl:StartTimer(self, time, function()
                        ...
                        LuaSkillCtrl:CallEffect(targetRole,
                                                (self.config).effectIdAttack,
                                                self, self.SkillEventFunc)
                    end) -- DECOMPILER ERROR at PC37: Confused about usage of register: R8 in 'UnsetPending'
                    ;
                    ((self.caster).recordTable)["5011_time"] =
                        ((self.caster).recordTable)["5011_time"] - 1
                end
            end
        end
        if ((self.caster).recordTable)["5011_time"] > 0 then
...
bs_5011.SkillEventFunc = function(self, effect, eventId, target)
    ...
    if eventId == eBattleEffectEvent.Trigger then
        ...
        if highAttRole ~= nil and highAttRole.Count > 0 and highAttRole[0] ~=
            nil then
            local hurt =
                ((highAttRole[0]).targetRole).pow * (self.arglist)[1] // 1000
