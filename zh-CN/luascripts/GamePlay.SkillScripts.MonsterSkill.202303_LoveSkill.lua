...
bs_202303.PlaySkill = function(self, data)
  ...
  local targetList = LuaSkillCtrl:CallTargetSelect(self, 32, 0)
...
bs_202303.OnAttackTrigger = function(self)
  ...
  LuaSkillCtrl:StartTimer(self, 6, heal, self, (self.arglist)[1] - 1, 2)
...
bs_202303.Onhurt = function(self)
    ...
    local target_hurt = LuaSkillCtrl:CallTargetSelect(self, 42, range)
    if target_hurt.Count > 0 then
        LuaSkillCtrl:CallEffect((target_hurt[0]).targetRole,
                                (self.config).effectHithurt, self)
        local skillResult = LuaSkillCtrl:CallSkillResultNoEffect(self,
                                                                 (target_hurt[0]).targetRole)
        LuaSkillCtrl:HurtResult(skillResult, (self.config).HurtConfig)
        skillResult:EndResult()
    end
end
...
bs_202303.Onheal = function(self)
    ...
    local target_heal = LuaSkillCtrl:CallTargetSelect(self, 20, range)
    if target_heal.Count > 0 then
        LuaSkillCtrl:CallEffect((target_heal[0]).targetRole,
                                (self.config).effectHitheal, self)
        local skillResult = LuaSkillCtrl:CallSkillResultNoEffect(self,
                                                                 (target_heal[0]).targetRole)
        LuaSkillCtrl:HealResult(skillResult, (self.config).HealConfig)
        skillResult:EndResult()
    end
end
