...
BattleUtil.GetChipAttrInfo = function(attrId, initValue, increaseValue, count, newcount)
  ...
  if not increaseValue then
    ...
    do
      local content = (string.format)("%s%s%s%s<color=#%s>%s%s</color>", atrName, sign, valueStr, (ConfigData.buildinConfig).ChipLevelUpSign, (ConfigData.buildinConfig).ChipLevelUpColor, newSign, newValueStr)
  ...
  for i = 0, playerRoleList.Count - 1 do
    ...
    if not (role.character).onBench then
      ...
      if heroData == nil then
        error("Can\'t find heroData by id, id = " .. tostring(role.roleDataId))
      else
        local MVPNum = 0
        local ok, damageDetail = (((CS.BattleStatistics).Instance).playerDamage):TryGetValue(role)
        if ok then
          MVPNum = MVPNum + damageDetail.damage * ((heroData.heroCfg).mvp_para)[(BattleUtil.mvpParaType).damage]
        ...
        if ok then
          MVPNum = MVPNum + takeDamageDetail.damage * ((heroData.heroCfg).mvp_para)[(BattleUtil.mvpParaType).injury]
        ...
        if ok then
          MVPNum = MVPNum + healDetail.selfHeal * ((heroData.heroCfg).mvp_para)[(BattleUtil.mvpParaType).selfHeal]
          MVPNum = MVPNum + healDetail.otherHeal * ((heroData.heroCfg).mvp_para)[(BattleUtil.mvpParaType).otherHeal]
