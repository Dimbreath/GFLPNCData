...
UIBattleSkadaItem.RefreshSkadaItem = function(self, roleIndex, roleSkada, sortType, belongPlayer)
  ...
  if not belongPlayer or not PathConsts:GetCharacterPicPath(((roleSkada.role).character):GetResName()) then
    local path = PathConsts:GetCharacterSmallPicPath(((roleSkada.role).character):GetResName())
  end
...
local UIBattleSkadaItem = class("UIBattleSkadaItem", UIBaseNode)
local base = UIBaseNode
local UIBattleSkadaValueItem = require(
                                   "Game.BattleResult.Skada.UIBattleSkadaValueItem")
local BattleSkadaEnum = require("Game.BattleResult.Skada.BattleSkadaEnum")
UIBattleSkadaItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
...
UIBattleSkadaItem.InitSkadaItem = function(self, resLoader, clickCallback)
    -- function num : 0_1 , upvalues : _ENV, BattleSkadaEnum, UIBattleSkadaValueItem
    if self.__init then return end
    self.__init = true
    self.resLoader = resLoader
    self.OnClickHeroPicEvent = clickCallback
    self.valueItems = {}
    for k, id in pairs(BattleSkadaEnum.eBattleSkadaType) do
        ...
        local valueItem = (UIBattleSkadaValueItem.New)()
        valueItem:Init(go)
        valueItem:InitSkadaValueItem(self:__GetValueItemColor(id)) -- DECOMPILER ERROR at PC31: Confused about usage of register: R10 in 'UnsetPending'
        ;
        (self.valueItems)[id] = valueItem
    end
end

UIBattleSkadaItem.RefreshSkadaItem = function(self, roleIndex, roleSkada,
                                              sortType, belongPlayer)
    -- function num : 0_2 , upvalues : _ENV
    self.roleIndex = roleIndex
    self.roleSkada = roleSkada
    if not belongPlayer or
        not PathConsts:GetCharacterPicPath(
            ((roleSkada.role).character):GetResName()) then
        local path = PathConsts:GetCharacterSmallPicPath(
                         ((roleSkada.role).character):GetResName())
    ...
    for k, v in ipairs(self.valueItems) do
        local detail = (roleSkada.detail)[k]
        v:SetSkadaValue(detail.proportion, detail.value)
        if sortType ~= k then (v.transform):SetAsLastSibling() end
    end
end
...
UIBattleSkadaItem.OnDelete = function(self)
    -- function num : 0_5 , upvalues : _ENV, base
    for k, v in pairs(self.valueItems) do v:Delete() end
    (base.OnDelete)(self)
end
