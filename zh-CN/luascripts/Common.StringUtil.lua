...
local split = function(split_string, pattern, search_pos_begin, plain)
    ...
    if not search_pos_begin then search_pos_begin = 1 end
    plain = plain ~= nil and plain
    local split_result = {}
    if (string.IsNullOrEmpty)(split_string) then return split_result end
    while 1 do
        ...
        if find_pos_begin then
            do
                ...
                if search_pos_begin < find_pos_begin then
                    cur_str = (string.sub)(split_string, search_pos_begin,
                                           find_pos_begin - 1)
                else
                    cur_str = ""
                end
    ...
    if search_pos_begin <= (string.len)(split_string) then
        split_result[#split_result + 1] =
            (string.sub)(split_string, search_pos_begin)
    else
        split_result[#split_result + 1] = ""
    end
...
local join = function(join_table, joiner)
    ...
    if #join_table == 0 then return "" end
    local fmt = "%s"
    for i = 2, #join_table do fmt = fmt .. joiner .. "%s" end
...
local contains = function(target_string, pattern, plain)
    ...
    if plain ~= nil or not true then
        local find_pos_begin, find_pos_end =
            (string.find)(target_string, pattern, 1, plain)
...
local startswith = function(target_string, start_pattern, plain)
    ...
    if plain ~= nil or not true then
        local find_pos_begin, find_pos_end =
            (string.find)(target_string, start_pattern, 1, plain)
...
local endswith = function(target_string, start_pattern, plain)
    ...
    if plain ~= nil or not true then
        local find_pos_begin, find_pos_end =
            (string.find)(target_string, start_pattern, -#start_pattern, plain)
...
local IsNullOrEmpty = function(target_string)
    ...
    if type(target_string) ~= "string" then return true end
    if target_string == "" or #target_string == 0 then return true end
    return false
end
