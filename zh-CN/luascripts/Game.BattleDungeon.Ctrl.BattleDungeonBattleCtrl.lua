...
BattleDungeonBattleCtrl.VictoryBattleEndCoroutine = function(self, battleEndState, resultData)
  ...
  local battleEndCoroutine = function()
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.DungeonResult, function(window)
      ...
      window:CompleteDungeon(((self.bdCtrl).objectCtrl).rewardMsg, resultData, mvpGrade, (self.battleRoomData).dungeonType, (self.bdCtrl).ATHRewardInfo)
...
BattleDungeonBattleCtrl.OnDelete = function(self)
  ...
  if ((self.bdCtrl).objectCtrl).dontShowResult then
    ...
    if self.battleEndState == nil then
      error("can\'t find battleEndState")
    else
      ;
...
-- params : ...
-- function num : 0 , upvalues : _ENV
local DungeonBattleBaseCtrl = require(
                                  "Game.Common.CommonGameCtrl.DungeonBattleBaseCtrl")
local BattleDungeonBattleCtrl = class("BattleDungeonBattleCtrl",
                                      DungeonBattleBaseCtrl)
local DungeonConst = require("Game.BattleDungeon.DungeonConst")
local DungeonBattleRoom = require("Game.BattleDungeon.Data.DungeonBattleRoom")
local ChipData = require("Game.PlayerData.Item.ChipData")
local util = require("XLua.Common.xlua_util")
BattleDungeonBattleCtrl.ctor = function(self, bdCtrl)
    ...
    (table.insert)((self.bdCtrl).ctrls, self)
    self.__battleStepLogic = BindCallback(self, self.DungeonBattleStepLogic);
    (self.bdCtrl):RegisterDungeonLogic((DungeonConst.LogicType).BattleStep,
                                       self.__battleStepLogic)
    self.__chipStepLogic = BindCallback(self, self.DungeonChipStepLogic);
    (self.bdCtrl):RegisterDungeonLogic((DungeonConst.LogicType).ChipStep,
                                       self.__chipStepLogic)
    self.__OnTimelineNoticeOpenResultUI =
        BindCallback(self, self.OnTimelineNoticeOpenResultUI)
    MsgCenter:AddListener(eMsgEventId.OnTimelineNoticeCreateResultUI,
                          self.__OnTimelineNoticeOpenResultUI)
end

BattleDungeonBattleCtrl.GetEffectPoolCtrl =
    function(self)
        -- function num : 0_1
        return ((self.bdCtrl).sceneCtrl).effectPoolCtrl
    end

BattleDungeonBattleCtrl.GetHeroObjectDic =
    function(self)
        -- function num : 0_2
        return ((self.bdCtrl).sceneCtrl).heroObjectDic
    end

BattleDungeonBattleCtrl.GetRoleAppearEffect =
    function(self)
        -- function num : 0_3
        return ((self.bdCtrl).sceneCtrl):GetRoleAppearEffect()
    end

BattleDungeonBattleCtrl.GetRoleDisappearEffect =
    function(self)
        -- function num : 0_4
        return ((self.bdCtrl).sceneCtrl):GetRoleDisappearEffect()
    end

BattleDungeonBattleCtrl.DungeonBattleStepLogic =
    function(self, monsterGroup)
        ...
        local btlMgr = (CS.BattleManager).Instance
        local IsWithFormation = (self.battleRoomData).formation
        local battleCtrl = btlMgr:StartNewBattle(battleRoomData,
                                                 (self.bdCtrl).dynPlayer, self,
                                                 not IsWithFormation)
        if IsWithFormation then
            battleCtrl:StartEnterDeployState()
        else
        ...
        if ((self.bdCtrl).sceneCtrl).sceneWave ~= nil then
            UIManager:CreateWindowAsync(UIWindowTypeID.DungeonWaveTip,
                                        function(window)
                -- function num : 0_5_0 , upvalues : self
                window:InjectWave(((self.bdCtrl).sceneCtrl).sceneWave)
                window:Show()
...
BattleDungeonBattleCtrl.BattleLoadReady =
    function(self)
        -- function num : 0_6
    end

BattleDungeonBattleCtrl.ReqStartBattle =
    function(self, battleRoomData, originRoleList, battleAction)
        -- function num : 0_7 , upvalues : _ENV
        if (self.battleRoomData).formation then
            local roleCount = originRoleList.Count
            local sendMsg = {}
            ...
            for i = 0, roleCount - 1 do
                local role = originRoleList[i]
                local heroId = role.roleDataId
                local pos = (BattleUtil.XYCoord2Pos)(role.x, role.y) -- DECOMPILER ERROR at PC20: Confused about usage of register: R13 in 'UnsetPending'
                ;
                (sendMsg.data)[heroId] = pos
            end
            ((self.bdCtrl).battleNetwork):CS_BATTLE_StartBattleWithForm(sendMsg,
                                                                        function(
                dataList)
                -- function num : 0_7_0 , upvalues : battleAction
                if battleAction ~= nil then battleAction() end
            end)
        else
            do if battleAction ~= nil then battleAction() end end
        end
    end

BattleDungeonBattleCtrl.ReqBattleSettle =
    function(self, battleEndState, playerRoleSettle, monsterRoleSettle)
        ...
        ((self.bdCtrl).battleNetwork):CS_BATTLE_BattleSettle(sendMsg,
                                                             function(dataList)
            -- function num : 0_8_0 , upvalues : win, _ENV, battleEndState, self
            if not win then
                UIManager:ShowWindowAsync(UIWindowTypeID.DungeonFailureResult,
                                          function(window)
                    -- function num : 0_8_0_0 , upvalues : battleEndState, self
                    if window == nil then return end
                    window:FailExploration(
                        function()
                            -- function num : 0_8_0_0_0 , upvalues : battleEndState
                            battleEndState:EndBattleAndClear()
                        end, function()
...
BattleDungeonBattleCtrl.VictoryBattleEndCoroutine =
    function(self, battleEndState, resultData)
        ...
        local battleEndCoroutine = function()
            -- function num : 0_9_0 , upvalues : CS_CameraController_Ins, battleController, mvpGrade, self, _ENV, battleEndState, resultData
            CS_CameraController_Ins:PlaySettlementCut(battleController,
                                                      mvpGrade.role)
            while self.__waitSettleResult do (coroutine.yield)() end
            if self.__settleTimelinePause then
                CS_CameraController_Ins:PauseSettlementCut(false)
            end
            while not self.__showResultUI do (coroutine.yield)() end
            if ((self.bdCtrl).objectCtrl).dontShowResult then
                (self.bdCtrl):ExitBattleDungeon(true)
                self.battleEndState = battleEndState
                return
            end
            UIManager:ShowWindowAsync(UIWindowTypeID.DungeonResult,
                                      function(window)
                -- function num : 0_9_0_0 , upvalues : self, resultData, mvpGrade, battleEndState
                if window == nil then return end
                window:CompleteDungeon(((self.bdCtrl).objectCtrl).rewardMsg,
                                       resultData, mvpGrade,
                                       (self.battleRoomData).dungeonType,
                                       (self.bdCtrl).ATHRewardInfo)
                window:SetContinueCallback(
                    function()
                        -- function num : 0_9_0_0_0 , upvalues : battleEndState, self
                        battleEndState:EndBattleAndClear();
                        (self.bdCtrl):ExitBattleDungeon(true)
...
BattleDungeonBattleCtrl.OnTimelineNoticeOpenResultUI =
    function(self)
        -- function num : 0_10 , upvalues : _ENV
        self.__showResultUI = true
        if self.__waitSettleResult then
...
BattleDungeonBattleCtrl.ReqBattleFreshFormation =
    function(self, battleController)
        -- function num : 0_11
    end

BattleDungeonBattleCtrl.DungeonChipStepLogic =
    function(self, chipDataGroup)
        -- function num : 0_12 , upvalues : _ENV, ChipData
        local rewardChipList = {}
        for k, v in ipairs(chipDataGroup.alg) do
            local chipData = (ChipData.New)(v, 1)
        ...
        UIManager:ShowWindowAsync(UIWindowTypeID.SelectChip, function(window)
            ...
            if window == nil then return end
            MsgCenter:Broadcast(eMsgEventId.OnSettleMentTimeLinePlayToEnd)
            window:InitSelectChip(rewardChipList, (self.bdCtrl).dynPlayer,
                                  BindCallback(self, self.__SelectChipComplete),
                                  false)
        end)
    end

BattleDungeonBattleCtrl.__SelectChipComplete =
    function(self, index, selectComplete)
        -- function num : 0_13
        index = index - 1;
        ((self.bdCtrl).battleNetwork):CS_BATTLE_AlgSelect(index,
                                                          function(dataList)
            -- function num : 0_13_0 , upvalues : self, selectComplete
            (self.bdCtrl):StartRunNextLogic()
...
BattleDungeonBattleCtrl.ReqGiveUpBattle =
    function(self, battleController)
        -- function num : 0_14 , upvalues : _ENV
        (battleController.fsm):ChangeState((CS.eBattleState).End);
        ((battleController.fsm).currentState):EndBattleAndClear();
        ((self.bdCtrl).battleNetwork):CS_BATTLE_Quit(
            function()
                -- function num : 0_14_0 , upvalues : _ENV
                BattleDungeonManager:ExitDungeon()
            end)
...
BattleDungeonBattleCtrl.OnDelete = function(self)
    ...
    (self.bdCtrl):UnRegisterDungeonLogic((DungeonConst.LogicType).BattleStep,
                                         self.__battleStepLogic);
    (self.bdCtrl):UnRegisterDungeonLogic((DungeonConst.LogicType).ChipStep,
                                         self.__chipStepLogic)
    MsgCenter:RemoveListener(eMsgEventId.OnTimelineNoticeCreateResultUI,
                             self.__OnTimelineNoticeOpenResultUI);
    (DungeonBattleBaseCtrl.OnDelete)(self)
end
