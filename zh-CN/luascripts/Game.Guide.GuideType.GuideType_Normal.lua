...
GuideType_Normal.StartGuide = function(self, guideCfg)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.Guide, function(guideWindow)
        ...
        (self.guideWindow):BindGuideSkipEvent(
...
GuideType_Normal.SkipGuide = function(self)
    -- function num : 0_2
    self:EndGuide(false)
end
...
GuideType_Normal.NextStep = function(self)
    -- function num : 0_4 , upvalues : _ENV, GuideConditionChecker
    ...
    if not (GuideConditionChecker.CheckGuideCondition)(guideStepCfg.condition,
                                                       guideStepCfg.condition_arg) then
        self:EndGuide(false)
        return
    end
    ...
    if (self.guideStepCfg).target_type == 0 then
        ...
        if (self.guideStepCfg).target_type == 1 then
            ...
            if (self.guideStepCfg).target_type == 2 then
                self:NextWaitScene(self.guideStepCfg, BindCallback(self,
                                                                   self.NextWaitSceneTarget,
                                                                   self.guideStepCfg))
            else
                if (self.guideStepCfg).target_type == 3 then
                    self:NextWaitScriptTarget(self.guideStepCfg)
                else
                    error("guideStepCfg is target_type error,id:" ..
                              tostring((self.guideStepCfg).id))
                    self:EndGuide(false)
                end
            end
        end
...
GuideType_Normal.RunCurStep = function(self)
    -- function num : 0_5 , upvalues : GuideEnum, _ENV, slipWaitTime
    self.waitTimer = nil;
    ...
    if step_type == (GuideEnum.StepType).LargeDialog then
        (self.guideWindow):ShowGuideLargeDialog(
            (LanguageUtil.GetLocaleText)((self.guideStepCfg).info),
            (self.guideStepCfg).info_type, (self.guideStepCfg).info_pos,
            self.__onCurStepFinish)
    else
        if step_type == (GuideEnum.StepType).Operate then
            ...
            if self.guideStepCfg ~= nil and (self.guideStepCfg).guide_skip then
                self.skipTimer = (TimerManager:GetTimer(slipWaitTime,
                                                        function()
                    ...
                    (self.skipTimer):Stop()
                    self.skipTimer = nil
                end, nil, false, false, self:IsTimerUnScale())):Start()
            end
        else
            if step_type == (GuideEnum.StepType).Avg then
                local avgCtrl = ControllerManager:GetController(
                                    ControllerTypeId.Avg, true)
                avgCtrl:ShowAvg((self.guideStepCfg).avg_name,
                                self.__onCurStepFinish, true, true)
            else
                do
                    if step_type == (GuideEnum.StepType).Code then
                        ...
                        if guideCode == nil then
                            warn((string.format)(
                                     "guide error id[%d],index[%d],stepId[%d],load script is null,interrupt",
                                     (self.guideCfg).id, self.guideIndex,
                                     (self.guideStepCfg).id))
                            self:EndGuide(false)
                            return
                        end
                        local status, current =
                            xpcall(guideCode, debug.traceback)
                        if not status then error(current) end
                    else
                        do
                            if step_type == (GuideEnum.StepType).HeroSmallTalk then
                                (self.guideWindow):ShowGuideHeroSmallTalk(
                                    (LanguageUtil.GetLocaleText)(
                                        (self.guideStepCfg).info),
                                    (self.guideStepCfg).info_type,
                                    (self.guideStepCfg).info_pos,
                                    self.__onCurStepFinish)
                            else
...
GuideType_Normal.__RunCurStepOperator = function(self)
    ...
    if (self.guideStepCfg).target_type == 1 then
        local targetWindow = UIManager:GetWindow(
                                 (self.guideStepCfg).target_parent)
        if targetWindow == nil then
            warn((string.format)(
                     "guide error id[%d],index[%d],stepId[%d],targetWindow is null",
                     (self.guideCfg).id, self.guideIndex, (self.guideStepCfg).id))
            self:EndGuide(false)
            return
        end
        local targetTrans = (targetWindow.transform):Find(
                                (self.guideStepCfg).target_name)
        if IsNull(targetTrans) then
            warn((string.format)(
                     "guide error id[%d],index[%d],stepId[%d],target Transform is null",
                     (self.guideCfg).id, self.guideIndex, (self.guideStepCfg).id))
            self:EndGuide(false)
            return
        end
    ...
    do self:__PlayOperatorGuide() end
end

GuideType_Normal.__PlayOperatorGuide = function(self)
    -- function num : 0_7 , upvalues : _ENV, Type_RectTransform, GuideEnum, CS_EventTriggerListener
    if IsNull(self.curTargetTransform) then
        warn((string.format)(
                 "guide error id[%d],index[%d],stepId[%d],play target Transform is null",
                 (self.guideCfg).id, self.guideIndex, (self.guideStepCfg).id))
        self:EndGuide(false)
        return
    end
    if not ((self.curTargetTransform).gameObject).activeInHierarchy then
        warn((string.format)(
                 "guide error id[%d],index[%d],stepId[%d],target is Hide",
                 (self.guideCfg).id, self.guideIndex, (self.guideStepCfg).id))
        self:EndGuide(false)
        return
    end
    ...
    if (self.guideStepCfg).target_type > 1 then
        isRectTrasform =
            (GR.IsType)(self.curTargetTransform, Type_RectTransform)
    end
    if not self:__RefreshOperatorGuidePos(isRectTrasform, true) then return end
    if (self.guideStepCfg).op_refresh then
        self.__opRefreshTimer = (TimerManager:GetTimer(1, (BindCallback(self,
                                                                        self.__RefreshOperatorGuidePos,
                                                                        isRectTrasform)),
                                                       nil, false, true,
                                                       self:IsTimerUnScale())):Start()
    end
    if (self.guideStepCfg).step_type == (GuideEnum.StepType).Operate then
        local eventTigger = (CS_EventTriggerListener.Get)(
                                self.curTargetTransform)
        eventTigger:onClick("+", self.__onCurStepFinish)
        local exButtonPlus = (self.curTargetTransform):FindComponent(
                                 eUnityComponentID.ExButtonPlus)
        ...
        do
            if (self.guideStepCfg).step_type == (GuideEnum.StepType).Highlight then
                (self.guideWindow):BindContinueBtnEvent(self.__onCurStepFinish,
                                                        (self.guideStepCfg).step_type)
            end
        end
    end
end

GuideType_Normal.__RefreshOperatorGuidePos =
    function(self, isRectTrasform, isFirst)
        -- function num : 0_8 , upvalues : _ENV
        local success = false
        if IsNull(self.curTargetTransform) then return false end
        if isRectTrasform then
            if ((self.curTargetTransform).gameObject).layer == LayerMask.UI then
                success = (self.guideWindow):PlayGuide(self.curTargetTransform,
                                                       UIManager.UICamera)
            else
                success = (self.guideWindow):PlayGuide(self.curTargetTransform,
                                                       UIManager:GetMainCamera())
            end
        else
            local boxCollider = (self.curTargetTransform):FindComponent(
                                    eUnityComponentID.BoxCollider)
            if IsNull(boxCollider) then
                warn((string.format)(
                         "guide error id[%d],index[%d],stepId[%d],target Collider is null",
                         (self.guideCfg).id, self.guideIndex,
                         (self.guideStepCfg).id))
                self:EndGuide(false)
                return
            end
            success = (self.guideWindow):PlayGuideBox(boxCollider,
                                                      UIManager:GetMainCamera())
        end
        do
            if not success then
                warn((string.format)(
                         "guide error id[%d],index[%d],stepId[%d],target is not in screen",
                         (self.guideCfg).id, self.guideIndex,
                         (self.guideStepCfg).id))
                self:EndGuide(false)
                return false
            end
            (self.guideWindow):ShowGuideAniInfo(
                (LanguageUtil.GetLocaleText)((self.guideStepCfg).info),
                (self.guideStepCfg).info_type, (self.guideStepCfg).info_pos,
                isFirst, (self.guideStepCfg).step_type)
            return true
        end
    end

GuideType_Normal.OnCurStepFinish = function(self)
    -- function num : 0_9 , upvalues : GuideEnum
    local step_type = (self.guideStepCfg).step_type
    if step_type ~= (GuideEnum.StepType).LargeDialog or step_type ==
        (GuideEnum.StepType).Operate then
        self:ClearOperatorGuide()
    else
    end
    -- DECOMPILER ERROR at PC38: Unhandled construct in 'MakeBoolean' P1

    if (((step_type ~= (GuideEnum.StepType).Avg or step_type ==
        (GuideEnum.StepType).Code) and step_type ~=
        (GuideEnum.StepType).HeroSmallTalk) or step_type ==
        (GuideEnum.StepType).Highlight) and self.__opRefreshTimer ~= nil then
...
GuideType_Normal.ClearOperatorGuide = function(self)
    -- function num : 0_10 , upvalues : _ENV, CS_EventTriggerListener
    ...
    if self.guideStepCfg == nil then return end
