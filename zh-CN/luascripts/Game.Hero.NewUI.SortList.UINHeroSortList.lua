...
UINHeroSortList.__OnInstantiateItem = function(self, go)
  -- function num : 0_13 , upvalues : UINHeroCardItem
  local heroItem = nil
...
UINHeroSortList.OnInit = function(self)
    ...
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui)
    self.isShowSelected = true
    self.heroItemDic = {} -- DECOMPILER ERROR at PC14: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRest).onInstantiateItem =
        BindCallback(self, self.__OnInstantiateItem) -- DECOMPILER ERROR at PC21: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRest).onChangeItem =
...
UINHeroSortList.InitHeroSortList = function(self, resloader, customSelectHero,
                                            onItemClick, ableNoSelect,
                                            ableSelectSame, isShowSelected,
                                            autoSelect)
    -- function num : 0_1 , upvalues : _ENV
    if isShowSelected == nil then isShowSelected = true end
    self.isShowSelected = isShowSelected
    if self.__isFirstInit then
        MsgCenter:AddListener(eMsgEventId.UpdateHero, self.__onUpdateHeroEvent)
        self.__isFirstInit = false
...
UINHeroSortList.SetHeroListAnchorPosOffset =
    function(self, anchorPosOffset)
        -- function num : 0_2
        self.__anchorPosOffset = anchorPosOffset
    end
...
UINHeroSortList.RefreshHeroSortList = function(self, funcSift, funcSort)
    ...
    if self.__customSelectHero ~= nil then
        (self.__customSelectHero)(curSelectHero)
    end -- DECOMPILER ERROR at PC50: Confused about usage of register: R4 in 'UnsetPending'
...
UINHeroSortList.SetInFormationHero = function(self, inFormationDic)
    ...
    if self.inFormationDic ~= nil then
        for heroId, v in pairs(self.inFormationDic) do
            local heroItem = self:__GetHeroItemById(heroId)
            if heroItem ~= nil then heroItem:SetSelectActive(false) end
        end
    end
    do
        self.inFormationDic = inFormationDic
        if inFormationDic == nil then return end
        for heroId, isBench in pairs(inFormationDic) do
            local heroItem = self:__GetHeroItemById(heroId)
            if heroItem ~= nil then
                heroItem:SetSelectActive(true, isBench)
...
UINHeroSortList.__RefreshSelectHero = function(self, heroData)
    ...
    do
        if heroData == self.selectHero and self.ableNoSelect and
            not self.ableSelectSame then
            local lastHeroItem = self:__GetHeroItemByData(self.selectHero)
            if lastHeroItem ~= nil and self.isShowSelected then
                lastHeroItem:SetSelectActive(false)
            end
...
UINHeroSortList.__OnInstantiateItem = function(self, go)
    ...
    heroItem:Init(go) -- DECOMPILER ERROR at PC16: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.heroItemDic)[go] = heroItem
end
                    ...
                    if self.showRedDot then
                        local ok, heroNode =
                            RedDotController:GetRedDotNode(
                                ...
                                RedDotStaticTypeId.HeroWindow, heroData.dataId)
...
UINHeroSortList.__GetHeroItemByIndex = function(self, index)
    -- function num : 0_17
    local go = ((self.ui).scrollRest):GetCellByIndex(index)
    if go ~= nil then return (self.heroItemDic)[go] end
    return nil
end
