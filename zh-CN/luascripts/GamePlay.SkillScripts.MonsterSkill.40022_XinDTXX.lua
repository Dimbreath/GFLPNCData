...
bs_40022.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.BeforePlaySkill, "bs_40022_1", 1,
                    self.OnBeforePlaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_40022_2", 1,
                    self.OnAfterHurt)
end

bs_40022.OnBeforePlaySkill = function(self, role, context)
    -- function num : 0_2 , upvalues : _ENV
    if (context.skill).maker == self.caster and (context.skill).dataId == 504 and
...
bs_40022.OnAttackTrigger = function(self, target, data)
    ...
    if targets ~= nil and (table.length)(targets) >= 2 and
        ...
        targetNext = targets[2]
        LuaSkillCtrl:CallEffect(targetNext, (self.config).damageEffectId, self) -- DECOMPILER ERROR at PC35: Confused about usage of register: R5 in 'UnsetPending'
        ;
        (self.config).skillCount = 2
    end
    LuaSkillCtrl:CallEffect(target, (self.config).damageEffectId, self)
...
bs_40022.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_4
    -- DECOMPILER ERROR at PC14: Confused about usage of register: R8 in 'UnsetPending'

    if sender == self.caster and skill.dataId == 40022 and
...
bs_40022.CallBack = function(self)
    ...
    if (self.config).totalDamage ~= 0 then
        LuaSkillCtrl:CallHeal((self.config).totalDamage, self, self.caster)
    end -- DECOMPILER ERROR at PC12: Confused about usage of register: R1 in 'UnsetPending'
    
    (self.config).totalDamage = 0 -- DECOMPILER ERROR at PC14: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.config).skillCount = 0
end
