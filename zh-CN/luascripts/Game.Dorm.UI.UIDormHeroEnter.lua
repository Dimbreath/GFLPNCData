...
UIDormHeroEnter.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Cancel, self, self.OnClickCancel) -- DECOMPILER ERROR at PC20: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC27: Confused about usage of register: R1 in 'UnsetPending'
    ;
...
UIDormHeroEnter.InitDormHeroEnter = function(self, allBindFntData, fntData,
                                             resloader, confirmEvent)
    -- function num : 0_1
    self.curFntData = fntData
    self.curHeroId = fntData:GetFntParam()
    self.allBindFntData = allBindFntData
    self.resloader = resloader
    self.confirmEvent = confirmEvent -- DECOMPILER ERROR at PC9: Confused about usage of register: R5 in 'UnsetPending'
    ;
...
UIDormHeroEnter.__OnChangeItem = function(self, go, index)
    ...
    local bind = ((self.allBindFntData).boundDic)[heroData.dataId] ~= nil and
                     heroData.dataId ~= self.curHeroId
    local select = self.selectItemIndex == listIndex
    heroItem:RefreshDormHeroEnterItem(listIndex, heroData,
                                      self.__onClickHeroItem, self.resloader,
                                      bind, select)
...
UIDormHeroEnter.OnClickConfirm = function(self)
    ...
    if newHeroData ~= nil then newHeroId = newHeroData.dataId end
    if newHeroId ~= self.curHeroId then
        if newHeroData ~= nil and ((self.allBindFntData).boundDic)[newHeroId] ~=
            nil then
            local info = (string.format)(
                             ConfigData:GetTipContent(
                                 TipContent.Dorm_ChangeBindConfirm),
                             newHeroData:GetName());
            ...
            do self:__OnConfirm(newHeroId) end
        end
    end
end
