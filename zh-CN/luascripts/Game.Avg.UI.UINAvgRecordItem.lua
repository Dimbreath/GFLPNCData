...
UINAvgRecordItem.InitAvgRecordItem = function(self, avgSystem, recordData,
                                              isLast)
    ...
    if actCfg.branch ~= nil then
        for k, v in ipairs(actCfg.branch) do
            local isSeleted = recordData.selectedActId ~= nil and
                                  recordData.selectedActId == v.jumpAct
            local str = avgCtrl:GetAvgLanguage(v.content)
            str = ((self.ui).tex_Content):GetIndex(isSeleted and 1 or 0,
                                                   tostring(k), str)
            content = content .. "\n" .. str
        end
    end
