...
bs_202203.InitSkill = function(self, isMidwaySkill)
    ...
    self.timenumSkill = {}
    self.summnerListSkill = {}
    self.startCleanSkill = false -- DECOMPILER ERROR at PC13: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).num = 0
end
...
bs_202203.OnAttackTrigger = function(self, grid, data)
    ...
    if summonerEntity ~= nil then
        (self.summonerEffectSkill)[summonerEntity] =
            LuaSkillCtrl:CallEffect(summonerEntity, (self.config).effectIdline,
                                    self, nil, nil, nil, false)
        local cb = BindCallback(self, self.CheckSummonerSkill, summonerEntity) -- DECOMPILER ERROR at PC94: Confused about usage of register: R7 in 'UnsetPending'
        ;
        (self.timenumSkill)[summonerEntity] =
...
bs_202203.CheckSummonerSkill = function(self, summonerEntity)
    -- function num : 0_4 , upvalues : _ENV
    if self.startCleanSkill then return end
    if summonerEntity.hp > 0 then
        local cusEffect = LuaSkillCtrl:CallEffect(self.caster,
                                                  (self.config).effectId3, self)
        local target2 = summonerEntity
        local collisionTrigger = BindCallback(self, self.OnCollision)
        self.skillEmissionSkill = LuaSkillCtrl:CallCircledEmissionStraightly(
                                      self, self.caster, target2, 10, 5,
        ...
        do
            if self.summonerEffectSkill ~= nil then
                if (self.summonerEffectSkill)[summonerEntity] ~= nil then
                    ((self.summonerEffectSkill)[summonerEntity]):Die() -- DECOMPILER ERROR at PC49: Confused about usage of register: R2 in 'UnsetPending'
                    ;
                    (self.summonerEffectSkill)[summonerEntity] = nil
                end
                if (self.timenumSkill)[summonerEntity] ~= nil then
                    ((self.timenumSkill)[summonerEntity]):Stop() -- DECOMPILER ERROR at PC59: Confused about usage of register: R2 in 'UnsetPending'
                    ;
                    (self.timenumSkill)[summonerEntity] = nil
                end
            end
            (table.removebyvalue)(self.summnerListSkill, summonerEntity) -- DECOMPILER ERROR at PC71: Confused about usage of register: R2 in 'UnsetPending'
            ;
...
bs_202203.OnCasterDie = function(self)
    ...
    if self.summnerListSkill ~= nil then
        for i = 1, #self.summnerListSkill do
            local summonerEntity = (self.summnerListSkill)[i]
            if (self.summonerEffectSkill)[summonerEntity] ~= nil then
                ((self.summonerEffectSkill)[summonerEntity]):Die() -- DECOMPILER ERROR at PC23: Confused about usage of register: R6 in 'UnsetPending'
                ;
                (self.summonerEffectSkill)[summonerEntity] = nil
            end
            if (self.timenumSkill)[summonerEntity] ~= nil then
                ((self.timenumSkill)[summonerEntity]):Stop() -- DECOMPILER ERROR at PC33: Confused about usage of register: R6 in 'UnsetPending'
                ;
                (self.timenumSkill)[summonerEntity] = nil
            end
        end
