...
DormRoomEntity.InitHouseObject = function(self, object, clickAction, resloader)
    ...
    self.transform = object.transform -- DECOMPILER ERROR at PC8: Confused about usage of register: R4 in 'UnsetPending'
    ;
    (self.transform).position = (DormUtil.HexToUnityCoord)(self.x, self.y)
    self.resloader = resloader;
    ((CS_EventTrigger.Get)(self.gameObject)):onClick("+", self.__onRoomClicked)
    self.clickAction = clickAction
    if self:IsEmptyRoom() then return end
    self:InitRoomHolder()
...
DormRoomEntity.InitRoomHolder = function(self)
    ...
    local groundX = (ConfigData.game_config).HouseGridCount *
                        (ConfigData.game_config).HouseGridWidth / 2 - offset
    local wallY = (ConfigData.game_config).HouseHeightCount *
                      (ConfigData.game_config).HouseGridWidth - offset
    local floorPos = (Vector3.New)(-groundX, 0, -groundX)
    self.characterHolder = (self.transform):FindComponent("Character",
                                                          eUnityComponentID.Transform) -- DECOMPILER ERROR at PC35: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.characterHolder).localPosition = floorPos
    self.fntHolder = (self.transform):FindComponent("Furniture",
                                                    eUnityComponentID.Transform) -- DECOMPILER ERROR at PC44: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.fntHolder).localPosition = floorPos
    self.floorHolder = (self.transform):FindComponent("FloorHolder",
                                                      eUnityComponentID.Transform) -- DECOMPILER ERROR at PC53: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.floorHolder).localPosition = floorPos
    self.wallHolder = {}
    for i = 1, 4 do
        local wall = (self.transform):FindComponent("Wall/" .. tostring(i),
                                                    eUnityComponentID.Transform)
        local wallLocalPos = ((Quaternion.Euler)(0, (i - 1) * 90, 0)):MulVec3(
                                 (Vector3.New)(groundX, wallY, -groundX - offset))
        local holder = (CS_GameObject("WallHolder" .. tostring(i))).transform -- DECOMPILER ERROR at PC95: Confused about usage of register: R12 in 'UnsetPending'
        ;
...
DormRoomEntity.InitFntMapData = function(self)
    ...
    local fntMap = {} -- DECOMPILER ERROR at PC20: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.mapData).fntMap = fntMap
    local groundMap = {} -- DECOMPILER ERROR at PC23: Confused about usage of register: R6 in 'UnsetPending'
    ;
...
DormRoomEntity.__GetFntMap = function(self, fntType, param)
    ...
    if map == nil then
        error(
            "fnt map is nil, fntType = " .. tostring(fntType) .. ", param = " ..
                tostring(param))
        return
    end
    return map
...
DormRoomEntity.UpdateFntMap = function(self, fntEntity, isAdd, areaList)
    ...
    if areaList == nil then areaList = fntEntity:GetFntAreaList() end
    ...
    for k, pos in pairs(areaList) do
        if isAdd and (DormUtil.FntPosOutMap)(pos.x, pos.y, fntType) then
            ...
            if map[pos.x] == nil then map[pos.x] = {} end
    ...
    for entity, _ in pairs(overlapEntityDic) do entity:SetFntOverlap(overlap) end
...
DormRoomEntity.FntMapOverlap = function(self)
    ...
    for k, v in pairs(self.fntObjDic) do
        if v:IsOverlap() then return true end
    end
    return false
end
...
DormRoomEntity.CreateFntEntity = function(self, fntData, isNew, callback)
    ...
    (self.resloader):LoadABAssetAsync(path, function(prefab)
        ...
        if IsNull(self.gameObject) then return end
        ...
        if callback ~= nil then callback(entity) end
    end)
end
...
DormRoomEntity.AddFntEntity = function(self, fntEntity, isNew)
    ...
    (self.fntObjDic)[fntEntity.gameObject] = fntEntity -- DECOMPILER ERROR at PC5: Confused about usage of register: R3 in 'UnsetPending'
    ;
...
DormRoomEntity.RemoveFntEntity = function(self, fntEntity)
    -- function num : 0_15
    self:UpdateFntMap(fntEntity, false) -- DECOMPILER ERROR at PC6: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (self.fntObjDic)[fntEntity.gameObject] = nil -- DECOMPILER ERROR at PC9: Confused about usage of register: R2 in 'UnsetPending'
    ;
