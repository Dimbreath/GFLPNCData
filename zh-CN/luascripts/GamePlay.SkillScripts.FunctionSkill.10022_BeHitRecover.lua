...
bs_10022.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10022_3", 1,
                    self.OnAfterHurt)
    self.heal_config = {}
end

bs_10022.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if target == self.caster and LuaSkillCtrl:CallRange(1, 1000) <=
        (self.arglist)[2] then
        self:PlayChipEffect()
...
bs_10022.SkillBack = function(self, effect, eventId, target)
    ...
    if eventId == eBattleEffectEvent.Trigger and self:IsReadyToTake() then
        (self.heal_config).baseheal_formula = (self.config).baseheal_formula -- DECOMPILER ERROR at PC15: Confused about usage of register: R4 in 'UnsetPending'
        ;
        (self.heal_config).correct_formula = (self.config).correct_formula -- DECOMPILER ERROR at PC19: Confused about usage of register: R4 in 'UnsetPending'
        ;
        (self.heal_config).heal_number = (self.config).heal_number
        local skillResult = LuaSkillCtrl:CallSkillResult(effect, self.caster)
        LuaSkillCtrl:HealResult(skillResult, self.heal_config)
