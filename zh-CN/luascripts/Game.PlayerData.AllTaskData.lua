...
AllTaskData.ctor = function(self)
    ...
    self.sectorTaskDatas = {}
    for _, id in ipairs((ConfigData.sector).id_sort_list) do
        local sectorCfg = (ConfigData.sector)[id] -- DECOMPILER ERROR at PC29: Confused about usage of register: R7 in 'UnsetPending'
        ;
        (self.sectorTaskDatas)[sectorCfg.achievement] = {}
    end
    self.normalTaskDatas = {}
...
AllTaskData.InitAllTaskData = function(self, questGroup)
    ...
    for k, v in pairs(questGroup.received) do
        ...
        if stcData == nil then
            ...
            local taskData = (TaskData.CreateTaskData)(v, stcData) -- DECOMPILER ERROR at PC35: Confused about usage of register: R9 in 'UnsetPending'
            ;
            (self.taskDatas)[k] = taskData
        end
    end
...
AllTaskData.InitGuideTaksData = function(self)
    ...
    if saveUserData ~= nil and saveUserData.guideData ~= nil and
        (table.count)((saveUserData.guideData).skipGuideTask) > 0 then
        local skipGuideTask = {}
        for taskId, _ in pairs((saveUserData.guideData).skipGuideTask) do
            if (self.taskDatas)[taskId] ~= nil then
                skipGuideTask[taskId] = true
            end
        end -- DECOMPILER ERROR at PC31: Confused about usage of register: R3 in 'UnsetPending'
...
AllTaskData.InitNormalTaskRedDot = function(self)
    -- function num : 0_6 , upvalues : _ENV, TaskEnum
    local ok, TaskWindowNode = RedDotController:GetRedDotNode(
                                   RedDotStaticTypeId.Main,
                                   RedDotStaticTypeId.Task)
    if not ok then return end
    for TaskType, arr in pairs(self.normalTaskDatas) do
        if (TaskEnum.eTaskType).DailyTask == TaskType or
            (TaskEnum.eTaskType).WeeklyTask == TaskType then
            local funcUnLockCrtl = ControllerManager:GetController(
                                       ControllerTypeId.FunctionUnlock, true)
            local isDailyTaskUnlock = funcUnLockCrtl:ValidateUnlock(
                                          proto_csmsg_SystemFunctionID.SystemFunctionID_DailyTask)
        end
        do
            if isDailyTaskUnlock then
                local taskPageNode = TaskWindowNode:AddChildWithPath(TaskType,
                                                                     RedDotDynPath.TaskPagePath)
                local taskUnitNode = taskPageNode:AddChild(
                                         RedDotStaticTypeId.TaskUnit)
                local completeCount = self:RefreshTaskDataArr(arr)
                taskUnitNode:SetRedDotCount(completeCount)
            end
            do
...
AllTaskData.InitSectorTaskRedDot = function(self)
    ...
    for _, id in ipairs((ConfigData.sector).id_sort_list) do
        local sectorCfg = (ConfigData.sector)[id]
        local sectorId = sectorCfg.id
        RedDotController:AddRedDotNodeWithPath(RedDotDynPath.SectorItemPath,
                                               RedDotStaticTypeId.Main,
...
AllTaskData.InitAchivLevelRedDot = function(self)
    ...
    local funcUnLockCrtl = ControllerManager:GetController(
                               ControllerTypeId.FunctionUnlock, true)
    local isAchiUnlock = funcUnLockCrtl:ValidateUnlock(
                             proto_csmsg_SystemFunctionID.SystemFunctionID_Achievement)
    if isAchiUnlock then
        local latestFinishAchi = nil
        for taskType, v in pairs(self.achievementDatas) do
            local pageNode = RedDotController:AddRedDotNodeWithPath(
                                 RedDotDynPath.AchivTaskPagePath,
                                 RedDotStaticTypeId.Main,
...
AllTaskData.RefreshTaskDataArr = function(self, taskDataArr)
    -- function num : 0_9 , upvalues : _ENV
    local count = 0
    if taskDataArr == nil then return end
    for _, taskData in pairs(taskDataArr) do
        if taskData:CheckComplete() then count = count + 1 end
    end
    return count
end
...
AllTaskData.__ExpireData = function(self)
        ...
        while 1 do
            while 1 do
                if #self.waitExpiredDatas > 0 then
                    ...
                    if taskData.expiredTm <= timeStamp then
                        (table.remove)(self.waitExpiredDatas, 1)
                        if (taskData.stcData).end_handle ~= nil and
                            (taskData.stcData).end_handle > 0 then
                            taskData.disappearTm =
                                taskData.expiredTm +
                                    (taskData.stcData).end_handle
                            taskData.expiredTm = 0;
                            (table.insert)(self.waitDisappearDatas, taskData)
                            -- DECOMPILER ERROR at PC71: LeaveBlock: unexpected jumping out IF_THEN_STMT
            ...
            end -- DECOMPILER ERROR at PC74: Confused about usage of register: R3 in 'UnsetPending'
    ...
    do
        ...
        do
            if #self.waitExpiredDatas > 0 then
                local duration = ((self.waitExpiredDatas)[1]).expiredTm -
                                     timeStamp
                if waitDuration == nil or duration < waitDuration then
                    waitDuration = duration
                end
            ...
            while 1 do
                while 1 do
                    if #self.waitDisappearDatas > 0 then
                        local taskData = (self.waitDisappearDatas)[1]
                        if taskData.disappearTm <= timeStamp then
                            (table.remove)(self.waitDisappearDatas, 1) -- DECOMPILER ERROR at PC119: Confused about usage of register: R4 in 'UnsetPending'
                            ;
                            (self.taskDatas)[taskData.id] = nil
                            MsgCenter:Broadcast(eMsgEventId.TaskDelete,
                                                taskData.id)
                            -- DECOMPILER ERROR at PC126: LeaveBlock: unexpected jumping out IF_THEN_STMT
            ...
            do
                do
                    if #self.waitDisappearDatas > 0 then
                        local duration =
                            ((self.waitDisappearDatas)[1]).disappearTm -
                                timeStamp
                        if waitDuration == nil or duration < waitDuration then
                            waitDuration = duration
                        end
                    ...
                    if waitDuration ~= nil then
                        local delay = (math.ceil)(waitDuration)
                        self.__waitExpiredTimer =
                            (TimerManager:GetTimer(delay, self.__ExpireData,
...
AllTaskData.IsSectorType = function(self, type)
    -- function num : 0_11 , upvalues : TaskEnum, _ENV
    local isSectorType = (TaskEnum.eTaskType).SectorTask <= type and type <
                             self.__sectorTaskTypeCeiling
    local sectorId = nil
    if isSectorType then
        for _, id in ipairs((ConfigData.sector).id_sort_list) do
            local sectorCfg = (ConfigData.sector)[id]
...
AllTaskData.IsNormalType = function(self, type)
    -- function num : 0_12 , upvalues : TaskEnum
    do
        return (TaskEnum.eTaskType).MainTask <= type and type <=
...
AllTaskData.IsAchivType = function(self, type)
    -- function num : 0_13 , upvalues : TaskEnum
    local isAchivType = (TaskEnum.eTaskType).Achievement <= type and type <
                            self.__achivTypeCeiling
    do return isAchivType end
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end
        ...
        if self:IsAchivType(taskType) then
            ...
            if taskData:CheckComplete() then
                MsgCenter:Broadcast(eMsgEventId.NewNotice,
                                    (HomeEnum.eNoticeType).Achievement,
...
AllTaskData.__UpdateSectorTaskRedDotCount =
    function(self, taskType, sectorId)
        -- function num : 0_16
    end

AllTaskData.__UpdateAchivTaskRedDotCount =
    function(self, taskType)
        -- function num : 0_17 , upvalues : _ENV
        local funcUnLockCrtl = ControllerManager:GetController(
                                   ControllerTypeId.FunctionUnlock, true)
        local isAchiUnlock = funcUnLockCrtl:ValidateUnlock(
        ...
        if isAchiUnlock then
            local latestFinishAchi = nil
            local ok, pageNode = RedDotController:GetRedDotNode(
                                     RedDotStaticTypeId.Main,
                                     RedDotStaticTypeId.MainSide,
                                     RedDotStaticTypeId.AchivLevel,
                                     RedDotStaticTypeId.AchivLevelPage, taskType)
...
AllTaskData.GetTaskData4Home = function(self)
    -- function num : 0_19 , upvalues : _ENV, TaskEnum
    for _, taskData in pairs(
                           (self.normalTaskDatas)[(TaskEnum.eTaskType).MainTask]) do
        do return taskData end
    end
end
