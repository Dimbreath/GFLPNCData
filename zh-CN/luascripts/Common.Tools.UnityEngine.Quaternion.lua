...
Quaternion.SetNormalize = function(self)
    ...
    local n = self.x * self.x + self.y * self.y + self.z * self.z + self.w *
                  self.w
    if n ~= 1 and n > 0 then
        n = 1 / sqrt(n)
        self.x = self.x * (n)
...
local MatrixToQuaternion = function(rot, quat)
    ...
    if trace > 0 then
        ...
        do
            local i = 1
            local q = {0, 0, 0}
            if (rot[1])[1] < (rot[2])[2] then i = 2 end
            if (rot[i])[i] < (rot[3])[3] then i = 3 end
            local j = _next[i]
            local k = _next[j]
...
Quaternion.LookRotation = function(forward, up)
    ...
    if mag <= 0 then
        error("error input forward to Quaternion.LookRotation" ..
                  tostring(forward))
        return nil
    end
    forward = forward / mag
    if not up then up = _up end
    ...
    if t > 0 then
        ...
        do
            local rot = {
                {right.x, up.x, forward.x}, {right.y, up.y, forward.y},
                {right.z, up.z, forward.z}
            }
            local q = {0, 0, 0}
            local i = 1
            if right.x < up.y then i = 2 end
            if (rot[i])[i] < forward.z then i = 3 end
            local j = _next[i]
            local k = _next[j]
            local t = (rot[i])[i] - (rot[j])[j] - (rot[k])[k] + 1
            local s = 0.5 / sqrt(t)
...
local UnclampedSlerp = function(q1, q2, t)
    ...
    if dot < 0 then
        dot = -dot
        q2 = setmetatable({x = -q2.x, y = -q2.y, z = -q2.z, w = -q2.w},
                          Quaternion)
    end
    if dot < 0.95 then
...
local SanitizeEuler = function(euler)
    ...
    if euler.x < negativeFlip then
        ...
        if positiveFlip < euler.x then euler.x = euler.x - two_pi end
    end
    if euler.y < negativeFlip then
        euler.y = euler.y + two_pi
    else
        if positiveFlip < euler.y then euler.y = euler.y - two_pi end
    end
    if euler.z < negativeFlip then
        euler.z = euler.z + two_pi
    else
        if positiveFlip < euler.z then euler.z = euler.z + two_pi end
    end
end
...
Quaternion.ToEulerAngles = function(self)
    ...
    if check < 0.999 then
        if check > -0.999 then
            local v = (Vector3.New)(-asin(check), atan2(2 * (x * z + w * y),
                                                        1 - 2 * (x * x + y * y)),
                                    atan2(2 * (x * y + w * z),
                                          1 - 2 * (x * x + z * z)))
            SanitizeEuler(v)
            ...
            do
                do
                    local v = (Vector3.New)(half_pi, atan2(2 * (x * y - w * z),
                                                           1 - 2 *
                                                               (y * y + z * z)),
                                            0)
                    SanitizeEuler(v)
                    v:Mul(rad2Deg)
                    do return v end
                    local v = (Vector3.New)(-half_pi, atan2(
                                                -2 * (x * y - w * z),
                                                1 - 2 * (y * y + z * z)), 0)
                    SanitizeEuler(v)
                    v:Mul(rad2Deg)
...
Quaternion.MulVec3 = function(self, point)
    ...
    vec.x = (1 - (num5 + num6)) * point.x + (num7 - num12) * point.y +
                (num8 + num11) * point.z
    vec.y = (num7 + num12) * point.x + (1 - (num4 + num6)) * point.y +
                (num9 - num10) * point.z
...
Quaternion.__mul = function(lhs, rhs)
    ...
    if Quaternion == getmetatable(rhs) then
        return (Quaternion.New)(lhs.w * rhs.x + lhs.x * rhs.w + lhs.y * rhs.z -
                                    lhs.z * rhs.y,
        ...
        if Vector3 == getmetatable(rhs) then return lhs:MulVec3(rhs) end
    end
end
...
Quaternion.unity_quaternion = (CS.UnityEngine).Quaternion -- DECOMPILER ERROR at PC120: Confused about usage of register: R35 in 'UnsetPending'
;
