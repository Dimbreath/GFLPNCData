...
UINOBUAccelerateItem.OnInit = function(self)
    ...
    if ((self.baseItem).ui).btn_Root ~= nil then
        (UIUtil.AddButtonListener)(((self.baseItem).ui).btn_Root, self,
                                   self.AddOne);
...
UINOBUAccelerateItem.InitItem = function(self, itemData, numChangeFunc)
    ...
    if (self.ui).CanvasGroup ~= nil then ((self.ui).CanvasGroup).alpha = 1 end -- DECOMPILER ERROR at PC22: Confused about usage of register: R4 in 'UnsetPending'
...
UINOBUAccelerateItem.InitEmptyItem = function(self, itemId)
    ...
    if (self.ui).CanvasGroup ~= nil then ((self.ui).CanvasGroup).alpha = 0.5 end
    if (self.ui).btn_Reduce ~= nil then
        (((self.ui).btn_Reduce).gameObject):SetActive(false)
    end -- DECOMPILER ERROR at PC32: Confused about usage of register: R3 in 'UnsetPending'
...
UINOBUAccelerateItem.UpdateSelectedNum =
    function(self, num, notCal, needNum)
        -- function num : 0_3 , upvalues : _ENV
        if not num then num = self.selectedNum end
        ...
        if (self.ui).tex_TotalCount ~= nil then
            if needNum == 0 then
                ...
                if num < needNum then
                    ((self.ui).tex_TotalCount):SetIndex(2, tostring(num),
                                                        tostring(needNum))
                else

                    ((self.ui).tex_TotalCount):SetIndex(1, tostring(num),
                                                        tostring(needNum))
                end
            end
        end
...
UINOBUAccelerateItem.PressAdd = function(self)
    ...
    do
        if self.itemNum <= self.selectedNum + addNum or self.addableTime <
            addNum * self.time then
            local newAddNum = (math.min)(self.itemNum - self.selectedNum,
                                         (math.ceil)(
                                             self.addableTime / self.time))
            if newAddNum > 0 then
                addNum = newAddNum
            else
                return
