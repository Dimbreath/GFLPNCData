...
UIATHDungeon.OnBattleStart = function(self)
  -- function num : 0_21 , upvalues : _ENV, cs_MessageCommon, base, CS_GSceneManager_Ins, eFmtFromModule
  ...
  local startBattleFunc = function(curSelectFormationId, callBack)
    -- function num : 0_21_2 , upvalues : _ENV, self, CS_GSceneManager_Ins
    ...
    BattleDungeonManager:SaveBattleWinRewardInfo(ATHRewardInfo)
...
, self.selectItemId))
  ...
  fmtCtrl:InitFromationCtrl(eFmtFromModule.ATHDungeon, ((self.selectChapterItem).cfg).id, enterFormationFunc, exitFormationFunc, startBattleFunc, (self.selectChapterItem).costStrengthNum)
end
...
local base = UIBaseWindow
local UINResourceGroup = require("Game.CommonUI.ResourceGroup.UINResourceGroup")
local UIATHDungeonData = require(
...
local UIATHSuitAreaItem = require("Game.ATHDungeon.UI.UIATHSuitAreaItem")
local UIATHSuitContentItem = require("Game.ATHDungeon.UI.UIATHSuitContentItem")
local UIATHDungeonChapterList = require(
...
UIATHDungeon.OnInit = function(self)
    ...
    self.onBattleStart = BindCallback(self, self.OnBattleStart);
    (UIUtil.AddButtonListener)((self.ui).btn_AllArea, self, self.OnAllAreaClick);
    (UIUtil.AddButtonListener)((self.ui).btn_SuitShowBg, self,
                               self.OnSuitShowBgClick)
    self.dungeonItemPool = (UIItemPool.New)(UIATHDungeonItem,
                                            (self.ui).tog_DungeonItem)
    self.areaExtraItemPool = (UIItemPool.New)(UIATHAreaExtraItem,
                                              (self.ui).areaItem)
    self.suitExtraItemPool = (UIItemPool.New)(UIATHSuitExtraItem,
                                              (self.ui).suitIconItem)
    self.suitDetailItemPool = (UIItemPool.New)(UIATHSuitDetailItem,
                                               (self.ui).suitDetailItem)
    self.suitAreaItemPool = (UIItemPool.New)(UIATHSuitAreaItem,
                                             (self.ui).suitAreaItem)
    self.suitContentItemPool = (UIItemPool.New)(UIATHSuitContentItem,
                                                (self.ui).suitContentItem)
    self.suitDetailItemDic = {};
    ((self.ui).tog_DungeonItem):SetActive(false)
    self.onItemClick = BindCallback(self, self.__onClickItem)
    self.onChapterItemClick = BindCallback(self, self.__onChapterItemClick)
    self.onAreaExtraItemClick = BindCallback(self, self._onAreaExtraItemClick)
    self.onSuitExtraItemClick = BindCallback(self, self._onSuitExtraItemClick)
    self.OnSuitDetailItemClick =
        BindCallback(self, self.__OnSuitDetailItemClick) -- DECOMPILER ERROR at PC110: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).suitDetailList).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC117: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).suitDetailList).onChangeItem =
...
UIATHDungeon.InitATHDungeon = function(self, selectItemId, sector3DWindow,
                                       onBackCallback)
    -- function num : 0_1 , upvalues : cs_ResLoader, _ENV
    self.resLoader = (cs_ResLoader.Create)()
    self.selectATHId = nil
...
UIATHDungeon.OnAllAreaClick = function(self)
    ...
    ((CS.MessageCommon).ShowMessageTips)(
        ConfigData:GetTipContent(TipContent.ATH_Dungeon_All_Area_Extra_Msg))
end
...
UIATHDungeon._onSuitExtraItemClick = function(self)
    -- function num : 0_9 , upvalues : eAthExtraType, _ENV
    if self.extraType ~= eAthExtraType.ExtraSuit then return end
    self.curSuitIdList = {}
    if #self.extraShow == 1 and (self.extraShow)[1] == 120002 then
        for suitId, v in pairs(ConfigData.ath_suit) do
            (table.insert)(self.curSuitIdList, suitId)
        ...
        do
            self.curSuitIdList = self.extraShow
            if #self.curSuitIdList < 1 then return end
            (table.sort)(self.curSuitIdList)
            self._curSelectSuitId = (self.curSuitIdList)[1];
            ((self.ui).suitDetailNode):SetActive(true)
            self:RefillSuitDetailList()
            if self._curSelectSuitId ~= nil then
                local suitItem = self:__GetSuitDetailItemById(
                                     self._curSelectSuitId)
                if suitItem ~= nil then
                    self:__OnSuitDetailItemClick(suitItem)
                end
            end
...
UIATHDungeon.__OnChangeItem = function(self, go, index)
    ...
    item:InitWithData((self.curSuitIdList)[index + 1], self.resLoader)
    item.clickEvent = self.OnSuitDetailItemClick -- DECOMPILER ERROR at PC32: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (self.suitDetailItemDic)[(self.curSuitIdList)[index + 1]] = item
    if self._curSelectSuitId == suitId then
        item:SetAthSuitDetailItemSelect(true)
...
UIATHDungeon.__OnSuitDetailItemClick = function(self, suitItem)
    ...
    local suitId = suitItem:GetAthSuitDetailItemId()
    local oldItem = self:__GetSuitDetailItemById(self._curSelectSuitId)
    if oldItem ~= nil then oldItem:SetAthSuitDetailItemSelect(false) end
    self._curSelectSuitId = suitId
    suitItem:SetAthSuitDetailItemSelect(true)
    self:RefreshSelectAthSuitInfo(suitId)
end
...
UIATHDungeon.__GetSuitItemByIndex = function(self, index)
    -- function num : 0_15
    local go = ((self.ui).suitDetailList):GetCellByIndex(index)
    if go ~= nil then return (self.suitDetailItemDic)[go] end
    return nil
end
...
UIATHDungeon.RefreshSelectAthSuitInfo = function(self, suitId)
    ...
    local areaList = {}
    for id, v in pairs(ConfigData.arithmetic) do
        if suitId == v.suit then (table.insert)(areaList, v.area_type) end
    end
    (table.sort)(areaList)
    for index = 1, #areaList do
        local item = (self.suitAreaItemPool):GetOne()
...
UIATHDungeon.__updateSelectATHItemDisplay =
    function(self, item)
        -- function num : 0_18 , upvalues : _ENV
        if item == nil or item.data == nil then return end
        local athUIData = item.data
        self.double = athUIData.double
        self.haveDouble = (math.max)(0, athUIData.double - athUIData.usedDouble) -- DECOMPILER ERROR at PC23: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.ui).tex_StoryName).text =
            (LanguageUtil.GetLocaleText)(athUIData.desName);
        ((self.ui).tex_LimitCount):SetIndex(0, tostring(self.haveDouble),
                                            tostring(athUIData.double))
        self:__loadChapterUI(athUIData)
        item:OnSelectDisplay(true)
    end
...
UIATHDungeon.__loadChapterUI = function(self, athUIData)
    ...
    if self.chaptersItemPool == nil or self.fstRewardItemPool == nil or
        ...
        self.chaptersItemPool = (self.chaptersUI):CreatePool(UIATHChapterItem,
                                                             RewardItem,
                                                             RewardItem)
    end
    (self.chaptersItemPool):HideAll();
    ...
    for index = 1, #athUIData.dungeonCfgList do
        local item = (self.chaptersItemPool):GetOne()
        local itemState, lockReason = athUIData:GetChapterState(
                                          ((athUIData.dungeonCfgList)[index]).id)
        item:InitWithData((athUIData.dungeonCfgList)[index], itemState, index,
                          self.fstRewardItemPool, self.mbRewardItemPool,
                          athUIData.moduelId, lockReason)
        item.clickEvent = self.onChapterItemClick;
        (table.insert)(chapterItemList, item)
    end
    (self.chaptersUI):UpdateWithChapterList(chapterItemList,
                                            athUIData.totalCompleteChapterCount,
                                            athUIData.totalChapterCount,
                                            self.onBattleStart, athUIData)
end

UIATHDungeon.__loadExtraShowUI = function(self, chapterData)
    -- function num : 0_20 , upvalues : UIATHDungeon, eAthExtraType, eAthAreaType, _ENV
    local extraType = chapterData.day_extra_type
    local extraShow = UIATHDungeon:GetATHExtraCfgData(
                          chapterData.day_extra_type, chapterData.day_extra_show)
    self.extraShow = extraShow
    self.extraType = extraType;
    ((self.ui).areaList):SetActive(false);
    ((self.ui).suitList):SetActive(false);
...
UIATHDungeon.OnBattleStart = function(self)
    ...
    if ((self.selectItem).data).totalDailyLimit ~= -1 and
        ((self.selectItem).data).totalDailyLimit <=
    ...
    if (PlayerDataCenter.stamina):GetCurrentStamina() <
    ...
    local startBattleFunc = function(curSelectFormationId, callBack)
        ...
        BattleDungeonManager:InjectBattleExitEvent(
            BindCallback(self, function(table, itemId)
                ...
                local loadATHUIFunc = BindCallback(self, function()
                    -- function num : 0_21_2_0_0 , upvalues : _ENV, itemId, self
                    UIManager:ShowWindowAsync(UIWindowTypeID.ATHDungeon,
                                              function(window)
                        -- function num : 0_21_2_0_0_0 , upvalues : itemId, self, _ENV
                        if window == nil then return end
                        window:InitATHDungeon(itemId, self.sector3DWindow,
                                              function()
                            -- function num : 0_21_2_0_0_0_0 , upvalues : _ENV
                            local sectorCtrl =
        ...
        (self.battleDungeonNetworkCtrl):CS_BATTLE_DungeonEnter(
            ((self.selectChapterItem).cfg).id, formationData, function()
                -- function num : 0_21_2_1 , upvalues : _ENV, callBack
    ...
    fmtCtrl:InitFromationCtrl(eFmtFromModule.ATHDungeon,
                              ((self.selectChapterItem).cfg).id,
                              enterFormationFunc, exitFormationFunc,
                              startBattleFunc,
                              (self.selectChapterItem).costStrengthNum)
end
