...
bs_10209.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10209_3", 1,
                    self.OnAfterHurt)
end

bs_10209.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack and not isMiss and
        LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[1] and
        (self.arglist)[3] > 0 and (self.arglist)[4] > 0 then
        local tier = 1
        self:PlayChipEffect()
        local time = (self.arglist)[4]
        local pre_tier = target:GetBuffTier((self.config).buffId)
