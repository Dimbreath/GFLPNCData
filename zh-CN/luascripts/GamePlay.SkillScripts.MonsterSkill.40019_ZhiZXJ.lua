...
bs_40019.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_40019_1", 2, self.OnBattleStart)
end

bs_40019.OnBattleStart = function(self)
...
bs_40019.PlaySkill = function(self, data)
  ...
  if LuaSkillCtrl:IsRoleAdjacent(self.caster, role) then
    ...
    self:CallCasterWait((self.config).pre_action_start_time + 15 + (self.config).pre_action_end_time)
...
bs_40019.PhaseMoveHandle = function(self, targetRole, emptyGrid)
  ...
  LuaSkillCtrl:StartTimer(self, 7, BindCallback(self, self.PlayActionAndDamage, targetRole))
end
...
bs_40019.DamageAndEffect = function(self, targetRole)
  ...
  local damageNum = LuaSkillCtrl:CallFormulaNumberWithSkill((self.config).damageFormular, self.caster, targetRole, self)
  LuaSkillCtrl:RemoveLife(damageNum, self, targetRole, true)
  LuaSkillCtrl:RemoveLife(damageNum // 2, self, self.caster, true)
...
bs_40019.config = {
    selectTargetId = 9,
    effectId = 10085,
    damageFormular = 10016,
    action_start = 1008,
    action_end = 1009,
    pre_action_end_time = 6,
...
bs_40019.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_0 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_40019_1", 2,
                    self.OnBattleStart)
end
...
bs_40019.OnBattleStart = function(self)
    -- function num : 0_1 , upvalues : _ENV
    if self:IsReadyToTake() then
        if LuaSkillCtrl:RoleContainsBuffFeature(self.caster,
                                                eBuffFeatureType.NotMove) then
            return
        end
        self:PlaySkill(self.config)
        self:OnSkillTake()
...
bs_40019.PlaySkill = function(self, data)
    ...
    local targetlist = LuaSkillCtrl:CallTargetSelect(self,
                                                     (self.config).selectTargetId,
                                                     20)
    if targetlist == nil or targetlist.Count <= 0 then return end
    local role, emptyGrid = self:CheckAndGetEmptyRole(targetlist)
    if role == nil or emptyGrid == nil then return end
    if LuaSkillCtrl:IsRoleAdjacent(self.caster, role) then
        self:CallCasterWait((self.config).pre_action_end_time + 5)
        self:PlayActionAndDamage(role)
    else
        LuaSkillCtrl:PreSetRolePos(emptyGrid, self.caster)
        local moveAttackTrigger = BindCallback(self, self.PhaseMoveHandle, role,
                                               emptyGrid) -- DECOMPILER ERROR at PC49: Confused about usage of register: R6 in 'UnsetPending'
        ;
        (role.recordTable).isPhaseSelect = true
        self:CallCasterWait((self.config).pre_action_start_time + 15 +
                                (self.config).pre_action_end_time)
        LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster,
                                               (self.config).action_start, 1,
                                               15, moveAttackTrigger)
    end
end
...
bs_40019.PhaseMoveHandle = function(self, targetRole, emptyGrid)
    -- function num : 0_3 , upvalues : _ENV
    LuaSkillCtrl:CanclePreSetPos(self.caster)
    LuaSkillCtrl:CallPhaseMove(self, self.caster, emptyGrid.x, emptyGrid.y, 6,
                               69)
    self.effect = LuaSkillCtrl:CallEffect(self.caster, (self.config).effectId1,
                                          self)
    LuaSkillCtrl:StartTimer(self, 7, BindCallback(self,
                                                  self.PlayActionAndDamage,
                                                  targetRole))
end
...
bs_40019.CheckAndGetEmptyRole = function(self, roleList)
    ...
    for i = 0, roleList.Count - 1 do
        local targetRole = (roleList[i]).targetRole
        if targetRole ~= nil and
            ((targetRole.recordTable).isPhaseSelect == nil or
                not (targetRole.recordTable).isPhaseSelect) then
            emptyGrid = LuaSkillCtrl:FindEmptyGridAroundRole(targetRole)
            if emptyGrid ~= nil then return targetRole, emptyGrid end
        end
    end
    for i = 0, roleList.Count - 1 do
        local targetRole = (roleList[i]).targetRole
...
bs_40019.PlayActionAndDamage = function(self, targetRole)
    ...
    (targetRole.recordTable).isPhaseSelect = false
    local lookAtCallBack = BindCallback(self,
                                        function(table, caster, targetRole)
        -- function num : 0_5_0
        caster:LookAtTarget(targetRole)
    end, self.caster, targetRole)
    LuaSkillCtrl:StartTimer(self, 3, lookAtCallBack)
    local attackEndTrigger =
        BindCallback(self, self.DamageAndEffect, targetRole)
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster,
                                           (self.config).action_end, 1,
                                           (self.config).pre_action_end_time + 3,
                                           attackEndTrigger);
    ((self.caster).auSource):PlayAudioById((self.config).audioId2)
    if self.effect ~= nil then
        (self.effect):Die()
...
bs_40019.DamageAndEffect = function(self, targetRole)
    ...
    local damageNum = LuaSkillCtrl:CallFormulaNumberWithSkill(
                          (self.config).damageFormular, self.caster, targetRole,
                          self)
    LuaSkillCtrl:RemoveLife(damageNum, self, targetRole, true)
    LuaSkillCtrl:RemoveLife(damageNum // 2, self, self.caster, true)
