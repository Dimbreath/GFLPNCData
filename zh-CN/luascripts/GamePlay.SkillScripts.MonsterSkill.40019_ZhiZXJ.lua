...
bs_40019.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_40019_1", 1,
...
bs_40019.FindPosAndMove = function(self)
    ...
    if ((self.caster).recordTable).lastAttackRole == role and
        LuaSkillCtrl:IsRoleAdjacent(role, self.caster) then
        self:CallCasterWait(35)
        local standAttackTrigger = BindCallback(self, self.OnStandAttackTrigger,
                                                role, data)
        LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster,
                                               (self.config).startAnimID, 1, 15,
                                               standAttackTrigger)
    else
        do
            ...
            if grid ~= nil then
                if ((self.caster).recordTable)["40019_restart"] == 5 then
                    local moveAttackTrigger =
                        BindCallback(self, self.OnMoveAttackTrigger, role, data,
                                     grid)
                    ...
                    do
                        do
                            local moveAttackTrigger =
                                BindCallback(self, self.OnMoveAttackTrigger,
                                             role, data, grid)
                            moveAttackTrigger()
                            ...
                            if ((self.caster).recordTable)["40019_restart"] > 0 then
                                ((self.caster).recordTable)["40019_restart"] =
                                    ((self.caster).recordTable)["40019_restart"] -
                                        1
                                self:FindPosAndMove()
                            else
                                targetlist =
                                    LuaSkillCtrl:CallTargetSelect(self, 1001, 10)
...
bs_40019.DamageAndEffect = function(self, target)
    ...
    if damageTarget == nil then return end
    if LuaSkillCtrl:IsAbleAttackTarget(self.caster, damageTarget, 1) then
        (self.caster):LookAtTarget(damageTarget)
        local attackEndTrigger = BindCallback(self, self.OnAttackEndTrigger,
                                              damageTarget, data)
        LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster,
                                               (self.config).endAnimID, 1, 4,
                                               attackEndTrigger);
...
bs_40019.OnFinish = function(self, grid, role, x, y)
    ...
    if grid.x == x and grid.y == y then
        ...
        do
            ...
            if ((self.caster).recordTable)["40019_restart"] > 0 then
                ((self.caster).recordTable)["40019_restart"] =
                    ((self.caster).recordTable)["40019_restart"] - 1
                self:FindPosAndMove()
            else
