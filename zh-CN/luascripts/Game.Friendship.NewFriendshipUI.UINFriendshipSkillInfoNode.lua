...
-- params : ...
-- function num : 0 , upvalues : _ENV
local UINFriendshipSkillInfoNode = class("UINFriendshipSkillInfoNode",
                                         UIBaseNode)
local base = UIBaseNode
...
UINFriendshipSkillInfoNode.InitSkillInfoNode =
    function(self, heroId, fosterCfg, onCancelClickAction)
        -- function num : 0_1
        self.heroId = heroId
        self.fosterCfg = fosterCfg
        self.fosterId = (fosterCfg[1]).id
...
UINFriendshipSkillInfoNode.Refresh = function(self, isUpgrade)
    -- function num : 0_2 , upvalues : _ENV
    self.curLevel = (PlayerDataCenter.allFriendshipData):GetForestLineLevel(
...
UINFriendshipSkillInfoNode.RefreshAllMaterial =
    function(self)
        -- function num : 0_3 , upvalues : _ENV
        self.fitMaterialRequire = true
        local nextCfg = (self.fosterCfg)[self.curLevel + 1]
...
UINFriendshipSkillInfoNode.RefreshNowSkillUI =
    function(self, isUpgrade)
        ...
        if self.curLevel <= 9 then
            ...
            ((self.ui).tex_curLv).text = tostring(self.curLevel)
        end -- DECOMPILER ERROR at PC25: Confused about usage of register: R2 in 'UnsetPending'
        
        ((self.ui).now_tex_Lv).text = "LV" .. tostring(self.curLevel) -- DECOMPILER ERROR at PC35: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).now_tex_Name).text = (LanguageUtil.GetLocaleText)(
                                            ((self.fosterCfg)[self.curLevel]).name)
        local curCfg = (self.fosterCfg)[self.curLevel]
        local currDes = (LanguageUtil.GetLocaleText)(curCfg.describe)
        for index, attrID in ipairs(curCfg.attributeIds) do
            local _, value = ConfigData:GetAttribute(
                                 (curCfg.attributeIds)[index],
                                 (curCfg.attributeNums)[index])
            if (string.endswith)(value, "%", true) then
                value = value .. "%"
            end
            if currDes == nil then
                ...
                currDes = (string.gsub)(currDes,
                                        "{" .. tostring(index - 1) .. "}",
                                        "<color=#FF8400>" .. value .. "</color>")
            end
        end -- DECOMPILER ERROR at PC90: Confused about usage of register: R4 in 'UnsetPending'
        
        ((self.ui).now_tex_Descr).text = currDes -- DECOMPILER ERROR at PC97: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).now_img_SkillIcon).sprite = CRH:GetSprite(curCfg.icon)
        if isUpgrade then
            ((self.ui).tex_curLv):StartScrambleTypeWriter();
...
UINFriendshipSkillInfoNode.RefreshNextSkillUI =
    function(self, isUpgrade)
        -- function num : 0_5 , upvalues : _ENV
        if #self.fosterCfg < self.curLevel + 1 then return end
        -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
        ...
        ((self.ui).next_tex_Lv).text = "LV" .. tostring(self.curLevel + 1) -- DECOMPILER ERROR at PC47: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).next_tex_Name).text =
            (LanguageUtil.GetLocaleText)(
                ((self.fosterCfg)[self.curLevel + 1]).name)
        local nextCfg = (self.fosterCfg)[self.curLevel + 1]
        local nextDes = (LanguageUtil.GetLocaleText)(nextCfg.describe)
        for index, attrID in ipairs(nextCfg.attributeIds) do
            local _, value = ConfigData:GetAttribute(
                                 (nextCfg.attributeIds)[index],
                                 (nextCfg.attributeNums)[index])
            if (string.endswith)(value, "%", true) then
                value = value .. "%"
            end
            if nextDes == nil then
                ...
                nextDes = (string.gsub)(nextDes,
                                        "{" .. tostring(index - 1) .. "}",
                                        "<color=#FF8400>" .. value .. "</color>")
            end
        end -- DECOMPILER ERROR at PC103: Confused about usage of register: R4 in 'UnsetPending'
        
        ((self.ui).next_tex_Descr).text = nextDes -- DECOMPILER ERROR at PC110: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).next_img_SkillIcon).sprite = CRH:GetSprite(nextCfg.icon)
        if isUpgrade then
            ((self.ui).tex_NextLv):StartScrambleTypeWriter();
...
UINFriendshipSkillInfoNode.RefreshLevelRequirement =
    function(self)
        -- function num : 0_6 , upvalues : _ENV
...
UINFriendshipSkillInfoNode.OnClickConfirm =
    function(self)
        -- function num : 0_7 , upvalues : _ENV, cs_MessageCommon
        if self.fitMaterialRequire and self.fitLevelRequire then
            self.fitLevelRequire = false
            ...
            (self.networkCtrl):CS_INTIMACY_UpgradeLine(self.heroId,
                                                       self.fosterId, function()
                -- function num : 0_7_0 , upvalues : self, _ENV
                self.curLevel =
                    (PlayerDataCenter.allFriendshipData):GetForestLineLevel(
                        self.heroId, self.fosterId)
                if #self.fosterCfg <= self.curLevel then
                    self:OnClickCancel()
                    return
            ...
            if not self.fitMaterialRequire then
                (cs_MessageCommon.ShowMessageTips)(
                    ConfigData:GetTipContent(
                        TipContent.Friendship_SkillUpgrade_MatInsufficient))
            else
                if not self.fitMaterialRequire then
                    (cs_MessageCommon.ShowMessageTips)(
                        ConfigData:GetTipContent(
                            TipContent.Friendship_SkillUpgrade_LevelInsufficient))
                end
            end
        end
    end

UINFriendshipSkillInfoNode.OnClickCancel =
    function(self)
        -- function num : 0_8 , upvalues : _ENV
        (UIUtil.PopFromBackStack)()
        if self.onCancelClickAction ~= nil then
