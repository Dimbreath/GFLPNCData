...
UIEpYTrack.InitEpYTrack = function(self, roomData, trunkSizeX, forkLength)
    -- function num : 0_1 , upvalues : _ENV
    self.roomData = roomData
    local sizeDeltaY = ((((self.ui).img_Trunk).transform).sizeDelta).y -- DECOMPILER ERROR at PC14: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (((self.ui).img_Trunk).transform).sizeDelta =
        (Vector2.New)(trunkSizeX, sizeDeltaY)
    local forkSizeY = ((((self.ui).img_LineUP).transform).sizeDelta).y
    local newSize = (Vector2.New)(forkLength, forkSizeY) -- DECOMPILER ERROR at PC28: Confused about usage of register: R7 in 'UnsetPending'
    ;
    (((self.ui).img_LineUP).transform).sizeDelta = newSize -- DECOMPILER ERROR at PC32: Confused about usage of register: R7 in 'UnsetPending'
    ;
    (((self.ui).img_LineDown).transform).sizeDelta = newSize;
    (((self.ui).img_Trunk).gameObject):SetActive(roomData:GetVisible())
    self.forkLineDic = {}
...
UIEpYTrack.SetTrunkLinePass = function(self, lineState, color, height)
    ...
    if lineState == (ExplorationEnum.eTrackLineState).Pass then
        ((self.ui).img_Trunk).color = color -- DECOMPILER ERROR at PC13: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.ui).img_Point).color = color
        sizeDelta.y = height
    else
        ...
        if lineState == (ExplorationEnum.eTrackLineState).Auto then
            ((self.ui).img_Trunk).color = ExplorationEnum.LineAutoModeColor -- DECOMPILER ERROR at PC27: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).img_Point).color = ExplorationEnum.LineAutoModeColor
            sizeDelta.y = height
        else
            ...
            ((self.ui).img_Trunk).color = self.lineDefaultColor -- DECOMPILER ERROR at PC37: Confused about usage of register: R5 in 'UnsetPending'
            ;
            ((self.ui).img_Point).color = self.pointColor
            sizeDelta.y = self.lineDefaultHeight
        end
...
UIEpYTrack.SetForkLinePass = function(self, forkRoom, mapCtrl, lineState, color,
                                      height)
    -- function num : 0_3 , upvalues : ExplorationEnum
    local forkLine = (self.forkLineDic)[forkRoom.position]
    if forkLine == nil then return end
    local sizeDelta = (forkLine.transform).sizeDelta
    if lineState == (ExplorationEnum.eTrackLineState).Pass then
        forkLine.color = color
        sizeDelta.y = height
