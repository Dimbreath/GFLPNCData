...
bs_40031.PlaySkill = function(self, data)
  ...
  if targetlast == nil or targetlast.hp <= 0 or (targetlast.recordTable).isPhaseSelect or LuaSkillCtrl:RoleContainsBuffFeature(targetlast, eBuffFeatureType.CtrlImmunity) then
    targetlast = nil
    ...
    if targetCount > 0 then
      for i = 0, targetCount - 1 do
        if (selectTargets[i]).targetRole ~= nil and not (((selectTargets[i]).targetRole).recordTable).isPhaseSelect and not LuaSkillCtrl:RoleContainsBuffFeature((selectTargets[i]).targetRole, eBuffFeatureType.CtrlImmunity) then
