...
AvgPlayController.InitAvgRedDot = function(self)
  ...
  local mainAvgTriggerCfg = ((ConfigData.story_avg).triggerTypeDic)[eAvgTriggerType.MainAvg]
  if mainAvgTriggerCfg ~= nil then
    for set_place,param1Cfg in pairs(mainAvgTriggerCfg) do
      ...
      if stageCfg == nil then
        ...
        RedDotController:AddRedDotNodeWithPath(RedDotDynPath.SectorLevelDifficultPath, RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, stageCfg.sector, stageCfg.difficulty)
        for param1,param2Cfg in pairs(param1Cfg) do
          for param2,avgId in pairs(param2Cfg) do
            local node = RedDotController:AddRedDotNodeWithPath(RedDotDynPath.SectorMainAvgPath, RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, stageCfg.sector, stageCfg.difficulty, RedDotStaticTypeId.MainAvg, avgId)
            local count = 0
...
AvgPlayController.UpdateMainAvgRedDot = function(self, avgId)
  ...
  local ok, node = RedDotController:GetRedDotNode(RedDotStaticTypeId.Main, RedDotStaticTypeId.Sector, stageCfg.sector, stageCfg.difficulty, RedDotStaticTypeId.MainAvg, avgId)
  if ok then
    local count = 0
    if self:IsAvgUnlock(avgId) and not self:IsAvgPlayed(avgId) then
      count = 1
