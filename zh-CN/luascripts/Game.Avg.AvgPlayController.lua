...
AvgPlayController.TryPlayAvg = function(self, avgTriggerType, place, param1,
    ...
    if ExplorationManager.epCtrl ~= nil then
        ...
        if ((ExplorationManager.epCtrl).autoCtrl):IsAutoModeRunning() and
            callBack ~= nil then callBack() end
        return
    end
    local avgCfg = self:CkeckAvgPlayable(avgTriggerType, place, param1, param2)
    if avgCfg ~= nil then
        (ControllerManager:GetController(ControllerTypeId.Avg, true)):StartAvg(
            avgCfg.script_id, nil, callBack)
    else
...
AvgPlayController.UpdateMainAvgState = function(self, avgId)
    ...
    MsgCenter:Broadcast(eMsgEventId.OnMainAvgStateChange, stageCfg.sector,
                        stageCfg.difficulty, avgId)
end
