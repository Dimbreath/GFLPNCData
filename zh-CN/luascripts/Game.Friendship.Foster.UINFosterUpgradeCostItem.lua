...
UINFosterUpgradeCostItem.InitCostItem = function(self, itemId, num)
    ...
    if self:IsHaveEnoughItem() then
        ...
        ((self.ui).tex_TotalCount):SetIndex(2, tostring(num),
                                            tostring(self.itemNum))
    end -- DECOMPILER ERROR at PC43: Confused about usage of register: R4 in 'UnsetPending'
...
UINFosterUpgradeCostItem.IsHaveEnoughItem =
    function(self)
        -- function num : 0_2
        if self.requireNum <= self.itemNum then return true end
        return false
    end
