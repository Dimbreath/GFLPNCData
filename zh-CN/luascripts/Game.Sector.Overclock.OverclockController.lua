...
OverclockController.SetOverclockDetail =
    function(self, msg)
        -- function num : 0_2 , upvalues : _ENV
        local overclock = msg.data
        if overclock == nil then
...
OverclockController.ReadMaxFreeNum = function(self)
    -- function num : 0_4 , upvalues : _ENV
    local buildingData =
        ((PlayerDataCenter.AllBuildingData).oasisBuilt)[eBuildingId.OasisOverclockTower]
    if buildingData == nil then return 0 end
    local level = buildingData.level
    local levelCfg = (buildingData.levelConfig)[level]
    for index, logic in ipairs(levelCfg.logic) do
        if logic == eLogicType.OverClockFreeNum then
...
OverclockController.IsBuildingFitUnlockRequest =
    function(unlockData)
        -- function num : 0_6 , upvalues : _ENV
        local BuildingData =
            ((PlayerDataCenter.AllBuildingData).oasisBuilt)[unlockData.build_id]
        if BuildingData == nil then return false end
        if BuildingData.level < unlockData.build_level then return false end
        return true
    end
...
OverclockController.__GetStageAllChips =
    function(self, fromModule)
        -- function num : 0_8 , upvalues : eFmtFromModule, _ENV
        if fromModule == eFmtFromModule.SectorLevel then
            local sectorStageCfg = (ConfigData.sector_stage)[self.levelId]
            if sectorStageCfg == nil then return end
            self.chipDic = {}
            for _, epId in pairs(sectorStageCfg.exploration_list) do
                local epCfg = (ConfigData.exploration)[epId]
                if epCfg ~= nil then
            ...
            do
                if fromModule == eFmtFromModule.Infinity then
                    self.chipDic = {}
                    local endlessLevel =
                        ((ConfigData.endless).levelDic)[self.levelId]
                    if endlessLevel == nil then return end
                    local endlessCfg =
                        ((ConfigData.endless)[endlessLevel.sectorId])[endlessLevel.index]
                    if endlessCfg == nil then return end
                    local chips = endlessCfg.chip
...
OverclockController.IsAllChipsLocked = function(self)
    -- function num : 0_11 , upvalues : _ENV
    for chipId, chipData in pairs(self.chipDic) do
        if self:IsChipUnlock(chipId) then return false end
    end
    return true
end
        ...
        do
            ...
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.Overclock_DonorHaveUnlockChip))
            do return nil end
        end
    end
end

OverclockController.AddOverClockOption =
    function(self, id)
        -- function num : 0_13
        -- DECOMPILER ERROR at PC7: Confused about usage of register: R2 in 'UnsetPending'
...
OverclockController.RemoveOverClockOption =
    function(self, id)
        -- function num : 0_14
        -- DECOMPILER ERROR at PC1: Confused about usage of register: R2 in 'UnsetPending'
...
OverclockController.SetChip2OverClockOption =
    function(self, id, chipId)
        -- function num : 0_15
        -- DECOMPILER ERROR at PC2: Confused about usage of register: R3 in 'UnsetPending'

        ((self.tempSelectedOverclockOptionList)[id]).param = chipId
        self:ChangeChip(id, chipId)
        if chipId == nil then self:NotSelect(id) end
    end

OverclockController.GetChip2OverClockOption =
    function(self, id)
        -- function num : 0_16
        if (self.tempSelectedOverclockOptionList)[id] == nil then
            return nil
...
OverclockController.OnConfirm = function(self)
    ...
    if self.selectChangEvent ~= nil then (self.selectChangEvent)() end
end
...
OverclockController.UseOverClock = function(self)
    ...
    for _, data in pairs(self.selectedOverclockOptionList) do
        ...
        if not (self.freeTimesDic)[data.id] then
            do
                (self.freeTimesDic)[data.id] =
                    (self:GetDailyFreeNum(data.id) <= 0 or 0) + 1
                -- DECOMPILER ERROR at PC19: LeaveBlock: unexpected jumping out IF_THEN_STMT
...
OverclockController.SetChangeChipCallback =
...
OverclockController.SetNotSelectCallback =
    function(self, callback)
        -- function num : 0_27 , upvalues : _ENV
        (table.insert)(self.notSelectCallbackList, callback)
    end

OverclockController.ShowChipSelectNode =
    function(self, overclockId)
        -- function num : 0_28
        if self.showChipNodeCallback ~= nil then
            (self.showChipNodeCallback)(overclockId)
        end
...
OverclockController.SetShowChipSelectNode =
    function(self, callback)
        -- function num : 0_29
        self.showChipNodeCallback = callback
    end

OverclockController.AddChangeGCallBack =
    function(self, callback)
        -- function num : 0_30 , upvalues : _ENV
        (table.insert)(self.changeGCallBackList, callback)
        self:__ChangeGCallBack()
...
OverclockController.RemoveChangeGCallBack =
    function(self, callback)
        -- function num : 0_31 , upvalues : _ENV
        (table.removebyvalue)(self.changeGCallBackList, callback)
    end
...
OverclockController.AddAdvancedGNum = function(self, num)
    ...
    if PlayerDataCenter:GetItemCount(ItemIdOfG) < self.advanceGNum + num then
        (cs_MessageCommon.ShowMessageTipsWithErrorSound)(
            ConfigData:GetTipContent(TipContent.Overclock_GInsufficient))
        return false
    end
    self.advanceGNum = self.advanceGNum + num
    self:__ChangeGCallBack()
...
OverclockController.MinAdvancedGNum = function(self, num)
    -- function num : 0_34
    if self.advanceGNum < num then return false end
    self.advanceGNum = self.advanceGNum - num
    self:__ChangeGCallBack()
    return true
end
...
OverclockController.AddAssembleNum = function(self)
    ...
    if self.__maxAssembleNum < self.__curAssembleNum + 1 then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.Overclock_AssembleNumLimit))
        return false
    end
    self.__curAssembleNum = self.__curAssembleNum + 1
    return true
