...
FormationData.CreateDefault = function(id)
    ...
    if isCSUnlock then
        formation.cst = (CommanderSkillModulaData.CreatDefaultFormationCST)(id)
        formation.userSkill = ((formation.cst).saving).skillActive
    end
    return formation
end
...
FormationData.Create = function(id)
    -- function num : 0_5 , upvalues : FormationData, _ENV, CommanderSkillModulaData
    local formation = (FormationData.New)()
    formation.id = id
    formation.data = {}
    local funcUnLockCrtl = ControllerManager:GetController(
...
FormationData.CreateDefultCST = function(formId)
    -- function num : 0_6 , upvalues : _ENV, CommanderSkillModulaData
    local formation = (PlayerDataCenter.formationDic)[formId]
    formation.cst = (CommanderSkillModulaData.CreatDefaultFormationCST)(formId)
    formation.userSkill = ((formation.cst).saving).skillActive
end

FormationData.IsIllegalCST = function(formId, change2Default)
    -- function num : 0_7 , upvalues : _ENV, CommanderSkillModulaData
    local formation = (PlayerDataCenter.formationDic)[formId]
    if formation.cst == nil or
        ((PlayerDataCenter.CommanderSkillModualData).CommanderSkillTreeDataDic)[(formation.cst).treeId] ==
        nil then
        if change2Default then
            formation.cst = (CommanderSkillModulaData.CreatDefaultFormationCST)(
                                formId)
            formation.userSkill = ((formation.cst).saving).skillActive
        end
        return true
