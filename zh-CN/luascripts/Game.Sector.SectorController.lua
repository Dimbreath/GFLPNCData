...
local SctResPillarEntity = require("Game.Sector.Entity.SctResPillarEntity")
...
SectorController.ctor = function(self)
  ...
  self.__startBuildFocusEnter = BindCallback(self, self.StartBuildFocusEnter)
  self.__startBuildFocusEixt = BindCallback(self, self.StartBuildFocusEixt)
  self.__onBuildFocusEnter = BindCallback(self, self.OnBuildFocusEnter)
  self.__onBuildFocusExit = BindCallback(self, self.OnBuildFocusExit)
  self.__ResetToNormalState = BindCallback(self, self.ResetToNormalState)
end

SectorController.OnInit = function(self)
  -- function num : 0_1 , upvalues : _ENV, eSectorState, cs_GameObject, cs_ResLoader, UI3DSectorCanvas, util
  ...
  ;
  ((self.bind).resPillar):SetActive(false)
  ...
  local initFunc = function()
    -- function num : 0_1_2 , upvalues : self, _ENV, eSectorState
    ...
    (((CS.EventTriggerListener).Get)((self.bind).homeCollider)):onClick("+", function()
      -- function num : 0_1_2_0 , upvalues : self, eSectorState
  ...
  local isSectorBuildingUnlock = (self.funcUnLockCrtl):ValidateUnlock(proto_csmsg_SystemFunctionID.SystemFunctionID_SectorBuilding)
...
SectorController.InitSectorItems = function(self)
  -- function num : 0_3 , upvalues : _ENV, SectorItemEntity
  ...
  for sectorId,go in ipairs((self.bind).sectorGoList) do
    if (ConfigData.sector)[sectorId] ~= nil then
      ...
      local sctProgressBuild = (self.uiCanvas):AddSctProgressBuild(sectorId, self.__startBuildFocusEnter)
      do
...
SectorController.DetectedGeneralDungeonUnlock = function(self)
  ...
  local lastSectorMentionId = self:CheckDungeonInSector(self.funcUnLockCrtl, lastSectorId, focusMetionList)
  ...
  if self.funcUnLockCrtl ~= nil then
    ...
    ;
    (self.uiCanvas):SetATHDungeonUnlock(unlock, unlockDes)
  end
end
...
SectorController.CheckDungeonUnlock = function(self, unLockCrtl, unlockId)
  ...
  local unlockDes = nil
  local isUnlock = (self.funcUnLockCrtl):ValidateUnlock(unlockId)
  if not isUnlock then
    unlockDes = (self.funcUnLockCrtl):GetFuncUnlockDecription(unlockId)
  end
...
SectorController.CheckDungeonInSector = function(self, funcUnLockCrtl, lastSectorId, focusMetionList)
  ...
  if dungeonList ~= nil and dungeonList.Count > 0 then
    for i = 0, dungeonList.Count - 1 do
      ...
      if focusMetionList[id] ~= nil and lastId < id and (self.funcUnLockCrtl):ValidateUnlock(moduelId) then
        lastId = id
      end
    end
  end
...
SectorController.ResetToNormalState = function(self, isRstTimeLineFocus)
  -- function num : 0_15 , upvalues : eSectorState, _ENV
  self.sctState = eSectorState.Normal
  ;
  (self.uiCanvas):Show()
  UIManager:ShowWindow(UIWindowTypeID.Sector)
  if not GuideManager:TryTriggerGuide(eGuideCondition.InSectorSceneNormal) or isRstTimeLineFocus then
    self:DetectedGeneralDungeonUnlock()
  end
  ;
...
SectorController.OnSectorItemClicked = function(self, sectorId)
  ...
  if self.__lastEpSectorId ~= nil and self.__lastEpSectorId ~= sectorId then
...
SectorController.StartBuildFocusEnter = function(self, sectorId)
  ...
  ;
  (((self.bind).pd_focusBuilding).gameObject):SetActive(true)
  ;
  (self.camCtrl):EnableSctCamDrag(false)
  local uiSector = UIManager:GetWindow(UIWindowTypeID.Sector)
  if uiSector ~= nil then
    uiSector:OnlyShowSctUITop(true)
  end
  ;
  (self.uiCanvas):Hide()
  ;
  (UIManager:ShowWindow(UIWindowTypeID.ClickContinue)):InitContinue(nil, nil, nil, Color.clear, false)
  ;
  (TimelineUtil.Play)((self.bind).pd_focusBuilding, self.__onBuildFocusEnter)
end
...
SectorController.OnBuildFocusExit = function(self)
  ...
  (self.uiCanvas):Show()
  ;
  (((self.bind).pd_focusBuilding).gameObject):SetActive(false)
  ;
  (self.camCtrl):EnableSctCamDrag(true)
  self:UpdateAllSctBuildRes()
  UIManager:HideWindow(UIWindowTypeID.ClickContinue)
  self:ResetToNormalState()
end
...
SectorController.OnCollectAllSctBuildResComplete = function(self, objList)
  ...
  ;
  (self.buildingNetworkCtr):UpdateRedDotSectorResMax()
  self:UpdateAllSctBuildRes()
...
SectorController.UpdateUncompletedEp = function(self)
  ...
  if lastEpStateCfg ~= nil then
    ...
    do
      if self.__lastEpSectorId ~= nil then
...
SectorController.NewSctResPillarEntity = function(self, parent, index, position)
  ...
  local mat = ((self.bind).resPillarMat)[index]
  local uiResPillar = (self.uiCanvas):NewUISctResPillar()
  entity:InitSctResPillar(go, position, mat, uiResPillar)
  return entity
end
...
SectorController.UpdateAllSctBuildRes = function(self)
  ...
  for sectorId,sctItem in pairs(self.sctItemDic) do
    ...
    for k,buildId in pairs((sctItem.sectorCfg).building) do
      ...
      if builtData ~= nil and builtData.resDatas ~= nil then
        ...
        for itemId,resData in pairs(resDatas) do
          ...
          self:__AddBuildRes(allResDic, resData, countMax)
          self:__AddBuildRes(curSctResDic, resData, countMax)
          hasRes = true
          allHasRes = true
        end
      end
    ...
    if hasRes then
      sctItem:UpdateSctBuildResPillar(curSctResDic)
    end
  end
  local uiSector = UIManager:GetWindow(UIWindowTypeID.Sector)
  if uiSector ~= nil then
...
SectorController.OnDelete = function(self)
  -- function num : 0_36 , upvalues : _ENV, PstConfig
...
SectorController.OnInit = function(self)
    ...
    self.__onSctStageStateChange =
        BindCallback(self, self.OnSctStageStateChange)
    MsgCenter:AddListener(eMsgEventId.OnSectorStageStateChange,
                          self.__onSctStageStateChange)
    self.uiCanvas = (UI3DSectorCanvas.New)();
    (self.uiCanvas):Init((self.bind).uICanvas)
    local initFunc = function()
        ...
        self:UpdateAllSctBuildRes()
        self:UpdateUncompletedEp();
        (((CS.EventTriggerListener).Get)((self.bind).homeCollider)):onClick("+",
                                                                            function()
            -- function num : 0_1_2_0 , upvalues : self, eSectorState
            if (self.camCtrl):InSctCamDrag() or self.sctState ~=
                eSectorState.Normal then return end
            self:OnBtnHomeClicked()
        end)
        self.queueWindow = UIManager:ShowWindow(UIWindowTypeID.BuildingQueue);
        (self.queueWindow):SetSectorType() -- DECOMPILER ERROR at PC33: Confused about usage of register: R0 in 'UnsetPending'
        ;
        (self.queueWindow).OnFinishBuildingEvent =
            BindCallback(self, self.FinishBuilding);
        (self.queueWindow):UpdateBuildingQueue(PlayerDataCenter.timestamp)
        self:InitRedDotEvent()
    end
    ...
    if self.funcUnLockCrtl == nil then return end
    local isSectorBuildingUnlock = (self.funcUnLockCrtl):ValidateUnlock(
                                       proto_csmsg_SystemFunctionID.SystemFunctionID_SectorBuilding)
    self:UnlockBuildFocusEnter(isSectorBuildingUnlock)
end

SectorController.UnlockBuildFocusEnter =
    function(self, bool)
        -- function num : 0_2 , upvalues : _ENV
        self.isSectorBuildingUnlock = bool
        if self.sctItemDic ~= nil then
            for key, value in pairs(self.sctItemDic) do
                ...
                local ok, node = RedDotController:GetRedDotNode(
                                     ...
                                     RedDotStaticTypeId.SectorBuilding, value.id)
                if node:GetRedDotCount() <= 0 then
                    do
                        value:ShowSctResRedDot(
                            not ok or not self.isSectorBuildingUnlock)
                        -- DECOMPILER ERROR at PC34: LeaveBlock: unexpected jumping out IF_THEN_STMT
...
SectorController.InitSectorItems = function(self)
    ...
    for sectorId, go in ipairs((self.bind).sectorGoList) do
        if (ConfigData.sector)[sectorId] ~= nil then
            ...
            local sctInfoItem = (self.uiCanvas):AddSctInfoItem(sectorId,
                                                               self.__onClickSectorItem)
            local sctProgressStage = (self.uiCanvas):AddSctProgressStage(
                                         sectorId, self.__onClickSectorItem)
            local sctProgressBuild = (self.uiCanvas):AddSctProgressBuild(
                                         sectorId, self.__startBuildFocusEnter)
            do
                local sctItemEntity = (SectorItemEntity.New)(self)
                sctItemEntity:InitSectorItemEntity(go, sectorId, sctInfoItem,
                                                   sctProgressStage,
                                                   sctProgressBuild,
                                                   self.uiCanvas)
                sctItemEntity:SetIsSectorBuildingUnlock(
                    self.isSectorBuildingUnlock) -- DECOMPILER ERROR at PC49: Confused about usage of register: R10 in 'UnsetPending'
                ;
                (self.sctItemDic)[sectorId] = sctItemEntity
                -- DECOMPILER ERROR at PC50: LeaveBlock: unexpected jumping out IF_THEN_STMT
...
SectorController.DetectedGeneralDungeonUnlock =
    function(self)
        -- function num : 0_4 , upvalues : _ENV
        local lastSectorId = self.lastSectorId
        local focusMetionList = ConfigData.sector_unlock_mention
        ...
        local lastSectorMentionId = self:CheckDungeonInSector(
                                        self.funcUnLockCrtl, lastSectorId,
                                        focusMetionList)
        lastSectorMentionId = self:FocusSectorAndMentioned(lastSectorMentionId,
                                                           focusMetionList)
        local selectSectorItem = (self.sctItemDic)[lastSectorId]
        if selectSectorItem ~= nil then
            selectSectorItem:SetSctItemSelect()
        ...
        if self.funcUnLockCrtl ~= nil then
            local unlock, unlockDes = nil, nil
            unlock = self:CheckDungeonUnlock(self.funcUnLockCrtl,
                                             proto_csmsg_SystemFunctionID.SystemFunctionID_friendship_sector_Ui);
            (self.uiCanvas):SetFriendshipDungeonUnlock(unlock, unlockDes)
            -- DECOMPILER ERROR at PC50: Overwrote pending register: R6 in 'AssignReg'
            ...
            unlock = self:CheckDungeonUnlock(self.funcUnLockCrtl,
                                             proto_csmsg_SystemFunctionID.SystemFunctionID_ATHDungeon);
            (self.uiCanvas):SetATHDungeonUnlock(unlock, unlockDes)
        end
    end

SectorController.RecordSelectModelDataLocaly =
    function(self, sectorId)
        -- function num : 0_5 , upvalues : _ENV, PstConfig
        if self.localModelData == nil then
...
SectorController.CheckDungeonUnlock = function(self, unLockCrtl, unlockId)
    -- function num : 0_6 , upvalues : _ENV
    if unLockCrtl == nil then
        unLockCrtl = ControllerManager:GetController(
                         ControllerTypeId.FunctionUnlock, true)
    end
    local unlockDes = nil
    local isUnlock = (self.funcUnLockCrtl):ValidateUnlock(unlockId)
...
SectorController.CheckDungeonInSector = function(self, funcUnLockCrtl,
                                                 lastSectorId, focusMetionList)
    -- function num : 0_7 , upvalues : cs_GameData_ins
    lastSectorId = lastSectorId * 10
    if funcUnLockCrtl == nil then return lastSectorId end
    local lastId = lastSectorId
    local dungeonList = (cs_GameData_ins.listDungeonTypeDatas):GetList()
    if dungeonList ~= nil and dungeonList.Count > 0 then
        for i = 0, dungeonList.Count - 1 do
            local id = (dungeonList[i]):GetSectorMentionId()
            local moduelId = (dungeonList[i]):GetFunctionId()
            if focusMetionList[id] ~= nil and lastId < id and
                (self.funcUnLockCrtl):ValidateUnlock(moduelId) then
                lastId = id
            end
        end
    end
    do return lastId end
end

SectorController.FocusSectorAndMentioned =
    function(self, sectorMentionId, focusMetionList)
        -- function num : 0_8 , upvalues : _ENV, PstConfig
        local remoteLastSectorMentionId =
        ...
        if remoteLastSectorMentionId < sectorMentionId then
            ...
            (NetworkManager:GetNetwork(NetworkTypeID.Object)):CS_User_ClientLastSectorMention(
                sectorMentionId, completeRecord)
            return sectorMentionId
        end
        do
            if self.localModelData == nil then
            ...
            local localSectorMentionId =
                (self.localModelData):GetLastLocalSectorMentionId()
            if localSectorMentionId ~= nil and localSectorMentionId > 0 then
                ((self.bind).camTarget):SetPosType(localSectorMentionId)
                return localSectorMentionId
            end
...
SectorController.InitRedDotEvent = function(self)
    ...
    for k, v in pairs(self.sctItemDic) do
        ...
        if node:GetRedDotCount() <= 0 then
            ...
            local ok, node = RedDotController:GetRedDotNode(
                                 RedDotStaticTypeId.Main,
                                 RedDotStaticTypeId.Sector,
                                 RedDotStaticTypeId.SectorBuilding, v.id)
            if node:GetRedDotCount() <= 0 then
                do
                    v:ShowSctResRedDot(not ok)
                    -- DECOMPILER ERROR at PC42: LeaveBlock: unexpected jumping out IF_THEN_STMT
    ...
    self.__sectorItemRedDotEvent = function(node)
        -- function num : 0_9_0 , upvalues : self
        ((self.sctItemDic)[node.nodeId]):ShowSctItemRedDot(
            node:GetRedDotCount() > 0)
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end

    RedDotController:AddListener(RedDotDynPath.SectorItemPath,
                                 self.__sectorItemRedDotEvent)
    self.__sectorBuildRedDotEvent = function(node)
        -- function num : 0_9_1 , upvalues : self
        ((self.sctItemDic)[node.nodeId]):ShowSctResRedDot(
            node:GetRedDotCount() > 0)
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end

    RedDotController:AddListener(RedDotDynPath.SectorBuildingPath,
                                 self.__sectorBuildRedDotEvent)
    -- DECOMPILER ERROR: 4 unprocessed JMP targets
end

SectorController.RemoveRedDotEvent = function(self)
    -- function num : 0_10 , upvalues : _ENV
    RedDotController:RemoveListener(RedDotDynPath.SectorItemPath,
                                    self.__sectorItemRedDotEvent)
    RedDotController:RemoveListener(RedDotDynPath.SectorBuildingPath,
                                    self.__sectorBuildRedDotEvent)
end
...
SectorController.__UpdateTimer = function(self)
    ...
    if isSecond then self:UpdateAllSctBuildRes() end
end
    ...
    if from == AreaConst.Home or from == AreaConst.Oasis then
        ...
        if from == AreaConst.Exploration then
            ...
            local func = function()
                ...
                if (PlayerDataCenter.sectorStage).lastSelectSector ~= nil then
                    UIManager:ShowWindowAsync(UIWindowTypeID.SectorLevel,
                                              function(window)
                        -- function num : 0_14_0_0 , upvalues : self, eSectorState, _ENV
                        ...
                        UIManager:HideWindow(UIWindowTypeID.Sector);
                        (self.queueWindow):Hide()
                        window:InitSectorLevel(
                            (PlayerDataCenter.sectorStage).lastSelectSector,
                            self.__ResetToNormalState)
                    end)
                else
                    self:ResetToNormalState()
                end
            ...
            self.__fromEpCoroutine = (GR.StartCoroutine)(
                                         (util.cs_generator)(func))
        else
            do
                if from == AreaConst.FriendshipMoments then
                    (self.sectorToHomeGo):SetActive(false);
...
SectorController.ResetToNormalState = function(self, isRstTimeLineFocus)
    ...
    (self.uiCanvas):Show()
    UIManager:ShowWindow(UIWindowTypeID.Sector)
    if not GuideManager:TryTriggerGuide(eGuideCondition.InSectorSceneNormal) or
        isRstTimeLineFocus then self:DetectedGeneralDungeonUnlock() end
    (self.queueWindow):Show()
end
...
SectorController.OnSectorItemClicked = function(self, sectorId)
    ...
    (self.queueWindow):Hide()
    local isSectorUnlock = (PlayerDataCenter.sectorStage):IsSectorUnlock(
                               sectorId)
    if not isSectorUnlock then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.Sector_Locked))
        return
    end
    if self.__lastEpSectorId ~= nil and self.__lastEpSectorId ~= sectorId then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.Sector_HasExpNotFinished))
        return
    end
    UIManager:ShowWindowAsync(UIWindowTypeID.SectorLevel, function(window)
        -- function num : 0_18_0 , upvalues : self, eSectorState, _ENV, sectorId
...
SectorController.OnBtnHomeClicked = function(self)
    -- function num : 0_19 , upvalues : eSectorState, _ENV
    if self.sctState ~= eSectorState.Normal then return end
    self.sctState = eSectorState.None
    AudioManager:PlayAudioById(1017)
    ControllerManager:DeleteController(ControllerTypeId.SectorController)
    if self.sectorToHomeGo ~= nil then
...
SectorController.OnSectorToHomeDirectorStopped =
    function(self, director)
        -- function num : 0_20 , upvalues : _ENV
        if self.sectorToHomeDirector == director then
            (self.sectorToHomeDirector):stopped("-",
                                                self.sectorToHomeDirectorStopped);
            ((CS.GSceneManager).Instance):LoadSceneByAB((Consts.SceneName).Main,
                                                        function()
                -- function num : 0_20_0 , upvalues : _ENV
                ControllerManager:DeleteController(
...
SectorController.OnHomeToSectorDirectorStopped =
    function(self, director)
        -- function num : 0_21 , upvalues : eSectorState
        if self.homeToSectorDirector == director then
            (self.homeToSectorDirector):stopped("-",
                                                self.homeToSectorDirectorStopped)
            self:__OnEnterSector()
            if self.sctState ~= eSectorState.SelectSectorLevel then
                self:ResetToNormalState()
            end
...
SectorController.StartBuildFocusEnter = function(self, sectorId)
    -- function num : 0_23 , upvalues : _ENV, eSectorState, cs_MessageCommon
    if not self.isSectorBuildingUnlock then return end
    if (ConfigData.sector)[sectorId] == nil or self.sctState ~=
        eSectorState.Normal then return end
    local isSectorUnlock = (PlayerDataCenter.sectorStage):IsSectorUnlock(
                               sectorId)
    if not isSectorUnlock then
    ...
    self.__curSectorId = sectorId
    local sctItem = (self.sctItemDic)[sectorId]
    local pos = sctItem:GetSctBuildFocusPos() -- DECOMPILER ERROR at PC37: Confused about usage of register: R5 in 'UnsetPending'
    ;
    (((self.bind).focusBuildingCamTarget).transform).position = pos
    self.sctState = eSectorState.FocusBuliding;
    (((self.bind).pd_focusBuilding).gameObject):SetActive(true);
    (self.camCtrl):EnableSctCamDrag(false)
    local uiSector = UIManager:GetWindow(UIWindowTypeID.Sector)
    if uiSector ~= nil then uiSector:OnlyShowSctUITop(true) end
    (self.uiCanvas):Hide();
...
SectorController.OnBuildFocusEnter = function(self)
    ...
    UIManager:ShowWindowAsync(UIWindowTypeID.SectorBuilding, function(window)
        ...
        if window == nil then return end
        local sectorCfg = (ConfigData.sector)[self.__curSectorId]
        window:InitBuilding(sectorCfg.building, self.__startBuildFocusEixt,
                            sectorCfg)
    end)
    UIManager:HideWindow(UIWindowTypeID.ClickContinue)
end
...
SectorController.OnBuildFocusExit = function(self)
    -- function num : 0_26 , upvalues : _ENV
    local uiSector = UIManager:GetWindow(UIWindowTypeID.Sector)
    if uiSector ~= nil then uiSector:OnlyShowSctUITop(false) end
    (self.uiCanvas):Show();
    (((self.bind).pd_focusBuilding).gameObject):SetActive(false);
    (self.camCtrl):EnableSctCamDrag(true)
    self:UpdateAllSctBuildRes()
...
SectorController.CollectAllSctBuildRes =
    function(self)
        -- function num : 0_27 , upvalues : _ENV
        local buildingIdDic = {}
        for k, sctItem in pairs(self.sctItemDic) do
        ...
        do
            if canGetRes then
                if self.__onCollectAllBuildResComplete == nil then
                    self.__onCollectAllBuildResComplete =
                        BindCallback(self, self.OnCollectAllSctBuildResComplete)
                end
                (self.buildingNetworkCtr):SendBuildingCollectGroup(
                    buildingIdDic, self.__onCollectAllBuildResComplete)
            end
        end
    end

SectorController.OnCollectAllSctBuildResComplete =
...
SectorController.ConfirmOver = function(self, id)
    ...
    if buildingData ~= nil and buildingData.belong == BuildingBelong.Sector then
        ...
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.Building_NoticeConstructFinish) ..
                buildingData.name)
    end
    for k, sctItem in pairs(self.sctItemDic) do
        for k2, buildingId in pairs((sctItem.sectorCfg).building) do
...
SectorController.OnSctStageStateChange =
    function(self, data)
        -- function num : 0_32 , upvalues : _ENV
        local sectorIdDic = {}
        for stageId, v in pairs(data) do
            if (PlayerDataCenter.sectorStage):IsStageComplete(stageId) then
                local stageCfg = (ConfigData.sector_stage)[stageId]
                if stageCfg == nil then
                    error("Can\'t get sector_stage cfg, stageId = " ..
                              tostring(stageId))
                    return
                end
                sectorIdDic[stageCfg.sector] = true
        ...
        for sectorId, v in pairs(sectorIdDic) do
            local sctItem = (self.sctItemDic)[sectorId]
            if sctItem ~= nil then sctItem:RefreshSctStageProress() end
        end
    end

SectorController.NewSctResPillarEntity =
    function(self, parent, index, position)
        -- function num : 0_33 , upvalues : SctResPillarEntity
        local go = ((self.bind).resPillar):Instantiate(parent)
        go:SetActive(true)
        local entity = (SctResPillarEntity.New)()
...
SectorController.UpdateAllSctBuildRes = function(self)
    ...
    for sectorId, sctItem in pairs(self.sctItemDic) do
        ...
        if hasRes then sctItem:UpdateSctBuildResPillar(curSctResDic) end
    end
    local uiSector = UIManager:GetWindow(UIWindowTypeID.Sector)
    if uiSector ~= nil then
        uiSector:UpdateUIAllSctBuildRes(allResDic, allHasRes)
...
SectorController.OnDelete = function(self)
    ...
    MsgCenter:RemoveListener(eMsgEventId.BuildingUpgradeComplete,
                             self.__ConfirmOver)
    MsgCenter:RemoveListener(eMsgEventId.OnSectorStageStateChange,
                             self.__onSctStageStateChange)
    self:RemoveRedDotEvent();
    (NetworkManager:GetNetwork(NetworkTypeID.Building)):OnRedDotBuildingTimerUpdate()
    for k, v in pairs(self.sctItemDic) do v:OnDelete() end
    if self.uiCanvas ~= nil then (self.uiCanvas):Delete() end
    if self.resLoader ~= nil then
        (self.resLoader):Put2Pool()
        self.resLoader = nil
    end
