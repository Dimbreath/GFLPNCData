...
LuaSkillCtrl.CallBuffLifeEvent = function(self, luaSkill, role, buffId,
                                          buffTier, duration, lifeEvent)
    -- function num : 0_15
    local battleBuff = nil
    if duration == nil then
        battleBuff = (self.cluaSkillCtrl):GenBuff(luaSkill.cskill, role, buffId,
                                                  buffTier)
    else
        battleBuff = (self.cluaSkillCtrl):GenTimeBuff(luaSkill.cskill, role,
                                                      buffId, buffTier, duration)
    end
    if battleBuff == nil then return nil end
    battleBuff:BindBuffLifeEvent(lifeEvent);
    (self.cluaSkillCtrl):AddBattleBuff(role, battleBuff)
    return battleBuff
end
...
LuaSkillCtrl.RemoveLife = function(self, hurt, luaSkill, target, isRealDmg,
                                   isTriggerSet, buff, isShowText)
    -- function num : 0_37
    if isShowText == nil then isShowText = true end
    return (self.battleCtrl):SetRoleHurt(hurt, luaSkill.cskill, luaSkill.caster,
                                         target, false, false,
                                         isRealDmg or false,
                                         isTriggerSet or false, buff, isShowText)
end
...
LuaSkillCtrl.CallPlayUltMovie = function(self)
    -- function num : 0_67
    if self.cluaSkillCtrl ~= nil then (self.cUltSkillCtrl):PlayUltMovie() end
end
...
LuaSkillCtrl.CallGetCircleSkillCollider =
    function(self, luaSkill, radius, influenceType, onEnter, onStay, onExit)
        -- function num : 0_73
        return (self.cluaSkillCtrl):CallGetCircleSkillCollider(luaSkill.caster,
                                                               radius,
                                                               influenceType,
                                                               onEnter, onStay,
                                                               onExit)
...
LuaSkillCtrl.CallGetRectSkillCollider = function(self, luaSkill, startTarget,
                                                 halfWidth, halfHeight,
                                                 dstTarget, influenceType,
...
LuaSkillCtrl.UnRegisterLuaTriggerById = function(self, luaTriggerId)
    -- function num : 0_105
    if self.luaTrigger == nil then return end
    if luaTriggerId ~= nil then
        (self.luaTrigger):RemoveListenerByType(luaTriggerId)
    end
end
...
LuaSkillCtrl.BroadcastLuaTrigger = function(self, luaTriggerId, ...)
    -- function num : 0_107
    if self.luaTrigger == nil or luaTriggerId == nil then return end
    (self.luaTrigger):Broadcast(luaTriggerId, ...)
end
