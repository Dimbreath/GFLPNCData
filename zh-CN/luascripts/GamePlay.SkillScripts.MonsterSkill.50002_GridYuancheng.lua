...
bs_50002.OnSetHurt = function(self, context)
  ...
  if context.sender ~= self.caster and context.roleType ~= eBattleRoleType.skillCaster and context.target == self.caster and (context.sender).attackRange > 1 and not context.isMiss and ((self.caster).recordTable).sheildValue_50002 ~= 0 then
...
bs_50002.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV, ShieldSkillBase
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.SetHurt, "bs_50002_1", 1, self.OnSetHurt) -- DECOMPILER ERROR at PC17: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.caster).recordTable).sheildValue_50002 =
        (self.caster).hp * 500 // 1000;
...
bs_50002.OnSetHurt = function(self, context)
    ...
    if context.sender ~= self.caster and context.roleType ~=
        eBattleRoleType.skillCaster and context.target == self.caster and
        (context.sender).attackRange > 1 and not context.isMiss and
        ((self.caster).recordTable).sheildValue_50002 ~= 0 then
        ((self.caster).recordTable).sheildValue_50002hurt = context.hurt
        local realHurt = context.hurt -
                             ((self.caster).recordTable).sheildValue_50002
        if realHurt >= 0 then
            (ShieldSkillBase.ClearShieldType)(self.caster,
                                              (self.config).shieldKey)
            context.hurt = realHurt -- DECOMPILER ERROR at PC44: Confused about usage of register: R3 in 'UnsetPending'
            ;
            ((self.caster).recordTable).sheildValue_50002 = 0
        else

            (ShieldSkillBase.ShieldBaseFunc)(context.hurt, self.caster,
                                             (self.config).shieldKey)
            context.hurt = 0 -- DECOMPILER ERROR at PC56: Confused about usage of register: R3 in 'UnsetPending'
            ;
            ((self.caster).recordTable).sheildValue_50002 = 0 - realHurt
        end
        if ((self.caster).recordTable).sheildValue_50002 == 0 and
