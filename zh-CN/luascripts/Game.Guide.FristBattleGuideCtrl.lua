...
FristBattleGuideCtrl.TryStartFirstBattleGuide =
    function(self, taskList)
        ...
        for k, taskData in pairs(taskList) do
            if (taskData.stcData).special_type ==
                (TaskEnum.eSpecialType).FirstBattleGuide1 then
                ...
                local formation = (FormationData.FromCustomData)(1,
                                                                 firstBattle1Config.formHeroList,
                                                                 firstBattle1Config.csTreeId,
                                                                 firstBattle1Config.formCSList)
                BattleDungeonManager:SaveFormation(formation)
                local battleDungeonNetwork =
                    NetworkManager:GetNetwork(NetworkTypeID.BattleDungeon)
                battleDungeonNetwork:CS_BATTLE_DungeonEnter(self.guideDungeonId,
                                                            formation);
                (UIManager:ShowWindow(UIWindowTypeID.ClickContinue)):InitContinue(
                    nil, nil, nil, Color.black, false)
                return true
            else
                do
                    if (taskData.stcData).special_type ==
                        (TaskEnum.eSpecialType).FirstBattleGuide2 then
                        self.battleGuideType = (taskData.stcData).special_type
                        self.guideDungeonId =
                            ((taskData.stcData).special_param)[1]
                        local firstBattle2Config =
                            FirstBattleConfig.firstBattle2
                        local formHeroList = {}
                        for k, v in pairs(firstBattle2Config.formHeroList) do
                            formHeroList[k] = v
                        end
                        formHeroList[(ConfigData.game_config).max_stage_hero + 1] =
                            firstBattle2Config.benchHero
                        local formation =
                            (FormationData.FromCustomData)(1, formHeroList,
                                                           firstBattle2Config.csTreeId,
                                                           firstBattle2Config.formCSList)
                        BattleDungeonManager:SaveFormation(formation)
                        local battleDungeonNetwork =
                            NetworkManager:GetNetwork(
                                NetworkTypeID.BattleDungeon)
                        battleDungeonNetwork:CS_BATTLE_DungeonEnter(
                            self.guideDungeonId, formation);
                        (UIManager:ShowWindow(UIWindowTypeID.ClickContinue)):InitContinue(
                            nil, nil, nil, Color.black, false)
                        return true
                    end
...
FristBattleGuideCtrl.GetFirstBattleConfig =
    function(self)
