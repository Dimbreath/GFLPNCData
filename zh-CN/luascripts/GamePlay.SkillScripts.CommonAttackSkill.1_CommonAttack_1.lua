...
bs_1.PlaySkill = function(self, passdata)
    ...
    local bewitch = (self.caster):ContainBuffFeature(eBuffFeatureType.Bewitch)
    if bewitch ~= self.lastBewitch then
        self.lastAttackRole = nil -- DECOMPILER ERROR at PC28: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.caster).recordTable).lastAttackRole = nil
        self.lastBewitch = bewitch
    else
        if self.lastAttackRole ~= nil and (self.lastAttackRole).belongNum ~=
    ...
    if targetList.Count ~= 0 then
        ...
        self.lastAttackRole = (targetList[0]).targetRole -- DECOMPILER ERROR at PC109: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.caster).recordTable).lastAttackRole = self.lastAttackRole;
        (self.caster):LookAtTarget(self.lastAttackRole)
        if LuaSkillCtrl:IsAbleAttackCheckExcludedDir(self.caster,
            ...
            LuaSkillCtrl:StartTimer(self, 3,
                ...
                -- function num : 0_2_0 , upvalues : _ENV, self
                if LuaSkillCtrl:IsAbleAttackTarget(self.caster, atkRole,
                                                   data.rangeOffset +
            ...
            self.rotateWaited = false
            if LuaSkillCtrl:IsAbleAttackTarget(self.caster, self.lastAttackRole,
                                               data.rangeOffset +
                                                   (self.caster).attackRange) then
                self:RealPlaySkill(self.lastAttackRole, data)
            end
        ...
        self.lastAttackRole = nil -- DECOMPILER ERROR at PC156: Confused about usage of register: R5 in 'UnsetPending'
        ;
        ((self.caster).recordTable).lastAttackRole = nil
    end
end
...
bs_1.CallSelectEffect = function(self)
    -- function num : 0_3 , upvalues : _ENV
    if ((self.caster).recordTable).completeFirstComatk and self.displaySelectEfc and
        self.lastAttackRole ~= nil then
        LuaSkillCtrl:CallSelectTargetEffect(self, self.lastAttackRole)
        self.displaySelectEfc = false
    end
end
        ...
        if LuaSkillCtrl:IsAbleAttackTarget(self.caster, target,
            ...
            do
                ...
                if LuaSkillCtrl:IsAbleAttackTarget(self.caster, target,
                    ...
                    if data.effectstartId4 ~= nil then
                        if atkActionId == data.action1 then
                            LuaSkillCtrl:CallEffect(target, data.effectstartId4,
                                                    self, nil, nil,
                                                    atkSpeedRatio)
                        else
                            LuaSkillCtrl:CallEffect(target, data.effectstartId5,
                                                    self, nil, nil,
                                                    atkSpeedRatio)
                        end
                    end
