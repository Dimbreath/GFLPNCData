...
UINUltSkillMpParticleItem.InitUltSkillMpParticle = function(self, startPos, endPos, ratio, completeEvent)
  ...
  ;
  (((self.ui).moveTween).tween):ChangeEndValue((Vector3.unity_vector3)(endPos.x, endPos.y, 0), true)
  ;
...
UINUltSkillMpParticleItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (((self.ui).moveTween).onComplete):AddListener(
        BindCallback(self, self.OnMoveTweenComplete))
    self.psEmission = ((self.ui).particle).emission
    self.psEmBurst = (self.psEmission):GetBurst(0)
end

UINUltSkillMpParticleItem.InitUltSkillMpParticle =
    function(self, startPos, endPos, ratio, completeEvent)
        -- function num : 0_1 , upvalues : _ENV
        self.completeEvent = completeEvent -- DECOMPILER ERROR at PC2: Confused about usage of register: R5 in 'UnsetPending'
        ;
        (self.transform).anchoredPosition = startPos;
        (((self.ui).moveTween).tween):ChangeEndValue(
            (Vector3.unity_vector3)(endPos.x, endPos.y, 0), true);
        ((self.ui).moveTween):DORestart()
        local particleCount = (math.ceil)(ratio / (self.ui).perParticleRatio)
        particleCount = (math.min)(particleCount, (self.ui).maxParticleCount)
        -- DECOMPILER ERROR at PC41: Confused about usage of register: R6 in 'UnsetPending'

        if (self.psEmBurst).minCount ~= particleCount or
            (self.psEmBurst).maxCount ~= particleCount then
            (self.psEmBurst).minCount = particleCount -- DECOMPILER ERROR at PC43: Confused about usage of register: R6 in 'UnsetPending'
            ;
            (self.psEmBurst).maxCount = particleCount;
            (self.psEmission):SetBurst(0, self.psEmBurst)
        end
...
UINUltSkillMpParticleItem.OnMoveTweenComplete =
    function(self)
        -- function num : 0_2
        if self.completeEvent ~= nil then (self.completeEvent)(self) end
    end
