...
CheckAvg.GetUnlockInfo = function(param)
  ...
  if avgCfg ~= nil then
    return (string.format)("完成剧情%s后解锁", (LanguageUtil.GetLocaleText)(avgCfg.name))
  end
  return ""
end
...
CheckAvg.IsCompletedAvg = function(avgId)
    -- function num : 0_1 , upvalues : _ENV
    if avgId == 0 then return true end
    return
        (ControllerManager:GetController(ControllerTypeId.AvgPlay, true)):IsAvgPlayed(
            avgId)
end
...
CheckAvg.GetUnlockInfo = function(param)
    ...
    local avgCfg = (ConfigData.story_avg)[avgId]
    if avgCfg ~= nil then
        return (string.format)("完成剧情%s后解锁",
                               (LanguageUtil.GetLocaleText)(avgCfg.name))
    end
    return ""
end
