...
bs_1049.config = {effectId = 10203, actionId = 1002, stunBuffId = 66, hitReduceBuffId = 115, 
hurtConfig = {hit_formula = 9991, basehurt_formula = 10076, crit_formula = 0, crithur_ratio = 0}
, audioId1 = 74}
...
bs_1049.PlaySkill = function(self, data)
  ...
  if grid ~= nil then
    ...
    LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, (self.config).actionId, 1, 15, triggerCallBack)
  end
end

bs_1049.OnActionCallBack = function(self, target, roles)
  -- function num : 0_2 , upvalues : _ENV
  LuaSkillCtrl:CallEffectWithArg(target, (self.config).effectId, self, false, self.OnEffectTrigger, roles)
end

bs_1049.OnEffectTrigger = function(self, roles, effect, eventId, target)
  -- function num : 0_3 , upvalues : _ENV
  if effect.dataId == (self.config).effectId and eventId == eBattleEffectEvent.Trigger and roles ~= nil and roles.Count > 0 then
    for i = 0, roles.Count - 1 do
      do
        ...
        LuaSkillCtrl:CallBuff(self, roles[i], (self.config).stunBuffId, 1, (self.arglist)[2])
        if (self.arglist)[4] > 0 then
          LuaSkillCtrl:StartTimer(nil, (self.arglist)[2], function()
    -- function num : 0_3_0 , upvalues : _ENV, self, roles, i
    LuaSkillCtrl:CallBuff(self, roles[i], (self.config).hitReduceBuffId, 1, (self.arglist)[3])
  end
)
        end
      end
