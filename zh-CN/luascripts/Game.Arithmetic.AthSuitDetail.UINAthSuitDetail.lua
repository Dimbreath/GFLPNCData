...
local UINAthSuitDetailItem = require(
                                 "Game.Arithmetic.AthSuitDetail.UINAthSuitDetailItem")
...
UINAthSuitDetail.OnInit = function(self)
    ...
    self.__OnAthSuitItemClick = BindCallback(self, self.OnAthSuitItemClick) -- DECOMPILER ERROR at PC30: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).scrollRect).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC37: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ...
    self.suitAttrPool = (UIItemPool.New)(UINAthSuitAttr,
                                         (self.ui).suitContentItem)
end

UINAthSuitDetail.InitAthSuitDetail = function(self, resLoader, heroSuitDic)
    -- function num : 0_1
    ...
    if self._isShowCurSuit then
        ...
        do
            ...
            do
                ...
                local SuitId = (self.curSuitIdList)[1]
                if SuitId ~= nil then
                    local suitItem = self:__GetSuitItemById(SuitId)
...
UINAthSuitDetail.__OnChangeItem = function(self, go, index)
    ...
    do
        ...
        item:InitAthSuitDetailItem(suitId, self.__OnAthSuitItemClick,
                                   self.resLoader, curCount)
...
UINAthSuitDetail.RefreshSelectAthSuitInfo =
    function(self, suitId)
...
UINAthSuitDetail.__GetSuitItemByIndex = function(self, index)
    ...
    if go ~= nil then return (self.suitItemDic)[go] end
    return nil
end
