...
bs_306.OnSetHurt = function(self, context)
    ...
    if (self.caster):GetBuffTier((self.config).buffId1) ~= 0 and
        (context.skill).isCommonAttack and context.hurt > 0 and context.target ==
        self.caster then
        context.hurt = 0
...
bs_306.OnBuffDie = function(self, buff, target, removeType)
    -- function num : 0_5
    if target == self.caster and buff.dataId == (self.config).buffId1 and
        self.effect ~= nil then
        (self.effect):Die()
        self.effect = nil
    end
end
