...
DynEpBuffChip.ExecuteChipDynPlayer = function(self, dynPlayer)
  -- function num : 0_5 , upvalues : ChipEnum, _ENV
  if dynPlayer == nil then
    return (ChipEnum.ePropActionResult).Skip
  end
  if dynPlayer:ContainTempChip(self) then
    error("该全局芯片已经生效:" .. tostring(self.dataId))
    return (ChipEnum.ePropActionResult).Fail
  else
    ;
    (self.chipBattleData):ExecutePropDynPlayer(dynPlayer, self:GetCount())
    dynPlayer:AddTempChip(self)
  end
end

DynEpBuffChip.RollbackChipDynPlayer = function(self, dynPlayer)
  -- function num : 0_6 , upvalues : ChipEnum
  if dynPlayer == nil or not dynPlayer:ContainTempChip(self) then
    return (ChipEnum.ePropActionResult).Skip
  end
  ;
  (self.chipBattleData):RollbackDynPlayer(dynPlayer, self:GetCount())
  dynPlayer:RemoveTempChip(self)
  return (ChipEnum.ePropActionResult).Success
end
...
DynEpBuffChip.ctor = function(self, dataId, num)
    ...
    self.chipCfg = chipCfg
    self.chipBattleData = (ChipBattleData.New)(self.chipCfg, self.count,
                                               (ExplorationEnum.eBattleSkillType).TempChip)
end
...
DynEpBuffChip.ExecuteChipHero = function(self, role)
    -- function num : 0_3 , upvalues : ChipEnum, _ENV
    if role == nil then return (ChipEnum.ePropActionResult).Skip end
    if role:ContainTempChip(self) then
        error("该全局芯片已经生效:" .. tostring(self.dataId))
        return (ChipEnum.ePropActionResult).Fail
    else
...
DynEpBuffChip.ExecuteChipDynPlayer = function(self, dynPlayer)
    -- function num : 0_5 , upvalues : ChipEnum, _ENV
    if dynPlayer == nil then return (ChipEnum.ePropActionResult).Skip end
    if dynPlayer:ContainTempChip(self) then
        error("该全局芯片已经生效:" .. tostring(self.dataId))
        return (ChipEnum.ePropActionResult).Fail
    else
