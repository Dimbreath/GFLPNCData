...
AvgNetwork.InitNetwork = function(self)
    -- function num : 0_1 , upvalues : _ENV
    self:RegisterNetwork(proto_csmsg_MSG_ID.MSG_SC_AVG_Complete, self,
                         proto_csmsg.SC_AVG_Complete, self.SC_AVG_Complete)
end
...
AvgNetwork.CS_AVG_Complete = function(self, avgId, callback)
    ...
    (self.__avgCompleteTab).avgId = avgId
    self:SendMsg(proto_csmsg_MSG_ID.MSG_CS_AVG_Complete,
                 proto_csmsg.CS_AVG_Complete, self.__avgCompleteTab)
    cs_WaitNetworkResponse:StartWait(proto_csmsg_MSG_ID.MSG_CS_AVG_Complete,
                                     callback,
                                     proto_csmsg_MSG_ID.MSG_SC_AVG_Complete)
end
    ...
    do
        if msg.ret ~= proto_csmsg_ErrorCode.None then
            ...
            cs_WaitNetworkResponse:AddWaitData(
                proto_csmsg_MSG_ID.MSG_CS_AVG_Complete, false)
            cs_WaitNetworkResponse:WaitComplete(
                proto_csmsg_MSG_ID.MSG_CS_AVG_Complete)
        end
        cs_WaitNetworkResponse:AddWaitData(
            proto_csmsg_MSG_ID.MSG_CS_AVG_Complete, true)
    end
end
