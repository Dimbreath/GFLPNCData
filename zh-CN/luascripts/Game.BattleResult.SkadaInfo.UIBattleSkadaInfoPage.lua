...
UIBattleSkadaInfoPage.SwitchRoleSkada = function(self, roleSkadaDetail)
  ...
  if not self.belongPlayer or not PathConsts:GetCharacterPicPath(((roleSkadaDetail.role).character):GetResName()) then
    local path = PathConsts:GetCharacterSmallPicPath(((roleSkadaDetail.role).character):GetResName())
  end
...
local UIBattleSkadaInfoPage = class("UIBattleSkadaInfoPage", UIBaseNode)
local base = UIBaseNode
local UIBattleSkadaInfoItem = require(
                                  "Game.BattleResult.SkadaInfo.UIBattleSkadaInfoItem")
local UIPie = require("Game.BattleResult.Pie.UIPie")
local BattleSkadaEnum = require("Game.BattleResult.Skada.BattleSkadaEnum")
UIBattleSkadaInfoPage.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, BattleSkadaEnum, UIBattleSkadaInfoItem, UIPie
    ...
    ((self.ui).skillItem):SetActive(false)
    self.infoItemPool = (UIItemPool.New)(UIBattleSkadaInfoItem,
                                         (self.ui).skillItem)
    self.pie = (UIPie.New)();
    (self.pie):Init((self.ui).pieChartNode)
end

UIBattleSkadaInfoPage.InitSkadaInfoPage =
    function(self, roleSkadaDetail, belongPlayer, resLoader, skadaType,
             onSwitchPlayerEvent)
        -- function num : 0_1
        self.skadaType = skadaType
        self.onSwitchPlayerEvent = onSwitchPlayerEvent
...
UIBattleSkadaInfoPage.SwitchRoleSkada = function(self, roleSkadaDetail)
    ...
    ;
    ((self.ui).tex_HeroName).text = (roleSkadaDetail.role).name
    if not self.belongPlayer or not PathConsts:GetCharacterPicPath(
        ((roleSkadaDetail.role).character):GetResName()) then
        local path = PathConsts:GetCharacterSmallPicPath(
                         ((roleSkadaDetail.role).character):GetResName())
    ...
    if self.belongPlayer then
        ...
        uvRect:Set(0, 0, 1, 1)
    end -- DECOMPILER ERROR at PC53: Confused about usage of register: R4 in 'UnsetPending'
...
UIBattleSkadaInfoPage.RefreshSkadaInfoUI =
    function(self)
        -- function num : 0_4 , upvalues : BattleSkadaEnum, _ENV
        ((self.ui).text_count):SetActive(
        ...
        if self.skadaType == (BattleSkadaEnum.eBattleSkadaType).Damage then
            ...
            for k, v in ipairs(detailList) do
                local infoItem = (self.infoItemPool):GetOne()
                local color = (self.pie):GetPieColor(k)
                infoItem:InitSkadaInfoItem(color, (v.csDetail).name,
                                           (v.csDetail).count,
                                           (v.csDetail).damage, v.proportion);
                (table.insert)(proportionTab, v.proportion)
            end
        elseif self.skadaType == (BattleSkadaEnum.eBattleSkadaType).TakeDamage then
            local detailList = (self.roleSkadaDetail)[self.skadaType]
            for k, v in ipairs(detailList) do
                local infoItem = (self.infoItemPool):GetOne()
                local color = (self.pie):GetPieColor(k)
                infoItem:InitSkadaInfoItem(color, (v.csDetail).name, nil,
                                           (v.csDetail).damage, v.proportion);
                (table.insert)(proportionTab, v.proportion)
            end
        elseif self.skadaType == (BattleSkadaEnum.eBattleSkadaType).Heal then
...
UIBattleSkadaInfoPage.OnClickLeft = function(self)
    -- function num : 0_5
    if self.onSwitchPlayerEvent ~= nil then (self.onSwitchPlayerEvent)(true) end
end

UIBattleSkadaInfoPage.OnClickRight = function(self)
    -- function num : 0_6
    if self.onSwitchPlayerEvent ~= nil then (self.onSwitchPlayerEvent)(false) end
end
