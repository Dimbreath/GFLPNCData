...
local guideCheckFunc = {
    ...
    [eGuideCondition.InHome] = function()
        -- function num : 0_1 , upvalues : _ENV, HomeEnum
        local homeController = ControllerManager:GetController(
                                   ControllerTypeId.HomeController, true)
        do
            return homeController.homeState == (HomeEnum.eHomeState).Normal
        end
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end,
    [eGuideCondition.InExploration] = function()
    ...
    [eGuideCondition.InExplorationScene] = function(condition_arg)
        -- function num : 0_3 , upvalues : _ENV
        local inExplorationScene =
            not ExplorationManager:IsInExplorationAndScene() or
                ((ExplorationManager.epCtrl).sceneCtrl):InBattleScene() == false
        if not inExplorationScene then return false end
        if #condition_arg == 0 then return true end
        if condition_arg[1] ~= ExplorationManager:GetCurExplorationId() then
            return false
        end
        do return true end
    ...
    [eGuideCondition.InBattleDeploy] = function()
        -- function num : 0_4 , upvalues : _ENV
        if ExplorationManager:IsInExplorationAndScene() then
            return ((ExplorationManager.epCtrl).battleCtrl):IsBattleState(
                       eBattleState.Deploy)
        end
    end,
    [eGuideCondition.InSelectChip] = function()
    ...
    [eGuideCondition.InFormation] = function(condition_arg)
        -- function num : 0_11 , upvalues : _ENV
        local formationWindow = UIManager:GetWindow(UIWindowTypeID.Formation)
        if formationWindow == nil then return false end
        local formationCtrl = ControllerManager:GetController(
                                  ControllerTypeId.Formation)
        if formationCtrl == nil then return false end
        if #condition_arg == 0 then return true end
        local fromModule, moduleArg = formationCtrl:GetFormatioFromModule()
        if condition_arg[1] ~= fromModule then return false end
        if #condition_arg > 1 and condition_arg[2] ~= moduleArg then
            return false
        end
        return true
    ...
    [eGuideCondition.InNewMonsterDetail] = function()
        -- function num : 0_15 , upvalues : _ENV
        if ExplorationManager:IsInExplorationAndScene() then
            local inBattleDeploy =
                ((ExplorationManager.epCtrl).battleCtrl):IsBattleState(
                    eBattleState.Deploy)
        end
        if not inBattleDeploy then return false end
        do
            return ((ExplorationManager.epCtrl).battleCtrl).hasNewEnemyDetail ==
                       true
        end
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end,
    ...
    [eGuideCondition.InLottery] = function()
        -- function num : 0_17 , upvalues : _ENV
        local lotteryCtrl = ControllerManager:GetController(
    ...
    [eGuideCondition.InATHStrengthen] = function()
        -- function num : 0_18 , upvalues : _ENV
        do
            return UIManager:GetWindow(UIWindowTypeID.AthStrengthen) ~= nil
        end
        -- DECOMPILER ERROR: 1 unprocessed JMP targets
    end,
    [eGuideCondition.FInHome] = function()
...
GuideConditionChecker.CheckGuideCondition =
    function(condition_type, condition_arg)
        -- function num : 0_22 , upvalues : guideCheckFunc
        local checkFunc = guideCheckFunc[condition_type]
        if checkFunc ~= nil then
            return checkFunc(condition_arg)
