    ...
    local generator = setmetatable({
        w_func = function()
            ...
            local status, current =
                xpcall(func, debug.traceback, unpack(params))
...
local auto_id_map = function()
    ...
    xlua.hotfix = function(cs, field, func)
        ...
        if map_info_of_type then
            ...
            do do return org_hotfix(cs, field, func) end end
        end
    end
...
local hotfix_ex = function(cs, field, func)
    -- function num : 0_7 , upvalues : _ENV, unpack
    assert(type(field) == "string" and type(func) == "function",
...
local enum_or_op = ((debug.getmetatable)(
                       (((CS.System).Reflection).BindingFlags).Public)).__bor
local enum_or_op_ex = function(first, ...)
    -- function num : 0_9 , upvalues : _ENV, enum_or_op
    for _, e in ipairs({...}) do first = enum_or_op(first, e) end
    return first
end

local createdelegate = function(delegate_cls, obj, impl_cls, method_name,
                                parameter_type_list)
    -- function num : 0_10 , upvalues : enum_or_op_ex, _ENV
    local flag = enum_or_op_ex((((CS.System).Reflection).BindingFlags).Public,
    ...
    if not parameter_type_list or
        not (typeof(impl_cls)):GetMethod(method_name, flag, nil,
                                         parameter_type_list, nil) then
        local m = (typeof(impl_cls)):GetMethod(method_name, flag)
    end
    return (((CS.System).Delegate).CreateDelegate)(typeof(delegate_cls), obj, m)
end
...
local state = function(csobj, state)
    ...
    state.__index = function(obj, k)
        -- function num : 0_11_0 , upvalues : _ENV, state, csobj_index
        if not rawget(state, k) then return csobj_index(obj, k) end
    end
