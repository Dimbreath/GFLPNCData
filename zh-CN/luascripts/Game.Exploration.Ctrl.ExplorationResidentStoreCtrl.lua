...
local ExplorationResidentStoreCtrl = class("ExplorationResidentStoreCtrl",
                                           ExplorationCtrlBase)
...
ExplorationResidentStoreCtrl.UpdateResidentStore =
    function(self, storeData)
        ...
        if storeData.data ~= nil then
            for idx, elem in ipairs(storeData.data) do
                local storeElem = {
                    ...
                    saled = elem.saled,
                    chipData = (ChipData.New)(elem.id, 1)
                };
...
ExplorationResidentStoreCtrl.OnResidentStoreRoomOpen =
    function(self)
...
ExplorationResidentStoreCtrl.SendStorePurchase =
    function(self, storeItem)
        ...
        if storeItem.price <= self.currency then
            ...
            (self.netWork):CS_EXPLORATION_RESIDENT_STORE_Purchase(storeItem.idx,
                                                                  BindCallback(
                                                                      self,
                                                                      self.OnStorePurchase))
        else
...
ExplorationResidentStoreCtrl.OnStorePurchase =
    function(self)
        -- function num : 0_6 , upvalues : cs_MessageCommon, _ENV
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.exploration_Stroe_BuySuccess) ..
                (LanguageUtil.GetLocaleText)(
                    (self.currStoreItem):GetStoreItemName()))
    end

ExplorationResidentStoreCtrl.ShowChipItemDetail =
    function(self, chipItem)
        -- function num : 0_7 , upvalues : _ENV
        self.detailWindow =
            UIManager:ShowWindow(UIWindowTypeID.GlobalChipDetail)
