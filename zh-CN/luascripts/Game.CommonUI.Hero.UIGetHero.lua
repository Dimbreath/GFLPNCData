...
UIGetHero.ShowNext = function(self)
  ...
  if #self.heroDataList < self.showIndex then
    self.haveNext = false
  end
end
...
UIGetHero._InitGetHero = function(self, heroData)
  ...
  (self.resloader):LoadABAssetAsync(PathConsts:GetCampPicPath(campIcon), function(texture)
    ...
    ;
    ((self.ui).img_CampSmall).texture = texture
  ...
  ;
  ((self.ui).img_Carrer):SetIndex(heroData.career - 1)
...
UIGetHero.PlayAllTween = function(self)
  ...
  local allTweens = (self.transform):GetComponentsInChildren(typeof(((CS.DG).Tweening).DOTweenAnimation))
