...
hurt_config = {hit_formula = 10010, basehurt_formula = 10096}
...
bs_1060.RealPlaySkill = function(self, target)
  ...
  for i = 0, targetList.Count - 1 do
    ...
    if emptyGrid ~= nil and LuaSkillCtrl:GetGridsDistance(emptyGrid.x, emptyGrid.y, target.x, target.y) < LuaSkillCtrl:GetGridsDistance(((targetList[i]).targetRole).x, ((targetList[i]).targetRole).y, target.x, target.y) and ((targetList[i]).targetRole).belongNum ~= eBattleRoleBelong.neutral then
...
bs_1060.PlaySkill = function(self, data)
    ...
    local gridTarget = LuaSkillCtrl:GetTargetWithGrid(gridTemp.x, gridTemp.y)
    local targetList = LuaSkillCtrl:CallTargetSelect(self, 9, 10)
    self.skillEffect = LuaSkillCtrl:CallEffect(gridTarget,
                                               (self.config).effectId, self,
                                               self.SkillEventFunc, nil, 1)
    self:RealPlaySkill(gridTarget)
end
...
bs_1060.RealPlaySkill = function(self, target)
    ...
    local targetList = LuaSkillCtrl:CallTargetSelect(self, 9, 10)
    for i = 0, targetList.Count - 1 do
        local emptyGrid = LuaSkillCtrl:CallFindEmptyGridClosedToTarget(target.x,
                                                                       target.y,
                                                                       (targetList[i]).targetRole)
        if emptyGrid ~= nil and
            LuaSkillCtrl:GetGridsDistance(emptyGrid.x, emptyGrid.y, target.x,
                                          target.y) <
            LuaSkillCtrl:GetGridsDistance(((targetList[i]).targetRole).x,
                                          ((targetList[i]).targetRole).y,
                                          target.x, target.y) and
            ((targetList[i]).targetRole).belongNum ~= eBattleRoleBelong.neutral then
            ((targetList[i]).targetRole):ResetRoleState()
            LuaSkillCtrl:CallPhaseMoveWithoutTurn(self,
                                                  (targetList[i]).targetRole,
                                                  emptyGrid.x, emptyGrid.y, 5)
        end
        LuaSkillCtrl:CallBuff(self, (targetList[i]).targetRole, 66, 1,
                              self.delay)
        LuaSkillCtrl:CallRealDamage(self, (targetList[i]).targetRole, nil,
                                    (self.config).hurt_config, nil, true)
    end
    self.realTime = self.realTime + self.delay
    if self.durTime <= self.realTime then
        self:RealSkillEnd()
...
bs_1060.PlayUltEffect = function(self)
    ...
    LuaSkillCtrl:CallFocusTimeLine(self.caster)
    LuaSkillCtrl:CallBuff(self, self.caster, 196, 1, 15)
    self:AddTrigger(eSkillTriggerType.AfterSelfUltMovieEnd, "bs_1060_2", 1,
                    self.OnMovieEnd)
    self:AddTrigger(eSkillTriggerType.OnSelfUltMovieFadeOut, "bs_1060_3", 1,
                    self.OnMovieFadeOut)
end
