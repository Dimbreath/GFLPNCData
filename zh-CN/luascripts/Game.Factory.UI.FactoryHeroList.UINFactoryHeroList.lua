...
local UINSiftCondition = require("Game.Hero.NewUI.SortList.UINSiftCondition")
local UINFactoryHeroListCard = require(
                                   "Game.Factory.UI.FactoryHeroList.UINFactoryHeroListCard")
local HeroFilterEnum = require("Game.Hero.NewUI.HeroFilterEnum")
local HeroSortEnum = require("Game.Hero.NewUI.HeroSortEnum")
local cs_ResLoader = CS.ResLoader
local cs_DoTween = ((CS.DG).Tweening).DOTween
...
UINFactoryHeroList.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Filter, self,
                               self.m_OnClickBtnFilter);
    (UIUtil.AddButtonListener)((self.ui).btn_Comfirm, self,
                               self.m_OnClickConfirm)
    self:SetHeroListTweens()
end

UINFactoryHeroList.SetCallback = function(self, EnergyAddCallback,
                                          confirmCallback)
    -- function num : 0_1
    self.EnergyAddCallback = EnergyAddCallback
    self.confirmCallback = confirmCallback
...
UINFactoryHeroList.InitFactoryHeroList =
    function(self, roomIndex)
        -- function num : 0_2 , upvalues : _ENV
        self.roomIndex = roomIndex;
        ((self.ui).topBtnGroup):SetActive(false);
        ((self.ui).sortConditionNode):SetActive(false);
...
UINFactoryHeroList.ReadCurEntered = function(self)
    ...
    for roomIndex, heroList in pairs(AllEnteredHeroIds) do
        if roomIndex == self.roomIndex then
            for index, value in ipairs(heroList) do
                ...
                (self.SelectedHeroIds)[index] = value -- DECOMPILER ERROR at PC25: Confused about usage of register: R12 in 'UnsetPending'
                ;
            ...
            do
                for index, value in ipairs(heroList) do
                    ...
                    (self.CardDatas)[value] =
                        {roomIndex = roomIndex, isInOtherFactory = true}
                end
                do
                    -- DECOMPILER ERROR at PC40: LeaveBlock: unexpected jumping out DO_STMT
    ...
    for heroId, cardData in pairs(self.CardDatas) do
        local cardItem = (self.heroSortList):__GetHeroItemById(heroId)
        if cardItem ~= nil then cardItem:RefreshFactoryUI(cardData) end
    end
    (self.heroSortList):SetCustonDataDic(self.CardDatas);
    (self.heroSortList):RefreshHeroSortList()
    self:UpdateEnergyAdd(true)
...
UINFactoryHeroList.m_InitHeroList = function(self)
    ...
    if self.heroSortList == nil then
        ...
        HeroSortList:InitHeroSortList(self.resloader, nil,
                                      self.__OnClickHeroItem, true, false,
                                      false, false)
        self.heroSortList = HeroSortList
    end
end
...
UINFactoryHeroList.m_OnClickHeroItem = function(self, heroData, selectItem)
    ...
    if ENTER_LIMIT <= #self.SelectedHeroIds then
        (cs_MessageCommon.ShowMessageTips)(
            (string.format)(ConfigData:GetTipContent(
                                TipContent.Factory_HeroEnterLimit),
                            tostring(ENTER_LIMIT)))
        return
    end
    local ApplySelect = function()
        -- function num : 0_5_0 , upvalues : _ENV, self, heroData, selectItem
    ...
    if (self.CardDatas)[heroData.dataId] ~= nil and
        ((self.CardDatas)[heroData.dataId]).isInOtherFactory then
        (cs_MessageCommon.ShowMessageBox)(
            ConfigData:GetTipContent(TipContent.Factory_ConfirmSwitchHero),
            ApplySelect, nil)
    else
        ApplySelect()
    end
...
UINFactoryHeroList.m_OnClickConfirm = function(self)
    -- function num : 0_8
    (self.factoryController):SetRoomHeroList(self.roomIndex,
                                             self.SelectedHeroIds,
                                             self.confirmCallback)
end

UINFactoryHeroList.m_InitSortButtonGroup =
    function(self)
        -- function num : 0_9 , upvalues : UINSortButtonGroup, HeroSortEnum
        if self.sortButtonGroup == nil then
            local sortButtonGroup = (UINSortButtonGroup.New)()
            ...
            sortButtonGroup:InitSortButtonGroup(HeroSortEnum.SortMannerDefine,
                                                self.__OnClickBtnSortItem)
            self.sortButtonGroup = sortButtonGroup
        end
    end

UINFactoryHeroList.m_OnClickBtnSortItem =
    function(self, sortFunc)
        -- function num : 0_10
        if self.heroSortList == nil then self:m_InitHeroList() end
        (self.heroSortList):RefreshHeroSortList(nil, sortFunc)
    end
...
UINFactoryHeroList.m_OnFilterConfirmAction =
...
UINFactoryHeroList.DeleteHeroListTweens =
    function(self)
        -- function num : 0_16
        if self.pageSequence ~= nil then
            (self.pageSequence):Kill()
