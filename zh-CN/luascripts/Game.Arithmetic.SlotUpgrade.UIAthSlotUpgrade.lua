...
local SlotUpgradeTog = require("Game.Arithmetic.SlotUpgrade.UIAthSlotUpgradeTog")
local UIAthMultSortList = require("Game.Arithmetic.SortList.UIAthMultSortList")
local UIAthDecomposeFliter = require("Game.Arithmetic.SlotUpgrade.UIAthDecomposeFliter")
...
UIAthSlotUpgrade.OnInit = function(self)
  ...
  self.SlotUpgradeTogPool = (UIItemPool.New)(SlotUpgradeTog, (self.ui).tog_AreaSelectObj)
  ;
  ...
  for key,_ in pairs((PlayerDataCenter.allAthData).slotAthDic) do
    local tog = (self.SlotUpgradeTogPool):GetOne(true)
  ...
  ;
  (UIUtil.AddButtonListener)(((self.sortListNode).ui).btn_Decompose, self, self.AthDecompose)
  ;
  (UIUtil.AddButtonListener)(((self.sortListNode).ui).btn_AutoDecompose, self, self.AthAutoDecompose)
  ;
  (UIUtil.AddValueChangedListener)(((self.sortListNode).ui).tog_AutoDecompose, self, self.AthAutoDecomposeTog)
  ;
  (UIUtil.AddButtonListener)(((self.sortListNode).ui).btn_OneKeyDecompose, self, self.AthOneKeyDecompose)
end
...
UIAthSlotUpgrade.RefeshSlotInfo = function(self, dataId, curSlotId)
  ...
  if dataId == nil or curSlotId == nil then
    error("非法的槽位刷新")
  end
  local slotLevel = (PlayerDataCenter.allAthData):GetHeroAthSlotLevel(dataId, curSlotId)
  ;
  ...
  if slotLevel == #ConfigData.ath_efficiency then
    ...
    ;
    ((self.ui).tex_AttriCount):SetIndex(0, tostring(FormatNum(efficiency / 10)))
  else
    do
      self.maxLevel = false
      if needFrgNum <= playerFrgNum then
        ...
        local efficiency = (PlayerDataCenter.allAthData):GetAthEfficiency(slotLevel + 1)
        ;
        ((self.ui).tex_AttriCount):SetIndex(0, tostring(FormatNum(efficiency / 10)))
      else
        do
          ...
          do
            ...
            ((self.ui).tex_AttriCount):SetIndex(0, tostring(FormatNum(efficiency / 10)))
            ;
            ((self.ui).tex_Condition):SetIndex(0, tostring(playerFrgNum), tostring(needFrgNum))
            -- DECOMPILER ERROR at PC130: Confused about usage of register: R6 in 'UnsetPending'

            ;
            ((self.ui).img_progressBar).fillAmount = playerFrgNum / needFrgNum
          end
        end
      end
    end
...
UIAthSlotUpgrade.OnClickUpgrade = function(self)
  ...
  if self.maxLevel then
    ((CS.MessageCommon).ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_slot_HasFullLevel))
    return 
  end
  if self.couldUpdate then
    local oldSlotLevel = (PlayerDataCenter.allAthData):GetHeroAthSlotLevel((self.heroData).dataId, self.curSlotId)
    do
      local oldEfficiency = (PlayerDataCenter.allAthData):GetAthEfficiency(oldSlotLevel)
      ;
      (self.athNetwork):CS_ATH_SlotUpgrade((self.heroData).dataId, self.curSlotId, function()
    -- function num : 0_3_0 , upvalues : _ENV, self, oldEfficiency, oldSlotLevel
    local slotLevel = (PlayerDataCenter.allAthData):GetHeroAthSlotLevel((self.heroData).dataId, self.curSlotId)
    local efficiency = (PlayerDataCenter.allAthData):GetAthEfficiency(slotLevel)
    UIManager:ShowWindowAsync(UIWindowTypeID.AthSlotUpgradeInfo, function(win)
    ...
    do
      ;
      ((CS.MessageCommon).ShowMessageTips)(ConfigData:GetTipContent(TipContent.arithmetic_Slot_MaterialShot))
    end
  end
end
...
UIAthSlotUpgrade.RefreshAthDepository = function(self)
  ...
  ;
  (self.sortListNode):SetClickSortAthEvent(function(athData)
    ...
    if not (self.sortListNode):isSelected(athData) then
      ...
      ;
      ((self.sortListNode):__GetAthItemByData(athData)):SelectAth(false)
    end
  end
)
end
...
UIAthSlotUpgrade.AthDecompose = function(self)
  ...
  for qID,nums in pairs(nums) do
    info = info .. tostring(nums) .. (string.format)(ConfigData:GetTipContent(TipContent.arithmetic_DecomposeInfo), eFliterKindColorName[qID], tostring(frgNums[qID]))
  end
  ;
  ((CS.MessageCommon).ShowMessageBox)(ConfigData:GetTipContent(TipContent.arithmetic_Slot_ConfirmDecompose) .. info, function()
...
UIAthSlotUpgrade.AthAutoDecompose = function(self)
  ...
  ;
  (self.decomposeFliterNode):SetConfirmCallback(function()
    -- function num : 0_6_0 , upvalues : self
    self.autoDecoDic = self:DecoDic2AutoDecoDic(self.DecoDic, true)
...
UIAthSlotUpgrade.__AthAutoDecomposeInitConfig = function(self)
  -- function num : 0_8 , upvalues : _ENV
  self.autoDecoDic = (PlayerDataCenter.allAthData).autoDeco
  if self.autoDecoDic == nil then
    self.autoDecoDic = {}
  end
...
UIAthSlotUpgrade.DecoDic2AutoDecoDic = function(self, DecoDic, trigger)
  ...
  if trigger then
    ...
    for _,value in ipairs(DecoDic.quality) do
      local elemName = (string.lower)((table.keyof)(eItemQualityType, value))
...
UIAthSlotUpgrade.AthOneKeyDecompose = function(self)
  ...
  (self.decomposeFliterNode):CleanSelect()
  ;
  (self.decomposeFliterNode):SetResetCallback(function()
  ...
  (self.decomposeFliterNode):SetConfirmCallback(function()
    ...
    (self.sortListNode):RefreshAthList(function(athData)
        ...
        do
          do
            ...
            if #(self.DecoDic).mainAttr ~= 0 then
              local tempBool = false
              for _,attrID in ipairs((self.DecoDic).mainAttr) do
              ...
              do
                do
                  ...
                  if #(self.DecoDic).quality ~= 0 then
                    local tempBool = false
                    for _,qualityID in ipairs((self.DecoDic).quality) do
                      if (athData.itemCfg).quality == qualityID then
                        tempBool = true
                        break
                      end
                    end
...
UIAthSlotUpgrade.GenFliteroptions = function(self)
  ...
  for i = 1, (ConfigData.game_config).athSlotCount do
    do
      local tempOption = {}
      tempOption.optionName = eFliterKindTypeOptionName[eFliterKindType.Area] .. i
  ...
  for key,value in pairs((ConfigData.game_config).athQualityEnum) do
    local tempOption = {}
    tempOption.optionName = eFliterKindColorName[value] .. eFliterKindTypeOptionName[eFliterKindType.Quality]
    tempOption.callback = function(bool)
  ...
  for key,value in pairs(ConfigData.attribute) do
    if value.algor_main_attribute == 1 then
      local tempOption = {}
      tempOption.optionName = eFliterKindTypeOptionName[eFliterKindType.MainAttr] .. (LanguageUtil.GetLocaleText)(value.name) .. key
      tempOption.callback = function(bool)
