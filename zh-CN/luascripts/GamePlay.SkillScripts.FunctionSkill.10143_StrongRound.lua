...
bs_10143.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10143_1", 1,
                    self.OnAfterHurt)
    self:AddTrigger(eSkillTriggerType.AfterBattleStart, "bs_10143_2", 1,
                    self.OnAfterBattleStart) -- DECOMPILER ERROR at PC20: Confused about usage of register: R2 in 'UnsetPending'
    ;
...
bs_10143.OnAfterBattleStart = function(self)
    ...
    if (self.caster).hp * 1000 // (self.caster).maxHp <= (self.arglist)[1] and
        ...
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).damageBuffId,
                              (self.arglist)[3])
        ...
        if targetList.Count > 0 then
            for i = 0, targetList.Count - 1 do
                if ((self.caster).recordTable)["10143_effectFlag"] and
                    (targetList[i]).targetRole ~= self.caster then
...
bs_10143.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_3 , upvalues : _ENV
    if target == self.caster and (self.caster).hp * 1000 // (self.caster).maxHp <=
        (self.arglist)[1] and self:IsReadyToTake() then
        if ((self.caster).recordTable)["10143_effectFlag"] then
            LuaSkillCtrl:CallEffect(self.caster, (self.config).effectId1, self)
        end
        LuaSkillCtrl:CallBuff(self, self.caster, (self.config).damageBuffId,
