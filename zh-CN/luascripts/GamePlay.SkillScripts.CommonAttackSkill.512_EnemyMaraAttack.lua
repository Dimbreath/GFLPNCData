...
bs_512.PlaySkill = function(self, passdata)
    ...
    if self.lastAttackRole ~= nil and (self.lastAttackRole).hp > 0 and
        (self.lastAttackRole).belongNum ~= eBattleRoleBelong.neutral then
        if not LuaSkillCtrl:IsAbleAttackTarget(self.caster, self.lastAttackRole,
                                               (self.config).rangeOffset +
...
bs_512.BlinkCaster = function(self, role)
    -- function num : 0_3 , upvalues : _ENV
    if role == nil or role.hp <= 0 or
        role:ContainBuffFeature(eBuffFeatureType.NotBeSelected) then return end
    local grid = LuaSkillCtrl:FindEmptyGridAroundRole(role)
...
bs_512.RealPlaySkill = function(self, target, data)
    ...
    if LuaSkillCtrl:CallRange(1, 1000) <= Roll then
        ...
        LuaSkillCtrl:CallRoleActionWithTrigger(self, self.caster, atkActionId,
                                               atkSpeedRatio, atkTriggerFrame,
                                               attackTrigger2)
        LuaSkillCtrl:CallEffect(target, (self.config).effectId33, self)
        LuaSkillCtrl:CallEffect(target, (self.config).effectId4, self)
    else
...
bs_512.OnAttackTrigger2 = function(self, target, data)
    ...
    if (skillResult.roleList).Count ~= 0 then
        for i = 0, (skillResult.roleList).Count - 1 do
            local role = (skillResult.roleList)[i]
            if LuaSkillCtrl:IsRoleAdjacent(role, self.caster) then
                local attackTrigger3 = BindCallback(self, self.OnAttackTrigger3,
                                                    role)
                LuaSkillCtrl:StartTimer(self, 10, attackTrigger3, nil, 0)
            end
        end
