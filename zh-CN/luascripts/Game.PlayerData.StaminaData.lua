...
StaminaData.UpdateStamina = function(self, data)
    ...
    if self.value ~= data or self.recoveTm ~= data.recoveTm or self.frag ~=
        data.frag then needUpdate = true end
    self.value = data.value
    self.recoveTm = data.recoveTm
    self.frag = data.frag
    if needUpdate then self:BroadcastStamina() end
end
...
StaminaData.GetCurrentStamina = function(self)
    ...
    if self.recoveTm == nil or self.output == nil then return 0, 0 end
    local stamina = self.value
    local remainSecond = 0
    if self.ceiling <= stamina then return stamina, remainSecond end
    local timestamp = PlayerDataCenter.timestamp
    local interval = timestamp - self.recoveTm
    if interval < -1 then return stamina, remainSecond end
    ...
    do return stamina, remainSecond end
end
...
StaminaData.StartTimer = function(self, time)
    ...
    self.__Timer = (TimerManager:GetTimer(time, self.BroadcastStamina, self,
                                          true)):Start()
end
...
StaminaData.BroadcastStamina = function(self)
    ...
    if remainSecond > 0 then
        ...
        if stamina < self.ceiling then self:StartTimer(1) end
    end
end
