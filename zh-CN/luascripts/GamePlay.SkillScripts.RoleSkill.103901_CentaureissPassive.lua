...
bs_103901.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : base, _ENV
    (base.InitSkill)(self, isMidwaySkill)
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_103901_3", 1,
                    self.OnAfterHurt)
end

bs_103901.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                 isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack and
        LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[1] and not isMiss then
        local targetX = target.x
        local targetY = target.y
        ...
        LuaSkillCtrl:CallBuff(self, target, (self.config).stunBuff, 1,
                              (self.arglist)[2])
        if (self.arglist)[3] == 1 and buffCheck ~= nil and target.x == targetX and
            target.y == targetY then
