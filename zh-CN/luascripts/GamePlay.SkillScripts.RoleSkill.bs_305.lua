...
bs_305.OnAttackTrigger = function(self, target, data)
  ...
  LuaSkillCtrl:CallEffectWithArgOverride(target, (self.config).effectId, self, nil, true, self.SkillEventFunc)
end
  ...
  if effect.dataId == (self.config).effectId and eventId == eBattleEffectEvent.Trigger and ((self.caster).recordTable)["305_Time"] > 0 then
    ...
    do
      if targetRole ~= nil then
        LuaSkillCtrl:CallEffectWithArgOverride(targetRole, (self.config).effectId, self, target.targetRole, true, self.SkillEventFunc)
      end
    end
  end
end
...
bs_305.OnAttackTrigger = function(self, target, data)
    ...
    ((self.caster).recordTable)["305_Time"] = 4
    LuaSkillCtrl:CallEffectWithArgOverride(target, (self.config).effectId, self,
                                           nil, true, self.SkillEventFunc)
end

bs_305.SkillEventFunc = function(self, effect, eventId, target)
    -- function num : 0_4 , upvalues : _ENV
    if effect.dataId == (self.config).effectId and eventId ==
        eBattleEffectEvent.Trigger and ((self.caster).recordTable)["305_Time"] >
        0 then
        local hurt = (self.caster).pow *
                         ((self.arglist)[1] - (self.arglist)[2] *
                             ((self.arglist)[4] -
                                 ((self.caster).recordTable)["305_Time"])) //
                         1000
        LuaSkillCtrl:RemoveLife(hurt, self, target)
        if ((self.caster).recordTable)["305_Time"] == (self.arglist)[4] and
            (self.arglist)[3] >= 0 then
            LuaSkillCtrl:CallBuff(self, target.targetRole, 66, 1, 15)
        end -- DECOMPILER ERROR at PC61: Confused about usage of register: R5 in 'UnsetPending'
        
        ((self.caster).recordTable)["305_Time"] =
            ((self.caster).recordTable)["305_Time"] - 1 -- DECOMPILER ERROR at PC68: Confused about usage of register: R5 in 'UnsetPending'
        ;
        (self.monsterRecord)[target.targetRole] =
            (self.monsterRecord)[target.targetRole] + 1
        local targetList = LuaSkillCtrl:CallTargetSelect(self, 34, 2,
                                                         target.targetRole)
        if targetList.Count == 0 then return end
        local targetRole = nil
        for i = 0, targetList.Count - 1 do
        ...
        do
            if targetRole ~= nil then
                LuaSkillCtrl:CallEffectWithArgOverride(targetRole,
                                                       (self.config).effectId,
                                                       self, target.targetRole,
                                                       true, self.SkillEventFunc)
            end
        end
    end
end
