...
LotteryShowCtrl.PlayAllDOTween = function(self)
    -- function num : 0_1 , upvalues : _ENV
    self.allTweensAnimation =
        (((self.bind).ui_buttom).transform):GetComponentsInChildren(
...
LotteryShowCtrl.LtrShowStart = function(self, showRareList, poolType)
    ...
    self.showRareList = showRareList
    self.poolType = poolType
    if not self:__LoadRes() then self:__OnShowStart() end
end
...
LotteryShowCtrl._OnClickStart = function(self)
    ...
    if self.waitStart then
        ...
        self:__ContinuePlayShow()
        self.waitStart = false;
        (self.ltrCtrl):ChangeLotteryState(
            (LotteryEnum.eLotteryState).LotteryAnimation)
    end
end
...
LotteryShowCtrl.__MainCoroutine = function(self)
    -- function num : 0_4 , upvalues : _ENV, cs_WaitForSeconds
    self:_ChangeRingMaskSize(1)
    while not self._circle1ScanComplete do (coroutine.yield)(nil) end
    (coroutine.yield)(cs_WaitForSeconds(0.5))
    self:_ShowBingo(self.circle1IndexList, self.circle1PointIndexDic,
                    eHeroCardRareType.R);
    (coroutine.yield)(cs_WaitForSeconds(1))
    if self.scanPointNum == 0 then
        self:_OnShowEnd()
    ...
    self:_BalckBgTween(1500, 2370, 0.155, 0.278)
    self:_ShowQuality1BingoExit()
    while self.scanWaveEndNum < self.scanPointNum do (coroutine.yield)(nil) end
    (coroutine.yield)(cs_WaitForSeconds(0.5))
    self:_ShowBingo(self.circle2IndexList, self.circle2PointIndexDic,
                    eHeroCardRareType.SR);
    (coroutine.yield)(cs_WaitForSeconds(1))
    if self.scanPointNum == 0 then
        self:_OnShowEnd()
        return
...
LotteryShowCtrl.__LoadRes = function(self)
    -- function num : 0_6 , upvalues : cs_ResLoader, _ENV, UINLtrShowPoint, UINLtrSPointBingo, UINLtrShowRing, UINLtrSWaveRing
    if self.__resLoaded then return false end
    self.resLoader = (cs_ResLoader.Create)()
    local path = PathConsts:GetLotteryModelPath("LotteryShow");
    ...
    (self.resLoader):LoadABAssetAsync(path, function(prefab)
        ...
        self.bind = {};
        (UIUtil.LuaUIBindingTable)(self.ltrShowGo, self.bind);
        (UIUtil.AddButtonListener)((self.bind).btn_start, self,
                                   self._OnClickStart)
        local codeActionBinding = ((self.bind).timeLine):FindComponent(
                                      eUnityComponentID.CodeActionBinding)
        codeActionBinding:RegisterAction("WaitStart",
                                         BindCallback(self, self.__OnWaitStart))
        codeActionBinding:RegisterAction("StartCamMove", BindCallback(self,
                                                                      self.__OnStartCamMove))
        codeActionBinding:RegisterAction("Quilaty1Stop", BindCallback(self,
                                                                      self.__OnQuilaty1Stop))
        codeActionBinding:RegisterAction("Quilaty2Stop", BindCallback(self,
                                                                      self.__OnQuilaty2Stop))
        codeActionBinding:RegisterAction("Quilaty3Stop", BindCallback(self,
                                                                      self.__OnQuilaty3Stop));
        ((self.bind).uI_LSPoint):SetActive(false)
        self.showPointPool = (UIItemPool.New)(UINLtrShowPoint,
                                              (self.bind).uI_LSPoint);
        ((self.bind).uI_LSPointBingo):SetActive(false)
        self.pointBingoPool = (UIItemPool.New)(UINLtrSPointBingo,
                                               (self.bind).uI_LSPointBingo);
        ((self.bind).imgRing):SetActive(false)
        self.showRingPool =
            (UIItemPool.New)(UINLtrShowRing, (self.bind).imgRing);
        ((self.bind).waveRing):SetActive(false)
        self.waveRingPool = (UIItemPool.New)(UINLtrSWaveRing,
                                             (self.bind).waveRing)
        self.__resLoaded = true
        UIManager:HideWindow(UIWindowTypeID.ClickContinue)
        self:__OnShowStart()
...
LotteryShowCtrl.__OnShowStart = function(self)
    ...
    self:_ClearMainCo()
    self._circle1ScanComplete = false
    self.mainCo = (GR.StartCoroutine)((util.cs_generator)(
                                          self.__MainCoroutineFunc))
    self:__StopShowCo()
    self.__ltrShowCo = (TimelineUtil.Play)((self.bind).timeLine)
    self:PlayAllDOTween()
end
...
LotteryShowCtrl.__OnStartCamMove = function(self)
    ...
    for k, rare in ipairs(self.showRareList) do
        ...
        if self.maxRare < rare then self.maxRare = rare end
    end
    local randomNum = 0
    local extraScanIndexDic = {}
    if self.poolType == (LotteryEnum.eLotteryPoolType).Paid and
        #self.showRareList == 10 then
        local count = (math.random)(0, randomNum)
        for i = 1, count do
            local index = (math.random)(1, #tempRIndexList);
            (table.remove)(tempRIndexList, index)
    ...
    do
        local tempIndexList = {}
        for i = 1, allCfgNum do (table.insert)(tempIndexList, i) end
        self.circle1IndexCfgBingoList = {}
        local indexList = {}
        for i = 1, allNum do
            local randomIndex = (math.random)(1, #tempIndexList)
            ...
            local tarPos = (Vector2.New)(tarPosCfg[1], tarPosCfg[2])
            local startPos = ((self.bind).uI_LSMainPoint).anchoredPosition
            local sqrMagnt = ((Vector2.New)(startPos.x - tarPos.x,
                                            startPos.y - tarPos.y)):SqrMagnitude();
        ...
        waveRing1:InitLtrSWaveRing(Vector2.zero, rareColor)
        local showRing1 = (self.showRingPool):GetOne()
        showRing1:InitLtrShowRing(((self.bind).uI_LSMainPoint).anchoredPosition,
                                  rareColor)
        AudioManager:PlayAudioById(1045)
        self.showRare1Tween = (((cs_DOTween.To)(
                                  function()
                -- function num : 0_9_1
                return 0
            end, function(x)
                -- function num : 0_9_2 , upvalues : showRing1, _ENV, indexList, pintIndex, self
                showRing1:SetLtrShowRingSize((Vector2.New)(x, x))
                if x > 1400 then showRing1:LtrShowRingFade() end
                local sqrMagnt = x / 2 * (x / 2)
                local indexCfg = indexList[pintIndex]
                if pintIndex <= #indexList and indexCfg.sqrMagnt <= sqrMagnt then
                    local pointItem = (self.showPointPool):GetOne()
                    local pos = indexCfg.tarPos
                    pointItem:InitLtrShowPoint(pos,
                                               ((self.bind).uI_LSMainPoint).anchoredPosition,
                                               indexCfg.rare) -- DECOMPILER ERROR at PC39: Confused about usage of register: R5 in 'UnsetPending'
                    ;
                    (self.circle1PointIndexDic)[pintIndex] = pointItem
                    pintIndex = pintIndex + 1
                end
            ...
            end, 1500, 4)):SetEase(cs_Ease.OutSine)):OnComplete(
                                  function()
                -- function num : 0_9_3 , upvalues : self
                self._circle1ScanComplete = true
            end);
...
LotteryShowCtrl._ShowBingo = function(self, circleIndexList,
                                      circlePointIndexDic, curRare)
    -- function num : 0_10 , upvalues : _ENV
    (self.pointBingoPool):HideAll()
    self.scanPointNum = 0
    for k, v in ipairs(circleIndexList) do
...
LotteryShowCtrl._ShowQuality1BingoExit =
    function(self)
        -- function num : 0_11 , upvalues : LtrShowCfg, _ENV, cs_WaitForSeconds
        local allCfgNum = #LtrShowCfg.CircleTwoPoint
        local allNum = self.scanPointNum + (math.random)(5, 10)
        if allCfgNum < allNum then
            error((string.format)(
                      "LtrShowCfg.CircleTwoPoint error, allNum:%s, allCfgNum:%S",
                      allNum, allCfgNum))
            return
        end
        local tempIndexList = {}
        ...
        for i = 1, allNum do
            ...
            local circle1IndexCfg =
                (self.circle1IndexList)[(self.circle1IndexCfgBingoList)[scanPointIndex]]
            local startPos = circle1IndexCfg.tarPos
            local pointRare = nil
            if isFirst then pointRare = circle1IndexCfg.rare end
            local sqrMagnt = ((Vector2.New)(startPos.x - tarPos.x,
                                            startPos.y - tarPos.y)):SqrMagnitude()
            local cfg = {
            ...
            if isFirst then
                (table.insert)(circle2IndexList, cfg)
                if eHeroCardRareType.SR < pointRare then
                    (table.insert)(self.circle2IndexCfgBingoList,
                                   #circle2IndexList)
                end
            end
            scanPointIndex = scanPointIndex + 1
        ...
        for k, v in ipairs(self.circle1IndexCfgBingoList) do
            ...
            local indexList = randomIndexList[scanPointIndex]
            AudioManager:PlayAudioById(1046)
            self:_ShowScanWave(rareColor, pointItem, circle1IndexCfg.extraScan,
                               indexList, isLastCircle)
            scanPointIndex = scanPointIndex + 1
        end
        -- DECOMPILER ERROR: 2 unprocessed JMP targets
    end

LotteryShowCtrl._ShowScanWave = function(self, rareColor, pointItem, isExtra,
                                         indexList, isCircleLast)
    -- function num : 0_12 , upvalues : cs_DOTween, _ENV, cs_Ease
    local waveRingItem = (self.waveRingPool):GetOne()
    local fromItem = pointItem
    local centerPos = (pointItem.transform).anchoredPosition
    ...
    ringItem:InitLtrShowRing(centerPos, rareColor)
    local scanOk = false
    self.showScanTween = (((cs_DOTween.To)(
                             function()
            -- function num : 0_12_0
            return 0
        end, function(x)
            -- function num : 0_12_1 , upvalues : ringItem, _ENV, indexList, pointIndex, self, centerPos, fromItem, isCircleLast, scanOk
            ringItem:SetLtrShowRingSize((Vector2.New)(x, x))
            if x > 1500 then ringItem:LtrShowRingFade() end
            local sqrMagnt = x / 2 * (x / 2)
            local indexCfg = indexList[pointIndex]
            if pointIndex <= #indexList and indexCfg.sqrMagnt <= sqrMagnt then
                local pointItem = (self.showPointPool):GetOne()
                local pos = indexCfg.tarPos
                pointItem:InitLtrShowPoint(pos, centerPos, indexCfg.rare,
                                           fromItem)
                if indexCfg.rare ~= nil then
                    if isCircleLast then
                        pointItem:ShowLtrSPointFlare()
                    end -- DECOMPILER ERROR at PC46: Confused about usage of register: R5 in 'UnsetPending'
                    
                    (self.circle2PointIndexDic)[indexCfg.scanPointIndex] =
                        pointItem
                    self.scanWaveEndNum = self.scanWaveEndNum + 1
                    scanOk = true
                end
        ...
        end, 2500, 6.67)):SetEase(cs_Ease.OutSine)):OnComplete(
                             function()
            -- function num : 0_12_2 , upvalues : isExtra, pointItem, scanOk, self
            if isExtra then pointItem:ShowLtrSPointFlare() end
            if not scanOk then
                self.scanWaveEndNum = self.scanWaveEndNum + 1
            end
        end);
...
LotteryShowCtrl._BalckBgTween = function(self, from, to, balckBgSizeFrom,
                                         balckBgSizeTo, delayTime)
    -- function num : 0_13 , upvalues : cs_DOTween, cs_Ease, _ENV
    if delayTime == nil then delayTime = 1.4 end
    self._blackBgTween = (((cs_DOTween.To)(
                             function()
            -- function num : 0_13_0 , upvalues : from
            return from
        end, function(x)
            -- function num : 0_13_1 , upvalues : from, to, balckBgSizeTo, balckBgSizeFrom, self
            local size = (x - from) / (to - from) *
                             (balckBgSizeTo - balckBgSizeFrom) + balckBgSizeFrom
            self:_SetBlackBgSize(size)
...
LotteryShowCtrl._ShowQuality2BingoExit =
    function(self)
        -- function num : 0_16 , upvalues : LtrShowCfg, _ENV, cs_WaitForSeconds
        self.scanWaveEndNum = 0
        local allCfgNum = #LtrShowCfg.CircleThreePoint
        ...
        if allCfgNum < allNum then
            error((string.format)(
                      "LtrShowCfg.CircleThreePoint error, allNum:%s, allCfgNum:%S",
                      allNum, allCfgNum))
            return
        end
        local tempIndexList = {}
        ...
        for i = 1, allNum do
            ...
            local circle2IndexCfg =
                (self.circle2IndexList)[(self.circle2IndexCfgBingoList)[scanPointIndex]]
            local startPos = circle2IndexCfg.tarPos
            local pointRare = nil
            if isFirst then pointRare = circle2IndexCfg.rare end
            local sqrMagnt = ((Vector2.New)(startPos.x - tarPos.x,
                                            startPos.y - tarPos.y)):SqrMagnitude()
            local cfg = {
                index = index,
...
LotteryShowCtrl.__CamReturn = function(self)
    ...
    ((self.bind).cM_vcam_1end):SetActive(true);
    (((self.bind).timeLine).gameObject):SetActive(false)
    self.backTimelineCo = (TimelineUtil.Play)((self.bind).timeLine_Back,
                                              self.__OnCamReturnEnd)
end
...
LotteryShowCtrl._KillWaveTweens = function(self)
    ...
    while #self.tweenList > 0 do
        local tween = (table.remove)(self.tweenList)
        if tween ~= nil then tween:Kill() end
    end
end
...
LotteryShowCtrl._SetBlackBgSize = function(self, size)
    -- function num : 0_26 , upvalues : cs_Material
    if self.blackBgMat == nil then
        self.blackBgMat = cs_Material(((self.bind).blackBg).material) -- DECOMPILER ERROR at PC12: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.bind).blackBg).material = self.blackBgMat
    end
    (self.blackBgMat):SetFloat("_Waiquan", size)
...
LotteryShowCtrl._ChangeRingMaskSize = function(self, circleId)
    -- function num : 0_28 , upvalues : cs_Material, LtrShowCfg
    if self.ringMaskMat == nil then
        self.ringMaskMat = cs_Material(((self.bind).img_RingMask).material) -- DECOMPILER ERROR at PC12: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.bind).img_RingMask).material = self.ringMaskMat
    end
    local cfg = (LtrShowCfg.CircleRingMask)[circleId];
...
LotteryShowCtrl.OnDelete = function(self)
    ...
    if self.showPointPool ~= nil then (self.showPointPool):DeleteAll() end
    if self.showRingPool ~= nil then (self.showRingPool):DeleteAll() end
    if self.waveRingPool ~= nil then (self.waveRingPool):DeleteAll() end
    if self.pointBingoPool ~= nil then (self.pointBingoPool):DeleteAll() end
    self:_KillWaveTweens()
end
