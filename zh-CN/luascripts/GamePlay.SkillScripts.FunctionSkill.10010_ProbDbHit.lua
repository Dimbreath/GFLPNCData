...
bs_10010.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10010_3", 1, self.OnAfterHurt)
end

bs_10010.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg)
...
bs_10010.InitSkill = function(self, isMidwaySkill)
    -- function num : 0_1 , upvalues : _ENV
    self:AddTrigger(eSkillTriggerType.AfterHurt, "bs_10010_3", 1,
                    self.OnAfterHurt)
end

bs_10010.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss,
                                isCrit, isRealDmg)
    -- function num : 0_2 , upvalues : _ENV
    if sender == self.caster and skill.isCommonAttack and
        ...
        if targetL ~= nil then
            self:PlayChipEffect();
            (self.caster):LookAtTarget(targetL)
            LuaSkillCtrl:CallEffect(targetL, (self.config).effectIdAttack, self,
                                    self.SkillEventFunc)
        end
    end
end
