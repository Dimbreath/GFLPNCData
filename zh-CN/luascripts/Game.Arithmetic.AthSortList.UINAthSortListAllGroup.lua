...
UINAthSortListAllGroup.InitAthSortListAllGroup =
    function(self, returnAthFunc, returnAthMatFunc)
        -- function num : 0_1
        self.returnAthFunc = returnAthFunc
        self.returnAthMatFunc = returnAthMatFunc
        self:__SetLayoutHeight(0)
...
UINAthSortListAllGroup.AddMatNode2Group =
    function(self, athMatUpNode)
        -- function num : 0_2 , upvalues : anchorLeftTop, _ENV
        (athMatUpNode.transform):SetParent(self.transform) -- DECOMPILER ERROR at PC6: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (athMatUpNode.transform).anchorMax = anchorLeftTop -- DECOMPILER ERROR at PC9: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (athMatUpNode.transform).anchorMin = anchorLeftTop -- DECOMPILER ERROR at PC19: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (athMatUpNode.transform).anchoredPosition =
            (Vector2.New)(((athMatUpNode.transform).sizeDelta).x / 2, 0)
        self:__SetLayoutHeight(((athMatUpNode.transform).sizeDelta).y)
        athMatUpNode:SetAthMatNodeChangeHeightFunc(self.__onSetLayoutHeight)
        self.hasMatNode = true
    end

UINAthSortListAllGroup.AddAthItem2Group =
    function(self, athItem, gridId)
        -- function num : 0_3 , upvalues : _ENV, gridWidth
        -- DECOMPILER ERROR at PC4: Confused about usage of register: R3 in 'UnsetPending'
        ...
        local x = gridId % 4 - 1
        if x == -1 then x = 3 end
        local posX = x * (gridWidth + 10) + ((athItem.transform).sizeDelta).x /
                         2 + 20
        local posY =
            -(y * (gridWidth + 10) + ((athItem.transform).sizeDelta).y / 2)
        local pos = (Vector2.New)(posX, posY) -- DECOMPILER ERROR at PC43: Confused about usage of register: R8 in 'UnsetPending'
        ;
        (athItem.transform).anchoredPosition = pos
        local height = -posY + ((athItem.transform).sizeDelta).y / 2
        if ((self.ui).layoutElement).preferredHeight < height then
...
UINAthSortListAllGroup.GetAthItemFromAllGroup =
    function(self, uid)
        -- function num : 0_4
        return (self.athItemDic)[uid]
    end
...
UINAthSortListAllGroup.ClealAllAthItem =
    function(self)
        -- function num : 0_6 , upvalues : _ENV
        for k, athItem in pairs(self.athItemDic) do
            (self.returnAthFunc)(athItem)
...
UINAthSortListAllGroup.__SetLayoutHeight =
