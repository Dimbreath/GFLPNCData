...
UINOverclockChipItem.InitOverclockChipItem =
    function(self, index, chipData, clickCallback)
        -- function num : 0_1
        self.index = index
        self.chipData = chipData;
        (self.chipItem):InitChipItem(self.chipData) -- DECOMPILER ERROR at PC11: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_ItemName).text = (self.chipData):GetName() -- DECOMPILER ERROR at PC17: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_QuailtyColor).color = (self.chipData):GetColor()
        local isUnlock = chipData.isUnlock
        if isUnlock then self.clickCallback = clickCallback end
        self:SetUnlockUI(isUnlock)
        self:SetSelectUI(false)
        self:Show()
    end
...
UINOverclockChipItem.OnChipItemClick = function(self)
    ...
    if self.clickCallback ~= nil then (self.clickCallback)(self) end
end
