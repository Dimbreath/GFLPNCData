...
UIEpUpgradeRoomItem.InitUpgradeRoomItem = function(self, roomId, moneyIconId, price, chipData, clickAction)
  ...
  if (self.chipData):GetChipMaxLevel() <= (self.chipData):GetCount() then
    ((self.ui).tex_ChipLevel):SetIndex(1)
    ;
    ((self.ui).Obj_LevelMax):SetActive(true)
  else
    ;
    ((self.ui).tex_ChipLevel):SetIndex(0, tostring((self.chipData):GetCount()))
    ;
    ((self.ui).Obj_LevelMax):SetActive(false)
  end
...
UIEpUpgradeRoomItem.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_chipItem, self,
                               self.__OnChipItemClicked)
end

UIEpUpgradeRoomItem.InitUpgradeRoomItem =
    function(self, roomId, moneyIconId, price, chipData, clickAction)
        -- function num : 0_1 , upvalues : UINChipItem, _ENV
        self.upgradeRoomId = roomId
        self.chipData = chipData
        self.index = chipData.idx
        ...
        self.chipItem = chipItem
        self.upgradePrice = price
        self:__ShowPrice(self.upgradePrice, moneyIconId) -- DECOMPILER ERROR at PC25: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.ui).tex_ItemName).text = (self.chipData):GetName() -- DECOMPILER ERROR at PC31: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ((self.ui).img_QuailtyColor).color = (self.chipData):GetColor()
        if (self.chipData):GetChipMaxLevel() <= (self.chipData):GetCount() then
            ((self.ui).tex_ChipLevel):SetIndex(1);
            ...
            ((self.ui).Obj_LevelMax):SetActive(false)
        end -- DECOMPILER ERROR at PC77: Confused about usage of register: R7 in 'UnsetPending'
...
UIEpUpgradeRoomItem.__ShowPrice = function(self, price, MoneyIconId)
    -- function num : 0_2 , upvalues : _ENV
    ((self.ui).originalPrice):SetActive(false) -- DECOMPILER ERROR at PC10: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).tex_Money).text = tostring(price) -- DECOMPILER ERROR at PC17: Confused about usage of register: R3 in 'UnsetPending'
    ;
    ((self.ui).img_Money).sprite = CRH:GetSprite(MoneyIconId)
end
...
UIEpUpgradeRoomItem.__OnChipItemClicked =
    function(self)
        -- function num : 0_4
