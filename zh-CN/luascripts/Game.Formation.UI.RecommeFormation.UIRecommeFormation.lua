...
UIRecommeFormation.OnInit = function(self)
  -- function num : 0_0 , upvalues : _ENV, UINRecommeFormationItem
...
UIRecommeFormation.InitRecommeFormation = function(self, isOpenCopy, recordInfo, fmtCtr)
  ...
  if (ConfigData.sector_stage)[(self.recordInfo).stageId] ~= nil then
    ...
    do
      ...
      do
        if ((self.ui).tog_Filter).isOn == (self.fmtCtr).isRecommeSortForCount then
          self:OnToggleFiler((self.fmtCtr).isRecommeSortForCount)
        else
          -- DECOMPILER ERROR at PC87: Confused about usage of register: R4 in 'UnsetPending'

          ;
          ((self.ui).tog_Filter).isOn = (self.fmtCtr).isRecommeSortForCount
        end
      end
    end
  end
...
UIRecommeFormation.OnToggleFiler = function(self, isSelected)
  ...
  (self.fmtCtr).isRecommeSortForCount = isSelected
  if isSelected then
...
UIRecommeFormation.__ShowList = function(self)
  ...
  for i = 1, #(self.recordInfo).list do
    ...
    item:InitItem(self.fmtCtr, ((self.recordInfo).list)[i], self.isOpenCopy)
  end
end
