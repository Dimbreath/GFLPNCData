...
FormationSceneCtrl.ctor = function(self)
    ...
    self.__lastMousePos = Vector2.zero
end

FormationSceneCtrl.InitFmtSceneCtrl = function(self, go, isOpenedOverClock,
                                               isOpenCampInfluence)
    ...
    self.__screenWidthRange = (Vector2.New)((self.bind).screenWidthOffset,
                                            screenWidth -
                                                (self.bind).screenWidthOffset)
    self.ui3dFmt = (UIN3DFormation.New)();
    (self.ui3dFmt):Init((self.bind).uI_3DFormation);
    (self.ui3dFmt):OpenCampInfluence(isOpenCampInfluence);
    (self.ui3dFmt):OpenOverClock(isOpenedOverClock);
    ((self.bind).overClockPlatform):SetActive(isOpenedOverClock);
    (self.ui3dFmt):Init3DFormation(function()
        -- function num : 0_1_0 , upvalues : self
        (self.fmtCtrl):FmtStartBattle()
    end, (self.fmtCtrl).stageId, (self.fmtCtrl).fromModule)
    ...
    for fmtIndex, go in ipairs((self.bind).platform) do
        ...
        do
            ...
            do
                ...
                entity:InitFmtPlatEntity(go, fmtIndex, self,
                                         self.__OnClickPlatform, isBench, unlcok)
                entity:SetFmtPlatformDragEvent(self.__OnBeginDrag,
                                               self.__OnDrag, self.__OnEndDrag) -- DECOMPILER ERROR at PC96: Confused about usage of register: R13 in 'UnsetPending'
                ;
                (self.platformEntityDic)[fmtIndex] = entity -- DECOMPILER ERROR at PC98: Confused about usage of register: R13 in 'UnsetPending'
                ;
...
FormationSceneCtrl.RefreshFmtScene = function(self, fmtData, forceAnim)
    ...
    for k, heroId in pairs(fmtData.data) do
        ...
        newHeroEntityIdDic[heroId] = heroEntity -- DECOMPILER ERROR at PC10: Confused about usage of register: R10 in 'UnsetPending'
        ;
        (self.heroEntityIdDic)[heroId] = nil
    end
    for k, heroEntity in pairs(self.heroEntityIdDic) do heroEntity:OnDelete() end
    ...
    for index, heroId in pairs(fmtData.data) do
        ...
        if heroData ~= nil then
            ...
            local heroEntity = self:__CreateHeroEntity(heroData,
                                                       (platformEntity.transform).position,
                                                       forceAnim) -- DECOMPILER ERROR at PC46: Confused about usage of register: R12 in 'UnsetPending'
            ;
...
FormationSceneCtrl.__CreateHeroEntity = function(self, heroData, position,
                                                 forceAnim)
    ...
    if heroEntity == nil then
        ...
        coFunc = heroEntity:InitFmtHeroEntity(go, heroData, (self.bind).camera) -- DECOMPILER ERROR at PC28: Confused about usage of register: R7 in 'UnsetPending'
        ;
        ...
        do
            if forceAnim then
                ...
                coFunc = function()
                    -- function num : 0_3_0 , upvalues : heroEntity
            ...
            do
                if self.__loadHeroCo == nil then
                    ...
                    self.__loadHeroCo = (GR.StartCoroutine)(
                                            (util.cs_generator)(loadHeroListFunc))
                end
                return heroEntity
            end
        end
...
FormationSceneCtrl.GetFmtPlatformEntityByGo =
    function(self, gameObject)
        -- function num : 0_5
        return (self.platformEntityGoDic)[gameObject]
    end

FormationSceneCtrl.SwapFmtPlatformHero =
    function(self, fromFmtIndex, toFmtIndex)
        -- function num : 0_6
...
FormationSceneCtrl.OnClickPlatform = function(self, fmtIndex)
    -- function num : 0_7
    if self.__dragPlatform ~= nil then return end
    local heroData = nil
    local heroEntity = (self.heroEntityDic)[fmtIndex]
    if heroEntity ~= nil then heroData = heroEntity:GetFmtHeroEntityData() end
...
FormationSceneCtrl.OnBeginDrag = function(self, platformItem, eventData)
    ...
    if self.__dragPlatform ~= nil then return end
    if not platformItem:IsFmtPlatformUnlock() then return end
...
FormationSceneCtrl.OnDrag = function(self, platformItem, eventData)
    ...
    if targetPlatform == nil and self.__swappedHero ~= nil then
        (self.__swappedHero):DragHeroEndTweenHeroDetailItem(
            ((self.__swappedPlatform).transform).position)
        self.__swappedHero = nil
        self.__swappedPlatform = nil
    end
    if targetPlatform ~= platformItem and targetPlatform ~=
        self.__swappedPlatform and targetPlatform:IsFmtPlatformUnlock() then
        ...
        if heroEntity ~= nil then
            heroEntity:DragHeroEndTweenHeroDetailItem(
                (platformItem.transform).position)
...
FormationSceneCtrl.OnEndDrag = function(self, platformItem, eventData)
    ...
    local targetPlatform = self:__GetTargetPlatform(
                               ((self.__dragHero).transform).position)
    if targetPlatform == nil or not targetPlatform:IsFmtPlatformUnlock() then
        (self.__dragHero):DragHeroEndTweenHeroDetailItem(
            (platformItem.transform).position)
    else
...
FormationSceneCtrl.__GetGroundPoint = function(self)
    ...
    mousePos.x = (math.clamp)(mousePos.x, (self.__screenWidthRange).x,
                              (self.__screenWidthRange).y)
    self:__RaycastGround(mousePos)
    if (self.__lastGroundPos).z < ((self.bind).groudPosZRange).x or
        ((self.bind).groudPosZRange).y < (self.__lastGroundPos).z then
        mousePos.y = (self.__lastMousePos).y
        self:__RaycastGround(mousePos)
    end -- DECOMPILER ERROR at PC35: Confused about usage of register: R2 in 'UnsetPending'
    
    (self.__lastGroundPos).y = self.__platformPosY
    self.__lastMousePos = mousePos
...
FormationSceneCtrl.__RaycastGround = function(self, mousePos)
    ...
    local hits = (CS_Physics.Raycast)((self.bind).camera,
                                      1 << LayerMask.Raycast, true, mousePos)
...
FormationSceneCtrl.__GetTargetPlatform =
    function(self, originPos)
        -- function num : 0_13 , upvalues : CS_Physics, _ENV
        originPos.y = originPos.y + 5
        local targetGo = nil
        local hits = (CS_Physics.Raycast)(originPos, Vector3.down,
                                          1 << LayerMask.Formation)
        for i = 0, hits.Length - 1 do
            local hitCollider = (hits[i]).collider
...
FormationSceneCtrl.SetFormationCameraActive =
...
FormationSceneCtrl.OnDelete = function(self)
    ...
    if self.__loadHeroCo ~= nil then (GR.StopCoroutine)(self.__loadHeroCo) end
