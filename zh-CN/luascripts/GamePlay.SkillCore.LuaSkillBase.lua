...
LuaSkillBase.TakeSkillPlay = function(self, data, isUltSkill, selectTargetCoord,
                                      selectRoles)
    -- function num : 0_8 , upvalues : _ENV
    if isUltSkill then
        self.ultHmp = LuaSkillCtrl:GetUltHMp()
        ...
        self:AddTrigger(eSkillTriggerType.AfterSelfUltEffectEnd,
                        "baseult_after", 1, BindCallback(self, self.PlaySkill,
                                                         data,
                                                         selectTargetCoord,
                                                         selectRoles))
        return true
...
LuaSkillBase.GetSelectTargetAndExecute =
    function(self, selectRoles, executeFunc)
        -- function num : 0_10
        if executeFunc == nil then return true end
        if selectRoles ~= nil and selectRoles.Count > 0 then
            for i = 0, selectRoles.Count - 1 do
                executeFunc(selectRoles[i])
            end
...
LuaSkillBase.AddLuaTrigger = function(self, luaTriggerId, action)
    ...
    if action ~= nil then
        ...
        if (self.luaTriggerDict)[luaTriggerId] ~= nil then
            error((string.format)(
                      "%s已经包含了%s的特殊Lua触发器，请检查%s",
                      (self.cskill).name, luaTriggerId, (self.cskill).dataId))
            return
        end -- DECOMPILER ERROR at PC23: Confused about usage of register: R4 in 'UnsetPending'
...
LuaSkillBase.AddLuaTriggerWithoutBindCB =
    function(self, luaTriggerId, action)
        -- function num : 0_21 , upvalues : _ENV
        if luaTriggerId == nil or action == nil then return end
        if (self.luaTriggerDict)[luaTriggerId] ~= nil then
            error((string.format)(
                      "%s已经包含了%s的特殊Lua触发器，请检查%s",
                      (self.cskill).name, luaTriggerId, (self.cskill).dataId))
            return
        end -- DECOMPILER ERROR at PC22: Confused about usage of register: R3 in 'UnsetPending'
...
LuaSkillBase.UnRegisterLuaTrigger = function(self, luaTriggerId)
    -- function num : 0_22 , upvalues : _ENV
    if luaTriggerId == nil then return end
    local cb = nil
    if (self.luaTriggerDict)[luaTriggerId] ~= nil then
        cb = (self.luaTriggerDict)[luaTriggerId]
    end
    if cb == nil then return end
    LuaSkillCtrl:UnRegisterLuaTrigger(luaTriggerId, cb) -- DECOMPILER ERROR at PC19: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.luaTriggerDict)[luaTriggerId] = nil;
    (table.removebyvalue)(self.luaTriggerList, luaTriggerId)
end
...
LuaSkillBase.RemoveAllLuaTrigger = function(self)
    -- function num : 0_23 , upvalues : _ENV
    if self.luaTriggerDict == nil or self.luaTriggerList == nil then return end
    for i = #self.luaTriggerList, 1, -1 do
        local triggerId = (self.luaTriggerList)[i]
        if (self.luaTriggerDict)[triggerId] ~= nil then
            local cb = (self.luaTriggerDict)[triggerId]
            LuaSkillCtrl:UnRegisterLuaTrigger(triggerId, cb) -- DECOMPILER ERROR at PC26: Confused about usage of register: R7 in 'UnsetPending'
            ;
            (self.luaTriggerDict)[triggerId] = nil
        end
    end
...
LuaSkillBase.PlayUltEffect =
    function(self, data, selectTargetCoord, selectRoles)
        -- function num : 0_31
        self:PlaySkill(data, selectTargetCoord, selectRoles)
    end
