...
bs_103801.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddSelfTrigger(eSkillTriggerType.AfterHurt, "bs_103801_3", 1, self.OnAfterHurt)
  if (self.arglist)[2] == 1 then
    self:AddSelfTrigger(eSkillTriggerType.SetHurt, "bs_103801_2", 2, self.OnSetHurt)
  end
end

bs_103801.OnSetHurt = function(self, context)
  -- function num : 0_2
  if (context.skill).isCommonAttack and context.sender == self.caster and not context.isMiss and context.target ~= self.caster and (context.target):GetBuffTier((self.config).buffId) > 0 then
    local hurtOffSet = context.hurt * (self.arglist)[3] // 1000
