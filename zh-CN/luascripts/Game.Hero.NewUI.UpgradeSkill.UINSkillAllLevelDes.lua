...
local UINSkillAllLevelDesItem = require(
                                    "Game.Hero.NewUI.UpgradeSkill.UINSkillAllLevelDesItem")
UINSkillAllLevelDes.OnInit = function(self)
    ...
    self.DesItemPool = (UIItemPool.New)(UINSkillAllLevelDesItem,
                                        (self.ui).obj_skillInfoItem);
...
UINSkillAllLevelDes.InitAllDes = function(self, skillData)
    ...
    local nowLevel = 0
    if skillData:GetIsUnlock() then nowLevel = skillData.level end
    ...
    ((self.ui).scrollList).verticalNormalizedPosition =
        1 - (nowLevel - 1) / maxLevel
end
