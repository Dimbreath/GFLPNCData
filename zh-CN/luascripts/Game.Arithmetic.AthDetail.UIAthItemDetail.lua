...
UIAthItemDetail.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Close, self, self.__OnClickClose);
    ((self.ui).aTHDetailItem):SetActive(false)
    self.detailItemPool = (UIItemPool.New)(UINAthDetailItem,
                                           (self.ui).aTHDetailItem)
    self.__onAthDataUpdate = BindCallback(self, self.__OnAthDataUpdate)
end

UIAthItemDetail.InitAthItemDetail = function(self, heroData, athData,
                                             replaceAthData)
    -- function num : 0_1 , upvalues : _ENV
    self.athData = athData
    self.replaceAthData = replaceAthData
    ...
    if replaceAthData == nil then
        ...
        if isAdd then
            ...
            detailItem:InitAthDetailItem(self, athData, heroData, false, false);
            (detailItem.transform):SetParent((self.ui).tableDetailPos)
        end -- DECOMPILER ERROR at PC50: Confused about usage of register: R6 in 'UnsetPending'
        ...
        do
            local addItem = (self.detailItemPool):GetOne()
            addItem:InitAthDetailItem(self, athData, heroData, true, true);
            (addItem.transform):SetParent((self.ui).replaceDetailPos) -- DECOMPILER ERROR at PC70: Confused about usage of register: R6 in 'UnsetPending'
            ;
            (addItem.transform).anchoredPosition = Vector2.zero
            local removeItem = (self.detailItemPool):GetOne()
            removeItem:InitAthDetailItem(self, replaceAthData, heroData, false,
                                         true);
            (removeItem.transform):SetParent((self.ui).listDetailPos) -- DECOMPILER ERROR at PC89: Confused about usage of register: R7 in 'UnsetPending'
            ;
            (removeItem.transform).anchoredPosition = Vector2.zero
        end
    end
...
UIAthItemDetail.SetAthItemDetailFunc = function(self, installFunc,
                                                uninstallFunc, replaceFunc)
    -- function num : 0_2
    self.installFunc = installFunc
    self.uninstallFunc = uninstallFunc
    self.replaceFunc = replaceFunc
...
UIAthItemDetail.OnClickInstallAth = function(self)
    -- function num : 0_4
    if self.installFunc ~= nil then (self.installFunc)(self.athData) end
end

UIAthItemDetail.OnClickUninstallAth = function(self)
    -- function num : 0_5
    if self.uninstallFunc ~= nil then (self.uninstallFunc)(self.athData) end
end
...
UIAthItemDetail.__OnAthDataUpdate = function(self, updateAthDic, heroSlot,
                                             deleteAth)
    -- function num : 0_7 , upvalues : _ENV
    if (self.athData ~= nil and deleteAth[(self.athData).uid] ~= nil) or
        self.replaceAthData ~= nil and deleteAth[(self.replaceAthData).uid] ~=
        nil then
        self:__OnClickClose()
        return
    end
    ...
    if self.__changeParentItem ~= nil and self.__getAthItemFunc ~= nil then
        ...
        self.__changeParentItem = nil
        local athItem =
            (self.__getAthItemFunc)((self.__changeParentAthData).uid)
        if athItem ~= nil then
            athItem:SetAthItemRootParent(self.transform)
            self.__changeParentItem = athItem
        end
...
UIAthItemDetail.OnHide = function(self)
    -- function num : 0_10 , upvalues : _ENV, base
    if self.__changeParentItem ~= nil and
        not IsNull((self.__changeParentItem).gameObject) then
        (self.__changeParentItem):SetAthItemRootParent(nil, true)
        self.__changeParentItem = nil
        self.__changeParentAthData = nil
    end
    MsgCenter:RemoveListener(eMsgEventId.OnAthDataUpdate1,
