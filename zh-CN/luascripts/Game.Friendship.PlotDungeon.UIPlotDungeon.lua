...
UIPlotDungeon.__updateDailyChanceAndStrength = function(self)
  ...
  self.totalLimit = ((ConfigData.game_config).DailyDungeonLimt)[self.moduelId]
...
UIPlotDungeon.__loadChapterUI = function(self, heroUIData)
  ...
  ;
  (self.chaptersUI):UpdateWithChapterList(chapterItemList, heroUIData.totalCompleteChapterCount, heroUIData.totalChapterCount, self.onBattleStart)
end
...
UIPlotDungeon.OnBattleStart = function(self)
  ...
  if (PlayerDataCenter.stamina).value < (self.selectChapterItem).costStrengthNum then
...
, self.selectHeroId))
    ;
    (self.battleDungeonNetworkCtrl):CS_BATTLE_DungeonEnter(((self.selectChapterItem).cfg).id, formationData, callBack)
    ControllerManager:DeleteController(ControllerTypeId.SectorController)
    UIManager:DeleteAllWindow()
  end

  fmtCtrl:InitFromationCtrl(eFmtFromModule.FriendshipDungeon, ((self.selectChapterItem).cfg).id, enterFormationFunc, exitFormationFunc, startBattleFunc)
end
