...
bs_1031.OnSetHurt = function(self, context)
    -- function num : 0_3 , upvalues : _ENV
    if ((self.caster).recordTable)["1031_Times"] ~= nil and
        ((self.caster).recordTable)["1031_Times"] > 0 and (context.target).hp <=
        ...
        do
            ...
            end -- DECOMPILER ERROR at PC95: Confused about usage of register: R2 in 'UnsetPending'
...
bs_1031.SkillEventFunc = function(self, target, effect, eventId)
    ...
    if effect.dataId == (self.config).effectId3 and eventId ==
        ...
        local healNum = LuaSkillCtrl:CallFormulaNumberWithSkill(
                            (self.config).healFormula, self.caster, nil, self)
        LuaSkillCtrl:CallHeal(healNum, self, target)
    end
end
