...
bs_1031.config = {selectId = 20, effectId1 = 10040, effectId2 = 10041, effectId3 = 10345}
...
bs_1031.OnSetHurt = function(self, context)
  ...
  if ((self.caster).recordTable)["1031_Times"] > 0 and (context.target).hp <= context.hurt and (context.target).belongNum == (self.caster).belongNum and (context.target).hp > 0 and (context.target).roleType == 1 then
    context.hurt = 0
...
bs_1031.SkillEventFunc = function(self, target, effect, eventId)
  ...
  if effect.dataId == (self.config).effectId3 and eventId == eBattleEffectEvent.Create then
    ...
    LuaSkillCtrl:CallHeal(target.maxHp * (self.arglist)[1] // 1000, self, target)
  end
end
