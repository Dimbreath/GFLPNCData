...
UINAthSortList.InitAthSortListNode = function(self, heroData, withMat,
                                              clickItemFunc, areaId)
    -- function num : 0_1
    self.heroData = heroData
    self.clickItemFunc = clickItemFunc
    self.areaId = areaId
    self.withMat = withMat
    if withMat then (self.athMatUpNode):Show() end
    (self.athListAll):SetNewAthItemFunc(self._newItemFunc);
    (self.athListAll):InitAthSortListAll(areaId, self.__OnClickAthItem,
...
UINAthSortList.RefillCurAthSortList = function(self)
    ...
    (self.athListAll):RefillAthSortListAll(self.__SiftFunction, self._sortFunc,
                                           self._curSuitDic)
