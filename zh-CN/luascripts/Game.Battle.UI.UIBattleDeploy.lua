...
UIBattleDeploy.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_NodeFetter, self,
                               self.OnBtnNodeFetterClicked)
    self.campFetterItems = {}
    self.__OnCampFetterClicked = BindCallback(self, self.OnCampFetterClicked) -- DECOMPILER ERROR at PC26: Confused about usage of register: R1 in 'UnsetPending'
    ;
...
UIBattleDeploy.InitBattleDeployUI = function(self, dynPlayer)
    ...
    for k, v in pairs(campCount) do
        ...
        fetterItem:Init(go)
        fetterItem:InitFetterItem(k, v, self.__OnCampFetterClicked) -- DECOMPILER ERROR at PC38: Confused about usage of register: R10 in 'UnsetPending'
        ;
...
UIBattleDeploy.OnCampFetterClicked = function(self, fetterItem)
    ...
    local campFetterList =
        ((ConfigData.camp_connection).fetterList)[fetterItem.campId]
    if campFetterList == nil then return end
    local activeFetter = ConfigData:GetCampFetter(fetterItem.campId,
                                                  fetterItem.fetters_num)
    local fetterNum = 0
    if activeFetter ~= nil then fetterNum = activeFetter.fetters_num end
    local textInfo = (LanguageUtil.GetLocaleText)((fetterItem.campCfg).name) ..
                         "羁绊:\n"
    for k, v in ipairs(campFetterList) do
        ...
        if fetterCfg.fetters_num == fetterNum then
            textInfo = textInfo ..
                           (string.format)("<color=#2be8eb>%d人:%s</color>\n",
                                           fetterCfg.fetters_num,
                                           fetterCfg:GetDescribe())
        else
            textInfo = textInfo ..
                           (string.format)("%d人:%s\n", fetterCfg.fetters_num,
                                           fetterCfg:GetDescribe())
        end
    end -- DECOMPILER ERROR at PC60: Confused about usage of register: R7 in 'UnsetPending'
...
UIBattleDeploy.ShowMonsterRoleInfo =
    function(self, battleCharacterView, monster)
...
UIBattleDeploy.IsHeroOrMonsterInfoShow =
    function(self)
        -- function num : 0_8
