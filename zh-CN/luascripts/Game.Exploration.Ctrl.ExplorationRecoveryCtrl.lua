...
local ExplorationRecoveryCtrl = class("ExplorationRecoveryCtrl", ExplorationCtrlBase)
...
ExplorationRecoveryCtrl.ctor = function(self, epCtrl)
  ...
  self.epCtrl = epCtrl
  self.__onRecoveryRoomUpdate = BindCallback(self, self.__OnRecoveryRoomUpdate)
  MsgCenter:AddListener(eMsgEventId.OnEventAndRecoveryRoomUpdate, self.__onRecoveryRoomUpdate)
end

ExplorationRecoveryCtrl.OnRecoveryRoomOpen = function(self, roomData)
  -- function num : 0_1 , upvalues : ExplorationEnum, _ENV
  if (roomData.eventData).eRoomType ~= (ExplorationEnum.eRoomType).recovery then
  ...
  if (self.recoveryData).param == nil then
    ...
    ;
    (self.uiWindow):InitEpEventRoom(self.recoveryData, self.__onChoiceItemClick)
  end
)
    else
      ;
    ...
    if (choicelist[((self.recoveryData).param).p1 + 1]).catId == (ExplorationEnum.eEventRoomChoiceType).Upgrade then
      local upgradeCfg = (ConfigData.event_upgrade)[((self.recoveryData).param).p2]
      do
...
ExplorationRecoveryCtrl.OnChoiceItemClick = function(self, choiceCfg, index, isAble, catId)
  -- function num : 0_2 , upvalues : _ENV
  if isAble ~= nil then
    error("RecoveryRoom choose has isAble")
  end
...
ExplorationRecoveryCtrl.OnChoiceItemSelectSuccess = function(self, msg)
...
ExplorationRecoveryCtrl.__ShowItemsMessageTips = function(self, itemDic)
  ...
  for id,count in pairs(itemDic) do
    ...
    if cfg == nil then
      ...
      ;
      (cs_MessageCommon.ShowMessageTips)((string.format)(tipText, name, count), true)
    end
  end
end

ExplorationRecoveryCtrl.__ShowBuffMessageTips = function(self, buffDic)
...
ExplorationRecoveryCtrl.__OnRecoveryRoomUpdate = function(self, roomData)
  -- function num : 0_6
...
ExplorationRecoveryCtrl.SendChipUpgrade = function(self, chipItem, currencyId, callback)
...
ExplorationRecoveryCtrl.SendSpecifyExit = function(self)
...
ExplorationRecoveryCtrl.OnDelete = function(self)
  ...
  MsgCenter:RemoveListener(eMsgEventId.OnRecoveryRoomUpdate, self.__onRecoveryRoomUpdate)
  self.waitRefersh = false
  self.uiWindow = nil
end
