...
local UINInfinityLevelItem = require(
                                 "Game.Sector.SectorLevel.UINInfinityLevelItem")
local ePlaceHolderType =
    (require("Game.Sector.Enum.SectorLevelInfinityEnum")).ePlaceHolderType
local eDynConfigData = require("Game.ConfigData.eDynConfigData")
local UINInfinityWheelItem = require(
                                 "Game.Sector.SectorLevel.UINInfinityWheelItem")
UINInfinityLevelCanvas.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_Roll2CurDepthTop, self,
                               self.Roll2CurDepth);
    (UIUtil.AddButtonListener)((self.ui).btn_Roll2CurDepthDown, self,
                               self.Roll2CurDepth) -- DECOMPILER ERROR at PC42: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopScrollRect).onInstantiateItem =
        BindCallback(self, self.__OnNewItem) -- DECOMPILER ERROR at PC49: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopScrollRect).onChangeItem =
        BindCallback(self, self.__OnChangeItem) -- DECOMPILER ERROR at PC56: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).loopScrollRect).onReturnItem =
        BindCallback(self, self.__OnReturnItem);
    (((self.ui).loopScrollRect).onValueChanged):AddListener(
        BindCallback(self, self.__OnValueChange))
    ...
    self._OnclickLevelItem = BindCallback(self, self.OnclickLevelItem) -- DECOMPILER ERROR at PC103: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.ui).obj_OnItemSelect =
        ((((self.ui).obj_arrows).transform).parent).gameObject
end

UINInfinityLevelCanvas.InitInfinityLevelCanvas =
    function(self, sectorId, lastEpStateCfg, tweenCompleteEvent)
        -- function num : 0_1
...
UINInfinityLevelCanvas.RefreshUncompletedEp =
...
UINInfinityLevelCanvas.GenLevels = function(self)
    ...
    for index, cfg in ipairs((ConfigData.endless)[self.sectorId]) do
        local isUnlock = ((CheckCondition.CheckLua)(cfg.pre_condition,
                                                    cfg.pre_para1, cfg.pre_para2))
        local lockInfo = nil
        if not isUnlock then
            lockInfo = (CheckCondition.GetUnlockInfoLua)(cfg.pre_condition,
                                                         cfg.pre_para1,
                                                         cfg.pre_para2)
        end
        ...
        passNum =
            isComplete or (PlayerDataCenter.infinityData).processingUpdate ==
                nil or
                ((PlayerDataCenter.infinityData).processingUpdate)[cfg.id] or 0;
        ...
        if isComplete or passNum > 0 then maxReachedIndex = index end
        if isComplete then maxCompletedIndex = index end
    end
    ...
    local num = #self.levelDataList -- DECOMPILER ERROR at PC167: Confused about usage of register: R4 in 'UnsetPending'
    ;
...
UINInfinityLevelCanvas.__OnNewItem = function(self, go)
    ...
    levelItem:Init(go) -- DECOMPILER ERROR at PC6: Confused about usage of register: R3 in 'UnsetPending'
    ;
...
UINInfinityLevelCanvas.__OnChangeItem = function(self, go, index)
    ...
    if levelData.isPlaceHoleder then
        levelItem:InitPlaceHolder(levelData, (self.ui).obj_IsTop,
                                  (self.ui).obj_IsDown)
    else
...
UINInfinityLevelCanvas.__OnValueChange =
    function(self, pos)
        -- function num : 0_8
        (self.wheel):RoteWheel(pos.y)
    end

UINInfinityLevelCanvas.OnclickLevelItem =
    function(self, levelData, levelItem)
        -- function num : 0_9
        if levelData.isPlaceHoleder then return end
...
UINInfinityLevelCanvas.m_MoveOnItemSelect =
    function(self, levelItem)
        -- function num : 0_10 , upvalues : _ENV
        (((self.ui).obj_OnItemSelect).transform):SetParent(
            ((levelItem.ui).obj_selectNode).transform) -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
        ;
        (((self.ui).obj_OnItemSelect).transform).anchoredPosition =
            (Vector2.New)(0, 0);
...
UINInfinityLevelCanvas.m_MoveCurDepth = function(self, levelItem)
    ...
    (((self.ui).obj_CurDepth).transform):SetParent(
        ((levelItem.ui).obj_curDepthNode).transform) -- DECOMPILER ERROR at PC16: Confused about usage of register: R2 in 'UnsetPending'
    ;
    (((self.ui).obj_CurDepth).transform).anchoredPosition = (Vector2.New)(0, 0);
    ((self.ui).obj_CurDepth):SetActive(true)
end

UINInfinityLevelCanvas.ShowLevelDetailWindow =
    function(self, callback)
        -- function num : 0_12 , upvalues : _ENV
        UIManager:ShowWindowAsync(UIWindowTypeID.SectorLevelDetail,
                                  function(win)
            -- function num : 0_12_0 , upvalues : self, callback
            local width, duration = win:GetLevelDetailWidthAndDuration()
            win:SetLevelDetaiHideStartEvent(
                function()
    ...
    function(self)
        ...
        if (PlayerDataCenter.infinityData).oldData ~= nil then
            ...
            do
                ...
                for levelId, arg in pairs(
                                        (PlayerDataCenter.infinityData).freshData) do
                    if arg and oldData[levelId] ~= arg then
                        local sectorId =
                            (((ConfigData.endless).levelDic)[levelId]).sectorId
                        local index =
                            (((ConfigData.endless).levelDic)[levelId]).index
                        local cfg = ((ConfigData.endless)[sectorId])[index]
                        if type(oldData[levelId]) == "number" or
                            oldData[levelId] == nil then
                            ...
                            for i = startIndex, #cfg.layer do
                                ...
                                if layerId == nil then
                                    error(
                                        "layerId is nil ,sectorId=" ..
                                            tostring(sectorId) .. " index=" ..
                                            tostring(i))
                                else
                                    local layerCfg =
                                        (ConfigData.endless_layer)[layerId]
                                    if layerCfg == nil then
                                        error(
                                            "layerCfg is nil with id=" ..
                                                tostring(layerId))
                                    else
                                        for index, itemId in
                                            ipairs(layerCfg.reward_itemIds) do
                                            if items[itemId] ~= nil then
                                                items[itemId] =
                                                    items[itemId] +
                                                        (layerCfg.reward_itemNums)[index]
                                            else
                                                items[itemId] =
                                                    (layerCfg.reward_itemNums)[index]
                                            end
                        ...
                        do
                            for index, itemId in
                                ipairs(cfg.clear_reward_itemIds) do
                                if items[itemId] ~= nil then
                                    items[itemId] =
                                        items[itemId] +
                                            (cfg.clear_reward_itemNums)[index]
                                else
                                    items[itemId] =
                                        (cfg.clear_reward_itemNums)[index]
                                end
                ...
                end -- DECOMPILER ERROR at PC126: Confused about usage of register: R3 in 'UnsetPending'
                ...
                UIManager:ShowWindowAsync(UIWindowTypeID.CommonReward,
                                          function(window)
                    -- function num : 0_14_0 , upvalues : ids, nums
                    if window == nil then return end
                    window:InitRewardsItem(ids, nums)
                end)
            end
        end
    end

UINInfinityLevelCanvas.PlayDiffLevelCanvasSwitchTween =
...
UINInfinityLevelCanvas.OnSwitchTweenComplete =
    function(self)
        -- function num : 0_16
...
UINInfinityLevelCanvas.OnSwitchTweenRewind =
    function(self)
        -- function num : 0_17
...
UINInfinityLevelCanvas.OnSwitchTweenEndEvent =
    function(self)
...
UINInfinityLevelCanvas.PlayMoveLeftTween =
    function(self, isLeft, offset, duration)
        -- function num : 0_19 , upvalues : _ENV
        do
            if self.__moveLeftTween == nil then
                local endValue = (Vector2.unity_vector2)(
                                     1 - offset / ((self.transform).rect).width,
                                     1)
                self.__moveLeftTween = (((self.ui).UI_SectorLevel):DOAnchorMax(
                                           endValue, duration)):SetAutoKill(
                                           false)
            end
            do
                if self.__moveLeftTween2 == nil then
                    local endValue2 = (Vector2.unity_vector2)(
                                          offset / ((self.transform).rect).width /
                                              2, 0)
                    self.__moveLeftTween2 =
                        (((self.ui).ui_wheel):DOAnchorMin(endValue2, duration)):SetAutoKill(
                            false)
                end
                if isLeft then
