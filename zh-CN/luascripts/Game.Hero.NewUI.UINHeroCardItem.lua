...
UINHeroCardItem.OnInit = function(self)
    ...
    (((self.ui).img_star).gameObject):SetActive(false);
    ((self.ui).obj_isBench):SetActive(false)
    self:SetRedDotActive(false) -- DECOMPILER ERROR at PC34: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.ui).starArr = {}
    for i = 1, (ConfigData.hero_rank).maxStar / 2 do
        local go = (((self.ui).img_star).gameObject):Instantiate()
...
UINHeroCardItem.InitHeroCardItem = function(self, heroData, resloader,
                                            clickedAction)
    -- function num : 0_1
    self.heroData = heroData
    self.resloader = resloader
...
UINHeroCardItem.RefreshHeroCardItem = function(self)
    ...
    self:__SetStarUI((self.heroData).star)
    self:__SetHeroLevel((self.heroData).level);
    ((self.ui).tex_HeroID):SetIndex(0, tostring((self.heroData).dataId)) -- DECOMPILER ERROR at PC26: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).tex_Name).text = (self.heroData):GetName() -- DECOMPILER ERROR at PC33: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).img_Quality).color = HeroRareColor[(self.heroData).rare] -- DECOMPILER ERROR at PC40: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).img_QualityLight).color = HeroRareColor[(self.heroData).rare] -- DECOMPILER ERROR at PC52: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ...
    if self.__initTexture then
        self.__initTexture = false
        self:__SetTexture(PathConsts:GetCharacterPicPath(
                              (self.heroData):GetResName()), (self.ui).img_Hero)
        local campIcon = (LanguageUtil.GetLocaleText)(
                             ((self.heroData):GetCampCfg()).icon)
        if campIcon ~= nil then
            self:__SetTexture(PathConsts:GetCampPicPath(campIcon),
                              (self.ui).img_Camp)
        end
    end
end
...
UINHeroCardItem.OnItemClicked = function(self)
    -- function num : 0_3
    if self.onClickedAction ~= nil then (self.onClickedAction)(self.heroData) end
end
...
UINHeroCardItem.__SetHeroLevel = function(self, levelNum)
    -- function num : 0_7 , upvalues : _ENV
    ((self.ui).obj_TrimTex):SetActive(levelNum / 10 % 10 ~= 1) -- DECOMPILER ERROR at PC15: Confused about usage of register: R2 in 'UnsetPending'
    ;
    ((self.ui).tex_Level).text = tostring(levelNum)
    -- DECOMPILER ERROR: 1 unprocessed JMP targets
end
...
UINHeroCardItem.__SetTexture = function(self, path, rawImageGo)
    ...
    (self.resloader):LoadABAssetAsync(path, function(texture)
        ...
        if heroData ~= self.heroData then return end
        rawImageGo.texture = texture
    end)
end
