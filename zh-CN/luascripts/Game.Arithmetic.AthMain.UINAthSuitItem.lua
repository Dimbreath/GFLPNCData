...
UINAthSuitItem.InitAthSuitItem = function(self, suitData, resLoader, clickFunc)
    ...
    ((self.ui).tex_SuitCount):SetIndex(0, tostring((self.suitData).curCount),
                                       tostring((self.suitData).maxCount))
    resLoader:LoadABAssetAsync(PathConsts:GetAtlasAssetPath("AthSuitIcon"),
                               function(spriteAtlas)
        -- function num : 0_1_0 , upvalues : self, suitData
        ...
        ((self.ui).img_SuitIcon).sprite =
            spriteAtlas:GetSprite((suitData.cfg).icon)
    end)
end
