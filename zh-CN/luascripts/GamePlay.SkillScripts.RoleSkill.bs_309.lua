...
bs_309.InitSkill = function(self, isMidwaySkill)
    ...
    self:AddTrigger(eSkillTriggerType.SetHeal, "bs_309_1", 1, self.OnSetHeal)
end
...
bs_309.SkillEventFunc = function(self, effect, eventId, target)
    ...
    if effect.dataId == (self.config).effectId1 and eventId ==
...
bs_309.OnSetHeal = function(self, context)
    ...
    if (context.skill).dataId == 309 then
        ...
        for i = 0, targetList.Count - 1 do
            local distance = LuaSkillCtrl:GetRoleGridsDistance(
                                 (targetList[i]).targetRole, context.target)
            if distance < 2 then
                LuaSkillCtrl:RemoveLife(context.heal * 300 // 1000, self,
                                        (targetList[i]).targetRole)
                if (self.arglist)[3] >= 0 then
                    LuaSkillCtrl:CallBuff(self, (targetList[i]).targetRole,
                                          (self.config).buffId, 1, 15)
                end
            end
        end
    end
