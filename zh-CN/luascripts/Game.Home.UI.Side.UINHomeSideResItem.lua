    ...
    do
        if self.itemId ~= data.id then
            ...
            if itemCfg == nil then
                error("Can\'t Find ItemCfg By ID:" .. data.id)
                return
            end -- DECOMPILER ERROR at PC26: Confused about usage of register: R3 in 'UnsetPending'
            
            ((self.ui).img_Icon).sprite = CRH:GetSprite(itemCfg.small_icon)
            self.itemId = data.id
        end -- DECOMPILER ERROR at PC32: Confused about usage of register: R2 in 'UnsetPending'
        
        ((self.ui).img_Period).fillAmount = data.progress -- DECOMPILER ERROR at PC38: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).img_Rate).fillAmount = data.count / data.countMax
    end
end
...
UINHomeSideResItem.PlayCollect = function(self)
    ...
    self.isCollecting = true
    local sequence = (cs_DOTween.Sequence)()
    sequence:Append(((self.ui).img_Rate):DOFillAmount(0,
                                                      ((self.ui).img_Rate).fillAmount *
                                                          2))
    sequence:AppendCallback(function()
        -- function num : 0_2_0 , upvalues : self
        self.isCollecting = false
