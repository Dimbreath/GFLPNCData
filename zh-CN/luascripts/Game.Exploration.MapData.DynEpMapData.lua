...
DynEpMapData.InitMapData = function(self, mapdata, nextMapBrief, opDetail)
    ...
    for i = 0, self.maxMapColNumber do
        ...
        (self.roomMap)[i] = {}
    end -- DECOMPILER ERROR at PC88: Confused about usage of register: R6 in 'UnsetPending'
    
    (self.mapColTypeList)[0] = (DynEpMapColType.New)(self, 0,
                                                     (ExplorationEnum.eTrackLineType).NormalETrack,
                                                     1)
    for i = 1, self.depth do
        ...
        end -- DECOMPILER ERROR at PC115: Confused about usage of register: R12 in 'UnsetPending'
        
        (self.mapColTypeList)[i] = (DynEpMapColType.New)(self, i, trackLineType,
                                                         maxColRoomCount)
    end
    for i = self.depth + 1, self.maxMapColNumber - 1 do
        ...
        (self.mapColTypeList)[i] = (DynEpMapColType.New)(self, i,
                                                         (ExplorationEnum.eTrackLineType).SingleTrack,
                                                         1)
    end -- DECOMPILER ERROR at PC142: Confused about usage of register: R6 in 'UnsetPending'
    
    (self.mapColTypeList)[self.maxMapColNumber] =
        (DynEpMapColType.New)(self, self.maxMapColNumber,
                              (ExplorationEnum.eTrackLineType).EndTrack, 1)
    local deployRoom = (DynEpRoomData.New)(0, 0,
                                           (ExplorationEnum.eRoomType).deploy)
    deployRoom:SetMapData(self)
    self.deployRoom = deployRoom
    local startRoom = (DynEpRoomData.New)(0, 0,
                                          (ExplorationEnum.eRoomType).start)
    ...
    local sortPosList = {}
    for k, v in pairs(mapdata.lineData) do (table.insert)(sortPosList, k) end
...
DynEpMapData.GetRoomByCoord = function(self, coord)
    ...
    if (self.roomMap)[x] == nil or ((self.roomMap)[x])[y] == nil then
        warn("Cant\'t find DynEpRoomData, pos = " .. tostring(x) .. "," ..
                 tostring(y))
        return nil
    end
...
DynEpMapData.GetFirstAbleRoomData = function(self)
    ...
    if (self.roomMap)[1] == nil then return nil end
    for k, roomData in pairs((self.roomMap)[1]) do
        if roomData ~= nil then return roomData end
    end
    return nil
end
