...
local UINAthDecompose = require(
                            "Game.Arithmetic.Strengthen.Decompose.UINAthDecompose")
local UINAthRefactor = require(
                           "Game.Arithmetic.Strengthen.Refactor.UINAthRefactor")
local cs_MessageCommon = CS.MessageCommon
local CS_Resloader = CS.ResLoader
local StrengthenQuality = eItemQualityType.Orange
UIAthStrengthen.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, UINAthSortList, CS_Resloader, UINAthStrItem, UINAthOptAffixItem, UINAthDecompose, UINAthRefactor, UINAthOptimal
    (UIUtil.AddValueChangedListener)((self.ui).tog_opt, self,
                                     self.OnClickOptimalNode);
    (UIUtil.AddValueChangedListener)((self.ui).tog_inherit, self,
                                     self.OnClickRefactor);
    (UIUtil.AddValueChangedListener)((self.ui).tog_Dismantle, self,
                                     self.OnClickDeco);
    (UIUtil.AddButtonListener)((self.ui).lockOpt, self, self.OnClickLockOpt);
    (UIUtil.AddButtonListener)((self.ui).lockFefactor, self,
                               self.OnClickLockRefactor);
    ...
    self.toggleList = {
        [(self.ui).tog_opt] = {
            icon = (((self.ui).tog_opt).transform):FindComponent("Img_Icon",
                                                                 eUnityComponentID.Image),
            name = (((self.ui).tog_opt).transform):FindComponent("Tex_Name",
                                                                 eUnityComponentID.Text)
        },
        [(self.ui).tog_inherit] = {
            icon = (((self.ui).tog_inherit).transform):FindComponent("Img_Icon",
                                                                     eUnityComponentID.Image),
            name = (((self.ui).tog_inherit).transform):FindComponent("Tex_Name",
                                                                     eUnityComponentID.Text)
        },
        [(self.ui).tog_Dismantle] = {
            icon = (((self.ui).tog_Dismantle).transform):FindComponent(
                "Img_Icon", eUnityComponentID.Image),
            name = (((self.ui).tog_Dismantle).transform):FindComponent(
                "Tex_Name", eUnityComponentID.Text)
        }
    }
end
...
UIAthStrengthen.InitAthStrengthen = function(self, athData, heroData)
    ...
    local unlockOpt = funcUnLockCrtl:ValidateUnlock(
                          proto_csmsg_SystemFunctionID.SystemFunctionID_Algorithm_optimize);
    (((self.ui).lockOpt).gameObject):SetActive(not unlockOpt)
    local unlockRefact = funcUnLockCrtl:ValidateUnlock(
                             proto_csmsg_SystemFunctionID.SystemFunctionID_Algorithm_reconsitution);
    (((self.ui).lockFefactor).gameObject):SetActive(not unlockRefact)
    -- DECOMPILER ERROR at PC37: Confused about usage of register: R6 in 'UnsetPending'
...
UIAthStrengthen.OnClickOptimalNode = function(self, isOn)
    ...
    if not isOn or not (Color.New)(0.113, 0.113, 0.113, 1) then
        local togColor = (Color.New)(0.647, 0.647, 0.647, 1)
    end -- DECOMPILER ERROR at PC45: Confused about usage of register: R3 in 'UnsetPending'
    
    (((self.toggleList)[(self.ui).tog_opt]).icon).color = togColor -- DECOMPILER ERROR at PC51: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (((self.toggleList)[(self.ui).tog_opt]).name).color = togColor
end
...
UIAthStrengthen.OnClickRefactor = function(self, isOn)
    ...
    if isOn then
        ...
        ((self.ui).tex_Title):SetIndex(1);
        (self.refactorNode):Show();
        (self.refactorNode):InitAthRefactor(self.sortListNode, self.athData,
                                            self.resLoader)
    else
    ...
    if not isOn or not (Color.New)(0.113, 0.113, 0.113, 1) then
        local togColor = (Color.New)(0.647, 0.647, 0.647, 1)
    end -- DECOMPILER ERROR at PC56: Confused about usage of register: R3 in 'UnsetPending'
    
    (((self.toggleList)[(self.ui).tog_inherit]).icon).color = togColor -- DECOMPILER ERROR at PC62: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (((self.toggleList)[(self.ui).tog_inherit]).name).color = togColor
end
...
UIAthStrengthen.OnClickDeco = function(self, isOn)
    ...
    (((self.toggleList)[(self.ui).tog_Dismantle]).icon).color = togColor -- DECOMPILER ERROR at PC47: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (((self.toggleList)[(self.ui).tog_Dismantle]).name).color = togColor
end
...
UIAthStrengthen.OnClickLockOpt = function(self)
    ...
    if not funcUnLockCrtl:ValidateUnlock(
        proto_csmsg_SystemFunctionID.SystemFunctionID_Algorithm_optimize) then
...
UIAthStrengthen.OnClickLockRefactor = function(self)
    ...
    local funcUnLockCrtl = ControllerManager:GetController(
                               ControllerTypeId.FunctionUnlock)
    if not funcUnLockCrtl:ValidateUnlock(
        proto_csmsg_SystemFunctionID.SystemFunctionID_Algorithm_reconsitution) then
        local msg = funcUnLockCrtl:GetFuncUnlockDecription(
                        proto_csmsg_SystemFunctionID.SystemFunctionID_Algorithm_reconsitution);
        (cs_MessageCommon.ShowMessageTips)(msg)
    end
...
UIAthStrengthen.__OnAthDataUpdate = function(self, updateAthDic, heroSlot,
                                             deleteAth)
    -- function num : 0_11
    if self.athData ~= nil and deleteAth[(self.athData).uid] ~= nil then
        self.athData = nil
