...
local UIEpUpgradeRoomItem = require(
                                "Game.Exploration.UI.UpgradeRoom.UIEpUpgradeRoomItem")
...
UIEpUpgradeRoom.InitUpgradeRoom = function(self, roomCtrl, upgradeCfg,
                                           refreshTime)
    ...
    do
        if (self.cfg).currency ~= nil then
            ...
            if itemCfg ~= nil then self.moneyIconId = itemCfg.icon end
        end
        self.maxRefreshTimes = 0
        if (self.cfg).refresh_times ~= nil then
            self.maxRefreshTimes =
                ((self.cfg).refresh_times)[#(self.cfg).refresh_times]
        end
        if self.maxRefreshTimes >= 0 then
            (((self.ui).refreshTimeText).gameObject):SetActive(true);
            ((self.ui).refreshTimeText):SetIndex(0, tostring(
                                                     self.maxRefreshTimes -
                                                         self.refreshTime))
        else
...
UIEpUpgradeRoom.LoadChipData = function(self)
    ...
    if self.selectedIndex == nil then self.selectedIndex = 1 end
    ((self.ui).chipList):ClearCells() -- DECOMPILER ERROR at PC14: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).chipList).onInstantiateItem =
        BindCallback(self, self.__ChipListInitItem) -- DECOMPILER ERROR at PC21: Confused about usage of register: R1 in 'UnsetPending'
    ;
    ((self.ui).chipList).onChangeItem = BindCallback(self,
                                                     self.__ChipListUpdateItem)
...
UIEpUpgradeRoom.__ChipListUpdateItem = function(self, go, index)
    ...
    local upgradePrice = ConfigData:CalculateEpChipUpgradePrice(self.roomId,
                                                                self.refreshTime)
    chipItem:InitUpgradeRoomItem(self.roomId, self.moneyIconId, upgradePrice,
                                 chipData, self.__OnChipItemClick)
...
UIEpUpgradeRoom.RefreshSelectItemDetail =
    function(self, selectItem)
        ...
        (self.chipDetailPanel):InitChipDetailPanel(self.selectedIndex, chipData,
                                                   (self.roomCtrl).dynPlayer,
                                                   self.resloader, nil, false,
                                                   false, true)
        if selectItem.upgradePrice ~= nil then
            if self.currency == nil then
                self.currency =
                    (ExplorationManager:GetDynPlayer()):GetItemCount(
                        (self.cfg).currency)
            end
...
UIEpUpgradeRoom.__onMoneyUpdate = function(self)
    ...
    print((self.cfg).currency)
    local currencyNum = (ExplorationManager:GetDynPlayer()):GetItemCount(
                            (self.cfg).currency)
...
UIEpUpgradeRoom.OnChipItemClick = function(self, chipItem)
    ...
    if lastChipItem ~= nil then lastChipItem:SetChipItemSelect(false) end
    self.selectedIndex = chipItem.index
    chipItem:SetChipItemSelect(true)
    self:RefreshSelectItemDetail(lastChipItem)
end
...
UIEpUpgradeRoom.OnUpgradeClicked = function(self)
    ...
    (self.roomCtrl):SendChipUpgrade(chipItem, (self.cfg).currency, BindCallback(
                                        self, self.PlaySuccessEfc, chipItem))
end
...
UIEpUpgradeRoom.OnChipDetailActiveChange =
...
UIEpUpgradeRoom.CheckRefreshTimeAndExit =
    function(self)
        -- function num : 0_18
        if self.maxRefreshTimes > -1 and self.maxRefreshTimes - self.refreshTime <=
            0 then self:OnRoomSkipClicked() end
    end
