...
bs_308.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.SetHurt, "bs_308_1", 1, self.OnSetHurt)
end
...
bs_308.callback = function(self)
    ...
    for i = 1, targetList.Count - 1 do
        LuaSkillCtrl:CallBuff(self, (targetList[i]).targetRole,
                              (self.config).buffId2, 1, (self.arglist)[4])
    end
end
...
bs_308.OnSetHurt = function(self, context)
    ...
    if (context.sender).belongNum == (self.caster).belongNum and
        (context.skill).dataId ~= 308 and
        (context.sender):GetBuffTier((self.config).buffId2) > 0 and
        (self.arglist)[5] >= 0 then
        LuaSkillCtrl:RemoveLife(context.hurt * 500 // 1000, self, context.target)
    end
end
