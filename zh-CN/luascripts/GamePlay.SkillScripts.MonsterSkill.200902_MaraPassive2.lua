...
bs_40030.config = {buffId = 163, effectId = 10229, audioId1 = 89, audioId2 = 90}
...
bs_40030.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddSelfTrigger(eSkillTriggerType.SetHurt, "bs_40030_2", 1, self.OnSetHurt)
  self.timer = LuaSkillCtrl:StartTimer(nil, (self.arglist)[1], function()
    -- function num : 0_1_0 , upvalues : self, _ENV
    if (self.caster).hp > 0 and (self.caster):GetBuffTier((self.config).buffId) < (self.arglist)[3] then
...
bs_40030.OnSetHurt = function(self, context)
  ...
  if context.target == self.caster and (self.caster):GetBuffTier((self.config).buffId) > 0 and context.sender ~= self.caster then
    LuaSkillCtrl:DispelBuff(self.caster, (self.config).buffId, 1)
    LuaSkillCtrl:RemoveLife(context.hurt, self, context.sender, true, true)
    context.hurt = 0
    LuaSkillCtrl:CallFloatText(self.caster, 7)
    if (self.caster):GetBuffTier((self.config).buffId) == 0 and self.loop ~= nil then
      (self.loop):Die()
      self.loop = nil
