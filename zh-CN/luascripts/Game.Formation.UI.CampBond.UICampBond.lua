...
local UINCampBondDetailItem = require(
                                  "Game.Formation.UI.CampBond.UINCampBondDetailItem")
UICampBond.OnInit = function(self)
    ...
    (UIUtil.AddButtonListener)((self.ui).btn_CloseDetail, self,
                               self.OnClickCloseDetail);
    ...
    self.bondItemPool =
        (UIItemPool.New)(UINCampBondItem, (self.ui).campBondItem);
    ((self.ui).bondDetailItem):SetActive(false)
    self.bondDetailItemPool = (UIItemPool.New)(UINCampBondDetailItem,
                                               (self.ui).bondDetailItem)
...
UICampBond.ShowBondDetail = function(self, bondItem)
    ...
    for k, v in ipairs(campFetterList) do
        ...
        detailItem:InitCampBondDetailItem(isValid, fetterCfg.fetters_num,
                                          maxCount, describe)
    end -- DECOMPILER ERROR at PC62: Confused about usage of register: R6 in 'UnsetPending'
    
    ((self.ui).campBondListContent).enabled = false -- DECOMPILER ERROR at PC65: Confused about usage of register: R6 in 'UnsetPending'
    ;
...
UICampBond.OnClickCloseDetail = function(self)
    -- function num : 0_3
    if self.showDetailBondItem == nil then return end
    ((self.ui).bondDetailNode):SetActive(false);
    (((self.ui).btn_CloseDetail).gameObject):SetActive(false);
    (self.showDetailBondItem):CampBondItemDetailTween(false, nil, function()
        ...
        ((self.ui).campBondListContent).enabled = true -- DECOMPILER ERROR at PC5: Confused about usage of register: R0 in 'UnsetPending'
        ;
