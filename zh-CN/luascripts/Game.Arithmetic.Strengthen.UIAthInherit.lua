...
UIAthInherit.OnInit = function(self)
    ...
    self.inhtChoiseSubAttrItemPool = (UIItemPool.New)(UINAthAttrItem,
                                                      (self.ui).RTog_SubAttriItem)
    self.allChoiseAttrDict = {}
    self.inhtedIconItem = (UIAthIconItem.New)();
    (self.inhtedIconItem):Init((self.ui).LUI_ATHIconItem)
    self.inhtedSubAttrItemPool = (UIItemPool.New)(UINAthAttrItem,
                                                  (self.ui).LTog_SubAttriItem)
...
UIAthInherit.OnInitData = function(self, athData)
    ...
    for k, v in ipairs(athData.affixList) do
        ...
        do
            ...
            if cfg == nil then
                ...
                attrItem:SetAttrPara(cfg.affix_para, v.quality) -- DECOMPILER ERROR at PC43: Confused about usage of register: R9 in 'UnsetPending'
                ;
                (self.allInhtAttrDict)[attrItem] = k - 1
                local valueChange = BindCallback(
                                        function(value)
                ...
                (((attrItem.ui).tog_SubAttriItem).onValueChanged):AddListener(
                    valueChange)
            end
        end
    end
    ...
    self.consumeItemCfg =
        (ConfigData.item)[(ConfigData.game_config).athInheritConsumeItemId] -- DECOMPILER ERROR at PC81: Confused about usage of register: R2 in 'UnsetPending'
    ;
...
UIAthInherit.OnItemUpdateEvent = function(self)
    ...
    self.palyerGetConsumeItemCount = PlayerDataCenter:GetItemCount(
                                         (ConfigData.game_config).athInheritConsumeItemId);
    ((self.ui).itemCostCountText):SetIndex(0, tostring(
                                               self.consumeItemCount .. "(" ..
                                                   self.palyerGetConsumeItemCount ..
                                                   ")"))
end
...
UIAthInherit.CheckBtnAvalible = function(self)
    ...
    if self.choiseAttrItem ~= nil and self.inhtAttrItem ~= nil and
        self.inheritAthData ~= nil and self.athData ~= nil then
        ((self.ui).btn_Inherit).interactable = true -- DECOMPILER ERROR at PC19: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ...
        ((self.ui).btn_Inherit).interactable = false -- DECOMPILER ERROR at PC28: Confused about usage of register: R1 in 'UnsetPending'
        ;
...
UIAthInherit.OnHide = function(self)
    -- function num : 0_5
    if self.sortListNode ~= nil then (self.sortListNode):Hide() end
    self:ClearAndUpdate()
end
...
UIAthInherit.SetInheritObject = function(self, athData)
    ...
    if athData.lockUnlock then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(TipContent.arithmetic_Inherit_Lock))
        return
    end
    ...
    for k, v in ipairs((self.inheritAthData).affixList) do
        ...
        do
            ...
            if cfg == nil then
                ...
                attrItem:InitAthAttrItem(cfg.affix_para, v.value)
                attrItem:SetAttrColor(v.quality)
                attrItem:SetAttrPara(cfg.affix_para, v.quality) -- DECOMPILER ERROR at PC72: Confused about usage of register: R10 in 'UnsetPending'
                ;
...
UIAthInherit.__OnInherit = function(self)
    ...
    for k, v in pairs(self.allInhtAttrDict) do
        if k.affix_para == (self.choiseAttrItem).affix_para and k ~=
            self.inhtAttrItem then
            (cs_MessageCommon.ShowMessageTips)(
                ConfigData:GetTipContent(
                    TipContent.arithmetic_Inherit_RepeateAttr))
            return
        end
    end
    if (self.choiseAttrItem).quality ~= 4 then
        (cs_MessageCommon.ShowMessageTips)(
            ConfigData:GetTipContent(
                TipContent.arithmetic_Inherit_AttrQualitylimt))
        return
    end
    if self.palyerGetConsumeItemCount < self.consumeItemCount then
        (cs_MessageCommon.ShowMessageTips)(
            (LanguageUtil.GetLocaleText)((self.consumeItemCfg).name) ..
                ConfigData:GetTipContent(
                    TipContent.arithmetic_Inherit_ItemInsufficient))
        return
    end
    (self.athNetwork):CS_ATH_AthInherit(athFromUid, athToUid, attrFromId,
                                        attrToId, self.__OnInheritComplete)
end
...
UIAthInherit.OnInheritComplete = function(self)
    ...
    (cs_MessageCommon.ShowMessageTips)(ConfigData:GetTipContent(
                                           TipContent.arithmetic_Inherit_Success))
    self:ClearAndUpdate()
end
