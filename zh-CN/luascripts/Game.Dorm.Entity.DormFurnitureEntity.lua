...
DormFurnitureEntity.InitFntEntity = function(self, obj, fntData)
    ...
    (self.transform):SetParent(self.rootTran, false)
    local sizeX, sizeY = (self.fntData):GetFntSize()
    local posOffset = (DormUtil.FntCoord2Unity)(sizeX / 2 - 0.5,
                                                sizeY / 2 - 0.5, self.type) -- DECOMPILER ERROR at PC37: Confused about usage of register: R6 in 'UnsetPending'
    ;
    (self.transform).localPosition = posOffset
    self:SetFntEntityPos((self.fntData).x, (self.fntData).y)
    self:__SetRotation()
...
DormFurnitureEntity.SetFntEntityPosFromUnity =
    function(self, unityPos)
        ...
        if move then
            local sizeX, sizeY = (self.fntData):GetFntSize()
            local outX, outY = (DormUtil.FntAreaOutMap)(newX, newY, sizeX,
                                                        sizeY, (self.fntData).r,
                                                        self.type)
            if outX then newX = oldX end
            if outY then newY = oldY end
            move = newX ~= oldX or newY ~= oldY
            if move then self:SetFntEntityPos(newX, newY, true) end
        end
        do return move, oldX, oldY end
        -- DECOMPILER ERROR: 5 unprocessed JMP targets
    end
...
DormFurnitureEntity.RotateFntEntity = function(self)
    -- function num : 0_4 , upvalues : _ENV
    (self.fntData):RotateFnt() -- DECOMPILER ERROR at PC11: Confused about usage of register: R1 in 'UnsetPending'
    ;
    (self.rootTran).eulerAngles = (Vector3.New)(0, (self.fntData).r, 0)
    if (self.fntData):CheckOutMap() then
        self:SetFntEntityPos((self.fntData).x, (self.fntData).y, true)
...
DormFurnitureEntity.__SetRotation = function(self)
    ...
    if self.type == eDormFntType.Wall then
        ...
        angle.y = (self.fntData).r
    end -- DECOMPILER ERROR at PC15: Confused about usage of register: R2 in 'UnsetPending'
...
DormFurnitureEntity.ShowFntOverlap = function(self)
    -- function num : 0_10
    if self.fntBottom ~= nil then (self.fntBottom):ShowOverlap(self.overlap) end
end
