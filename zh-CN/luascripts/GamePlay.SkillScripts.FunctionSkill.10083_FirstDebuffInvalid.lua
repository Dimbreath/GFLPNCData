...
bs_10083.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddTrigger(eSkillTriggerType.BeforeAddBuff, "bs_10083_2", 1, self.OnBeforeAddBuff)
end
...
bs_10083.InitSkill = function(self, isMidwaySkill)
    ...
    ((self.caster).recordTable).buffTime = (self.arglist)[1]
    self:AddTrigger(eSkillTriggerType.BeforeAddBuff, "bs_10083_2", 1,
...
bs_10083.OnBeforeAddBuff = function(self, target, context)
    ...
    if (context.buff).buffType == 2 and target == self.caster and
        ((self.caster).recordTable).buffTime > 0 then
        (context.buff).tier = 0
        self:PlayChipEffect() -- DECOMPILER ERROR at PC22: Confused about usage of register: R3 in 'UnsetPending'
        ;
        ((self.caster).recordTable).buffTime =
            ((self.caster).recordTable).buffTime - 1
    end
end
