...
UINLtrPoolUpHero.InitLtrPoolUpHero = function(self, rewardData, resLoader)
  ...
  ((self.ui).img_Quality):SetIndex((rewardData.rankCfg).rare - 1)
...
UINLtrPoolUpHero.InitLtrPoolUpHero = function(self, rewardData, resLoader)
    ...
    if resCfg == nil then
        error("resource model Cfg is null,id:" ..
                  tostring((rewardData.heroCfg).src_id))
        return
    end
    resLoader:LoadABAssetAsync(PathConsts:GetCharacterPicPath(resCfg.res_Name),
                               function(texture)
        -- function num : 0_1_0 , upvalues : _ENV, self
    ...
    local careerCfg = (ConfigData.career)[(rewardData.heroCfg).career] -- DECOMPILER ERROR at PC43: Confused about usage of register: R5 in 'UnsetPending'
    ;
    ((self.ui).img_Carrer).sprite = CRH:GetSprite(careerCfg.icon,
                                                  CommonAtlasType.CareerCamp) -- DECOMPILER ERROR at PC51: Confused about usage of register: R5 in 'UnsetPending'
    ;
