...
UISector.OnInit = function(self)
    ...
    (self.resourceGroup):SetResourceIds({1003, 1004});
...
UISector.OnClickBackBtn = function(self)
    -- function num : 0_4
    if self.sctCtrl ~= nil then (self.sctCtrl):OnBtnHomeClicked() end
end

UISector.UpdateUIAllSctBuildRes = function(self, allResDic, hasRes)
    -- function num : 0_5 , upvalues : _ENV
    self.__showOutputInfo = hasRes
    if self.__onlyShowTop then return end
    self:__ShowOutputInfo(self.__showOutputInfo)
    if not self.__showOutputInfo then return end
    local allResList = {}
    for k, v in pairs(allResDic) do (table.insert)(allResList, v) end
    ...
    for k, allResData in ipairs(allResList) do
        if (self.ui).resShowCount < k then return end
        local resList = {};
