...
UIOasisBuildingItemDetail.UpdateState = function(self)
  ...
  if (self.buildingItem).currentRemainSecond ~= nil and (self.buildingItem).currentRemainSecond > 0 then
    self.remainSecond = (self.buildingItem).currentRemainSecond
    -- DECOMPILER ERROR at PC38: Confused about usage of register: R1 in 'UnsetPending'

    ;
    ((self.ui).img_Updating).fillAmount = (self.buildingItem).currentProgress
  else
  ...
  do
    ...
    if self.remainSecond ~= nil and self.remainSecond > 0 then
      ((self.ui).tex_Time).text = TimestampToTime(self.remainSecond, false)
    end
      ...
      if state == (BuildingItem.itemState).normal then
        ...
        do
          ...
          if self.accelerateNode ~= nil then
            (self.accelerateNode):InitAccelerateNode(self.remainSecond, (self.UISidle).AccelerateEvent, self.id)
            self.accNeedItemCount = (self.accelerateNode).finalItemNum
          end
          self:UpdateAccCostItem()
          do
            local accBtnTextIndex = state == (BuildingItem.itemState).constructing and 1 or 0
            ;
...
UIOasisBuildingItemDetail.UpdateAccCostItem = function(self)
...
UIOasisBuildingItemDetail.UpdateProcessEvent = function(self, progress, remainTimeText, second)
  ...
  ;
  ((self.ui).tex_Time).text = remainTimeText
...
UIOasisBuildingItemDetail.__OnAccelerate = function(self)
  -- function num : 0_17
  if self.accelerateNode == nil then
    return 
  end
