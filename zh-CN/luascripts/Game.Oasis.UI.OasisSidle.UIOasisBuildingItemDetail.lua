...
UIOasisBuildingItemDetail.OnInit = function(self)
    -- function num : 0_0 , upvalues : _ENV, CostMatInfo, UINOBUAccelerateNode, InfoItem
    self.__resloader = resLoader;
    (UIUtil.LuaUIBindingTable)(self.transform, self.ui);
    (UIUtil.AddButtonListener)((self.ui).btn_Upgrade, self,
                               self.__OnClickUpdrage);
    (UIUtil.AddButtonListener)((self.ui).btn_BuildAcc, self, self.__OnAccelerate)
    self.costMatPool = (UIItemPool.New)(CostMatInfo, (self.ui).costItem)
    self.accelerateNode = (UINOBUAccelerateNode.New)();
    (self.accelerateNode):Init((self.ui).quickUpgraded)
    self.accNeedItemCount = 0
...
UIOasisBuildingItemDetail.UpdateState = function(self)
    -- function num : 0_3 , upvalues : _ENV, BuildingItem
    ...
    if (self.buildingItem).currentRemainSecond ~= nil and
        ...
        self.remainSecond = (self.buildingItem).currentRemainSecond -- DECOMPILER ERROR at PC38: Confused about usage of register: R1 in 'UnsetPending'
        ;
        ((self.ui).img_Updating).fillAmount =
            (self.buildingItem).currentProgress
    else
        local needTimestamp = (self.buildingData):GetNextLevelNeedTime()
        self.remainSecond = needTimestamp -- DECOMPILER ERROR at PC46: Confused about usage of register: R2 in 'UnsetPending'
        ;
        ((self.ui).img_Updating).fillAmount = 0
    end
    do
        -- DECOMPILER ERROR at PC59: Confused about usage of register: R1 in 'UnsetPending'

        if self.remainSecond ~= nil and self.remainSecond > 0 then
            ((self.ui).tex_Time).text =
                TimestampToTime(self.remainSecond, false)
        end
        ...
        if state == (BuildingItem.itemState).disnormal then
            ...
            if state == (BuildingItem.itemState).normal then
                ...
                if not isHideBtn then self:UpdateCostItems() end
                ...
                do
                    ...
                    if self.accelerateNode ~= nil then
                        (self.accelerateNode):InitAccelerateNode(
                            self.remainSecond, (self.UISidle).AccelerateEvent,
                            self.id)
                        self.accNeedItemCount =
                            (self.accelerateNode).finalItemNum
                    end
                    self:UpdateAccCostItem()
                    do
                        local accBtnTextIndex =
                            state == (BuildingItem.itemState).constructing and 1 or
                                0;
                        ((self.ui).tex_AccBtn):SetIndex(accBtnTextIndex)
                        self:UpdateLogic()
    ...
    function(self)
        ...
        for _, id in ipairs(
            ...
            if (PlayerDataCenter.itemDic)[id] ~= nil then
                ...
                item:UpdateUI(id, self.accNeedItemCount, itemData:GetCount())
            else
                do
                    do
                        item:UpdateUI(id, self.accNeedItemCount, 0)
...
UIOasisBuildingItemDetail.UpdateJumpLogic =
    function(self)
        -- function num : 0_10 , upvalues : _ENV, JumpManager
        local jumpTargetId = (self.buildingData):GetJumpTargetId()
        if jumpTargetId then
            (((self.ui).btn_Function).onClick):RemoveAllListeners();
            (((self.ui).btn_Function).gameObject):SetActive(true);
            (UIUtil.AddButtonListener)((self.ui).btn_Function, self, function()
                -- function num : 0_10_0 , upvalues : JumpManager, jumpTargetId
                JumpManager:Jump(jumpTargetId)
            end)
        else

            (((self.ui).btn_Function).gameObject):SetActive(false)
        end
    end
...
UIOasisBuildingItemDetail.UpdateProcessEvent =
    function(self, progress, remainTimeText, second)
        -- function num : 0_16
        self.remainSecond = second -- DECOMPILER ERROR at PC3: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).tex_Time).text = remainTimeText -- DECOMPILER ERROR at PC6: Confused about usage of register: R4 in 'UnsetPending'
        ;
        ((self.ui).img_Updating).fillAmount = progress
        if self.accelerateNode ~= nil then
            (self.accelerateNode):UpdateTime(second);
            (self.accelerateNode):AutoAdd(self.remainSecond)
            self.accNeedItemCount = (self.accelerateNode).finalItemNum
            self:UpdateAccCostItem()
        end
    end

UIOasisBuildingItemDetail.__OnAccelerate =
    function(self)
        -- function num : 0_17
        if self.accelerateNode == nil then return end
        (self.accelerateNode):OnClickConfirm()
    end
...
UIOasisBuildingItemDetail.OnDelete = function(self)
    ...
    if self.accelerateNode ~= nil then (self.accelerateNode):OnDelete() end
    (base.OnDelete)(self)
end
