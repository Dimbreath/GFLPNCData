    ...
    (self.resloader):LoadABAssetAsync(path, function(prefab)
        ...
        do
            if (self.imgCfg).posId ~= nil then
                ...
                if commonPicCtrl:SetPosType(posType) == false then
                    warn((string.format)(
                             "[SetPosType]Avg hero posId error, imgPath = %s, posType = %s",
                             (self.imgCfg).imgPath, posType))
                end
            end
            self.commonPicCtrl = commonPicCtrl -- DECOMPILER ERROR at PC39: Confused about usage of register: R3 in 'UnsetPending'
            ;
            (self.ui).rawImg = go:FindComponent(eUnityComponentID.RawImage)
            self:InitAvgHeroPicParam()
            self.loadResComplete = true
...
UINAvgHeroPic.InitAvgHeroPicParam = function(self)
    ...
    (((self.ui).rawImg).transform).localPosition = Vector3.zero -- DECOMPILER ERROR at PC27: Confused about usage of register: R3 in 'UnsetPending'
    ;
    (self.transform).localPosition = pos
    local rot = nil
    if imgCfg.rot == nil then
    ...
    if imgCfg.scale == nil then
        ...
        scale = (Vector3.New)((imgCfg.scale)[1], (imgCfg.scale)[2],
                              (imgCfg.scale)[3])
    end -- DECOMPILER ERROR at PC65: Confused about usage of register: R5 in 'UnsetPending'
    
    (self.transform).localScale = scale
    if not imgCfg.isDark or not Color.gray then local color = Color.white end
    if imgCfg.alpha ~= nil or not (((self.ui).rawImg).color).a then
        local alpha = imgCfg.alpha
    end
    color.a = alpha -- DECOMPILER ERROR at PC88: Confused about usage of register: R7 in 'UnsetPending'
    ;
    ((self.ui).rawImg).color = color
    self:__ShowCommunication(imgCfg.comm)
end
...
UINAvgHeroPic.GetAvgHeroPicResetData = function(self, posPresetId)
    ...
    local picPosData = (self.commonPicCtrl):GetPosTypeData(posType)
    if picPosData == nil then
        warn((string.format)("Avg hero posId error, imgPath = %s, posType = %s",
                             (self.imgCfg).imgPath, posType))
    end
    return picPosData
end
...
UINAvgHeroPic.__ShowCommunication = function(self, show)
    -- function num : 0_6 , upvalues : _ENV
    local avgWindow = UIManager:GetWindow(UIWindowTypeID.Avg)
    if avgWindow == nil then return end
    if show and self.commItem == nil and self.commonPicCtrl ~= nil then
        local commItem, heroMat = avgWindow:GetHeroCommItem()
        local pos = (self.commonPicCtrl).avgCommPos
        commItem:InitAvgHeroCommItem(pos, (self.ui).breatheRoot) -- DECOMPILER ERROR at PC27: Confused about usage of register: R6 in 'UnsetPending'
        ;
        ((self.ui).rawImg).material = heroMat
        self.commItem = commItem
    else
        do
            if not show and self.commItem ~= nil then
                ((self.ui).breatheRoot):SetParent(self.transform)
                avgWindow:ReturnHeroCommItem(self.commItem) -- DECOMPILER ERROR at PC45: Confused about usage of register: R3 in 'UnsetPending'
                ;
                ((self.ui).rawImg).material = nil
                self.commItem = nil
            end
