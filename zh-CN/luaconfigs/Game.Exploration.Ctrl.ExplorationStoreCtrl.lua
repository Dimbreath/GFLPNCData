...
ExplorationStoreCtrl.OnStoreRoomOpen = function(self, roomData, isFirstOpen)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.EpStoreRoom, function(window)
    ...
    if isFirstOpen then
      ...
      if not ((self.epCtrl).autoCtrl):IsAutoModeRunning() then
...
ExplorationStoreCtrl.SendStorePurchase = function(self, index, price, completeAction)
  ...
  local waitChipDiff = false
  if ConfigData:GetItemType(storeData.id) == eItemType.GlobalChip then
    waitChipDiff = not (self.dynPlayer):IsNormalChipFullLevel(storeData.id)
  end
  ...
  if price <= money + couldLoanMoney or price <= 0 then
    self.waitRefersh = true
    ;
    (self.netWork):CS_EXPLORATION_STORE_Purchase(index - 1, (self.roomData).position, waitChipDiff, function()
