...
local eSectorState = {None = 0, Normal = 1, ShowSto = 2, SelectSectorLevel = 3, DungeonWindow = 4, DailyChallenge = 5, WeeklyChallenge = 6}
...
SectorController.SetFrom = function(self, from, argFunc, fromArg, lastSatgeData)
  -- function num : 0_16 , upvalues : _ENV, eSectorState, SectorLevelDetailEnum, DungeonTypeData, util
  ...
  local waitEnterCo = function()
    -- function num : 0_16_0 , upvalues : from, _ENV, self, fromArg, lastSatgeData, eSectorState, SectorLevelDetailEnum, DungeonTypeData, argFunc
    if from == AreaConst.Home or from == AreaConst.Oasis then
      ...
      -- function num : 0_16_0_0 , upvalues : self, eSectorState, _ENV, lastSelectSector
      if window == nil then
        return 
      end
      ...
      window:InitSectorLevel(lastSelectSector, self.__ResetToNormalState)
      ;
      (UIUtil.CloseOneCover)("lastSelectSector")
            ...
            while 1 do
              if (PlayerDataCenter.sectorStage).lastChallengeType == proto_csmsg_SystemFunctionID.SystemFunctionID_DailyChallenge or UncompletedEpSectorStateCfg ~= nil and UncompletedEpSectorStateCfg.difficulty == (SectorLevelDetailEnum.eDifficulty).daily_challenge then
      ...
      -- function num : 0_16_0_1 , upvalues : self, lastSatgeData
      if window == nil then
        return 
      end
      self:EnbleSectorUI3D(true)
        ...
        window:OnClickDailyBtn()
          ...
          if (PlayerDataCenter.sectorStage).lastChallengeType == proto_csmsg_SystemFunctionID.SystemFunctionID_WeeklyChallenge or UncompletedEpSectorStateCfg ~= nil and UncompletedEpSectorStateCfg.difficulty == (SectorLevelDetailEnum.eDifficulty).weekly_challenge then
            if (PlayerDataCenter.allWeeklyChallengeData):IsExistChallenge() then
              UIManager:ShowWindowAsync(UIWindowTypeID.DailyChallenge, function(window)
      -- function num : 0_16_0_2 , upvalues : self, lastSatgeData
      if window == nil then
        return 
      end
      self:EnbleSectorUI3D(true)
      ...
      window:InitWeeklyChallenge(function(tohome)
        -- function num : 0_16_0_2_0 , upvalues : self
        self:ResetToNormalState(tohome)
      end
)
      if lastSatgeData == nil or not lastSatgeData.isWin then
        ...
        do
          if from == AreaConst.FriendshipMoments then
            ...
            if from == AreaConst.DungeonBattle then
              ...
              (self.homeToSectorGo):SetActive(true)
              self:DetectedGeneralDungeonUnlock()
              if lastSatgeData ~= nil and lastSatgeData.dungeonType == (DungeonTypeData.eDungeonType).fragDungeon then
                self:EnbleSectorUI3D(false)
                self:__OnEnterSector()
              else
                ;
...
SectorController.ResetToNormalState = function(self, toHome)
  -- function num : 0_18 , upvalues : eSectorState, _ENV, JumpManager
  ...
  if toHome or JumpManager:GetIsJumping() or GuideManager:TryTriggerGuide(eGuideCondition.InSectorSceneNormal) then
  end
end
...
SectorController.OnSectorItemClicked = function(self, sectorId, difficuty, stageCfg)
  -- function num : 0_23 , upvalues : _ENV, cs_MessageCommon, eSectorState
  if self:IsDisableClick() then
    return 
  end
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.SectorLevel, function(window)
    -- function num : 0_23_0 , upvalues : self, eSectorState, _ENV, sectorId, difficuty, stageCfg
    if window == nil then
      return 
    end
    self.sctState = eSectorState.SelectSectorLevel
...
SectorController.OnBtnHomeClicked = function(self)
  -- function num : 0_26 , upvalues : eSectorState, _ENV
...
SectorController.OnHomeToSectorDirectorStopped = function(self, director)
  ...
  if self.homeToSectorDirector == director then
    ...
    self:__OnEnterSector()
    if self.sctState == eSectorState.None then
      if self.__sectorFromArg == nil then
...
SectorController.__OnEnterSector = function(self)
  ...
  UIManager:ShowWindowAsync(UIWindowTypeID.Sector, function(window)
    ...
    ;
    (UIUtil.SetTopStatus)(self, self.OnBtnHomeClicked, {ConstGlobalItem.SKey}, nil, nil)
  end
)
end

SectorController.ShowStrategyOverview = function(self, sectorId, buildId)
  -- function num : 0_30 , upvalues : _ENV, cs_MessageCommon, eSectorState
  if self:IsDisableClick() then
    return 
  end
...
SectorController.ConfirmOver = function(self, id)
  -- function num : 0_35 , upvalues : _ENV, BuildingBelong, cs_MessageCommon
  local buildingData = ((PlayerDataCenter.AllBuildingData).built)[id]
...
SectorController.IsDisableClick = function(self)
  -- function num : 0_41 , upvalues : eSectorState
