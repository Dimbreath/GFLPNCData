...
bs_101001.config = {buffId_149 = 149, buffId_151 = 151, buffId_66 = 66, audioId1 = 75, 
heal_config = {baseheal_formula = 10116}
}
...
bs_101001.OnSetHurt = function(self, context)
  ...
  if context.target == self.caster and self:IsReadyToTake() and LuaSkillCtrl:GetGridsDistance((context.sender).x, (context.sender).y, (self.caster).x, (self.caster).y) <= 1 and LuaSkillCtrl:CallRange(1, 1000) <= (self.arglist)[3] and LuaSkillCtrl:RoleContainsBuffFeature(context.sender, eBuffFeatureType.NotBeSelected) ~= true and LuaSkillCtrl:RoleContainsBuffFeature(context.sender, eBuffFeatureType.Exiled) ~= true then
    ...
    if buff ~= nil and (context.sender).x == targetX and (context.sender).y == targetY then
      LuaSkillCtrl:CallBuff(self, context.sender, (self.config).buffId_66, 1, (self.arglist)[5])
    else
...
bs_101001.OnAfterAddBuff = function(self, buff, target)
  ...
  if buff.dataId == 66 and target.belongNum ~= (self.caster).belongNum then
