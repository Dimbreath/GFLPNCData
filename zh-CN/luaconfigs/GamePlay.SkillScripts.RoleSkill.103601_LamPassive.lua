    ...
    do
      do
        ...
        if (self.arglist)[5] > 0 and (self.arglist)[1] < self.speed_num then
          ...
          ;
          (self.caster):AddRoleProperty(eHeroAttr.critDamage, -ret_old_damage, eHeroAttrType.Extra)
          local ret = self.speed_num - (self.arglist)[1]
          local ret_damage = ret * (self.arglist)[4] // (self.arglist)[2]
          ;
          (self.caster):AddRoleProperty(eHeroAttr.critDamage, ret_damage, eHeroAttrType.Extra)
        else
          do
