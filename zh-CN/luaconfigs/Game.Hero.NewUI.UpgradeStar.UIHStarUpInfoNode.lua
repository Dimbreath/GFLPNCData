...
UIHStarUpInfoNode.__RefreshPreviewItem = function(self, previewRank)
  ...
  local skinPreData = self:__GetSkinPreviewItemData(previewRank)
  if skinPreData ~= nil then
    (table.insert)(self.itemDataList, skinPreData)
  end
  ;
...
UIHStarUpInfoNode.__GenSkillPreviewItemData = function(self, previewRank)
  ...
  local addSkillDataFunc = function(skillData, tipsId, preUSkilllevel, unlockAdvance)
  ...
  for _,skillData in ipairs((self.heroData).skillList) do
    ...
    do
      do
        ...
        do
          if skillData:IsNormalSkill() and (skillData.skillCfg).advanced_open_star == previewRank then
            local tipsId = 613
...
UIHStarUpInfoNode.__GetSkinPreviewItemData = function(self, previewRank)
  -- function num : 0_12 , upvalues : _ENV, previewType
  ...
  local preRankCfg = ((ConfigData.hero_star)[heroId])[previewRank]
  local skinId = nil
