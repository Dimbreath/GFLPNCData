...
DormHouseCtrl.OnEnterDormScene = function(self)
  ...
  if not IsNull(((self.dormCtrl).comRes).selectRoomEffectPrefab) then
    self.selectRoomEffect = (((self.dormCtrl).comRes).selectRoomEffectPrefab):Instantiate()
    ;
...
DormHouseCtrl.AddNewRoom = function(self, x, y, spos, roomData)
  ...
  if roomData ~= nil then
    ...
    roomPath = PathConsts:GetDormRoomPath((ConfigData.game_config).DormEmptyRoomRes)
  end
  ;
...
DormHouseCtrl.OnHouseClicked = function(self, roomEntity)
  ...
  if roomEntity.roomData == nil and (state == eDormState.House or state == eDormState.HouseEdit) then
  ...
  if state == eDormState.House then
    ...
    continueWindow:InitContinue()
    -- DECOMPILER ERROR at PC47: Confused about usage of register: R4 in 'UnsetPending'

    ;
    (((self.dormCtrl).bind).roomTarget).position = (roomEntity.transform).position
    ;
    (TimelineUtil.Play)(((self.dormCtrl).bind).tl_dormToRoom, self.__onD2HTimelineComplete, nil, true)
    ;
    ...
    opWindow:InitDormHouseOperate((roomEntity.transform).position, (self.editHouseData):IsEmptyOrBpRoom((self.curEditSelectRoom).spos))
  end
)
      end
    end
...
DormHouseCtrl.ReplaceHouseRoom = function(self, roomId, roomData)
  ...
  ;
  (CS_DormCameraCtrl.Instance):RemoveDormTargetGroup((self.curEditSelectRoom).transform)
  DestroyUnityObject((self.curEditSelectRoom).gameObject)
  local roomEntity = (DormRoomEntity.New)()
  if roomData == nil then
    roomData = (DormRoomData.CreateNewRoom)(spos, roomId)
  end
  roomEntity:InitHouseData((self.curEditSelectRoom).x, (self.curEditSelectRoom).y, spos, roomData)
  ...
  (self.resloader):LoadABAssetAsync(roomPath, function(prefab)
    ...
    if (self.roomEnityDic)[spos] ~= roomEntity then
      return 
    end
    local go = prefab:Instantiate(self.dormHolder)
    roomEntity:InitHouseObject(go, self.__onHouseClicked, self.resloader)
...
DormHouseCtrl.RecycleHouseRoom = function(self)
  ...
  (CS_DormCameraCtrl.Instance):RemoveDormTargetGroup((self.curEditSelectRoom).transform)
  DestroyUnityObject((self.curEditSelectRoom).gameObject)
  self:AddNewRoom((self.curEditSelectRoom).x, (self.curEditSelectRoom).y, (self.curEditSelectRoom).spos)
  self.curEditSelectRoom = nil
  UIManager:HideWindow(UIWindowTypeID.DormOperate)
  local editWindow = UIManager:GetWindow(UIWindowTypeID.DormEdit)
  if editWindow ~= nil then
    editWindow:HideEditRoomList()
  end
end
...
DormHouseCtrl.DecomposeHouseRoom = function(self)
  ...
  ;
  ((self.dormCtrl).dormNetwork):CS_DORM_RoomDecomposition(houseId, param, formHouse)
  UIManager:HideWindow(UIWindowTypeID.DormOperate)
  local editWindow = UIManager:GetWindow(UIWindowTypeID.DormEdit)
  if editWindow ~= nil then
    editWindow:HideEditRoomList()
...
DormHouseCtrl.RecvHouseRoomDecompose = function(self, success, prefabUpdate)
  -- function num : 0_15 , upvalues : CS_DormCameraCtrl, _ENV
  if not success then
    return 
  end
  local roomData = (self.curEditSelectRoom).roomData
  ...
  ;
  (self.editHouseData):ApplyDecomposeHouseRoom(formHouse, param, prefabUpdate, (self.dormCtrl).allDormData)
  ;
  (CS_DormCameraCtrl.Instance):RemoveDormTargetGroup((self.curEditSelectRoom).transform)
  DestroyUnityObject((self.curEditSelectRoom).gameObject)
  self:AddNewRoom((self.curEditSelectRoom).x, (self.curEditSelectRoom).y, (self.curEditSelectRoom).spos)
...
DormHouseCtrl.ExitRoom = function(self)
  ...
  ;
  (TimelineUtil.Rewind)(((self.dormCtrl).bind).tl_dormToRoom, self.__onH2DTimelineComplete, true)
  ;
  ...
  ;
  (self.dormCtrl):EmitExitDormRoom()
end

DormHouseCtrl.OnD2HTimelineComplete = function(self, pd)
  -- function num : 0_17 , upvalues : eDormState, _ENV
  -- DECOMPILER ERROR at PC2: Confused about usage of register: R2 in 'UnsetPending'

  (self.dormCtrl).state = eDormState.Room
  UIManager:HideWindow(UIWindowTypeID.ClickContinue)
  ;
  (self.dormCtrl):EmitEnterDormRoom()
end

DormHouseCtrl.OnH2DTimelineComplete = function(self, pd)
  -- function num : 0_18 , upvalues : eDormState, _ENV, CS_DormCameraCtrl
  -- DECOMPILER ERROR at PC2: Confused about usage of register: R2 in 'UnsetPending'
