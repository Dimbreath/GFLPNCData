...
DormAIVisitState.StartState = function(self)
  -- function num : 0_2 , upvalues : DormAIStateBase, DormUtil, _ENV
  ...
  local success = (self.charEntity):DoMoveLogic(pos, BindCallback(self, self.VisitOtherDormRoom))
  if success then
    (self.charEntity):AnimatorCrossFade((self.actionCfg).action_name, (self.actionCfg).trans_time)
  else
    ;
...
DormAIVisitState.VisitOtherDormRoom = function(self)
  ...
  local pointList = ((self.charEntity).roomCharCtrl):FindNearActivePoint(16, (self.charEntity).x, (self.charEntity).y)
  if #pointList <= 0 then
    self.exeState = (DormAIStateBase.AIExecuteState).Fail
    ;
    (self.charEntity):AnimatorStand()
  ...
  local success = (self.charEntity):DoMoveLogic(pos, BindCallback(self, self.OnExit))
  if success then
    (self.charEntity):AnimatorCrossFade((self.actionCfg).action_name, (self.actionCfg).trans_time)
  else
    ;
