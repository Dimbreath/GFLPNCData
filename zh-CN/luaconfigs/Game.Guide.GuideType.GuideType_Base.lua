...
GuideType_Base.RunNextInternal = function(self, guideStepCfg)
  ...
  if guideStepCfg.delay ~= nil and guideStepCfg.delay > 0 then
    self.waitTimer = (TimerManager:GetTimer(guideStepCfg.delay, self.RunCurStep, self, true, false, self:IsTimerUnScale())):Start()
  else
    self:RunCurStep()
  end
end
...
GuideType_Base.NextWaitUIWindow = function(self, guideStepCfg)
  ...
  if guideStepCfg.wait_target then
    ...
    if waitWindow ~= nil then
      ...
      self.waitTimer = (TimerManager:GetTimer(0.2, waitWindowFunc, nil, false, false, self:IsTimerUnScale())):Start()
    end
  else
    do
      if self:NeedWaitWarn() then
...
GuideType_Base.NextWaitSceneTarget = function(self, guideStepCfg)
  ...
  if guideStepCfg.wait_target then
    ...
    if not IsNull(targetObj) then
      ...
      self.waitTimer = (TimerManager:GetTimer(0.2, waitTargetFunc, nil, false, false, self:IsTimerUnScale())):Start()
    end
  else
    do
      if self:NeedWaitWarn() then
...
GuideType_Base.NextWaitScene = function(self, guideStepCfg, nextAction)
  ...
  if scene_Name == targetSceneName then
    ...
    if guideStepCfg.wait_target then
      ...
      do
        local waitTargetFunc = function()
    -- function num : 0_10_0 , upvalues : CS_GSceneManager_Ins, targetSceneName, self, nextAction, _ENV, waittime, fallbackTime, guideStepCfg
