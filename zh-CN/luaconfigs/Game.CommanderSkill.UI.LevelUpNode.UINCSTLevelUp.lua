...
UINCSTLevelUp.OnInit = function(self)
  ...
  ;
  (UIUtil.AddButtonListener)((self.ui).btn_Upgrade, self, self._OnClickConfirm)
  ;
...
UINCSTLevelUp._CalAndRefreshExpBar = function(self)
  ...
  local newLevel, addLevel, exp, expLimit = (self.treeData):TryAddExp(addExp)
  -- DECOMPILER ERROR at PC85: Confused about usage of register: R9 in 'UnsetPending'

  if not self._waitLvUpTween then
    ((self.ui).img_Bar).fillAmount = exp / expLimit
    self:_UpdateLevel(newLevel)
  end
  ;
  (((self.ui).tex_Exp).gameObject):SetActive(true)
  ;
  ((self.ui).tex_Exp):SetIndex(0, tostring(exp), tostring(expLimit))
  self:_RefreshConfirmBtnState(addExp > 0)
...
UINCSTLevelUp._OnClickConfirm = function(self)
  ...
  self.oldLevel = (self.treeData).level
  AudioManager:PlayAudioById(1103)
  local newLevel, addLevel, exp, expLimit = (self.treeData):TryAddExp(0)
  local cstDataDiff = {oldRate = exp / expLimit, oldLv = (self.treeData).level}
  self._waitLvUpTween = true
  ;
  (self.cmdSkillCtrl):ReqCmdSkillUp((self.treeData).treeId, itemTab, cstDataDiff)
end

UINCSTLevelUp.OnCSTLevelUpComplete = function(self, cstDataDiff)
  -- function num : 0_15
  self.curExp = (self.treeData).curExp
  self:_Clear()
  self:_RefreshIsFullLvUI()
  local skillDataList = self:_LevelSkillData(self.oldLevel, (self.treeData).level)
  if #skillDataList ~= 0 then
    self:_PopUnlockSkillMessageTip(skillDataList)
    ;
    ((self.cstMainRoot).skillNode):ShowCSTSkillUnlockBlueDot(skillDataList)
  end
  self:_ShowLvUpTween(cstDataDiff)
end

UINCSTLevelUp._ShowLvUpTween = function(self, cstDataDiff)
  -- function num : 0_16 , upvalues : UINTweenUtil, _ENV
  self:_KillLvUpSeq(false)
  local newLevel, addLevel, exp, expLimit = (self.treeData):TryAddExp(0)
  local expDiffData = {oldLevel = cstDataDiff.oldLv, oldRate = cstDataDiff.oldRate, level = (self.treeData).level, rate = exp / expLimit}
  self.lvUpSeq = (UINTweenUtil.CreateExpBarSequence)((self.ui).img_Bar, expDiffData, nil, function()
    -- function num : 0_16_0 , upvalues : self
    self._waitLvUpTween = false
    self:_CalAndRefreshExpBar()
  end
, function(tempLevel)
    -- function num : 0_16_1 , upvalues : self, _ENV
    local isMaxLevel = #self.levelCfg <= tempLevel
    if isMaxLevel then
      (((self.ui).tex_NextLevel).gameObject):SetActive(not isMaxLevel)
      ;
      ((self.ui).obj_Arrow):SetActive(not isMaxLevel)
    ...
    if tempLevel > 9 then
      ((self.ui).tex_CurLevel):SetIndex(1, tostring(tempLevel))
    else
      ((self.ui).tex_CurLevel):SetIndex(0, tostring(tempLevel))
    end
    -- DECOMPILER ERROR: 4 unprocessed JMP targets
  end
)
...
UINCSTLevelUp._KillLvUpSeq = function(self, needComplete)
  -- function num : 0_17
