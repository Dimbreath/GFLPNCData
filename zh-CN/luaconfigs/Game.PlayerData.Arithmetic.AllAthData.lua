...
AllAthData.GetHeroCanLoadedSlot = function(self, heroData, slotId, ignoreSpace)
  ...
  do
    if not ignoreSpace then
      local ok = nil
      ok = self:GetHeroAthFreeSpace(heroData, slotId)
      if not ok then
    ...
    for k,athData in pairs(athDic) do
      ...
      if suitId ~= 0 and conflictId ~= 0 then
        if not installedAthSuitDic[suitId] then
          do
            installedAthSuitDic[suitId] = {}
    ...
    for k,data in pairs(slotList) do
      ...
      if (ignoreSpace or data.size <= freeSpace) and installedAthIdDic[data.id] == nil and (installedAthSuitDic[suitId] == nil or (installedAthSuitDic[suitId])[suitCfltId] == nil) then
        ATHCount = 1
        break
      end
    end
...
AllAthData.GetAthNumById = function(self, cfgId)
...
AllAthData.GetAllAthIdNumDic = function(self)
...
AllAthData.GetSuitAthList = function(self, suitId, suitPartId)
  -- function num : 0_31 , upvalues : _ENV
  local list = {}
  for uid,athData in pairs(self.athDic) do
    if athData:GetAthSuit() == suitId and (suitPartId == nil or athData:GetAthSuitConflictId() == suitPartId) then
...
AllAthData.IsAthSuitIdConflict = function(self, suitIdA, conflictIdA, suitIdB, conflictIdB)
  -- function num : 0_32
  do return suitIdA ~= 0 and suitIdA == suitIdB and conflictIdA ~= 0 and conflictIdA == conflictIdB end
  -- DECOMPILER ERROR: 1 unprocessed JMP targets
end

AllAthData.IsAthSuitConflict = function(self, athDataA, athDataB)
  -- function num : 0_33
...
AllAthData.SetAthItemAttrShow = function(self, show)
  -- function num : 0_34 , upvalues : _ENV
  self._athItemAttrShow = show
  MsgCenter:Broadcast(eMsgEventId.OnAthItemDetailShow, show)
end

AllAthData.IsAthItemAttrShow = function(self)
  -- function num : 0_35
