...
HeroSkillData.GetLevelDescribe = function(self, level, unlockAdvance)
  -- function num : 0_14 , upvalues : _ENV
  if self.type ~= eHeroSkillType.LifeSkill then
    if unlockAdvance == nil then
      unlockAdvance = self:IsSkillUnlockAdvance()
    end
    return (self.battleSkillCfg):GetLevelDescribe(level, unlockAdvance)
  else
...
HeroSkillData.GetCurLevelDescribe = function(self, unlockAdvance)
  ...
  if self.type ~= eHeroSkillType.LifeSkill then
    ...
    return self:GetLevelDescribe(curLevel, unlockAdvance)
  else
...
HeroSkillData.GetColorLevelDescribe = function(self, level, color)
  ...
  if self.type ~= eHeroSkillType.LifeSkill then
    return (self.battleSkillCfg):GetClolorfulLevelDescribe(level, color, self:IsSkillUnlockAdvance())
  else
    return (LanguageUtil.GetLocaleText)(((self.lifeSkillCfg)[self.level]).describe)
  end
end
