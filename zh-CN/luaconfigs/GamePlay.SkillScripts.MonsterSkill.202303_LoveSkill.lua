...
, skill_time = 80, start_time = 24, skill_range = 2, select_enemy_id = 24, select_hero_id = 42, skill_interval = 6, hurt_delay = 5, heal_delay = 3, over_time = 48}
...
bs_202303.Onhurt = function(self)
  ...
  if target_hurt.Count > 0 then
    ...
    LuaSkillCtrl:PlayAuHit(self, (target_hurt[0]).targetRole)
...
bs_202303.Onheal = function(self)
  ...
  do
    if target_heal.Count > 0 then
      ...
      _healtarget = _target
      if _target.eBattleRoleType == 4 and (target_heal[1]).targetRole ~= nil then
