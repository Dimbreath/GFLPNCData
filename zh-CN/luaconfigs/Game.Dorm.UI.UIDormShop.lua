...
UIDormShop.InitDormShop = function(self, itemType, shopData)
  ...
  ;
  (self.sortListNode):SetItemClickAction(BindCallback(self, self.OnDormShopItemClicked))
  local data = nil
...
UIDormShop.OnDormShopItemClicked = function(self, item)
  ...
  if item.isSellOut then
    return 
  end
  if (item.itemCfg).type == eItemType.DormRoom then
    UIManager:ShowWindowAsync(UIWindowTypeID.DormRoomDetail, function(window)
    ...
    if (item.itemCfg).type == eItemType.DormFurniture then
      UIManager:ShowWindowAsync(UIWindowTypeID.DormFntDetail, function(window)
