...
SectorStageData.GetSelectDifficult = function(self)
  -- function num : 0_5
  return self.__lastSelectDiff or 1
end
...
SectorStageData.IsSectorClear = function(self, sectorId)
  -- function num : 0_11 , upvalues : _ENV, eDifficulty
  local sectorCfg = (ConfigData.sector)[sectorId]
  if sectorCfg == nil then
    return false
  end
    ...
    do
      ...
      do
        ...
        if difficult == eDifficulty.nightmare then
          local stageCfg = nil
          for sectorId,_ in ipairs((ConfigData.sector).id_sort_list) do
            if self:IsSectorUnlock(R10_PC65) then
              ...
              R10_PC65 = R10_PC65(self, endStageId)
              if not R10_PC65 then
                R10_PC65 = ipairs
                R10_PC65 = R10_PC65(nightmareStageList)
                for _,stageId in R10_PC65 do
                  if self:IsStageUnlock(stageId) and not self:IsStageComplete(stageId) then
