...
bs_10206.InitSkill = function(self, isMidwaySkill)
  ...
  self:AddSetHurtTrigger("bs_10206_2", 1, self.OnSetHurt, self.caster)
...
bs_10206.OnAfterPlaySkill = function(self, skill, role)
  ...
  if skill.maker == self.caster and not skill.isCommonAttack then
    ...
    if self.effect == nil then
      self.effect = LuaSkillCtrl:CallEffect(self.caster, (self.config).nEffectId, self)
    end
  end
end
...
bs_10206.OnArriveAction = function(self, role)
  ...
  LuaSkillCtrl:CallEffect(self.caster, (self.config).nEffectId2, self)
  self.flag = false
end
