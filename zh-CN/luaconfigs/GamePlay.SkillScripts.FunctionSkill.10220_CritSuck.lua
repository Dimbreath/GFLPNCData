...
bs_10220.OnAfterHurt = function(self, sender, target, skill, hurt, isMiss, isCrit, isRealDmg, isTriggerSet)
  ...
  if target == self.caster and not isMiss and hurt > 0 then
    ...
    if (self.arglist)[1] <= self.triggerNum then
      LuaSkillCtrl:CallBuff(self, self.caster, (self.config).buffId_shixue, 1, nil, true)
      self:PlayChipEffect()
      self:OnSkillTake()
      self.triggerNum = 0
    end
