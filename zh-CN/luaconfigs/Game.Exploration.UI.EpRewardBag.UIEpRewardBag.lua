...
UIEpRewardBag.OnInit = function(self)
  ...
  self._OnSettleCompleteFunc = BindCallback(self, self._OnSettleComplete)
end
...
UIEpRewardBag.InitEpRewardBag = function(self, rewardList, stageCfg, inEp, firstClearRewardDic, canGetReward, epModuleId, stageId)
  ...
  do
    ...
    self:_SetModifier(inEp)
    if not inEp then
      (UIUtil.SetTopStatus)(nil, nil, nil, nil, nil, nil, true)
    end
  end
end
...
UIEpRewardBag._OnClickGetReward = function(self)
  ...
  if self.selectedAll then
    pickInfo.pickState = proto_csmsg_stmStorePickInfoPickState.stmStorePickInfoPickStatePickAll
    for k,rewardData in ipairs(self.rewardDataList) do
    ...
    do
      pickInfo.pickState = proto_csmsg_stmStorePickInfoPickState.stmStorePickInfoPickStatePickPartial
      do
        local choiceDic = {}
        for k,rewardData in ipairs(self.rewardDataList) do
          if rewardData.selected then
        ...
        pickInfo.choices = choiceDic
        if self.inEp then
          ExplorationManager:SendSettle(self._OnSettleCompleteFunc, true, pickInfo, true)
        else
          ExplorationManager:SendLastSettle(pickInfo, self._OnSettleCompleteFunc)
        end
      end
    end
  end
...
UIEpRewardBag._OnSettleComplete = function(self, objList)
  ...
  local msg = objList[0]
  do
    for id,num in pairs(msg.firstClearRewards) do
      -- DECOMPILER ERROR at PC28: Confused about usage of register: R8 in 'UnsetPending'

      (self.rewardDic)[id] = (self.rewardDic)[id] + num
    end
  end
  if self.inEp then
    self:_OnClickClose()
    ...
    -- function num : 0_6_0 , upvalues : rewardIds, rewardNums, self
    if window == nil then
      return 
    end
...
UIEpRewardBag._OnClickGiveup = function(self)
  ...
  local exitFunc = function()
    ...
    local pickInfo = {}
    pickInfo.pickState = proto_csmsg_stmStorePickInfoPickState.stmStorePickInfoPickStateAbandon
    self.rewardDic = (table.GetDefaulValueTable)(0)
    if self.inEp then
      ExplorationManager:SendSettle(self._OnSettleCompleteFunc, true, pickInfo, false)
    else
      ExplorationManager:GiveUpLastExploration(pickInfo)
      self:_OnClickClose()
    end
  end
