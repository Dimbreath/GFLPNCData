...
UINDifficultList.InitDifficeltLevel = function(self, sectorId, autoStateCfg, isUnCompleteEp, levelItemClickEvent, levelAvgMainClickEvent, lAvgSubClickEvent, clickBackFunc)
  ...
  do
    ...
    local lastSelectDiff = (PlayerDataCenter.sectorStage):GetSelectDifficult()
...
UINDifficultList._OnMainLevelStateChange = function(self, sectorId, difficulty, stageId)
  ...
  if sectorId == self.sectorId and difficulty == (PlayerDataCenter.sectorStage):GetSelectDifficult() then
...
UINDifficultList.ChangeLevelDifficulty = function(self, difficulty)
  ...
  local lastSelectDiff = (PlayerDataCenter.sectorStage):GetSelectDifficult()
  if lastSelectDiff == difficulty then
    return 
  end
  ...
  ;
  (self.newDiffLevelCanvas):InitDiffLevelCanvas(self.sectorId, self.lastEpStateCfg, true, difficulty, self.levelItemClickEvent, self.levelAvgMainClickEvent, self.lAvgSubClickEvent, self.__onChangeDiffComplete, self.resLoader, self.__sectorLevelTipsGuides, self.clickBackFunc)
  if isUpTween then
